const fs = require('fs');
const dest = 'D:\\II. Automation\\CRM_AUTO\\tests\\demo_test\\Deal_Element\\tc-CMR-Edit-DealElement-2-product-lines.spec.ts';
 ...
 *
 * Command to run:
 * npx playwright test --grep "CMR-Edit-DealElement-2-product-lines:" --project=chromium
 *
 ...
 * 3. Select the product line named "[A2144B]" to be edited
 * 4. Change the product to "[A2146C]"
...
test.describe('CMR-Edit-DealElement-2-product-lines - Verify the Opp has Reseller and Distributor', () => {
  ...
  test('CMR-Edit-DealElement-2-product-lines: Verify the Opp has Reseller and Distributor', async ({ page }, testInfo) => {
    ...
    await test.step('Step II.3-4: Select [A2144B] row and change product to [A2146C]', async () => {
      console.log('Step II.3-4: Changing product [A2144B] \\u2192 [A2146C]');
      await dealElementPage.changeProductInRow('[A2144B]', '[A2146C]');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'II.3-4 - Product changed to [A2146C]');
    });
    ...
    await test.step('Final Summary', async () => {
      ...
      console.log(\`   URL_Opp#1      : \${url_Opp1}\`);
      console.log(\`   Name_EndUser#1 : "\${name_EndUser1}"\`);
      console.log('   II.3-4: [A2144B] replaced by [A2146C]');
      console.log('   III.1 : [A2144B] not in Order Lines \\u2014 confirmed');
      console.log('   III.2 : [A2146C] in Order Lines \\u2014 confirmed');
      console.log('==================================================\\n');
    });
  });
});
