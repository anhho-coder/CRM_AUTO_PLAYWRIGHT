...
const dir = path.join(
  'D:\\II. Automation\\CRM_AUTO\\tests\\1.Project_CRM\\9.CRM_Module',
  'CRM_2338_Opportunity-Expected-Revenue-Calculation',
  '1.Deal_Elements-tab-to-Opportunities',
  '1.2.Deal-items-on-Deal_Elements-tab',
  '1.2.2.Edit-deal-item'
);

const filePath = path.join(dir, 'tc-crm-2338-1-2-2-1-Verify-user-is-able-to-edit-Product-name.spec.ts');
 ...
 * Test Case ID: CRM-2338_1.2.2.1
 *
 * Summary: Verify that the user is able to edit Product name
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\\.2\\.2\\.1:" --project=chromium
 *
 ...
 *
 * VIII. Verification:
 * 1. The product line named [A2144B] does NOT display
 * 2. The product line named [A2146C] displays
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.2.2.1 - Verify that the user is able to edit Product name', () => {
  ...
  test('CRM-2338_1.2.2.1: Verify that the user is able to edit Product name', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(\`VIII.1: Verify \${product_line1} is NOT in Order Lines\`, async () => {
      console.log(\`\\n=== VIII. VERIFICATION POINTS ===\`);
      console.log(\`Step VIII.1: Verifying \${product_line1} is no longer in order lines\`);
      const hasOldProduct = await dealElementPage.isProductInOrderLines(product_line1);
      expect(hasOldProduct, \`VIII.1: Product \${product_line1} should NOT be in Order Lines after edit\`).toBe(false);
      console.log(\`✓ VIII.1: \${product_line1} confirmed REMOVED from Order Lines\`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, \`VIII.1 - \${product_line1} removed\`);
    });

    await test.step(\`VIII.2: Verify \${product_edit1} is present in Order Lines\`, async () => {
      console.log(\`Step VIII.2: Verifying \${product_edit1} is present in order lines\`);
      const hasNewProduct = await dealElementPage.isProductInOrderLines(product_edit1);
      expect(hasNewProduct, \`VIII.2: Product \${product_edit1} should be present in Order Lines\`).toBe(true);
      console.log(\`✓ VIII.2: \${product_edit1} confirmed PRESENT in Order Lines\`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, \`VIII.2 - \${product_edit1} present\`);
    });

    await test.step('Final Summary', async () => {
      console.log('\\n✅ TEST PASSED: CRM-2338_1.2.2.1 verification completed successfully');
      console.log(\`   EndUser#1              : "\${name_EndUser1}"\`);
      console.log(\`   URL_Opp#1              : \${url_Opp1}\`);
      console.log(\`   VIII.1: \${product_line1} removed from Order Lines — confirmed\`);
      console.log(\`   VIII.2: \${product_edit1} present in Order Lines — confirmed\`);
