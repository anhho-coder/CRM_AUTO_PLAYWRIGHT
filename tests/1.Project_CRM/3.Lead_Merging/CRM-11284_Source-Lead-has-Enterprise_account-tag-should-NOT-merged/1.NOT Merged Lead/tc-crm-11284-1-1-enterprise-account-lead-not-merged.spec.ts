...
/**
 * ============================================================================
 *  CRM-11284_1.1 - Enterprise-account lead is protected from auto-merge
 * ============================================================================
 *  Test Case ID    : CRM-11284_1.1
 *  Jira            : CRM-11284  (Post-EA - Support Ticket, Resolved 2026-07-14)
 *  Automation-Type : new
 *  Automation-Date : 2026-07-14
 *
 *  Summary: Verify the CRM-11284 fix - two duplicate leads on the Enterprise-account
 *  domain (@naval-group.com) are auto-tagged "Enterprise account" and are therefore
 *  NOT auto-merged. Both leads stay Active and both carry the "Enterprise account" tag.
 *
 *  Command to run:
 *    npx playwright test --grep "CRM-11284_1\.1:" --project=chromium-headless
 *
 *  Source manual TC (from Khang Nguyen's conclusion comment, 2026-07-14):
 *    Both leads are made merge-eligible with the team-standard recipe (see
 *    config/LEAD_MERGING_TEST_RULES.md) so the ONLY variable is the email domain:
 ...
 *    1. Create the older lead (Lead #1) on the Enterprise-account domain.
 *    2. Create a new lead (Lead #2) with an email on the same Enterprise-account domain.
 *    3. Let the automatic processing run.
 *    Expected:
 *       - Both leads stay (no merge): both Active = TRUE, no merge log note on either.
 *       - Both carry the "Enterprise account" tag (auto-applied by domain).
 * ============================================================================
 */

test.describe('CRM-11284_1.1 - Enterprise account lead is protected from auto-merge', () => {
  ...
  test.beforeEach(async ({ page, context }) => {
    createdLeadUrls = [];
    // Fresh session + deny geolocation ("Know your location" popup)
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    // Tear down the leads this test created (best-effort, never fails the run).
  ...
  test('CRM-11284_1.1: Enterprise-account lead is protected - leads on an Enterprise domain do NOT auto-merge and both carry the "Enterprise account" tag', async ({ page }, testInfo) => {
    ...
    const tcId = 'CRM-11284_1.1';
    // ONE unique Enterprise-domain email, shared by both leads so they are a duplicate pair.
    ...
    // Gathered actuals (populated in the "open lead" steps, asserted in Verification).
    let l1Active = true, l2Active = true;
    let l1MergeMsg = false, l2MergeMsg = false;
    let l1Tags = '', l2Tags = '';

    // PRE-CONDITION I: Login, open CRM > Leads, create Lead #1 (older, Enterprise domain)
    await test.step('Pre-condition I: Login as Odoo Administrator, open CRM > Leads, and create Lead #1 (older) on the Enterprise-account domain', async () => {
      console.log(`\n=== ${tcId} : PRE-CONDITION I - LEAD #1 (older, Enterprise domain) ===`);
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Pre-condition I - Lead #1 created (Enterprise domain)');
    });

    // PRE-CONDITION II: create Lead #2 (newer, same Enterprise domain email)
    await test.step('Pre-condition II: Create a new lead (Lead #2) with an email on the same Enterprise-account domain', async () => {
      console.log(`\n=== ${tcId} : PRE-CONDITION II - LEAD #2 (newer, same Enterprise email) ===`);
      const created = await createMergeEligibleLead(leadPage, page, {
        leadName: lead2Name,
        email: sharedEmail,
    ...
    // STEP 2: open Lead #1 and read its final state
    await test.step('Step 2: Open Lead #1 and read its final state (tag, Active, merge log)', async () => {
    ...
    // STEP 3: open Lead #2 and read its final state
    ...
    // STEP 4 (Verification): both leads stay (no merge) + both carry the Enterprise account tag
    await test.step('Step 4: Verify both leads stay (no merge) and both carry the "Enterprise account" tag', async () => {
      ...
      console.log(`     Actual   : "${l1Tags}"`);
      console.log(`     Result   : ${l1HasEnt ? 'PASS' : 'FAIL'}`);
      console.log(`  Verify #6 - Lead #2 carries the "${ENTERPRISE_TAG}" tag:`);
      console.log(`     Expected : contains "${ENTERPRISE_TAG}"`);
      console.log(`     Actual   : "${l2Tags}"`);
      console.log(`     Result   : ${l2HasEnt ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
      console.log(`OVERALL: ${overallPass ? 'PASS' : 'FAIL'} - Enterprise-domain leads ${overallPass ? 'were protected from auto-merge and both tagged "' + ENTERPRISE_TAG + '"' : 'did NOT behave as expected'}`);
      console.log('===============================================\n');
