 ...
 * Test Case ID: CRM-1992_1.2.1.1.8
 *
 ...
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1992_1\.2\.1\.1\.8 -" --project=chromium
...
test.describe('CRM-1992_1.2.1.1.8 - Lead Merging: Same Public Email with Deal Registration (Trial from Free version)', () => {
  ...
  test.skip('CRM-1992_1.2.1.1.8 [CRM-8929] [CRM-10787]: Verify merging leads with same public email (Created Manually = TRUE, Deal Registration, Trial from Free version)', async ({ page }, testInfo) => {
    ...
    await test.step('Condition #1: Create Lead #1 (Created Manually = TRUE, Trial from Free version)', async () => {
      console.log('=== CONDITION #1: CREATING LEAD #1 (Created Manually = TRUE, Trial from Free version) ===');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED');
      console.log(`   Lead #1: Trial from Free version, Tags: Can_Merge, Trial download`);
