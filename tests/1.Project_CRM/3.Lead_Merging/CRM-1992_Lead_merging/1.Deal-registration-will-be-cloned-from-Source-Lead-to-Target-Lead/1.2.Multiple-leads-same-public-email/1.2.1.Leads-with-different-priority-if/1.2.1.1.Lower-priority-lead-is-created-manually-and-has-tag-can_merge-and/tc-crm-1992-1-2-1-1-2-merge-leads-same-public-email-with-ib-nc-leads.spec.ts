 ...
 * Test Case ID: CRM-1992_1.2.1.1.2
 *
 ...
 * and the higher priority lead has Lead form = IB NC Leads
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1992_1\.2\.1\.1\.2 -" --project=chromium
 ...
 *    3.1. Tag field contains 2 values: "Can_Merge", "Renewal"
...
test.describe('CRM-1992_1.2.1.1.2 - Lead Merging: Same Public Email with Deal Registration (IB NC Leads)', () => {
  ...
  test.skip('CRM-1992_1.2.1.1.2 [CRM-8929] [CRM-10787]: Verify merging leads with same public email (Created Manually = TRUE, Deal Registration, IB NC Leads)', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = TRUE, IB NC Leads, Deal Registration)
    await test.step('Condition #1: Create Lead #1 with Deal Registration (Created Manually = TRUE, IB NC Leads)', async () => {
      console.log('=== CONDITION #1: CREATING LEAD #1 (Created Manually = TRUE, IB NC Leads) ===');
      
      // Generate shared public email with @gmail.com
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Deal Registration, Tag = Can_Merge)
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      // Step 3.1: Verify Tags contain "Can_Merge", "Renewal"
      await test.step('Step 3.1: Tag field contains "Can_Merge", "Renewal"', async () => {
        const tagsText = await leadPage.getTagsText();
        
        expect(tagsText).toContain('Can_Merge');
        expect(tagsText).toContain('Renewal');
        console.log(`  ✓ Step 3.1: Tags contain "Can_Merge" and "Renewal"`);
      });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging with Deal Registration verified successfully');
      console.log(`   Lead #1 (${lead1Id}): Active=TRUE, Is Won=Pending, Tags: Can_Merge, Renewal`);
      console.log(`   - Created Manually: TRUE`);
      console.log(`   - Lead Form: IB NC Leads`);
      ...
      console.log(`   Public Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">\u2705</span>LEAD MERGING TEST - PASSED (Deal Registration - IB NC Leads - Public Email)</h2>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Tags:</span> Can_Merge, Renewal</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Lead merging with Deal Registration verified successfully<br>
        Both leads had Deal Registration configured<br>
        Lead #2 merged into Lead #1<br>
        Public Email: ${sharedEmail}<br>
        <br>
...
`;
      ...
      await testInfo.attach('\ud83d\udccb Lead Merging (Deal Registration - IB NC Leads - Public Email) - Test Summary', {
