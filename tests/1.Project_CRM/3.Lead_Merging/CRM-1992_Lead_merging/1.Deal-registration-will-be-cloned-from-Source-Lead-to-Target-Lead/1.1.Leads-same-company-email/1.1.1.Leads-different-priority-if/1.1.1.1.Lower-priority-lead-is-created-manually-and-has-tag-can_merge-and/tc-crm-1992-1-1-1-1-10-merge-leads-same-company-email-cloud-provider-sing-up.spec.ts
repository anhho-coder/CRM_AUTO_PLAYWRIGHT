 ...
 * Test Case ID: CRM-1992_1.1.1.1.10
 * 
 ...
 * and the higher priority lead has Lead form = Cloud Provider Sing Up
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1992_1\.1\.1\.1\.10 -" --project=chromium
 ...
 *    - Lead form textbox = Cloud Provider Sing Up
 * 4. Press "SAVE" button
 ...
 *    3.1. Tag field contains 2 values: "Can_Merge", "Partner signup"
 ...
 *    4.1. Lead form textbox = Cloud Provider Sing Up
...
test.describe('CRM-1992_1.1.1.1.10 - Lead Merging: Same Company Email with Deal Registration (Cloud Provider Sing Up)', () => {
  ...
  test.skip('CRM-1992_1.1.1.1.10 [CRM-10787]: Verify merging leads with same company email (Cloud Provider Sing Up, Created Manually = TRUE, Deal Registration)', async ({ page }, testInfo) => {
    ...
    const tcId = 'CRM-1992_1.1.1.1.10';
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = FALSE, Cloud Provider Sing Up)
    await test.step('Condition #1: Create Lead #1 (Created Manually = FALSE, Cloud Provider Sing Up)', async () => {
      console.log('=== CONDITION #1: CREATING LEAD #1 (Created Manually = FALSE, Cloud Provider Sing Up) ===');
      ...
      // Fill Lead Form = Cloud Provider Sing Up
      await leadPage.fillLeadForm('Cloud Provider Sing Up');
      console.log(`  - Lead Form: Cloud Provider Sing Up`);
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      // Step 3.1: Verify Tags contain "Can_Merge", "Partner signup"
      await test.step('Step 3.1: Tag field contains "Can_Merge", "Partner signup"', async () => {
        const tagsText = await leadPage.getTagsText();
        
        expect(tagsText).toContain('Can_Merge');
        expect(tagsText).toContain('Partner signup');
        console.log(`  ✓ Step 3.1: Tags contain "Can_Merge" and "Partner signup"`);
      });
    ...
    await test.step('Step 4: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      await test.step('Step 4.1: Lead form textbox = Cloud Provider Sing Up', async () => {
        const leadForm = await leadPage.getLeadFormValue();
        
        expect(leadForm).toBe('Cloud Provider Sing Up');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging with Deal Registration (Cloud Provider Sing Up) verified successfully');
      console.log(`   Lead #1 (${lead1Id}): Active=TRUE, Is Won=Pending, Tags: Can_Merge, Partner signup`);
      console.log(`   - Created Manually: FALSE`);
      console.log(`   - Lead Form: Cloud Provider Sing Up`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">\u2705</span>LEAD MERGING TEST - PASSED (Cloud Provider Sing Up)</h2>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Tags:</span> Can_Merge, Partner signup</div>
      ...
      <div class="info-row"><span class="label">Lead Form:</span> Cloud Provider Sing Up</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Lead merging with Deal Registration (Cloud Provider Sing Up) verified successfully<br>
        Lead #2 (Deal Registration) merged into Lead #1 (Cloud Provider Sing Up)<br>
...
`;
      ...
      await testInfo.attach('\ud83d\udccb Lead Merging (Cloud Provider Sing Up) - Test Summary', {
