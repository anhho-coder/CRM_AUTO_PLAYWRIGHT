 ...
 * Test Case ID: CRM-1992_1.1.1.1.8
 * 
 ...
 * and the higher priority lead has Lead form = Trial from Free version
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1992_1\.1\.1\.1\.8 -" --project=chromium
 ...
 *    3.1. Tag field contains 2 values: "Can_Merge", "Trial download"
 ...
 *    4.1. Lead form textbox = Trial from Free version
...
test.describe('CRM-1992_1.1.1.1.8 - Lead Merging: Same Company Email with Deal Registration (Trial from Free version)', () => {
  ...
  test.skip('CRM-1992_1.1.1.1.8 [CRM-10787]: Verify merging leads with same company email (Trial from Free version, Created Manually = TRUE, Deal Registration)', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = FALSE, Trial from Free version)
    await test.step('Condition #1: Create Lead #1 (Created Manually = FALSE, Trial from Free version)', async () => {
      console.log('=== CONDITION #1: CREATING LEAD #1 (Created Manually = FALSE, Trial from Free version) ===');
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      // Step 3.1: Verify Tags contain "Can_Merge", "Trial download"
      await test.step('Step 3.1: Tag field contains "Can_Merge", "Trial download"', async () => {
        const tagsText = await leadPage.getTagsText();
        
        expect(tagsText).toContain('Can_Merge');
        expect(tagsText).toContain('Trial download');
        console.log(`  ✓ Step 3.1: Tags contain "Can_Merge" and "Trial download"`);
      });
    ...
    await test.step('Step 4: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      await test.step('Step 4.1: Lead form textbox = Trial from Free version', async () => {
        const leadForm = await leadPage.getLeadFormValue();
        
        expect(leadForm).toBe('Trial from Free version');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging with Deal Registration (Trial from Free version) verified successfully');
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">\u2705</span>LEAD MERGING TEST - PASSED (Trial from Free version)</h2>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Tags:</span> Can_Merge, Trial download</div>
      ...
      <div class="info-row"><span class="label">Lead Form:</span> Trial from Free version</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Lead merging with Deal Registration (Trial from Free version) verified successfully<br>
        Lead #2 (Deal Registration) merged into Lead #1 (Trial from Free version)<br>
...
`;
      ...
      await testInfo.attach('\ud83d\udccb Lead Merging (Trial from Free version) - Test Summary', {
