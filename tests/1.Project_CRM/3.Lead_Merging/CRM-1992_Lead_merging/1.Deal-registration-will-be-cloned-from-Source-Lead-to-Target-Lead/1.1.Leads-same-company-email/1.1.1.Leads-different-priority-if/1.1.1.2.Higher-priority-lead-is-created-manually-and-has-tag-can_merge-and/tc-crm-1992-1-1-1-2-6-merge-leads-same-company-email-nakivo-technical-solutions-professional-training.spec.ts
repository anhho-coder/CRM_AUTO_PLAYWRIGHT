 ...
 * Test Case ID: CRM-1992_1.1.1.2.6
 * 
 ...
 * 3. Click at "CRM Developer" tab at the bottom of page
 *    - Lead form textbox = NAKIVO Technical Solutions Professional Training
 * 4. Press "SAVE" button
 ...
 *    8.1. Lead form textbox = NAKIVO Technical Solutions Professional Training
...
test.describe('CRM-1992_1.1.1.2.6 - Lead Merging: Same Company Email with Deal Registration (NAKIVO Technical Solutions Professional Training)', () => {
  ...
  test('CRM-1992_1.1.1.2.6 [CRM-10787]: Verify merging leads with same company email (Created Manually = TRUE, Tag = Can_Merge, NAKIVO Technical Solutions Professional Training)', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Lead Form = NAKIVO Technical Solutions Professional Training, Deal Registration)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Lead Form = NAKIVO Technical Solutions Professional Training, Deal Registration)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Lead Form = NAKIVO Technical Solutions Professional Training, Deal Registration) ===');
      ...
      // Fill Lead Form = NAKIVO Technical Solutions Professional Training
      await leadPage.fillLeadForm('NAKIVO Technical Solutions Professional Training');
      console.log(`  - Lead Form: NAKIVO Technical Solutions Professional Training`);
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      // Step 3.1: Verify Tags contain "Can_Merge" (1 value only)
      await test.step('Step 3.1: Tag field contains 1 value: "Can_Merge"', async () => {
        ...
        console.log(`  ✓ Step 3.1: Tags contain "Can_Merge" (1 value)`);
      });
    ...
    await test.step('Step 7: Verify the following fields on Lead #2', async () => {
      ...
      // Step 7.1: Verify Tags field contains nothing
      await test.step('Step 7.1: Tag field contains nothing', async () => {
        const tagsText = await leadPage.getTagsText();
        ...
        console.log(`  ✓ Step 7.1: Tags field is empty (no tags)`);
      });
    ...
    await test.step('Step 8: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      // Step 8.1: Verify Lead form = NAKIVO Technical Solutions Professional Training
      await test.step('Step 8.1: Lead form textbox = NAKIVO Technical Solutions Professional Training', async () => {
        const leadForm = await leadPage.getLeadFormValue();
        
        expect(leadForm).toBe('NAKIVO Technical Solutions Professional Training');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging with Deal Registration (NAKIVO Technical Solutions Professional Training) verified successfully');
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">\u2705</span>LEAD MERGING TEST - PASSED (NAKIVO Technical Solutions Professional Training)</h2>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Email:</span> ${sharedEmail}</div>
      <div class="info-row"><span class="label">Tags:</span> (empty)</div>
      ...
      <div class="info-row"><span class="label">Lead Form:</span> NAKIVO Technical Solutions Professional Training</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Lead merging with Deal Registration (NAKIVO Technical Solutions Professional Training) verified successfully<br>
        Lead #2 (NAKIVO Technical Solutions Professional Training) merged into Lead #1 (Tag: Can_Merge)<br>
...
`;
      ...
      await testInfo.attach('\ud83d\udccb Lead Merging (NAKIVO Technical Solutions Professional Training) - Test Summary', {
