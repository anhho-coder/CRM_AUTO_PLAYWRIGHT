 ...
 * Test Case ID: CRM-1992_1.1.1.2.5
 * 
 ...
 * and the higher priority lead has Lead form = Partner Download
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1992_1\.1\.1\.2\.5 -" --project=chromium
 ...
 * 3. Click at "CRM Developer" tab at the bottom of page
 *    - Lead form textbox = Partner Download
 * 4. Press "SAVE" button
 ...
 *    3.1. Tag field contains 2 values: "Can_Merge", "Partner download"
 ...
 *    7.1. Tag field contains "Partner download"
 ...
 *    8.1. Lead form textbox = Partner Download
...
test.describe('CRM-1992_1.1.1.2.5 - Lead Merging: Same Company Email with Deal Registration (Partner Download)', () => {
  ...
  test.skip('CRM-1992_1.1.1.2.5 [CRM-10787]: Verify merging leads with same company email (Created Manually = TRUE, Tag = Can_Merge, Partner Download)', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Lead Form = Partner Download, Deal Registration)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Lead Form = Partner Download, Deal Registration)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Lead Form = Partner Download, Deal Registration) ===');
      ...
      // Fill Lead Form = Partner Download
      await leadPage.fillLeadForm('Partner Download');
      console.log(`  - Lead Form: Partner Download`);
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      // Step 3.1: Verify Tags contain "Can_Merge", "Partner download" (2 values)
      await test.step('Step 3.1: Tag field contains 2 values: "Can_Merge", "Partner download"', async () => {
        const tagsText = await leadPage.getTagsText();
        
        expect(tagsText).toContain('Can_Merge');
        expect(tagsText).toContain('Partner download');
        console.log(`  ✓ Step 3.1: Tags contain "Can_Merge" and "Partner download" (2 values)`);
      });
    ...
    await test.step('Step 7: Verify the following fields on Lead #2', async () => {
      ...
      // Step 7.1: Verify Tags field contains "Partner download"
      await test.step('Step 7.1: Tag field contains "Partner download"', async () => {
        const tagsText = await leadPage.getTagsText();
        
        expect(tagsText).toContain('Partner download');
        console.log(`  ✓ Step 7.1: Tags contain "Partner download"`);
      });
    ...
    await test.step('Step 8: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      // Step 8.1: Verify Lead form = Partner Download
      await test.step('Step 8.1: Lead form textbox = Partner Download', async () => {
        const leadForm = await leadPage.getLeadFormValue();
        
        expect(leadForm).toBe('Partner Download');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging with Deal Registration (Partner Download) verified successfully');
      console.log(`   Lead #1 (${lead1Id}): Active=TRUE, Is Won=Pending, Tags: Can_Merge, Partner download (2 values)`);
      console.log(`   - Created Manually: TRUE`);
      console.log(`   - Lead Form: BLANK`);
      console.log(`   - Deal Registration: TRUE (cloned from Lead#2)`);
      console.log(`   Lead #2 (${lead2Id}): Active=FALSE, Is Won=Lost, Lost Reason=Duplicate, Tags: Partner download`);
      console.log(`   - Created Manually: FALSE`);
      console.log(`   - Lead Form: Partner Download`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">\u2705</span>LEAD MERGING TEST - PASSED (Partner Download)</h2>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Tags:</span> Can_Merge, Partner download (2 values)</div>
      <div class="info-row"><span class="label">Company Name:</span> Company Name Lead 1</div>
      <div class="info-row"><span class="label">Location:</span> Belgium, Brussels Capital</div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Email:</span> ${sharedEmail}</div>
      <div class="info-row"><span class="label">Tags:</span> Partner download</div>
      ...
      <div class="info-row"><span class="label">Lead Form:</span> Partner Download</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Lead merging with Deal Registration (Partner Download) verified successfully<br>
        Lead #2 (Partner Download) merged into Lead #1 (Tag: Can_Merge)<br>
...
`;
      ...
      await testInfo.attach('\ud83d\udccb Lead Merging (Partner Download) - Test Summary', {
