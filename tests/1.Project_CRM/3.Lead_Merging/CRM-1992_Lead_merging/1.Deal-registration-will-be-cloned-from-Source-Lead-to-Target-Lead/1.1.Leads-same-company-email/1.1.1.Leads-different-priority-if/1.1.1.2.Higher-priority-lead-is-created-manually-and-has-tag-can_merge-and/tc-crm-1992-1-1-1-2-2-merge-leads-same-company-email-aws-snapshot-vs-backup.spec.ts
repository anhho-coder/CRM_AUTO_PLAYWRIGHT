 ...
 * Test Case ID: CRM-1992_1.1.1.2.2
 * 
 ...
 * and the higher priority lead has Lead form = AWS Snapshot vs. Backup
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1992_1\.1\.1\.2\.2 -" --project=chromium
 ...
 * 3. Click at "CRM Developer" tab at the bottom of page
 *    - Lead form textbox = AWS Snapshot vs. Backup
 * 4. Press "SAVE" button
 ...
 *    8.1. Lead form textbox = AWS Snapshot vs. Backup
...
test.describe('CRM-1992_1.1.1.2.2 - Lead Merging: Same Company Email with Deal Registration (AWS Snapshot vs. Backup)', () => {
  ...
  test.skip('CRM-1992_1.1.1.2.2 [CRM-10787]: Verify merging leads with same company email (Created Manually = TRUE, Tag = Can_Merge, AWS Snapshot vs. Backup)', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Lead Form = AWS Snapshot vs. Backup, Deal Registration)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Lead Form = AWS Snapshot vs. Backup, Deal Registration)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Lead Form = AWS Snapshot vs. Backup, Deal Registration) ===');
      ...
      // Fill Lead Form = AWS Snapshot vs. Backup
      await leadPage.fillLeadForm('AWS Snapshot vs. Backup');
      console.log(`  - Lead Form: AWS Snapshot vs. Backup`);
    ...
    await test.step('Step 8: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      // Step 8.1: Verify Lead form = AWS Snapshot vs. Backup
      await test.step('Step 8.1: Lead form textbox = AWS Snapshot vs. Backup', async () => {
        const leadForm = await leadPage.getLeadFormValue();
        
        expect(leadForm).toBe('AWS Snapshot vs. Backup');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging with Deal Registration (AWS Snapshot vs. Backup) verified successfully');
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">\u2705</span>LEAD MERGING TEST - PASSED (AWS Snapshot vs. Backup)</h2>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Lead Form:</span> AWS Snapshot vs. Backup</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Lead merging with Deal Registration (AWS Snapshot vs. Backup) verified successfully<br>
        Lead #2 (AWS Snapshot vs. Backup) merged into Lead #1 (Tag: Can_Merge)<br>
...
`;
      ...
      await testInfo.attach('\ud83d\udccb Lead Merging (AWS Snapshot vs. Backup) - Test Summary', {
