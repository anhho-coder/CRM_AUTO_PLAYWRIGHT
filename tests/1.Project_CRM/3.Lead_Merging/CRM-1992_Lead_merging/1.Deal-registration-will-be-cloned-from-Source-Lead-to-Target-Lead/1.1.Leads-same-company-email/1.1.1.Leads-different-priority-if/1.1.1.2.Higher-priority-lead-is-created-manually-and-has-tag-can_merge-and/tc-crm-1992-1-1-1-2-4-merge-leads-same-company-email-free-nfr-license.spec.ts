 ...
 * Test Case ID: CRM-1992_1.1.1.2.4
 * 
 ...
 * and the higher priority lead has Lead form = Free NFR License
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1992_1\.1\.1\.2\.4 -" --project=chromium
 ...
 * 3. Click at "CRM Developer" tab at the bottom of page
 *    - Lead form textbox = Free NFR License
 * 4. Press "SAVE" button
 ...
 *    3.1. Tag field contains 2 values: "Can_Merge", "Free download"
 ...
 *    7.1. Tag field contains "Free download"
 ...
 *    8.1. Lead form textbox = Free NFR License
...
test.describe('CRM-1992_1.1.1.2.4 - Lead Merging: Same Company Email with Deal Registration (Free NFR License)', () => {
  ...
  test.skip('CRM-1992_1.1.1.2.4 [CRM-10787]: Verify merging leads with same company email (Created Manually = TRUE, Tag = Can_Merge, Free NFR License)', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Lead Form = Free NFR License, Deal Registration)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Lead Form = Free NFR License, Deal Registration)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Lead Form = Free NFR License, Deal Registration) ===');
      ...
      // Fill Lead Form = Free NFR License
      await leadPage.fillLeadForm('Free NFR License');
      console.log(`  - Lead Form: Free NFR License`);
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      // Step 3.1: Verify Tags contain "Can_Merge", "Free download" (2 values)
      await test.step('Step 3.1: Tag field contains 2 values: "Can_Merge", "Free download"', async () => {
        const tagsText = await leadPage.getTagsText();
        
        expect(tagsText).toContain('Can_Merge');
        expect(tagsText).toContain('Free download');
        console.log(`  ✓ Step 3.1: Tags contain "Can_Merge" and "Free download" (2 values)`);
      });
    ...
    await test.step('Step 7: Verify the following fields on Lead #2', async () => {
      ...
      // Step 7.1: Verify Tags field contains "Free download"
      await test.step('Step 7.1: Tag field contains "Free download"', async () => {
        const tagsText = await leadPage.getTagsText();
        
        expect(tagsText).toContain('Free download');
        console.log(`  ✓ Step 7.1: Tags contain "Free download"`);
      });
    ...
    await test.step('Step 8: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      // Step 8.1: Verify Lead form = Free NFR License
      await test.step('Step 8.1: Lead form textbox = Free NFR License', async () => {
        const leadForm = await leadPage.getLeadFormValue();
        
        expect(leadForm).toBe('Free NFR License');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging with Deal Registration (Free NFR License) verified successfully');
      console.log(`   Lead #1 (${lead1Id}): Active=TRUE, Is Won=Pending, Tags: Can_Merge, Free download (2 values)`);
      console.log(`   - Created Manually: TRUE`);
      console.log(`   - Lead Form: BLANK`);
      console.log(`   - Deal Registration: TRUE (cloned from Lead#2)`);
      console.log(`   Lead #2 (${lead2Id}): Active=FALSE, Is Won=Lost, Lost Reason=Duplicate, Tags: Free download`);
      console.log(`   - Created Manually: FALSE`);
      console.log(`   - Lead Form: Free NFR License`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">\u2705</span>LEAD MERGING TEST - PASSED (Free NFR License)</h2>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Tags:</span> Can_Merge, Free download (2 values)</div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Email:</span> ${sharedEmail}</div>
      <div class="info-row"><span class="label">Tags:</span> Free download</div>
      ...
      <div class="info-row"><span class="label">Lead Form:</span> Free NFR License</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Lead merging with Deal Registration (Free NFR License) verified successfully<br>
        Lead #2 (Free NFR License) merged into Lead #1 (Tag: Can_Merge)<br>
...
`;
      ...
      await testInfo.attach('\ud83d\udccb Lead Merging (Free NFR License) - Test Summary', {
