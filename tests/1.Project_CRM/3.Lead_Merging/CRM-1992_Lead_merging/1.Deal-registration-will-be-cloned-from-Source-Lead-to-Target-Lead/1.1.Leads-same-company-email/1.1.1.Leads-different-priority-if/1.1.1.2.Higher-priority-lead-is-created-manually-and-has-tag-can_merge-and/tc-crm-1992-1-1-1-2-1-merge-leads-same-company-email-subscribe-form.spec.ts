 ...
 * Test Case ID: CRM-1992_1.1.1.2.1
 * 
 ...
 * and the higher priority lead has Lead form = Subscribe Form
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1992_1\.1\.1\.2\.1 -" --project=chromium
...
test.describe('CRM-1992_1.1.1.2.1 - Lead Merging: Same Company Email with Deal Registration (Subscribe Form)', () => {
  ...
  test.skip('CRM-1992_1.1.1.2.1 [CRM-10787]: Verify merging leads with same company email (Created Manually = TRUE, Tag = Can_Merge, Subscribe Form)', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Lead Form = Subscribe Form, Deal Registration)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Lead Form = Subscribe Form, Deal Registration)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Lead Form = Subscribe Form, Deal Registration) ===');
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      // Step 3.1: Verify Tags contain "Can_Merge" only (1 value)
    ...
    await test.step('Step 7: Verify the following fields on Lead #2', async () => {
      ...
      // Step 7.1: Verify Tags field is empty (contains nothing)
      await test.step('Step 7.1: Tag field contains nothing', async () => {
        ...
        console.log(`  ✓ Step 7.1: Tags field is empty (contains nothing)`);
      });
    ...
    await test.step('Step 8: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      // Step 8.1: Verify Lead form = Subscribe Form
      await test.step('Step 8.1: Lead form textbox = Subscribe Form', async () => {
        const leadForm = await leadPage.getLeadFormValue();
        
        expect(leadForm).toBe('Subscribe Form');
...
<body>
  <div class="container">
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Email:</span> ${sharedEmail}</div>
      <div class="info-row"><span class="label">Tags:</span> Empty (no tags)</div>
