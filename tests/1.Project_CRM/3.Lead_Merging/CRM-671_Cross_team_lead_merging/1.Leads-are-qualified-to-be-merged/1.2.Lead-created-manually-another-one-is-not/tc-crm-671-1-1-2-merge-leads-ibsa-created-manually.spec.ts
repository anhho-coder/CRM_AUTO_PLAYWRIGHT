...
/**
 * Lead Merging Test - Different Sales Teams (IBSA with Created Manually)
 * Test Case ID: CRM-671_1.1.2
 * 
 * Summary: Verify that the merging lead happens successfully when the leads from the same 
 * company domain email but different priority if a lead of them is created manually
 * 
 * Command to run:
 * npx playwright test --grep "CRM-671_1.1.2" --project=chromium
 * 
 * Pre-condition:
...
*    3.6. Sales Team dropdown list is any value
...
*    7.6. Sales Team dropdown list is any value
...
test.describe('CRM-671_1.1.2 - Lead Merging: Cleared Sales Teams (Created Manually)', () => {
  ...
  test('Verify merging leads happens with cleared sales teams (Created Manually = TRUE, Lead Form = BLANK)', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = TRUE, Lead Form = BLANK)
    await test.step('Condition #1: Create Lead #1 with Cleared Sales Team (Created Manually = TRUE)', async () => {
      console.log('\n=== CREATING LEAD #1 (Sales Team Cleared - Created Manually = TRUE) ===');
      ...
      // Generate Lead #1 name with datetime
    ...
    // CONDITION #2: Create Lead #2 (Subscribe Form - Sales Team Cleared)
    await test.step('Condition #2: Create Lead #2 with Subscribe Form (Sales Team Cleared)', async () => {
      console.log('=== CREATING LEAD #2 (Sales Team Cleared - Subscribe Form) ===');
      
      // Click CREATE button
      await leadPage.clickCreate();
      console.log('✓ Lead #2 creation form opened');
      
      // Generate Lead #2 name with datetime
    ...
    await test.step('Step 3: Verify the following', async () => {
      ...
      await test.step('Step 3.1: Verify Tag field contains 1 value: "Can_Merge"', async () => {
        ...
        console.log(`  ✓ Step 3.1: Tag field contains 1 value: "Can_Merge"`);
      });
      ...
      // Step 3.6: Sales Team = any value (have or have not value)
      await test.step('Step 3.6: Verify Sales Team = any value (readonly)', async () => {
        const salesTeamValue = await leadPage.getSalesTeamValue();
        // Accept any value including empty
        const displayValue = salesTeamValue.trim() || '(empty)';
        console.log(`  ✓ Step 3.6: Sales Team = ${displayValue} (any value - have or have not value)`);
      });
    ...
    await test.step('Step 7: Verify the following', async () => {
      ...
      // Step 7.6: Sales Team = any value (have or have not value)
      await test.step('Step 7.6: Verify Sales Team = any value (readonly)', async () => {
        const salesTeamValue = await leadPage.getSalesTeamValue();
        // Accept any value including empty
        const displayValue = salesTeamValue.trim() || '(empty)';
        console.log(`  ✓ Step 7.6: Sales Team = ${displayValue} (any value - have or have not value)`);
      });
    ...
    await test.step('Final Summary', async () => {
      console.log('✅ TEST PASSED: Cross-team lead merging with Created Manually verified successfully');
      console.log(`   Lead #1 (${lead1Id}): Sales Team Cleared, Created Manually=TRUE, Lead Form=BLANK, Active=TRUE, Is Won=Pending`);
      console.log(`   Lead #2 (${lead2Id}): Sales Team Cleared, Subscribe Form, Active=FALSE, Is Won=Lost, Lost Reason=Duplicate`);
      console.log(`   Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">✅</span>CROSS-TEAM LEAD MERGING TEST (Created Manually) - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="lead-section">
      <div class="lead-title">📋 Lead #1 - Sales Team Cleared (Target Lead - Created Manually = TRUE)</div>
      <div class="info-row"><span class="label">Lead ID:</span> ${lead1Id}</div>
      <div class="info-row"><span class="label">Sales Team:</span> Cleared</div>
    ...
    <div class="lead-section">
      <div class="lead-title">📋 Lead #2 - Sales Team Cleared (Source Lead)</div>
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #1 Verifications (Sales Team Cleared)</div>
      ...
      <div class="checkpoint-detail"><span class="label">Sales Team:</span> <span class="result-pass">✓ Any value (auto-assigned)</span></div>
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #2 Verifications (Sales Team Cleared)</div>
      <div class="checkpoint-detail"><span class="label">Tags:</span> <span class="result-pass">✓ Empty (no tags)</span></div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead #1 (Sales Team Cleared): Active, Created Manually=TRUE, Lead Form=BLANK<br>
        Lead #2 (Sales Team Cleared): Inactive (Lost - Duplicate)<br>
...
`;
      ...
      await testInfo.attach('📋 Cross-Team Lead Merging (Created Manually) Verification Summary', {
