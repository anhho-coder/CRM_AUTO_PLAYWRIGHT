...
/**
 * Lead Merging Test - NO MERGE: IBSA and CMD Teams
 * Test Case ID: CRM-671_2.1.2
 * 
 * Summary: Verify that the merging leads do NOT happens if having 2 leads with the same email,
 * but a lead assigned to Install Base team, and another lead is assigned to CMD team (except Marketing)
 * 
 * Command to run:
 * npx playwright test --grep "CRM-671_2.1.2" --project=chromium
 * 
 * Pre-condition:
 ...
 *    7.6. Sales Team dropdown list = CMD
...
test.describe('CRM-671_2.1.2 - NO MERGE: IBSA and CMD Teams', () => {
  ...
  test('Verify that leads do NOT merge when IBSA team and CMD team have same email', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (CMD Team)
    await test.step('Condition #2: Create Lead #2 with CMD team', async () => {
      console.log('=== CREATING LEAD #2 (CMD Team) ===');
      ...
      // Set Sales Team to CMD
      await leadPage.selectSalesTeam('CMD');
      console.log(`  - Sales Team: CMD`);
    ...
    await test.step('Step 1: Wait to confirm NO merge happens', async () => {
      ...
      console.log(`IBSA team (Lead #1) and CMD team (Lead #2) should NOT merge despite same email`);
    ...
    await test.step('Step 7: Verify Lead #2 fields (should remain unchanged)', async () => {
      ...
      // Step 7.6: Sales Team = CMD
      await test.step('Step 7.6: Verify Sales Team = CMD (readonly)', async () => {
        const salesTeamValue = await leadPage.getSalesTeamValue();
        expect(salesTeamValue).toContain('CMD');
        console.log(`  ✓ Step 7.6: Sales Team = CMD`);
      });
    ...
    await test.step('Final Summary', async () => {
      console.log('✅ TEST PASSED: NO MERGE occurred between IBSA and CMD teams as expected');
      console.log(`   Lead #1 (${lead1Id}): IBSA team, Active=TRUE, Is Won=Pending`);
      console.log(`   Lead #2 (${lead2Id}): CMD team, Active=TRUE, Is Won=Pending`);
      console.log(`   Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">🚫</span>NO MERGE TEST - IBSA AND CMD TEAMS - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="no-merge">
      <div class="no-merge-title">✓ NO MERGE OCCURRED (As Expected)</div>
      <div>IBSA team and CMD team leads with same email should NOT merge</div>
    </div>
    ...
    <div class="lead-section">
      <div class="lead-title">📋 Lead #2 - CMD Team (Remained Independent)</div>
      <div class="info-row"><span class="label">Lead ID:</span> ${lead2Id}</div>
      <div class="info-row"><span class="label">Sales Team:</span> CMD</div>
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #2 Verifications (CMD Team - No Merge)</div>
      <div class="checkpoint-detail"><span class="label">Tags:</span> <span class="result-pass">✓ BLANK</span></div>
      ...
      <div class="checkpoint-detail"><span class="label">Sales Team:</span> <span class="result-pass">✓ CMD</span></div>
      <div class="checkpoint-detail"><span class="label">CRM Developer:</span> <span class="result-pass">✓ Lead Form=BLANK, Active=TRUE, Is Won=Pending</span></div>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead #2 (CMD): Active, Independent<br>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (IBSA and CMD) Verification Summary', {
