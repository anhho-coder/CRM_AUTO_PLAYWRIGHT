...
/**
 * Lead Merging Test - NO MERGE: IBSA and Website Teams
 * Test Case ID: CRM-671_2.1.8
 * 
 * Summary: Verify that the merging leads do NOT happens if having 2 leads with the same email,
 * but a lead assigned to Install Base team, and another lead is assigned to Website team (except Marketing)
 * 
 * Command to run:
 * npx playwright test --grep "CRM-671_2.1.8" --project=chromium
 * 
 * Pre-condition:
 ...
 *    - Sales Team dropdown list = Website
 *    - Salesperson dropdown list is cleared
 *    - Created manually checkbox = FALSE
 * 3. Click at "CRM Developer" tab at the bottom of page
 *    - Lead form textbox = BLANK
 ...
 *    7.6. Sales Team dropdown list = Website
...
test.describe.skip('CRM-671_2.1.8 - NO MERGE: IBSA and Website Teams - SKIPPED: No Website Sales Team', () => {
  ...
  test('Verify that leads do NOT merge when IBSA team and Website team have same email', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Website Team)
    await test.step('Condition #2: Create Lead #2 with Website team', async () => {
      console.log('=== CREATING LEAD #2 (Website Team) ===');
      ...
      // Set Sales Team to Website
      await leadPage.selectSalesTeam('Website');
      console.log(`  - Sales Team: Website`);
    ...
    await test.step('Step 1: Wait to confirm NO merge happens', async () => {
      ...
      console.log(`IBSA team (Lead #1) and Website team (Lead #2) should NOT merge despite same email`);
    ...
    await test.step('Step 7: Verify Lead #2 fields (should remain unchanged)', async () => {
      ...
      // Step 7.6: Sales Team = Website
      await test.step('Step 7.6: Verify Sales Team = Website (readonly)', async () => {
        const salesTeamValue = await leadPage.getSalesTeamValue();
        expect(salesTeamValue).toContain('Website');
        console.log(`  ✓ Step 7.6: Sales Team = Website`);
      });
    ...
    await test.step('Final Summary', async () => {
      console.log('✅ TEST PASSED: NO MERGE occurred between IBSA and Website teams as expected');
      console.log(`   Lead #1 (${lead1Id}): IBSA team, Active=TRUE, Is Won=Pending`);
      console.log(`   Lead #2 (${lead2Id}): Website team, Active=TRUE, Is Won=Pending`);
      console.log(`   Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">🚫</span>NO MERGE TEST - IBSA AND WEBSITE TEAMS - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="no-merge">
      <div class="no-merge-title">✓ NO MERGE OCCURRED (As Expected)</div>
      <div>IBSA team and Website team leads with same email should NOT merge</div>
    </div>
    ...
    <div class="lead-section">
      <div class="lead-title">📋 Lead #2 - Website Team (Remained Independent)</div>
      <div class="info-row"><span class="label">Lead ID:</span> ${lead2Id}</div>
      <div class="info-row"><span class="label">Sales Team:</span> Website</div>
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #2 Verifications (Website Team - No Merge)</div>
      <div class="checkpoint-detail"><span class="label">Tags:</span> <span class="result-pass">✓ BLANK</span></div>
      ...
      <div class="checkpoint-detail"><span class="label">Sales Team:</span> <span class="result-pass">✓ Website</span></div>
      <div class="checkpoint-detail"><span class="label">CRM Developer:</span> <span class="result-pass">✓ Lead Form=BLANK, Active=TRUE, Is Won=Pending</span></div>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead #2 (Website): Active, Independent<br>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (IBSA and Website) Verification Summary', {
