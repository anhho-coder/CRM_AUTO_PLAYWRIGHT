...
/**
 * Lead Merging Test - NO MERGE: IBSA and BDR Teams
 * Test Case ID: CRM-671_2.1.4
 * 
 * Summary: Verify that the merging leads do NOT happens if having 2 leads with the same email,
 * but a lead assigned to Install Base team, and another lead is assigned to BDR team (except Marketing)
 * 
 * Command to run:
 * npx playwright test --grep "CRM-671_2.1.4" --project=chromium
 * 
 * Pre-condition:
 ...
 *    7.6. Sales Team dropdown list = BDEU (Because assign to BDR will automatically change to BDEU)
...
test.describe('CRM-671_2.1.4 - NO MERGE: IBSA and BDR Teams', () => {
  ...
  test('Verify that leads do NOT merge when IBSA team and BDR team have same email', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (BDR Team)
    await test.step('Condition #2: Create Lead #2 with BDR team', async () => {
      console.log('=== CREATING LEAD #2 (BDR Team) ===');
      ...
      // Set Sales Team to BDR
      await leadPage.selectSalesTeam('BDR');
      console.log(`  - Sales Team: BDR`);
    ...
    await test.step('Step 1: Wait to confirm NO merge happens', async () => {
      ...
      console.log(`IBSA team (Lead #1) and BDR team (Lead #2) should NOT merge despite same email`);
    ...
    await test.step('Step 7: Verify Lead #2 fields (should remain unchanged)', async () => {
      ...
      // Step 7.6: Sales Team = BDEU
      await test.step('Step 7.6: Verify Sales Team = BDEU (readonly)', async () => {
        const salesTeamValue = await leadPage.getSalesTeamValue();
    ...
    await test.step('Final Summary', async () => {
      console.log('✅ TEST PASSED: NO MERGE occurred between IBSA and BDR teams as expected');
      console.log(`   Lead #1 (${lead1Id}): IBSA team, Active=TRUE, Is Won=Pending`);
      console.log(`   Lead #2 (${lead2Id}): BDR team, Active=TRUE, Is Won=Pending`);
      console.log(`   Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">🚫</span>NO MERGE TEST - IBSA AND BDR TEAMS - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="no-merge">
      <div class="no-merge-title">✓ NO MERGE OCCURRED (As Expected)</div>
      <div>IBSA team and BDR team leads with same email should NOT merge</div>
    </div>
    ...
    <div class="lead-section">
      <div class="lead-title">📋 Lead #2 - BDR Team (Remained Independent)</div>
      <div class="info-row"><span class="label">Lead ID:</span> ${lead2Id}</div>
      <div class="info-row"><span class="label">Sales Team:</span> BDR</div>
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #2 Verifications (BDR Team - No Merge)</div>
      <div class="checkpoint-detail"><span class="label">Tags:</span> <span class="result-pass">✓ BLANK</span></div>
      ...
      <div class="checkpoint-detail"><span class="label">Sales Team:</span> <span class="result-pass">✓ BDR</span></div>
      <div class="checkpoint-detail"><span class="label">CRM Developer:</span> <span class="result-pass">✓ Lead Form=BLANK, Active=TRUE, Is Won=Pending</span></div>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead #2 (BDR): Active, Independent<br>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (IBSA and BDR) Verification Summary', {
