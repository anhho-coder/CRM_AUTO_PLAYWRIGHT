 ...
 * 
 * Command to run:
 * npx playwright test --grep "CRM-671_2.1.1" --project=chromium
 * 
 * Pre-condition:
...
test.describe('CRM-671_2.1.1 - NO MERGE: IBSA and CMR Teams', () => {
  ...
  test('Verify that leads do NOT merge when IBSA team and CMR team have same email', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (CMR Team)
    await test.step('Condition #2: Create Lead #2 with CMR team', async () => {
      console.log('=== CREATING LEAD #2 (CMR Team) ===');
    ...
    await test.step('Step 1: Wait to confirm NO merge happens', async () => {
      ...
      console.log(`IBSA team (Lead #1) and CMR team (Lead #2) should NOT merge despite same email`);
    ...
    await test.step('Final Summary', async () => {
      console.log('✅ TEST PASSED: NO MERGE occurred between IBSA and CMR teams as expected');
      console.log(`   Lead #1 (${lead1Id}): IBSA team, Active=TRUE, Is Won=Pending`);
      console.log(`   Lead #2 (${lead2Id}): CMR team, Active=TRUE, Is Won=Pending`);
      console.log(`   Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">🚫</span>NO MERGE TEST - IBSA AND CMR TEAMS - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="no-merge">
      <div class="no-merge-title">✓ NO MERGE OCCURRED (As Expected)</div>
      <div>IBSA team and CMR team leads with same email should NOT merge</div>
    </div>
    ...
    <div class="lead-section">
      <div class="lead-title">📋 Lead #2 - CMR Team (Remained Independent)</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead #2 (CMR): Active, Independent<br>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (IBSA and CMR) Verification Summary', {
