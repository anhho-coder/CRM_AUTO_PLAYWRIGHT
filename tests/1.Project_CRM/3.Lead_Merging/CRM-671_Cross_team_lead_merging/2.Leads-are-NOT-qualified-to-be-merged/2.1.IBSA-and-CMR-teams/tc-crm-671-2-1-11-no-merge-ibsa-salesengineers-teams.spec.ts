...
/**
 * Lead Merging Test - NO MERGE: IBSA and Sales Engineers Teams
 * Test Case ID: CRM-671_2.1.11
 * 
 * Summary: Verify that the merging leads do NOT happens if having 2 leads with the same email,
 * but a lead assigned to Install Base team, and another lead is assigned to Sales Engineers team (except Marketing)
 * 
 * Command to run:
 * npx playwright test --grep "CRM-671_2.1.11" --project=chromium
 * 
 * Pre-condition:
 ...
 *    - Sales Team dropdown list = Sales Engineers
 *    - Salesperson dropdown list is cleared
 *    - Created manually checkbox = FALSE
 * 3. Click at "CRM Developer" tab at the bottom of page
 *    - Lead form textbox = BLANK
 ...
 *    7.6. Sales Team dropdown list = Sales Engineers
...
test.describe('CRM-671_2.1.11 - NO MERGE: IBSA and Sales Engineers Teams', () => {
  ...
  test('Verify that leads do NOT merge when IBSA team and Sales Engineers team have same email', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Sales Engineers Team)
    await test.step('Condition #2: Create Lead #2 with Sales Engineers team', async () => {
      console.log('=== CREATING LEAD #2 (Sales Engineers Team) ===');
      ...
      // Set Sales Team to Sales Engineers
      await leadPage.selectSalesTeam('Sales Engineers');
      console.log(`  - Sales Team: Sales Engineers`);
    ...
    await test.step('Step 1: Wait to confirm NO merge happens', async () => {
      ...
      console.log(`IBSA team (Lead #1) and Sales Engineers team (Lead #2) should NOT merge despite same email`);
    ...
    await test.step('Step 7: Verify Lead #2 fields (should remain unchanged)', async () => {
      ...
      // Step 7.6: Sales Team = Sales Engineers
      await test.step('Step 7.6: Verify Sales Team = Sales Engineers (readonly)', async () => {
        const salesTeamValue = await leadPage.getSalesTeamValue();
        expect(salesTeamValue).toContain('Sales Engineers');
        console.log(`  ✓ Step 7.6: Sales Team = Sales Engineers`);
      });
    ...
    await test.step('Final Summary', async () => {
      console.log('✅ TEST PASSED: NO MERGE occurred between IBSA and Sales Engineers teams as expected');
      console.log(`   Lead #1 (${lead1Id}): IBSA team, Active=TRUE, Is Won=Pending`);
      console.log(`   Lead #2 (${lead2Id}): Sales Engineers team, Active=TRUE, Is Won=Pending`);
      console.log(`   Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">🚫</span>NO MERGE TEST - IBSA AND SALES ENGINEERS TEAMS - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="no-merge">
      <div class="no-merge-title">✓ NO MERGE OCCURRED (As Expected)</div>
      <div>IBSA team and Sales Engineers team leads with same email should NOT merge</div>
    </div>
    ...
    <div class="lead-section">
      <div class="lead-title">📋 Lead #2 - Sales Engineers Team (Remained Independent)</div>
      <div class="info-row"><span class="label">Lead ID:</span> ${lead2Id}</div>
      <div class="info-row"><span class="label">Sales Team:</span> Sales Engineers</div>
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #2 Verifications (Sales Engineers Team - No Merge)</div>
      <div class="checkpoint-detail"><span class="label">Tags:</span> <span class="result-pass">✓ BLANK</span></div>
      ...
      <div class="checkpoint-detail"><span class="label">Sales Team:</span> <span class="result-pass">✓ Sales Engineers</span></div>
      <div class="checkpoint-detail"><span class="label">CRM Developer:</span> <span class="result-pass">✓ Lead Form=BLANK, Active=TRUE, Is Won=Pending</span></div>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead #2 (Sales Engineers): Active, Independent<br>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (IBSA and Sales Engineers) Verification Summary', {
