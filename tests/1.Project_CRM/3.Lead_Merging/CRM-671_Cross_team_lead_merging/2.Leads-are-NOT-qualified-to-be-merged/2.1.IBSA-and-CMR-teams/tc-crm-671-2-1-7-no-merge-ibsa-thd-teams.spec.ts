...
/**
 * Lead Merging Test - NO MERGE: IBSA and THD Teams
 * Test Case ID: CRM-671_2.1.7
 * 
 * Summary: Verify that the merging leads do NOT happens if having 2 leads with the same email,
 * but a lead assigned to Install Base team, and another lead is assigned to THD team (except Marketing)
 * 
 * Command to run:
 * npx playwright test --grep "CRM-671_2.1.7" --project=chromium
 * 
 * Pre-condition:
 ...
 *    7.6. Sales Team dropdown list = THD
...
test.describe('CRM-671_2.1.7 - NO MERGE: IBSA and THD Teams', () => {
  ...
  test('Verify that leads do NOT merge when IBSA team and THD team have same email', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (THD Team)
    await test.step('Condition #2: Create Lead #2 with THD team', async () => {
      console.log('=== CREATING LEAD #2 (THD Team) ===');
      ...
      // Set Sales Team to THD
      await leadPage.selectSalesTeam('THD');
      console.log(`  - Sales Team: THD`);
    ...
    await test.step('Step 1: Wait to confirm NO merge happens', async () => {
      ...
      console.log(`IBSA team (Lead #1) and THD team (Lead #2) should NOT merge despite same email`);
    ...
    await test.step('Step 7: Verify Lead #2 fields (should remain unchanged)', async () => {
      ...
      // Step 7.6: Sales Team = THD
      await test.step('Step 7.6: Verify Sales Team = THD (readonly)', async () => {
        const salesTeamValue = await leadPage.getSalesTeamValue();
        expect(salesTeamValue).toContain('THD');
        console.log(`  ✓ Step 7.6: Sales Team = THD`);
      });
    ...
    await test.step('Final Summary', async () => {
      console.log('✅ TEST PASSED: NO MERGE occurred between IBSA and THD teams as expected');
      console.log(`   Lead #1 (${lead1Id}): IBSA team, Active=TRUE, Is Won=Pending`);
      console.log(`   Lead #2 (${lead2Id}): THD team, Active=TRUE, Is Won=Pending`);
      console.log(`   Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">🚫</span>NO MERGE TEST - IBSA AND THD TEAMS - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="no-merge">
      <div class="no-merge-title">✓ NO MERGE OCCURRED (As Expected)</div>
      <div>IBSA team and THD team leads with same email should NOT merge</div>
    </div>
    ...
    <div class="lead-section">
      <div class="lead-title">📋 Lead #2 - THD Team (Remained Independent)</div>
      <div class="info-row"><span class="label">Lead ID:</span> ${lead2Id}</div>
      <div class="info-row"><span class="label">Sales Team:</span> THD</div>
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #2 Verifications (THD Team - No Merge)</div>
      <div class="checkpoint-detail"><span class="label">Tags:</span> <span class="result-pass">✓ BLANK</span></div>
      ...
      <div class="checkpoint-detail"><span class="label">Sales Team:</span> <span class="result-pass">✓ THD</span></div>
      <div class="checkpoint-detail"><span class="label">CRM Developer:</span> <span class="result-pass">✓ Lead Form=BLANK, Active=TRUE, Is Won=Pending</span></div>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead #2 (THD): Active, Independent<br>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (IBSA and THD) Verification Summary', {
