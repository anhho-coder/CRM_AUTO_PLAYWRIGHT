...
/**
 * Lead Merging Test - NO MERGE: IBSA and BDEU Teams
 * Test Case ID: CRM-671_2.1.3
 * 
 * Summary: Verify that the merging leads do NOT happens if having 2 leads with the same email,
 * but a lead assigned to Install Base team, and another lead is assigned to BDEU team (except Marketing)
 * 
 * Command to run:
 * npx playwright test --grep "CRM-671_2.1.3" --project=chromium
 * 
 * Pre-condition:
...
test.describe('CRM-671_2.1.3 - NO MERGE: IBSA and BDEU Teams', () => {
  ...
  test('Verify that leads do NOT merge when IBSA team and BDEU team have same email', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (BDEU Team)
    await test.step('Condition #2: Create Lead #2 with BDEU team', async () => {
      console.log('=== CREATING LEAD #2 (BDEU Team) ===');
    ...
    await test.step('Step 1: Wait to confirm NO merge happens', async () => {
      ...
      console.log(`IBSA team (Lead #1) and BDEU team (Lead #2) should NOT merge despite same email`);
    ...
    await test.step('Final Summary', async () => {
      console.log('✅ TEST PASSED: NO MERGE occurred between IBSA and BDEU teams as expected');
      console.log(`   Lead #1 (${lead1Id}): IBSA team, Active=TRUE, Is Won=Pending`);
      console.log(`   Lead #2 (${lead2Id}): BDEU team, Active=TRUE, Is Won=Pending`);
      console.log(`   Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">🚫</span>NO MERGE TEST - IBSA AND BDEU TEAMS - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="no-merge">
      <div class="no-merge-title">✓ NO MERGE OCCURRED (As Expected)</div>
      <div>IBSA team and BDEU team leads with same email should NOT merge</div>
    </div>
    ...
    <div class="lead-section">
      <div class="lead-title">📋 Lead #2 - BDEU Team (Remained Independent)</div>
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #2 Verifications (BDEU Team - No Merge)</div>
      <div class="checkpoint-detail"><span class="label">Tags:</span> <span class="result-pass">✓ BLANK</span></div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead #2 (BDEU): Active, Independent<br>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (IBSA and BDEU) Verification Summary', {
