...
/**
 * Lead Merging Test - NO MERGE: BDEU and Marketing - BDR Teams with Existing Individual Contact (Child)
 * Test Case ID: CRM-1664_1.2.2
 * 
 * Summary: Verify that the merging leads will NOT happen if one Lead is assigned to Marketing - BDR 
 * and selecting an existing Individual contact
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1664_1.2.2" --project=chromium
 * 
 * Pre-condition:
 ...
 *    - "Email" textbox = Company email (with template Test-Reseller@Reseller-company + current date + current time + millisecond.com)
 ...
 * 6. Press "Contacts & Addresses" tab
 * 7. Press "ADD" button and wait for "Create Contacts" window appears
 * 8. On "Create Contacts" window do the following:
 *    8.1. Contact Name textbox with template: "CHILD" + current date time + Contact_name_Contact#1 where created above
 *         (Remember the Contact name of Child_Contact#1, called Contact_name_Child_Contact#1)
 *    8.2. Email textbox = with template: "CHILD" + current date time + domain of Email_Contact#1 where created above
 *         (Remember the Email of Child_Contact#1, called Email_Child_Contact#1)
 ...
 *    - Email textbox = Email_Child_Contact#1 from Child_Contact#1 above
 *    - (in the Address section)
 *    - Company Name textbox = Company Name Lead 1
 *    - Street dropdown list = 123street
 *    - Country dropdown list = Belgium
 *    - State dropdown list = Flanders
 *    - Sales Team dropdown list is BDEU
 *    - Salesperson dropdown list is cleared
 ...
 *    - Sales Team dropdown list is Marketing - BDR
 *    - Salesperson dropdown list is cleared
 *    - Created manually checkbox is FALSE
 * 3. Click at "CRM Developer" tab at the bottom of page
 ...
 *    3.6. Sales Team dropdown list is BDEU
 *    3.7. Email textbox = Email_Child_Contact#1
 ...
 *    7.6. Sales Team dropdown list is Marketing - BDR
 *    7.7. Email textbox = Email_Child_Contact#1
...
test.describe('CRM-1664_1.2.2 - NO MERGE: BDEU and Marketing - BDR Teams with Existing Individual Contact', () => {
  ...
  test('Verify that leads do NOT merge when BDEU team and Marketing - BDR team have same email from existing individual contact', async ({ page }, testInfo) => {
    ...
    const tcId = 'CRM-1664_1.2.2';
    let contactName: string;
    let contactEmail: string;
    let contactId: string;
    let childContactName: string;
    let childContactEmail: string;
    ...
    await test.step('Condition I - Steps 3-4: Create Company contact', async () => {
      ...
      await page.waitForTimeout(CommonUtils.waitTimes.savingPage);
      ...
      console.log(`  Contact_name_Contact#1: ${contactName}`);
    ...
    // Step 5: Press EDIT button
    await test.step('Condition I - Step 5: Press EDIT button', async () => {
      console.log('Step 5: Clicking EDIT button');
      
      await contactPage.clickEdit();
      await page.waitForTimeout(CommonUtils.waitTimes.standard);
      
      console.log('✓ Contact edit mode opened');
    });

    // Step 6-9: Add Child Contact
    await test.step('Condition I - Steps 6-9: Create Child Contact', async () => {
      console.log('Steps 6-9: Creating Child Contact');
      
      // Generate child contact details
      const timestamp = CommonUtils.generateTimestamp();
      const emailDomain = contactEmail.split('@')[1];
      childContactName = `CHILD${timestamp}_${contactName}`;
      childContactEmail = `CHILD${timestamp}@${emailDomain}`;
      
      console.log(`  - Child Contact Name: ${childContactName} (Contact_name_Child_Contact#1)`);
      console.log(`  - Child Email: ${childContactEmail} (Email_Child_Contact#1)`);
      
      // Create child contact using page object methods
      await contactPage.createChildContact(childContactName, childContactEmail);
      console.log('  ✓ Child contact created and dialog closed');
      
      // Save parent contact to persist child
      await contactPage.clickSave();
      await page.waitForTimeout(CommonUtils.waitTimes.long);
      console.log(`✓ Child Contact created: ${childContactName} (${childContactEmail})`);
      console.log(`  Contact_name_Child_Contact#1: ${childContactName}`);
      console.log(`  Email_Child_Contact#1: ${childContactEmail}\n`);
      
      // Capture screenshot after child creation
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Condition I - Child Contact Created`);
    });

    // Step 10: Save parent contact
    // Already done in previous step

    // Step 11: Press "Application" icon
    await test.step('Condition I - Step 11: Press Application icon', async () => {
      console.log('Step 11: Clicking Application icon');
    ...
    // Step 12: Navigate to CRM module
    await test.step('Condition I - Step 12: Navigate to CRM module', async () => {
      console.log('Step 12: Clicking CRM button');
    ...
    // Step 13: Navigate to Leads page
    await test.step('Condition I - Step 13: Navigate to Leads page', async () => {
      console.log('Step 13: Navigating to CRM > Leads page');
    ...
    await test.step('Condition II: Create Lead #1 with BDEU team and License form', async () => {
      ...
      // Use child contact email (Email_Child_Contact#1)
      await leadPage.fillEmail(childContactEmail);
      console.log(`  - Email: ${childContactEmail} (Email_Child_Contact#1)`);
    ...
    await test.step('Condition III: Create Lead #2 with Marketing - BDR team and Trial Download QNAP form', async () => {
      ...
      // Use the same child contact email (Email_Child_Contact#1)
      await leadPage.fillEmail(childContactEmail);
      console.log(`  - Email: ${childContactEmail} (Email_Child_Contact#1)`);
    ...
    await test.step('Step 3: Verify Lead #1 fields (should remain unchanged)', async () => {
      ...
      // Step 3.2: Company Name = contactName (parent contact)
      await test.step('Step 3.2: Verify Company Name = Contact_name_Contact#1 (parent contact)', async () => {
      ...
      // Step 3.7: Email = Email_Child_Contact#1
      await test.step('Step 3.7: Verify Email = Email_Child_Contact#1 (readonly)', async () => {
        const emailText = await leadPage.getEmailReadonly();
        expect(emailText).toContain(childContactEmail.split('@')[0]);
        console.log(`  ✓ Step 3.7: Email = ${childContactEmail} (Email_Child_Contact#1)`);
      });
    });
    ...
    await test.step('Step 7: Verify Lead #2 fields (should remain unchanged)', async () => {
      ...
      // Step 7.2: Company Name = contactName (parent contact)
      await test.step('Step 7.2: Verify Company Name = Contact_name_Contact#1 (parent contact)', async () => {
        const companyNameText = await leadPage.getCompanyNameReadonly();
        expect(companyNameText).toContain(contactName);
      ...
      // Step 7.7: Email = Email_Child_Contact#1
      await test.step('Step 7.7: Verify Email = Email_Child_Contact#1 (readonly)', async () => {
        const emailText = await leadPage.getEmailReadonly();
        expect(emailText).toContain(childContactEmail.split('@')[0]);
        console.log(`  ✓ Step 7.7: Email = ${childContactEmail} (Email_Child_Contact#1)`);
      });
    });
    ...
    await test.step('Final Summary', async () => {
      ...
      console.log(`   Parent Contact (${contactId}): ${contactName}, Email: ${contactEmail}`);
      console.log(`   Child Contact: ${childContactName}, Email: ${childContactEmail}`);
      ...
      console.log(`   Both leads used Child Contact Email: ${childContactEmail}`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">🚫</span>NO MERGE TEST - BDEU AND MARKETING - BDR TEAMS (WITH EXISTING INDIVIDUAL CONTACT) - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="no-merge">
      <div class="no-merge-title">✓ NO MERGE OCCURRED (As Expected)</div>
      <div>BDEU team and Marketing - BDR team leads with same email from existing individual (child) contact should NOT merge</div>
    </div>
    
    <div class="contact-section">
      <div class="contact-title">👤 Parent Contact #1 (Existing Company Contact)</div>
    ...
    <div class="contact-section">
      <div class="contact-title">👶 Child Contact (Individual Contact)</div>
      <div class="info-row"><span class="label">Contact Name:</span> ${childContactName}</div>
      <div class="info-row"><span class="label">Email:</span> ${childContactEmail} (Used by both leads)</div>
      <div class="info-row"><span class="label">Relationship:</span> Child of ${contactName}</div>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Email:</span> ${childContactEmail} (from Child Contact)</div>
    ...
    <div class="checkpoint">
      ...
      <div class="checkpoint-detail"><span class="label">Company Name:</span> <span class="result-pass">✓ ${contactName} (Contact_name_Contact#1)</span></div>
      ...
      <div class="checkpoint-detail"><span class="label">Email:</span> <span class="result-pass">✓ ${childContactEmail} (Email_Child_Contact#1)</span></div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Parent Contact: ${contactName} (${contactEmail})<br>
        Child Contact: ${childContactName} (${childContactEmail})<br>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (BDEU and Marketing - BDR with Existing Individual Contact) Verification Summary', {
