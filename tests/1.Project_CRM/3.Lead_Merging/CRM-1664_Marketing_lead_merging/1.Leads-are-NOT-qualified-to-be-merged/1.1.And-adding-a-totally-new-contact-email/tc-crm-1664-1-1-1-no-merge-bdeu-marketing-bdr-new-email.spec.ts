 ...
 * 
 * Command to run:
 * npx playwright test --grep "CRM-1664_1.1.1" --project=chromium
 * 
 * Pre-condition:
...
test.describe('CRM-1664_1.1.1 - NO MERGE: BDEU and Marketing - BDR Teams with New Email', () => {
  ...
  test('Verify that leads do NOT merge when BDEU team and Marketing - BDR team have same new email', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (BDEU Team with License form)
    await test.step('Condition #1: Create Lead #1 with BDEU team and License form', async () => {
      console.log('\n=== CONDITION I: CREATE LEAD #1 (BDEU Team with License form) ===');
      
      // Generate shared email that will be used for both leads
      sharedEmail = CommonUtils.generateEmail('Test', 'company');
      console.log(`Generated shared email (Email_Lead#1): ${sharedEmail}`);
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Condition I - Lead #1 Created (ID: ${lead1Id})`);
    });

    // CONDITION #2: Create Lead #2 (Marketing - BDR Team with Trial Download QNAP form)
    await test.step('Condition #2: Create Lead #2 with Marketing - BDR team and Trial Download QNAP form', async () => {
      console.log('=== CONDITION II: CREATE LEAD #2 (Marketing - BDR Team with Trial Download QNAP form) ===');
      ...
      await leadPage.fillEmail(sharedEmail);
      console.log(`  - Email: ${sharedEmail} (Email_Lead#1 from Condition#1)`);
      ...
      // Capture screenshot after creation
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Condition II - Lead #2 Created (ID: ${lead2Id})`);
    });
    ...
    await test.step('Step 7: Verify Lead #2 fields (should remain unchanged)', async () => {
      ...
      // Step 7.7: Email = Email_Lead#1
      await test.step('Step 7.7: Verify Email = Email_Lead#1 (readonly)', async () => {
        const emailText = await leadPage.getEmailReadonly();
        expect(emailText).toContain(sharedEmail.split('@')[0]);
    ...
    await test.step('Final Summary', async () => {
      ...
      console.log(`   Email: ${sharedEmail} (Email_Lead#1 - new email)`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">🚫</span>NO MERGE TEST - BDEU AND MARKETING - BDR TEAMS (NEW EMAIL) - VERIFICATION SUMMARY</h2>
    </div>
    
    <div class="no-merge">
      <div class="no-merge-title">✓ NO MERGE OCCURRED (As Expected)</div>
      <div>BDEU team and Marketing - BDR team leads with same new email should NOT merge</div>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Email:</span> ${sharedEmail} (Email_Lead#1)</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Email: ${sharedEmail} (new email)<br>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (BDEU and Marketing - BDR with New Email) Verification Summary', {
