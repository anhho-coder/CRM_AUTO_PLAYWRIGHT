/**
 * Lead Merging (Rule 7) - matrix ST-2: Sales Team set on one lead only (Salesperson empty)
 * Test Case ID: CRM-9059_7.1.2
 * Automation-Type: new
 * Automation-Date: 2026-07-06
 *
 * Summary: Two same-email leads; Lead#1 Sales Team = BDEU, Lead#2 Sales Team = (empty),
 *   Salesperson empty on both -> Rule 7 does NOT block (Sales Team is not non-empty on BOTH)
 *   -> the leads MERGE.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-9059_7\.1\.2 " --project=chromium
 *
 * Source manual TC (matrix ST-2):
...
test.describe('CRM-9059_7.1.2 - Lead Merging Rule 7: Sales Team set on one lead only (MERGE)', () => {
  ...
  test('CRM-9059_7.1.2: Verify leads MERGE when only one lead has a Sales Team (the other empty)', async ({ page }, testInfo) => {
