...
/**
 * Lead Merging (Rule 7) - matrix ST-4: DIFFERENT non-empty Sales Team (Salesperson empty) -> NO merge
 * Test Case ID: CRM-9059_7.1.4
 ...
 * lead has a DIFFERENT non-empty Sales Team (Lead#1 = BDEU, Lead#2 = CMR) and Salesperson empty.
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-9059_7\.1\.4" --project=chromium
...
test.describe('CRM-9059_7.1.4 - Lead Merging Rule 7: Sales Team different (NO merge)', () => {
  ...
  test.skip('CRM-9059_7.1.4 [CRM-9059]: Verify leads do NOT merge with same company email and DIFFERENT Sales Team', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = TRUE, Tag = Can_Merge, blank form, Sales Team = BDEU)
    await test.step('Condition #1: Create Lead #1 (Created Manually = TRUE, Tag = Can_Merge, Sales Team = BDEU)', async () => {
      console.log('=== CONDITION #1: CREATING LEAD #1 (Created Manually = TRUE, Tag = Can_Merge, Sales Team = BDEU) ===');
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, no tag, Sales Team = CMR - DIFFERENT)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Sales Team = CMR)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Sales Team = CMR) ===');
    ...
    await test.step('Step 7: Verify the following fields on Lead #2', async () => {
      ...
      await test.step('Step 7.6: Sales Team dropdown list = CMR', async () => {
        ...
        console.log(`  ✓ Step 7.6: Sales Team = ${salesTeam}`);
      });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Leads did NOT merge (different Sales Team - Rule 7 blocks)');
      console.log(`   Lead #1 (${lead1Id}): Active=TRUE, Is Won=Pending, Sales Team=BDEU`);
      console.log(`   Lead #2 (${lead2Id}): Active=TRUE, Is Won=Pending, Sales Team=CMR (NOT merged)`);
