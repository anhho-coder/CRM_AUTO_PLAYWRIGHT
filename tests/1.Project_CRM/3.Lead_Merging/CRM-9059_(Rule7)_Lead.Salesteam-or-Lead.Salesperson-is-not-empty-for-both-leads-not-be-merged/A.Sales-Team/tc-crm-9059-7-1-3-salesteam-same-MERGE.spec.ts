...
/**
 * Lead Merging (Rule 7) - matrix ST-3: Sales Team SAME on both (MERGE)
 ...
 * leads have the SAME Sales Team (Rule 7 does not block - positive control). Rule 7 blocks
...
test.describe('CRM-9059_7.1.3 - Lead Merging Rule 7: same Sales Team on both (MERGE)', () => {
  ...
  test('CRM-9059_7.1.3: Verify leads MERGE with same company email and the SAME Sales Team on both', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = TRUE, Tag = Can_Merge, blank form, Sales Team = BDEU, Salesperson CLEARED)
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, no tag, Sales Team = BDEU, Salesperson CLEARED)
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging verified (same Sales Team - Rule 7 does not block)');
