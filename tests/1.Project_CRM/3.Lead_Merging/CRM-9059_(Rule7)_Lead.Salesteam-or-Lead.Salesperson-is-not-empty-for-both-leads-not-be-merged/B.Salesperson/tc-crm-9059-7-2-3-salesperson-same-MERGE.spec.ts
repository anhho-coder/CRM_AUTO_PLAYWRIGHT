...
/**
 * Lead Merging (Rule 7) - matrix SP-3: Sales Team EMPTY + same Salesperson on both (MERGE)
 * Test Case ID: CRM-9059_7.2.3
 * Automation-Type: new
 * Automation-Date: 2026-07-07
 *
 * Summary: Verify that two merge-eligible leads with the SAME company email MERGE when both
 * have the SAME Salesperson but empty Sales Team (Rule 7 does not block - positive control). Rule 7 blocks
 * only when Sales Team OR Salesperson is non-empty on BOTH leads AND different.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-9059_7\.2\.3 -" --project=chromium
 *
 * Pre-condition:
...
test.describe('CRM-9059_7.2.3 - Lead Merging Rule 7: same Salesperson on both (MERGE)', () => {
  ...
  test('CRM-9059_7.2.3: Verify leads MERGE with same company email and the SAME Salesperson on both', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = TRUE, Tag = Can_Merge, blank form, Sales Team EMPTY, same Salesperson)
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, no tag, Sales Team EMPTY, same Salesperson)
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging verified (empty Sales Team + same Salesperson - Rule 7 does not block)');
