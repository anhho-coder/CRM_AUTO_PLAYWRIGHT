...
/**
 * Lead Merging (Rule 7) - matrix SP-1: Sales Team + Salesperson EMPTY on both (MERGE)
 * Test Case ID: CRM-9059_7.2.1
 * Automation-Type: new
 * Automation-Date: 2026-07-07
 *
 * Summary: Verify that two merge-eligible leads with the SAME company email MERGE when neither
 * lead has a Sales Team or Salesperson (Rule 7 does not block - positive control). Rule 7 blocks
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-9059_7\.2\.1 -" --project=chromium
 *
 * Pre-condition:
...
test.describe('CRM-9059_7.2.1 - Lead Merging Rule 7: Salesperson empty on both (MERGE)', () => {
  ...
  test('CRM-9059_7.2.1: Verify leads MERGE with same company email and empty Salesperson on both', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = TRUE, Tag = Can_Merge, blank form, Sales Team + Salesperson EMPTY)
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, no tag, Sales Team + Salesperson EMPTY)
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging verified (empty Sales Team + Salesperson - Rule 7 does not block)');
