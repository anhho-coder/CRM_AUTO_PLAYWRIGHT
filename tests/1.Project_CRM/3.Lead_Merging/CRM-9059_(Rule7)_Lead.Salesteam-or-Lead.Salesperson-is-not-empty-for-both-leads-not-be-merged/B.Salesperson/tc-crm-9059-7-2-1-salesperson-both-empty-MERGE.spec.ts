/**
 * Lead Merging (Rule 7) - matrix SP-1: Salesperson empty on BOTH (Sales Team empty)
 * Test Case ID: CRM-9059_7.2.1
 * Automation-Type: new
 * Automation-Date: 2026-07-06
 *
 * Summary: Two same-email leads with Salesperson + Sales Team empty on both -> Rule 7 does NOT
 *   block (positive control) -> the leads MERGE.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-9059_7\.2\.1 " --project=chromium
 *
 * Source manual TC (matrix SP-1):
...
test.describe('CRM-9059_7.2.1 - Lead Merging Rule 7: Salesperson empty on both (MERGE)', () => {
  ...
  test('CRM-9059_7.2.1: Verify leads MERGE when Salesperson + Sales Team are empty on both (baseline)', async ({ page }, testInfo) => {
