...
/**
 * Lead Merging (Rule 7) - matrix SP-2: Salesperson on Lead#1 only (MERGE)
 * Test Case ID: CRM-9059_7.2.2
 * Automation-Type: new
 * Automation-Date: 2026-07-07
 *
 * Summary: Verify that two merge-eligible leads with the SAME company email MERGE when Salesperson
 * is set on Lead#1 only and cleared on Lead#2 (Rule 7 does not block - positive control). Rule 7 blocks
...
test.describe('CRM-9059_7.2.2 - Lead Merging Rule 7: Salesperson on Lead#1 only (MERGE)', () => {
  ...
  test('CRM-9059_7.2.2: Verify leads MERGE with same company email and Salesperson set on Lead#1 only', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = TRUE, Tag = Can_Merge, blank form, Sales Team CLEARED, Salesperson = Thomas Semerich)
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, no tag, Sales Team CLEARED, Salesperson CLEARED)
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Lead merging verified (Salesperson on Lead#1 only - Rule 7 does not block)');
