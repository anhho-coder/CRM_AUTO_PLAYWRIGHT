...
/**
 * Lead Merging (Rule 7) - matrix C-2: DIFFERENT non-empty Sales Team with SAME non-empty Salesperson -> NO merge
 ...
 * lead has a DIFFERENT non-empty Sales Team (Lead#1 = BDEU, Lead#2 = CMR) but the SAME non-empty Salesperson
 * (Lead#1 = Thomas Semerich, Lead#2 = Thomas Semerich). Rule 7 must block the merge. This is the CRM-9059 defect
 * case: currently the leads STILL merge, so the test is skipped until the bug is fixed.
 *
 ...
 *    - Sales Team dropdown list = CMR (DIFFERENT from Lead#1)
 *    - Salesperson dropdown list = Thomas Semerich (SAME as Lead#1)
 ...
 * different non-empty Sales Team currently still merge). Un-skip once CRM-9059 is fixed.
 */

test.describe('CRM-9059_7.3.2 - Lead Merging Rule 7: different Sales Team, same Salesperson (NO merge)', () => {
  ...
  // SKIPPED due to known defect CRM-9059 (Rule 7 merge-guard broken - leads with different Sales Team still merge)
  test.skip('CRM-9059_7.3.2 [CRM-9059]: Verify leads do NOT merge with DIFFERENT Sales Team but same Salesperson', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Created Manually = TRUE, Tag = Can_Merge, blank form, Sales Team = BDEU, Salesperson = Thomas Semerich)
    await test.step('Condition #1: Create Lead #1 (Created Manually = TRUE, Tag = Can_Merge, Sales Team = BDEU, Salesperson = Thomas Semerich)', async () => {
      console.log('=== CONDITION #1: CREATING LEAD #1 (Created Manually = TRUE, Tag = Can_Merge, Sales Team = BDEU, Salesperson = Thomas Semerich) ===');
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, no tag, Sales Team = CMR - DIFFERENT, Salesperson = Thomas Semerich - SAME)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Sales Team = CMR, Salesperson = Thomas Semerich)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Sales Team = CMR, Salesperson = Thomas Semerich) ===');
      ...
      console.log(`  - Sales Team: CMR (DIFFERENT from Lead#1)`);

      await leadPage.selectSalesperson('Thomas Semerich');
      console.log(`  - Salesperson: Thomas Semerich (SAME as Lead#1)`);
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      await test.step('Step 3.6: Sales Team dropdown list = BDEU', async () => {
    ...
    await test.step('Step 7: Verify the following fields on Lead #2', async () => {
      ...
      await test.step('Step 7.6: Sales Team dropdown list = CMR', async () => {
        const salesTeam = await leadPage.getSalesTeamValue();
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Leads did NOT merge (different Sales Team but same Salesperson - Rule 7 blocks)');
      console.log(`   Lead #1 (${lead1Id}): Active=TRUE, Is Won=Pending, Sales Team=BDEU, Salesperson=Thomas Semerich`);
      console.log(`   Lead #2 (${lead2Id}): Active=TRUE, Is Won=Pending, Sales Team=CMR, Salesperson=Thomas Semerich (NOT merged)`);
