...
/**
 * Lead Merging (Rule 7) - matrix C-1: SAME non-empty Sales Team (different Salesperson) -> NO merge
 ...
 * lead has the SAME non-empty Sales Team (BDEU) but DIFFERENT Salesperson (Lead#1 = Thomas Semerich, Lead#2 = Mark Jawad).
 ...
 *    - Sales Team dropdown list = BDEU (SAME as Lead#1)
 ...
 *    7.6. Sales Team dropdown list = BDEU
 ...
 * same Sales Team but different Salesperson currently still merge). Un-skip once CRM-9059 is fixed.
 */

test.describe('CRM-9059_7.3.1 - Lead Merging Rule 7: same Sales Team, different Salesperson (NO merge)', () => {
  ...
  // SKIPPED due to known defect CRM-9059 (Rule 7 merge-guard broken - leads with same Sales Team but different Salesperson still merge)
  test.skip('CRM-9059_7.3.1 [CRM-9059]: Verify leads do NOT merge with same Sales Team but DIFFERENT Salesperson', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, no tag, Sales Team = BDEU - SAME, Salesperson = Mark Jawad - DIFFERENT)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Sales Team = BDEU, Salesperson = Mark Jawad)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Sales Team = BDEU, Salesperson = Mark Jawad) ===');
      ...
      await leadPage.selectSalesTeam('BDEU');
      console.log(`  - Sales Team: BDEU (SAME as Lead#1)`);
    ...
    await test.step('Step 7: Verify the following fields on Lead #2', async () => {
      ...
      await test.step('Step 7.6: Sales Team dropdown list = BDEU', async () => {
        const salesTeam = await leadPage.getSalesTeamValue();
        expect(salesTeam).toContain('BDEU');
        console.log(`  ✓ Step 7.6: Sales Team = BDEU`);
      });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Leads did NOT merge (same Sales Team, different Salesperson - Rule 7 blocks)');
      console.log(`   Lead #1 (${lead1Id}): Active=TRUE, Is Won=Pending, Sales Team=BDEU, Salesperson=Thomas Semerich`);
      console.log(`   Lead #2 (${lead2Id}): Active=TRUE, Is Won=Pending, Sales Team=BDEU, Salesperson=Mark Jawad (NOT merged)`);
