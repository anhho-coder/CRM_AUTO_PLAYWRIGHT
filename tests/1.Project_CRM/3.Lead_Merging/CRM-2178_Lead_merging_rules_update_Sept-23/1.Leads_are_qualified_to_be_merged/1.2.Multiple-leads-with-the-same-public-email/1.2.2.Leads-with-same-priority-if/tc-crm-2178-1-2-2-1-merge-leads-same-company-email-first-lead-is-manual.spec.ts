...
test.describe('CRM-2178_1.2.2.1 - Lead Merging: Same Public Email with Same Priority', () => {
  ...
  test('Verify merging leads with same public email and same priority', async ({ page }, testInfo) => {
    ...
    await test.step('Condition #1: Create Lead #1 (Created Manually = TRUE, Tag = Can_Merge, Lead Form = BLANK)', async () => {
      ...
      // Generate shared public email that will be used for both leads (Test + current date + current time + @gmail.com)
      const now = new Date();
      ...
      sharedEmail = `Test${currentDate}${currentTime}@gmail.com`;
