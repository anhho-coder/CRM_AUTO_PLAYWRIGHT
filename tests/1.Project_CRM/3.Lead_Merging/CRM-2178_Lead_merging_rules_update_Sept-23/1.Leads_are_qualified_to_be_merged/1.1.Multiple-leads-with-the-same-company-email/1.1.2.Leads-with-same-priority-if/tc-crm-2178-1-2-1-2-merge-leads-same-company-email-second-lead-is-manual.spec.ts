...
/**
 * Lead Merging Test - Same Company Email with Same Priority
 * Test Case ID: CRM-2178_1.1.2.2
 * 
 * Summary: Verify that the merging lead happens successfully when the leads with the same 
 * company email and same priority if a second created lead is created manually and has tag "can_merge"
 * 
 * Command to run:
 * npx playwright test --grep "CRM-2178_1.1.2.2" --project=chromium
 * 
 * Pre-condition:
...
test.describe('CRM-2178_1.1.2.2 - Lead Merging: Same Company Email with Same Priority', () => {
  ...
  test('Verify merging leads with same company email and same priority', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Tag = Can_Merge)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Tag = Can_Merge)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Tag = Can_Merge) ===');
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      // Step 3.1: Verify Tags contain "Can_Merge", "Product Registration"
      await test.step('Step 3.1: Tag field contains "Can_Merge", "Product Registration"', async () => {
    ...
    await test.step('Step 4: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      await test.step('Step 4.1: Lead form textbox = License', async () => {
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">✅</span>LEAD MERGING TEST - CRM-2178_1.1.1.1</h2>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Lead merging with different priority verified successfully<br>
...
`;
      ...
      await testInfo.attach('📋 Lead Merging - Test Summary', {
