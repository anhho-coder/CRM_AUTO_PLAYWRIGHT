...
/**
 * Lead Merging Test - Same Company Email with Same Priority (Deal Registration)
 * Test Case ID: CRM-2178_1.1.2.1
 * 
 * Summary: Verify that the merging lead happens successfully when the leads with the same 
 * company email and same priority if a first created lead is created manually and has tag "can_merge"
 * 
 * Command to run:
 * npx playwright test --grep "CRM-2178_1.1.2.1" --project=chromium
 * 
 * Pre-condition:
 ...
 *    3.1. Tag field contains 2 values: "Can_Merge", "Product Registration"
 ...
 *    7.1. Tag field contains "Product Registration"
...
test.describe('CRM-2178_1.1.2.1 - Lead Merging: Same Company Email with Same Priority', () => {
  ...
  test('Verify merging leads with same company email and same priority', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Lead Form = License, Deal Registration)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Lead Form = License, Deal Registration)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Lead Form = License, Deal Registration) ===');
    ...
    await test.step('Step 3: Verify the following fields on Lead #1', async () => {
      ...
      // Step 3.1: Verify Tags contain "Can_Merge" and "Product Registration" (2 values)
      await test.step('Step 3.1: Tag field contains 2 values: "Can_Merge", "Product Registration"', async () => {
        ...
        console.log(`  ✓ Step 3.1: Tags contain "Can_Merge" and "Product Registration"`);
      });
    ...
    await test.step('Step 7: Verify the following fields on Lead #2', async () => {
      ...
      // Step 7.1: Verify Tags contain "Product Registration"
      await test.step('Step 7.1: Tag field contains "Product Registration"', async () => {
        const tagsText = await leadPage.getTagsText();
        
        expect(tagsText).toContain('Product Registration');
        console.log(`  ✓ Step 7.1: Tags contain "Product Registration"`);
      });
    ...
    await test.step('Step 8: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      // Step 8.1: Verify Lead form = License
