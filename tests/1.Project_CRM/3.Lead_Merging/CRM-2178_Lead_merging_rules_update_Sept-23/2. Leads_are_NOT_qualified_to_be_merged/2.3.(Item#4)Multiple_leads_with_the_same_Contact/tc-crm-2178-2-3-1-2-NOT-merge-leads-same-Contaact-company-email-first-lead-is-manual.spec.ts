 ...
 *      - "Email" textbox = Company email (with template Test-Company@company + current date + current time + millisecond.com)
 *        (Remember the email domain of Contact#1, called Email_Domain_Contact#1 as @company + current date + current time + millisecond.com)
 ...
 *    - Email textbox = the same Email_Contact#1
 ...
 *    2.1. Tag field contains "Trial download"
 *    2.2. Company Name textbox = Contact_name_Contact#1
 *    2.3. Street dropdown list = 123street
 *    2.4. Country dropdown list = Belgium
 *    2.5. State dropdown list = Flanders
 *    2.6. Sales Team dropdown list has a value or not
 *    2.7. Email textbox = Email_Contact#1
 * 3. Click at "CRM Developer" tab at the bottom of page and verify the following:
 *    3.1. Lead form textbox = Download Free Trial
 *    3.2. Active checkbox = TRUE
 *    3.3. Is Won = Pending
 *    3.4. Lost Reason = BLANK
 */

test.describe('CRM-2178_2.3.1.1 - Lead Merging: Same Contact with Contact.Level NOT set (NO Merging)', () => {
  ...
  // FIXME: Test marked as fixme due to known bug CRM-9059
  test.skip('Verify merging leads will NOT happen with same Contact and Contact.Level NOT set', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Lead Form = Download Free Trial)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Lead Form = Download Free Trial)', async () => {
      console.log('=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Lead Form = Download Free Trial) ===');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: NO Lead merging verified - same Contact with Contact.Level NOT set');
      console.log(`   Lead #1 (${lead1Id}): Active=TRUE, Is Won=Pending, Tags: Can_Merge (1 value)`);
      console.log(`   - Email: ${email1} (Email_Contact#1)`);
      console.log(`   - Created Manually: TRUE`);
      console.log(`   - Lead Form: BLANK`);
      console.log(`   - Company Name: ${contactName} (Contact_name_Contact#1)`);
      console.log(`   - Location: Belgium, Flanders`);
      console.log(`   Lead #2 (${lead2Id}): Active=TRUE, Is Won=Pending, Tags: Trial download`);
      console.log(`   - Email: ${email2} (Email_Contact#1 - same as Lead #1)`);
      console.log(`   - Created Manually: FALSE`);
      console.log(`   - Lead Form: Download Free Trial`);
      console.log(`   - Company Name: ${contactName} (Contact_name_Contact#1)`);
      console.log(`   - Location: Belgium, Flanders (auto-set from Contact#1)`);
      console.log(`   Contact, Lead #1, and Lead #2 all share the same email`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">\u2705</span>NO LEAD MERGING TEST - PASSED (Same Contact Email)</h2>
    </div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Email:</span> ${email1} (same as Email_Contact#1)</div>
    ...
    <div class="lead-section">
      ...
      <div class="info-row"><span class="label">Email:</span> ${email2} (same as Email_Contact#1)</div>
      <div class="info-row"><span class="label">Tags:</span> Trial download</div>
      <div class="info-row"><span class="label">Company Name:</span> ${contactName} (Contact_name_Contact#1)</div>
      <div class="info-row"><span class="label">Location:</span> Belgium, Flanders (auto-set from Contact#1)</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Contact #1: ${contactEmail}<br>
        Lead #1 Email: ${email1} (same as Contact #1)<br>
        Lead #2 Email: ${email2} (same as Contact #1)<br>
        Contact, Lead #1, and Lead #2 all share the SAME email<br>
        Lead #2 location auto-set from Contact#1 (Belgium/Flanders)<br>
...
`;
      ...
      await testInfo.attach('\ud83d\udccb NO Lead Merging (Same Contact Email) - Test Summary', {
