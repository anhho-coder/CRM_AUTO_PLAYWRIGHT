 ...
 *    - Sales Team dropdown list is IBSA
 ...
 *    - Email textbox = Email_Lead#1 from Lead#1
 *    - (in the Address section)
 *      - Street dropdown list = 123street
 *      - Country dropdown list = United States
 *      - State dropdown list = Texas (US)
 ...
 *    3.6. Sales Team dropdown list is IBSA
 ...
 *    7.1. Tag field contains 1 value: "Partner signup"
 ...
 *    8.1. Lead form textbox = Partner signup
 *    8.2. Active checkbox = TRUE
 *    8.3. Is Won = Pending
 *    8.4. Lost Reason = BLANK
 */

test.describe('CRM-542_2.4.1 - Lead Merging: IB Renewal (IBNC Team) and Partner Signup Leads', () => {
  ...
  test('Verify merging lead happens successfully when leads from IB renewal (IBNC team) and Partner signup', async ({ page }, testInfo) => {
    ...
    // CONDITION #2: Create Lead #2 (Partner signup)
    await test.step('Condition #2: Create Lead #2 with Partner signup form', async () => {
      console.log('\n=== CREATING LEAD #2 (Partner signup) ===');
    ...
    await test.step('Step 3: Verify the following', async () => {
      ...
      // Step 3.1: Tag field contains 1 value: "Renewal"
      await test.step('Step 3.1: Verify Tag field contains 1 value: "Renewal"', async () => {
        ...
        console.log(`  ✓ Step 3.1: Tag field contains 1 value: "Renewal"`);
      });
    ...
    await test.step('Step 7: Verify the following', async () => {
      ...
      // Step 7.1: Tag field contains 1 value: "Partner signup"
      await test.step('Step 7.1: Verify Tag field contains 1 value: "Partner signup"', async () => {
        // In readonly mode, get tags text directly
        const tagsText = await leadPage.getTagsText();
        ...
        console.log(`  ✓ Step 7.1: Tag field contains 1 value: "Partner signup"`);
      });
    ...
    await test.step('Step 8: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      // Step 8.1: Lead form textbox = Partner signup
      await test.step('Step 8.1: Verify Lead form textbox = Partner signup', async () => {
        const leadFormValue = await leadPage.getLeadFormValue();
        expect(leadFormValue).toBe('Partner signup');
        console.log(`  ✓ Step 8.1: Lead form textbox = ${leadFormValue}`);
      });
      
      // Step 8.2: Active checkbox = TRUE
      await test.step('Step 8.2: Verify Active checkbox = TRUE', async () => {
        ...
        console.log(`  ✓ Step 8.2: Active checkbox = TRUE`);
      });
      ...
      await test.step('Step 8.4: Verify Lost Reason = BLANK', async () => {
        ...
        console.log(`    Approach 2 (textContent): ${lostReasonValue2 === '' ? 'BLANK' : lostReasonValue2}`);
    ...
    await test.step('Final Summary', async () => {
      ...
      console.log(`   Lead #1 (${lead1Id}): Active=TRUE, Is Won=Pending, Tags: Renewal`);
      console.log(`   Lead #2 (${lead2Id}): Active=TRUE, Is Won=Pending, Tags: Partner signup`);
      console.log(`   Email: ${sharedEmail}`);
...
<body>
  <div class="container">
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #1 Verifications (Steps 3-4)</div>
      <div class="checkpoint-detail"><span class="label">Tags:</span> <span class="result-pass">✓ Renewal only</span></div>
    ...
    <div class="checkpoint">
      <div class="checkpoint-title">✓ Lead #2 Verifications (Steps 7-8)</div>
      <div class="checkpoint-detail"><span class="label">Tags:</span> <span class="result-pass">✓ Partner signup only</span></div>
      <div class="checkpoint-detail"><span class="label">Company Name:</span> <span class="result-pass">✓ Company Name Lead 1</span></div>
      <div class="checkpoint-detail"><span class="label">Address:</span> <span class="result-pass">✓ 123street, Texas, United States</span></div>
      <div class="checkpoint-detail"><span class="label">Sales Team:</span> <span class="result-pass">✓ CMR</span></div>
      <div class="checkpoint-detail"><span class="label">CRM Developer:</span> <span class="result-pass">✓ Partner signup, Active=TRUE, Is Won=Pending, Lost Reason=BLANK</span></div>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead #1 (\${lead1Id}): Active, Tags: Renewal<br>
        Lead #2 (\${lead2Id}): Active, Tags: Partner signup, Company merged to Lead 1<br>
