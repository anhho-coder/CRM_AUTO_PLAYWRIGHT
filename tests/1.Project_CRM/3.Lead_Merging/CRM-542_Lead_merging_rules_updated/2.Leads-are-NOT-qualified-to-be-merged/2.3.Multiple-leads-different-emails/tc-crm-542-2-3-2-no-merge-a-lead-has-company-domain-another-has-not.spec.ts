...
/**
 * Lead Merging Test - NO MERGE: One Lead Has Existing Company Domain, Another Has Not
 * Test Case ID: CRM-542_2.3.2
 * 
 * Summary: Verify that the merging leads do NOT happens when the leads from different emails and a lead belong to an existing domain, the other is not
 * 
 * Command to run:
 * npx playwright test --grep "CRM-542_2.3.2" --project=chromium
 * 
 * Pre-condition:
 ...
 * 
 * II. Condition#2 to create Lead#1:
 ...
 * 
 * III. Condition#3 to create Lead#2:
 * 1. Click at "CRM" button
 * 2. On "CRM" page, on menu on top of page, select "Leads" item then "Leads" sub-item
 * 3. On "Leads" page, click at "CREATE" button
 * 4. Enter the following information:
...
test.describe('CRM-542_2.3.2 - NO MERGE: One Lead Has Existing Company Domain, Another Has Not', () => {
  ...
  test('Verify that leads do NOT merge when one lead has existing company domain and another has not', async ({ page }, testInfo) => {
    ...
    // CONDITION II: Navigate to CRM and create Lead #1
    await test.step('Condition II - Steps 1-2: Navigate to CRM > Leads', async () => {
      console.log('=== CONDITION II: CREATE LEAD #1 ===');
    ...
    // CONDITION III: Navigate to CRM and create Lead #2
    await test.step('Condition III - Steps 1-2: Navigate to CRM > Leads', async () => {
      console.log('=== CONDITION III: CREATE LEAD #2 ===');
      console.log('Steps 1-2: Navigating to CRM > Leads');
    ...
    await test.step('Condition III - Steps 3-7: Create Lead #2 with Download Free Trial form', async () => {
      ...
      // Generate Lead #2 name and email with company2 domain (NOT existing partner domain)
      ...
      // Use email with company2 domain (different from Contact#1's domain)
      await leadPage.fillEmail(lead2Email);
      console.log(`  - Email: ${lead2Email} (Email_Lead#2 with company2 domain - NOT existing partner)`);
    ...
    await test.step('Final Summary', async () => {
      console.log('✅ TEST PASSED: NO MERGE occurred - One lead has existing domain, another does not');
      console.log(`   Contact #1 (${contact1Id}): ${contact1Name}, Email: ${contact1Email}`);
      console.log(`   Lead #1 (${lead1Id}): Uses Contact#1 domain, CMR team, Company=Company Name Lead 1, Lead Form=IB NC Leads, Tag=Renewal, Email=${lead1Email}, Active=TRUE`);
      console.log(`   Lead #2 (${lead2Id}): Uses company2 domain (NOT existing partner), CMR team, Company=Company Name Lead 2, Lead Form=Download Free Trial, Tag=Trial download, Email=${lead2Email}, Active=TRUE`);
      console.log(`   Both leads remain independent - NO MERGE occurred: Lead#1 has existing partner domain, Lead#2 does not`);
      console.log('==================================================\n');
...
<body>
  <div class="container">
    ...
    <div class="no-merge">
      ...
      <div>Leads with different email domains: Lead#1 has existing partner domain, Lead#2 does not</div>
    </div>
    
    <div class="contact-section">
      <div class="contact-title">👤 Contact #1 (Existing Partner Domain)</div>
    ...
    <div class="lead-section">
      <div class="lead-title">📋 Lead #1 - Uses Existing Partner Domain</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Contact #1: ${contact1Name} (${contact1Email}) - Existing partner domain<br>
        Lead #1 (CMR): Uses existing partner domain, Company=Company Name Lead 1, IB NC Leads form, Renewal tag, Email=${lead1Email}, Active<br>
        Lead #2 (CMR): Uses company2 domain (NOT existing partner), Company=Company Name Lead 2, Download Free Trial form, Trial download tag, Email=${lead2Email}, Active<br>
        Both leads remain active - NO MERGE: One lead has existing partner domain, the other does not
      </div>
    </div>
  </div>
</body>
...
`;
      ...
      await testInfo.attach('📋 NO MERGE Test (One Lead Has Existing Domain, Another Has Not) Verification Summary', {
