...
/**
 * Lead Merging Test - Same Company Domain Email with Equal Priority
 * Test Case ID: CRM-542_1.1.2.1
 ...
 *    - Lead form textbox = Trial download
 * 4. Press "SAVE" button
 ...
 *    3.8. Contact Name textbox = Contact Name Lead 1
 * 4. Click at "CRM Developer" tab at the bottom of page and verify the following:
 *    4.1. Lead form textbox = Trial download
 ...
 *    8.1. Lead form textbox = Trial download
...
test.describe('CRM-542_1.1.2.1 - Lead Merging: Same Company Domain Email with Equal Priority (Different Fields)', () => {
  ...
  test('[CRM-11755] Verify merging lead happens successfully when leads have equal priority with different filled-in fields', async ({ page }, testInfo) => {
    ...
    // CONDITION #1: Create Lead #1 (Trial download - without Contact Name)
    await test.step('Condition #1: Create Lead #1 with Trial download form', async () => {
      console.log('\n=== CREATING LEAD #1 (Trial download - Equal Priority) ===');
    ...
    // CONDITION #2: Create Lead #2 (Trial download - with Contact Name)
    await test.step('Condition #2: Create Lead #2 with Trial download form', async () => {
      console.log('=== CREATING LEAD #2 (Trial download - Equal Priority) ===');
    ...
    await test.step('Step 3: Verify the following', async () => {
      ...
      // Step 3.8: Contact Name = Contact Name Lead 2
      await test.step('Step 3.8: Verify Contact Name = Contact Name Lead 2 (readonly)', async () => {
        ...
        console.log(`  ✓ Step 3.8: Contact Name = Contact Name Lead 2`);
      });
    });
    ...
    await test.step('Step 4: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      // Step 4.1: Lead form textbox = Trial download
      await test.step('Step 4.1: Verify Lead form textbox = Trial download', async () => {
    ...
    await test.step('Step 8: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      // Step 8.1: Lead form textbox = Trial download
      await test.step('Step 8.1: Verify Lead form textbox = Trial download', async () => {
        const leadFormValue = await leadPage.getLeadFormValue();
        expect(leadFormValue).toBe('Download Free Trial');
    ...
    await test.step('Final Summary', async () => {
      ...
      console.log(`   Lead #1 (${lead1Id}): Trial download (equal priority)`);
      console.log(`   Lead #1: Active=TRUE, Is Won=Pending, Tag: Trial download`);
      console.log(`   Lead #2 (${lead2Id}): Trial download (equal priority)`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">✅</span>LEAD MERGING TEST - CRM-542_1.1.2.1</h2>
    </div>
    
    <div class="lead-section">
      <div class="lead-title">📋 Lead #1 - Trial download (Target Lead)</div>
      ...
      <div class="info-row"><span class="label">Lead Form:</span> Download Free Trial (Equal Priority)</div>
      <div class="info-row"><span class="label">Company Name:</span> Company Name Lead 1</div>
      <div class="info-row"><span class="label">Contact Name:</span> Not set initially</div>
    </div>
    
    <div class="lead-section">
      <div class="lead-title">📋 Lead #2 - Trial download (Source Lead)</div>
    ...
    <div class="checkpoint">
      ...
      <div class="checkpoint-detail"><span class="label">Contact Name:</span> <span class="result-pass">✓ Contact Name Lead 1</span></div>
    ...
    <div class="checkpoint">
      ...
      <div class="checkpoint-detail"><span class="label">Company Name:</span> <span class="result-pass">✓ Company Name Lead 1 (inherited)</span></div>
      <div class="checkpoint-detail"><span class="label">Address:</span> <span class="result-pass">✓ 123street, Texas, United States</span></div>
      <div class="checkpoint-detail"><span class="label">Sales Team:</span> <span class="result-pass">✓ CMR</span></div>
      <div class="checkpoint-detail"><span class="label">Email:</span> <span class="result-pass">✓ ${sharedEmail}</span></div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Lead merging completed successfully with equal priority<br>
        Both leads had Trial download form (equal priority)<br>
        Different filled-in fields handled correctly<br>
