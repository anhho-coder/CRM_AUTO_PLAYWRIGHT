 ...
 * 
 * Command to run:
 * npx playwright test --grep "LeadMerging-Exploratory_5.1" --project=chromium
 * 
  * Pre-condition:
 ...
 * 6. Press "Application" icon on left-top of screen and wait for Lead Merging happened
 * 
 ...
 *    5.2. There is the text as "[LEAD 3], has been merged into this lead." where [LEAD 3] is Lead Name of Lead#3
 * 
...
test.describe('LeadMerging-Exploratory_5.1 - Source Lead Already Merged from Another Source Lead', () => {
  ...
  test('Verify merging lead when source lead is already merged from another source lead', async ({ page }, testInfo) => {
    ...
    const tcId = 'LeadMerging-Exploratory_5.1';
    ...
    // CONDITION #2: Create Lead #2 (Created Manually = FALSE, Download Free Trial)
    await test.step('Condition #2: Create Lead #2 (Created Manually = FALSE, Lead Form = Download Free Trial)', async () => {
      console.log('\n=== CONDITION #2: CREATING LEAD #2 (Created Manually = FALSE, Lead Form = Download Free Trial) ===');
      ...
      // Press "Application" icon on left-top of screen and wait for Lead Merging
      await homePage.clickApplicationMenu();
      await homePage.waitForPageReady();
      
      // Wait for Lead #2 to merge into Opp #1
      console.log('⏳ Waiting for Lead #2 to merge into Opp #1...');
      await page.waitForTimeout(60000); // Wait 1 minute for merging
      console.log('✓ Lead #2 merging process completed\n');
    });
    ...
    await test.step('Step 5: On the Log area, verify the following', async () => {
      ...
      // Step 5.1: Verify Lead #2 merge message
      await test.step('Step 5.1: Verify Lead #2 merge message in Log area', async () => {
        const hasMergeMessage = await opportunityPage.hasSourceLeadMergeMessage(lead2Name);
        
        expect(hasMergeMessage).toBeTruthy();
        console.log(`  ✓ Step 5.1: Found log message: "${lead2Name}, has been merged into this lead."`);
      });
      
      // Step 5.2: Verify Lead #3 merge message
      await test.step('Step 5.2: Verify Lead #3 merge message in Log area', async () => {
        const hasMergeMessage = await opportunityPage.hasSourceLeadMergeMessage(lead3Name);
        
        expect(hasMergeMessage).toBeTruthy();
        console.log(`  ✓ Step 5.2: Found log message: "${lead3Name}, has been merged into this lead."`);
      });
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Source lead already merged from another source lead verified successfully');
      console.log(`   Opp #1 (${opp1Id}): Active=TRUE, Is Won=Pending, Tags: Renewal, Trial download`);
      console.log(`   - Created Manually: FALSE`);
      console.log(`   - Lead Form: IB NC Leads`);
      console.log(`   - Target opportunity (receives both merged leads)`);
      console.log(`   Lead #2 (${lead2Id}): Active=FALSE, Is Won=Lost, Lost Reason=Duplicate`);
      console.log(`   - Created Manually: FALSE`);
      console.log(`   - Lead Form: Download Free Trial`);
      console.log(`   - Merged into Opp #1 (first source lead)`);
      console.log(`   Lead #3 (${lead3Id}): Active=FALSE, Is Won=Lost, Lost Reason=Duplicate`);
      console.log(`   - Created Manually: FALSE`);
      console.log(`   - Lead Form: Download Free Trial`);
      console.log(`   - Merged into Opp #1 (second source lead, already merged from Lead #2)`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">✅</span>SOURCE LEAD ALREADY MERGED FROM ANOTHER SOURCE LEAD - LeadMerging-Exploratory_5.1</h2>
    </div>
    ...
    <div class="steps-section">
      ...
      <div class="step-group">
        <div class="step-group-title">Condition #2: Creating Lead #2 (Created Manually = FALSE, Lead Form = Download Free Trial)</div>
        ...
        <div class="step-item">Application icon pressed</div>
        <div class="step-item">Waiting 60 seconds for Lead #2 to merge into Opp #1...</div>
        <div class="step-item">Lead #2 merging process completed</div>
      </div>
      ...
      <div class="step-group">
        ...
        <div class="step-item">Step 5.1: Found log message: "${lead2Name}, has been merged into this lead."</div>
        <div class="step-item">Step 5.2: Found log message: "${lead3Name}, has been merged into this lead."</div>
      </div>
      
      <div class="step-group">
        <div class="step-group-title">Step 6: Open Lead #2 (First Source - Merged)</div>
        <div class="step-item">Lead #2 opened successfully</div>
      </div>
      
      <div class="step-group">
        <div class="step-group-title">Step 7: Verify Lead #2 Fields (First Source - Merged)</div>
      ...
      <div class="step-group">
        <div class="step-group-title">Step 9: Verify Log Area (Lead #2)</div>
        <div class="step-item">Step 9.1: Found log message: "This lead has been merged into ${opp1Name}."</div>
      </div>
      
      <div class="step-group">
        <div class="step-group-title">Step 10: Open Lead #3 (Second Source - Merged)</div>
        <div class="step-item">Lead #3 opened successfully</div>
      </div>
      
      <div class="step-group">
        <div class="step-group-title">Step 11: Verify Lead #3 Fields (Second Source - Merged)</div>
    ...
    <div class="entity-section">
      <div class="entity-title">📋 Opp #1 - Target Opportunity (Active - Receives Both Merged Leads)</div>
      ...
      <div class="info-row"><span class="label">Note:</span> Target opportunity (receives both Lead #2 and Lead #3)</div>
    ...
    <div class="entity-section">
      <div class="entity-title">📋 Lead #2 - First Source Lead (Merged into Opp #1)</div>
      ...
      <div class="info-row"><span class="label">Note:</span> First source lead - merged into Opp #1</div>
    ...
    <div class="entity-section">
      <div class="entity-title">📋 Lead #3 - Second Source Lead (Merged into Opp #1)</div>
      ...
      <div class="info-row"><span class="label">Created Manually:</span> FALSE</div>
      <div class="info-row"><span class="label">Lead Form:</span> Download Free Trial</div>
      <div class="info-row"><span class="label">Note:</span> Second source lead - merged into Opp #1 (source already merged scenario)</div>
      <div class="info-row"><span class="label">Active:</span> FALSE</div>
      <div class="info-row"><span class="label">Is Won:</span> Lost</div>
      <div class="info-row"><span class="label">Lost Reason:</span> Duplicate</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Source lead already merged from another source lead verified successfully<br>
        Lead #2 (Download Free Trial) merged first into Opp #1<br>
        Lead #3 (Download Free Trial) merged second into Opp #1<br>
        Both source leads successfully merged into target opportunity<br>
...
`;
      ...
      await testInfo.attach('📋 Source Lead Already Merged - Test Summary', {
