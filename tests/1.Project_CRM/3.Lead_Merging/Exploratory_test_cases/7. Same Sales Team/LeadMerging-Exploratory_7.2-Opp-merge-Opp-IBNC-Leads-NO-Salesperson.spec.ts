...
/**
 * Opp Merging to Opp Test - Same Sales Team Target Has NULL Salesperson
 * Test Case ID: LeadMerging-Exploratory_7.2
 * Automation-Type: refactored
 * Automation-Date: 2026-08-07
 *
 ...
 *    - Salesperson dropdown list is BLANK
 ...
 *    - Sales Team dropdown list is "BDEU"
 *    - Salesperson dropdown list is "Mark Jawad"
 *    - Created manually checkbox is FALSE
 * 3. Click at "CRM Developer" tab at the bottom of page
 ...
 *    5.1. There is the text as "[Opp 1], has been merged into this lead." where [Opp 1] is Opp Name of Opp#1
 * 6. Open the Opp#1 using URL_Opp#1
 * 7. Verify the following:
 *    7.1. Tag field contains "Trial download"
 *    7.2. Company Name textbox = Company Name Opp 1 (which auto set due to created Contact)
 *    7.3. Street dropdown list = 123street
 *    7.4. Country dropdown list = Belgium
 *    7.5. State dropdown list = Flanders
 ...
 *    9.1. There is the text as "This lead has been merged into [Opp 2]." where [Opp 2] is Opp Name of Opp#2
 */

test.describe('LeadMerging-Exploratory_7.2 - Opp Merging to Opp: Same Sales Team Target Has NULL Salesperson', () => {
  ...
  test('Verify merging lead when leads have same sales Team but Target has NULL salesperson', async ({ page }, testInfo) => {
    ...
    const tcId = 'LeadMerging-Exploratory_7.2';
    ...
    await test.step('Condition #1: Create Opp#1 (Created Manually = FALSE, Lead Form = Download Free Trial)', async () => {
      ...
      // Clear Salesperson (set to BLANK)
      await opportunityPage.clearSalesperson();
      console.log(`  - Salesperson: BLANK`);
    ...
    await test.step('Condition #2: Create Opp #2 (Created Manually = FALSE, Lead Form = IB NC Leads)', async () => {
      ...
      // Set Salesperson to Mark Jawad
      await opportunityPage.selectSalesperson('Mark Jawad');
    ...
    await test.step('Step 1: Wait for 1 minute for Lead Merging happened', async () => {
      ...
      // Wait for lead merging to happen (stays at Target Lead page - Lead #2)
      mergeNotificationFound = await leadPage.waitForLeadMergingHappen_OnTargetLead(opp1Name, 6, 30000);
    ...
    if (!mergeNotificationFound) {
      test.skip(true, mergeDeferSkipReason('LeadMerging-Exploratory_7.2'));
    }
    expect(mergeNotificationFound).toBeTruthy();

    // STEP 2: Open Opp #2 (Target)
    await test.step('Step 2: Open the Opp#2 using URL_Opp#2', async () => {
      console.log('\n=== STEP 2: OPEN OPP #2 (TARGET) ===');
    ...
    await test.step('Step 3: Verify the following fields on Opp #2', async () => {
      console.log('=== STEP 3: VERIFY OPP #2 FIELDS (READONLY MODE) ===\n');
      ...
      await test.step('Step 3.4: Country dropdown list = United States', async () => {
        const addressText = await opportunityPage.getAddressReadonly();
      ...
      await test.step('Step 3.5: State dropdown list = Texas', async () => {
        const addressText = await opportunityPage.getAddressReadonly();
    ...
    await test.step('Step 5: On the Log area, verify the following', async () => {
      ...
      await test.step('Step 5.1: Verify merge message in Log area', async () => {
        const hasMergeMessage = await opportunityPage.hasSourceLeadMergeMessage(opp1Name);
        
        expect(hasMergeMessage).toBeTruthy();
        console.log(`  ✓ Step 5.1: Found log message: "${opp1Name}, has been merged into this lead."`);
      });
    });

    // STEP 6: Open Opp #1 (Source)
    await test.step('Step 6: Open the Opp#1 using URL_Opp#1', async () => {
      console.log('\n=== STEP 6: OPEN OPP #1 (SOURCE) ===');
    ...
    // STEP 7: Verify Opp #1 fields
    await test.step('Step 7: Verify the following fields on Opp #1', async () => {
      console.log('=== STEP 7: VERIFY OPP #1 FIELDS (READONLY MODE) ===\n');
      
      // Step 7.1: Verify Tags contain "Trial download"
      await test.step('Step 7.1: Tag field contains "Trial download"', async () => {
        const tagsText = await opportunityPage.getTagsText();
        
        expect(tagsText).toContain('Trial download');
        console.log(`  ✓ Step 7.1: Tags contain "Trial download"`);
      });
      ...
      await test.step('Step 7.4: Country dropdown list = Belgium', async () => {
        const addressText = await opportunityPage.getAddressReadonly();
        
        expect(addressText).toContain('Belgium');
        console.log(`  ✓ Step 7.4: Country = Belgium`);
      });

      // Step 7.5: Verify State
      await test.step('Step 7.5: State dropdown list = Flanders', async () => {
        const addressText = await opportunityPage.getAddressReadonly();
        
        expect(addressText).toContain('Flanders');
        console.log(`  ✓ Step 7.5: State = Flanders`);
      });
    ...
    await test.step('Step 8: Click at "CRM Developer" tab and verify the following', async () => {
      ...
      await test.step('Step 8.1: Lead form textbox = Download Free Trial', async () => {
        const leadForm = await opportunityPage.getLeadFormValue();
    ...
    await test.step('Step 9: On the Log area, verify the following', async () => {
      ...
      await test.step('Step 9.1: Verify merge message in Log area', async () => {
        const hasMergeMessage = await opportunityPage.hasTargetLeadMergeMessage(opp2Name);
        
        expect(hasMergeMessage).toBeTruthy();
        console.log(`  ✓ Step 9.1: Found log message: "This lead has been merged into ${opp2Name}."`);
      });
    });

    // Final Summary
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: Same Sales Team different salesperson scenario verified successfully');
      console.log(`   Opp #2 (${opp2Id}): Active=TRUE, Is Won=Pending, Tags: Renewal, Trial download`);
      console.log(`   - Created Manually: FALSE`);
      console.log(`   - Lead Form: IB NC Leads`);
      console.log(`   - Sales Team: BDEU`);
      console.log(`   - Salesperson: Mark Jawad`);
...
<body>
  <div class="container">
    <div class="header">
      <h2><span class="icon">✅</span>SAME SALES TEAM DIFFERENT SALESPERSON TEST - LeadMerging-Exploratory_7.1</h2>
    </div>
    ...
    <div class="steps-section">
      ...
      <div class="step-group">
        ...
        <div class="step-item">Salesperson: BLANK</div>
      ...
      <div class="step-group">
        ...
        <div class="step-item">Sales Team: BDEU</div>
        <div class="step-item">Salesperson: Mark Jawad</div>
        <div class="step-item">Created Manually: FALSE</div>
      ...
      <div class="step-group">
        <div class="step-group-title">Step 2: Open Opp #2 (Target)</div>
      ...
      <div class="step-group">
        <div class="step-group-title">Step 4: Verify CRM Developer Tab (Opp #2)</div>
      ...
      <div class="step-group">
        <div class="step-group-title">Step 5: Verify Log Area (Opp #2)</div>
        <div class="step-item">Step 5.1: Found log message: "${opp1Name}, has been merged into this lead."</div>
      </div>
      
      <div class="step-group">
        <div class="step-group-title">Step 6: Open Opp #1 (Source)</div>
        <div class="step-item">Opp #1 opened successfully</div>
      </div>
      
      <div class="step-group">
        <div class="step-group-title">Step 7: Verify Opp #1 Fields (Source - Merged)</div>
        <div class="step-item">Step 7.1: Tags contain "Trial download"</div>
        ...
        <div class="step-item">Step 7.4: Country = Belgium</div>
        <div class="step-item">Step 7.5: State = Flanders</div>
        <div class="step-item">Step 7.6: Sales Team = verified</div>
        <div class="step-item">Step 7.7: Email = ${sharedEmail}</div>
      </div>
      
      <div class="step-group">
        <div class="step-group-title">Step 8: Verify CRM Developer Tab (Opp #1)</div>
        <div class="step-item">Step 8.1: Lead form = Download Free Trial</div>
      ...
      <div class="step-group">
        <div class="step-group-title">Step 9: Verify Log Area (Opp #1)</div>
        <div class="step-item">Step 9.1: Found log message: "This lead has been merged into ${opp2Name}."</div>
      </div>
    </div>
    
    <div class="entity-section">
      <div class="entity-title">📋 Opp #2 - Target Opportunity (Active)</div>
    ...
    <div class="entity-section">
      <div class="entity-title">📋 Opp #1 - Source Opportunity (Merged)</div>
      ...
      <div class="info-row"><span class="label">Sales Team:</span> BDEU</div>
      <div class="info-row"><span class="label">Salesperson:</span> BLANK</div>
      <div class="info-row"><span class="label">Created Manually:</span> FALSE</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        Same Sales Team different salesperson scenario verified successfully<br>
        Both opportunities have Sales Team: BDEU<br>
        Opp #1 (Thomas Semerich) merged into Opp #2 (Mark Jawad)<br>
...
`;
      ...
      await testInfo.attach('📋 Same Sales Team Different Salesperson - Test Summary', {
