 ...
 * Test Case ID: CRM-2482_3.1.1.1.2
 *
 * Summary: Verify the new contact/ new Opp are created after importing new audience using xls file
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_3.1.1.1.2" --project=chromium
 *
 * Pre-condition:
 ...
 * 5. Create an Import Audience XLS file:
 ...
 *         - Tags          = test
 *    5.2. Use template XLS file: XLS-AudienceTemplate-1.xls
 *    5.3. Create XLS-Audience-copy1.xls and fill test data into respective columns
 *    5.4. Save the file to test-data/investment-module/XLS-Audience-copy1.xls
 *
 * Steps to reproduce:
 ...
 * 5. Select XLS-Audience-copy1 file and wait
 ...
 * 2. Delete the created XLS file (XLS-Audience-copy1.xls)
 */

test.describe('CRM-2482_3.1.1.1.2 - Verify new contact/Opp created after importing audience via XLS', () => {
  ...
  test('CRM-2482_3.1.1.1.2: Verify new contact/Opp created after importing new audience using xls file', async ({ page }, testInfo) => {
    ...
    // ---- Step 5: Create an Import Audience XLS file ----
    let audienceData: Awaited<ReturnType<typeof investmentPage.createImportAudienceXLSFile>>;

    await test.step('Step 5: Create an Import Audience XLS file (XLS-Audience-copy1.xls)', async () => {
      console.log('=== STEP 5: CREATE IMPORT AUDIENCE XLS FILE ===');

      audienceData = await investmentPage.createImportAudienceXLSFile();

      expect(fs.existsSync(audienceData.outputPath), `Output XLS was not created at: ${audienceData.outputPath}`).toBeTruthy();

      console.log('  ✓ XLS file created and verified — file exists on disk');
      console.log(`  ✓ Output path      : ${audienceData.outputPath}`);
      console.log(`  ✓ Contact Name     : ${audienceData.contactName}`);
      console.log(`  ✓ Email_Contact#1  : ${audienceData.emailContact1}`);
      console.log(`  ✓ Company          : ${audienceData.companyName}`);
    });
    ...
    // Step 3–5: Click "UPLOAD YOUR FILE" button and upload the XLS
    await test.step('Step 3-5: Click "UPLOAD YOUR FILE", navigate to folder and select XLS-Audience-copy1', async () => {
      console.log('=== STEP 3-5: UPLOAD XLS FILE ===');

      await investmentPage.uploadAudienceCSV(audienceData.outputPath);

      console.log(`✓ XLS file uploaded: ${audienceData.outputPath}`);
    });
    ...
    // Tear down Step 2: Delete XLS file
    await test.step('Tear down Step 2: Delete XLS file XLS-Audience-copy1.xls', async () => {
      console.log('Tear down Step 2: Deleting XLS file');

      if (fs.existsSync(audienceData.outputPath)) {
        fs.unlinkSync(audienceData.outputPath);
        expect(fs.existsSync(audienceData.outputPath), `XLS file was not deleted: ${audienceData.outputPath}`).toBeFalsy();
        console.log(`✓ XLS file deleted: ${audienceData.outputPath}`);
      } else {
        console.log(`  ℹ️ XLS file not found (already deleted or never created): ${audienceData.outputPath}`);
      }
    });
