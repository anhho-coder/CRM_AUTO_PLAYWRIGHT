 ...
 * Test Case ID: CRM-2482_3.1.1.1.3
 *
 * Summary: Verify the new contact/ new Opp are created after importing new audience using XLSX file
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_3.1.1.1.3" --project=chromium
 *
 * Pre-condition:
 ...
 * 5. Create an Import Audience XLSX file:
 ...
 *         - Tags          = test
 *    5.2. Use template XLSX file: XLSX-AudienceTemplate-1.xlsx
 *    5.3. Create XLSX-Audience-copy1.xlsx and fill test data into respective columns
 *    5.4. Save the file to test-data/investment-module/XLSX-Audience-copy1.xlsx
 *
 * Steps to reproduce:
...
test.describe('CRM-2482_3.1.1.1.3 - Verify new contact/Opp created after importing audience via XLSX', () => {
  ...
  test('CRM-2482_3.1.1.1.3: Verify new contact/Opp created after importing new audience using XLSX file', async ({ page }, testInfo) => {
    ...
    // ---- Step 5: Create an Import Audience XLSX file ----
    let audienceData: Awaited<ReturnType<typeof investmentPage.createImportAudienceXLSXFile>>;

    await test.step('Step 5: Create an Import Audience XLSX file (XLSX-Audience-copy1.xlsx)', async () => {
      console.log('=== STEP 5: CREATE IMPORT AUDIENCE XLSX FILE ===');

      audienceData = await investmentPage.createImportAudienceXLSXFile();

      expect(fs.existsSync(audienceData.outputPath), `Output XLSX was not created at: ${audienceData.outputPath}`).toBeTruthy();

      console.log('  ✓ XLSX file created and verified — file exists on disk');
    ...
    // Step 3–5: Click "UPLOAD YOUR FILE" button and upload the XLSX
    await test.step('Step 3-5: Click "UPLOAD YOUR FILE", navigate to folder and select XLSX-Audience-copy1', async () => {
      console.log('=== STEP 3-5: UPLOAD XLSX FILE ===');

      await investmentPage.uploadAudienceCSV(audienceData.outputPath);

      console.log(`✓ XLSX file uploaded: ${audienceData.outputPath}`);
    });
    ...
    // Tear down Step 2: Delete XLSX file
    await test.step('Tear down Step 2: Delete XLSX file XLSX-Audience-copy1.xlsx', async () => {
      console.log('Tear down Step 2: Deleting XLSX file');

      if (fs.existsSync(audienceData.outputPath)) {
        fs.unlinkSync(audienceData.outputPath);
        expect(fs.existsSync(audienceData.outputPath), `XLSX file was not deleted: ${audienceData.outputPath}`).toBeFalsy();
        console.log(`✓ XLSX file deleted: ${audienceData.outputPath}`);
      } else {
        console.log(`  ℹ️ XLSX file not found (already deleted or never created): ${audienceData.outputPath}`);
      }
    });
