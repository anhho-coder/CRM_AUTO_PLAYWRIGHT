 ...
 * Test Case ID: CRM-2482_2.1.1.5
 * 
 * Summary: Verify the main fields of a Investment record includes "JIRA ID" field
 * 
 * Command to run:
 * npx playwright test "tests/1.Project_CRM/4.Investments/CRM-2482_Investments_module-General_fields/2.Investment_record/2.1.Main_section/2.1.1.List_of_fields/tc-crm-2482-2-1-1-5-Verify-JIRAID-field.spec.ts" --project=chromium
 * 
 * Pre-condition:
 ...
 * 2. Verify "JIRA ID" field is a text field
 */

test.describe('CRM-2482_2.1.1.5 - Verify JIRA ID Field', () => {
  ...
  test('CRM-2482_2.1.1.5: Verify the main fields of a Investment record includes "JIRA ID" field', async ({ page }, testInfo) => {
    ...
    // Step 4: Verify JIRA ID field
    await test.step('Step 4: Verify "JIRA ID" field is a text field', async () => {
      console.log('Step 4: Verifying "JIRA ID" field');
      
      const isTextFieldValid = await investmentPage_MainSection.verifyJiraIDField();
      expect(isTextFieldValid).toBeTruthy();
      
      console.log('  ✓ "JIRA ID" field exists');
      console.log('  ✓ "JIRA ID" field is a text input');
      console.log('✓ JIRA ID field verified successfully');
    });
