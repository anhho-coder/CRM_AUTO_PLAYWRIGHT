 ...
 * Test Case ID: CRM-2482_2.2.9.3
 *
 * Summary: Verify the Quotes tab of an Investment record includes "Quotation date" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.3" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Quotation date" row = current date
 *
...
test.describe('CRM-2482_2.2.9.3 - Verify the Quotes tab of an Investment record includes "Quotation date" field', () => {
  ...
  test('CRM-2482_2.2.9.3: Verify the Quotes tab of an Investment record includes "Quotation date" field', async ({ page }, testInfo) => {
    ...
    // Variable to hold Investment URL for verification
    let investmentUrl = '';
    ...
    // Step 3: Verify Quotation date = current date
    await test.step('Step 3: Verify "Quotation date" row = current date', async () => {
      console.log('=== STEP 3: VERIFY QUOTATION DATE ===');

      const rawValue = await investmentPage.getQuotesFirstRowCellTextOrEmpty('Quotation date');
      const actualValue = rawValue.split(' ')[0]; // strip time portion if present (e.g. "03/18/2026 10:30:00" → "03/18/2026")
      console.log(`  3.1 Quotation date actual  : "${actualValue}" (raw: "${rawValue}")`);
      console.log(`  3.1 Quotation date expected: "${currentDate}"`);

      expect(actualValue, `Step 3.1: "Quotation date" in Quotes tab should be current date "${currentDate}"`).toBe(currentDate);

      console.log(`  ✓ 3.1 Quotation date = ${currentDate} — verified`);
    });
