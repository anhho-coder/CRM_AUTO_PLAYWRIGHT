 ...
 * Test Case ID: CRM-2482_2.2.9.7
 *
 * Summary: Verify the Quotes tab of an Investment record includes "Total in Company Currency" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.7" --project=chromium
 *
 * Pre-condition:
 ...
 * 11. Get the value of "Total in Company Currency" called Quotation_Total_Company_Currency_Contact#1
 * 12. Press "CONFIRM" button and wait to create a Sales Order
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Quotes" tab
 * 3. Verify the following:
 *    3.1. Value at "Total in Company Currency" row = value of Quotation_Total_Company_Currency_Contact#1
 *
...
test.describe('CRM-2482_2.2.9.7 - Verify the Quotes tab of an Investment record includes "Total in Company Currency" field', () => {
  ...
  test('CRM-2482_2.2.9.7: Verify the Quotes tab of an Investment record includes "Total in Company Currency" field', async ({ page }, testInfo) => {
    ...
    // Variables to hold Investment URL and Total in Company Currency for verification
    let investmentUrl = '';
    ...
    // Pre-condition II.11: Get Total in Company Currency (Quotation_Total_Company_Currency_Contact#1)
    await test.step('Pre-condition II.11: Get the value of "Total in Company Currency" (Quotation_Total_Company_Currency_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.11: GET TOTAL IN COMPANY CURRENCY ===');

      await page.waitForTimeout(CommonUtils.waitTimes.extraLong);
      totalInCompanyCurrency = await quotationPage.getTotalInCompanyCurrency();
      console.log(`  ✓ Quotation_Total_Company_Currency_Contact#1 : ${totalInCompanyCurrency}`);
    });

    // Pre-condition II.12: Press "CONFIRM" button to create Sales Order
    await test.step('Pre-condition II.12: Press "CONFIRM" button and wait to create a Sales Order', async () => {
      console.log('=== PRE-CONDITION II.12: CONFIRM QUOTATION ===');
    ...
    // Step 3: Verify Total in Company Currency = Quotation_Total_Company_Currency_Contact#1
    await test.step('Step 3: Verify "Total in Company Currency" row = value of Quotation_Total_Company_Currency_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY TOTAL IN COMPANY CURRENCY ===');

      const actualValue = await investmentPage.getQuotesFirstRowCellText('Total in Company Currency');
      console.log(`  3.1 Total in Company Currency actual  : "${actualValue}"`);
      console.log(`  3.1 Total in Company Currency expected: "${totalInCompanyCurrency}" (Quotation_Total_Company_Currency_Contact#1)`);

      expect(actualValue, `Step 3.1: "Total in Company Currency" in Quotes tab should match Quotation_Total_Company_Currency_Contact#1`).toBe(totalInCompanyCurrency);

      console.log(`  ✓ 3.1 Total in Company Currency = ${totalInCompanyCurrency} — verified`);
    });
