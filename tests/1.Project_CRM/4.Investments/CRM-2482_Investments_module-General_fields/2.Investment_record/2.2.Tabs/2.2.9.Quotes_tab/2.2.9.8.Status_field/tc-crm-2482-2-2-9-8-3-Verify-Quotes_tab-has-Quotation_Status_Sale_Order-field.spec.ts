 ...
 * Test Case ID: CRM-2482_2.2.9.8.3
 *
 * Summary: Verify the Quotes tab of an Investment record includes "Status" field = Sale Order
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.8\.3" --project=chromium
 *
 * Pre-condition:
 ...
 * 12. Get the status of Quotation called Quotation_Status_Contact#1
 *
 * Steps to reproduce:
...
test.describe('CRM-2482_2.2.9.8.3 - Verify the Quotes tab of an Investment record includes "Status" field = Sale Order', () => {
  ...
  test('CRM-2482_2.2.9.8.3: Verify the Quotes tab of an Investment record includes "Status" field = Sale Order', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition II.11: Press "CONFIRM" button and wait to create a Sales Order', async () => {
      ...
      //await quotationPage.waitForLockButtonToAppear();
      console.log('✓ CONFIRM clicked — Sales Order created');
    });

    // Pre-condition II.12: Get Quotation Status (Quotation_Status_Contact#1)
    await test.step('Pre-condition II.12: Get the status of Quotation (Quotation_Status_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.12: GET QUOTATION STATUS ===');
