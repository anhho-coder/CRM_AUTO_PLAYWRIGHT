...
import { LoginPage, HomePage, InvestmentPage, OpportunityPage, DealElementPage, QuotationPage } from '@pages';
 ...
 * Test Case ID: CRM-2482_2.2.9.8.2
 *
 * Summary: Verify the Quotes tab of an Investment record includes "Status" field = Quotation Sent
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.8\.2" --project=chromium
 *
 * Pre-condition:
 ...
 * 11. Press "SEND BY EMAIL" button
 * 12. Once the Email form window appears completely, press "SEND" button
 * 13. Get the status of Quotation called Quotation_Status_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Quotes" tab
 * 3. Verify the following:
 *    3.1. Value at "Status" row = value of Quotation_Status_Contact#1
 *
...
test.describe('CRM-2482_2.2.9.8.2 - Verify the Quotes tab of an Investment record includes "Status" field = Quotation Sent', () => {
  ...
  test('CRM-2482_2.2.9.8.2: Verify the Quotes tab of an Investment record includes "Status" field = Quotation Sent', async ({ page }, testInfo) => {
    ...
    // Variables to hold Investment URL and Quotation Status for verification
    let investmentUrl = '';
    let quotationStatus = '';
    ...
    // Pre-condition II.11: Press "SEND BY EMAIL" button
    await test.step('Pre-condition II.11: Press "SEND BY EMAIL" button', async () => {
      console.log('=== PRE-CONDITION II.11: CLICK SEND BY EMAIL ===');

      await page.waitForTimeout(CommonUtils.waitTimes.extraLong);
      await quotationPage.clickSendByEmail();

      console.log('✓ "SEND BY EMAIL" button clicked — email dialog opened');
    });

    // Pre-condition II.12: Press "SEND" button in email dialog
    await test.step('Pre-condition II.12: Press "SEND" button in email dialog', async () => {
      console.log('=== PRE-CONDITION II.12: CLICK SEND IN EMAIL DIALOG ===');

      await quotationPage.sendEmail();
      //Quoc Anh: wait to get the Status of Quotation. 
      await quotationPage.waitForConfirmButtonToDisappear();
      //await page.waitForTimeout(CommonUtils.waitTimes.extraLong);
      console.log('✓ Email sent — Quotation status updated to Quotation Sent');
    });

    // Pre-condition II.13: Get Quotation Status (Quotation_Status_Contact#1)
    await test.step('Pre-condition II.13: Get the status of Quotation (Quotation_Status_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.13: GET QUOTATION STATUS ===');

      await page.waitForTimeout(CommonUtils.waitTimes.long);
      quotationStatus = await quotationPage.getQuotationStatus();
      // Quoc Anh: Additional wait to ensure status is updated to Quotation Sent before we proceed to verification in Quotes tab, as sometimes there is a delay in status update after sending email
      await page.waitForTimeout(CommonUtils.waitTimes.long);
      console.log(`  ✓ Quotation_Status_Contact#1 : ${quotationStatus}`);
    });
    ...
    await test.step('Step 2: Click at "Quotes" tab', async () => {
      console.log('=== STEP 2: CLICK QUOTES TAB ===');
    ...
    // Step 3: Verify Status = Quotation_Status_Contact#1
    await test.step('Step 3: Verify "Status" row = value of Quotation_Status_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY STATUS VALUE ===');
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        actualValue = await investmentPage.getQuotesFirstRowCellText('Status');
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Status actual  : "${actualValue}"`);
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Status expected: "${quotationStatus}" (Quotation_Status_Contact#1)`);

        if (actualValue === quotationStatus) break;
      ...
      expect(actualValue, 'Step 3.1: "Status" in Quotes tab should match Quotation_Status_Contact#1').toBe(quotationStatus);

      console.log(`  ✓ 3.1 Status = ${quotationStatus} — verified`);
    });
