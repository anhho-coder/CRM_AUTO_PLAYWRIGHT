 ...
 * Test Case ID: CRM-2482_2.2.9.8.1
 *
 * Summary: Verify the Quotes tab of an Investment record includes "Status" field = Quotation
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.8\.1" --project=chromium
 *
 * Pre-condition:
 ...
 * 11. Get the status of Quotation called Quotation_Status_Contact#1
 *
 * Steps to reproduce:
...
test.describe('CRM-2482_2.2.9.8.1 - Verify the Quotes tab of an Investment record includes "Status" field = Quotation', () => {
  ...
  test('CRM-2482_2.2.9.8.1: Verify the Quotes tab of an Investment record includes "Status" field = Quotation', async ({ page }, testInfo) => {
    ...
    // Pre-condition II.11: Get Quotation Status (Quotation_Status_Contact#1)
    await test.step('Pre-condition II.11: Get the status of Quotation (Quotation_Status_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.11: GET QUOTATION STATUS ===');
    ...
    await test.step('Step 3: Verify "Status" row = value of Quotation_Status_Contact#1', async () => {
      ...
      const actualValue = await investmentPage.getQuotesFirstRowCellText('Status');
      console.log(`  3.1 Status actual  : "${actualValue}"`);
      console.log(`  3.1 Status expected: "${quotationStatus}" (Quotation_Status_Contact#1)`);
