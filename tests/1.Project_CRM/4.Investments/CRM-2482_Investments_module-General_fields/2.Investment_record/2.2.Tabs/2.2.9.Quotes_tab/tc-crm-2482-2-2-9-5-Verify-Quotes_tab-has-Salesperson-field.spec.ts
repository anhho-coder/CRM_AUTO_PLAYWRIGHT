 ...
 * Test Case ID: CRM-2482_2.2.9.5
 *
 * Summary: Verify the Quotes tab of an Investment record includes "Salesperson" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.5" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Salesperson" row = createdByName (from User file)
 *
...
test.describe('CRM-2482_2.2.9.5 - Verify the Quotes tab of an Investment record includes "Salesperson" field', () => {
  ...
  test('CRM-2482_2.2.9.5: Verify the Quotes tab of an Investment record includes "Salesperson" field', async ({ page }, testInfo) => {
    ...
    // Variables to hold Investment URL for verification
    let investmentUrl = '';
    ...
    // Step 3: Verify Salesperson = createdByName
    await test.step('Step 3: Verify "Salesperson" row = createdByName (from User file)', async () => {
      console.log('=== STEP 3: VERIFY SALESPERSON ===');

      const expectedValue = users.admin_crm.createdByName;
      const actualValue = await investmentPage.getQuotesFirstRowCellTextOrEmpty('Salesperson');
      console.log(`  3.1 Salesperson actual  : "${actualValue}"`);
      console.log(`  3.1 Salesperson expected: "${expectedValue}" (createdByName)`);

      expect(actualValue, `Step 3.1: "Salesperson" in Quotes tab should be "${expectedValue}"`).toBe(expectedValue);

      console.log(`  ✓ 3.1 Salesperson = ${expectedValue} — verified`);
    });
