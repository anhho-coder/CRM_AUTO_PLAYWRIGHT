 ...
 * Test Case ID: CRM-2482_2.2.9.6
 *
 * Summary: Verify the Quotes tab of an Investment record includes "Sales Team" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.6" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Sales Team" row = BLANK
 *
...
test.describe('CRM-2482_2.2.9.6 - Verify the Quotes tab of an Investment record includes "Sales Team" field', () => {
  ...
  test('CRM-2482_2.2.9.6: Verify the Quotes tab of an Investment record includes "Sales Team" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Sales Team = BLANK
    await test.step('Step 3: Verify "Sales Team" row = BLANK', async () => {
      console.log('=== STEP 3: VERIFY SALES TEAM ===');

      const actualValue = await investmentPage.getQuotesFirstRowCellTextOrEmpty('Sales Team');
      console.log(`  3.1 Sales Team actual  : "${actualValue}"`);
      console.log(`  3.1 Sales Team expected: "" (BLANK)`);

      expect(actualValue, 'Step 3.1: "Sales Team" in Quotes tab should be BLANK').toBe('');

      console.log('  ✓ 3.1 Sales Team = BLANK — verified');
    });
