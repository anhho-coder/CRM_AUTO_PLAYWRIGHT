 ...
 * Test Case ID: CRM-2482_2.2.9.1
 *
 * Summary: Verify the Quotes tab of a Investment record includes "Quotation Number" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.1" --project=chromium
 *
 * Pre-condition:
 ...
 * 12. Get the value of "Quotation Number" called Quotation_Number_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Quotes" tab
 * 3. Verify the following:
 *    3.1. Value at "Quotation Number" row = value of Quotation_Number_Contact#1
 *
...
test.describe('CRM-2482_2.2.9.1 - Verify the Quotes tab of an Investment record includes "Quotation Number" field', () => {
  ...
  test('CRM-2482_2.2.9.1: Verify the Quotes tab of an Investment record includes "Quotation Number" field', async ({ page }, testInfo) => {
    ...
    // Variables to hold Investment URL and Quotation Number for verification
    let investmentUrl = '';
    let quotationNumber = '';
    ...
    // Pre-condition II.11: Press "CONFIRM" button to create Sales Order
    await test.step('Pre-condition II.11: Press "CONFIRM" button and wait to create a Sales Order', async () => {
      console.log('=== PRE-CONDITION II.11: CONFIRM QUOTATION ===');
    ...
    // Pre-condition II.12: Get Quotation Number (Quotation_Number_Contact#1)
    await test.step('Pre-condition II.12: Get the value of "Quotation Number" (Quotation_Number_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.12: GET QUOTATION NUMBER ===');

      quotationNumber = await quotationPage.getSalesOrderNumber();
      console.log(`  ✓ Quotation_Number_Contact#1 : ${quotationNumber}`);
    });
    ...
    // Step 3: Verify Quotation Number value matches Quotation_Number_Contact#1
    await test.step('Step 3: Verify "Quotation Number" row = value of Quotation_Number_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY QUOTATION NUMBER VALUE ===');

      const actualQuotationNumber = await investmentPage.getQuotesFirstRowCellText('Quotation Number');
      console.log(`  3.1 Quotation Number actual  : "${actualQuotationNumber}"`);
      console.log(`  3.1 Quotation Number expected: "${quotationNumber}" (Quotation_Number_Contact#1)`);

      expect(actualQuotationNumber, 'Step 3.1: Quotation Number in Quotes tab should match Quotation_Number_Contact#1').toBe(quotationNumber);

      console.log('  ✓ 3.1 Quotation Number matches Quotation_Number_Contact#1 — verified');
    });
