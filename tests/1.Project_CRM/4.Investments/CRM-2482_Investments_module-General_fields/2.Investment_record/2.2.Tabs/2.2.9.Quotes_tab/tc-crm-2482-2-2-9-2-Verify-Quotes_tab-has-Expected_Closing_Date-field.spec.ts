 ...
 * Test Case ID: CRM-2482_2.2.9.2
 *
 * Summary: Verify the Quotes tab of an Investment record includes "Expected closing date" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.2" --project=chromium
 *
 * Pre-condition:
...
test.describe('CRM-2482_2.2.9.2 - Verify the Quotes tab of an Investment record includes "Expected closing date" field', () => {
  ...
  test('CRM-2482_2.2.9.2: Verify the Quotes tab of an Investment record includes "Expected closing date" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Expected closing date = BLANK
    await test.step('Step 3: Verify "Expected closing date" row = BLANK', async () => {
      console.log('=== STEP 3: VERIFY EXPECTED CLOSING DATE ===');

      const actualValue = await investmentPage.getQuotesFirstRowCellTextOrEmpty('Expected closing date');
      console.log(`  3.1 Expected closing date actual  : "${actualValue}"`);
      console.log(`  3.1 Expected closing date expected: "" (BLANK)`);

      expect(actualValue, 'Step 3.1: "Expected closing date" in Quotes tab should be blank').toBe('');

      console.log('  ✓ 3.1 Expected closing date = BLANK — verified');
    });
