 ...
 * Test Case ID: CRM-2482_2.2.9.4
 *
 * Summary: Verify the Quotes tab of an Investment record includes "Payer" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.9\.4" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Payer" row = value of "Company_Name_Contact#1"
 *
...
test.describe('CRM-2482_2.2.9.4 - Verify the Quotes tab of an Investment record includes "Payer" field', () => {
  ...
  test('CRM-2482_2.2.9.4: Verify the Quotes tab of an Investment record includes "Payer" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Payer = Company_Name_Contact#1
    await test.step('Step 3: Verify "Payer" row = value of Company_Name_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY PAYER ===');

      const actualValue = await investmentPage.getQuotesFirstRowCellTextOrEmpty('Payer');
      const expectedValue = audienceData.companyName;
      console.log(`  3.1 Payer actual  : "${actualValue}"`);
      console.log(`  3.1 Payer expected: "${expectedValue}" (Company_Name_Contact#1)`);

      expect(actualValue, `Step 3.1: "Payer" in Quotes tab should be "${expectedValue}" (Company_Name_Contact#1)`).toBe(expectedValue);

      console.log(`  ✓ 3.1 Payer = ${expectedValue} — verified`);
    });
