 ...
 * Test Case ID: CRM-2482_2.2.8.1
 *
 * Summary: Verify the Lead tab of a Investment record includes "Expected Closing" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.1" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Expected Closing" row = BLANK
 *
...
test.describe('CRM-2482_2.2.8.1 - Verify the Lead tab of a Investment record includes "Expected Closing" field', () => {
  ...
  test('CRM-2482_2.2.8.1: Verify the Lead tab of a Investment record includes "Expected Closing" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Expected Closing = BLANK
    await test.step('Step 3: Verify "Expected Closing" value in Leads tab is blank', async () => {
      console.log('=== STEP 3: VERIFY EXPECTED CLOSING ===');

      const expectedClosingText = await investmentPage.getLeadsFirstRowCellText('Expected Closing');
      console.log(`  3.1 Expected Closing actual  : "${expectedClosingText}"`);
      console.log(`  3.1 Expected Closing expected: "" (BLANK)`);
      expect(expectedClosingText.trim(), 'Step 3.1: Expected Closing should be blank').toBe('');
      console.log('  ✓ 3.1 Expected Closing is blank — verified');
    });
