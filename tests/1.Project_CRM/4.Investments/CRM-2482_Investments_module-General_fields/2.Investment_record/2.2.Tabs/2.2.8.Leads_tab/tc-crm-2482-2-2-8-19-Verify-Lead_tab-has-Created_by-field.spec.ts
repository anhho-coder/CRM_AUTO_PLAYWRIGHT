 ...
 * Test Case ID: CRM-2482_2.2.8.19
 *
 * Summary: Verify the Lead tab of a Investment record includes "Created by" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.19" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Created by" row = createdByName (from User file)
 *
...
test.describe('CRM-2482_2.2.8.19 - Verify the Lead tab of a Investment record includes "Created by" field', () => {
  ...
  test('CRM-2482_2.2.8.19: Verify the Lead tab of a Investment record includes "Created by" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Created by = createdByName
    await test.step('Step 3: Verify "Created by" value in Leads tab equals createdByName', async () => {
      console.log('=== STEP 3: VERIFY CREATED BY ===');

      const createdByText = await investmentPage.getLeadsFirstRowCellText('Created by');
      console.log(`  3.1 Created by actual  : "${createdByText}"`);
      console.log(`  3.1 Created by expected: "${users.admin_crm.createdByName}"`);
      expect(createdByText, 'Step 3.1: Created by should equal createdByName').toBe(users.admin_crm.createdByName);
      console.log('  ✓ 3.1 Created by verified');
    });
