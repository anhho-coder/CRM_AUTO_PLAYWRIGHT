 ...
 * Test Case ID: CRM-2482_2.2.8.6
 *
 * Summary: Verify the Lead tab of a Investment record includes "Customer" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.6" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Customer" row = Company_Contact#1
 *
...
test.describe('CRM-2482_2.2.8.6 - Verify the Lead tab of a Investment record includes "Customer" field', () => {
  ...
  test('CRM-2482_2.2.8.6: Verify the Lead tab of a Investment record includes "Customer" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Customer = Company_Contact#1
    await test.step('Step 3: Verify "Customer" value in Leads tab equals Company_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY CUSTOMER ===');

      const customerText = await investmentPage.getLeadsFirstRowCellText('Customer');
      console.log(`  3.1 Customer actual  : "${customerText}"`);
      console.log(`  3.1 Customer expected: "${audienceData.companyName}"`);
      expect(customerText, 'Step 3.1: Customer should equal Company_Contact#1').toBe(audienceData.companyName);
      console.log('  ✓ 3.1 Customer verified');
    });
