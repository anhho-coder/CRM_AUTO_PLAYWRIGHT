 ...
 * Test Case ID: CRM-2482_2.2.8.18
 *
 * Summary: Verify the Lead tab of a Investment record includes "Created on" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.18" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Created on" row = Current Date
 *
...
test.describe('CRM-2482_2.2.8.18 - Verify the Lead tab of a Investment record includes "Created on" field', () => {
  ...
  test('CRM-2482_2.2.8.18: Verify the Lead tab of a Investment record includes "Created on" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Created on = Current Date
    await test.step('Step 3: Verify "Created on" value in Leads tab equals current date', async () => {
      console.log('=== STEP 3: VERIFY CREATED ON ===');

      const createdOnText = await investmentPage.getLeadsFirstRowCellText('Created on');
      const createdOnDate = createdOnText.trim().split(' ')[0]; // extract date part only (MM/DD/YYYY)
      console.log(`  3.1 Created on actual  : "${createdOnText}"`);
      console.log(`  3.1 Created on date    : "${createdOnDate}"`);
      console.log(`  3.1 Created on expected: "${currentDate}"`);
      expect(createdOnDate, 'Step 3.1: Created on should equal current date').toBe(currentDate);
      console.log('  ✓ 3.1 Created on verified');
    });
