 ...
 * Test Case ID: CRM-2482_2.2.8.16
 *
 * Summary: Verify the Lead tab of a Investment record includes "Closed Date" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.16" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Closed Date" row = BLANK
 *
...
test.describe('CRM-2482_2.2.8.16 - Verify the Lead tab of a Investment record includes "Closed Date" field', () => {
  ...
  test('CRM-2482_2.2.8.16: Verify the Lead tab of a Investment record includes "Closed Date" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Closed Date = BLANK
    await test.step('Step 3: Verify "Closed Date" value in Leads tab is BLANK', async () => {
      console.log('=== STEP 3: VERIFY CLOSED DATE ===');

      const closedDateText = await investmentPage.getLeadsFirstRowCellText('Closed Date');
      console.log(`  3.1 Closed Date actual  : "${closedDateText}"`);
      console.log(`  3.1 Closed Date expected: "" (BLANK)`);
      expect(closedDateText.trim(), 'Step 3.1: Closed Date should be blank').toBe('');
      console.log('  ✓ 3.1 Closed Date verified (blank)');
    });
