 ...
 * Test Case ID: CRM-2482_2.2.8.2
 *
 * Summary: Verify the Lead tab of a Investment record includes "Opportunity" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.2" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Opportunity" row = Name_Contact#1
 *
...
test.describe('CRM-2482_2.2.8.2 - Verify the Lead tab of a Investment record includes "Opportunity" field', () => {
  ...
  test('CRM-2482_2.2.8.2: Verify the Lead tab of a Investment record includes "Opportunity" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Opportunity = Name_Contact#1
    await test.step('Step 3: Verify "Opportunity" value in Leads tab equals Name_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY OPPORTUNITY ===');

      const opportunityText = await investmentPage.getLeadsFirstRowCellText('Opportunity');
      console.log(`  3.1 Opportunity actual  : "${opportunityText}"`);
      console.log(`  3.1 Opportunity expected: "${audienceData.contactName}"`);
      expect(opportunityText, 'Step 3.1: Opportunity should equal Name_Contact#1').toBe(audienceData.contactName);
      console.log('  ✓ 3.1 Opportunity verified');
    });
