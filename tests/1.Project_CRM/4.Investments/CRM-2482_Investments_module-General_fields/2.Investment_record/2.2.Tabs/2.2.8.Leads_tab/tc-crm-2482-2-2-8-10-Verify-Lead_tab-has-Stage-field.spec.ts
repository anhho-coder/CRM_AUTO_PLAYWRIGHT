 ...
 * Test Case ID: CRM-2482_2.2.8.10
 *
 * Summary: Verify the Lead tab of a Investment record includes "Stage" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.10" --project=chromium
 *
 * Pre-condition:
...
test.describe('CRM-2482_2.2.8.10 - Verify the Lead tab of a Investment record includes "Stage" field', () => {
  ...
  test('CRM-2482_2.2.8.10: Verify the Lead tab of a Investment record includes "Stage" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Stage = "New"
    await test.step('Step 3: Verify "Stage" value in Leads tab equals "New"', async () => {
      console.log('=== STEP 3: VERIFY STAGE ===');

      const stageText = await investmentPage.getLeadsFirstRowCellText('Stage');
      console.log(`  3.1 Stage actual  : "${stageText}"`);
      console.log(`  3.1 Stage expected: "New"`);
      expect(stageText, 'Step 3.1: Stage should equal "New"').toBe('New');
      console.log('  ✓ 3.1 Stage verified');
    });
