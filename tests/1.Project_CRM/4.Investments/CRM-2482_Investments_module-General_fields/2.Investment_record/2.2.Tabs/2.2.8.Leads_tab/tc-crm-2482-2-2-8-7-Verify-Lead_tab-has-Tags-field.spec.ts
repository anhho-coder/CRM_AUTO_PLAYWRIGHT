 ...
 * Test Case ID: CRM-2482_2.2.8.7
 *
 * Summary: Verify the Lead tab of a Investment record includes "Tags" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.7" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Tags" row = Tags_Contact#1
 *
...
test.describe('CRM-2482_2.2.8.7 - Verify the Lead tab of a Investment record includes "Tags" field', () => {
  ...
  test('CRM-2482_2.2.8.7: Verify the Lead tab of a Investment record includes "Tags" field', async ({ page }, testInfo) => {
    ...
    await test.step('Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      console.log(`  ✓ Tags_Contact#1        : ${audienceData.tags}`);
    });
    ...
    // Step 3: Verify Tags = Tags_Contact#1
    await test.step('Step 3: Verify "Tags" value in Leads tab equals Tags_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY TAGS ===');

      const tagsText = await investmentPage.getLeadsFirstRowCellText('Tags');
      console.log(`  3.1 Tags actual  : "${tagsText}"`);
      console.log(`  3.1 Tags expected: "${audienceData.tags}"`);
      expect(tagsText, 'Step 3.1: Tags should equal Tags_Contact#1').toBe(audienceData.tags);
      console.log('  ✓ 3.1 Tags verified');
    });
