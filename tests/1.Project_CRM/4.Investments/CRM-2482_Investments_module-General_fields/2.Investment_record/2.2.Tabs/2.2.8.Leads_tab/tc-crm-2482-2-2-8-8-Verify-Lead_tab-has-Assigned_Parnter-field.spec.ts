 ...
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.8" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Assigned Partner" row = BLANK
 *
...
test.describe('CRM-2482_2.2.8.8 - Verify the Lead tab of a Investment record includes "Assigned Partner" field', () => {
  ...
  test('CRM-2482_2.2.8.8: Verify the Lead tab of a Investment record includes "Assigned Partner" field', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Verify "Assigned Partner" value in Leads tab is blank', async () => {
      ...
      const assignedPartnerText = await investmentPage.getLeadsFirstRowCellText('Assigned Partner');
      console.log(`  3.1 Assigned Partner actual  : "${assignedPartnerText}"`);
      console.log(`  3.1 Assigned Partner expected: "" (BLANK)`);
      expect(assignedPartnerText.trim(), 'Step 3.1: Assigned Partner should be blank').toBe('');
      console.log('  ✓ 3.1 Assigned Partner is blank — verified');
    });
