 ...
 * Test Case ID: CRM-2482_2.2.8.3
 *
 * Summary: Verify the Lead tab of a Investment record includes "Country" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.3" --project=chromium
 *
 * Pre-condition:
 ...
 *         - Phone         = 1234125125
 *         - Country       = China                                           (Country_Contact#1)
 ...
 *    3.1. Value at "Country" row = Country_Contact#1
 *
...
test.describe('CRM-2482_2.2.8.3 - Verify the Lead tab of a Investment record includes "Country" field', () => {
  ...
  test('CRM-2482_2.2.8.3: Verify the Lead tab of a Investment record includes "Country" field', async ({ page }, testInfo) => {
    ...
    await test.step('Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      console.log(`  ✓ Country_Contact#1     : ${audienceData.country}`);
    ...
    // Step 3: Verify Country = Country_Contact#1
    await test.step('Step 3: Verify "Country" value in Leads tab equals Country_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY COUNTRY ===');

      const countryText = await investmentPage.getLeadsFirstRowCellText('Country');
      console.log(`  3.1 Country actual  : "${countryText}"`);
      console.log(`  3.1 Country expected: "${audienceData.country}"`);
      expect(countryText, 'Step 3.1: Country should equal Country_Contact#1').toBe(audienceData.country);
      console.log('  ✓ 3.1 Country verified');
    });
