 ...
 * Test Case ID: CRM-2482_2.2.8.4
 *
 * Summary: Verify the Lead tab of a Investment record includes "Phone" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.4" --project=chromium
 *
 * Pre-condition:
 ...
 *         - Phone         = 1234125125                                    (Phone_Contact#1)
 *         - Country       = China
 ...
 *    3.1. Value at "Phone" row = Phone_Contact#1
 *
...
test.describe('CRM-2482_2.2.8.4 - Verify the Lead tab of a Investment record includes "Phone" field', () => {
  ...
  test('CRM-2482_2.2.8.4: Verify the Lead tab of a Investment record includes "Phone" field', async ({ page }, testInfo) => {
    ...
    await test.step('Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      console.log(`  ✓ Phone_Contact#1       : ${audienceData.phone}`);
    ...
    // Step 3: Verify Phone = Phone_Contact#1
    await test.step('Step 3: Verify "Phone" value in Leads tab equals Phone_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY PHONE ===');

      const phoneText = await investmentPage.getLeadsFirstRowCellText('Phone');
      console.log(`  3.1 Phone actual  : "${phoneText}"`);
      console.log(`  3.1 Phone expected: "${audienceData.phone}"`);
      expect(phoneText, 'Step 3.1: Phone should equal Phone_Contact#1').toBe(audienceData.phone);
      console.log('  ✓ 3.1 Phone verified');
    });
