 ...
 * Test Case ID: CRM-2482_2.2.8.5
 *
 * Summary: Verify the Lead tab of a Investment record includes "Email" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.5" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Email" row = Email_Contact#1
 *
...
test.describe('CRM-2482_2.2.8.5 - Verify the Lead tab of a Investment record includes "Email" field', () => {
  ...
  test('CRM-2482_2.2.8.5: Verify the Lead tab of a Investment record includes "Email" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Email = Email_Contact#1
    await test.step('Step 3: Verify "Email" value in Leads tab equals Email_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY EMAIL ===');

      const emailText = await investmentPage.getLeadsFirstRowCellText('Email');
      console.log(`  3.1 Email actual  : "${emailText}"`);
      console.log(`  3.1 Email expected: "${audienceData.emailContact1}"`);
      expect(emailText, 'Step 3.1: Email should equal Email_Contact#1').toBe(audienceData.emailContact1);
      console.log('  ✓ 3.1 Email verified');
    });
