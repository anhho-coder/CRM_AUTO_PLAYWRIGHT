 ...
 * Test Case ID: CRM-2482_2.2.8.9
 *
 * Summary: Verify the Lead tab of a Investment record includes "Next Activity Deadline" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.8.9" --project=chromium
 *
 * Pre-condition:
 ...
 *    - Channel                  = Channel
 *    - Countries                = Albania
 *    - Date start               = current date                        (Date_start_Investment#1)
 ...
 *    3.1. Value at "Next Activity Deadline" row = Date_start_Investment#1 + 1 day
 *
...
test.describe('CRM-2482_2.2.8.9 - Verify the Lead tab of a Investment record includes "Next Activity Deadline" field', () => {
  ...
  test('CRM-2482_2.2.8.9: Verify the Lead tab of a Investment record includes "Next Activity Deadline" field', async ({ page }, testInfo) => {
    ...
    // Date_start_Investment#1 + 1 day
    const oneDayLater = new Date(now);
    oneDayLater.setDate(oneDayLater.getDate() + 1);
    const tdMM = String(oneDayLater.getMonth() + 1).padStart(2, '0');
    const tdDD = String(oneDayLater.getDate()).padStart(2, '0');
    const tdYYYY = oneDayLater.getFullYear();
    const dateStartPlus1Day = `${tdMM}/${tdDD}/${tdYYYY}`;
    ...
    // Step 3: Verify Next Activity Deadline = Date_start_Investment#1 + 1 day
    await test.step('Step 3: Verify "Next Activity Deadline" value in Leads tab equals Date_start_Investment#1 + 1 day', async () => {
      console.log('=== STEP 3: VERIFY NEXT ACTIVITY DEADLINE ===');

      const nextActivityText = await investmentPage.getLeadsFirstRowCellText('Next Activity Deadline');
      const nextActivityDate = nextActivityText.trim().split(' ')[0]; // extract date part only (MM/DD/YYYY)
      console.log(`  3.1 Next Activity Deadline actual  : "${nextActivityText}"`);
      console.log(`  3.1 Next Activity Deadline date    : "${nextActivityDate}"`);
      console.log(`  3.1 Next Activity Deadline expected: "${dateStartPlus1Day}" (Date_start_Investment#1 + 1 day)`);
      expect(nextActivityDate, 'Step 3.1: Next Activity Deadline should equal Date_start_Investment#1 + 1 day').toBe(dateStartPlus1Day);
      console.log('  ✓ 3.1 Next Activity Deadline verified');
    });
