 ...
 * Test Case ID: CRM-2482_2.2.4.1.8
 *
 * Summary: Verify the Audience tab of a Investment record includes "Invoice Paid" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.4.1.8" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Invoice Paid" row = Value of "0.00"
 *
...
test.describe('CRM-2482_2.2.4.1.8 - Verify the Audience tab of a Investment record includes "Invoice Paid" field', () => {
  ...
  test('CRM-2482_2.2.4.1.8: Verify the Audience tab of a Investment record includes "Invoice Paid" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Invoice Paid = 0.00
    await test.step('Step 3: Verify "Invoice Paid" value in Audience tab equals 0.00', async () => {
      console.log('=== STEP 3: VERIFY INVOICE PAID ===');

      const invoicePaidText = await investmentPage.getAudienceFirstRowCellText('Invoice Paid');
      console.log(`  3.1 Invoice Paid actual  : "${invoicePaidText}"`);
      console.log(`  3.1 Invoice Paid expected: "0.00"`);
      expect(invoicePaidText.trim(), 'Step 3.1: Invoice Paid should equal 0.00').toBe('0.00');
      console.log('  ✓ 3.1 Invoice Paid verified');
    });
