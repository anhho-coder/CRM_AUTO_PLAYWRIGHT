 ...
 * Test Case ID: CRM-2482_2.2.4.1.2
 *
 * Summary: Verify the Audience tab of a Investment record includes "Partner Email" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.4.1.2" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Partner Email" row = Email_Contact#1
 *
...
test.describe('CRM-2482_2.2.4.1.2 - Verify the Audience tab of a Investment record includes "Partner Email" field', () => {
  ...
  test('CRM-2482_2.2.4.1.2: Verify the Audience tab of a Investment record includes "Partner Email" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Partner Email = Email_Contact#1
    await test.step('Step 3: Verify "Partner Email" value in Audience tab equals Email_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY PARTNER EMAIL ===');

      const partnerEmailText = await investmentPage.getAudienceFirstRowCellText('Partner Email');
      console.log(`  3.1 Partner Email actual  : "${partnerEmailText}"`);
      console.log(`  3.1 Partner Email expected: "${audienceData.emailContact1}"`);
      expect(partnerEmailText, 'Step 3.1: Partner Email should equal Email_Contact#1').toBe(audienceData.emailContact1);
      console.log('  ✓ 3.1 Partner Email verified');
    });
