 ...
 * Test Case ID: CRM-2482_2.2.4.1.11
 *
 * Summary: Verify the Audience tab of a Investment record includes "Source" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.4.1.11" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Source" row = Value of "file"
 *
...
test.describe('CRM-2482_2.2.4.1.11 - Verify the Audience tab of a Investment record includes "Source" field', () => {
  ...
  test('CRM-2482_2.2.4.1.11: Verify the Audience tab of a Investment record includes "Source" field', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Verify "Source" value in Audience tab equals "file"', async () => {
      console.log('=== STEP 3: VERIFY SOURCE ===');

      const sourceText = await investmentPage.getAudienceFirstRowCellText('Source');
      console.log(`  3.1 Source actual  : "${sourceText}"`);
      console.log(`  3.1 Source expected: "file"`);
      expect(sourceText.trim(), 'Step 3.1: Source should equal file').toBe('file');
      console.log('  ✓ 3.1 Source verified');
    });
