...
/**
 * Investments - Audience Tab Verification Test
 * Test Case ID: CRM-2482_2.2.4.1.3
 *
 * Summary: Verify the Audience tab of a Investment record includes "Event lead type" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.4.1.3" --project=chromium
 *
 * Pre-condition:
 ...
 *         - Company       = TEST_company_name_<timestamp>              (Company_Contact#1)
 *         - Phone         = 1234125125
 *         - Country       = China
 *         - Sales Team    = CMR
 ...
 *    3.1. Value at "Event lead type" row = value of "Prospective end user"
 *
...
test.describe('CRM-2482_2.2.4.1.3 - Verify the Audience tab of a Investment record includes "Event lead type" field', () => {
  ...
  test('CRM-2482_2.2.4.1.3: Verify the Audience tab of a Investment record includes "Event lead type" field', async ({ page }, testInfo) => {
    ...
    await test.step('Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      console.log(`  ✓ Company_Contact#1 : ${audienceData.companyName}`);
    });
    ...
    // Step 3: Verify Event lead type = Prospective end user
    await test.step('Step 3: Verify "Event lead type" value in Audience tab equals Prospective end user', async () => {
      console.log('=== STEP 3: VERIFY EVENT LEAD TYPE ===');

      const eventLeadTypeText = await investmentPage.getAudienceFirstRowCellText('Event lead type');
      console.log(`  3.1 Event lead type actual  : "${eventLeadTypeText}"`);
      console.log(`  3.1 Event lead type expected: "Prospective end user"`);
      expect(eventLeadTypeText, 'Step 3.1: Event lead type should equal Prospective end user').toBe('Prospective end user');
      console.log('  ✓ 3.1 Event lead type verified');
    });
