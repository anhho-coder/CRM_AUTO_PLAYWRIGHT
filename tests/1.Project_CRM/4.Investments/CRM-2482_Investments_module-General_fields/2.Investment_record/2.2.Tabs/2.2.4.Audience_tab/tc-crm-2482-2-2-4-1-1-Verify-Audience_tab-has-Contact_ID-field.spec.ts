 ...
 * Test Case ID: CRM-2482_2.2.4.1.1
 *
 * Summary: Verify the Audience tab of a Investment record includes "Contact ID" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.4.1.1" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Contact ID" row contains value of Company_Contact#1
 *
...
test.describe('CRM-2482_2.2.4.1.1 - Verify the Audience tab of a Investment record includes "Contact ID" field', () => {
  ...
  test('CRM-2482_2.2.4.1.1: Verify the Audience tab of a Investment record includes "Contact ID" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Contact ID = Company_Contact#1
    await test.step('Step 3: Verify "Contact ID" value in Audience tab equals Company_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY CONTACT ID ===');

      const contactIDText = await investmentPage.getAudienceFirstRowCellText('Contact ID');
      console.log(`  3.1 Contact ID actual  : "${contactIDText}"`);
      console.log(`  3.1 Contact ID expected: "${audienceData.companyName}"`);
      expect(contactIDText, 'Step 3.1: Contact ID should contain Company_Contact#1').toContain(audienceData.companyName);
      console.log('  ✓ 3.1 Contact ID verified');
    });
