 ...
 * Test Case ID: CRM-2482_2.2.10.4
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Distributor" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.4" --project=chromium
 *
 * Pre-condition:
 ...
 * 19. Get the value of "Distributor" called Invoice_Distributor_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Invoice" tab
 * 3. Verify the following:
 *    3.1. Value at "Distributor" row = value of Invoice_Distributor_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.4 - Verify the Invoices tab of an Investment record includes "Distributor" field = Invoice_Distributor_Contact#1', () => {
  ...
  test('CRM-2482_2.2.10.4: Verify the Invoices tab of an Investment record includes "Distributor" field = Invoice_Distributor_Contact#1', async ({ page }, testInfo) => {
    ...
    // Pre-condition II.19: Get Distributor (Invoice_Distributor_Contact#1)
    await test.step('Pre-condition II.19: Get "Distributor" (Invoice_Distributor_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.19: GET DISTRIBUTOR ===');

      await page.waitForTimeout(CommonUtils.waitTimes.long);
      invoiceDistributor = await invoicePage.getDistributor();
      console.log(`  ✓ Invoice_Distributor_Contact#1 : ${invoiceDistributor}`);
    });
    ...
    // Step 3: Verify Distributor = Invoice_Distributor_Contact#1
    await test.step('Step 3: Verify "Distributor" row = value of Invoice_Distributor_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY DISTRIBUTOR VALUE ===');
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          actualValue = await investmentPage.getInvoicesFirstRowCellText('Distributor');
        } catch (error) {
          const msg = error instanceof Error ? error.message : String(error);
          if (msg.includes('column header "Distributor" not found')) {
            // Odoo may hide empty columns; treat missing header as blank value
            actualValue = '';
            console.log(`  ⚠ Attempt ${attempt}/${MAX_RETRIES} — "Distributor" column not visible; treating as BLANK`);
          } else {
            throw error;
          }
        }

        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Distributor actual  : "${actualValue}"`);
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Distributor expected: "${invoiceDistributor}" (Invoice_Distributor_Contact#1)`);

        if (actualValue === invoiceDistributor) {
          matched = true;
          break;
        }
      ...
      expect(matched, 'Step 3.1: Failed to match Distributor value after all retries').toBeTruthy();
      expect(actualValue, 'Step 3.1: "Distributor" in Invoices tab should match Invoice_Distributor_Contact#1').toBe(invoiceDistributor);

      console.log(`  ✓ 3.1 Distributor = ${invoiceDistributor} — verified`);
    });
