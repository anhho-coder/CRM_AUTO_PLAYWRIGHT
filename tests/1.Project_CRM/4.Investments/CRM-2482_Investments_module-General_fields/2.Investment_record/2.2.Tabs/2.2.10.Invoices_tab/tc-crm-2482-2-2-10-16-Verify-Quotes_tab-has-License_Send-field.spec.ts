 ...
 * Test Case ID: CRM-2482_2.2.10.16
 *
 * Summary: Verify the Invoices tab of an Investment record includes "License Send" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.16" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "License Send" row is UN-CHECKED
 *
...
test.describe('CRM-2482_2.2.10.16 - Verify the Invoices tab of an Investment record includes "License Send" field', () => {
  ...
  test('CRM-2482_2.2.10.16: Verify the Invoices tab of an Investment record includes "License Send" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify "License Send" checkbox is UN-CHECKED
    await test.step('Step 3: Verify "License Send" row is UN-CHECKED', async () => {
      console.log('=== STEP 3: VERIFY LICENSE SEND CHECKBOX STATE ===');

      const isChecked = await investmentPage.getInvoicesFirstRowCheckboxState('License Send');

      console.log(`  3.1 License Send checkbox state: ${isChecked ? 'CHECKED' : 'UN-CHECKED'}`);
      console.log(`  3.1 License Send expected      : UN-CHECKED (false)`);

      expect(isChecked, 'Step 3.1: "License Send" in Invoices tab should be UN-CHECKED').toBe(false);

      console.log(`  ✓ 3.1 License Send = UN-CHECKED — verified`);
    });
