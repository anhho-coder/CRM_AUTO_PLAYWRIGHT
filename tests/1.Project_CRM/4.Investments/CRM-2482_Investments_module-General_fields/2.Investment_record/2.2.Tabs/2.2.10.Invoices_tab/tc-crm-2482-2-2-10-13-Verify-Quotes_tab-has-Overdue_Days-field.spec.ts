 ...
 * Test Case ID: CRM-2482_2.2.10.13
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Overdue Days" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.13" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Overdue Days" row = 0
 *
...
test.describe('CRM-2482_2.2.10.13 - Verify the Invoices tab of an Investment record includes "Overdue Days" field = 0', () => {
  ...
  test('CRM-2482_2.2.10.13: Verify the Invoices tab of an Investment record includes "Overdue Days" field = 0', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify Overdue Days = 0
    await test.step('Step 3: Verify "Overdue Days" row = 0', async () => {
      console.log('=== STEP 3: VERIFY OVERDUE DAYS VALUE ===');
      console.log(`  3.1 Overdue Days expected: "${EXPECTED_OVERDUE_DAYS}"`);

      // Pass EXPECTED_OVERDUE_DAYS as expectedValue so the method retries/reloads only when
      // actual ≠ expected. If both are '' (blank), it returns immediately without any reload.
      const actualValue = await investmentPage.getInvoicesFirstRowCellText('Overdue Days', EXPECTED_OVERDUE_DAYS);

      console.log(`  3.1 Overdue Days actual  : "${actualValue}"`);

      expect(actualValue, 'Step 3.1: "Overdue Days" in Invoices tab should be 0').toBe(EXPECTED_OVERDUE_DAYS);

      console.log(`  ✓ 3.1 Overdue Days = "${EXPECTED_OVERDUE_DAYS}" — verified`);
    });
