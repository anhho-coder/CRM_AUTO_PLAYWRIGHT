 ...
 * Test Case ID: CRM-2482_2.2.10.2
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Source Document" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.2" --project=chromium
 *
 * Pre-condition:
 ...
 * 19. Get the value of "Invoice Number" called Invoice_Number_Contact#1
 * 20. Click at "Other Infor" tab and verify the following:
 *     20.1. Get the value of "Source Document" called Invoice_Source_Document_Contact#1
 *     20.2. Verify value of "Source Document" on page = value of "Sales Order Number"
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Invoices" tab
 * 3. Verify the following:
 *    3.1. Value at "Source Document" row = value of Invoice_Source_Document_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.2 - Verify the Invoices tab of an Investment record includes "Source Document" field = Invoice_Source_Document_Contact#1', () => {
  ...
  test('CRM-2482_2.2.10.2: Verify the Invoices tab of an Investment record includes "Source Document" field = Invoice_Source_Document_Contact#1', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition II.11: Press "CONFIRM" button and wait to create a Sales Order', async () => {
      ...
      saleOrderNumber = await quotationPage.getSalesOrderNumber();
      console.log(`  ✓ Sale_Order_Number_Contact#1 : ${saleOrderNumber}`);
    ...
    // Pre-condition II.19: Get Invoice Number (Invoice_Number_Contact#1)
    await test.step('Pre-condition II.19: Get "Invoice Number" (Invoice_Number_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.19: GET INVOICE NUMBER ===');

      await page.waitForTimeout(CommonUtils.waitTimes.long);
      invoiceNumber = await invoicePage.getInvoiceNumber();
      console.log(`  ✓ Invoice_Number_Contact#1 : ${invoiceNumber}`);
    });

    // Pre-condition II.20: Verify Source Document in Other Infor tab
    await test.step('Pre-condition II.20: Click "Other Infor" tab and verify Source Document = Sale_Order_Number_Contact#1', async () => {
      console.log('=== PRE-CONDITION II.20: VERIFY SOURCE DOCUMENT ON INVOICE ===');

      await invoicePage.clickOtherInforTab();
      invoiceSourceDocument = await invoicePage.getSourceDocument();
      console.log(`  ✓ Invoice_Source_Document_Contact#1 : ${invoiceSourceDocument}`);
      console.log(`  ✓ Sale_Order_Number_Contact#1       : ${saleOrderNumber}`);

      expect(invoiceSourceDocument, 'Pre-condition II.20.2: "Source Document" on Invoice should match Sale_Order_Number_Contact#1').toBe(saleOrderNumber);

      console.log(`  ✓ Source Document on Invoice = ${saleOrderNumber} — verified`);
    });
    ...
    // Step 2: Click "Invoices" tab
    await test.step('Step 2: Click at "Invoices" tab', async () => {
    ...
    // Step 3: Verify Source Document = Invoice_Source_Document_Contact#1
    await test.step('Step 3: Verify "Source Document" row = value of Invoice_Source_Document_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY SOURCE DOCUMENT VALUE ===');

      const MAX_RETRIES = 5;
      const RETRY_INTERVAL_MS = CommonUtils.waitTimes.extraLong;
      let actualValue = '';

      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        actualValue = await investmentPage.getInvoicesFirstRowCellText('Source Document');
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Source Document actual  : "${actualValue}"`);
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Source Document expected: "${invoiceSourceDocument}" (Invoice_Source_Document_Contact#1)`);

        if (actualValue === invoiceSourceDocument) break;
      ...
      expect(actualValue, 'Step 3.1: "Source Document" in Invoices tab should match Invoice_Source_Document_Contact#1').toBe(invoiceSourceDocument);

      console.log(`  ✓ 3.1 Source Document = ${invoiceSourceDocument} — verified`);
    });
