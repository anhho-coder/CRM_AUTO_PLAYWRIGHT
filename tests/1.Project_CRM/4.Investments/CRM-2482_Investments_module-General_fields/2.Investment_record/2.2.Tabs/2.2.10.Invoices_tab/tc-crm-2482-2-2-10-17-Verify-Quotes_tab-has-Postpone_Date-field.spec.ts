 ...
 * Test Case ID: CRM-2482_2.2.10.17
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Postpone Date" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.17" --project=chromium
 *
 * Pre-condition:
 ...
 *    3.1. Value at "Postpone Date" row = BLANK
 *
...
test.describe('CRM-2482_2.2.10.17 - Verify the Invoices tab of an Investment record includes "Postpone Date" field', () => {
  ...
  test('CRM-2482_2.2.10.17: Verify the Invoices tab of an Investment record includes "Postpone Date" field', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify "Postpone Date" = BLANK
    await test.step('Step 3: Verify "Postpone Date" row = BLANK', async () => {
      console.log('=== STEP 3: VERIFY POSTPONE DATE VALUE ===');
      console.log(`  3.1 Postpone Date expected: "" (BLANK)`);

      // Pass '' as expectedValue — method treats blank/not-found as '' without retrying
      const actualValue = await investmentPage.getInvoicesFirstRowCellText('Postpone Date', '');

      console.log(`  3.1 Postpone Date actual  : "${actualValue}"`);

      expect(actualValue, 'Step 3.1: "Postpone Date" in Invoices tab should be BLANK').toBe('');

      console.log(`  ✓ 3.1 Postpone Date = BLANK — verified`);
    });
