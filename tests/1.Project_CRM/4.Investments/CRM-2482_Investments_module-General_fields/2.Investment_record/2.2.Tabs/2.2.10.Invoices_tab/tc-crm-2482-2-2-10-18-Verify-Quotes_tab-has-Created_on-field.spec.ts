 ...
 * Test Case ID: CRM-2482_2.2.10.18
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Created on" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.18" --project=chromium
 *
 * Pre-condition:
 ...
 * 19. Get the value of "Invoice Date" called Invoice_Invoice_Date_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Invoice" tab
 * 3. Verify the following:
 *    3.1. Value at "Created on" row (date portion only) EQUALS value of Invoice_Invoice_Date_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.18 - Verify the Invoices tab of an Investment record includes "Created on" field', () => {
  ...
  test('CRM-2482_2.2.10.18: Verify the Invoices tab of an Investment record includes "Created on" field', async ({ page }, testInfo) => {
    ...
    let invoiceInvoiceDateContact1 = '';
    ...
    // Pre-condition II.19: Get "Invoice Date" (Invoice_Invoice_Date_Contact#1)
    await test.step('Pre-condition II.19: Get "Invoice Date" (Invoice_Invoice_Date_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.19: GET INVOICE DATE ===');

      await page.waitForTimeout(CommonUtils.waitTimes.long);
      invoiceInvoiceDateContact1 = await invoicePage.getInvoiceDate();

      console.log(`  ✓ Invoice_Invoice_Date_Contact#1 : "${invoiceInvoiceDateContact1}"`);
    });
    ...
    // Step 3: Verify "Created on" CONTAINS Invoice_Invoice_Date_Contact#1
    await test.step('Step 3: Verify "Created on" row CONTAINS value of Invoice_Invoice_Date_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY CREATED ON VALUE ===');
      console.log(`  3.1 Created on expected (contains): "${invoiceInvoiceDateContact1}" (Invoice_Invoice_Date_Contact#1)`);

      const rawValue = await investmentPage.getInvoicesFirstRowCellText('Created on');
      // "Created on" cell contains a full datetime — extract the date portion only (MM/DD/YYYY)
      const actualValue = rawValue.split(' ')[0];

      console.log(`  3.1 Created on raw     : "${rawValue}"`);
      console.log(`  3.1 Created on actual  : "${actualValue}" (date only)`);

      expect(actualValue, 'Step 3.1: "Created on" in Invoices tab should equal Invoice_Invoice_Date_Contact#1').toBe(invoiceInvoiceDateContact1);

      console.log(`  ✓ 3.1 Created on equals "${invoiceInvoiceDateContact1}" — verified`);
    });
