 ...
 * Test Case ID: CRM-2482_2.2.10.19
 *
 * Summary: Verify the Invoices tab of an Investment record includes "License State" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.19" --project=chromium
 *
 * Pre-condition:
...
test.describe('CRM-2482_2.2.10.19 - Verify the Invoices tab of an Investment record includes "License State" field', () => {
  ...
  test('CRM-2482_2.2.10.19: Verify the Invoices tab of an Investment record includes "License State" field', async ({ page }, testInfo) => {
    ...
    let invoiceActuallyReceived = ''; // kept for Pre-condition II.19 (unchanged)
    ...
    // Step 3: Verify "License State" = BLANK
    await test.step('Step 3: Verify "License State" row = BLANK', async () => {
      console.log('=== STEP 3: VERIFY LICENSE STATE VALUE ===');
      console.log(`  3.1 License State expected: "" (BLANK)`);

      // Pass '' as expectedValue — method treats blank/not-found as '' without retrying
      const actualValue = await investmentPage.getInvoicesFirstRowCellText('License State', '');

      console.log(`  3.1 License State actual  : "${actualValue}"`);

      expect(actualValue, 'Step 3.1: "License State" in Invoices tab should be BLANK').toBe('');

      console.log(`  ✓ 3.1 License State = BLANK — verified`);
    });
