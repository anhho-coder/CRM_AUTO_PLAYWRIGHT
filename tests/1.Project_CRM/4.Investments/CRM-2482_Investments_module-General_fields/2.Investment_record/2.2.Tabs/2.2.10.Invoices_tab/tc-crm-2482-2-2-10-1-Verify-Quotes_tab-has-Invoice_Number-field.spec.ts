 ...
 * Test Case ID: CRM-2482_2.2.10.1
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Number" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.1:" --project=chromium
 * 
 * Pre-condition:
 ...
 *    3.1. Value at "Number" row = value of Invoice_Number_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.1 - Verify the Invoices tab of an Investment record includes "Number" field = Invoice_Number_Contact#1', () => {
  ...
  test('CRM-2482_2.2.10.1: Verify the Invoices tab of an Investment record includes "Number" field = Invoice_Number_Contact#1', async ({ page }, testInfo) => {
    ...
    // Variables to hold Investment URL, Payment Amount, and Invoice Number for verification
    let investmentUrl = '';
    ...
    // Step 3: Verify Number = Invoice_Number_Contact#1
    await test.step('Step 3: Verify "Number" row = value of Invoice_Number_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY INVOICE NUMBER VALUE ===');
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        actualValue = await investmentPage.getInvoicesFirstRowCellText('Number');
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Number actual  : "${actualValue}"`);
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Number expected: "${invoiceNumber}" (Invoice_Number_Contact#1)`);

        if (actualValue === invoiceNumber) break;
      ...
      expect(actualValue, 'Step 3.1: "Number" in Invoices tab should match Invoice_Number_Contact#1').toBe(invoiceNumber);

      console.log(`  ✓ 3.1 Number = ${invoiceNumber} — verified`);
    });
