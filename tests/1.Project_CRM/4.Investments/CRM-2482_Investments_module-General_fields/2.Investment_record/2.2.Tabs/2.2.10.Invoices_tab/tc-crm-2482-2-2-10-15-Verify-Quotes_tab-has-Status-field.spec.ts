 ...
 * Test Case ID: CRM-2482_2.2.10.15
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Status" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.15" --project=chromium
 *
 * Pre-condition:
 ...
 * 19. Get the status of Invoice called Invoice_Status_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Invoice" tab
 * 3. Verify the following:
 *    3.1. Value at "Status" row = value of Invoice_Status_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.15 - Verify the Invoices tab of an Investment record includes "Status" field = Invoice_Status_Contact#1', () => {
  ...
  test('CRM-2482_2.2.10.15: Verify the Invoices tab of an Investment record includes "Status" field = Invoice_Status_Contact#1', async ({ page }, testInfo) => {
    ...
    // Pre-condition II.19: Get the status of Invoice (Invoice_Status_Contact#1)
    await test.step('Pre-condition II.19: Get the status of Invoice (Invoice_Status_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.19: GET INVOICE STATUS ===');

      await page.waitForTimeout(CommonUtils.waitTimes.long);
      invoiceStatusContact1 = await invoicePage.getInvoiceStatus();

      console.log(`  ✓ Invoice_Status_Contact#1 : "${invoiceStatusContact1}"`);
    });
    ...
    // Step 3: Verify Status = Invoice_Status_Contact#1
    await test.step('Step 3: Verify "Status" row = value of Invoice_Status_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY STATUS VALUE ===');
      console.log(`  3.1 Status expected: "${invoiceStatusContact1}" (Invoice_Status_Contact#1)`);

      const actualValue = await investmentPage.getInvoicesFirstRowCellText('Status', invoiceStatusContact1);

      console.log(`  3.1 Status actual  : "${actualValue}"`);

      expect(actualValue, 'Step 3.1: "Status" in Invoices tab should match Invoice_Status_Contact#1').toBe(invoiceStatusContact1);

      console.log(`  ✓ 3.1 Status = "${invoiceStatusContact1}" — verified`);
    });
