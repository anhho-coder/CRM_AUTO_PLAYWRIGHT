 ...
 * Test Case ID: CRM-2482_2.2.10.3
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Invoice Date" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.3" --project=chromium
 *
 * Pre-condition:
 ...
 * 19. Get the value of "Invoice Date" called Invoice_Date_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Invoice" tab
 * 3. Verify the following:
 *    3.1. Value at "Invoice Date" row = value of Invoice_Date_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.3 - Verify the Invoices tab of an Investment record includes "Invoice Date" field = Invoice_Date_Contact#1', () => {
  ...
  test('CRM-2482_2.2.10.3: Verify the Invoices tab of an Investment record includes "Invoice Date" field = Invoice_Date_Contact#1', async ({ page }, testInfo) => {
    ...
    // Pre-condition II.19: Get Invoice Date (Invoice_Date_Contact#1)
    await test.step('Pre-condition II.19: Get "Invoice Date" (Invoice_Date_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.19: GET INVOICE DATE ===');

      await page.waitForTimeout(CommonUtils.waitTimes.long);
      invoiceDateContact1 = await invoicePage.getInvoiceDate();
      console.log(`  ✓ Invoice_Date_Contact#1 : ${invoiceDateContact1}`);
    });
    ...
    // Step 3: Verify Invoice Date = Invoice_Date_Contact#1
    await test.step('Step 3: Verify "Invoice Date" row = value of Invoice_Date_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY INVOICE DATE VALUE ===');
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        actualValue = await investmentPage.getInvoicesFirstRowCellText('Invoice Date');
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Invoice Date actual  : "${actualValue}"`);
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Invoice Date expected: "${invoiceDateContact1}" (Invoice_Date_Contact#1)`);

        if (actualValue === invoiceDateContact1) break;
      ...
      expect(actualValue, 'Step 3.1: "Invoice Date" in Invoices tab should match Invoice_Date_Contact#1').toBe(invoiceDateContact1);

      console.log(`  ✓ 3.1 Invoice Date = ${invoiceDateContact1} — verified`);
    });
