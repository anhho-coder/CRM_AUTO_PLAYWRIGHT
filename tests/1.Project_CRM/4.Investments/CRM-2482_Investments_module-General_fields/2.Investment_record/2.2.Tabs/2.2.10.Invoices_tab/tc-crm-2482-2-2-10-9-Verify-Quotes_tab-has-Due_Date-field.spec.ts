 ...
 * Test Case ID: CRM-2482_2.2.10.9
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Due Date" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.9" --project=chromium
 *
 * Pre-condition:
 ...
 * 19. Get the value of "Due Date" called Invoice_Due_Date_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Invoice" tab
 * 3. Verify the following:
 *    3.1. Value at "Due Date" row = value of Invoice_Due_Date_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.9 - Verify the Invoices tab of an Investment record includes "Due Date" field = Invoice_Due_Date_Contact#1', () => {
  ...
  test('CRM-2482_2.2.10.9: Verify the Invoices tab of an Investment record includes "Due Date" field = Invoice_Due_Date_Contact#1', async ({ page }, testInfo) => {
    ...
    // Pre-condition II.19: Get "Due Date" from the Invoice (Invoice_Due_Date_Contact#1)
    await test.step('Pre-condition II.19: Get "Due Date" from Invoice (Invoice_Due_Date_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.19: GET DUE DATE FROM INVOICE ===');

      invoiceDueDate = await invoicePage.getDueDate();

      console.log(`  ✓ Invoice_Due_Date_Contact#1 : "${invoiceDueDate}"`);
    });
    ...
    // Step 3: Verify Due Date = Invoice_Due_Date_Contact#1
    await test.step('Step 3: Verify "Due Date" row = value of Invoice_Due_Date_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY DUE DATE VALUE ===');
      console.log(`  3.1 Due Date expected: "${invoiceDueDate}" (Invoice_Due_Date_Contact#1)`);

      // Pass invoiceDueDate as expectedValue so the method retries/reloads only when
      // actual ≠ expected. If both are '' (blank), it returns immediately without any reload.
      const actualValue = await investmentPage.getInvoicesFirstRowCellText('Due Date', invoiceDueDate);

      console.log(`  3.1 Due Date actual  : "${actualValue}"`);

      expect(actualValue, 'Step 3.1: "Due Date" in Invoices tab should match Invoice_Due_Date_Contact#1').toBe(invoiceDueDate);

      console.log(`  ✓ 3.1 Due Date = "${invoiceDueDate}" — verified`);
    });
