 ...
 * Test Case ID: CRM-2482_2.2.10.7
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Salesperson" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.7" --project=chromium
 *
 * Pre-condition:
 ...
 * 19. Get the value of "Salesperson" called Invoice_Salesperson_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Invoice" tab
 * 3. Verify the following:
 *    3.1. Value at "Salesperson" row = value of Invoice_Salesperson_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.7 - Verify the Invoices tab of an Investment record includes "Salesperson" field = Invoice_Salesperson_Contact#1', () => {
  ...
  test('CRM-2482_2.2.10.7: Verify the Invoices tab of an Investment record includes "Salesperson" field = Invoice_Salesperson_Contact#1', async ({ page }, testInfo) => {
    ...
    // Pre-condition II.19: Get Salesperson (Invoice_Salesperson_Contact#1)
    await test.step('Pre-condition II.19: Get "Salesperson" (Invoice_Salesperson_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.19: GET SALESPERSON ===');

      await page.waitForTimeout(CommonUtils.waitTimes.long);
      invoiceSalesperson = await invoicePage.getSalesperson();
      console.log(`  ✓ Invoice_Salesperson_Contact#1 : ${invoiceSalesperson}`);
    });
    ...
    // Step 3: Verify Salesperson = Invoice_Salesperson_Contact#1
    await test.step('Step 3: Verify "Salesperson" row = value of Invoice_Salesperson_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY SALESPERSON VALUE ===');
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          actualValue = await investmentPage.getInvoicesFirstRowCellText('Salesperson');
        } catch (error) {
          const msg = error instanceof Error ? error.message : String(error);
          if (msg.includes('column header "Salesperson" not found')) {
            // Odoo may hide empty columns; treat missing header as blank value
            actualValue = '';
            console.log(`  ⚠ Attempt ${attempt}/${MAX_RETRIES} — "Salesperson" column not visible; treating as BLANK`);
          } else {
            throw error;
          }
        }

        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Salesperson actual  : "${actualValue}"`);
        console.log(`  3.1 Attempt ${attempt}/${MAX_RETRIES} — Salesperson expected: "${invoiceSalesperson}" (Invoice_Salesperson_Contact#1)`);

        if (actualValue === invoiceSalesperson) {
          matched = true;
          break;
        }
      ...
      expect(matched, 'Step 3.1: Failed to match Salesperson value after all retries').toBeTruthy();
      expect(actualValue, 'Step 3.1: "Salesperson" in Invoices tab should match Invoice_Salesperson_Contact#1').toBe(invoiceSalesperson);

      console.log(`  ✓ 3.1 Salesperson = ${invoiceSalesperson} — verified`);
    });
