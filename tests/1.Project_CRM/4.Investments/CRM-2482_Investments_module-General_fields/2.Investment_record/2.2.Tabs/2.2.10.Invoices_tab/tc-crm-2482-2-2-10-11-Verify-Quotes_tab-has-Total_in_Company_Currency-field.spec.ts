 ...
 * Test Case ID: CRM-2482_2.2.10.11
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Total in Company Currency" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.11" --project=chromium
 *
 * Pre-condition:
 ...
 * 19. Click at "Payments" tab and verify the following:
 *     19.1. Get the value of "Total in Company Currency" called Invoice_Total_in_Company_Currency_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Invoice" tab
 * 3. Verify the following:
 *    3.1. Value at "Total in Company Currency" row = value of Invoice_Total_in_Company_Currency_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.11 - Verify the Invoices tab of an Investment record includes "Total in Company Currency" field = Invoice_Total_in_Company_Currency_Contact#1', () => {
  ...
  test('CRM-2482_2.2.10.11: Verify the Invoices tab of an Investment record includes "Total in Company Currency" field = Invoice_Total_in_Company_Currency_Contact#1', async ({ page }, testInfo) => {
    ...
    let invoiceTotalInCompanyCurrency = '';
    ...
    // Pre-condition II.19: Click at "Payments" tab and get "Total in Company Currency" (Invoice_Total_in_Company_Currency_Contact#1)
    await test.step('Pre-condition II.19: Click at "Payments" tab and get "Total in Company Currency" (Invoice_Total_in_Company_Currency_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.19: GET TOTAL IN COMPANY CURRENCY FROM INVOICE ===');

      await invoicePage.clickPaymentsTab();
      invoiceTotalInCompanyCurrency = await invoicePage.getTotalInCompanyCurrencyOnInvoice();

      console.log(`  ✓ Invoice_Total_in_Company_Currency_Contact#1 : "${invoiceTotalInCompanyCurrency}"`);    
    });
    ...
    // Step 3: Verify Total in Company Currency = Invoice_Total_in_Company_Currency_Contact#1
    await test.step('Step 3: Verify "Total in Company Currency" row = value of Invoice_Total_in_Company_Currency_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY TOTAL IN COMPANY CURRENCY VALUE ===');
      console.log(`  3.1 Total in Company Currency expected: "${invoiceTotalInCompanyCurrency}" (Invoice_Total_in_Company_Currency_Contact#1)`);

      // Pass invoiceTotalInCompanyCurrency as expectedValue so the method retries/reloads only when
      // actual ≠ expected. If both are '' (blank), it returns immediately without any reload.
      const actualValue = await investmentPage.getInvoicesFirstRowCellText('Total in Company Currency', invoiceTotalInCompanyCurrency);

      console.log(`  3.1 Total in Company Currency actual  : "${actualValue}"`);

      expect(actualValue, 'Step 3.1: "Total in Company Currency" in Invoices tab should match Invoice_Total_in_Company_Currency_Contact#1').toBe(invoiceTotalInCompanyCurrency);

      console.log(`  ✓ 3.1 Total in Company Currency = "${invoiceTotalInCompanyCurrency}" — verified`);
    });
