 ...
 * Test Case ID: CRM-2482_2.2.10.10
 *
 * Summary: Verify the Invoices tab of an Investment record includes "Total" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2\.2\.10\.10" --project=chromium
 *
 * Pre-condition:
 ...
 *     19.1. Get the value of "Payment Amount" called Invoice_Payment_Amount_Contact#1
 *
 * Steps to reproduce:
 * 1. Launch the webpage with URL_Investment#1
 * 2. Click at "Invoice" tab
 * 3. Verify the following:
 *    3.1. Value at "Total" row = value of Invoice_Payment_Amount_Contact#1
 *
...
test.describe('CRM-2482_2.2.10.10 - Verify the Invoices tab of an Investment record includes "Total" field = Invoice_Payment_Amount_Contact#1', () => {
  ...
  test('CRM-2482_2.2.10.10: Verify the Invoices tab of an Investment record includes "Total" field = Invoice_Payment_Amount_Contact#1', async ({ page }, testInfo) => {
    ...
    // Pre-condition II.19: Click at "Payments" tab and get "Payment Amount" (Invoice_Payment_Amount_Contact#1)
    await test.step('Pre-condition II.19: Click at "Payments" tab and get "Payment Amount" (Invoice_Payment_Amount_Contact#1)', async () => {
      console.log('=== PRE-CONDITION II.19: GET PAYMENT AMOUNT FROM PAYMENTS TAB ===');

      await invoicePage.clickPaymentsTab();
      invoicePaymentAmount = await invoicePage.getPaymentAmountFromPaymentsTab();

      console.log(`  ✓ Invoice_Payment_Amount_Contact#1 : "${invoicePaymentAmount}"`);
    });
    ...
    // Step 3: Verify Total = Invoice_Payment_Amount_Contact#1
    await test.step('Step 3: Verify "Total" row = value of Invoice_Payment_Amount_Contact#1', async () => {
      console.log('=== STEP 3: VERIFY TOTAL VALUE ===');
      console.log(`  3.1 Total expected: "${invoicePaymentAmount}" (Invoice_Payment_Amount_Contact#1)`);

      // Pass invoicePaymentAmount as expectedValue so the method retries/reloads only when
      // actual ≠ expected. If both are '' (blank), it returns immediately without any reload.
      const actualValue = await investmentPage.getInvoicesFirstRowCellText('Total', invoicePaymentAmount);

      console.log(`  3.1 Total actual  : "${actualValue}"`);

      expect(actualValue, 'Step 3.1: "Total" in Invoices tab should match Invoice_Payment_Amount_Contact#1').toBe(invoicePaymentAmount);

      console.log(`  ✓ 3.1 Total = "${invoicePaymentAmount}" — verified`);
    });
