 ...
 * Test Case ID: CRM-2482_2.2.2.1.1
 *
 * Summary: Verify the ROI blocks of a Investment record includes "Date" row
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.2.1.1" --project=chromium
 *
 * Pre-condition:
...
test.describe('CRM-2482_2.2.2.1.1 - Verify Planned Expenses Tab includes Date row', () => {
  ...
  test('CRM-2482_2.2.2.1.1: Verify the ROI blocks of a Investment record includes "Date" row', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click at "Date" field', async () => {
      ...
      await investmentPage.clickPlannedExpensesDateField();
    ...
    await test.step('Step 5: Verify "Date" field is a calendar field', async () => {
      ...
      const isCalendarFieldValid = await investmentPage.verifyPlannedExpensesDateField();
