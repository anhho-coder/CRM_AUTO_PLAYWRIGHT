 ...
 * Test Case ID: CRM-2482_2.2.2.1.5
 *
 * Summary: Verify the ROI blocks of a Investment record includes "IC" row
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.2.1.5" --project=chromium
 *
 * Pre-condition:
 ...
 * 4. Click at "IC" field
 * 5. Verify "IC" field is a calendar field
 */

test.describe('CRM-2482_2.2.2.1.5 - Verify Planned Expenses Tab includes IC row', () => {
  ...
  test('CRM-2482_2.2.2.1.5: Verify the ROI blocks of a Investment record includes "IC" row', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click at "IC" field', async () => {
      ...
      await investmentPage.clickPlannedExpensesICField();

      console.log('\u2713 "IC" field clicked');
    });

    // Step 5: Verify IC field is a calendar field
    await test.step('Step 5: Verify "IC" field is a calendar field', async () => {
      console.log('=== STEP 5: VERIFY "IC" FIELD ===');

      const isCalendarFieldValid = await investmentPage.verifyPlannedExpensesICField();
      expect(isCalendarFieldValid).toBeTruthy();

      console.log('  \u2713 "IC" field exists');
      console.log('  \u2713 "IC" field is a calendar field');
