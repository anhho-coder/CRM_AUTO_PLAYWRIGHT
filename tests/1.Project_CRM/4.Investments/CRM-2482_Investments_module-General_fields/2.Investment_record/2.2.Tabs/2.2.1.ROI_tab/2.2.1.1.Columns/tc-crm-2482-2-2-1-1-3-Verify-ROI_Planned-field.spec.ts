 ...
 * Test Case ID: CRM-2482_2.2.1.1.3
 *
 * Summary: Verify the ROI blocks of a Investment record includes "ROI" column
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.1.1.3" --project=chromium
 *
 * Pre-condition:
 ...
 * 2. Click at "ROI" tab
 * 3. Verify "ROI-Planned" field is a read only field
 */

test.describe('CRM-2482_2.2.1.1.3 - Verify ROI Tab includes ROI column', () => {
  ...
  test('CRM-2482_2.2.1.1.3: Verify the ROI blocks of a Investment record includes "ROI" column', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify "ROI-Planned" field is a read only field
    await test.step('Step 3: Verify "ROI-Planned" field is a read only field', async () => {
      console.log('=== STEP 3: VERIFY "ROI-Planned" FIELD ===');

      const isReadOnlyValid = await investmentPage.verifyROIPlannedField();
      expect(isReadOnlyValid).toBeTruthy();

      console.log('  ✓ "ROI-Planned" field exists');
      console.log('  ✓ "ROI-Planned" field is a read only field');
      console.log('✓ ROI-Planned field verified successfully');
    });
