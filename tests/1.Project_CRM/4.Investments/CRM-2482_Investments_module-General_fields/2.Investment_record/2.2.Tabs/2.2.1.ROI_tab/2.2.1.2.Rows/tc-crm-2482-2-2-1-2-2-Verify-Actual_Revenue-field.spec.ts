 ...
 * Test Case ID: CRM-2482_2.2.1.2.2
 *
 * Summary: Verify the ROI blocks of a Investment record includes "Actual" row
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.1.2.2" --project=chromium
 *
 * Pre-condition:
 ...
 * 2. Click at "ROI" tab
 * 3. Verify "Actual-Revenue" field is a read only field
 */

test.describe('CRM-2482_2.2.1.2.2 - Verify ROI Tab includes Actual row', () => {
  ...
  test('CRM-2482_2.2.1.2.2: Verify the ROI blocks of a Investment record includes "Actual" row', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify "Actual-Revenue" field is a read only field
    await test.step('Step 3: Verify "Actual-Revenue" field is a read only field', async () => {
      console.log('=== STEP 3: VERIFY "Actual-Revenue" FIELD ===');

      const isReadOnlyValid = await investmentPage.verifyActualRevenueField();
      expect(isReadOnlyValid).toBeTruthy();

      console.log('  ✓ "Actual-Revenue" field exists');
      console.log('  ✓ "Actual-Revenue" field is a read only field');
      console.log('✓ Actual-Revenue field verified successfully');
    });
