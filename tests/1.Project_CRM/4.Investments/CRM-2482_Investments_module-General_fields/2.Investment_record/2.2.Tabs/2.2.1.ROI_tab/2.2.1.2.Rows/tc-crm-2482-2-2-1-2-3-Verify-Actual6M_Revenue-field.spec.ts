...
/**
 * Investments - ROI Tab Row Verification Test
 * Test Case ID: CRM-2482_2.2.1.2.3
 *
 * Summary: Verify the ROI blocks of a Investment record includes "Actual 6M" row
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.1.2.3" --project=chromium
 *
 * Pre-condition:
 ...
 * 2. Click at "ROI" tab
 * 3. Verify "Actual 6M-Revenue" field is a read only field
 */

test.describe('CRM-2482_2.2.1.2.3 - Verify ROI Tab includes Actual 6M row', () => {
  ...
  test('CRM-2482_2.2.1.2.3: Verify the ROI blocks of a Investment record includes "Actual 6M" row', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify "Actual 6M-Revenue" field is a read only field
    await test.step('Step 3: Verify "Actual 6M-Revenue" field is a read only field', async () => {
      console.log('=== STEP 3: VERIFY "Actual 6M-Revenue" FIELD ===');

      const isReadOnlyValid = await investmentPage.verifyActual6MRevenueField();
      expect(isReadOnlyValid).toBeTruthy();

      console.log('  ✓ "Actual 6M-Revenue" field exists');
      console.log('  ✓ "Actual 6M-Revenue" field is a read only field');
      console.log('✓ Actual 6M-Revenue field verified successfully');
    });
