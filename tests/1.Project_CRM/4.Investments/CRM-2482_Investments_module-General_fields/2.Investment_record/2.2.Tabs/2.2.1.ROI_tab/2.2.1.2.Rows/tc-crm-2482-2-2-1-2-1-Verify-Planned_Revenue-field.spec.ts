 ...
 * Test Case ID: CRM-2482_2.2.1.2.1
 *
 * Summary: Verify the ROI blocks of a Investment record includes "Planned" row
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.1.2.1" --project=chromium
 *
 * Pre-condition:
 ...
 * 2. Click at "ROI" tab
 * 3. Verify "Planned-Revenue" field is a text field
 */

test.describe('CRM-2482_2.2.1.2.1 - Verify ROI Tab includes Planned row', () => {
  ...
  test('CRM-2482_2.2.1.2.1: Verify the ROI blocks of a Investment record includes "Planned" row', async ({ page }, testInfo) => {
    ...
    // Step 3: Verify "Planned-Revenue" field is a text field
    await test.step('Step 3: Verify "Planned-Revenue" field is a text field', async () => {
      console.log('=== STEP 3: VERIFY "Planned-Revenue" FIELD ===');

      const isTextFieldValid = await investmentPage.verifyRevenuePlannedField();
      expect(isTextFieldValid).toBeTruthy();

      console.log('  ✓ "Planned-Revenue" field exists');
      console.log('  ✓ "Planned-Revenue" field is a text input');
      console.log('✓ Planned-Revenue field verified successfully');
    });
