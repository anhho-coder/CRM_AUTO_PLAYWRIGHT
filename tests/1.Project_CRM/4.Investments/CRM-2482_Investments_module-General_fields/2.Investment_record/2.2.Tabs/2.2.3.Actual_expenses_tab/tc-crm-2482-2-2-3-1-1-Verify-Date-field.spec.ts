 ...
 * Test Case ID: CRM-2482_2.2.3.1.1
 *
 * Summary: Verify the Actual expenses tab of a Investment record includes "Date" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.3.1.1" --project=chromium
 *
 * Pre-condition:
 ...
 * 4. Click at "Date" field
 * 5. Verify "Date" field is a calendar field
 */

test.describe('CRM-2482_2.2.3.1.1 - Verify Actual Expenses Tab includes Date field', () => {
  ...
  test('CRM-2482_2.2.3.1.1: Verify the Actual expenses tab of a Investment record includes "Date" field', async ({ page }, testInfo) => {
    ...
    // Step 4: Click at Date field
    await test.step('Step 4: Click at "Date" field', async () => {
      console.log('=== STEP 4: CLICK "Date" FIELD ===');

      await investmentPage.clickActualExpensesDateField();

      console.log('\u2713 "Date" field clicked');
    });

    // Step 5: Verify Date field is a calendar field
    await test.step('Step 5: Verify "Date" field is a calendar field', async () => {
      console.log('=== STEP 5: VERIFY "Date" FIELD ===');

      const isCalendarFieldValid = await investmentPage.verifyActualExpensesDateField();
      expect(isCalendarFieldValid).toBeTruthy();

      console.log('  \u2713 "Date" field exists');
      console.log('  \u2713 "Date" field is a calendar field');
      console.log('\u2713 Date field verified successfully');
    });
