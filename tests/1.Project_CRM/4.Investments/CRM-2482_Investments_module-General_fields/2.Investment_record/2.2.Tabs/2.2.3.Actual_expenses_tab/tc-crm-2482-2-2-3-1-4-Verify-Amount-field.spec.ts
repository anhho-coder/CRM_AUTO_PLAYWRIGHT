 ...
 * Test Case ID: CRM-2482_2.2.3.1.4
 *
 * Summary: Verify the Actual expenses tab of a Investment record includes "Amount" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2482_2.2.3.1.4" --project=chromium
 *
 * Pre-condition:
 ...
 * 4. Click at "Amount" field
 * 5. Verify "Amount" field is a number field
 */

test.describe('CRM-2482_2.2.3.1.4 - Verify Actual Expenses Tab includes Amount field', () => {
  ...
  test('CRM-2482_2.2.3.1.4: Verify the Actual expenses tab of a Investment record includes "Amount" field', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Press "Add a line" link and wait', async () => {
      ...
      await investmentPage.actualExpenses_clickAddALine();

      console.log('✓ "Add a line" clicked and new row appeared');
    });

    // Step 4: Click at Amount field
    await test.step('Step 4: Click at "Amount" field', async () => {
      console.log('=== STEP 4: CLICK "Amount" FIELD ===');

      await investmentPage.clickActualExpensesAmountField();

      console.log('\u2713 "Amount" field clicked');
    });

    // Step 5: Verify Amount field is a number field
    await test.step('Step 5: Verify "Amount" field is a number field', async () => {
      console.log('=== STEP 5: VERIFY "Amount" FIELD ===');

      const isNumberFieldValid = await investmentPage.verifyActualExpensesAmountField();
      expect(isNumberFieldValid).toBeTruthy();

      console.log('  \u2713 "Amount" field exists');
      console.log('  \u2713 "Amount" field is a number field');
      console.log('\u2713 Amount field verified successfully');
    });
