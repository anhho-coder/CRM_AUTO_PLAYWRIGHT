...
/**
 * Investments - Track Conversion Date End Required Field Test
 * Test Case ID: CRM-3326_3.1.2
 *
 * Summary: Verify the "Track conversion date end" is a required field after clearing the field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3326_3.1.2" --project=chromium
 *
 * Pre-condition:
 ...
 * 2. Clear the value:
 *    - "Track conversion date end" date = BLANK
 * 3. Press "SAVE" button
 * 4. Verify "Track conversion date end" date is a required field by showing this warning:
 *    "The following fields are invalid:
 *    Track conversion date end"
 * 5. Set the value:
...
test.describe('CRM-3326_3.1.2 - Verify Track conversion date end is a required field after clearing', () => {
  ...
  test('CRM-3326_3.1.2: Verify Track conversion date end is a required field after clearing the field', async ({ page }) => {
    ...
    // Step 2: Clear "Track conversion date end"
    await test.step('Step 2: Clear "Track conversion date end" date = BLANK', async () => {
      console.log('=== STEP 2: CLEAR TRACK CONVERSION DATE END ===');

      await investmentPage.clearTrackConversionDateEnd();

      const valueAfterClear = await investmentPage.getTrackConversionDateEndValue();
      console.log(`✓ Track conversion date end cleared. Current value: "${valueAfterClear}"`);
    });
    ...
    await test.step('Step 4: Verify warning "The following fields are invalid - Track conversion date end"', async () => {
      ...
      expect(warningText, 'Step 4: Warning should list "Track conversion date end"').toContain('Track conversion date end');

      console.log('  ✓ Required field warning correctly displayed for "Track conversion date end"');
    });

    // Step 5: Set Track conversion date end = current date + 6 months
    await test.step('Step 5: Set "Track conversion date end" date = current date + 6 months', async () => {
      console.log('=== STEP 5: SET TRACK CONVERSION DATE END ===');

      await investmentPage.fillTrackConversionDateEnd(dateEndPlus6Months);

      console.log(`✓ Track conversion date end set to: ${dateEndPlus6Months}`);
    });
