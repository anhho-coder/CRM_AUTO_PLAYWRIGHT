 ...
 * Test Case ID: CRM-3374_3.1
 *
 * Summary: Verify error message displays when NOT entering value at "Investment ID" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_3\.1:" --project=chromium
 * npx playwright test --grep "CRM-9854" --project=chromium
 *
 * Pre-condition:
 ...
 *         - Tags          = test
 *         - Investment ID = BLANK [EMPTY]                                    (InvestmentID_Contact#1 — blank/empty)
 ...
 *    "Lines [2] \"Investment ID\" is empty"
...
test.describe('CRM-3374_3.1 - Verify error message displays when NOT entering value at "Investment ID" field', () => {
  ...
  test('CRM-3374_3.1 [CRM-9854]: Verify error message displays when NOT entering value at "Investment ID" field', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);

    testInfo.annotations.push({ type: 'defect', description: 'CRM-9854 - fixed, re-running to verify' });
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ investmentId: '' });
      ...
      console.log('  ✓ InvestmentID_Contact#1   : (blank — empty value)');
    });
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "Investment ID" is empty', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "Investment ID" is empty';
      ...
      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"Investment ID" is empty');
    ...
    await test.step('Step 10: Verify "Email" row has no value of Email_Contact#1', async () => {
      ...
      // Step 10.1: Verify Email_Contact#1 is NOT present (import was rejected due to blank Investment ID)
