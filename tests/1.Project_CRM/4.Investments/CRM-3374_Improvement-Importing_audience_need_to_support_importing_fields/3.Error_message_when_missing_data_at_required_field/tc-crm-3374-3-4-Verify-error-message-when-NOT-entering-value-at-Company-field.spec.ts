 ...
 * Test Case ID: CRM-3374_3.4
 *
 * Summary: Verify error message displays when NOT entering value at "Company" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_3\.4:" --project=chromium
 * npx playwright test --grep "CRM-9855" --project=chromium
 *
 * Pre-condition:
...
test.describe('CRM-3374_3.4 - Verify error message displays when NOT entering value at "Company" field', () => {
  ...
  test('CRM-3374_3.4 [CRM-9855]: Verify error message displays when NOT entering value at "Company" field', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);

    testInfo.annotations.push({ type: 'defect', description: 'CRM-9855 - fixed, re-running to verify' });
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ company: '', investmentId: 'test', contact: 'test' });
      ...
      console.log('  ✓ Company_Contact#1        : (blank — empty value)');
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "Company" is empty', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "Company" is empty';
      ...
      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"Company" is empty');
    ...
    await test.step('Step 10: Verify "Email" row has no value of Email_Contact#1', async () => {
      ...
      // Step 10.1: Verify Email_Contact#1 is NOT present (import was rejected due to blank Company)
