 ...
 * Test Case ID: CRM-3374_3.3
 *
 * Summary: Verify error message displays when NOT entering value at "Email" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_3\.3:" --project=chromium
 *
 * Pre-condition:
 ...
 *    "Lines [2] \"Email\" is empty"
 * 9. Click at "Leads" tab
 * 10. Verify the following:
 *    10.1. There is no value of Name_Contact#1 at "Contact Name" row
 *
...
test.describe('CRM-3374_3.3 - Verify error message displays when NOT entering value at "Email" field', () => {
  ...
  test('CRM-3374_3.3: Verify error message displays when NOT entering value at "Email" field', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ email: '', investmentId: 'test', contact: 'test' });
      ...
      console.log('  ✓ Email_Contact#1          : (blank — empty value)');
      console.log(`  ✓ Sales_Team_Contact#1     : ${audienceData.salesTeam}`);
      console.log(`  ✓ Salesperson_Contact#1    : ${audienceData.salesperson}`);
      console.log('  ✓ InvestmentID_Contact#1   : test');
      console.log('  ✓ Contact_Contact#1        : test');
    });
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "Email" is empty', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "Email" is empty';

      const actualErrorText = await investmentPage.waitForImportErrorMessage('is empty');

      console.log(`  Actual error message  : "${actualErrorText}"`);
      console.log(`  Expected error message: "${expectedError}"`);

      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"Email" is empty');
    ...
    // Step 10: Verify no Name_Contact#1 in Leads tab
    await test.step('Step 10: Verify "Contact Name" row has no value of Name_Contact#1', async () => {
      console.log('=== STEP 10: VERIFY CONTACT NAME NOT IN LEADS TAB ===');

      // Step 10.1: Verify Name_Contact#1 is NOT present (import was rejected due to blank Email)
      await test.step('Step 10.1: There is no value of Name_Contact#1 at "Contact Name" row', async () => {
        const contactNameText = await investmentPage.getLeadsFirstRowCellText('Contact Name').catch(() => '');

        console.log(`  10.1 Contact Name actual  : "${contactNameText}"`);
        console.log(`  10.1 Contact Name expected: NOT "${audienceData.contactName}"`);

        expect(contactNameText, 'Step 10.1: Name_Contact#1 should NOT be present in Leads tab').not.toBe(audienceData.contactName);
        console.log('  ✓ 10.1 Name_Contact#1 is not present in Leads tab — import was rejected');
      });
    });
