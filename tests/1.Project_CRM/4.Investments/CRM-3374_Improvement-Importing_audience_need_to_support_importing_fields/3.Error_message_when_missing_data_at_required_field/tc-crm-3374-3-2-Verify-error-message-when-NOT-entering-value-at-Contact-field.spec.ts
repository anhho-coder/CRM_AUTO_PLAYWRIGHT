 ...
 * Test Case ID: CRM-3374_3.2
 *
 * Summary: Verify error message displays when NOT entering value at "Contact" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_3\.2:" --project=chromium
 *
 * Pre-condition:
...
test.describe('CRM-3374_3.2 - Verify error message displays when NOT entering value at "Contact Name" field', () => {
  ...
  test('CRM-3374_3.2: Verify error message displays when NOT entering value at "Contact Name" field', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ contactName: '', investmentId: 'test' });
      ...
      console.log('  ✓ Name_Contact#1           : (blank — empty value)');
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "Contact Name" is empty', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "Contact Name" is empty';
      ...
      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"Contact Name" is empty');
    ...
    await test.step('Step 10: Verify "Email" row has no value of Email_Contact#1', async () => {
      ...
      // Step 10.1: Verify Email_Contact#1 is NOT present (import was rejected due to blank Contact Name)
