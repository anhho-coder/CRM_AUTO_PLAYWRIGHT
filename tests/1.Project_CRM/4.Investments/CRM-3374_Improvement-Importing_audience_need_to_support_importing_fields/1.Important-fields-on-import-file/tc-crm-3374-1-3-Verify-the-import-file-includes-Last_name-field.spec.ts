 ...
 * Test Case ID: CRM-3374_1.3
 *
 * Summary: Verify the import file includes "Last name" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.3:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Last name" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.3 - Verify the import file includes "Last name" field', () => {
  ...
  test('CRM-3374_1.3: Verify the import file includes "Last name" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Last name" field exists in the xlsx file
    await test.step('Step 1: Verify the "Last name" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Last name" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'last name');
      const hasLastName = colIndex !== -1;

      if (hasLastName) {
        console.log(`  ✓ "Last name" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasLastName, 'Step 1: The downloaded xlsx template should contain "Last name" column').toBe(true);

      console.log('  ✓ "Last name" field exists in the xlsx file — verified');
    });
