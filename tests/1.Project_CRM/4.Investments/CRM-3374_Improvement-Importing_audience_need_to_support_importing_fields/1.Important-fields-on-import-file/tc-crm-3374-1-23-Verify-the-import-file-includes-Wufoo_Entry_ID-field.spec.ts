 ...
 * Test Case ID: CRM-3374_1.23
 *
 * Summary: Verify the import file includes "Wufoo Entry ID" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.23:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Wufoo Entry ID" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.23 - Verify the import file includes "Wufoo Entry ID" field', () => {
  ...
  test('CRM-3374_1.23: Verify the import file includes "Wufoo Entry ID" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Wufoo Entry ID" field exists in the xlsx file
    await test.step('Step 1: Verify the "Wufoo Entry ID" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Wufoo Entry ID" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'wufoo entry id');
      const hasWufooEntryId = colIndex !== -1;

      if (hasWufooEntryId) {
        console.log(`  ✓ "Wufoo Entry ID" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasWufooEntryId, 'Step 1: The downloaded xlsx template should contain "Wufoo Entry ID" column').toBe(true);

      console.log('  ✓ "Wufoo Entry ID" field exists in the xlsx file — verified');
    });
