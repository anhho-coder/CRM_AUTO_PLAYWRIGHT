 ...
 * Test Case ID: CRM-3374_1.13
 *
 * Summary: Verify the import file includes "Zip" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.13:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Zip" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.13 - Verify the import file includes "Zip" field', () => {
  ...
  test('CRM-3374_1.13: Verify the import file includes "Zip" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Zip" field exists in the xlsx file
    await test.step('Step 1: Verify the "Zip" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Zip" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'zip');
      const hasZip = colIndex !== -1;

      if (hasZip) {
        console.log(`  ✓ "Zip" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasZip, 'Step 1: The downloaded xlsx template should contain "Zip" column').toBe(true);

      console.log('  ✓ "Zip" field exists in the xlsx file — verified');
    });
