 ...
 * Test Case ID: CRM-3374_1.6
 *
 * Summary: Verify the import file includes "Company" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.6:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Company" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.6 - Verify the import file includes "Company" field', () => {
  ...
  test('CRM-3374_1.6: Verify the import file includes "Company" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Company" field exists in the xlsx file
    await test.step('Step 1: Verify the "Company" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Company" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'company');
      const hasCompany = colIndex !== -1;

      if (hasCompany) {
        console.log(`  ✓ "Company" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasCompany, 'Step 1: The downloaded xlsx template should contain "Company" column').toBe(true);

      console.log('  ✓ "Company" field exists in the xlsx file — verified');
    });
