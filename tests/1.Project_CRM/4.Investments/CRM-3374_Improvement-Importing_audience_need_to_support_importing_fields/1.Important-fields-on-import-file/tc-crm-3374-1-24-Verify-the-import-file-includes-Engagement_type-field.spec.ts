 ...
 * Test Case ID: CRM-3374_1.24
 *
 * Summary: Verify the import file includes "Engagement type" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.24:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Engagement type" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.24 - Verify the import file includes "Engagement type" field', () => {
  ...
  test('CRM-3374_1.24: Verify the import file includes "Engagement type" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Engagement type" field exists in the xlsx file
    await test.step('Step 1: Verify the "Engagement type" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Engagement type" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'engagement type');
      const hasEngagementType = colIndex !== -1;

      if (hasEngagementType) {
        console.log(`  ✓ "Engagement type" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasEngagementType, 'Step 1: The downloaded xlsx template should contain "Engagement type" column').toBe(true);

      console.log('  ✓ "Engagement type" field exists in the xlsx file — verified');
    });
