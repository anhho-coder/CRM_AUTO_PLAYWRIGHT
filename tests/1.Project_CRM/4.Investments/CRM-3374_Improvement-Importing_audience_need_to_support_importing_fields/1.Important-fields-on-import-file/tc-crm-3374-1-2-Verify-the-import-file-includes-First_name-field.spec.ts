 ...
 * Test Case ID: CRM-3374_1.2
 *
 * Summary: Verify the import file includes "First name" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.2:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "First name" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.2 - Verify the import file includes "First name" field', () => {
  ...
  test('CRM-3374_1.2: Verify the import file includes "First name" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "First name" field exists in the xlsx file
    await test.step('Step 1: Verify the "First name" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "First name" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'first name');
      const hasFirstName = colIndex !== -1;

      if (hasFirstName) {
        console.log(`  ✓ "First name" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasFirstName, 'Step 1: The downloaded xlsx template should contain "First name" column').toBe(true);

      console.log('  ✓ "First name" field exists in the xlsx file — verified');
    });
