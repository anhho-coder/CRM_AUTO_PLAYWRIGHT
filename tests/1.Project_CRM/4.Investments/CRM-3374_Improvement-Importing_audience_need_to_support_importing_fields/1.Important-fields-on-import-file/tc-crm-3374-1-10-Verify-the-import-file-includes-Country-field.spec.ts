 ...
 * Test Case ID: CRM-3374_1.10
 *
 * Summary: Verify the import file includes "Country" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.10:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Country" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.10 - Verify the import file includes "Country" field', () => {
  ...
  test('CRM-3374_1.10: Verify the import file includes "Country" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Country" field exists in the xlsx file
    await test.step('Step 1: Verify the "Country" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Country" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'country');
      const hasCountry = colIndex !== -1;

      if (hasCountry) {
        console.log(`  ✓ "Country" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasCountry, 'Step 1: The downloaded xlsx template should contain "Country" column').toBe(true);

      console.log('  ✓ "Country" field exists in the xlsx file — verified');
    });
