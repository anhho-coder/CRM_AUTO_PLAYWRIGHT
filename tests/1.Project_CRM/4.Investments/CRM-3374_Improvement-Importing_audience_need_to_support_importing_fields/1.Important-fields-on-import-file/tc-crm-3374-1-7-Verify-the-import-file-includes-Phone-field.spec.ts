 ...
 * Test Case ID: CRM-3374_1.7
 *
 * Summary: Verify the import file includes "Phone" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.7:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Phone" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.7 - Verify the import file includes "Phone" field', () => {
  ...
  test('CRM-3374_1.7: Verify the import file includes "Phone" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Phone" field exists in the xlsx file
    await test.step('Step 1: Verify the "Phone" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Phone" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'phone');
      const hasPhone = colIndex !== -1;

      if (hasPhone) {
        console.log(`  ✓ "Phone" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasPhone, 'Step 1: The downloaded xlsx template should contain "Phone" column').toBe(true);

      console.log('  ✓ "Phone" field exists in the xlsx file — verified');
    });
