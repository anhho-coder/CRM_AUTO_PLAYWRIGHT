 ...
 * Test Case ID: CRM-3374_1.21
 *
 * Summary: Verify the import file includes "Type" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.21:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Type" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.21 - Verify the import file includes "Type" field', () => {
  ...
  test('CRM-3374_1.21: Verify the import file includes "Type" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Type" field exists in the xlsx file
    await test.step('Step 1: Verify the "Type" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Type" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'type');
      const hasType = colIndex !== -1;

      if (hasType) {
        console.log(`  ✓ "Type" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasType, 'Step 1: The downloaded xlsx template should contain "Type" column').toBe(true);

      console.log('  ✓ "Type" field exists in the xlsx file — verified');
    });
