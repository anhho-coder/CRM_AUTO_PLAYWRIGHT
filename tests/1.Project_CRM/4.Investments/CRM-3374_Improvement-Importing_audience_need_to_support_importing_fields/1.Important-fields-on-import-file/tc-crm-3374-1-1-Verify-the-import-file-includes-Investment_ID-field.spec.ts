 ...
 * Test Case ID: CRM-3374_1.1
 *
 * Summary: Verify the import file includes "Investment ID" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.1:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Investment ID" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.1 - Verify the import file includes "Investment ID" field', () => {
  ...
  test('CRM-3374_1.1: Verify the import file includes "Investment ID" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Investment ID" field exists in the xlsx file
    await test.step('Step 1: Verify the "Investment ID" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Investment ID" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'investment id');
      const hasInvestmentID = colIndex !== -1;

      if (hasInvestmentID) {
        console.log(`  ✓ "Investment ID" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasInvestmentID, 'Step 1: The downloaded xlsx template should contain "Investment ID" column').toBe(true);

      console.log('  ✓ "Investment ID" field exists in the xlsx file — verified');
    });
