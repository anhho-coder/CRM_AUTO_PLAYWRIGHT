 ...
 * Test Case ID: CRM-3374_1.19
 *
 * Summary: Verify the import file includes "Tags for all leads" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.19:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Tags for all leads" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.19 - Verify the import file includes "Tags for all leads" field', () => {
  ...
  test('CRM-3374_1.19: Verify the import file includes "Tags for all leads" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Tags for all leads" field exists in the xlsx file
    await test.step('Step 1: Verify the "Tags for all leads" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Tags for all leads" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'tags for all leads');
      const hasTagsForAllLeads = colIndex !== -1;

      if (hasTagsForAllLeads) {
        console.log(`  ✓ "Tags for all leads" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasTagsForAllLeads, 'Step 1: The downloaded xlsx template should contain "Tags for all leads" column').toBe(true);

      console.log('  ✓ "Tags for all leads" field exists in the xlsx file — verified');
    });
