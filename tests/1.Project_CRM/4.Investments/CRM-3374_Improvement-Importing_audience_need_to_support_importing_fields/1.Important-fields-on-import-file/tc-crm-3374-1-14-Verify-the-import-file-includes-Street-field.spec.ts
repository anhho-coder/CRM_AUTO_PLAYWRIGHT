 ...
 * Test Case ID: CRM-3374_1.14
 *
 * Summary: Verify the import file includes "Street" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.14:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Street" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.14 - Verify the import file includes "Street" field', () => {
  ...
  test('CRM-3374_1.14: Verify the import file includes "Street" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Street" field exists in the xlsx file
    await test.step('Step 1: Verify the "Street" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Street" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'street');
      const hasStreet = colIndex !== -1;

      if (hasStreet) {
        console.log(`  ✓ "Street" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasStreet, 'Step 1: The downloaded xlsx template should contain "Street" column').toBe(true);

      console.log('  ✓ "Street" field exists in the xlsx file — verified');
    });
