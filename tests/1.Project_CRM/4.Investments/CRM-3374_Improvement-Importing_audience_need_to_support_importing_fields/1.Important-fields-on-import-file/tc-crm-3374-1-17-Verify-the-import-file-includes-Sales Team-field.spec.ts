 ...
 * Test Case ID: CRM-3374_1.17
 *
 * Summary: Verify the import file includes "Sales Team" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.17:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Sales Team" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.17 - Verify the import file includes "Sales Team" field', () => {
  ...
  test('CRM-3374_1.17: Verify the import file includes "Sales Team" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Sales Team" field exists in the xlsx file
    await test.step('Step 1: Verify the "Sales Team" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Sales Team" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'sales team');
      const hasSalesTeam = colIndex !== -1;

      if (hasSalesTeam) {
        console.log(`  ✓ "Sales Team" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasSalesTeam, 'Step 1: The downloaded xlsx template should contain "Sales Team" column').toBe(true);

      console.log('  ✓ "Sales Team" field exists in the xlsx file — verified');
    });
