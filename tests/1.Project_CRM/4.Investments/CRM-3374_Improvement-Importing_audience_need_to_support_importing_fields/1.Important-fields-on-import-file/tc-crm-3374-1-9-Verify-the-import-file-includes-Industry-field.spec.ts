 ...
 * Test Case ID: CRM-3374_1.9
 *
 * Summary: Verify the import file includes "Industry" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.9:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Industry" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.9 - Verify the import file includes "Industry" field', () => {
  ...
  test('CRM-3374_1.9: Verify the import file includes "Industry" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Industry" field exists in the xlsx file
    await test.step('Step 1: Verify the "Industry" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Industry" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'industry');
      const hasIndustry = colIndex !== -1;

      if (hasIndustry) {
        console.log(`  ✓ "Industry" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasIndustry, 'Step 1: The downloaded xlsx template should contain "Industry" column').toBe(true);

      console.log('  ✓ "Industry" field exists in the xlsx file — verified');
    });
