 ...
 * Test Case ID: CRM-3374_1.16
 *
 * Summary: Verify the import file includes "Source" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.16:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Source" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.16 - Verify the import file includes "Source" field', () => {
  ...
  test('CRM-3374_1.16: Verify the import file includes "Source" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Source" field exists in the xlsx file
    await test.step('Step 1: Verify the "Source" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Source" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'source');
      const hasSource = colIndex !== -1;

      if (hasSource) {
        console.log(`  ✓ "Source" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasSource, 'Step 1: The downloaded xlsx template should contain "Source" column').toBe(true);

      console.log('  ✓ "Source" field exists in the xlsx file — verified');
    });
