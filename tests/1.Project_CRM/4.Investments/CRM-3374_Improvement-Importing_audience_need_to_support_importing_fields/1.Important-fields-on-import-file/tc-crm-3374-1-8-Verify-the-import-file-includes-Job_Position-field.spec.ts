 ...
 * Test Case ID: CRM-3374_1.8
 *
 * Summary: Verify the import file includes "Job Position" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.8:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Job Position" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.8 - Verify the import file includes "Job Position" field', () => {
  ...
  test('CRM-3374_1.8: Verify the import file includes "Job Position" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Job Position" field exists in the xlsx file
    await test.step('Step 1: Verify the "Job Position" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Job Position" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'job position');
      const hasJobPosition = colIndex !== -1;

      if (hasJobPosition) {
        console.log(`  ✓ "Job Position" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasJobPosition, 'Step 1: The downloaded xlsx template should contain "Job Position" column').toBe(true);

      console.log('  ✓ "Job Position" field exists in the xlsx file — verified');
    });
