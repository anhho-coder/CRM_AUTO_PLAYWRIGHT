 ...
 * Test Case ID: CRM-3374_1.18
 *
 * Summary: Verify the import file includes "Salesperson" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.18:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Salesperson" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.18 - Verify the import file includes "Salesperson" field', () => {
  ...
  test('CRM-3374_1.18: Verify the import file includes "Salesperson" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Salesperson" field exists in the xlsx file
    await test.step('Step 1: Verify the "Salesperson" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Salesperson" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'salesperson');
      const hasSalesperson = colIndex !== -1;

      if (hasSalesperson) {
        console.log(`  ✓ "Salesperson" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasSalesperson, 'Step 1: The downloaded xlsx template should contain "Salesperson" column').toBe(true);

      console.log('  ✓ "Salesperson" field exists in the xlsx file — verified');
    });
