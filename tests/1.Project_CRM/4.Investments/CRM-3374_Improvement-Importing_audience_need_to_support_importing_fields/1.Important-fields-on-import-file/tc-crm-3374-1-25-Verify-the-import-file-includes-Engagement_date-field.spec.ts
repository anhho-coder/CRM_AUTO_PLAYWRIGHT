 ...
 * Test Case ID: CRM-3374_1.25
 *
 * Summary: Verify the import file includes "Engagement date" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.25:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Engagement date" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.25 - Verify the import file includes "Engagement date" field', () => {
  ...
  test('CRM-3374_1.25: Verify the import file includes "Engagement date" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Engagement date" field exists in the xlsx file
    await test.step('Step 1: Verify the "Engagement date" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Engagement date" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'engagement date');
      const hasEngagementDate = colIndex !== -1;

      if (hasEngagementDate) {
        console.log(`  ✓ "Engagement date" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasEngagementDate, 'Step 1: The downloaded xlsx template should contain "Engagement date" column').toBe(true);

      console.log('  ✓ "Engagement date" field exists in the xlsx file — verified');
    });
