 ...
 * Test Case ID: CRM-3374_1.22
 *
 * Summary: Verify the import file includes "Wufoo Hash" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.22:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Wufoo Hash" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.22 - Verify the import file includes "Wufoo Hash" field', () => {
  ...
  test('CRM-3374_1.22: Verify the import file includes "Wufoo Hash" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Wufoo Hash" field exists in the xlsx file
    await test.step('Step 1: Verify the "Wufoo Hash" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Wufoo Hash" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'wufoo hash');
      const hasWufooHash = colIndex !== -1;

      if (hasWufooHash) {
        console.log(`  ✓ "Wufoo Hash" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasWufooHash, 'Step 1: The downloaded xlsx template should contain "Wufoo Hash" column').toBe(true);

      console.log('  ✓ "Wufoo Hash" field exists in the xlsx file — verified');
    });
