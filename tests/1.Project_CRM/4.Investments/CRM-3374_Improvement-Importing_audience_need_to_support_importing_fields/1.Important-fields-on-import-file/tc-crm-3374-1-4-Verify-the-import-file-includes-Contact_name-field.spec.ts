 ...
 * Test Case ID: CRM-3374_1.4
 *
 * Summary: Verify the import file includes "Contact name" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.4:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Contact name" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.4 - Verify the import file includes "Contact name" field', () => {
  ...
  test('CRM-3374_1.4: Verify the import file includes "Contact name" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Contact name" field exists in the xlsx file
    await test.step('Step 1: Verify the "Contact name" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Contact name" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'contact name');
      const hasContactName = colIndex !== -1;

      if (hasContactName) {
        console.log(`  ✓ "Contact name" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasContactName, 'Step 1: The downloaded xlsx template should contain "Contact name" column').toBe(true);

      console.log('  ✓ "Contact name" field exists in the xlsx file — verified');
    });
