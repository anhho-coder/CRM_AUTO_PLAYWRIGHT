 ...
 * Test Case ID: CRM-3374_1.12
 *
 * Summary: Verify the import file includes "City" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.12:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "City" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.12 - Verify the import file includes "City" field', () => {
  ...
  test('CRM-3374_1.12: Verify the import file includes "City" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "City" field exists in the xlsx file
    await test.step('Step 1: Verify the "City" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "City" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'city');
      const hasCity = colIndex !== -1;

      if (hasCity) {
        console.log(`  ✓ "City" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasCity, 'Step 1: The downloaded xlsx template should contain "City" column').toBe(true);

      console.log('  ✓ "City" field exists in the xlsx file — verified');
    });
