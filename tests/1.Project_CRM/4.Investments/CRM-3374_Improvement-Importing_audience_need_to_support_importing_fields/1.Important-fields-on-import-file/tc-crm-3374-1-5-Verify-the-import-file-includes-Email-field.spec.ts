 ...
 * Test Case ID: CRM-3374_1.5
 *
 * Summary: Verify the import file includes "Email" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.5:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Email" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.5 - Verify the import file includes "Email" field', () => {
  ...
  test('CRM-3374_1.5: Verify the import file includes "Email" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Email" field exists in the xlsx file
    await test.step('Step 1: Verify the "Email" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Email" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'email');
      const hasEmail = colIndex !== -1;

      if (hasEmail) {
        console.log(`  ✓ "Email" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasEmail, 'Step 1: The downloaded xlsx template should contain "Email" column').toBe(true);

      console.log('  ✓ "Email" field exists in the xlsx file — verified');
    });
