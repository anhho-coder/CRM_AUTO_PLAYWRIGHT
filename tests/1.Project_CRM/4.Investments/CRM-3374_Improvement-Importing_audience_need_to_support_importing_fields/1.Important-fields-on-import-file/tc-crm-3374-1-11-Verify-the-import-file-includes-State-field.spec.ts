 ...
 * Test Case ID: CRM-3374_1.11
 *
 * Summary: Verify the import file includes "State" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.11:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "State" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.11 - Verify the import file includes "State" field', () => {
  ...
  test('CRM-3374_1.11: Verify the import file includes "State" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "State" field exists in the xlsx file
    await test.step('Step 1: Verify the "State" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "State" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'state');
      const hasState = colIndex !== -1;

      if (hasState) {
        console.log(`  ✓ "State" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasState, 'Step 1: The downloaded xlsx template should contain "State" column').toBe(true);

      console.log('  ✓ "State" field exists in the xlsx file — verified');
    });
