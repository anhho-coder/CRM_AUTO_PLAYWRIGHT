 ...
 * Test Case ID: CRM-3374_1.15
 *
 * Summary: Verify the import file includes "Notes" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_1\.15:" --project=chromium
 * 
 * Pre-condition:
 * I. Download template:
 ...
 *
 * Steps to reproduce:
 * 1. The "Notes" field exist in the xlsx file
 *
 * Tear down (Clean up test data):
 * 1. Delete the downloaded xlsx file
 */

test.describe('CRM-3374_1.15 - Verify the import file includes "Notes" field', () => {
  ...
  test('CRM-3374_1.15: Verify the import file includes "Notes" field', async ({ page }, testInfo) => {
    ...
    // Step 1: Verify "Notes" field exists in the xlsx file
    await test.step('Step 1: Verify the "Notes" field exists in the downloaded xlsx file', async () => {
      console.log('\n=== STEP 1: VERIFY "Notes" FIELD IN XLSX ===');
      ...
      const colIndex = headers.findIndex(h => h.toLowerCase() === 'notes');
      const hasNotes = colIndex !== -1;

      if (hasNotes) {
        console.log(`  ✓ "Notes" field found at column ${toColumnLetter(colIndex)} (index ${colIndex})`);
      }

      expect(hasNotes, 'Step 1: The downloaded xlsx template should contain "Notes" column').toBe(true);

      console.log('  ✓ "Notes" field exists in the xlsx file — verified');
    });
