 ...
 * Test Case ID: CRM-3374_2.3
 *
 * Summary: Verify error message displays when entering invalid value at "State" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_2\.3:" --project=chromium
 *
 * Pre-condition:
 ...
 *         - Phone         = 1234125125
 *         - Country       = China
 *         - State         = TEST                                          (State_Contact#1 — invalid value)
...
test.describe('CRM-3374_2.3 - Verify error message displays when entering invalid value at "State" field', () => {
  ...
  test('CRM-3374_2.3: Verify error message displays when entering invalid value at "State" field', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ state: 'TEST', investmentId: 'test' });
      ...
      console.log(`  ✓ State_Contact#1       : ${audienceData.state}  (invalid — invalid State)`);
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "State" is not valid', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "State" is not valid';
      ...
      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"State" is not valid');
    ...
    await test.step('Step 10: Verify "Email" row has no value of Email_Contact#1', async () => {
      ...
      // Step 10.1: Verify Email_Contact#1 is NOT present (import was rejected due to invalid State)
