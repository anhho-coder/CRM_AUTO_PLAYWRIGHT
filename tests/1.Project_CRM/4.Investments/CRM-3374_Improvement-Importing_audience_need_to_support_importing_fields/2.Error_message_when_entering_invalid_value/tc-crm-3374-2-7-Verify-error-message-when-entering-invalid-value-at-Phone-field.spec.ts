 ...
 * Test Case ID: CRM-3374_2.7
 *
 * Summary: Verify error message displays when entering invalid value at "Phone" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_2\.7:" --project=chromium
 ...
 *         - Phone         = TEXT                                          (Phone_Contact#1 — invalid value)
 *         - Country       = China
 ...
 *    "Lines [2] \"Phone\" format is invalid (allowed characters: +, space, (), -, digits)"
...
test.describe('CRM-3374_2.7 - Verify error message displays when entering invalid value at "Phone" field', () => {
  ...
  test('CRM-3374_2.7: Verify error message displays when entering invalid value at "Phone" field', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);

    testInfo.annotations.push({ type: 'defect', description: 'CRM-9857 - fixed, re-running to verify' });
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ phone: 'TEXT', investmentId: 'test' });
      ...
      console.log(`  ✓ Phone_Contact#1          : ${audienceData.phone}  (invalid — invalid Phone)`);
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "Phone" format is invalid (allowed characters: +, space, (), -, digits)', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "Phone" format is invalid (allowed characters: +, space, (), -, digits)';

      const actualErrorText = await investmentPage.waitForImportErrorMessage('format is invalid');

      console.log(`  Actual error message  : "${actualErrorText}"`);
      console.log(`  Expected error message: "${expectedError}"`);

      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"Phone" format is invalid (allowed characters: +, space, (), -, digits)');
    ...
    await test.step('Step 10: Verify "Email" row has no value of Email_Contact#1', async () => {
      ...
      // Step 10.1: Verify Email_Contact#1 is NOT present (import was rejected due to invalid Phone)
