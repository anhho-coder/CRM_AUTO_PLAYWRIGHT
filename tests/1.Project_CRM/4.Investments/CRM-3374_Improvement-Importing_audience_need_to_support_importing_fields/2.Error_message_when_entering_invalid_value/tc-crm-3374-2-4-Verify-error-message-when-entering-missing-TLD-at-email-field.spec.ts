 ...
 * Test Case ID: CRM-3374_2.4
 *
 * Summary: Verify error message displays when entering invalid value at "Email" field (missing TLD)
 *
 * Command to run:
 ...
 *         - Email         = test@invalid-email                            (Email_Contact#1 — invalid, missing TLD)
...
test.describe('CRM-3374_2.4 - Verify error message displays when entering invalid value at "Email" field', () => {
  ...
  test('CRM-3374_2.4: Verify error message displays when entering invalid value at "Email" field (missing TLD)', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ email: 'test@invalid-email', investmentId: 'test' });
      ...
      console.log(`  ✓ Email_Contact#1       : ${audienceData.emailContact1}  (invalid — missing TLD)`);
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "Email" is not valid', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "Email" is not valid';
      ...
      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"Email" is not valid');
    ...
    await test.step('Step 10: Verify "Email" row has no value of Email_Contact#1', async () => {
      ...
      // Step 10.1: Verify Email_Contact#1 is NOT present (import was rejected due to invalid Email — missing TLD)
