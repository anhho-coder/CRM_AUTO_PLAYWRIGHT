 ...
 * Test Case ID: CRM-3374_2.6
 *
 * Summary: Verify error message displays when entering invalid value at "Salesperson" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_2\.6:" --project=chromium
 *
 * Pre-condition:
 ...
 *         - Salesperson   = TEST                                          (Salesperson_Contact#1 — invalid value)
 *         - Tags          = test
 *         - Investment ID = test
 ...
 *    "Lines [2] \"Salesperson\" is not valid"
...
test.describe('CRM-3374_2.6 - Verify error message displays when entering invalid value at "Salesperson" field', () => {
  ...
  test('CRM-3374_2.6: Verify error message displays when entering invalid value at "Salesperson" field', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ salesperson: 'TEST', investmentId: 'test' });
      ...
      console.log(`  ✓ Salesperson_Contact#1    : ${audienceData.salesperson}  (invalid — invalid Salesperson)`);
    });
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "Salesperson" is not valid', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "Salesperson" is not valid';
      ...
      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"Salesperson" is not valid');
    ...
    await test.step('Step 10: Verify "Email" row has no value of Email_Contact#1', async () => {
      ...
      // Step 10.1: Verify Email_Contact#1 is NOT present (import was rejected due to invalid Salesperson)
