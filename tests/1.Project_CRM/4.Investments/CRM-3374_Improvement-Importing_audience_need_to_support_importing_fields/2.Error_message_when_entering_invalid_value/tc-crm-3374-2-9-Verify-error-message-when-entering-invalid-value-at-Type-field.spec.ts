 ...
 * Test Case ID: CRM-3374_2.9
 *
 * Summary: Verify error message displays when entering invalid value at "Type" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_2\.9:" --project=chromium
 ...
 *         - Tags          = test
 *         - Type          = TEST                                          (Type_Contact#1 — invalid value)
 *         - Investment ID = test
...
test.describe('CRM-3374_2.9 - Verify error message displays when entering invalid value at "Type" field', () => {
  ...
  test('CRM-3374_2.9: Verify error message displays when entering invalid value at "Type" field', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ type: 'TEST', investmentId: 'test' });
      ...
      console.log(`  ✓ Type_Contact#1        : TEST  (invalid — invalid Type)`);
    });
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "Type" is not valid', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "Type" is not valid';
      ...
      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"Type" is not valid');
    ...
    await test.step('Step 10: Verify "Email" row has no value of Email_Contact#1', async () => {
      ...
      // Step 10.1: Verify Email_Contact#1 is NOT present (import was rejected due to invalid Type)
