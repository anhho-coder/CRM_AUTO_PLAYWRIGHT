 ...
 * Test Case ID: CRM-3374_2.2
 *
 * Summary: Verify error message displays when entering invalid value at "Country" field
 *
 * Command to run:
 * npx playwright test --grep "CRM-3374_2\.2:" --project=chromium
 *
 * Pre-condition:
 ...
 *         - Phone         = 1234125125
 *         - Country       = TEST                                          (Country_Contact#1 — invalid value)
...
test.describe('CRM-3374_2.2 - Verify error message displays when entering invalid value at "Country" field', () => {
  ...
  test('CRM-3374_2.2: Verify error message displays when entering invalid value at "Country" field', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 5: Create an Import Audience file (CSV-Audience-copy1.csv)', async () => {
      ...
      audienceData = await investmentPage.createImportAudienceXLSXFile({ country: 'TEST', investmentId: 'test' });
      ...
      console.log(`  ✓ Country_Contact#1     : ${audienceData.country}  (invalid — invalid Country)`);
    ...
    await test.step('Step 8: Verify the Error message displays as: Lines [2] "Country" is not valid', async () => {
      console.log('=== STEP 8: VERIFY ERROR MESSAGE ===');

      const expectedError = 'Lines [2] "Country" is not valid';
      ...
      expect(actualErrorText, 'Step 8: Error message mismatch').toContain('"Country" is not valid');
    ...
    await test.step('Step 10: Verify "Email" row has no value of Email_Contact#1', async () => {
      ...
      // Step 10.1: Verify Email_Contact#1 is NOT present (import was rejected due to invalid Country)
