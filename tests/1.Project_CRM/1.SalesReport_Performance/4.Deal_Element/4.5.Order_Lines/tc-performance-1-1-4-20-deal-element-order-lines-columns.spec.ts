...
/**
 * Deal Element data verification - Order Lines columns
 * Test Case ID: TC.Performance.1.1.4.20
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.20:" --project=SalesReport_Performance
 *
 * Objective: Verify the Order Lines tab shows exactly the 16 expected columns, with the expected captions, in
 * the expected left-to-right order - including the discount columns Special Discount Amount,
 * Promo Discount Amount, Partner Discount and Partner Discount Amount.
 *
 * Pre-condition:
 ...
 *
 * Verification:
 * - The Order Lines list shows exactly 16 named columns
 * - The captions and their order match the expected list (Product first, Subtotal last)
 *
...
const TC = 'TC.Performance.1.1.4.20';
...
/**
 * The Order Lines columns as pre-production renders them (verified 2026-09-17). The leading
 * drag-handle column carries no caption and is not part of the list.
 *
 * NOTE: the partner-discount columns are captioned "Partner Discount" and "Partner Discount
 * Amount" on the screen - there is no "(%)" suffix on either of them.
 */
...
test.describe(`${TC} - Order Lines columns`, () => {
  ...
  test(`${TC}: Verify the name, number and order of the Order Lines columns`, async ({ page }) => {
