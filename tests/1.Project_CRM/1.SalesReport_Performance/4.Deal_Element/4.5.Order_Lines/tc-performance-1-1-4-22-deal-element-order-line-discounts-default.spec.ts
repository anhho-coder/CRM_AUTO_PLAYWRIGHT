...
/**
 * Deal Element data verification - Order line discount columns
 * Test Case ID: TC.Performance.1.1.4.22
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.22:" --project=SalesReport_Performance
 *
 * Objective: Verify that an order line added without any discount reports 0.00 in all five discount columns
 * - Discount (%), Special Discount Amount, Promo Discount Amount, Partner Discount and Partner
 * Discount Amount.
 *
 * Pre-condition:
 ...
 * Verification:
 * - Discount (%) = 0.00
 * - Special Discount Amount = 0.00
 * - Promo Discount Amount = 0.00
 * - Partner Discount = 0.00
 * - Partner Discount Amount = 0.00
 *
...
const TC = 'TC.Performance.1.1.4.22';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the order line discount columns',
};
...
test.describe(`${TC} - Order line discount columns`, () => {
  ...
  test(`${TC}: Verify every discount column of the order line is 0.00 when no discount applies`, async ({ page }) => {
