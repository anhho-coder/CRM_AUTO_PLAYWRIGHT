...
/**
 * Deal Element data verification - Distributor and Reseller Discount defaults
 * Test Case ID: TC.Performance.1.1.4.16
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.16:" --project=SalesReport_Performance
 *
 * Objective: Verify that an Opportunity without a Distributor and without a Reseller produces a Deal Element
 * whose two partner-discount fields read 0.00 and grant no amount.
 *
 * Pre-condition:
 ...
 *
 * Verification:
 * - Distributor Discount (%) = 0.00 ( $ 0.00 )
 * - Reseller Discount (%) = 0.00 ( $ 0.00 )
 *
...
const TC = 'TC.Performance.1.1.4.16';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Distributor and Reseller discounts',
};

test.describe(`${TC} - Distributor and Reseller Discount defaults`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify Distributor Discount (%) and Reseller Discount (%) are 0.00 when the Opportunity carries no partner`, async ({ page }) => {
