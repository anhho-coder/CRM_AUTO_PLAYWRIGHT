...
/**
 * Deal Element data verification - Delivery Address taken from the Opportunity
 * Test Case ID: TC.Performance.1.1.4.9
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.9:" --project=SalesReport_Performance
 *
 * Objective: Verify the Deal Element Delivery Address is not left empty and carries the Company Odoo created
 * for the Opportunity from the e-mail domain.
 *
 * Pre-condition:
 ...
 * - Delivery Address = the Company shown on the Opportunity
 *
...
const TC = 'TC.Performance.1.1.4.9';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Delivery Address against the Opportunity',
};

test.describe(`${TC} - Delivery Address taken from the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the Delivery Address is filled and matches the Customer of the Opportunity`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      record('Delivery Address is filled', 'filled', deliveryAddress.length > 0 ? 'filled' : '(empty)');
      record('Delivery Address = Opportunity Company', oppCompany, deliveryAddress);
      printVerify();

      expect(deliveryAddress, 'the Delivery Address must not be empty').not.toBe('');
      expect(deliveryAddress, 'the Delivery Address must be the Company of the Opportunity').toBe(
        oppCompany
      );
    });
  });
