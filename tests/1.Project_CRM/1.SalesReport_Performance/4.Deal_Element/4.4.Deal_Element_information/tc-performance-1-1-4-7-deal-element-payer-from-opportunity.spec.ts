...
/**
 * Deal Element data verification - Payer taken from the Opportunity
 * Test Case ID: TC.Performance.1.1.4.7
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.7:" --project=SalesReport_Performance
 *
 * Objective: Verify the Deal Element Payer is not left empty and carries the Company Odoo created for the
 ...
 * Verification:
 * - Payer is filled
 * - Payer = the Company shown on the Opportunity
 *
...
const TC = 'TC.Performance.1.1.4.7';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Payer against the Opportunity',
};

test.describe(`${TC} - Payer taken from the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the Payer is filled and matches the Customer of the Opportunity`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      record('Payer is filled', 'filled', payer.length > 0 ? 'filled' : '(empty)');
      record('Payer = Opportunity Company', oppCompany, payer);
      printVerify();

      expect(payer, 'the Payer must not be empty').not.toBe('');
      expect(payer, 'the Payer must be the Company of the Opportunity').toBe(oppCompany);
    });
  });
});
