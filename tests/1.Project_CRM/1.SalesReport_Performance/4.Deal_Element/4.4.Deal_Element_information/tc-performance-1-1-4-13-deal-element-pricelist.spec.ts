...
/**
 * Deal Element data verification - Pricelist kept by the Deal Element
 * Test Case ID: TC.Performance.1.1.4.13
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.13:" --project=SalesReport_Performance
 *
 * Objective: Verify the Pricelist field carries "Public Pricelist_USD (USD)" - the currency every amount on
...
const TC = 'TC.Performance.1.1.4.13';
...
test.describe(`${TC} - Pricelist kept by the Deal Element`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the Pricelist selected on the Deal Element is kept after SAVE`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const pricelist = await dealElementPage.getFieldDisplayValue('pricelist_id');

      record('Pricelist', 'Public Pricelist_USD (USD)', pricelist);
      printVerify();

      expect(pricelist, 'the Deal Element must keep the selected Pricelist').toBe(
        'Public Pricelist_USD (USD)'
      );
    });
  });
});
