...
/**
 * Deal Element data verification - Invoice Address taken from the Opportunity
 * Test Case ID: TC.Performance.1.1.4.8
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.8:" --project=SalesReport_Performance
 *
 * Objective: Verify the Deal Element Invoice Address is not left empty and carries the Company Odoo created
 ...
 * - Invoice Address = the Company shown on the Opportunity
 *
...
const TC = 'TC.Performance.1.1.4.8';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Invoice Address against the Opportunity',
};

test.describe(`${TC} - Invoice Address taken from the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the Invoice Address is filled and matches the Customer of the Opportunity`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      record('Invoice Address is filled', 'filled', invoiceAddress.length > 0 ? 'filled' : '(empty)');
      record('Invoice Address = Opportunity Company', oppCompany, invoiceAddress);
      printVerify();

      expect(invoiceAddress, 'the Invoice Address must not be empty').not.toBe('');
      expect(invoiceAddress, 'the Invoice Address must be the Company of the Opportunity').toBe(
        oppCompany
      );
    });
  });
