...
/**
 * Deal Element data verification - Total in Company Currency
 * Test Case ID: TC.Performance.1.1.4.17
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.17:" --project=SalesReport_Performance
 *
 * Objective: Verify the Total in Company Currency is filled and reports the same amount as the single order
 * line, the Deal Element being priced in the company currency (USD).
 *
 * Pre-condition:
 ...
 * - Total in Company Currency = the order line Subtotal
 *
...
const TC = 'TC.Performance.1.1.4.17';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying Total in Company Currency against the Order Lines',
};
...
test.describe(`${TC} - Total in Company Currency`, () => {
  ...
  test(`${TC}: Verify Total in Company Currency equals the total of the Order Lines`, async ({ page }) => {
