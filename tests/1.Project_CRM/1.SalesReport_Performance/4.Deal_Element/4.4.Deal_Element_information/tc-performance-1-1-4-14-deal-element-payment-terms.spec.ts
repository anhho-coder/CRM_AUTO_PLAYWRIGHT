...
/**
 * Deal Element data verification - Payment Terms kept by the Deal Element
 * Test Case ID: TC.Performance.1.1.4.14
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.14:" --project=SalesReport_Performance
 *
 * Objective: Verify the Payment Terms field carries the "Immediate Payment" term selected before SAVE.
 *
 * Pre-condition:
...
const TC = 'TC.Performance.1.1.4.14';
...
test.describe(`${TC} - Payment Terms kept by the Deal Element`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the Payment Terms selected on the Deal Element is kept after SAVE`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const paymentTerms = await dealElementPage.getFieldDisplayValue('payment_term_id');

      record('Payment Terms', DATA.paymentTerm, paymentTerms);
      printVerify();

      expect(paymentTerms, 'the Deal Element must keep the selected Payment Terms').toBe(
        DATA.paymentTerm
      );
    });
  });
