...
/**
 * Deal Element data verification - Fields that stay empty on creation
 * Test Case ID: TC.Performance.1.1.4.19
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.19:" --project=SalesReport_Performance
 *
 * Objective: Verify a Deal Element that has only been created (not confirmed, no promotion, no purchase
...
const TC = 'TC.Performance.1.1.4.19';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the fields that must stay empty',
};

test.describe(`${TC} - Fields that stay empty on creation`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify Confirmation Date, Promotion and PO are empty on a newly created Deal Element`, async ({ page }) => {
