...
/**
 * Deal Element data verification - Reseller and Distributor taken from the Opportunity
 * Test Case ID: TC.Performance.1.1.4.11
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.11:" --project=SalesReport_Performance
 *
 * Objective: Verify that an Opportunity carrying a Reseller and a Distributor produces a Deal Element whose
 * Reseller and Distributor are those same two partners, and whose contact fields stay empty because
 * neither partner record carries a contact.
 *
 * Pre-condition:
 ...
 *
 * Verification:
 * - Reseller = the Reseller of the Opportunity
 * - Distributor = the Distributor of the Opportunity
...
const TC = 'TC.Performance.1.1.4.11';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Reseller and the Distributor against the Opportunity',
};

test.describe(`${TC} - Reseller and Distributor taken from the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the Reseller and the Distributor are filled and match the Opportunity`, async ({ page }) => {
