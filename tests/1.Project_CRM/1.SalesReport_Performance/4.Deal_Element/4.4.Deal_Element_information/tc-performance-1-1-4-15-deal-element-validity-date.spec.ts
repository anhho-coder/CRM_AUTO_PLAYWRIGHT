...
/**
 * Deal Element data verification - Validity default
 * Test Case ID: TC.Performance.1.1.4.15
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.15:" --project=SalesReport_Performance
 *
 * Objective: Verify the Validity field is filled automatically and lands 30 days after the creation day, in
 * MM/DD/YYYY.
 *
 * Pre-condition:
 ...
 *
 * Verification:
 * - Validity is filled and formatted MM/DD/YYYY
 * - Validity is 30 days after today (29-31 accepted: the server and the runner can sit in
 *   different time zones, which shifts the day by one)
 *
...
const TC = 'TC.Performance.1.1.4.15';
...
test.describe(`${TC} - Validity default`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the Validity date defaults to 30 days after the day the Deal Element is created`, async ({ page }) => {
