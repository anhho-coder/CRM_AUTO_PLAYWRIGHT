...
/**
 * Deal Element data verification - Send mail and Online Payment defaults
 * Test Case ID: TC.Performance.1.1.4.18
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.18:" --project=SalesReport_Performance
 *
 * Objective: Verify the two checkboxes of the information area keep their defaults on a Deal Element that was
 * just created: Send mail off, Online Payment on.
 *
 * Pre-condition:
...
const TC = 'TC.Performance.1.1.4.18';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Send mail and Online Payment checkboxes',
};

test.describe(`${TC} - Send mail and Online Payment defaults`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify Send mail is unticked and Online Payment is ticked on a new Deal Element`, async ({ page }) => {
