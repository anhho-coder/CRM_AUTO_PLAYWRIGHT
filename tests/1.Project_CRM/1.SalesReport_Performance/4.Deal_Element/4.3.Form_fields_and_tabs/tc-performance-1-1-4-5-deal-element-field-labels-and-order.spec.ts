...
/**
 * Deal Element data verification - Field names and order of the information area
 * Test Case ID: TC.Performance.1.1.4.5
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.5:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Deal Element information area carries exactly the 19 expected fields, with the
 * expected captions, in the expected top-to-bottom order.
 *
 * Pre-condition:
 ...
 *
 * Verification:
 * - The information area shows exactly 19 labelled fields
 * - The captions and their order match the expected list (Payer first, PO last)
 *
...
const TC = 'TC.Performance.1.1.4.5';
...
/**
 * The Deal Element information area as pre-production renders it, read top-left down and
 * then top-right down. Grounded on the live form on 2026-09-17 - the technical fields Odoo
 * keeps hidden behind attrs are not part of it.
 */
...
test.describe(`${TC} - Field names and order of the information area`, () => {
  ...
  test(`${TC}: Verify the field names and their order in the Deal Element information area`, async ({ page }) => {
