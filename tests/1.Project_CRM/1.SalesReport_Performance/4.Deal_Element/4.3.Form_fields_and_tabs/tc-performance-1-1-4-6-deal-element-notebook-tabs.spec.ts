...
/**
 * Deal Element data verification - Notebook tabs of the Deal Element
 * Test Case ID: TC.Performance.1.1.4.6
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.6:" --project=SalesReport_Performance
 *
 * Objective: Verify the Deal Element carries exactly the three notebook tabs Order Lines, Optional Products
 * and Other Information, in that order.
 *
 * Pre-condition:
 ...
 *
 * Verification:
 * - The notebook shows exactly 3 tabs
 * - The tabs are Order Lines, Optional Products, Other Information, in that order
 *
...
const TC = 'TC.Performance.1.1.4.6';
...
const EXPECTED_TABS = ['Order Lines', 'Optional Products', 'Other Information'];

test.describe(`${TC} - Notebook tabs of the Deal Element`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the name, number and order of the Deal Element notebook tabs`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const tabs = await dealElementPage.getNotebookTabs();

      record('Number of notebook tabs', EXPECTED_TABS.length, tabs.length);
      record('Notebook tabs, in order', EXPECTED_TABS.join(' | '), tabs.join(' | '));
      printVerify();

      expect(tabs, 'the Deal Element must carry exactly the three expected tabs, in order').toEqual(
        EXPECTED_TABS
      );
    });
  });
