...
/**
 * Deal Element data verification - Buttons on the saved Deal Element
 * Test Case ID: TC.Performance.1.1.4.4
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.4:" --project=SalesReport_Performance
 *
 * Objective: Verify that once the Deal Element is saved the control panel switches to EDIT + Action and
 * the "NEW QUOTATION" header button is still the only header button.
 *
 * Pre-condition:
 ...
 *
 * Verification:
 * - The control panel shows exactly 2 buttons: EDIT, Action (in that order)
...
const TC = 'TC.Performance.1.1.4.4';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the buttons on the saved Deal Element',
};

test.describe(`${TC} - Buttons on the saved Deal Element`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the buttons offered by a saved Deal Element`, async ({ page }) => {
