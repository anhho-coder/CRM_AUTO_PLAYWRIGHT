...
/**
 * Deal Element data verification - Buttons on the new Deal Element
 * Test Case ID: TC.Performance.1.1.4.3
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.3:" --project=SalesReport_Performance
 *
 * Objective: Verify that a Deal Element opened from the Opportunity offers exactly the SAVE and DISCARD
 * control-panel buttons and the single "NEW QUOTATION" header button - no more, no fewer.
 *
 * Pre-condition:
 ...
 *
 * Verification:
 * - The control panel shows exactly 2 buttons: SAVE, DISCARD (in that order)
 * - The form header shows exactly 1 button: NEW QUOTATION
 *
...
const TC = 'TC.Performance.1.1.4.3';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the buttons on the unsaved Deal Element',
};

test.describe(`${TC} - Buttons on the new Deal Element`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  ...
  test(`${TC}: Verify the buttons offered by a new, unsaved Deal Element`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      record('Form header buttons, in order', 'NEW QUOTATION', headerButtons.join(' | '));
      printVerify();
      ...
      expect(headerButtons, 'the Deal Element header offers NEW QUOTATION only').toEqual([
        'NEW QUOTATION',
      ]);
    });
  });
