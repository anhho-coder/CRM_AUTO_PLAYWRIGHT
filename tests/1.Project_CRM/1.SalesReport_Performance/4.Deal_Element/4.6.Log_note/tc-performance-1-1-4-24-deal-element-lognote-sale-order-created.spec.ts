...
/**
 * Deal Element data verification - Log note - Sale Order created
 * Test Case ID: TC.Performance.1.1.4.24
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.4\\.24:" --project=SalesReport_Performance
 *
 * Objective: Verify that creating a Deal Element writes the "Sale Order created" note in the chatter, and that
 * a freshly created Deal Element carries exactly the two creation notes.
 *
 * Pre-condition:
 ...
 *
 * Verification:
 * - The chatter carries a note reading exactly "Sale Order created"
 * - A freshly created Deal Element carries exactly 2 notes (the field summary and this one)
 *
...
const TC = 'TC.Performance.1.1.4.24';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the "Sale Order created" log note',
};
...
test.describe(`${TC} - Log note - Sale Order created`, () => {
  ...
  test(`${TC}: Verify the Log note reports "Sale Order created" when the Deal Element is created`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const note = await dealElementPage.waitForChatterMessage(/^Sale Order created$/m);
