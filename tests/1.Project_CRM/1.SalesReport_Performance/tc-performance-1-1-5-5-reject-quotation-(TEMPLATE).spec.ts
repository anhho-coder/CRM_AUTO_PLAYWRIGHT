...
/**
 * Performance Test - Reject Quotation
 * Test Case ID: TC.Performance.1.1.5.5
 * 
 * Summary: Verify the consumer time for rejecting Quotation is less than 1 min
 *
 * Command to run:
 * npx playwright test --grep "TC\.Performance\.1\.1\.5\.5:" --project=chromium
 *
 * Pre-condition:
 ...
 *    + Ordered Qty = 20 (This will trigger the Approval rule for Quotation > 4k$)
 * 11. Finally, press "SAVE" button on the top page and wait
 * 12. Press "NEW QUOTATION" button and wait
 * 
 * Steps run:
 * 1. Press "TO APPROVE" button
 * 2. Copy the URL and close the current browser
 * 3. Open another browser and login as Max account
 * 4. Paste the URL at step#2 to open the Quotation
 * 5. Play as Max account and press "REJECT" button
 * 6. Once the dialog "Reject Reason" appear, enter "TEST" value
 * 7. Press "REJECT" button on the dialog "Reject Reason"
 * 
 * Verification:
 * - Time consuming after pressing "REJECT" button on the dialog "Reject Reason" takes less than 1 minute
 */

test.describe('TC.Performance.1.1.5.5 - Reject Quotation Performance', () => {
  const PERFORMANCE_THRESHOLD = 60000; // 1 minute in milliseconds
    ...
    test('TC.Performance.1.1.5.5: Verify consumer time for rejecting Quotation is less than 1 min', async ({ page, browser }, testInfo) => {
      ...
      // Step 11: Add a product in Order Lines section with Qty = 20
      await test.step('Step 11: At "Order Lines" section - Add a product and set Ordered Qty = 20', async () => {
        stepStartTime = Date.now();
        console.log('Step 11: Adding product with Qty=20');
        ...
        // Set Ordered Qty to 20
        console.log('  - Setting Ordered Qty to 20...');
        ...
        console.log('  - Ordered Qty: Set to 20');
  ...
  // ============================================================
      ...
      // Steps run - Step 1 & 2: Press "TO APPROVE" button, copy URL and close browser
      let quotationUrl: string;
      await test.step('Steps run - Step 1 & 2: Press "TO APPROVE" button, copy the URL and close the current browser', async () => {
        stepStartTime = Date.now();
        ...
        // Capture screenshot from Thomas's browser
        await CommonUtils.captureAndAttachScreenshot(
          page,
          testInfo,
          'Thomas - Quotation created and TO APPROVE requested'
        );
      ...
      await test.step('Steps run - Step 4: Paste the URL at step#2 to open the Quotation', async () => {
        ...
        // Capture screenshot from Max's browser after opening quotation
        await CommonUtils.captureAndAttachScreenshot(
          maxPage,
          testInfo,
          'Max - Quotation opened for approval'
        );
    ...
    // Steps run - Step 5, 6 & 7: Press "REJECT" button, handle dialog and measure performance
    let rejectTime: number = 0;
    let rejectSeconds: string = '0';
    await test.step('Steps run - Step 5, 6 & 7: Play as Max account, press "REJECT" button, enter "TEST" in dialog and press "REJECT" on dialog', async () => {
      console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('PERFORMANCE TEST - Step 3: Rejecting Quotation as Max (Performance Measurement)');
      console.log('  - Step 5: Clicking REJECT button');
      console.log('  - Step 6: Entering "TEST" in Reject Reason dialog');
      console.log('  - Step 7: Clicking REJECT button on dialog');
      
      // START PERFORMANCE MEASUREMENT - Click REJECT button and handle dialog
      const maxQuotationPage = new QuotationPage(maxPage);
      rejectTime = await maxQuotationPage.clickReject();
      rejectSeconds = (rejectTime / 1000).toFixed(2);
      ...
      await CommonUtils.captureAndAttachScreenshot(
        maxPage,
        testInfo,
        `Quotation Rejected - Time: ${rejectSeconds}s`
      );
      
      performanceMetrics['REJECT Process (Max)'] = rejectTime;
      console.log(`\n⏱️  Time after pressing "REJECT" button on dialog: ${rejectSeconds} seconds`);
    });
    
    // Verification: Ensure time after pressing REJECT button is less than 1 minute
    console.log('\n📋 Performance Verification:');
    console.log(`   - Measured time: ${rejectSeconds} seconds`);
    console.log(`   - Threshold: 60 seconds`);
    
    if (rejectTime < PERFORMANCE_THRESHOLD) {
      const margin = (60 - parseFloat(rejectSeconds)).toFixed(2);
      const percentage = ((parseFloat(rejectSeconds) / 60) * 100).toFixed(2);
      console.log(`   - Result: PASS ✓`);
      console.log(`   - Margin: ${margin} seconds under threshold`);
      console.log(`   - Performance: ${percentage}% of threshold`);
    } else {
      const overrun = (parseFloat(rejectSeconds) - 60).toFixed(2);
      console.log(`   - Result: FAIL ✗`);
      console.log(`   - Overrun: ${overrun} seconds over threshold`);
    }
    
    expect(rejectTime).toBeLessThan(PERFORMANCE_THRESHOLD);
    ...
    console.log(`${'Total Test Execution Time'.padEnd(30)} ${totalSeconds.padStart(8)}s`);
    console.log('━'.repeat(50));
    
    // Capture final screenshots from both browsers
    await test.step('Capture final screenshots and performance summary', async () => {
      await CommonUtils.captureAndAttachScreenshot(
        maxPage,
        testInfo,
        `Max - Performance Test Passed - Rejection Time: ${rejectSeconds}s`
      );
      
      await CommonUtils.captureAndAttachScreenshot(
        page,
        testInfo,
        `Thomas - Performance Test Passed - Rejection Time: ${rejectSeconds}s`
      );
    });
    
    // Attach videos in order: Thomas first, then Max
    await test.step('Attach videos to report', async () => {
      // Get Max's video path before closing context
      const maxVideoPath = await maxPage.video()?.path();
      
      // Close Max's context to finalize video
      await maxContext.close();
      console.log('✓ Max\'s browser context closed');
      
      // Attach Max's video to test report (will appear after Thomas's video which is auto-attached)
      if (maxVideoPath) {
        await testInfo.attach('max-manager-video', {
          path: maxVideoPath,
          contentType: 'video/webm'
        });
        console.log('✓ Max\'s video attached to report');
      }
    });
  });
});
