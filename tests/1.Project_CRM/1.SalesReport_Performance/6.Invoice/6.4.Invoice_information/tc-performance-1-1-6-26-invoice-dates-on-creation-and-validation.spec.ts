...
/**
 * Invoice data verification - Invoice Date, Due Date and Paid Date are empty on a draft Invoice and dated on validation
 * Test Case ID: TC.Performance.1.1.6.26
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.26:" --project=SalesReport_Performance
 *
 ...
 *
 * Verification:
 * - On the draft, Invoice Date is empty
 * - On the draft, Due Date is empty
 * - On the draft, Paid Date is empty
 * - After VALIDATE the Invoice Date is filled
 * - After VALIDATE the Due Date equals the Invoice Date, because the Payment Terms are Immediate Payment
 *
...
const TC = 'TC.Performance.1.1.6.26';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the three date fields before and after VALIDATE',
};

test.describe(`${TC} - Invoice Date, Due Date and Paid Date are empty on a draft Invoice and dated on validation`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify Invoice Date, Due Date and Paid Date on a draft and on a validated Invoice`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      console.log(STEP.s7);
      const invoiceDateDraft = await invoicePage.getFieldDisplayValue('date_invoice');
      const dueDateDraft = await invoicePage.getFieldDisplayValue('date_due');
      const paidDateDraft = await invoicePage.getFieldDisplayValue('paid_date');
