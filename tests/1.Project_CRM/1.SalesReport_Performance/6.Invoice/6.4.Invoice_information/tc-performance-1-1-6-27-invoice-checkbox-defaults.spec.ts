...
/**
 * Invoice data verification - The checkbox defaults of a newly created Invoice
 * Test Case ID: TC.Performance.1.1.6.27
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.27:" --project=SalesReport_Performance
 *
 * Objective: Verify a newly created Invoice leaves Certificate Required, Don't send license and License Send unticked, and ticks Can Edit Actually Received.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the "Certificate Required", "Can Edit Actually Received", "Don't send license" and "License Send" checkboxes
 *
 * Verification:
 * - Certificate Required is unticked
 * - Can Edit Actually Received is ticked
 * - Don't send license is unticked
 * - License Send is unticked
 *
...
const TC = 'TC.Performance.1.1.6.27';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the four checkboxes of the Invoice information area',
};

test.describe(`${TC} - The checkbox defaults of a newly created Invoice`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the checkbox defaults of a newly created Invoice`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      console.log(STEP.s7);
      const certificateRequired = await invoicePage.isFieldChecked('certificate_required');
      const canEditActuallyReceived = await invoicePage.isFieldChecked('can_edit_actually_received');
      const dontSendLicense = await invoicePage.isFieldChecked('x_studio_dont_send_license');
      const licenseSend = await invoicePage.isFieldChecked('license');
      console.log(`  - Certificate Required: ${certificateRequired} | Can Edit Actually Received: ${canEditActuallyReceived}`);
      console.log(`  - Don't send license  : ${dontSendLicense} | License Send: ${licenseSend}`);

      record('Certificate Required', 'unticked', certificateRequired ? 'ticked' : 'unticked');
      record('Can Edit Actually Received', 'ticked', canEditActuallyReceived ? 'ticked' : 'unticked');
      record("Don't send license", 'unticked', dontSendLicense ? 'ticked' : 'unticked');
      record('License Send', 'unticked', licenseSend ? 'ticked' : 'unticked');
      printVerify();

      expect(certificateRequired, 'Certificate Required must default to unticked').toBe(false);
      expect(canEditActuallyReceived, 'Can Edit Actually Received must default to ticked').toBe(true);
      expect(dontSendLicense, "Don't send license must default to unticked").toBe(false);
      expect(licenseSend, 'License Send must default to unticked on a draft Invoice').toBe(false);
    });
  });
});
