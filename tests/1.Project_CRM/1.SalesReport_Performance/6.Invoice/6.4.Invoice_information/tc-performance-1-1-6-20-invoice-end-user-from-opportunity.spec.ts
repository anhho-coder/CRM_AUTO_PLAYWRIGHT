...
/**
 * Invoice data verification - The End User on the Invoice is filled and matches the Customer of the Opportunity
 * Test Case ID: TC.Performance.1.1.6.20
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.20:" --project=SalesReport_Performance
 *
 ...
 *
 * Verification:
 * - The End User is filled (not empty)
 * - The End User is the Company of the Opportunity
 * - The End User equals the Payer, because the Opportunity names no separate end customer
 *
...
const TC = 'TC.Performance.1.1.6.20';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the End User',
};

test.describe(`${TC} - The End User on the Invoice is filled and matches the Customer of the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the End User on the Invoice is filled and matches the Customer of the Opportunity`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      console.log(STEP.s7);
      const endUser = await invoicePage.getFieldPartnerName('partner_end_user_id');
      const payer = await invoicePage.getFieldPartnerName('partner_id');
      console.log(`  - End User: ${endUser} | Payer: ${payer}`);

      record('End User is filled', 'not empty', endUser || '(empty)', endUser !== '');
      record('End User matches the Opportunity Company', oppCompany, endUser);
      record('End User equals the Payer', payer, endUser);
      printVerify();

      expect(endUser, 'the Invoice End User must be filled').not.toBe('');
      expect(endUser, 'the Invoice End User must be the Company of the Opportunity').toBe(oppCompany);
