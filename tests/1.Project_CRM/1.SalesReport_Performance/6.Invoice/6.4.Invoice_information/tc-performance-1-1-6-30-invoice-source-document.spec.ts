...
/**
 * Invoice data verification - The Source Document on the Invoice is the Sales Order it was generated from
 * Test Case ID: TC.Performance.1.1.6.30
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.30:" --project=SalesReport_Performance
 *
 * Objective: Verify the "Source Document" of the Other Info tab names the Sales Order the invoice was generated from, i.e. the SO the CONFIRM of the Quotation produced.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Open the "Other Info" notebook tab of the Invoice
 * 2. Read the "Source Document" field
 *
 * Verification:
 * - The Source Document is filled (not empty)
 * - The Source Document reads a Sales Order number of the form SO<number>
 *
...
const TC = 'TC.Performance.1.1.6.30';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the Source Document on the Other Info tab',
};

test.describe(`${TC} - The Source Document on the Invoice is the Sales Order it was generated from`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the Source Document on the Invoice is the Sales Order it was generated from`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      ...
      const shaped = /^SO\d+$/.test(sourceDocument);
      console.log(`  - Source Document: "${sourceDocument}"`);

      record('Source Document is filled', 'not empty', sourceDocument || '(empty)', sourceDocument !== '');
      record('Source Document is a Sales Order number', 'SO<number>', sourceDocument || '(empty)', shaped);
      printVerify();

      expect(sourceDocument, 'the Invoice Source Document must be filled').not.toBe('');
      expect(shaped, `the Source Document must name a Sales Order, read "${sourceDocument}"`).toBe(true);
    });
  });
});
