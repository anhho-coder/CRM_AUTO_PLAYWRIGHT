...
/**
 * Invoice data verification - The Currency on the Invoice is the currency of the Deal Element pricelist
 * Test Case ID: TC.Performance.1.1.6.24
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.24:" --project=SalesReport_Performance
 *
 * Objective: Verify an Invoice raised from a Deal Element priced in Public Pricelist_USD is denominated in USD, and that the totals are printed in that currency.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the "Currency" field of the Invoice information area
 * 2. Read the Subtotal of the totals block under the Invoice Lines
 *
 * Verification:
 * - The Currency is filled (not empty)
 * - The Currency reads USD - the currency of Public Pricelist_USD
 * - The Subtotal is printed with the $ sign
 *
...
const TC = 'TC.Performance.1.1.6.24';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the Currency',
};

test.describe(`${TC} - The Currency on the Invoice is the currency of the Deal Element pricelist`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the Currency on the Invoice is the currency of the Deal Element pricelist`, async ({ page }) => {
