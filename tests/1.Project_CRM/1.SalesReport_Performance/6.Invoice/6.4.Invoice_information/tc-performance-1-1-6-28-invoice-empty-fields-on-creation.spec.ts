...
/**
 * Invoice data verification - Actually Received, Quantity of used license, PO and Postpone Date on a newly created Invoice
 * Test Case ID: TC.Performance.1.1.6.28
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.28:" --project=SalesReport_Performance
 *
 * Objective: Verify a newly created Invoice starts with Actually Received 0.00 and Quantity of used license 0, and leaves PO and Postpone Date empty.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read "Actually Received", "Quantity of used license", "PO" and "Postpone Date"
 *
 * Verification:
 * - Actually Received is 0.00
 * - Quantity of used license is 0
 * - PO is empty
 * - Postpone Date is empty
 *
...
const TC = 'TC.Performance.1.1.6.28';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the numeric defaults and the empty fields',
};

test.describe(`${TC} - Actually Received, Quantity of used license, PO and Postpone Date on a newly created Invoice`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify Actually Received, Quantity of used license, PO and Postpone Date on a newly created Invoice`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      ...
      const po = await invoicePage.getFieldDisplayValue('partner_purchase_order');
      const postponeDate = await invoicePage.getFieldDisplayValue('postpone_date');
      console.log(`  - Actually Received: ${actuallyReceived} | Quantity of used license: ${quantityUsedLicense}`);
      console.log(`  - PO: "${po}" | Postpone Date: "${postponeDate}"`);

      record('Actually Received', 0, actuallyReceived);
      record('Quantity of used license', 0, quantityUsedLicense);
      record('PO is empty', '(empty)', po || '(empty)');
      record('Postpone Date is empty', '(empty)', postponeDate || '(empty)');
      printVerify();

      expect(actuallyReceived, 'a new Invoice must start with Actually Received 0.00').toBe(0);
      expect(quantityUsedLicense, 'a new Invoice must start with Quantity of used license 0').toBe(0);
      expect(po, 'a new Invoice must leave PO empty').toBe('');
      expect(postponeDate, 'a new Invoice must leave Postpone Date empty').toBe('');
    });
  });
});
