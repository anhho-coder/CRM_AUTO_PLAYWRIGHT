...
/**
 * Invoice data verification - A draft Invoice prints "Draft Invoice" instead of an invoice number
 * Test Case ID: TC.Performance.1.1.6.29
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.29:" --project=SalesReport_Performance
 *
 * Objective: Verify Odoo withholds the invoice number while the Invoice is draft and prints the caption "Draft Invoice" in its place, so the sequence is only consumed on validation.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the caption the sheet prints where the invoice number goes
 * 2. Read the invoice number field
 *
 * Verification:
 * - The sheet prints "Draft Invoice"
 * - The invoice number is empty
 * - The active stage is DRAFT
 *
...
const TC = 'TC.Performance.1.1.6.29';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the caption of the draft Invoice',
};

test.describe(`${TC} - A draft Invoice prints "Draft Invoice" instead of an invoice number`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify a draft Invoice prints "Draft Invoice" instead of an invoice number`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      ...
      console.log(`  - Caption: "${caption}" | number: "${number}" | stage: "${stage}"`);

      record('Caption on a draft Invoice', 'Draft Invoice', caption);
      record('Invoice number on a draft Invoice', '(empty)', number || '(empty)');
      record('Active stage', 'DRAFT', stage);
      printVerify();

      expect(caption, 'a draft Invoice must print "Draft Invoice" where the number goes').toBe('Draft Invoice');
      expect(number, 'a draft Invoice must not carry an invoice number yet').toBe('');
      expect(stage, 'a newly created Invoice must sit on DRAFT').toBe('DRAFT');
    });
  });
});
