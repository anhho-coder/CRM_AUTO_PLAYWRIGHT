...
/**
 * Invoice data verification - The Payment Terms on the Invoice match the Payment Term selected on the Deal Element
 * Test Case ID: TC.Performance.1.1.6.23
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.23:" --project=SalesReport_Performance
 *
 * Objective: Verify the Payment Term chosen on the Deal Element travels down the chain and lands on the Invoice as its Payment Terms.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the "Payment Terms" field of the Invoice information area
 *
 * Verification:
 * - The Payment Terms are filled (not empty)
 * - The Payment Terms read "Immediate Payment" - the Payment Term selected on the Deal Element
 *
...
const TC = 'TC.Performance.1.1.6.23';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the Payment Terms',
};

test.describe(`${TC} - The Payment Terms on the Invoice match the Payment Term selected on the Deal Element`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the Payment Terms on the Invoice match the Payment Term of the Deal Element`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      ...
      record('Payment Terms are filled', 'not empty', paymentTerms || '(empty)', paymentTerms !== '');
      record('Payment Terms match the Deal Element', DATA.paymentTerm, paymentTerms);
      printVerify();

      expect(paymentTerms, 'the Invoice Payment Terms must be filled').not.toBe('');
      expect(paymentTerms, 'the Invoice Payment Terms must be the Deal Element Payment Term').toBe(DATA.paymentTerm);
    });
  });
});
