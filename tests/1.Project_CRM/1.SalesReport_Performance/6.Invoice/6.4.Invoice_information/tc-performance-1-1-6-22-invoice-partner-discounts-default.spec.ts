...
/**
 * Invoice data verification - Distributor Discount (%) and Reseller Discount (%) default to zero on the Invoice
 * Test Case ID: TC.Performance.1.1.6.22
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.22:" --project=SalesReport_Performance
 *
 * Objective: Verify both partner discount rows of an Invoice raised from a direct Opportunity read 0.00 for the percentage and $ 0.00 for the money they grant.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the "Distributor Discount (%)" and "Reseller Discount (%)" rows of the Invoice
 *
 * Verification:
 * - Distributor Discount (%) is 0.00 and grants $ 0.00
 * - Reseller Discount (%) is 0.00 and grants $ 0.00
 *
...
const TC = 'TC.Performance.1.1.6.22';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the two partner discount rows',
};

test.describe(`${TC} - Distributor Discount (%) and Reseller Discount (%) default to zero on the Invoice`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify Distributor Discount (%) and Reseller Discount (%) are zero on the Invoice`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      console.log(STEP.s7);
      const distributorRow = await invoicePage.getFieldRowTextByLabel('Distributor Discount (%)');
      const resellerRow = await invoicePage.getFieldRowTextByLabel('Reseller Discount (%)');
      const distributorPct = await invoicePage.getFieldNumber('distributor_discount');
      const resellerPct = await invoicePage.getFieldNumber('reseller_discount');
      console.log(`  - Distributor Discount (%) row: "${distributorRow}"`);
      console.log(`  - Reseller Discount (%) row   : "${resellerRow}"`);

      const zeroMoney = (row: string) => /\$\s*0\.00/.test(row);

      record('Distributor Discount percentage', 0, distributorPct);
      record('Distributor Discount money is $ 0.00', '$ 0.00 in the row', distributorRow, zeroMoney(distributorRow));
      record('Reseller Discount percentage', 0, resellerPct);
      record('Reseller Discount money is $ 0.00', '$ 0.00 in the row', resellerRow, zeroMoney(resellerRow));
      printVerify();

      expect(distributorPct, 'Distributor Discount (%) must default to 0.00').toBe(0);
      expect(zeroMoney(distributorRow), `Distributor Discount must grant $ 0.00, row read "${distributorRow}"`).toBe(true);
      expect(resellerPct, 'Reseller Discount (%) must default to 0.00').toBe(0);
      expect(zeroMoney(resellerRow), `Reseller Discount must grant $ 0.00, row read "${resellerRow}"`).toBe(true);
    });
  });
});
