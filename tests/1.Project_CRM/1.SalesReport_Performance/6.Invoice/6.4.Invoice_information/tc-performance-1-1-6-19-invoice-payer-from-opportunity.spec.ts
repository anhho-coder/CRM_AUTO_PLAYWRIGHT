...
/**
 * Invoice data verification - The Payer on the Invoice is filled and matches the Customer of the Opportunity
 * Test Case ID: TC.Performance.1.1.6.19
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.19:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.6.19';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the Payer',
};

test.describe(`${TC} - The Payer on the Invoice is filled and matches the Customer of the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the Payer on the Invoice is filled and matches the Customer of the Opportunity`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      ...
      expect(payer, 'the Invoice Payer must be the Company of the Opportunity').toBe(oppCompany);
    });
  });
});
