...
/**
 * Invoice data verification - The Salesperson on the Invoice is the user who created the chain and the Sales Team is empty
 * Test Case ID: TC.Performance.1.1.6.25
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.25:" --project=SalesReport_Performance
 *
 * Objective: Verify the Invoice names the logged-in user as its Salesperson, and that the Sales Team stays empty because the Opportunity was created with its Sales Team cleared.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the "Salesperson" and the "Sales Team" fields of the Invoice information area
 *
 * Verification:
 * - The Salesperson is filled (not empty)
 * - The Salesperson is the user that created the chain
 * - The Sales Team is empty, because the Opportunity was created with Sales Team cleared
 *
...
const TC = 'TC.Performance.1.1.6.25';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the Salesperson and the Sales Team',
};

test.describe(`${TC} - The Salesperson on the Invoice is the user who created the chain and the Sales Team is empty`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the Salesperson and the Sales Team on the Invoice`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      console.log(STEP.s7);
      const salesperson = await invoicePage.getFieldPartnerName('user_id');
      const salesTeam = await invoicePage.getFieldPartnerName('team_id');
      const expectedSalesperson = users.admin_crm.createdByName;
      console.log(`  - Salesperson: "${salesperson}" | Sales Team: "${salesTeam}"`);
      ...
      expect(salesperson, 'the Invoice Salesperson must be the user that created the chain').toBe(expectedSalesperson);
      expect(salesTeam, 'the Invoice Sales Team must stay empty when the Opportunity cleared it').toBe('');
    });
  });
});
