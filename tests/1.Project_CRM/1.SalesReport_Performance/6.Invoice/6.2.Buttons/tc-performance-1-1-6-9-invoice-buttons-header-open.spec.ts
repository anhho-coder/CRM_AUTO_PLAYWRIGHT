...
/**
 * Invoice buttons - The buttons the header offers on an OPEN (validated) Invoice
 * Test Case ID: TC.Performance.1.1.6.9
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.9:" --project=SalesReport_Performance
 *
 ...
 * 2. Read the buttons the Invoice header renders, left to right
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.6.9';
...
const STEP = {
  ...
  s7: 'Step 7: Validating the Invoice and verifying the OPEN header buttons',
};

test.describe(`${TC} - The buttons the header offers on an OPEN (validated) Invoice`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the number, the names and the order of the buttons on an OPEN Invoice`, async ({ page }) => {
