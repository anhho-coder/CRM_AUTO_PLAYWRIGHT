...
/**
 * Invoice buttons - The number, the names and the order of the control-panel buttons on the Invoice
 * Test Case ID: TC.Performance.1.1.6.5
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.5:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.6.5';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the control-panel buttons of the Invoice',
};

test.describe(`${TC} - The number, the names and the order of the control-panel buttons on the Invoice`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the number, the names and the order of the control-panel buttons on the Invoice`, async ({ page }) => {
