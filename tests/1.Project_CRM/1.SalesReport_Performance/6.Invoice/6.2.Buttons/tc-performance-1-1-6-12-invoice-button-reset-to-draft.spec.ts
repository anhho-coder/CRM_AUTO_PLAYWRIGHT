...
/**
 * Invoice buttons - The RESET TO DRAFT button returns a cancelled Invoice to Draft
 * Test Case ID: TC.Performance.1.1.6.12
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.12:" --project=SalesReport_Performance
 *
 ...
 *
 * Steps run:
 * 1. Press the "VALIDATE" button, then the "CANCEL" button of the Invoice header
 * 2. Press the "RESET TO DRAFT" button
 * 3. Read the stage, the caption and the header buttons
 *
 * Verification:
 * - The active stage is DRAFT again
 * - The sheet prints "Draft Invoice" where the number goes
 * - The header offers VALIDATE, PREVIEW, CANCEL again
 *
...
const TC = 'TC.Performance.1.1.6.12';
...
const STEP = {
  ...
  s7: 'Step 7: Validating, cancelling, then resetting the Invoice to Draft',
};

test.describe(`${TC} - The RESET TO DRAFT button returns a cancelled Invoice to Draft`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the RESET TO DRAFT button returns a cancelled Invoice to Draft`, async ({ page }) => {
