...
/**
 * Invoice buttons - The PREVIEW button opens the customer portal view of the Invoice
 * Test Case ID: TC.Performance.1.1.6.7
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.7:" --project=SalesReport_Performance
 *
 ...
 *
 * Steps run:
 * 1. Press the "PREVIEW" button of the Invoice header
 * 2. Read the URL the browser lands on
 *
 * Verification:
 * - The browser leaves the backend form and lands on the portal
 * - The portal URL carries the /my/invoices path
 *
...
const TC = 'TC.Performance.1.1.6.7';
...
const STEP = {
  ...
  s7: 'Step 7: Pressing PREVIEW and verifying the customer portal opens',
};

test.describe(`${TC} - The PREVIEW button opens the customer portal view of the Invoice`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the PREVIEW button opens the customer portal view of the Invoice`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      console.log(STEP.s7);
      await invoicePage.clickStatusbarButtonByName('preview_invoice', CommonUtils.waitTimes.pageLoad);
      await page.waitForTimeout(CommonUtils.waitTimes.extraLong);
      await page.waitForLoadState('domcontentloaded').catch(() => {});

      const url = page.url();
      const onPortal = /\/my\/invoices/.test(url);
      console.log(`  - Landed on: ${url}`);

      record('PREVIEW leaves the backend form', 'not a backend form URL', /view_type=form/.test(url) ? 'still on the form' : 'left the form',
        !/view_type=form/.test(url));
      record('PREVIEW opens the customer portal invoice page', 'URL contains /my/invoices', url, onPortal);
      printVerify();

      expect(onPortal, `PREVIEW must open the customer portal invoice page, landed on ${url}`).toBe(true);
    });
  });
});
