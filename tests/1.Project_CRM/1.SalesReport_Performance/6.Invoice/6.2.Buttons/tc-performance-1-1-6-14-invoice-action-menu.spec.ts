...
/**
 * Invoice buttons - The entries the Action menu of the Invoice offers
 * Test Case ID: TC.Performance.1.1.6.14
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.14:" --project=SalesReport_Performance
 *
 ...
 *
 * Steps run:
 * 1. Open the "Action" dropdown of the control panel
 * 2. Read the items it offers, top to bottom
 *
 * Verification:
 * - The Action menu carries exactly 6 items
 * - They read Bookings, Dasboard - Invoices, Delete, Share, Export Transaction, Send Reminder in that order
...
const TC = 'TC.Performance.1.1.6.14';
...
const STEP = {
  ...
  s7: 'Step 7: Opening the Action menu and verifying its items',
};

test.describe(`${TC} - The entries the Action menu of the Invoice offers`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the number, the names and the order of the items in the Invoice Action menu`, async ({ page }) => {
