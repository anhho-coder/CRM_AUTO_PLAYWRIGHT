...
/**
 * Invoice buttons - The CANCEL button cancels the Invoice
 * Test Case ID: TC.Performance.1.1.6.10
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.10:" --project=SalesReport_Performance
 *
 ...
 *
 * Steps run:
 * 1. Press the "VALIDATE" button of the Invoice header
 * 2. Press the "CANCEL" button of the Invoice header
 * 3. Reload the form and read the stage and the invoice number
 *
 * Verification:
 * - After CANCEL the active stage is CANCELLED
 * - CANCELLED joins the stages the statusbar offers
...
const TC = 'TC.Performance.1.1.6.10';
...
const STEP = {
  ...
  s7: 'Step 7: Validating, then cancelling the Invoice',
};

test.describe(`${TC} - The CANCEL button cancels the Invoice`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the CANCEL button cancels the Invoice`, async ({ page }) => {
