...
/**
 * Invoice buttons - The reports the Print menu of the Invoice offers
 * Test Case ID: TC.Performance.1.1.6.13
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.13:" --project=SalesReport_Performance
 *
 ...
 *
 * Steps run:
 * 1. Open the "Print" dropdown of the control panel
 * 2. Read the items it offers, top to bottom
 *
 * Verification:
 * - The Print menu carries exactly 3 items
 * - They read Invoices, Invoices without Payment, Invoices Siemens in that order
 *
...
const TC = 'TC.Performance.1.1.6.13';
...
const STEP = {
  ...
  s7: 'Step 7: Opening the Print menu and verifying its items',
};

test.describe(`${TC} - The reports the Print menu of the Invoice offers`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the number, the names and the order of the items in the Invoice Print menu`, async ({ page }) => {
