...
/**
 * Invoice buttons - The EDIT button puts the Invoice form in edit mode
 * Test Case ID: TC.Performance.1.1.6.6
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.6:" --project=SalesReport_Performance
 *
 ...
 *
 * Steps run:
 * 1. Press the "EDIT" button of the control panel
 * 2. Read the control-panel buttons again
 *
 * Verification:
 * - The control panel no longer offers EDIT
 * - It offers SAVE and DISCARD instead
 *
...
const TC = 'TC.Performance.1.1.6.6';
...
const STEP = {
  ...
  s7: 'Step 7: Pressing EDIT and verifying the form is editable',
};

test.describe(`${TC} - The EDIT button puts the Invoice form in edit mode`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the EDIT button puts the Invoice form in edit mode`, async ({ page }) => {
