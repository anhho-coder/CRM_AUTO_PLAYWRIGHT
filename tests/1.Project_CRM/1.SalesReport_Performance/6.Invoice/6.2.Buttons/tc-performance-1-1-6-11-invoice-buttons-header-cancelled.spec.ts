...
/**
 * Invoice buttons - The buttons the header offers on a CANCELLED Invoice
 * Test Case ID: TC.Performance.1.1.6.11
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.11:" --project=SalesReport_Performance
 *
 ...
 * 2. Reload the form and read the buttons the header renders, left to right
 *
 * Verification:
 * - The header carries exactly 2 buttons
 * - They read PREVIEW, RESET TO DRAFT in that order
 * - VALIDATE, REGISTER PAYMENT and ADD CREDIT NOTE are no longer offered
 *
...
const TC = 'TC.Performance.1.1.6.11';
...
const STEP = {
  ...
  s7: 'Step 7: Validating, cancelling, then verifying the CANCELLED header buttons',
};

test.describe(`${TC} - The buttons the header offers on a CANCELLED Invoice`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the number, the names and the order of the buttons on a CANCELLED Invoice`, async ({ page }) => {
