...
/**
 * Invoice Invoice Lines - The name, the number and the order of the Invoice Lines columns
 * Test Case ID: TC.Performance.1.1.6.31
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.31:" --project=SalesReport_Performance
 *
 * Objective: Verify the Invoice Lines list carries the thirteen documented columns, in the documented order, Partner Discount and Partner Discount Amount among them.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the column headers of the "Invoice Lines" tab, left to right
 *
 * Verification:
 * - The Invoice Lines list carries exactly 13 named columns
 * - They read Product, Start Date, End Date, Quantity, Unit of Measure, Price, Special Discount (%), Special Discount Amount, Partner Discount, Partner Discount Amount, Subtotal After All Discounts, Booked value (company currency), Subtotal in that order
...
const TC = 'TC.Performance.1.1.6.31';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the Invoice Lines columns',
};

test.describe(`${TC} - The name, the number and the order of the Invoice Lines columns`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the name, the number and the order of the Invoice Lines columns`, async ({ page }) => {
