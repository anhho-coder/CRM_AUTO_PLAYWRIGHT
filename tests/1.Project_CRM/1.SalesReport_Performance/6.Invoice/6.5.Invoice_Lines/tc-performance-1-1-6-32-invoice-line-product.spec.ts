...
/**
 * Invoice Invoice Lines - The Invoice line carries the product of the Deal Element
 * Test Case ID: TC.Performance.1.1.6.32
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.32:" --project=SalesReport_Performance
 *
 * Objective: Verify the Invoice raised from the Sales Order carries exactly one line, and that the line names the NAKIVO product added on the Deal Element with the quantity and the unit of measure it was added with.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the number of rows of the "Invoice Lines" tab
 * 2. Read the Product, the Quantity and the Unit of Measure of the first row
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.6.32';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the product of the Invoice line',
};

test.describe(`${TC} - The Invoice line carries the product of the Deal Element`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the Invoice line carries the product of the Deal Element`, async ({ page }) => {
