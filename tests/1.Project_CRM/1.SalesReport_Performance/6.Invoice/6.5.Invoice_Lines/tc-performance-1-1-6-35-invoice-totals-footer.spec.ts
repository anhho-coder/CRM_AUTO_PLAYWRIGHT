...
/**
 * Invoice Invoice Lines - The totals block under the Invoice Lines and the Amount Due that only a posted Invoice shows
 * Test Case ID: TC.Performance.1.1.6.35
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.35:" --project=SalesReport_Performance
 *
 * Objective: Verify the totals block of a draft Invoice carries Subtotal, Partner Discount and Total - and that Amount Due joins it only once the Invoice is validated.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the rows of the totals block under the Invoice Lines on the draft Invoice
 * 2. Press the "VALIDATE" button of the Invoice header
 * 3. Read the rows of the totals block again
 *
 * Verification:
 * - The draft totals block carries exactly 3 rows: Subtotal, Partner Discount, Total
 * - "Amount Due" is not shown on a draft Invoice
 * - The Total equals the Subtotal, no discount applying
 * - After VALIDATE the totals block carries an Amount Due row
 * - The Amount Due equals the Total
 *
...
const TC = 'TC.Performance.1.1.6.35';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the totals block before and after VALIDATE',
};

test.describe(`${TC} - The totals block under the Invoice Lines and the Amount Due that only a posted Invoice shows`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the totals block under the Invoice Lines on a draft and on a validated Invoice`, async ({ page }) => {
    ...
    await test.step(STEP.s7, async () => {
      console.log(STEP.s7);
      const draftTotals = await invoicePage.getInvoiceTotalsFooter();
      ...
      const draftSubtotal = money((draftTotals.find((t) => t.label === 'Subtotal') || { value: '' }).value);
      const draftTotal = money((draftTotals.find((t) => t.label === 'Total') || { value: '' }).value);
      ...
      const openTotals = await invoicePage.getInvoiceTotalsFooter();
      const openLabels = openTotals.map((t) => t.label);
      const amountDue = money((openTotals.find((t) => t.label === 'Amount Due') || { value: '' }).value);
