...
/**
 * Invoice Invoice Lines - Every discount column of the Invoice line is zero when no discount applies
 * Test Case ID: TC.Performance.1.1.6.33
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.33:" --project=SalesReport_Performance
 *
 * Objective: Verify the Invoice line of an order placed without any discount reads 0.00 in all four discount columns.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the Special Discount (%), Special Discount Amount, Partner Discount and Partner Discount Amount cells of the first Invoice line
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.6.33';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the discount columns of the Invoice line',
};

test.describe(`${TC} - Every discount column of the Invoice line is zero when no discount applies`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify every discount column of the Invoice line is 0.00 when no discount applies`, async ({ page }) => {
