...
/**
 * Invoice layout - Invoice Address and Delivery Address are not rendered on the Invoice form
 * Test Case ID: TC.Performance.1.1.6.18
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.18:" --project=SalesReport_Performance
 *
 * Objective: Verify the Invoice form does NOT show an Invoice Address or a Delivery Address row - unlike the Quotation, which carries both - so a test case that expects them is asserting a field this screen never renders.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read every visible field label of the Invoice information area
 * 2. Read whether the Delivery Address widget (partner_shipping_id) is rendered visibly
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.6.18';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying Invoice Address and Delivery Address are absent',
};

test.describe(`${TC} - Invoice Address and Delivery Address are not rendered on the Invoice form`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify Invoice Address and Delivery Address are not rendered on the Invoice form`, async ({ page }) => {
