...
/**
 * Invoice layout - The name, the number and the order of the Invoice notebook tabs
 * Test Case ID: TC.Performance.1.1.6.17
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.17:" --project=SalesReport_Performance
 *
 ...
 *
 * Steps run:
 * 1. Read the notebook tab captions of the Invoice form, left to right
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.6.17';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the Invoice notebook tabs',
};

test.describe(`${TC} - The name, the number and the order of the Invoice notebook tabs`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the name, the number and the order of the Invoice notebook tabs`, async ({ page }) => {
