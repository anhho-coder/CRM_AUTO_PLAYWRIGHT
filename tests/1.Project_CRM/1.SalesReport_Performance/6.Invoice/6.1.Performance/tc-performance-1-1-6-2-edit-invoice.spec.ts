...
/**
 * Performance Test - Edit Invoice
 * Test Case ID: TC.Performance.1.1.6.2
 * 
 * Summary: Verify the consumer time for editing Invoice is less than 1 min
 *
 * Command to run:
 * npx playwright test --grep "TC\.Performance\.1\.1\.6\.2:" --project=chromium
 *
 * Pre-condition:
 ...
 * 16. Press "EDIT" button and wait
 * 17. Change the value of "Payment Terms" field to "15 Days"
 * 
 * Steps run:
...
test.describe('TC.Performance.1.1.6.2 - Edit Invoice Performance', () => {
  const PERFORMANCE_THRESHOLD = 60000; // 1 minute in milliseconds
        ...
        test('TC.Performance.1.1.6.2: Verify consumer time for editing Invoice is less than 1 min', async ({ page, browser }, testInfo) => {
    ...
    performanceMetrics['Save Deal Element'] = Date.now() - stepStartTime;
      ...
      // Step 16: Press "EDIT" button
      await test.step('Step 16: Press "EDIT" button and wait', async () => {
        stepStartTime = Date.now();
        console.log('Step 16: Clicking EDIT button');
        
        // Click EDIT button using POM
        await invoicePage.clickEdit();
        
        performanceMetrics['Click Edit Button'] = Date.now() - stepStartTime;
        console.log('✓ Invoice opened in edit mode');
      });

      // Step 17: Change Payment Terms to "15 Days"
      await test.step('Step 17: Change the value of "Payment Terms" field to "15 Days"', async () => {
        stepStartTime = Date.now();
        console.log('Step 17: Changing Payment Terms to "15 Days"');
        
        // Change Payment Terms using POM
        await invoicePage.changePaymentTerms('15 Days');
        
        performanceMetrics['Change Payment Terms'] = Date.now() - stepStartTime;
        console.log('✓ Payment Terms changed to 15 Days');
      });
      ...
      await test.step('Steps run - Step 1: Press "SAVE" button (Performance measurement)', async () => {
        console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.log('PERFORMANCE TEST - Saving Invoice Edit (Performance Measurement)');
        
        // Click SAVE button and measure performance using POM
        saveTime = await invoicePage.clickSaveAndWaitForCompletion();
        saveSeconds = (saveTime / 1000).toFixed(2);
        
        performanceMetrics['SAVE Invoice Edit Process'] = saveTime;
        ...
        Object.entries(performanceMetrics).filter(([step]) => !step.includes('SAVE Invoice Edit')).forEach(([step, time]) => {
          const seconds = (time / 1000).toFixed(2);
          console.log(`  ${step.padEnd(35)} ${seconds.padStart(8)}s`);
        });
        console.log('\nTEST STEPS:');
        ['SAVE Invoice Edit Process'].forEach(step => {
      ...
      // Verification: Verify Invoice edit saved (not included in performance measurement)
      await test.step('Verification: Verify Invoice edit saved (not included in performance measurement)', async () => {
        console.log('\nVerification: Verifying Invoice edit saved (not included in performance measurement)');
        
        // Verify invoice is saved
        const pageContent = await page.textContent('body').catch(() => '') || '';
        const isSaved = pageContent.includes('Invoice') && !pageContent.includes('Edit');
        
        if (isSaved) {
          console.log('  ✓ Invoice edit saved successfully');
        } else {
          console.log('  ⚠ Invoice save verification: Status unclear');
        }
    ...
    await test.step('Capture final screenshot and performance summary', async () => {
      await CommonUtils.captureAndAttachScreenshot(
        page,
        testInfo,
        `✅ Performance Test Passed - Invoice Edit Time: ${saveSeconds}s`
      );
    });
  });
});
