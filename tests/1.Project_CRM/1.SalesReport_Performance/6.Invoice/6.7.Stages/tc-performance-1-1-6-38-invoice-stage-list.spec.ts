...
/**
 * Invoice stages - The name, the number and the order of the stages the Invoice statusbar offers
 * Test Case ID: TC.Performance.1.1.6.38
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.38:" --project=SalesReport_Performance
 *
 * Objective: Verify the statusbar of an Invoice offers the three documented stages - DRAFT, OPEN, PAID - in that order.
 *
 * Pre-condition:
 ...
 *
 * Steps run:
 * 1. Read the stages the Invoice statusbar offers, left to right
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.6.38';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the stages of the Invoice statusbar',
};

test.describe(`${TC} - The name, the number and the order of the stages the Invoice statusbar offers`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the name, the number and the order of the stages the Invoice statusbar offers`, async ({ page }) => {
