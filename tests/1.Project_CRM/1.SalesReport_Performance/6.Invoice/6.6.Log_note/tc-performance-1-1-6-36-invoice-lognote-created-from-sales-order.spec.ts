...
/**
 * Invoice log note - The Log note reports the Sales Order the Invoice was created from
 * Test Case ID: TC.Performance.1.1.6.36
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.6\\.36:" --project=SalesReport_Performance
 *
 * Objective: Verify the chatter of a newly created Invoice carries the note "This invoice has been created from: SO<number>", naming the Sales Order it was generated from.
 *
 * Pre-condition:
 ...
 * 2. Find the note that reports where the invoice was created from
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.6.36';
...
const STEP = {
  ...
  s7: 'Step 7: Verifying the "created from" log note',
};

test.describe(`${TC} - The Log note reports the Sales Order the Invoice was created from`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: Verify the Log note reports the Sales Order the Invoice was created from`, async ({ page }) => {
