...
/**
 * Performance Test - Create Invoice
 * Test Case ID: TC.Performance.1.1.6.1
 * 
 * Summary: Verify the consumer time for creating Invoice is less than 1 min
 *
 * Command to run:
 * npx playwright test --grep "TC\.Performance\.1\.1\.6\.1:" --project=chromium
 *
 * Pre-condition:
 ...
 * 
 * Steps run:
 * 1. On "Invoice Order" window, press "CREATE AND VIEW INVOICES" button and wait
 * 
 * Verification:
 * - Time consuming after pressing "CREATE AND VIEW INVOICES" button takes less than 1 minute
 */

test.describe('TC.Performance.1.1.6.1 - Create Invoice Performance', () => {
  const PERFORMANCE_THRESHOLD = 60000; // 1 minute in milliseconds
      ...
      test('TC.Performance.1.1.6.1: Verify consumer time for creating Invoice is less than 1 min', async ({ page, browser }, testInfo) => {
        ...
        let invoiceTime: number = 0;
        let invoiceSeconds: string = '0';
        ...
        // Step 11: Add a product in Order Lines section
        await test.step('Step 11: At "Order Lines" section - Add a product', async () => {
          stepStartTime = Date.now();
          console.log('Step 11: Adding product');
          
          // Add product line with quantity = 1
    ...
    // Step 14: Press "CONFIRM" button to create Sales Order
    await test.step('Step 14: Press "CONFIRM" button and wait to create a Sales Order', async () => {
      stepStartTime = Date.now();
      console.log('Step 14: Confirming quotation to create Sales Order');
    ...
    // Step 15: Press "CREATE INVOICE" button
    await test.step('Step 15: On "Sales Order" screen, press "CREATE INVOICE" button and wait', async () => {
      stepStartTime = Date.now();
      console.log('Step 15: Clicking CREATE INVOICE button');
    ...
    // Steps run - Step 1: Press "CREATE AND VIEW INVOICES" button and verify performance
    await test.step('Steps run - Step 1: On "Invoice Order" window, press "CREATE AND VIEW INVOICES" button (Performance measurement)', async () => {
      console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('PERFORMANCE TEST - Creating Invoice (Performance Measurement)');
      
      // START PERFORMANCE MEASUREMENT - Click CREATE AND VIEW INVOICES button using POM
      invoiceTime = await invoicePage.clickCreateAndViewInvoices();
      invoiceSeconds = (invoiceTime / 1000).toFixed(2);
      
      performanceMetrics['CREATE AND VIEW INVOICES Process'] = invoiceTime;
      
      // Calculate total elapsed time
      const totalElapsedTime = Date.now() - startTime;
      const totalElapsedSeconds = (totalElapsedTime / 1000).toFixed(2);
      
      console.log(`\n⏱️  Invoice Creation Operation Time: ${invoiceSeconds} seconds`);
      console.log(`⏱️  Total Time (including all steps): ${totalElapsedSeconds} seconds`);
      
      // Performance Verification: Ensure time after pressing CREATE AND VIEW INVOICES button is less than 1 minute
      expect(invoiceTime).toBeLessThan(PERFORMANCE_THRESHOLD);
      console.log(`\n✅ PERFORMANCE TEST PASSED: ${invoiceSeconds}s < 60s`);
      
      // Detailed performance breakdown
      console.log('\n📊 Detailed Performance Breakdown:');
      console.log('━'.repeat(50));
      console.log('PRE-CONDITIONS:');
      Object.entries(performanceMetrics).filter(([step]) => !step.includes('CREATE AND VIEW INVOICES')).forEach(([step, time]) => {
        const seconds = (time / 1000).toFixed(2);
        console.log(`  ${step.padEnd(35)} ${seconds.padStart(8)}s`);
      });
      console.log('\nTEST STEPS:');
      ['CREATE AND VIEW INVOICES Process'].forEach(step => {
      ...
      // Performance Summary
      console.log('\n📊 Performance Summary (Invoice Creation Operation):');
      console.log(`   - Operation Time: ${invoiceSeconds} seconds`);
      console.log(`   - Threshold: 60 seconds`);
      console.log(`   - Margin: ${(60 - parseFloat(invoiceSeconds)).toFixed(2)} seconds under threshold`);
      console.log(`   - Performance: ${((parseFloat(invoiceSeconds) / 60) * 100).toFixed(2)}% of threshold`);
      console.log(`   - Total Test Time: ${totalElapsedSeconds} seconds (includes all pre-conditions and steps)`);
    });
    
    // Verification: Verify Invoice creation (not included in performance measurement)
    await test.step('Verification: Verify Invoice creation (not included in performance measurement)', async () => {
      console.log('\nVerification: Verifying Invoice creation (not included in performance measurement)');
      
      // Verify invoice page is displayed
      const pageContent = await page.textContent('body').catch(() => '') || '';
      const invoiceCreated = pageContent.includes('Invoice') || pageContent.includes('Customer Invoice');
      
      if (invoiceCreated) {
        console.log('  ✓ Invoice created successfully');
      } else {
        console.log('  ⚠ Invoice creation verification: Status unclear');
      }
    ...
    await test.step('Capture final screenshot and performance summary', async () => {
      await CommonUtils.captureAndAttachScreenshot(
        page,
        testInfo,
        `✅ Performance Test Passed - Invoice Creation Time: ${invoiceSeconds}s`
      );
    });
  });
});
