...
/**
 * Lead data verification - Lead Form
 * Test Case ID: TC.Performance.1.1.1.8
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.8:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.8';
...
test.describe(`${TC} - Lead Form`, () => {
  ...
  test(`${TC}: Verify the Lead Form on the saved Lead matches the selected value`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(STEP.s5);
      await leadPage.clickCRMDeveloperTab();
      const leadForm = await leadPage.readFieldValue('x_studio_lead_sorce');
      console.log('VERIFY - Lead Form on the saved Lead');
