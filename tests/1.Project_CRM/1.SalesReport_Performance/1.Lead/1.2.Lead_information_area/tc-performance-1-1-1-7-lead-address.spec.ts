...
/**
 * Lead data verification - Address
 * Test Case ID: TC.Performance.1.1.1.7
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.7:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.7';
...
test.describe(`${TC} - Address`, () => {
  ...
  test(`${TC}: Verify the Address block on the saved Lead matches the entered values`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - Address block on the saved Lead');
