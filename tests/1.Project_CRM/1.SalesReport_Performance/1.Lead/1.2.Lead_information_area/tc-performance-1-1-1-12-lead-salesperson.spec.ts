...
/**
 * Lead data verification - Salesperson
 * Test Case ID: TC.Performance.1.1.1.12
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.12:" --project=SalesReport_Performance
 *
 * Objective: Verify the Salesperson is filled on the saved Lead by the assignment job
 *
...
const TC = 'TC.Performance.1.1.1.12';
...
const STEP = {
  ...
  s5: 'Step 5: Waiting for the assignment job and verifying the Salesperson',
};
...
test.describe(`${TC} - Salesperson`, () => {
  ...
  test(`${TC}: Verify the Salesperson is filled on the saved Lead by the assignment job`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - Salesperson on the saved Lead');
