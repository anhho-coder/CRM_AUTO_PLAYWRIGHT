...
/**
 * Lead data verification - Timezone
 * Test Case ID: TC.Performance.1.1.1.16
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.16:" --project=SalesReport_Performance
 *
 * Objective: Verify the Timezone and the Timezone offset on the saved Lead are filled and consistent
 *
...
const TC = 'TC.Performance.1.1.1.16';
...
test.describe(`${TC} - Timezone`, () => {
  ...
  test(`${TC}: Verify the Timezone and the Timezone offset on the saved Lead are filled and consistent`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - Timezone on the saved Lead');
