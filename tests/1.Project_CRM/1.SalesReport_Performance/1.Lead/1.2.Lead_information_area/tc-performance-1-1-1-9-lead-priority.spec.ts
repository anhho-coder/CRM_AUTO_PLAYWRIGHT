...
/**
 * Lead data verification - Priority
 * Test Case ID: TC.Performance.1.1.1.9
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.9:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.9';
...
test.describe(`${TC} - Priority`, () => {
  ...
  test(`${TC}: Verify the Priority the system applies to a new Lead is shown on the form and logged`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(STEP.s5);
      const note = await leadPage.waitForChatterMessage(
        /^Priority \(System\): /,
      ...
      console.log('VERIFY - Priority on the saved Lead');
