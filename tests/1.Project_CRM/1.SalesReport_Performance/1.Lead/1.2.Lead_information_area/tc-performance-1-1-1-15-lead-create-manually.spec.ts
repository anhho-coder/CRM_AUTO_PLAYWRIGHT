...
/**
 * Lead data verification - Create manually
 * Test Case ID: TC.Performance.1.1.1.15
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.15:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.15';
...
test.describe(`${TC} - Create manually`, () => {
  ...
  test(`${TC}: Verify the Create manually flag keeps the FALSE state set at creation`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - Create manually on the saved Lead');
