...
/**
 * Lead data verification - Sales Team
 * Test Case ID: TC.Performance.1.1.1.13
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.13:" --project=SalesReport_Performance
 *
 * Objective: Verify the Sales Team is filled on the saved Lead by the assignment job
 *
...
const TC = 'TC.Performance.1.1.1.13';
...
const STEP = {
  ...
  s5: 'Step 5: Waiting for the assignment job and verifying the Sales Team',
};
...
test.describe(`${TC} - Sales Team`, () => {
  ...
  test(`${TC}: Verify the Sales Team is filled on the saved Lead by the assignment job`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - Sales Team on the saved Lead');
