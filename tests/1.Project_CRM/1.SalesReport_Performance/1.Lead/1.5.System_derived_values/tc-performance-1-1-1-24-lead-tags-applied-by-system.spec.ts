...
/**
 * Lead data verification - Tags - applied by the system
 * Test Case ID: TC.Performance.1.1.1.24
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.24:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.24';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the tag the system applies from the Lead Form',
};

test.describe(`${TC} - Tags - applied by the system`, () => {
  ...
  test(`${TC}: Verify the system tags a Lead from its Lead Form when the user chooses no tag`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - tag applied by the system (no tag chosen by the user)');
      console.log(`  Lead Form : "${DATA.leadForm}"`);
