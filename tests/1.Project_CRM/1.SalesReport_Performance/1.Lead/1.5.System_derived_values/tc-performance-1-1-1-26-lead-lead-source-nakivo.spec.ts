...
/**
 * Lead data verification - Lead Source - Nakivo (Lead created without an owner)
 * Test Case ID: TC.Performance.1.1.1.26
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.26:" --project=SalesReport_Performance
 *
 * Objective: Verify a Lead created without an owner is assigned by the job and classified Lead Source = Nakivo
 *
...
const TC = 'TC.Performance.1.1.1.26';
...
const STEP = {
  ...
  s5: 'Step 5: Waiting for the assignment job and verifying the derived Lead Source',
};
...
test.describe(`${TC} - Lead Source - Nakivo (Lead created without an owner)`, () => {
  ...
  test(`${TC}: Verify a Lead created without an owner is assigned by the job and classified Lead Source = Nakivo`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(STEP.s5);
      const leadSource = await leadPage.waitForFieldValue(
        'lead_source',
        'Nakivo',
      ...
      console.log('VERIFY - Lead Source of a Lead created WITHOUT an owner');
