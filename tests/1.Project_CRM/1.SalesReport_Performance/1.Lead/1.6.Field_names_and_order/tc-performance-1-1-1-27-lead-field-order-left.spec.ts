...
/**
 * Lead data verification - Field names and order - left column
 * Test Case ID: TC.Performance.1.1.1.27
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.27:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.27';
...
test.describe(`${TC} - Field names and order - left column`, () => {
  ...
  test(`${TC}: Verify the field names and their order in the left column of the Lead information area`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      const EXPECTED = [
        ...
        { label: 'Country Lead', field: 'x_studio_source_country_lead' },
        { label: 'Lead Form', field: 'x_studio_lead_sorce' },
      ...
      console.log('VERIFY - field names and order, left column of the Lead information area');
