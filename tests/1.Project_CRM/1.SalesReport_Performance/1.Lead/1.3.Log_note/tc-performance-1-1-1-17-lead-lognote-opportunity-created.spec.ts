...
/**
 * Lead data verification - Log note - Opportunity created
 * Test Case ID: TC.Performance.1.1.1.17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.17:" --project=SalesReport_Performance
 *
 * Objective: Verify the "Opportunity created" log note reports the entered Lead data
 *
...
const TC = 'TC.Performance.1.1.1.17';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the "Opportunity created" log note',
};

test.describe(`${TC} - Log note - Opportunity created`, () => {
  ...
  test(`${TC}: Verify the "Opportunity created" log note reports the entered Lead data`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - "Opportunity created" log note');
