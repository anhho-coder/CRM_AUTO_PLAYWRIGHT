...
/**
 * Lead data verification - Log note - Customer created
 * Test Case ID: TC.Performance.1.1.1.18
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.18:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.18';
...
test.describe(`${TC} - Log note - Customer created`, () => {
  ...
  test(`${TC}: Verify the customer-creation log note reports the automatically created Customer`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - customer-creation log note');
      ...
      expect(customer.fields['Partner Contact Email'], 'Partner Contact Email in the note').toBe(DATA.email);
