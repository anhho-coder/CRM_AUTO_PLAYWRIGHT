...
/**
 * Lead data verification - Email - invalid format refused on the manual path
 * Test Case ID: TC.Performance.1.1.1.19
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.19:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.19';
...
test.describe(`${TC} - Email - invalid format refused on the manual path`, () => {
  ...
  test(`${TC}: Verify a manually created Lead cannot be saved with an e-mail containing a space`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - invalid e-mail refused on the manual path');
