...
/**
 * Lead data verification - Contact Name - left empty
 * Test Case ID: TC.Performance.1.1.1.21
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.21:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.21';
...
test.describe(`${TC} - Contact Name - left empty`, () => {
  ...
  test(`${TC}: Verify a Lead can be saved with an empty Contact Name`, async ({ page }) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - Lead saved with an empty Contact Name');
