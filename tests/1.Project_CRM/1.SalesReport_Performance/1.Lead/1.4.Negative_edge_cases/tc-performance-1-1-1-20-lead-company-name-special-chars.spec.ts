...
/**
 * Lead data verification - Company Name - special characters
 * Test Case ID: TC.Performance.1.1.1.20
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.1\\.20:" --project=SalesReport_Performance
 *
...
const TC = 'TC.Performance.1.1.1.20';
...
test.describe(`${TC} - Company Name - special characters`, () => {
  ...
  test(`${TC}: Verify a Company Name containing special characters is stored exactly as entered`, async ({ page }) => {
    ...
    const DATA = {
      ...
      companyName: `TEST-Contact & Co. "O'Brien" <${unique}>`,
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('VERIFY - Company Name with special characters');
