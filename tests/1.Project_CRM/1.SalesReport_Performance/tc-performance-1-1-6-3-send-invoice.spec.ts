...
/**
 * Performance Test - Send Invoice
 * Test Case ID: TC.Performance.1.1.6.3
 * 
 * Summary: Verify the consumer time for sending Invoice is less than 1 min
 *
 * Command to run:
 * npx playwright test --grep "TC\.Performance\.1\.1\.6\.3:" --project=chromium
 *
 * Pre-condition:
 ...
 * 17. Press "SEND & PRINT" button and wait
 * 
 * Steps run:
 * 1. Once the "Send Invoice" window appear, press "SEND" button and wait
 * 
 * Verification:
 * - Time consuming after pressing "SEND" button takes less than 1 minute
 */

test.describe('TC.Performance.1.1.6.3 - Send Invoice Performance', () => {
  const PERFORMANCE_THRESHOLD = 60000; // 1 minute in milliseconds
        ...
        test('TC.Performance.1.1.6.3: Verify consumer time for sending Invoice is less than 1 min', async ({ page, browser }, testInfo) => {
    ...
    performanceMetrics['Save Deal Element'] = Date.now() - stepStartTime;
      ...
      // Step 17: Press "SEND & PRINT" button
      await test.step('Step 17: Press "SEND & PRINT" button and wait', async () => {
        stepStartTime = Date.now();
        console.log('Step 17: Opening Send Invoice dialog');
        
        // Click SEND & PRINT using POM
        await invoicePage.clickSendAndPrint();
        
        performanceMetrics['Open Send Dialog'] = Date.now() - stepStartTime;
        console.log('✓ Send Invoice dialog opened');
      });

      // Steps run - Step 1: Press "SEND" button and verify performance
      await test.step('Steps run - Step 1: Once the "Send Invoice" window appear, press "SEND" button (Performance measurement)', async () => {
        console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.log('PERFORMANCE TEST - Sending Invoice (Performance Measurement)');
        
        // Click SEND button and measure performance using POM
        sendTime = await invoicePage.clickSendAndWaitForCompletion();
        sendSeconds = (sendTime / 1000).toFixed(2);
        
        performanceMetrics['SEND Invoice Process'] = sendTime;
        ...
        Object.entries(performanceMetrics).filter(([step]) => !step.includes('SEND Invoice')).forEach(([step, time]) => {
          const seconds = (time / 1000).toFixed(2);
          console.log(`  ${step.padEnd(35)} ${seconds.padStart(8)}s`);
        });
        console.log('\nTEST STEPS:');
        ['SEND Invoice Process'].forEach(step => {
      ...
      // Verification: Verify Invoice sent (not included in performance measurement)
      await test.step('Verification: Verify Invoice sent (not included in performance measurement)', async () => {
        console.log('\nVerification: Verifying Invoice sent (not included in performance measurement)');
        
        // Verify invoice was sent
        const pageContent = await page.textContent('body').catch(() => '') || '';
        const isSent = pageContent.includes('Invoice') || pageContent.includes('Sent');
        
        if (isSent) {
          console.log('  ✓ Invoice sent successfully');
        } else {
          console.log('  ⚠ Invoice send verification: Status unclear');
        }
    ...
    await test.step('Capture final screenshot and performance summary', async () => {
      await CommonUtils.captureAndAttachScreenshot(
        page,
        testInfo,
        `✅ Performance Test Passed - Invoice Send Time: ${sendSeconds}s`
      );
    });
  });
});
