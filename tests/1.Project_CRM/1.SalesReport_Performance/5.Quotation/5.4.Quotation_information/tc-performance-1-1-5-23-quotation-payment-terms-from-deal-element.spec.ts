...
/**
 * Quotation data verification - The Payment Terms on the Quotation matches the Payment Term selected on the Deal Element
 * Test Case ID: TC.Performance.1.1.5.23
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.23:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Payment Term chosen on the Deal Element is carried over to the Quotation.
 *
 * Pre-condition:
 ...
 * 2. Read the "Payment Terms" field of the Quotation information area
 *
 * Verification:
 * - The Payment Terms is the one selected on the Deal Element (Immediate Payment)
 *
...
const TC = 'TC.Performance.1.1.5.23';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Payment Terms',
};

test.describe(`${TC} - The Payment Terms on the Quotation matches the Payment Term selected on the Deal Element`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Payment Terms on the Quotation matches the Payment Term selected on the Deal Element`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const paymentTerms = await quotationPage.getFieldDisplayValue('payment_term_id');

      record('Payment Terms carried over from the Deal Element', DATA.paymentTerm, paymentTerms,
        paymentTerms.includes(DATA.paymentTerm));
      printVerify();
