...
/**
 * Quotation data verification - Send mail is unticked and Online Payment is ticked on a new Quotation
 * Test Case ID: TC.Performance.1.1.5.27
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.27:" --project=SalesReport_Performance
 *
 * Objective: Verify the two checkboxes a new Quotation starts on: "Send mail" is OFF (nothing is mailed behind the back of the salesperson) and "Online Payment" is ON.
 *
 * Pre-condition:
 ...
 * 2. Read the "Send mail" and "Online Payment" checkboxes of the information area
 *
 * Verification:
 * - Send mail is unticked
 * - Online Payment is ticked
 *
...
const TC = 'TC.Performance.1.1.5.27';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the checkbox defaults',
};

test.describe(`${TC} - Send mail is unticked and Online Payment is ticked on a new Quotation`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify Send mail is unticked and Online Payment is ticked on a new Quotation`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const sendMail = await quotationPage.isFieldChecked('send_mail');
      const onlinePayment = await quotationPage.isFieldChecked('require_payment');
