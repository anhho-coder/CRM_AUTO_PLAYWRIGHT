...
/**
 * Quotation data verification - The Total in Company Currency on a Quotation priced in a foreign currency is filled and converted
 * Test Case ID: TC.Performance.1.1.5.26
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.26:" --project=SalesReport_Performance
 *
 * Objective: Verify that "Total in Company Currency" is not a decoration: on a Quotation priced in EUR it is filled, greater than zero and DIFFERENT from the EUR Total, i.e. it really is the total converted into the company currency. The field is developer-only, so the record is re-opened with debug mode ON to read it - see TC.Performance.1.1.5.37.
 *
 * Pre-condition:
 ...
 * 4. Press "DEAL ELEMENT", select Pricelist = Public Pricelist_EUR (EUR) and Payment Term =
 ...
 * 2. Re-open the Quotation with Odoo developer (debug) mode ON
 * 3. Read the "Total in Company Currency" field of the information area
 * 4. Read the Quotation Total under the Order Lines
 *
 * Verification:
 * - Total in Company Currency is filled and greater than 0
 * - The Quotation Total (EUR) is greater than 0
 * - Total in Company Currency differs from the EUR Total - it is the converted amount
 *
...
const TC = 'TC.Performance.1.1.5.26';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying Total in Company Currency',
};

test.describe(`${TC} - The Total in Company Currency on a Quotation priced in a foreign currency is filled and converted`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Total in Company Currency on a Quotation priced in a foreign currency is filled and converted`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      await quotationPage.reopenCurrentRecordInDebugMode(baseUrl);
      const totalInCompanyCurrency = await quotationPage.getFieldNumber('amount_total_company_signed');
