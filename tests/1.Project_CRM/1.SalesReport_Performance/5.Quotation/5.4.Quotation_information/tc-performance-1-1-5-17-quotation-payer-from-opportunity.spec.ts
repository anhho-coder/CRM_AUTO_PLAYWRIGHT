...
/**
 * Quotation data verification - The Payer on the Quotation is filled and matches the Customer of the Opportunity
 * Test Case ID: TC.Performance.1.1.5.17
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.17:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Quotation inherits its Payer from the chain Opportunity -> Deal Element -> Quotation, i.e. the Payer is the Company Odoo created for the Opportunity.
 *
 * Pre-condition:
 ...
 * 2. Read the "Payer" field of the Quotation information area
 *
 * Verification:
 * - The Payer is filled (not empty)
 * - The Payer is the Company of the Opportunity
 *
...
const TC = 'TC.Performance.1.1.5.17';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Payer',
};

test.describe(`${TC} - The Payer on the Quotation is filled and matches the Customer of the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Payer on the Quotation is filled and matches the Customer of the Opportunity`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      record('Payer matches the Opportunity Company', oppCompany, payer);
      printVerify();

      expect(payer, 'the Quotation Payer must be filled').not.toBe('');
      expect(payer, 'the Quotation Payer must be the Company of the Opportunity').toBe(oppCompany);
    });
  });
});
