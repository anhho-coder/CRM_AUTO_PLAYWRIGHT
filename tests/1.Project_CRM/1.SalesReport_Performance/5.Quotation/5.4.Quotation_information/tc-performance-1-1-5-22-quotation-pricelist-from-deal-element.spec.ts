...
/**
 * Quotation data verification - The Pricelist on the Quotation matches the Pricelist selected on the Deal Element
 * Test Case ID: TC.Performance.1.1.5.22
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.22:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Pricelist chosen on the Deal Element is carried over to the Quotation.
 *
 * Pre-condition:
 ...
 * 2. Read the "Pricelist" field of the Quotation information area
 *
 * Verification:
 * - The Pricelist is the one selected on the Deal Element (Public Pricelist_USD)
 *
...
const TC = 'TC.Performance.1.1.5.22';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Pricelist',
};

test.describe(`${TC} - The Pricelist on the Quotation matches the Pricelist selected on the Deal Element`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Pricelist on the Quotation matches the Pricelist selected on the Deal Element`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const pricelist = await quotationPage.getFieldDisplayValue('pricelist_id');

      record('Pricelist carried over from the Deal Element', DATA.pricelist, pricelist,
        pricelist.includes(DATA.pricelist));
      printVerify();
