...
/**
 * Quotation data verification - Distributor, Reseller, their contacts, Promotion and PO are empty on a Quotation created from an Opportunity that carries none
 * Test Case ID: TC.Performance.1.1.5.28
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.28:" --project=SalesReport_Performance
 *
 * Objective: Verify that the fields nobody filled are genuinely empty on a new Quotation - no stray default is written into the partner, contact, promotion or PO fields.
 *
 * Pre-condition:
 ...
 * 2. Read the "Distributor", "Distributor contact", "Reseller", "Reseller contact", "Promotion" and "PO" fields
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.5.28';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the fields left empty',
};

test.describe(`${TC} - Distributor, Reseller, their contacts, Promotion and PO are empty on a Quotation created from an Opportunity that carries none`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify Distributor, Reseller, their contacts, Promotion and PO are empty on a Quotation created from an Opportunity that carries none`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      const distributorContact = await quotationPage.getFieldPartnerName('distributor_contact_id');
      const reseller = await quotationPage.getFieldPartnerName('reseller_id');
      const resellerContact = await quotationPage.getFieldPartnerName('reseller_contact_id');
      const promotion = await quotationPage.getFieldDisplayValue('promotion_id');
      const po = await quotationPage.getFieldDisplayValue('partner_purchase_order');
