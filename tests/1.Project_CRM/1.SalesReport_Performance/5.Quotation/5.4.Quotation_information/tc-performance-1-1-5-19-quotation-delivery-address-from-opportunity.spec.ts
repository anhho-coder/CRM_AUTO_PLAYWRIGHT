...
/**
 * Quotation data verification - The Delivery Address on the Quotation is filled and matches the Customer of the Opportunity
 * Test Case ID: TC.Performance.1.1.5.19
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.19:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Quotation inherits its Delivery Address from the chain Opportunity -> Deal Element -> Quotation.
 *
 * Pre-condition:
 ...
 * 2. Read the "Delivery Address" field of the Quotation information area
 *
 * Verification:
 * - The Delivery Address is filled (not empty)
 * - The Delivery Address is the Company of the Opportunity
 *
...
const TC = 'TC.Performance.1.1.5.19';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Delivery Address',
};

test.describe(`${TC} - The Delivery Address on the Quotation is filled and matches the Customer of the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Delivery Address on the Quotation is filled and matches the Customer of the Opportunity`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      record('Delivery Address is filled', 'not empty', deliveryAddress || '(empty)', deliveryAddress !== '');
      record('Delivery Address matches the Opportunity Company', oppCompany, deliveryAddress);
      printVerify();

      expect(deliveryAddress, 'the Quotation Delivery Address must be filled').not.toBe('');
      expect(deliveryAddress, 'the Delivery Address must be the Company of the Opportunity').toBe(oppCompany);
    });
  });
});
