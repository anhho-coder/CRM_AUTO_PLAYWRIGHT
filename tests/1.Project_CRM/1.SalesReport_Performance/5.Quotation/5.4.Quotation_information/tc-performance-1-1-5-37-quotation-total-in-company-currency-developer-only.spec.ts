...
/**
 * Quotation data verification - Total in Company Currency is a developer-only field on the Quotation
 * Test Case ID: TC.Performance.1.1.5.37
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.37:" --project=SalesReport_Performance
 *
 * Objective: Verify that "Total in Company Currency" is NOT part of the screen a salesperson works on: a normal session shows 17 fields without it, and the very same record shows 18 fields with it once Odoo developer (debug) mode is switched on.
 *
 * Pre-condition:
 ...
 * 2. Read the field names of the Quotation information area in the normal session
 * 3. Re-open the same Quotation with Odoo developer (debug) mode ON
 * 4. Read the field names again
 *
 * Verification:
 * - In a normal session the information area shows 17 fields and carries NO "Total in Company Currency"
 * - With debug mode ON the same record shows 18 fields
 * - The extra field is "Total in Company Currency", at position 11
 *
...
const TC = 'TC.Performance.1.1.5.37';
...
test.describe(`${TC} - Total in Company Currency is a developer-only field on the Quotation`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify Total in Company Currency is a developer-only field on the Quotation`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const normalLabels = await quotationPage.getVisibleFieldLabels();
      await quotationPage.reopenCurrentRecordInDebugMode(baseUrl);
      const debugLabels = await quotationPage.getVisibleFieldLabels();
