...
/**
 * Quotation data verification - The Reseller and the Distributor on the Quotation match the ones entered on the Opportunity
 * Test Case ID: TC.Performance.1.1.5.21
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.21:" --project=SalesReport_Performance
 *
 * Objective: Verify that an Opportunity carrying a Reseller and a Distributor produces a Quotation whose Reseller and Distributor are those same two partners, and whose contact fields stay empty because neither partner record carries a contact.
 *
 * Pre-condition:
 ...
 * 2. Read the "Reseller", "Distributor", "Reseller contact" and "Distributor contact" fields
 *
 * Verification:
 * - The Reseller is the Reseller entered on the Opportunity
 * - The Distributor is the Distributor entered on the Opportunity
 * - The Reseller contact is empty
 * - The Distributor contact is empty
 *
...
const TC = 'TC.Performance.1.1.5.21';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Reseller and the Distributor',
};

test.describe(`${TC} - The Reseller and the Distributor on the Quotation match the ones entered on the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Reseller and the Distributor on the Quotation match the ones entered on the Opportunity`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const reseller = await quotationPage.getFieldPartnerName('reseller_id');
      const distributor = await quotationPage.getFieldPartnerName('distributor_id');
      const resellerContact = await quotationPage.getFieldDisplayValue('reseller_contact_id');
      const distributorContact = await quotationPage.getFieldDisplayValue('distributor_contact_id');

      record('Reseller = Opportunity Reseller', DATA.reseller, reseller);
      record('Distributor = Opportunity Distributor', DATA.distributor, distributor);
      ...
      expect(reseller, 'the Reseller must come from the Opportunity').toBe(DATA.reseller);
      expect(distributor, 'the Distributor must come from the Opportunity').toBe(DATA.distributor);
