...
/**
 * Quotation data verification - The End User on the Quotation is filled and matches the Customer of the Opportunity
 * Test Case ID: TC.Performance.1.1.5.20
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.20:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Quotation inherits its End User from the chain Opportunity -> Deal Element -> Quotation.
 *
 * Pre-condition:
 ...
 * 2. Read the "End User" field of the Quotation information area
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.5.20';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the End User',
};

test.describe(`${TC} - The End User on the Quotation is filled and matches the Customer of the Opportunity`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the End User on the Quotation is filled and matches the Customer of the Opportunity`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      expect(endUser, 'the Quotation End User must be filled').not.toBe('');
