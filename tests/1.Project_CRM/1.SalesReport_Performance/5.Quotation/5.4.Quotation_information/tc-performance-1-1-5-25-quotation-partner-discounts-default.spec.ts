...
/**
 * Quotation data verification - The Distributor Discount (%) and the Reseller Discount (%) are 0.00 on a Quotation without partners
 * Test Case ID: TC.Performance.1.1.5.25
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.25:" --project=SalesReport_Performance
 *
 * Objective: Verify that a Quotation created from an Opportunity that carries no Distributor and no partner level grants no partner discount - both percentages read 0.00.
 *
 * Pre-condition:
 ...
 * 2. Read the "Distributor Discount (%)" and "Reseller Discount (%)" rows of the information area
 *
 * Verification:
 * - Distributor Discount (%) is 0.00
 * - Reseller Discount (%) is 0.00
 *
...
const TC = 'TC.Performance.1.1.5.25';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the partner discounts',
};

test.describe(`${TC} - The Distributor Discount (%) and the Reseller Discount (%) are 0.00 on a Quotation without partners`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Distributor Discount (%) and the Reseller Discount (%) are 0.00 on a Quotation without partners`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const distributorDiscount = await quotationPage.getFieldNumber('distributor_discount');
      const resellerDiscount = await quotationPage.getFieldNumber('reseller_discount');
