...
/**
 * Quotation data verification - The Validity date on the Quotation is 30 days after the day it is created
 * Test Case ID: TC.Performance.1.1.5.24
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.24:" --project=SalesReport_Performance
 *
 * Objective: Verify that a new Quotation is valid for 30 days - the "Validity" field holds the creation day plus 30 days.
 *
 * Pre-condition:
 ...
 * 2. Read the "Validity" field of the Quotation information area
 *
 * Verification:
 * - The Validity date is the day the Quotation was created + 30 days
 *
...
const TC = 'TC.Performance.1.1.5.24';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Validity date',
};

test.describe(`${TC} - The Validity date on the Quotation is 30 days after the day it is created`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Validity date on the Quotation is 30 days after the day it is created`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const validityRaw = await quotationPage.getFieldDisplayValue('validity_date');
      ...
      const expectedText = `${expectedDate.getMonth() + 1}/${expectedDate.getDate()}/${expectedDate.getFullYear()}`;
