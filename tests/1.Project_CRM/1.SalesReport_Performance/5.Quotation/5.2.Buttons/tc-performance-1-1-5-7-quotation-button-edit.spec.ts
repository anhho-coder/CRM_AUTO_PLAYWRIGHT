...
/**
 * Quotation data verification - EDIT button on the Quotation
 * Test Case ID: TC.Performance.1.1.5.7
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.7:" --project=SalesReport_Performance
 *
 * Objective: Verify that pressing EDIT swaps the control panel over to SAVE + DISCARD, i.e. the Quotation form is editable, and that DISCARD brings the read-mode control panel back.
 *
 * Pre-condition:
 ...
 * 2. Press "EDIT" on the Quotation
 * 3. Observe the control panel
 *
 * Verification:
 * - The control panel shows exactly 2 buttons, in this order: SAVE, DISCARD
 * - The Quotation header still shows the 7 buttons of a draft Quotation
 *
...
const TC = 'TC.Performance.1.1.5.7';
...
const STEP = {
  ...
  s6: 'Step 6: Pressing EDIT and verifying the control panel',
};

/** The Quotation header of a draft Quotation - see TC.Performance.1.1.5.6. */
...
test.describe(`${TC} - EDIT button on the Quotation`, () => {
  ...
  test(`${TC}: Verify the EDIT button puts the Quotation form in edit mode`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      const editModeButtons = await quotationPage.getControlPanelButtons();
      const headerButtons = await quotationPage.getStatusbarButtons();
