...
/**
 * Quotation data verification - Duplicate button on the Quotation
 * Test Case ID: TC.Performance.1.1.5.12
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.12:" --project=SalesReport_Performance
 *
 * Objective: Verify that Duplicate lands the tester on a SECOND, different Quotation record that carries the same Payer and is itself on the Quotation stage.
 *
 * Pre-condition:
 ...
 * 2. Read the Quotation number and the Payer
 * 3. Press "Duplicate"
 *
 * Verification:
 * - A different record is opened (its id is not the id of the source Quotation)
 * - The copy carries a Quotation number of its own
 * - The copy carries the same Payer as the source Quotation
 * - The copy is on the Quotation stage
 *
...
const TC = 'TC.Performance.1.1.5.12';
...
const STEP = {
  ...
  s6: 'Step 6: Pressing Duplicate and verifying the copy',
};
...
test.describe(`${TC} - Duplicate button on the Quotation`, () => {
  ...
  test(`${TC}: Verify the Duplicate button creates a copy of the Quotation`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const sourceNumber = await quotationPage.getSalesOrderNumber();
      const sourcePayer = await quotationPage.getPayerName();

      const duplicate = await quotationPage.clickDuplicateQuotation();
      const copyNumber = await quotationPage.getSalesOrderNumber();
      const copyPayer = await quotationPage.getPayerName();
      const copyStage = await quotationPage.getActiveStatusBarStage();
      console.log(`  Quotation copy created: ${page.url()}`);

      record('Landed on a different record', 'true', String(duplicate.navigated));
      record('Copy record id differs from the source', `not ${duplicate.sourceRecordId}`, duplicate.newRecordId,
        duplicate.newRecordId !== '' && duplicate.newRecordId !== duplicate.sourceRecordId);
      ...
      record('Backend popup', '(none)', duplicate.popupText === '' ? '(none)' : duplicate.popupText);
      printVerify();

      expect(duplicate.navigated, 'Duplicate must open the copy').toBe(true);
      expect(duplicate.newRecordId, 'the copy must be a different record').not.toBe(duplicate.sourceRecordId);
      ...
      expect(duplicate.popupText, 'Duplicate must not raise an Odoo error dialog').toBe('');
    });
  });
});
