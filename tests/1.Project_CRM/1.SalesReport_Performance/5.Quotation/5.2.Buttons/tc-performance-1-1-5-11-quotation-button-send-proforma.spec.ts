...
/**
 * Quotation data verification - SEND PRO-FORMA INVOICE button on the Quotation
 * Test Case ID: TC.Performance.1.1.5.11
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.11:" --project=SalesReport_Performance
 *
 * Objective: Verify that SEND PRO-FORMA INVOICE opens the e-mail composer with the Payer proposed as the recipient and the Quotation number in the subject, and that Cancel closes it without sending. NOTE: pre-production sends the pro-forma through the SAME mail template as SEND BY EMAIL, so the subject reads "NAKIVO, Inc. Quotation (Ref SO...)" and does NOT name the pro-forma - verified on the live form on 2026-09-17.
 *
 * Pre-condition:
 ...
 * 2. Press "SEND PRO-FORMA INVOICE"
...
const TC = 'TC.Performance.1.1.5.11';
...
const STEP = {
  ...
  s6: 'Step 6: Pressing SEND PRO-FORMA INVOICE and verifying the composer',
};
...
test.describe(`${TC} - SEND PRO-FORMA INVOICE button on the Quotation`, () => {
  ...
  test(`${TC}: Verify the SEND PRO-FORMA INVOICE button opens the mail composer for the pro-forma invoice`, async ({ page }) => {
