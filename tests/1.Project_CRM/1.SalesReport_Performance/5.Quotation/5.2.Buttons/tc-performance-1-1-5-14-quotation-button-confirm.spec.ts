...
/**
 * Quotation data verification - CONFIRM button on the Quotation
 * Test Case ID: TC.Performance.1.1.5.14
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.14:" --project=SalesReport_Performance
 *
 * Objective: Verify that CONFIRM moves the record on to the Sale Order stage - the statusbar reads Sale Order, CONFIRM is consumed and the LOCK button appears.
 *
 * Pre-condition:
 ...
 * - The statusbar highlights the SALE ORDER stage
 * - The CONFIRM button is gone and the LOCK button is offered
 *
...
const TC = 'TC.Performance.1.1.5.14';
...
const STEP = {
  ...
  s6: 'Step 6: Pressing CONFIRM and verifying the Sale Order stage',
};
...
test.describe(`${TC} - CONFIRM button on the Quotation`, () => {
  ...
  test(`${TC}: Verify the CONFIRM button turns the Quotation into a Sale Order`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      record('Confirmed to Sale Order', 'true', String(verdict.confirmed));
      record('Stage after CONFIRM', EXPECTED_STAGE_AFTER_CONFIRM, stageAfter);
      record('CONFIRM button gone', 'true', String(verdict.confirmButtonGone));
      record('LOCK button offered', 'true', String(verdict.lockVisible));
      printVerify();

      expect(stageAfter, 'CONFIRM must move the record on to the Sale Order stage').toBe(
        EXPECTED_STAGE_AFTER_CONFIRM
      );
      expect(verdict.confirmButtonGone, 'CONFIRM must be consumed by the confirmation').toBe(true);
      expect(verdict.confirmed, 'the record must end up a Sale Order').toBe(true);
    });
  });
});
