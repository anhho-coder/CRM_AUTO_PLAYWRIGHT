...
/**
 * Quotation data verification - PREVIEW button on the Quotation
 * Test Case ID: TC.Performance.1.1.5.8
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.8:" --project=SalesReport_Performance
 *
 * Objective: Verify that PREVIEW opens the customer-facing portal page of this Quotation (the /my/orders route), i.e. the button does what its caption promises.
 *
 * Pre-condition:
 ...
 * 2. Read the Quotation number (SO...)
 * 3. Press "PREVIEW"
 *
 * Verification:
 * - The customer portal page of the Quotation is opened - its URL carries "/my/orders/"
 * - The portal page shows the Quotation number of the record PREVIEW was pressed on
 *
...
const TC = 'TC.Performance.1.1.5.8';
...
const STEP = {
  ...
  s6: 'Step 6: Pressing PREVIEW and verifying the portal page',
};
...
test.describe(`${TC} - PREVIEW button on the Quotation`, () => {
  ...
  test(`${TC}: Verify the PREVIEW button opens the customer portal view of the Quotation`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      record('Portal URL carries /my/orders/', 'true', String(/\/my\/orders\//.test(preview.url)));
      record('Portal page shows the Quotation number', soNumber, preview.bodyText.includes(soNumber) ? soNumber : '(not found)');
      printVerify();

      expect(preview.url, 'PREVIEW must open the customer portal page of the Quotation').toMatch(
        /\/my\/orders\//
      );
      expect(preview.bodyText, 'the portal page must show this Quotation').toContain(soNumber);
    });
  });
});
