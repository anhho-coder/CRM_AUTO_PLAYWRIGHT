...
/**
 * Quotation data verification - Buttons on a newly created Quotation
 * Test Case ID: TC.Performance.1.1.5.6
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.6:" --project=SalesReport_Performance
 *
 * Objective: Verify that a newly created Quotation carries exactly the 7 expected header buttons, with the expected captions, in the expected left-to-right order, and that the control panel offers EDIT and Action.
 *
 * Pre-condition:
 ...
 * 2. Observe the buttons of the Quotation header and of the control panel
 *
 * Verification:
 * - The form header shows exactly 7 buttons, in this order:
 *   1. SEND BY EMAIL
 *   2. SEND PRO-FORMA INVOICE
 *   3. PRINT
 *   4. CONFIRM
 *   5. PREVIEW
 *   6. CANCEL
 *   7. Duplicate
 * - The control panel shows exactly 2 buttons, in this order: EDIT, Action
 *
...
const TC = 'TC.Performance.1.1.5.6';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the buttons on the new Quotation',
};

/**
 * The Quotation header as pre-production renders it for a freshly created Quotation (state =
 * draft), read left to right. Grounded on the live form on 2026-09-17 by a DOM dump of the live
 * screen - the buttons Odoo keeps in the DOM and hides for
 * the current state are not part of it.
 */
...
test.describe(`${TC} - Buttons on a newly created Quotation`, () => {
  ...
  test(`${TC}: Verify the number, the names and the order of the buttons on a newly created Quotation`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      const controlPanelButtons = await quotationPage.getControlPanelButtons();
