...
/**
 * Quotation data verification - Notebook tabs of the Quotation
 * Test Case ID: TC.Performance.1.1.5.16
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.16:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Quotation notebook carries exactly the expected tabs, with the expected captions, in the expected left-to-right order.
 *
 * Pre-condition:
 ...
 * 2. Observe the notebook tabs at the bottom of the form
 *
 * Verification:
 * - The notebook shows exactly 4 tabs, in this order:
 *   1. Order Lines
 *   2. Optional Products
 *   3. Other Information
 *   4. Approvals
...
const TC = 'TC.Performance.1.1.5.16';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the notebook tabs',
};
...
test.describe(`${TC} - Notebook tabs of the Quotation`, () => {
  ...
  test(`${TC}: Verify the name, number and order of the Quotation notebook tabs`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const tabs = await quotationPage.getNotebookTabs();
