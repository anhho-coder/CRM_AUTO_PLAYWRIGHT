...
/**
 * Quotation data verification - Field names and order of the Quotation information area
 * Test Case ID: TC.Performance.1.1.5.15
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.15:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Quotation information area carries exactly the expected labelled fields, with the expected captions, in the expected top-to-bottom order (left column first, then right).
 *
 * Pre-condition:
 ...
 * 2. Observe the fields of the Quotation information area
 *
 * Verification:
 * - The information area shows exactly 17 labelled fields, in this order:
 *   1. Payer
 *   2. Invoice Address
 *   3. Delivery Address
 *   4. End User
 *   5. Distributor
 *   6. Distributor contact
 *   7. Reseller
 *   8. Reseller contact
 *   9. Distributor Discount (%)
 *   10. Reseller Discount (%)
 *   11. Send mail
 *   12. Validity
 *   13. Pricelist
 *   14. Payment Terms
...
const TC = 'TC.Performance.1.1.5.15';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the field names and their order',
};

/**
 * The Quotation information area as pre-production renders it, read top-left down and then
 * top-right down. Grounded on the live form on 2026-09-17 by a DOM dump of the live
 * screen - the technical fields Odoo keeps hidden behind
 * attrs are not part of it.
 */
...
test.describe(`${TC} - Field names and order of the Quotation information area`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the field names and their order in the Quotation information area`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const labels = await quotationPage.getVisibleFieldLabels();
