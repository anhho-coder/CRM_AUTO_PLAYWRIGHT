...
/**
 * Quotation data verification - Log note of the created Quotation
 * Test Case ID: TC.Performance.1.1.5.33
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.33:" --project=SalesReport_Performance
 *
 * Objective: Verify that creating the Quotation leaves a trace in the chatter: a log note reporting that the Sale Order was created.
 *
 * Pre-condition:
 ...
 * 2. Read the Log note area of the Quotation
 *
 * Verification:
 * - The chatter carries at least one log note
 * - One log note reports "Sale Order created"
 *
...
const TC = 'TC.Performance.1.1.5.33';
...
test.describe(`${TC} - Log note of the created Quotation`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Log note reports "Sale Order created" when the Quotation is created`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const note = await quotationPage.waitForChatterMessage(/Sale Order created/i);
      const messages = await quotationPage.getChatterMessages();
