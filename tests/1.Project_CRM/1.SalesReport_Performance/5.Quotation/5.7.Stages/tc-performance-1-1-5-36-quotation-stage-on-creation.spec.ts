...
/**
 * Quotation data verification - Stage of a newly created Quotation
 * Test Case ID: TC.Performance.1.1.5.36
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.36:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Quotation the Deal Element creates starts on the first stage - the statusbar highlights it, and the server holds the same state.
 *
 * Pre-condition:
 ...
 * 2. Observe which stage the statusbar highlights
 *
 * Verification:
 * - The highlighted stage is the first one of the statusbar
 * - The status the server holds for the record is Quotation
 *
...
const TC = 'TC.Performance.1.1.5.36';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the stage of the new Quotation',
};
...
test.describe(`${TC} - Stage of a newly created Quotation`, () => {
  ...
  test(`${TC}: Verify a newly created Quotation sits on the first stage of the statusbar`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      console.log(STEP.s6);
      const stages = await quotationPage.getStatusBarStages();
      const activeStage = await quotationPage.getActiveStatusBarStage();
      const serverStatus = await quotationPage.getQuotationStatusFromServer();
