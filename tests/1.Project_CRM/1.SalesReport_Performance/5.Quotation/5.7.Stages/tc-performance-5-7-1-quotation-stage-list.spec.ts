...
/**
 * Quotation data verification - Stage list of the Quotation
 * Test Case ID: TC.Performance.5.7.1
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.35:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Quotation statusbar carries exactly the expected stages, with the expected captions, in the expected left-to-right order.
 *
 * Pre-condition:
 ...
 * 2. Observe the stages at the top right of the Quotation form
 *
 * Verification:
 * - The statusbar shows exactly 3 stages, in this order:
 *   1. QUOTATION
 *   2. QUOTATION SENT
 *   3. SALE ORDER
 *
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the stage list',
};

/**
 * The stages the Quotation statusbar offers, left to right, as pre-production renders them.
 * Grounded on the live form on 2026-09-17 by the a DOM dump of the live screen.
 */
...
test.describe(`${TC} - Stage list of the Quotation`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the name, number and order of the stages the Quotation statusbar offers`, async ({ page }) => {
