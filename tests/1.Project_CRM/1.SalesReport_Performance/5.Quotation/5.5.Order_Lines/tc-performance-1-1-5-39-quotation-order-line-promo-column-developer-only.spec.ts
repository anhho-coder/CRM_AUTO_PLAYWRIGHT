...
/**
 * Quotation data verification - Promo Discount Amount column of the Quotation Order Lines
 * Test Case ID: TC.Performance.1.1.5.39
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.39:" --project=SalesReport_Performance
 *
 * Objective: Verify that "Promo Discount Amount" is NOT part of the Order Lines list a salesperson works on: a normal session shows 13 columns without it, and the very same record shows 14 columns with it once Odoo developer (debug) mode is switched on.
 *
 * Pre-condition:
 ...
 * 2. Open the "Order Lines" tab and read the column headers
 * 3. Re-open the same Quotation with Odoo developer (debug) mode ON
 * 4. Open the "Order Lines" tab and read the column headers again
 *
 * Verification:
 * - In a normal session the Order Lines list shows 13 columns and carries NO "Promo Discount Amount"
 * - With debug mode ON the same record shows 14 columns
 * - The extra column is "Promo Discount Amount", at position 10
 *
...
const TC = 'TC.Performance.1.1.5.39';
...
test.describe(`${TC} - Promo Discount Amount column of the Quotation Order Lines`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the Promo Discount Amount column of the Quotation Order Lines is developer-only`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      const normalColumns = await quotationPage.getOrderLineColumns();

      await quotationPage.reopenCurrentRecordInDebugMode(baseUrl);
      await quotationPage.clickOrderLinesTab();
      const debugColumns = await quotationPage.getOrderLineColumns();
