...
/**
 * Quotation data verification - Columns of the Quotation Order Lines
 * Test Case ID: TC.Performance.1.1.5.29
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.29:" --project=SalesReport_Performance
 *
 * Objective: Verify that the Order Lines list of the Quotation carries exactly the expected columns - including the discount columns Partner Discount and Partner Discount Amount - with the expected captions, in the expected left-to-right order.
 *
 * Pre-condition:
 ...
 * 2. Open the "Order Lines" tab
 * 3. Observe the column headers of the Order Lines list
 *
 * Verification:
 * - The Order Lines list shows exactly 13 named columns, in this order:
 *   1. Product
 *   2. Start Date
 *   3. End Date
 *   4. Ordered Qty
 ...
 *   8. Special Discount(%)
 *   9. Special Discount Amount
 *   10. Partner Discount
 *   11. Partner Discount Amount
 *   12. Subtotal After All Discounts
 *   13. Subtotal
 *
...
const TC = 'TC.Performance.1.1.5.29';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the Order Lines columns',
};

/**
 * The Order Lines columns as pre-production renders them on a Quotation, left to right.
 * Grounded on the live form on 2026-09-17 with a DOM dump of the live
 * screen. Columns Odoo renders without a caption (the drag handle, the delete column) are not part
 * of it.
 */
...
test.describe(`${TC} - Columns of the Quotation Order Lines`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  ...
  test(`${TC}: Verify the name, number and order of the Quotation Order Lines columns`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      const columns = await quotationPage.getOrderLineColumns();
