...
/**
 * Quotation data verification - Order line of the Quotation
 * Test Case ID: TC.Performance.1.1.5.30
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.30:" --project=SalesReport_Performance
 *
 * Objective: Verify that the product added on the Deal Element reaches the Quotation unchanged: one order line, the NAKIVO Backup product, Ordered Qty 1, a filled Unit of Measure and a Unit Price greater than zero.
 *
 * Pre-condition:
 ...
 * 3. Read the first order line
 *
 * Verification:
 * - The Order Lines list carries exactly 1 line
 * - The Product of the line is the NAKIVO Backup product added on the Deal Element
 * - Ordered Qty is 1
 * - The Unit of Measure is filled
...
const TC = 'TC.Performance.1.1.5.30';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the order line',
};
...
test.describe(`${TC} - Order line of the Quotation`, () => {
  ...
  test(`${TC}: Verify the Quotation order line carries the product of the Deal Element, with quantity 1 and its Unit of Measure`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      const rowCount = await quotationPage.getOrderLineRowCount();
      const cells = await quotationPage.getOrderLineRowCells(0);
      const product = cells['Product'] || '';
      const orderedQty = parseFloat((cells['Ordered Qty'] || '').replace(/,/g, '')) || 0;
      const uom = cells['Unit of Measure'] || '';
      const unitPrice = parseFloat((cells['Unit Price'] || '').replace(/,/g, '')) || 0;

      record('Number of order lines', 1, rowCount);
      record('Product of the line', `contains "${DATA.product}"`, product || '(empty)',
        product.includes(DATA.product));
