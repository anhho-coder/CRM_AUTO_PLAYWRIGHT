...
/**
 * Quotation data verification - Subtotals of the Quotation order line
 * Test Case ID: TC.Performance.1.1.5.32
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.32:" --project=SalesReport_Performance
 *
 * Objective: Verify the arithmetic of the order line: with no discount applied, "Subtotal before discount", "Subtotal After All Discounts" and "Subtotal" all equal Unit Price x Ordered Qty.
 *
 * Pre-condition:
 ...
 * 3. Read Unit Price, Ordered Qty and the three subtotal cells of the first order line
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.5.32';
...
test.describe(`${TC} - Subtotals of the Quotation order line`, () => {
  ...
  test(`${TC}: Verify the Quotation order line subtotals equal Unit Price multiplied by Ordered Qty`, async ({ page }) => {
    ...
    await test.step(STEP.s6, async () => {
      ...
      const unitPrice = num('Unit Price');
      const orderedQty = num('Ordered Qty');
      const expectedSubtotal = Math.round(unitPrice * orderedQty * 100) / 100;
      const beforeDiscount = num('Subtotal before discount');
      const afterAllDiscounts = num('Subtotal After All Discounts');
