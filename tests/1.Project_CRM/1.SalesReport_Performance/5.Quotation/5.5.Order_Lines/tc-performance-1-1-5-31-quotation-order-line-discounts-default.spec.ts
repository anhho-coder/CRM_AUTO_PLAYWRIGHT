...
/**
 * Quotation data verification - Discount columns of the Quotation order line
 * Test Case ID: TC.Performance.1.1.5.31
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Command to run:
 * npx playwright test --grep "TC\\.Performance\\.1\\.1\\.5\\.31:" --project=SalesReport_Performance
 *
 * Objective: Verify that a Quotation built from an Opportunity with no partner and no promotion grants no discount at all - every discount cell of the order line reads 0.00.
 *
 * Pre-condition:
 ...
 * 3. Read the discount cells of the first order line
 *
 * Verification:
...
const TC = 'TC.Performance.1.1.5.31';
...
const STEP = {
  ...
  s6: 'Step 6: Verifying the discount columns',
};

/**
 * The discount columns a NORMAL session shows on the Quotation Order Lines list - see
 * TC.Performance.1.1.5.29. "Promo Discount Amount" is developer-only and is covered by
 * TC.Performance.1.1.5.39.
 */
...
test.describe(`${TC} - Discount columns of the Quotation order line`, () => {
  ...
  test(`${TC}: Verify every discount column of the Quotation order line is 0.00 when no discount applies`, async ({ page }) => {
