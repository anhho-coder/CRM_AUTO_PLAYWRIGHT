...
/**
 * =============================================================================================
 *  The APPROVE button moves the License from Draft to Approved and swaps the header button set
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.7.7
 ...
 * ---------------------------------------------------------------------------------------------
 *  Summary
 *    Creates its own licence, presses APPROVE and verifies both halves of what that button does: the
 *    state becomes Approved and the header swaps its four Draft buttons for the seven the Approved
 *    state offers, in the documented order.
 * ---------------------------------------------------------------------------------------------
 *  Command to run
 *    npx playwright test --grep "TC\.Performance\.1\.1\.7\.7:" --project=SalesReport_Performance
 * ---------------------------------------------------------------------------------------------
 *  Source manual TC
 *
 ...
 *
 *  Steps to reproduce
 *   1. Press the "APPROVE" button of the Draft licence
 *   2. Reload the licence record and read its state and its header buttons
 *
 *  Verification
 *   - The licence state becomes APPROVED
 *   - The header carries exactly 7 buttons
 *   - They read SEND BY EMAIL, SEND WITH CERTIFICATE, CERTIFICATE, SEND CERTIFICATE, CANCEL, SET TO DRAFT, TEST CREATING LICENSE FROM LM in that order
 * ---------------------------------------------------------------------------------------------
 *  Grounding
...
const TC = 'TC.Performance.1.1.7.7';
...
const STEP = {
  ...
  s1: 'Step 1: Press the "APPROVE" button of the Draft licence',
  s2: 'Step 2: Reload the licence record and read its state and its header buttons',
  verify: 'Verification',
} as const;

test.describe(`${TC} - The APPROVE button moves the License from Draft to Approved and swaps the header button set`, () => {
  let oppUrl = '';
  let dealElementUrl = '';
  let quotationUrl = '';
  let invoiceUrl = '';
  ...
  test(`${TC}: The APPROVE button moves the License from Draft to Approved and swaps the header button set`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s2, async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - licence approved');
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      const EXPECTED = [
        ...
        'CANCEL',
        'SET TO DRAFT',
        'TEST CREATING LICENSE FROM LM',
      ];

      record('The licence was in Draft before APPROVE', 'DRAFT', stateBefore);
      record('The licence state after APPROVE', 'APPROVED', stateAfter);
      record('Number of buttons in the Approved licence header', EXPECTED.length, headerLabels.length);
      ...
      expect(stateAfter, 'APPROVE must move the licence to Approved').toBe('APPROVED');
      expect(headerLabels, 'the Approved licence header must carry exactly 7 buttons').toHaveLength(EXPECTED.length);
      expect(headerLabels, 'the Approved button names and their order must match the documented list').toEqual(EXPECTED);
    });
  });
});
