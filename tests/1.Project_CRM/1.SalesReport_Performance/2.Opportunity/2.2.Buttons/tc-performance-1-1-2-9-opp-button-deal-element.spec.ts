 ...
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.9
 ...
 *  test case owns: verify that pressing DEAL ELEMENT on a saved Opportunity opens a new, unsaved sale.order form that is bound to that Opportunity.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.9:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.9 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.9
 ...
 *    4. Press the DEAL ELEMENT button of the saved Opportunity
 *    5. Read the record that opens
 *
 *  Verification:
 *    5. A new Deal Element opens:
 *    - The URL carries model=sale.order and active_id=<the Opportunity id>
 *    - The breadcrumb reads Pipeline > <the Opportunity> > New
 *    - The form is in edit mode and the record is not saved yet
 *
...
const TC = 'TC.Performance.1.1.2.9';
...
const STEP = {
  ...
  s4: 'Step 4: Press the DEAL ELEMENT button of the saved Opportunity',
  s5: 'Step 5: Read the record that opens',
};
...
test.describe(`${TC} - Buttons`, () => {
  ...
  test(`${TC}: Verify the DEAL ELEMENT button opens a new Deal Element for the Opportunity`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      record('It is bound to the Opportunity this test created', `active_id=${oppId}`, targetUrl.includes(`active_id=${oppId}`) ? `active_id=${oppId}` : targetUrl);
      record('The breadcrumb keeps the Opportunity and ends on a new record', `${DATA.oppName} > New`, breadcrumb.slice(-2).join(' > '));
      ...
      expect(targetUrl, 'the new Deal Element must be bound to this Opportunity').toContain(`active_id=${oppId}`);
      expect(breadcrumb[breadcrumb.length - 1], 'the breadcrumb of the new Deal Element').toBe('New');
      expect(breadcrumb[breadcrumb.length - 2], 'the breadcrumb must keep the Opportunity').toBe(DATA.oppName);
