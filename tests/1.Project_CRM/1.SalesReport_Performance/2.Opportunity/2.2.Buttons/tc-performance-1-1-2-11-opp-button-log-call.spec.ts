 ...
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.11
 ...
 *  test case owns: verify that pressing LOG CALL on a saved Opportunity opens the "Log call" wizard, that the wizard carries the expected fields and buttons, and that CANCEL closes it without touching the Opportunity.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.11:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.11 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.11
 ...
 *    4. Press the LOG CALL button of the saved Opportunity
 *    5. Read the dialog that opens, then dismiss it with CANCEL
 *
 *  Verification:
 *    5. The "Log call" dialog opens:
 *    - Title = Log call
 *    - Fields = Call type | Date | Quote requested | Slides requested
 *    - Buttons = OK | CANCEL
...
const TC = 'TC.Performance.1.1.2.11';

const EXPECTED_TITLE = 'Log call';
...
const STEP = {
  ...
  s4: 'Step 4: Press the LOG CALL button of the saved Opportunity',
...
test.describe(`${TC} - Buttons`, () => {
  ...
  test(`${TC}: Verify the LOG CALL button opens the "Log call" dialog with its fields`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      await opportunityPage.clickStatusbarButtonByName('log_call');
    ...
    await test.step(STEP.s5, async () => {
      ...
      record('The dialog fields and their order', EXPECTED_FIELDS.join(' | '), labels.join(' | '));
      ...
      expect(title, 'the title of the LOG CALL dialog').toBe(EXPECTED_TITLE);
      expect(labels, 'the fields of the LOG CALL dialog').toEqual(EXPECTED_FIELDS);
      expect(buttons, 'the buttons of the LOG CALL dialog').toEqual(EXPECTED_BUTTONS);
