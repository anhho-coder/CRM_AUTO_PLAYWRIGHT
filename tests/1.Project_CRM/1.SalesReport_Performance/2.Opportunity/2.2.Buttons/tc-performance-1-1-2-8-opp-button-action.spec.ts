 ...
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.8
 ...
 *  test case owns: verify that the Action menu of a saved Opportunity offers exactly the expected actions, in the expected order.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.8:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.8 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.8
 ...
 *    4. Open the Action menu of the saved Opportunity and read its entries
 *
 *  Verification:
 *    4. The Action menu offers exactly 14 actions, in this order:
 *    - Delete | Duplicate | Mark Late Activities as Done | Mark All Activities as Done | Mark as Fake | Assign salesman of assigned partner | Unarhive | Lost Read | ActiveCampaign "Stop Automation" | Restore | Log Feature Request | Reschedule late activities | Merge | Export NAKIVO Survey report
 *
...
const TC = 'TC.Performance.1.1.2.8';

/**
 * The actions the Action menu of an Opportunity offers (pre-production, 2026-09-21).
 * "Unarhive" is spelled that way in the product - the typo is asserted on purpose.
 */
const EXPECTED = [
    'Delete',
    'Duplicate',
    'Mark Late Activities as Done',
    'Mark All Activities as Done',
    'Mark as Fake',
    'Assign salesman of assigned partner',
    'Unarhive',
    'Lost Read',
    'ActiveCampaign "Stop Automation"',
    'Restore',
    'Log Feature Request',
    'Reschedule late activities',
    'Merge',
    'Export NAKIVO Survey report',
  ];
...
const STEP = {
  ...
  s4: 'Step 4: Open the Action menu of the saved Opportunity and read its entries',
};
...
test.describe(`${TC} - Buttons`, () => {
  ...
  test(`${TC}: Verify the actions the Action menu of an Opportunity offers`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const actual = await opportunityPage.getControlPanelMenuItems('Action');
      actual.forEach((item, i) => console.log(`  Action entry #${i + 1}: ${item}`));
      record('Number of Action entries', EXPECTED.length, actual.length);
      EXPECTED.forEach((expectedItem, i) => {
        record(`Action entry #${i + 1}`, expectedItem, actual[i] === undefined ? '(missing)' : actual[i]);
      });
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual.length, 'the NUMBER of actions the Action menu offers').toBe(EXPECTED.length);
      expect(actual, 'the actions the Action menu offers, in order').toEqual(EXPECTED);
    });
  });
});
