...
/**
 * =============================================================================================
 *  Opportunity screen verification - Sub-menu
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.34
 *  Jira            : (authored with the batch - the Xray issue is created after the first green run)
 *  Automation-Type : new
 *  Automation-Date : 2026-09-21
 *  Feature folder  : tests/1.Project_CRM/1.SalesReport_Performance/2.Opportunity/2.5.Sub_menu
 *
 *  Summary
 *  ---------------------------------------------------------------------------------------------
 ...
 *  test case owns: verify that each sub-menu button of a newly created Opportunity prints the counter 0, i.e. that the Opportunity starts with no Demo, no Meeting, no Chain and no Quotation.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.34:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.34 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.34
 ...
 *    4. Read the counter each sub-menu button prints
 *
 *  Verification:
 *    4. Every sub-menu button prints the counter 0:
 *    - Demo = 0 | Meeting = 0 | Chains = 0 | Quotation(s) = 0
 *
...
const TC = 'TC.Performance.1.1.2.34';

/** The sub-menu buttons whose counter is read (pre-production, 2026-09-21). */
const EXPECTED = [
    'Demo',
    'Meeting',
    'Chains',
...
const STEP = {
  ...
  s4: 'Step 4: Read the counter each sub-menu button prints',
};

test.describe(`${TC} - Sub-menu`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify every sub-menu counter of a newly created Opportunity reports zero`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const counters: Array<{ caption: string; value: number }> = [];
      for (const caption of EXPECTED) {
        const value = await opportunityPage.getSmartButtonCount(caption);
        counters.push({ caption, value });
        console.log(`  ${caption.padEnd(14)}: ${Number.isNaN(value) ? '(button not shown)' : value}`);
        record(`The ${caption} counter of a new Opportunity`, 0, Number.isNaN(value) ? '(button not shown)' : value);
      }
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      counters.forEach((c) => {
        expect(Number.isNaN(c.value), `the sub-menu button "${c.caption}" must be shown`).toBe(false);
        expect(c.value, `the ${c.caption} counter of a newly created Opportunity`).toBe(0);
      });
    });
  });
});
