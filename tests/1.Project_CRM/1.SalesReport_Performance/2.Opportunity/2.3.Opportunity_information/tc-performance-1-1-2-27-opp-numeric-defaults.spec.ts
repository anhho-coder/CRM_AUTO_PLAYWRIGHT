...
/**
 * =============================================================================================
 *  Opportunity screen verification - Numeric defaults
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.27
 ...
 *  test case owns: verify the Budget, the Score, the Lifetime Revenue and the Followup Count a newly created Opportunity starts on.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.27:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.27 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.27
 ...
 *    4. Read the numeric fields of the information area on the saved form
 *
 *  Verification:
 *    4. The numeric fields of the information area read:
 *    - Budget = 0.00
 *    - Score = 2.00
...
const TC = 'TC.Performance.1.1.2.27';

/** The numeric defaults of a newly created Opportunity (pre-production, 2026-09-21). */
const EXPECTED_NUMBERS = [
  { field: 'budget', label: 'Budget', value: 0 },
  { field: 'score', label: 'Score', value: 2 },
  { field: 'lifetime_revenue', label: 'Lifetime Revenue', value: 0 },
  { field: 'followup_count', label: 'Followup Count', value: 0 },
];
...
const STEP = {
  ...
  s4: 'Step 4: Read the numeric fields of the information area on the saved form',
};

test.describe(`${TC} - Numeric defaults`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the numeric fields of a newly created Opportunity carry their default values`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      for (const item of EXPECTED_NUMBERS) {
        const actual = await opportunityPage.getFieldNumber(item.field);
        console.log(`  ${item.label.padEnd(18)}: ${actual}`);
        record(`${item.label} of a newly created Opportunity`, item.value, actual);
      }
      printVerify();
      ...
      for (const item of EXPECTED_NUMBERS) {
        const actual = await opportunityPage.getFieldNumber(item.field);
        expect(actual, `${item.label} of a newly created Opportunity`).toBe(item.value);
      }
    });
  });
});
