...
/**
 * =============================================================================================
 *  Opportunity screen verification - Opportunity name
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.14
 ...
 *  test case owns: verify that the Opportunity name stored on the saved record is exactly the value entered on the CREATE form, with no truncation and no re-formatting.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.14:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.14 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.14
 ...
 *    4. Read the Opportunity name on the saved form
 *
 *  Verification:
 *    4. The Opportunity name shows exactly the entered value "AUTO TC.Performance.1.1.2.14 <unique>"
 *    - No truncation, no re-formatting, no replacement by another value
 *    - The title block above the information area carries the same name
 *
...
const TC = 'TC.Performance.1.1.2.14';
...
const STEP = {
  ...
  s4: 'Step 4: Read the Opportunity name on the saved form',
};

test.describe(`${TC} - Opportunity name`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the Opportunity name on the saved Opportunity matches the entered value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const actual = await opportunityPage.getOpportunityNameValue();
      const title = await opportunityPage.getFormTitleText();
      console.log(`  Opportunity name entered : ${DATA.oppName}`);
      console.log(`  Opportunity name stored  : ${actual}`);
      console.log(`  Form title block         : ${title}`);
      record('The Opportunity name on the saved form', DATA.oppName, actual);
      record('The form title carries the Opportunity name', 'contains the name', title.includes(DATA.oppName) ? 'contains the name' : title);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual, 'the Opportunity name on the saved form').toBe(DATA.oppName);
      expect(title, 'the form title must carry the Opportunity name').toContain(DATA.oppName);
    });
  });
});
