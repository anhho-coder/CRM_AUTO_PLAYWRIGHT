...
/**
 * =============================================================================================
 *  Opportunity screen verification - Checkbox defaults
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.28
 ...
 *  test case owns: verify that every checkbox the information area of a newly created Opportunity renders is unticked, including the Create manually flag the CREATE form cleared.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.28:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.28 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.28
 ...
 *    4. Read every checkbox of the information area on the saved form
 *
 *  Verification:
 *    4. Every checkbox of the information area is unticked:
 *    - Quoted, Testing, Targeted, Is imported leadQualified, Another Offices, Top Deal, New address ?, Development required and Create manually are all FALSE
 *
...
const TC = 'TC.Performance.1.1.2.28';

/** Every checkbox the information area of an Opportunity renders (pre-production, 2026-09-21). */
const CHECKBOXES = [
  { field: 'quoted', label: 'Quoted' },
  { field: 'testing', label: 'Testing' },
  { field: 'targeted', label: 'Targeted' },
  { field: 'is_imported_lead_qualified', label: 'Is imported leadQualified' },
  { field: 'another_offices', label: 'Another Offices' },
  { field: 'x_studio_top_deal', label: 'Top Deal' },
  { field: 'new_address', label: 'New address ?' },
  { field: 'development_required', label: 'Development required' },
  { field: 'is_create_manual', label: 'Create manually' },
];
...
const STEP = {
  ...
  s4: 'Step 4: Read every checkbox of the information area on the saved form',
};
...
test.describe(`${TC} - Checkbox defaults`, () => {
  ...
  test(`${TC}: Verify the state of every checkbox of the Opportunity information area on a newly created Opportunity`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const states: Array<{ label: string; actual: boolean | null }> = [];
      for (const item of CHECKBOXES) {
        const actual = await opportunityPage.isFieldChecked(item.field);
        states.push({ label: item.label, actual });
        console.log(`  ${item.label.padEnd(26)}: ${actual === null ? '(no checkbox on the form)' : actual}`);
        record(`${item.label} is unticked on a new Opportunity`, 'false', actual === null ? '(no checkbox on the form)' : String(actual));
      }
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      states.forEach((s) => {
        expect(s.actual, `${s.label} must render a checkbox on the Opportunity form`).not.toBeNull();
        expect(s.actual, `${s.label} must be unticked on a newly created Opportunity`).toBe(false);
      });
    });
  });
});
