...
/**
 * =============================================================================================
 *  Opportunity screen verification - Salesperson
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.23
 ...
 *  test case owns: verify that an Opportunity saved with an empty Salesperson is given one by the assignment job, and that the job picks the Salesperson the routing rules name for this data.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.23:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.23 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.23
 ...
 *    4. Read the Salesperson on the saved form, reloading until the assignment job has run
 *
 *  Verification:
 *    4. The Salesperson field, left empty on the CREATE form, is filled by the assignment job
 *    - Salesperson = Thomas Semerich (the owner of BDEU_Andrew_Turk_May_2023 for this data)
 *
...
const TC = 'TC.Performance.1.1.2.23';

/** The Salesperson the routing rules give this data - the same expectation as the baseline TC.Performance.1.1.2.1. */
...
const STEP = {
  ...
  s4: 'Step 4: Read the Salesperson on the saved form, reloading until the assignment job has run',
};

test.describe(`${TC} - Salesperson`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the Salesperson is filled on the saved Opportunity by the assignment job`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const actual = await opportunityPage.waitForFieldDisplayValue('user_id', EXPECTED_SALESPERSON, 6);
      console.log(`  Salesperson entered on the CREATE form : (cleared)`);
      console.log(`  Salesperson on the saved Opportunity   : ${actual}`);
      record('The Salesperson is no longer empty', 'a non-empty Salesperson', actual === '' ? '(empty)' : 'a non-empty Salesperson');
      record('The Salesperson the assignment job picked', EXPECTED_SALESPERSON, actual);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual, 'the assignment job must fill the Salesperson').not.toBe('');
      expect(actual, 'the Salesperson the assignment job picked').toBe(EXPECTED_SALESPERSON);
    });
  });
});
