...
/**
 * =============================================================================================
 *  Opportunity screen verification - Lead Source
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.25
 ...
 *  test case owns: verify that an Opportunity whose Sales Team and Salesperson were cleared on the CREATE form is classified as a Nakivo lead, not as a Partner lead.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.25:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.25 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.25
 ...
 *    4. Read the Lead Source on the saved form
 *
 *  Verification:
 *    4. Lead Source = Nakivo
 *    - because no Sales Team and no Salesperson were set at creation
 *
...
const TC = 'TC.Performance.1.1.2.25';
...
const STEP = {
  ...
  s4: 'Step 4: Read the Lead Source on the saved form',
};

test.describe(`${TC} - Lead Source`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify an Opportunity created without an owner is classified Lead Source = Nakivo`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const actual = await opportunityPage.waitForFieldDisplayValue('lead_source', EXPECTED_SOURCE, 6);
      console.log(`  Lead Source : ${actual}`);
      record('The Lead Source of an Opportunity created without an owner', EXPECTED_SOURCE, actual);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual, 'the Lead Source of an Opportunity created without an owner').toBe(EXPECTED_SOURCE);
    });
  });
});
