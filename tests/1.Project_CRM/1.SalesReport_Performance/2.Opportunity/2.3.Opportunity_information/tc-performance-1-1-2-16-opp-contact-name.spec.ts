...
/**
 * =============================================================================================
 *  Opportunity screen verification - Contact Name
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.16
 ...
 *  test case owns: verify that the Contact Name stored on the saved Opportunity is exactly the value entered on the CREATE form.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.16:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.16 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.16
 ...
 *    4. Read the Contact Name on the saved form
 *
 *  Verification:
 *    4. Contact Name shows exactly the entered value "Contact Name TC.Performance.1.1.2.16"
 *    - The value is not overwritten by the Company Name nor by the e-mail local part
 *
...
const TC = 'TC.Performance.1.1.2.16';
...
const STEP = {
  ...
  s4: 'Step 4: Read the Contact Name on the saved form',
};

test.describe(`${TC} - Contact Name`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the Contact Name on the saved Opportunity matches the entered value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const actual = await opportunityPage.getFieldDisplayValue('contact_name');
      console.log(`  Contact Name entered : ${DATA.contactName}`);
      console.log(`  Contact Name stored  : ${actual}`);
      record('The Contact Name on the saved form', DATA.contactName, actual);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual, 'the Contact Name on the saved form').toBe(DATA.contactName);
    });
  });
});
