...
/**
 * =============================================================================================
 *  Opportunity screen verification - Company (Customer)
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.17
 ...
 *  test case owns: verify that saving an Opportunity with an e-mail creates the Customer behind it and links it in the Company field of the information area.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.17:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.17 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.17
 ...
 *    4. Read the Company (Customer) on the saved form
 *
 *  Verification:
 *    4. The Company field is filled and links the Customer "company<unique>.com"
 *    - The Customer was created by the system, not by the tester
 *
...
const TC = 'TC.Performance.1.1.2.17';
...
const STEP = {
  ...
  s4: 'Step 4: Read the Company (Customer) on the saved form',
};

test.describe(`${TC} - Company (Customer)`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the Company (Customer) is created automatically and linked to the Opportunity`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const actual = await opportunityPage.getFieldPartnerName('partner_id');
      console.log(`  Company (Customer) linked : ${actual}`);
      console.log(`  Expected from the e-mail  : ${expectedCompany}`);
      record('A Customer is linked to the Opportunity', 'a non-empty Customer', actual === '' ? '(empty)' : 'a non-empty Customer');
      record('The linked Customer is the one created from the e-mail domain', expectedCompany, actual);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual, 'a Customer must be linked to the Opportunity').not.toBe('');
      expect(actual, 'the Customer created from the e-mail domain').toBe(expectedCompany);
    });
  });
});
