...
/**
 * =============================================================================================
 *  Opportunity screen verification - Contact
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.18
 ...
 *  test case owns: verify that the Contact field of the saved Opportunity links the contact person the system created under the Customer, printed as "<Company>, <Contact Name>".
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.18:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.18 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.18
 ...
 *    4. Read the Contact on the saved form
 *
 *  Verification:
 *    4. The Contact field reads "company<unique>.com, Contact Name TC.Performance.1.1.2.18"
 *    - i.e. the Company created from the e-mail domain, followed by the entered Contact Name
 *
...
const TC = 'TC.Performance.1.1.2.18';
...
const STEP = {
  ...
  s4: 'Step 4: Read the Contact on the saved form',
};

test.describe(`${TC} - Contact`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the Contact of the saved Opportunity carries the Company and the Contact Name`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const expectedContact = `${expectedCompany}, ${DATA.contactName}`;
      const actual = await opportunityPage.waitForFieldDisplayValue('contact_partner_id', expectedContact, 5);
      console.log(`  Contact stored   : ${actual}`);
      console.log(`  Contact expected : ${expectedContact}`);
      record('The Contact links the person created under the Customer', expectedContact, actual);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual, 'the Contact of the saved Opportunity').toBe(expectedContact);
    });
  });
});
