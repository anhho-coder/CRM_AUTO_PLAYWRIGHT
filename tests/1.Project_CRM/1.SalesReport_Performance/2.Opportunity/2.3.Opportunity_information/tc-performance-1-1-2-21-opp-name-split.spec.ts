...
/**
 * =============================================================================================
 *  Opportunity screen verification - First / Middle / Last Name
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.21
 ...
 *  test case owns: verify that the system splits the entered Contact Name into First Name, Middle Name and Last Name on the saved Opportunity.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.21:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.21 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.21
 ...
 *    4. Read the First Name, the Middle Name and the Last Name on the saved form
 *
 *  Verification:
 *    4. The Contact Name "Contact Name TC.Performance.1.1.2.21" is split into:
 *    - First Name = Contact
 *    - Middle Name = Name
 *    - Last Name = TC.Performance.1.1.2.21
 *
...
const TC = 'TC.Performance.1.1.2.21';
...
const STEP = {
  ...
  s4: 'Step 4: Read the First Name, the Middle Name and the Last Name on the saved form',
};

test.describe(`${TC} - First / Middle / Last Name`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the First, Middle and Last Name are split from the Contact Name`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const first = await opportunityPage.getFieldDisplayValue('first_name');
      const middle = await opportunityPage.getFieldDisplayValue('middle_name');
      const last = await opportunityPage.getFieldDisplayValue('last_name');
      console.log(`  Contact Name entered : ${DATA.contactName}`);
      console.log(`  First Name           : ${first}`);
      console.log(`  Middle Name          : ${middle}`);
      console.log(`  Last Name            : ${last}`);
      record('First Name is the first word of the Contact Name', parts[0], first);
      record('Middle Name is the second word of the Contact Name', parts[1], middle);
      record('Last Name is the last word of the Contact Name', parts[parts.length - 1], last);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(first, 'the First Name split from the Contact Name').toBe(parts[0]);
      expect(middle, 'the Middle Name split from the Contact Name').toBe(parts[1]);
      expect(last, 'the Last Name split from the Contact Name').toBe(parts[parts.length - 1]);
    });
  });
});
