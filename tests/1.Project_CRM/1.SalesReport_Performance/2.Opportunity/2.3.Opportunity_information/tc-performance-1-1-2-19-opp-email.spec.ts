...
/**
 * =============================================================================================
 *  Opportunity screen verification - Email
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.19
 ...
 *  test case owns: verify that the Email stored on the saved Opportunity is exactly the value entered on the CREATE form.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.19:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.19 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.19
 ...
 *    4. Read the Email on the saved form
 *
 *  Verification:
 *    4. Email shows exactly the entered value "Test-Opp@company<unique>.com"
 *    - No lower-casing, no truncation, no re-formatting
 *
...
const TC = 'TC.Performance.1.1.2.19';
...
const STEP = {
  ...
  s4: 'Step 4: Read the Email on the saved form',
};

test.describe(`${TC} - Email`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the Email on the saved Opportunity matches the entered value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const actual = await opportunityPage.getFieldDisplayValue('partner_address_email');
      console.log(`  Email entered : ${DATA.email}`);
      console.log(`  Email stored  : ${actual}`);
      record('The Email on the saved form', DATA.email, actual);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual, 'the Email on the saved form').toBe(DATA.email);
    });
  });
});
