...
/**
 * =============================================================================================
 *  Opportunity screen verification - Pricelist
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.22
 ...
 *  test case owns: verify that a newly created Opportunity carries the default Pricelist and the currency that Pricelist is expressed in.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.22:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.22 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.22
 ...
 *    4. Read the Pricelist and the Pricelist Currency on the saved form
 *
 *  Verification:
 *    4. The information area shows:
...
const TC = 'TC.Performance.1.1.2.22';
...
const STEP = {
  ...
  s4: 'Step 4: Read the Pricelist and the Pricelist Currency on the saved form',
};

test.describe(`${TC} - Pricelist`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the Pricelist and the Pricelist Currency of a newly created Opportunity`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const pricelist = await opportunityPage.waitForFieldDisplayValue('pricelist_id', EXPECTED_PRICELIST, 5);
      const currency = await opportunityPage.getFieldDisplayValue('currency_id');
      console.log(`  Pricelist          : ${pricelist}`);
      console.log(`  Pricelist Currency : ${currency}`);
      record('The Pricelist of a new Opportunity', EXPECTED_PRICELIST, pricelist);
      record('The Pricelist Currency of a new Opportunity', EXPECTED_CURRENCY, currency);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(pricelist, 'the Pricelist of a new Opportunity').toBe(EXPECTED_PRICELIST);
      expect(currency, 'the Pricelist Currency of a new Opportunity').toBe(EXPECTED_CURRENCY);
    });
  });
});
