...
/**
 * =============================================================================================
 *  Opportunity screen verification - Address
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.20
 ...
 *  test case owns: verify that the Street, the State and the Country entered on the CREATE form are all stored and printed in the Address block of the saved Opportunity.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.20:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.20 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.20
 ...
 *    4. Read the Address block on the saved form
 *
 *  Verification:
 *    4. The Address block shows the entered values:
 *    - Street = 123street
 *    - State = Connecticut
 *    - Country = United States
 *
...
const TC = 'TC.Performance.1.1.2.20';
...
const STEP = {
  ...
  s4: 'Step 4: Read the Address block on the saved form',
};
...
test.describe(`${TC} - Address`, () => {
  ...
  test(`${TC}: Verify the Address block (Street, State, Country) on the saved Opportunity matches the entered values`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const street = await opportunityPage.getFieldDisplayValue('street');
      const addressRow = await opportunityPage.getFieldRowTextByLabel('Address');
      console.log(`  Street stored : ${street}`);
      console.log(`  Address row   : ${addressRow}`);
      record('The Street on the saved form', DATA.street, street);
      record('The Address block carries the State', DATA.state, addressRow.includes(DATA.state) ? DATA.state : addressRow);
      record('The Address block carries the Country', DATA.country, addressRow.includes(DATA.country) ? DATA.country : addressRow);
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(street, 'the Street on the saved form').toBe(DATA.street);
      expect(addressRow, 'the Address block must carry the State').toContain(DATA.state);
      expect(addressRow, 'the Address block must carry the Country').toContain(DATA.country);
    });
  });
});
