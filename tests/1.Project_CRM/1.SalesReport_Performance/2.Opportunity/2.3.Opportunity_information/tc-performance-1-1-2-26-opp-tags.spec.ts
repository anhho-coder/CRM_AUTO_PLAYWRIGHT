...
/**
 * =============================================================================================
 *  Opportunity screen verification - Tags
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.26
 ...
 *  test case owns: verify that an Opportunity created with Lead Form = License and no tag chosen by the tester is tagged by the system.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.26:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.26 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.26
 ...
 *    4. Read the Tags on the saved form, reloading until the system has tagged the record
 *
 *  Verification:
 *    4. The Opportunity carries exactly 1 tag, applied by the system:
 *    - Tags = Product Registration
 *    - although the tester chose no tag on the CREATE form
 *
...
const TC = 'TC.Performance.1.1.2.26';

/** The tag the system derives from Lead Form = License (pre-production, 2026-09-21). */
const EXPECTED_TAGS = [
...
const STEP = {
  ...
  s4: 'Step 4: Read the Tags on the saved form, reloading until the system has tagged the record',
};

test.describe(`${TC} - Tags`, () => {
  let oppUrl = '';
  ...
  test(`${TC}: Verify the system tags the Opportunity from its Lead Form`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const tags = await opportunityPage.waitForTagList(6);
      console.log(`  Lead Form chosen : ${DATA.leadForm}`);
      console.log(`  Tags on the saved Opportunity: ${tags.join(' | ')}`);
      record('Number of tags', EXPECTED_TAGS.length, tags.length);
      record('The tag the system applied', EXPECTED_TAGS.join(' | '), tags.join(' | '));
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(tags.length, 'the NUMBER of tags the system applied').toBe(EXPECTED_TAGS.length);
      expect(tags, 'the tag the system derived from the Lead Form').toEqual(EXPECTED_TAGS);
    });
  });
});
