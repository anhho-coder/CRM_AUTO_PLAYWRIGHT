 ...
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.36
 ...
 *  test case owns: verify that the log note the system writes when it creates the Customer behind the Opportunity reports the Customer it created, the Company Name, the Partner Contact Email and the Pricelist.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.36:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.36 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.36
 ...
 *    4. Read the customer-creation log note in the chatter
 *
 *  Verification:
 *    4. The chatter carries a log note that starts with "Customer:" and reports:
 *    - Customer = company<unique>.com
 *    - Company Name = company<unique>.com
 *    - Partner Contact Email = Test-Opp@company<unique>.com
...
const TC = 'TC.Performance.1.1.2.36';

const EXPECTED_PRICELIST = 'Public Pricelist_USD (USD)';
...
const STEP = {
  ...
  s4: 'Step 4: Read the customer-creation log note in the chatter',
};
...
test.describe(`${TC} - Log note`, () => {
  ...
  test(`${TC}: Verify the customer-creation log note reports the Customer, the Company Name, the e-mail and the Pricelist`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const note = await opportunityPage.waitForChatterMessage(/^Customer: /);
      console.log('  The customer-creation log note:');
      console.log(`${(note || '(not found)').split('\n').map((l) => '    ' + l).join('\n')}`);
      record('The chatter carries a customer-creation log note', 'FOUND', note ? 'FOUND' : 'NOT FOUND');
      const fields = note ? opportunityPage.parseLogNoteFields(note) : {};
      record('Customer reported by the log note', expectedCompany, fields['Customer'] ?? '(missing)');
      record('Company Name reported by the log note', expectedCompany, fields['Company Name'] ?? '(missing)');
      record('Partner Contact Email reported by the log note', DATA.email, fields['Partner Contact Email'] ?? '(missing)');
      record('Pricelist reported by the log note', EXPECTED_PRICELIST, fields['Pricelist'] ?? '(missing)');
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(note, 'the chatter must carry a customer-creation log note').not.toBeNull();
      expect(fields['Customer'], 'the Customer the log note reports').toBe(expectedCompany);
      expect(fields['Company Name'], 'the Company Name the log note reports').toBe(expectedCompany);
      expect(fields['Partner Contact Email'], 'the Partner Contact Email the log note reports').toBe(DATA.email);
      expect(fields['Pricelist'], 'the Pricelist the log note reports').toBe(EXPECTED_PRICELIST);
    });
  });
});
