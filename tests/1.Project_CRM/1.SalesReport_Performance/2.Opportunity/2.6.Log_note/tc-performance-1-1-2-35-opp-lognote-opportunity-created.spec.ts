 ...
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.35
 ...
 *  test case owns: verify that saving the Opportunity writes an "Opportunity created" log note into the chatter, and that the note reports the Contact Name and the Email that were entered plus the state the record starts in.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.35:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.35 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.35
 ...
 *    4. Read the "Opportunity created" log note in the chatter
 *
 *  Verification:
 *    4. The chatter carries a log note that starts with "Opportunity created" and reports:
 *    - Contact Name = Contact Name TC.Performance.1.1.2.35
 *    - Email = Test-Opp@company<unique>.com
 *    - Active = true
 *    - Priority (System) = Low
...
const TC = 'TC.Performance.1.1.2.35';
...
const STEP = {
  ...
  s4: 'Step 4: Read the "Opportunity created" log note in the chatter',
};
...
test.describe(`${TC} - Log note`, () => {
  ...
  test(`${TC}: Verify the "Opportunity created" log note reports the entered Opportunity data`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      record('Contact Name reported by the log note', DATA.contactName, fields['Contact Name'] ?? '(missing)');
      record('Email reported by the log note', DATA.email, fields['Email'] ?? '(missing)');
      ...
      expect(fields['Contact Name'], 'the Contact Name the log note reports').toBe(DATA.contactName);
      expect(fields['Email'], 'the Email the log note reports').toBe(DATA.email);
