 ...
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.39
 ...
 *  test case owns: verify that the stage a newly created Opportunity sits in is NEW, i.e. the first stage of the pipeline, and that it is the only highlighted stage on the bar.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.39:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.39 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.39
 ...
 *    4. Read the stage the saved Opportunity sits in
 *
 *  Verification:
 *    4. The Opportunity sits in the NEW stage
 *    - NEW is the first stage of the pipeline and the only highlighted one
 *
...
const TC = 'TC.Performance.1.1.2.39';

const EXPECTED_STAGE = 'NEW';
...
const STEP = {
  ...
  s4: 'Step 4: Read the stage the saved Opportunity sits in',
};
...
test.describe(`${TC} - Stages`, () => {
  ...
  test(`${TC}: Verify a newly created Opportunity starts in the NEW stage`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const active = await opportunityPage.getActiveStatusBarStage();
      const stages = await opportunityPage.getStatusBarStages();
      console.log(`  Active stage : ${active}`);
      console.log(`  Stage bar    : ${stages.join(' | ')}`);
      record('The stage a newly created Opportunity sits in', EXPECTED_STAGE, active);
      record('That stage is the FIRST one of the pipeline', EXPECTED_STAGE, stages[0] ?? '(empty bar)');
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(active, 'the stage a newly created Opportunity sits in').toBe(EXPECTED_STAGE);
      expect(stages[0], 'the first stage of the pipeline').toBe(EXPECTED_STAGE);
    });
  });
});
