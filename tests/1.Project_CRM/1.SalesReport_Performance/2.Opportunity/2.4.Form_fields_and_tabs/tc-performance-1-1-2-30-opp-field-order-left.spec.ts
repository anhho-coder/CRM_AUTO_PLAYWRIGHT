 ...
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.30
 ...
 *  test case owns: verify that the left column of the Opportunity information area carries exactly the expected field labels, in the expected top-to-bottom order.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.30:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.30 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.30
 ...
 *    4. Read the field labels of the LEFT column of the information area
 *
 *  Verification:
 *    4. The left column carries exactly 24 fields, in this order:
 *    - Company Name | Contact Name | Company | Contact | Pricelist Currency | First Name | Middle Name | Last Name | Address | Email | Email CC | Phone | Linkedin | Country Lead | Mobile | Salesperson | Create manually | Sales Team | Lead Source | Reseller | Reseller contact | Distributor | Distributor contact | Followup Count
 *
...
const TC = 'TC.Performance.1.1.2.30';

/** The left column of the Opportunity information area, top to bottom (pre-production, 2026-09-21). */
const EXPECTED = [
    'Company Name',
    'Contact Name',
    'Company',
...
const STEP = {
  ...
  s4: 'Step 4: Read the field labels of the LEFT column of the information area',
};
...
test.describe(`${TC} - Fields`, () => {
  ...
  test(`${TC}: Verify the field names and their order in the left column of the Opportunity information area`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const actual = await opportunityPage.getColumnFieldLabels('left');
      actual.forEach((label, i) => console.log(`  Left #${String(i + 1).padStart(2)}: ${label}`));
      record('Number of fields in the left column', EXPECTED.length, actual.length);
      EXPECTED.forEach((expectedLabel, i) => {
        record(`Left column field #${i + 1}`, expectedLabel, actual[i] === undefined ? '(missing)' : actual[i]);
      });
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual.length, 'the NUMBER of fields in the left column').toBe(EXPECTED.length);
      expect(actual, 'the field names and their ORDER in the left column').toEqual(EXPECTED);
    });
  });
});
