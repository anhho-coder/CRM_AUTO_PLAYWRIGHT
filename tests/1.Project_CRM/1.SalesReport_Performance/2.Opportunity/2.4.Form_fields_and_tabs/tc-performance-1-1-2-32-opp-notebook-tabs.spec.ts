 ...
 * =============================================================================================
 *  Test Case ID    : TC.Performance.1.1.2.32
 ...
 *  test case owns: verify that the notebook at the bottom of a newly created Opportunity carries exactly the expected tabs, in the expected left-to-right order.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "TC\\.Performance\\.1\\.1\\.2\\.32:" --project=SalesReport_Performance
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *       - Opportunity     = AUTO TC.Performance.1.1.2.32 <unique>
 *       - Contact Name    = Contact Name TC.Performance.1.1.2.32
 ...
 *    4. Read the notebook tabs at the bottom of the form
 *
 *  Verification:
 *    4. The notebook carries exactly 14 tabs, in this order:
 *    - Internal Notes | Initial Contact Data | Follow-up | Assigned Partner | Qualification info | Approvals | Deal registration | Website Sessions | Environment | Deal Elements | CRM Developer | BAAS Info | Trial Products | BAAS Interactions
 *    - "Customer Invoices" is NOT offered on a newly created Opportunity (it appears once the Opportunity has invoices)
 *
...
const TC = 'TC.Performance.1.1.2.32';

/**
 * The notebook tabs a newly created Opportunity carries (pre-production, 2026-09-21).
 * "Customer Invoices" is absent by design - it only appears once the Opportunity has invoices.
 */
const EXPECTED = [
    'Internal Notes',
...
const STEP = {
  ...
  s4: 'Step 4: Read the notebook tabs at the bottom of the form',
};
...
test.describe(`${TC} - Fields`, () => {
  ...
  test(`${TC}: Verify the name, the number and the order of the Opportunity notebook tabs`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s4, async () => {
      ...
      const actual = await opportunityPage.getNotebookTabs();
      actual.forEach((tab, i) => console.log(`  Tab #${String(i + 1).padStart(2)}: ${tab}`));
      record('Number of notebook tabs', EXPECTED.length, actual.length);
      EXPECTED.forEach((expectedTab, i) => {
        record(`Notebook tab #${i + 1}`, expectedTab, actual[i] === undefined ? '(missing)' : actual[i]);
      });
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - verification done`);

      expect(actual.length, 'the NUMBER of notebook tabs').toBe(EXPECTED.length);
