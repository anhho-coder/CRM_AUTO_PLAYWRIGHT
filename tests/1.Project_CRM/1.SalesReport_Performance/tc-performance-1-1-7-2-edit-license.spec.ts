 ...
 * 19. Press "SAVE" button and wait
 * 20. Press "EDIT" button and wait
 * 21. Change "Support Type" to "24/7"
 * 
 * Steps run:
...
test.describe('TC.Performance.1.1.7.2 - Edit License Performance', () => {
  const PERFORMANCE_THRESHOLD = 60000; // 1 minute in milliseconds
         ...
         test('TC.Performance.1.1.7.2: Verify consumer time for editing License is less than 1 min', async ({ page, browser }, testInfo) => {
           ...
           let saveEditTime: number = 0;
           let saveEditSeconds: string = '0';
           ...
           await test.step('Step 9: Once the "Deal Element" screen shows up - Select Pricelist & Payment Term', async () => {
             stepStartTime = Date.now();
       ...
       // Step 19: Press "SAVE" button
       await test.step('Step 19: Press "SAVE" button and wait', async () => {
         stepStartTime = Date.now();
         console.log('Step 19: Saving License');
         
         // Save license using POM (not measuring performance yet)
         const saveLicenseTime = await licensePage.clickSaveAndWaitForCompletion();
         
         performanceMetrics['Save License'] = saveLicenseTime;
         console.log('✓ License saved successfully');
       });

       // Step 20: Press "EDIT" button
       await test.step('Step 20: Press "EDIT" button and wait', async () => {
         stepStartTime = Date.now();
         console.log('Step 20: Clicking EDIT button');
         
         // Click EDIT using POM
         await licensePage.clickEdit();
         
         performanceMetrics['Enter Edit Mode'] = Date.now() - stepStartTime;
         console.log('✓ License in edit mode');
       });

       // Step 21: Change Support Type to "24/7"
       await test.step('Step 21: Change "Support Type" to "24/7"', async () => {
         stepStartTime = Date.now();
         console.log('Step 21: Changing Support Type to "24/7"');
         
         // Change Support Type using POM
         await licensePage.changeSupportType('24/7');
         
         performanceMetrics['Change Support Type'] = Date.now() - stepStartTime;
         console.log('✓ Support Type changed to "24/7"');
       });
       ...
       await test.step('Steps run - Step 1: Press "SAVE" button (Performance measurement)', async () => {
         console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
         console.log('PERFORMANCE TEST - Saving License Edit (Performance Measurement)');
         
         // Click SAVE button and measure performance using POM
         saveEditTime = await licensePage.clickSaveAndWaitForCompletion();
         saveEditSeconds = (saveEditTime / 1000).toFixed(2);
         
         performanceMetrics['SAVE License Edit Process'] = saveEditTime;
         
         // Calculate total elapsed time
         const totalElapsedTime = Date.now() - startTime;
         const totalElapsedSeconds = (totalElapsedTime / 1000).toFixed(2);
         
         console.log(`\n⏱️  Save Operation Time: ${saveEditSeconds} seconds`);
         ...
         expect(saveEditTime).toBeLessThan(PERFORMANCE_THRESHOLD);
         console.log(`\n✅ PERFORMANCE TEST PASSED: ${saveEditSeconds}s < 60s`);
         
         // Detailed performance breakdown
         console.log('\n📊 Detailed Performance Breakdown:');
         console.log('━'.repeat(50));
         console.log('PRE-CONDITIONS:');
         Object.entries(performanceMetrics).filter(([step]) => !step.includes('SAVE License Edit')).forEach(([step, time]) => {
           const seconds = (time / 1000).toFixed(2);
           console.log(`  ${step.padEnd(35)} ${seconds.padStart(8)}s`);
         });
         console.log('\nTEST STEPS:');
         ['SAVE License Edit Process'].forEach(step => {
         ...
         console.log(`   - Save Operation Time: ${saveEditSeconds} seconds`);
         console.log(`   - Threshold: 60 seconds`);
         console.log(`   - Margin: ${(60 - parseFloat(saveEditSeconds)).toFixed(2)} seconds under threshold`);
         console.log(`   - Performance: ${((parseFloat(saveEditSeconds) / 60) * 100).toFixed(2)}% of threshold`);
         console.log(`   - Total Test Time: ${totalElapsedSeconds} seconds (includes all pre-conditions and steps)`);
       });
       
       // Verification: Verify License edit saved (not included in performance measurement)
       await test.step('Verification: Verify License edit saved (not included in performance measurement)', async () => {
         console.log('\nVerification: Verifying License edit saved (not included in performance measurement)');
         
         // Verify license edit was saved
         const pageContent = await page.textContent('body').catch(() => '') || '';
         const isSaved = pageContent.includes('License') && pageContent.includes('24/7');
         
         if (isSaved) {
           console.log('  ✓ License edit saved successfully');
         } else {
           console.log('  ⚠ License save verification: Status unclear');
         }
    ...
    await test.step('Capture final screenshot and performance summary', async () => {
      await CommonUtils.captureAndAttachScreenshot(
        page,
        testInfo,
        `✅ Performance Test Passed - License Edit Time: ${saveEditSeconds}s`
      );
    });
  });
});
