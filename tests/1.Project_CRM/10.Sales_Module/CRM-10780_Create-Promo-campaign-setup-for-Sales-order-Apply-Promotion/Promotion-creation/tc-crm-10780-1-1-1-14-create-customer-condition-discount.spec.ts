...
/**
 * Create a Promotion Program with a specific-customer condition (10% on-order discount, auto-applied).
 * Test Case ID: CRM-10780_1.1.1.14   (Jira: CRM-10857 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Sales Manager creates a Promotion Program (Sales > Products > Promotion Programs) with
 *          Automatically Applied promo, a Discount reward of 10% applied On Order, restricted to a
 *          specific Customer (customer A).
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_1\.1\.1\.14:" --project=chromium
 *
 * Source manual TC (Jira CRM-10857)
 ...
 *     Record created successfully, promotion only applies to customer A
 *
 * Design notes:
...
test.describe('CRM-10780_1.1.1.14 - Create promotion with specific customer condition', () => {
  let createdUrl = '';
  let promoName = '';
  ...
  test('CRM-10780_1.1.1.14: Create promotion with specific customer condition', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Create new Promotion with 10% on-order discount restricted to a specific customer', async () => {
      await promotionPage.clickCreate();
      promoName = promotionPage.generatePromotionName('TEST- Customer A 10pct ');
      ...
      // TODO: select specific customer (customer A) - no PromotionPage method yet
      createdUrl = await promotionPage.save();
      console.log('Saved Promotion "' + promoName + '" at ' + createdUrl);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10857 - Promotion created');
