...
/**
 * Create auto-applied promotion with 10% discount applied to multiple specific products.
 * Test Case ID: CRM-10780_1.1.1.5   (Jira: CRM-10848 - Post-EA Test Case)
 * Automation-Type: new
 ...
 *          = On Specific Product (product 1, product 2).
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_1\.1\.1\.5:" --project=chromium
 *
 * Source manual TC (Jira CRM-10848)
 ...
 *     Record created successfully, Promotion only applies to product 1, product 2
 *
 * Design notes:
...
test.describe('CRM-10780_1.1.1.5 - Create promotion applied to multiple specific products', () => {
  let createdUrl = '';
  let promoName = '';
  ...
  test('CRM-10780_1.1.1.5: Create promotion applied to multiple specific products', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Create new Promotion with Promo Code Usage = Automatically Applied, Reward = Discount, Apply Discount = Percentage - 10%, Discount Apply On = On Specific Product (product 1, product 2)', async () => {
      await promotionPage.clickCreate();
      promoName = promotionPage.generatePromotionName('TEST- 10% Specific Products ');
      ...
      await promotionPage.selectDiscountApplyOn('On Specific Products');
      await promotionPage.setSpecificProducts(['[A2144B]', '[A2145B]']);
      createdUrl = await promotionPage.save();
      console.log('Saved Promotion "' + promoName + '" at ' + createdUrl);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10848 - Promotion created');
