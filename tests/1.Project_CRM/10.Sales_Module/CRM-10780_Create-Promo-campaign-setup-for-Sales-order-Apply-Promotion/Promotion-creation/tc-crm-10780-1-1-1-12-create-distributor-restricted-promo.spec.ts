...
/**
 * Create a Promotion Program restricted to Distributors (Automatically Applied, 10% discount on order).
 * Test Case ID: CRM-10780_1.1.1.12   (Jira: CRM-10855 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Sales Manager creates a Promotion Program (Sales > Products > Promotion Programs) with
 *          Automatically Applied promo code usage, a 10% Discount reward applied On Order, restricted
 *          to Distributors (For Distributor = true).
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_1\.1\.1\.12:" --project=chromium
 *
 * Source manual TC (Jira CRM-10855)
 ...
 *     Record created successfully, Promotion only applies for Distributor
 *
 * Design notes:
...
test.describe('CRM-10780_1.1.1.12 - Create promotion restricted to Distributors', () => {
  let createdUrl = '';
  let promoName = '';
  ...
  test('CRM-10780_1.1.1.12: Create promotion restricted to Distributors', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Create new Promotion with Automatically Applied, 10% discount on order, For Distributor = true', async () => {
      await promotionPage.clickCreate();
      promoName = promotionPage.generatePromotionName('TEST- Distributor 10% ');
      ...
      // TODO: set For Distributor = true - no PromotionPage method yet
      createdUrl = await promotionPage.save();
      console.log('Saved Promotion "' + promoName + '" at ' + createdUrl);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10855 - Promotion created');
