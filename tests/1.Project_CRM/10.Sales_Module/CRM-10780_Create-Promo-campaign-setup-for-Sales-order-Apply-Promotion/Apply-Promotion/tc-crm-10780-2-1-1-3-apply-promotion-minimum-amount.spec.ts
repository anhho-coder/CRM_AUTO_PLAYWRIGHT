...
/**
 * Apply a promotion when the order total meets the "Minimum Purchase Of" rule.
 * Test Case ID: CRM-10780_2.1.1.3   (Jira: CRM-10863 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Salesperson opens an Opportunity, goes to its Deal Element (sale.order) and adds a product
 *          with quantity so that the order total reaches/exceeds the promotion's Minimum Purchase Of
 *          (1000$). Applying the Automatically-Applied "Promotion A" (10% on order, min purchase 1000$)
 *          then succeeds - the order qualifies and the total is reduced by the promotion discount.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_2\.1\.1\.3:" --project=chromium
 *
 * Source manual TC (Jira CRM-10863)
 ...
 *   (the packaged create flow validated by CRM-10844). Precondition "Minimum Purchase Of = 1000$" maps to
 *   { minPurchaseAmount: 1000 }; "Apply Discount = Percentage - 10%" maps to { discountPercentage: 10 };
 *   "Discount Apply On = On Order" is the default.
 * - Step 3 ("Select product to have total =1000$"): the default product [A2144B] = $329 per unit. To make
 *   the order total meet/exceed the 1000$ minimum, add it with quantity 4 (4 x $329 = $1316 >= 1000), so the
 *   order QUALIFIES for the promotion. This is a POSITIVE case: the eligible order accepts the promo.
 * - "Apply promotion A" (step 4) on this Nakivo Deal Element = set the editable "Promotion" field
 *   (promotion_id Many2one) to Promotion A, then SAVE. Because the order total meets the minimum-purchase
 *   rule, the promo is added as a discount line in Order Lines ("X% discount on total amount") and the order
 *   total is reduced. The Promotion field must be set while the form is in edit mode (after adding the line).
...
test.describe('CRM-10780_2.1.1.3 - Apply promotion when order total meets minimum-amount rule', () => {
  let promoName = '';
  let promoUrl = '';
  let urlContact = '';
  let urlOpp = '';
  ...
  test('CRM-10780_2.1.1.3: Apply promotion when order total meets minimum-amount rule', async ({ page }, testInfo) => {
    ...
    //   Apply Discount = Percentage - 10%, Discount Apply On = On Order, Minimum Purchase Of = 1000$
    // ============================================================
    ...
    await test.step('Pre-condition B.1: Create EndUser Contact (with Pricelist)', async () => {
      ...
      contactName = `TEST-EndUser_CRM-10863_${timestamp}`;
      contactEmail = `test-enduser-crm10863-${timestamp}@enduser-company.com`;
    ...
    await test.step('Pre-condition B.2: Create a qualifying Opportunity (using the EndUser email)', async () => {
      ...
      const oppName = `TEST Opp CRM-10863 ${timestamp}`;
      await opportunityPage.fillOpportunityName(oppName);
      await opportunityPage.fillEmail(contactEmail);
      await opportunityPage.fillCompanyName('Company CRM-10863');
      ...
      // These three are CRM-2338-specific data-hygiene extras (not required by CRM-10863) and some are
    ...
    // ============================================================
    // Steps (mirrors Jira CRM-10863 manual steps 1-4)
    // ============================================================
    ...
    await test.step('Step 3: Select product to have total =1000$', async () => {
      // [A2144B] "Socket" = $329/unit. Add quantity 4 (4 x $329 = $1316) so the order total meets/exceeds
      // the promotion's Minimum Purchase Of = 1000$, making the order QUALIFY for the promotion.
      ...
      const totalForRule = await dealElementPage.getAmountTotal();
      console.log(`✓ Step 3: product added (order lines = ${lineCount}, order total = ${totalForRule})`);
      let __verifyPassed = false;
      try {
        expect(lineCount, 'Order should contain the added product line').toBeGreaterThan(0);
        expect(totalForRule, 'Order total should meet/exceed the 1000$ Minimum Purchase Of').toBeGreaterThanOrEqual(1000);
        __verifyPassed = true;
      } finally {
    ...
    await test.step('Step 4: Apply promotion A', async () => {
      ...
      try {
        ...
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'Step 4 - Promotion field settable - verify', passed: __verifyPassed }).catch(() => {});
      }
      ...
      //  - The order qualifies (total >= 1000$ minimum), so Promotion A appears as a discount line.
      //  - The order Total is reduced.
      let __verifyPassed2 = false;
      try {
        expect(promoLinePresent || linesAfter > linesBefore,
          'Promotion A should be added as a discount line in Order Lines (order qualifies, total >= 1000$)').toBeTruthy();
        expect(totalAfter, 'Order Total should be reduced after applying Promotion A').toBeLessThan(totalBefore);
        __verifyPassed2 = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'Step 4 - Promotion applied - verify', passed: __verifyPassed2 }).catch(() => {});
      }
      console.log(`✅ Promotion A applied successfully: Total ${totalBefore} -> ${totalAfter} (line added=${promoLinePresent})`);
    });
  });
});
