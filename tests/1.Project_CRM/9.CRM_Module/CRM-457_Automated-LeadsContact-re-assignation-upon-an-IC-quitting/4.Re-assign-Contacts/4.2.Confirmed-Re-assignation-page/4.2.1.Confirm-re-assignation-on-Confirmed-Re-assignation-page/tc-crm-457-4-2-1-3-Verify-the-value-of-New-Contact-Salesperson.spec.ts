 ...
 *         (Remember this value as selected_Re-assignment_to#1)
...
test.describe('CRM-457_4.2.2.3 - Verify Lead.Salesperson is changed to the assigned to salesperson after confirming the re-assignation', () => {
    ...
    if (url_Contact1) {
      await test.step('IV. TEAR DOWN: Clean up test data', async () => {
        ...
        await test.step('IV.1: Delete Contact#1', async () => {
          ...
          await CommonUtils.captureAndAttachScreenshot(teardownTab, testInfo, 'Tear down - Contact#1 page opened');
          await teardownContactPage.clickActionMenu();
          console.log('  ✓ Action menu opened');
          await CommonUtils.captureAndAttachScreenshot(teardownTab, testInfo, 'Tear down - Action menu opened');
          await teardownContactPage.clickActionDeleteOption();
          console.log('  ✓ Delete option selected');
          await CommonUtils.captureAndAttachScreenshot(teardownTab, testInfo, 'Tear down - Delete confirmation dialog');
          await teardownContactPage.confirmDeleteDialog();
          console.log('  ✓ Contact#1 deleted successfully');
          await CommonUtils.captureAndAttachScreenshot(teardownTab, testInfo, 'Tear down - Contact#1 deleted');
  ...
  test('CRM-457_4.2.2.3: Verify Lead.Salesperson is changed to the assigned to salesperson after confirming the re-assignation', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.3: Re-assignment to = "Alan Osseiran" (selected_Re-assignment_to#1)');
    ...
    await test.step('III. Verification - Open Contact#1 and verify Salesperson = selected_Re-assignment_to#1', async () => {
      console.log('\n=== III. VERIFICATION POINTS ===');

      await test.step('III.1: Open new tab and navigate to URL_Contact#1', async () => {
        console.log('III.1: Opening Contact#1 in new tab');
        const contactTab = await page.context().newPage();
        contactTab.on('dialog', async (dialog) => { await dialog.accept(); });
        ...
        console.log('✓ III.1: Contact#1 page opened');
        await CommonUtils.captureAndAttachScreenshot(contactTab, testInfo, 'III.1 - Contact#1 opened');

        await test.step('III.2: Verify Salesperson = selected_Re-assignment_to#1', async () => {
          const actualSalesperson = await contactPageVerify.getSalespersonValue();
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_4.2.2.3 verification completed successfully');
