 ...
 * Test Case ID: CRM-457_4.2.1.1
 *
 * Summary: Verify value at "Total" text = 1/1 after confirming the re-assignation
 *
 * Command to run:
 * npx playwright test --grep "CRM-457_4\.2\.1\.1:" --project=chromium
 *
...
test.describe('CRM-457_4.2.1.1 - Verify value at "Total" text = 1/1 after confirming the re-assignation', () => {
  ...
  test('CRM-457_4.2.1.1: Verify value at "Total" text = 1/1 after confirming the re-assignation', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "1/1"', async () => {
      ...
      console.log('✓ III.1: Total text = "1/1" - qualified Contact correctly counted for re-assignment');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'III.1 - Total 1-1 verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_4.2.1.1 verification completed successfully');
      console.log(`   Contact#1 URL  : ${url_Contact1}`);
      console.log(`   Contact#1 Name : ${contact1Name}`);
      console.log(`   Email_Contact#1: ${contact1Email}`);
      console.log('   III.1: Total = "1/1" - qualified Contact correctly counted for re-assignment');
