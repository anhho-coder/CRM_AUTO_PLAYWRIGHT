 ...
 * Test Case ID: CRM-457_4.1.2.2
 *
 * Summary: Verify the qualified Contacts with selecting 2 states have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep "CRM-457_4\.1\.2\.2:" --project=chromium
 *
 ...
 *    5.6. Set : "Country state"        = "Lower Austria"
 *
...
test.describe('CRM-457_4.1.2.2 - Verify qualified Contacts with 2 states transfered to another Salesperson', () => {
  ...
  test('CRM-457_4.1.2.2: Verify the qualified Contacts with 2 states have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition Step 5: Copy URL of Contact#1', async () => {
      ...
      // The app uses hash-based SPA routing: the "new record" URL is
      // web?#id=&action=... (empty id) and the saved-record URL is
      // web#id=615105&action=... (numeric id).
      // waitForURL() misses hash-only changes in SPAs, so we poll
      // window.location.href directly inside the browser instead.
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.6: Country state = "Lower Austria"');
      await reAssignationPage.selectCountryState('Lower Austria');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_4.1.2.2 verification completed successfully');
