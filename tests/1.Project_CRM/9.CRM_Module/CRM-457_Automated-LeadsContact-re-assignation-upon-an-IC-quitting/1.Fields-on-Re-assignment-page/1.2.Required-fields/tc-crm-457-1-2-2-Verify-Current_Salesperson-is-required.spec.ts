 ...
 * Test Case ID: CRM-457_1.2.2
 *
 * Summary: Verify "Current Salesperson" combobox is required field on "Re-assignment" page
 *
 * Command to run:
 * npx playwright test --grep "CRM-457_1\.2\.2:" --project=chromium
 *
 * I. Pre-condition:
 ...
 *    5.2. Set : "Current Salesperson" = BLANK
...
test.describe('CRM-457_1.2.2 - Verify Current Salesperson is required on Re-assignment page', () => {
  ...
  test('CRM-457_1.2.2: Verify "Current Salesperson" combobox is required field on "Re-assignment" page', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 5: Set field values on Re-assignation page', async () => {
      ...
      console.log('  Step 5.2: Set "Current Salesperson" = BLANK');
      await reAssignationPage.clearCurrentSalesperson();
    ...
    await test.step('II. Verification - Step 1: Verify error message displays for "Current Salesperson"', async () => {
      console.log('\n=== II. VERIFICATION POINTS ===');
      ...
      try {
        ...
        expect(errorText).toContain('Current Salesperson');

        console.log('  ✓ II.1: Error message displays "The following fields are invalid: Current Salesperson"');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'II.1 - Validation error for Current Salesperson', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_1.2.2 verification completed successfully');
      console.log('   II.1: Error message "Current Salesperson" is required field is displayed');
      console.log('==================================================\n');
    });
  });
});