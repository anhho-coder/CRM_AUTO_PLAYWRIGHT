 ...
 *
 * Command to run:
 * npx playwright test --grep CRM-457_3\.1\.2\.1: --project=chromium
 *
 ...
 * 4. Find the "Country" = "Poland" by doing the steps:
 *    . Press "Filter" dropdown list
 *    . Then press "Add Customer Filter"
 ...
 *    . Press "APPLY" button
 *    . Press "Filter" dropdown list again to exit "Filter" mode
 * 5. Find the "Country" = "Germany" by doing the steps:
 *    . Press "Filter" dropdown list
 *    . Then press "Add Customer Filter"
 *    . On Dropdown_List#1, select "Country"
 *    . On Dropdown_List#2, select "is equal to"
 ...
 *    - Opp name textbox        = TEST Opp 1 CRM-457_3.1.2.1
 ...
 *    1.2. On "Opps" page, click at "CREATE" button
 * 2. Enter the following information:
 *    - Opp name textbox        = TEST Opp 2 CRM-457_3.1.2.1
...
test.describe('CRM-457_3.1.2.1 - Verify qualified Opps from 2 countries transferred to another Salesperson', () => {
  ...
  test.beforeEach(async ({ browser, context }) => {
    ...
    // ============================================================
    // I. Clean Opps (Poland + Germany / Thomas Semerich)
    // ============================================================
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    let hasTeardown = oppEntries.some(([, , url]) => !!url);
    if (hasTeardown) {
      console.log('\n=== V. TEAR DOWN ===');
    }
  ...
  test('CRM-457_3.1.2.1: Verify the qualified Opps from 2 countries have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    const tcId     = 'CRM-457_3.1.2.1';
    const oppName1 = `TEST Opp 1 ${tcId}`;
    const oppName2 = `TEST Opp 2 ${tcId}`;
    const email1   = CommonUtils.generateEmail('Test-Opp1', 'company');
    const email2   = CommonUtils.generateEmail('Test-Opp2', 'company');
    ...
    await test.step('Pre-condition I Step 2: Fill Opp#1 information', async () => {
      ...
      await opportunityPage.selectCountry('Poland');

      console.log('  2.6: State = "Lublin"');
      await opportunityPage.selectState('Lublin');
    ...
    // ==============================================================
    // II. PRE-CONDITION: Create Opp#2 (Germany / Hilden)
    // ==============================================================

    await test.step('Pre-condition II Step 1: Navigate to CRM', async () => {
      console.log(`\n=== II. PRE-CONDITION (Opp#2 - Germany/Hilden) ===`);
    ...
    await test.step('Pre-condition II Step 2: Fill Opp#2 information', async () => {
      ...
      await opportunityPage.selectCountry('Germany');

      console.log('  2.6: State = "Hilden"');
      await opportunityPage.selectState('Hilden');
    ...
    await test.step('IV. Verification - Step 1: Verify Total text = "0/2"', async () => {
      ...
      console.log('✓ IV.1: Total text = "0/2" - qualified Opps from 2 countries correctly counted for re-assignment');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.1 - Total 0-2 verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-457_3.1.2.1 verification completed successfully');
      console.log(`   Opp#1 URL : ${url_Opp1}`);
      console.log(`   Opp#2 URL : ${url_Opp2}`);
      console.log('   IV.1: Total = "0/2" - qualified Opps from 2 countries correctly counted for re-assignment');
