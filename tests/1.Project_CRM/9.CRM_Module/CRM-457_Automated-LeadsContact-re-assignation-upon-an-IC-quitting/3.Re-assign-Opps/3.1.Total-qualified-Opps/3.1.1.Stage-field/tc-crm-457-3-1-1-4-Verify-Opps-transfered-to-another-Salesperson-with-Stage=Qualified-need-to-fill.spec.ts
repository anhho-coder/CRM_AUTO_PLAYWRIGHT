 ...
 * Test Case ID: CRM-457_3.1.1.4
 *
 * Summary: Verify the qualified Opps with Stage = Qualified have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_3\.1\.1\.4: --project=chromium
 *
 ...
 *    - Opp name textbox        = TEST Opp 1 CRM-457_3.1.1.4
 ...
 * 6. On "Stage", select the Stage = Qualified
...
test.describe('CRM-457_3.1.1.4 - Verify qualified Opps with Stage=Qualified transferred to another Salesperson', () => {
  ...
  test('CRM-457_3.1.1.4: Verify the qualified Opps with Stage=Qualified have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition Step 6: Select Stage = QUALIFIED', async () => {
      console.log('Step 6: Selecting Stage = QUALIFIED');
      await opportunityPage.selectStage('Qualified');
      console.log('\u2713 Stage set to QUALIFIED');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Pre-condition 6 - Stage QUALIFIED selected');
    });
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.6: Stage = "Qualified"');
      await reAssignationPage.selectStage('Qualified');
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      try {
        ...
        console.log('\u2713 III.1: Total text = "0/1" - qualified Opp with Stage=Qualified correctly counted for re-assignment');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Total 0-1 verified - Stage=Qualified', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-457_3.1.1.4 verification completed successfully');
      console.log(`   Opp#1 URL    : ${url_Opp1}`);
      console.log('   III.1: Total = "0/1" - qualified Opp with Stage=Qualified correctly counted for re-assignment');
