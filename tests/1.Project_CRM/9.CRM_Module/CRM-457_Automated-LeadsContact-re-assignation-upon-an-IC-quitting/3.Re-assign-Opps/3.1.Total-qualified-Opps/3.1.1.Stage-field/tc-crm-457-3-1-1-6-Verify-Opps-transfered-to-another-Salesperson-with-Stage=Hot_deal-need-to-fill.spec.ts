 ...
 * Test Case ID: CRM-457_3.1.1.6
 *
 * Summary: Verify the qualified Opps with Stage = Hot Deal have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_3\.1\.1\.6: --project=chromium
 *
 ...
 *    - Opp name textbox        = TEST Opp 1 CRM-457_3.1.1.6
 ...
 * 6. On "Stage", select the Stage = Hot Deal
...
test.describe('CRM-457_3.1.1.6 - Verify qualified Opps with Stage=Hot Deal transferred to another Salesperson', () => {
  ...
  test('CRM-457_3.1.1.6: Verify the qualified Opps with Stage=Hot Deal have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition Step 6: Select Stage = HOT DEAL', async () => {
      console.log('Step 6: Selecting Stage = HOT DEAL');
      await opportunityPage.selectStage('Hot Deal');
      console.log('\u2713 Stage set to HOT DEAL');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Pre-condition 6 - Stage HOT DEAL selected');
    });
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.6: Stage = "Hot Deal"');
      await reAssignationPage.selectStage('Hot Deal');
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      try {
        ...
        console.log('\u2713 III.1: Total text = "0/1" - qualified Opp with Stage=Hot Deal correctly counted for re-assignment');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Total 0-1 verified - Stage=Hot Deal', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-457_3.1.1.6 verification completed successfully');
      console.log(`   Opp#1 URL    : ${url_Opp1}`);
      console.log('   III.1: Total = "0/1" - qualified Opp with Stage=Hot Deal correctly counted for re-assignment');
