 ...
 * Test Case ID: CRM-457_3.1.1.7
 *
 * Summary: Verify the qualified Opps with Stage = Purchase Approval have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_3\.1\.1\.7: --project=chromium
 *
 ...
 *    - Opp name textbox        = TEST Opp 1 CRM-457_3.1.1.7
 ...
 * 6. On "Stage", select the Stage = Purchase Approval
 ...
 *    5.6. Set : "Stage"               = "Purchase Approval"
 *
...
test.describe('CRM-457_3.1.1.7 - Verify qualified Opps with Stage=Purchase Approval transferred to another Salesperson', () => {
  ...
  test.beforeEach(async ({ browser, context }) => {
    ...
    // ============================================================
    // I. Clean Opps (Belgium / Flanders / Sergey Karachin / Active)
    // ============================================================
  ...
  test('CRM-457_3.1.1.7: Verify the qualified Opps with Stage=Purchase Approval have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition Step 6: Select Stage = PURCHASE APPROVAL', async () => {
      console.log('Step 6: Selecting Stage = PURCHASE APPROVAL');
      await opportunityPage.selectStage('Purchase Approval');
      console.log('\u2713 Stage set to PURCHASE APPROVAL');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Pre-condition 6 - Stage PURCHASE APPROVAL selected');
    });
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.6: Stage = "Purchase Approval"');
      await reAssignationPage.selectStage('Purchase Approval');
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      try {
        ...
        console.log('\u2713 III.1: Total text = "0/1" - qualified Opp with Stage=Purchase Approval correctly counted for re-assignment');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Total 0-1 verified - Stage=Purchase Approval', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-457_3.1.1.7 verification completed successfully');
      console.log(`   Opp#1 URL    : ${url_Opp1}`);
      console.log('   III.1: Total = "0/1" - qualified Opp with Stage=Purchase Approval correctly counted for re-assignment');
