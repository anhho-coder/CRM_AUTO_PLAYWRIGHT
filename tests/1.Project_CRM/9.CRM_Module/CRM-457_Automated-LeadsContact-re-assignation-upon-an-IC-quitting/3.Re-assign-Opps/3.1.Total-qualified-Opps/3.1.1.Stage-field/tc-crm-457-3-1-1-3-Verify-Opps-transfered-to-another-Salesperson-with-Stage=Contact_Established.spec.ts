 ...
 * Test Case ID: CRM-457_3.1.1.3
 *
 * Summary: Verify the qualified Opps with Stage = Contact Established have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_3\.1\.1\.3: --project=chromium
 *
 ...
 *
 * II. Pre-condition#1 to create Opp#1:
 * 1. On "CRM" page:
 *    1.1. Click at "view list" button
 *    1.2. On "Opps" page, click at "CREATE" button
 * 2. Enter the following information:
 *    - Opp name textbox        = TEST Opp 1 CRM-457_3.1.1.3
 ...
 * 4. On "Stage", select the Stage = Contact Established
 ...
 *    5.6. Set : "Stage"               = "Contact Established"
 *
...
test.describe('CRM-457_3.1.1.3 - Verify qualified Opps with Stage=Contact Established transferred to another Salesperson', () => {
  ...
  test('CRM-457_3.1.1.3: Verify the qualified Opps with Stage=Contact Established have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition Step 4: Select Stage = CONTACT ESTABLISHED', async () => {
      console.log('Step 4: Selecting Stage = CONTACT ESTABLISHED');
      await opportunityPage.selectStage('Contact Established');
      console.log('\u2713 Stage set to CONTACT ESTABLISHED');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Pre-condition 4 - Stage CONTACT ESTABLISHED selected');
    });
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.6: Stage = "Contact Established"');
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      try {
        ...
        console.log('\u2713 III.1: Total text = "0/1" - qualified Opp with Stage=Contact Established correctly counted for re-assignment');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Total 0-1 verified - Stage=Contact Established', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-457_3.1.1.3 verification completed successfully');
      console.log(`   Opp#1 URL    : ${url_Opp1}`);
      console.log('   III.1: Total = "0/1" - qualified Opp with Stage=Contact Established correctly counted for re-assignment');
