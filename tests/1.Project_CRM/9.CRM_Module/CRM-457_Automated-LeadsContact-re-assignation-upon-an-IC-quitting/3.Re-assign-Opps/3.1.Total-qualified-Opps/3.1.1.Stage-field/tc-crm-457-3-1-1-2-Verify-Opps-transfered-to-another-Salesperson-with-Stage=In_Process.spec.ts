 ...
 * Test Case ID: CRM-457_3.1.1.2
 *
 * Summary: Verify the qualified Opps with Stage = In Process have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_3\.1\.1\.2: --project=chromium
 *
 ...
 *    - Opp name textbox        = TEST Opp 1 CRM-457_3.1.1.2
 ...
 * 4. On "Stage", select the Stage = In Process
...
test.describe('CRM-457_3.1.1.2 - Verify qualified Opps with Stage=In Process transferred to another Salesperson', () => {
  ...
  test('CRM-457_3.1.1.2: Verify the qualified Opps with Stage=In Process have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition Step 4: Select Stage = IN PROCESS', async () => {
      console.log('Step 4: Selecting Stage = IN PROCESS');
      await opportunityPage.selectStage('In Process');
      console.log('\u2713 Stage set to IN PROCESS');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Pre-condition 4 - Stage IN PROCESS selected');
    });
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.6: Stage = "In Process"');
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      try {
        ...
        console.log('\u2713 III.1: Total text = "0/1" - qualified Opp with Stage=In Process correctly counted for re-assignment');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Total 0-1 verified - Stage=In Process', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-457_3.1.1.2 verification completed successfully');
      console.log(`   Opp#1 URL    : ${url_Opp1}`);
      console.log('   III.1: Total = "0/1" - qualified Opp with Stage=In Process correctly counted for re-assignment');
