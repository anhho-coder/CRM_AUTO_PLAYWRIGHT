 ...
 * Test Case ID: CRM-457_3.1.1.8
 *
 * Summary: Verify the qualified Opps with Stage = Won have NOT been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_3\.1\.1\.8: --project=chromium
 *
 ...
 *    - Opp name textbox        = TEST Opp 1 CRM-457_3.1.1.8
 ...
 * 6. On "Stage", doing the following steps:
 *    6.1. Press "MORE" button
 *    6.2. Select Stage = Won
...
test.describe('CRM-457_3.1.1.8 - Verify qualified Opps with Stage=Won have NOT been transferred to another Salesperson', () => {
  ...
  test('CRM-457_3.1.1.8: Verify the qualified Opps with Stage=Won have NOT been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition Step 6: Select Stage = WON via MORE button', async () => {
      console.log('Step 6: Selecting Stage = WON via MORE button');
      console.log('  6.1: Clicking MORE button');
      console.log('  6.2: Selecting Stage = Won');
      await opportunityPage.selectStageViaMore('Won');
      console.log('\u2713 Stage set to WON');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Pre-condition 6 - Stage WON selected');
    });
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.6: Stage = "Won"');
      await reAssignationPage.selectStage('Won');
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 0"', async () => {
      console.log('\n=== III. VERIFICATION POINTS ===');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-457_3.1.1.8 verification completed successfully');
      console.log(`   Opp#1 URL    : ${url_Opp1}`);
      console.log('   III.1: Total = "0/0" - qualified Opp with Stage=Won correctly NOT counted for re-assignment');
