 ...
 * Test Case ID: CRM-457_3.1.1.1
 *
 * Summary: Verify the qualified Opps with Stage = New have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_3\.1\.1\.1: --project=chromium
 *
 ...
 *    - Opp name textbox        = TEST Opp 1 CRM-457_3.1.1.1
...
test.describe('CRM-457_3.1.1.1 - Verify qualified Opps with Stage=New transferred to another Salesperson', () => {
  ...
  test('CRM-457_3.1.1.1: Verify the qualified Opps with Stage=New have been transferred to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      console.log('✓ III.1: Total text = "0/1" - qualified Opp with Stage=New correctly counted for re-assignment');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'III.1 - Total 0-1 verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-457_3.1.1.1 verification completed successfully');
      console.log(`   Opp#1 URL    : ${url_Opp1}`);
      console.log('   III.1: Total = "0/1" - qualified Opp with Stage=New correctly counted for re-assignment');
