 ...
 * Test Case ID: CRM-457_3.2.1.3
 *
 * Summary: Verify Opp.Salesperson is changed to the assigned to salesperson after confirming the re-assignation
 *
 * Command to run:
 * npx playwright test --grep CRM-457_3\.2\.1\.3: --project=chromium
 *
 ...
 *    - Opp name textbox        = TEST Opp 1 CRM-457_3.2.1.3
 ...
 *         (Remember selected_Re-assignment_to#1 = "Alan Osseiran")
 ...
 * 1. Open new tab then launch URL_Opp#1
...
test.describe('CRM-457_3.2.1.3 - Verify Opp.Salesperson is changed to the assigned to salesperson after confirming the re-assignation', () => {
  ...
  test('CRM-457_3.2.1.3: Verify Opp.Salesperson is changed to the assigned to salesperson after confirming the re-assignation', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log(`  \u2713 selectedReAssignmentTo1 = "${selectedReAssignmentTo1}"`);
    ...
    await test.step('III. Verification - Step 1: Open new tab and launch URL_Opp#1', async () => {
      console.log('\n=== III. VERIFICATION POINTS ===');
      console.log(`Step III.1: Opening new tab and launching URL_Opp#1: ${url_Opp1}`);
      const verifyTab = await page.context().newPage();
      const oppPageVerify = new LeadPage(verifyTab);
      ...
      console.log('\u2713 III.1: Opp#1 page opened in new tab');
      await CommonUtils.captureAndAttachScreenshot(verifyTab, testInfo, 'III.1 - Opp#1 page opened for verification');

      await test.step('III. Verification - Step 2: Verify Salesperson = selectedReAssignmentTo1', async () => {
      ...
      console.log('\u2713 III: Verification tab closed');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-457_3.2.1.3 verification completed successfully');
      console.log(`   Opp#1 URL             : ${url_Opp1}`);
      console.log(`   selectedReAssignmentTo1: "${selectedReAssignmentTo1}"`);
      console.log('   III.2: Opp.Salesperson changed to assigned salesperson after CONFIRM');
