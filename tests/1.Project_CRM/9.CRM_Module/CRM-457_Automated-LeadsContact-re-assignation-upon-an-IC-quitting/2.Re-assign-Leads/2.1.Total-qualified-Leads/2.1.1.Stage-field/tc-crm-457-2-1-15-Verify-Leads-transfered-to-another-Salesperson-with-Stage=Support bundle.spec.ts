 ...
 * Test Case ID: CRM-457_2.1.15
 *
 * Summary: Verify the qualified Leads with Stage = Support bundle received have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.1\.15: --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.1.15
 ...
 *    5.6. Set : "Stage"               = "New"
 *    5.7. Set : "Stage"               = "Support bundle received"
 *
...
test.describe('CRM-457_2.1.15 - Verify qualified Leads with Stage=Support bundle received transfered to another Salesperson', () => {
  ...
  test('CRM-457_2.1.15: Verify the qualified Leads with Stage=Support bundle received have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.7: Stage = "Support bundle received"');
      await reAssignationPage.selectStage('Support bundle received');

      console.log('✓ All filter fields set');
      await page.waitForTimeout(CommonUtils.waitTimes.standard);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Step II.5.7 - Stage set to Support bundle received');
    });
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      try {
        ...
        console.log('✓ III.1: Total text = "0/1" - qualified Lead with Stage=Support bundle received correctly counted for re-assignment');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Total 0-1 verified - Stage=Support bundle received', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.1.15 verification completed successfully');
      console.log(`   Lead#1 URL   : ${url_Lead1}`);
      console.log('   III.1: Total = "0/1" - qualified Lead with Stage=Support bundle received correctly counted for re-assignment');
