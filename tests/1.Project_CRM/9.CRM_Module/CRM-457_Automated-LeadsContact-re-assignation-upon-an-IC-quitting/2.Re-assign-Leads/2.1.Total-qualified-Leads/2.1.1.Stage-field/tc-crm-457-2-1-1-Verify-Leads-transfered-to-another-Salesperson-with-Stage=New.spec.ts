 ...
 * Test Case ID: CRM-457_2.1.1
 *
 * Summary: Verify the qualified Leads with Stage = New have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.1\.1: --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.1.1
...
test.describe('CRM-457_2.1.1 - Verify qualified Leads with Stage=New transfered to another Salesperson', () => {
  ...
  test('CRM-457_2.1.1: Verify the qualified Leads with Stage=New have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      try {
        ...
        console.log('✓ III.1: Total text = "0/1" - qualified Lead with Stage=New correctly counted for re-assignment');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Total 0-1 verified - Stage=New', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.1.1 verification completed successfully');
