 ...
 * Test Case ID: CRM-457_2.1.9
 *
 * Summary: Verify the qualified Leads with Stage = Purchase Approval have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.1\.9: --project=chromium
 *
 ...
 *    5.6. Set : "Stage"                = "New"
 *    5.7. Set : "Stage"                = "Purchase Approval"
 *
...
test.describe('CRM-457_2.1.9 - Verify qualified Leads with Stage=Purchase Approval transfered to another Salesperson', () => {
  ...
  test('CRM-457_2.1.9: Verify the qualified Leads with Stage=Purchase Approval have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.7: Stage = "Purchase Approval"');
      await reAssignationPage.selectStage('Purchase Approval');

      console.log('✓ All filter fields set');
      await page.waitForTimeout(CommonUtils.waitTimes.standard);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Step II.5.7 - Stage set to Purchase Approval');
    });
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      try {
        ...
        console.log('✓ III.1: Total text = "0/1" - qualified Lead with Stage=Purchase Approval correctly counted for re-assignment');
        __verifyPassed = true;
      } finally {
    ...
    await test.step('Final Summary', async () => {
      ...
      console.log('   III.1: Total = "0/1" - qualified Lead with Stage=Purchase Approval correctly counted for re-assignment');
