 ...
 * Test Case ID: CRM-457_2.1.16
 *
 * Summary: Verify the qualified Leads with Stage = Activated have been transfered to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.1\.16: --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.1.16
...
test.describe('CRM-457_2.1.16 - Verify qualified Leads with Stage=Activated transfered to another Salesperson', () => {
  ...
  test('CRM-457_2.1.16: Verify the qualified Leads with Stage=Activated have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.7: Stage = "Activated"');
      await reAssignationPage.selectStage('Activated');

      console.log('✓ All filter fields set');
      await page.waitForTimeout(CommonUtils.waitTimes.standard);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Step II.5.7 - Stage set to Activated');
    });
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      try {
        ...
        console.log('✓ III.1: Total text = "0/1" - qualified Lead with Stage=Activated correctly counted for re-assignment');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Total 0-1 verified - Stage=Activated', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.1.16 verification completed successfully');
      console.log(`   Lead#1 URL   : ${url_Lead1}`);
      console.log('   III.1: Total = "0/1" - qualified Lead with Stage=Activated correctly counted for re-assignment');
