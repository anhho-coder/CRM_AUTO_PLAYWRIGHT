 ...
 * Test Case ID: CRM-457_2.1.7
 *
 * Summary: Verify the qualified Leads with Stage = Hot Deal have been transferred to another Salesperson correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.1\.7: --project=chromium
 *
 ...
 *    - Lead name textbox      = TEST Lead 1 CRM-457_2.1.7
...
test.describe('CRM-457_2.1.7 - Verify qualified Leads with Stage=Hot Deal transferred to another Salesperson', () => {
  ...
  test('CRM-457_2.1.7: Verify the qualified Leads with Stage=Hot Deal have been transferred to another Salesperson correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.7: Stage = "Hot Deal"');
    ...
    await test.step('III. Verification - Step 1: Verify Total text = "0 / 1"', async () => {
      ...
      console.log('✓ III.1: Total text = "0/1" - qualified Lead with Stage=Hot Deal correctly counted for re-assignment');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'III.1 - Total 0-1 verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.1.7 verification completed successfully');
      console.log(`   Lead#1 URL   : ${url_Lead1}`);
      console.log('   III.1: Total = "0/1" - qualified Lead with Stage=Hot Deal correctly counted for re-assignment');
