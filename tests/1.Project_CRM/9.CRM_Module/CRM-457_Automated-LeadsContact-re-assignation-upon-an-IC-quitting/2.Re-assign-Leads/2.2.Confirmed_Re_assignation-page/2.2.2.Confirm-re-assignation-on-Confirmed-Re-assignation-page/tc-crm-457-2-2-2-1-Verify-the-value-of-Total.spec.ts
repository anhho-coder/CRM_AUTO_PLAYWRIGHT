 ...
 * Test Case ID: CRM-457_2.2.2.1
 *
 * Summary: Verify value at "Total" text = 1/1 after confirming the re-assignation
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.2\.2\.1: --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.2.2.1
...
test.describe('CRM-457_2.2.2.1 - Verify value at "Total" text = 1/1 after confirming the re-assignation', () => {
  ...
  test('CRM-457_2.2.2.1: Verify value at "Total" text = 1/1 after confirming the re-assignation', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('III. Verification - Step 1: Verify "Total" text = 1/1', async () => {
      ...
      try {
        ...
        console.log(`✓ III.1: "Total" = "1/1" - verified correctly after confirming re-assignation`);
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Total value verified', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.2.2.1 verification completed successfully');
      console.log(`   Lead#1 URL : ${url_Lead1}`);
      console.log(`   III.1: "Total" = "1/1" - verified after confirming re-assignation`);
