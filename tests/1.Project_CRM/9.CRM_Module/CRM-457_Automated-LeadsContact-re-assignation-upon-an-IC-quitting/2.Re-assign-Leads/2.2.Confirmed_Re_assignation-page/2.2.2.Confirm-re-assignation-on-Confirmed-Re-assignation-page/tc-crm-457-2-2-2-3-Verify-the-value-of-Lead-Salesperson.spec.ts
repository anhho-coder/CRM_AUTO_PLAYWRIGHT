 ...
 * Test Case ID: CRM-457_2.2.2.3
 *
 * Summary: Verify Lead.Salesperson is changed to the assigned to salesperson after confirming the re-assignation
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.2\.2\.3: --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.2.2.3
 ...
 *         (Remember the value of selected "Re-assignment to", called selected_ReAssignmentTo#1)
 ...
 * 1. Open new tab then launch the URL_Lead#1
 * 2. Verify the value of "Salesperson" = selected_ReAssignmentTo#1
 *
...
test.describe('CRM-457_2.2.2.3 - Verify Lead.Salesperson is changed to the assigned to salesperson after confirming the re-assignation', () => {
  ...
  test('CRM-457_2.2.2.3: Verify Lead.Salesperson is changed to the assigned to salesperson after confirming the re-assignation', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.3: Re-assignment to = "Alan Osseiran" (remember as selected_ReAssignmentTo#1)');
      await reAssignationPage.selectReAssignmentTo('Alan Osseiran');
      selectedReAssignmentTo1 = 'Alan Osseiran';
      console.log(`  ✓ selected_ReAssignmentTo#1 = "${selectedReAssignmentTo1}"`);
    ...
    await test.step('III. Verification - Step 1: Open new tab and navigate to URL_Lead#1', async () => {
      console.log('\n=== III. VERIFICATION POINTS ===');
      console.log('Step III.1: Opening new tab and navigating to URL_Lead#1');
      ...
      console.log('✓ III.1: Lead#1 page opened in new tab');
      await CommonUtils.captureAndAttachScreenshot(newTab, testInfo, 'III.1 - Lead#1 page opened');

      await test.step('III. Verification - Step 2: Verify Lead.Salesperson = selected_ReAssignmentTo#1', async () => {
        const salespersonValue = await newTabLeadPage.getSalespersonValue();
        console.log(`  Salesperson received          : "${salespersonValue}"`);
        console.log(`  Expected (selected_ReAssignmentTo#1) : "${selectedReAssignmentTo1}"`);

        let __verifyPassed = false;
        try {
          expect(salespersonValue).toBe(selectedReAssignmentTo1);

          console.log(`✓ III.2: Lead.Salesperson = "${selectedReAssignmentTo1}" - verified correctly after re-assignation`);
          __verifyPassed = true;
        } finally {
          await CommonUtils.captureVerifyEvidence(newTab, testInfo, { name: 'III.2 - Lead Salesperson verified', passed: __verifyPassed }).catch(() => {});
        }
      });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.2.2.3 verification completed successfully');
      console.log(`   Lead#1 URL                   : ${url_Lead1}`);
      console.log(`   selected_ReAssignmentTo#1    : ${selectedReAssignmentTo1}`);
      console.log(`   III.2: Lead.Salesperson = "${selectedReAssignmentTo1}" - verified after confirming re-assignation`);
