 ...
 * Test Case ID: CRM-457_2.2.2.2
 *
 * Summary: Verify Stage of process = "TRANSFER" after confirming the re-assignation
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.2\.2\.2: --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.2.2.2
...
test.describe('CRM-457_2.2.2.2 - Verify Stage of process = "TRANSFER" after confirming the re-assignation', () => {
  ...
  test('CRM-457_2.2.2.2: Verify Stage of process = "TRANSFER" after confirming the re-assignation', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('III. Verification - Step 1: Verify Stage of process = "TRANSFER"', async () => {
      console.log('\n=== III. VERIFICATION POINTS ===');
      const stageOfProcess = await reAssignationPage.getStageOfProcessText();
      console.log(`  Stage of process received : "${stageOfProcess}"`);
      console.log(`  Expected                  : "TRANSFER"`);

      let __verifyPassed = false;
      try {
        expect(stageOfProcess).toBe('TRANSFER');

        console.log(`✓ III.1: Stage of process = "TRANSFER" - verified correctly after confirming re-assignation`);
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Stage of process verified', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.2.2.2 verification completed successfully');
      console.log(`   Lead#1 URL         : ${url_Lead1}`);
      console.log(`   III.1: Stage of process = "TRANSFER" - verified after confirming re-assignation`);
