 ...
 * Test Case ID: CRM-457_2.2.2.7
 *
 * Summary: Verify value at "Total" text = 1/1 after confirming the re-assignation with 2 stages
 *
 * Command to run:
 * npx playwright test --grep "CRM-457_2\.2\.2\.7:" --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.2.2.7
 ...
 * 4. Press "SAVE" button  (Lead default stage = New)
 ...
 *    5.6. Set : "Stage"               = "New"
 *    5.7. Set : "Stage"               = "In Process"  (second stage)
...
test.describe('CRM-457_2.2.2.7 - Verify value at "Total" text = 1/1 after confirming re-assignation with 2 stages', () => {
  ...
  test('CRM-457_2.2.2.7: Verify value at "Total" text = 1/1 after confirming re-assignation with 2 stages', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition Step 2: Fill Lead#1 information', async () => {
      ...
      console.log('✓ All Lead#1 fields filled  (Stage will default to "New")');
    ...
    await test.step('Pre-condition Step 4: Press SAVE button', async () => {
      ...
      console.log('✓ Lead#1 saved (Stage = "New" by default)');
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.7: Stage = "In Process"  (second stage)');
    ...
    await test.step('III. Verification - Step 1: Verify "Total" text = 1/1', async () => {
      ...
      console.log('✓ III.1: "Total" = "1/1" - verified correctly after confirming re-assignation with 2 stages (New + In Process)');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'III.1 - Total 1-1 verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.2.2.7 verification completed successfully');
      console.log(`   Lead#1 URL : ${url_Lead1}`);
      console.log('   III.1: "Total" = "1/1" - verified after confirming re-assignation with 2 stages (New + In Process)');
