 ...
 * Test Case ID: CRM-457_2.2.2.6
 *
 * Summary: Verify value at "Total" text = 1/1 after confirming the re-assignation with 2 states
 *
 * Command to run:
 * npx playwright test --grep "CRM-457_2\.2\.2\.6:" --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.2.2.6
 ...
 *    5.6. Set : "Country state"       = "Brussels Capital Region"  (second state)
 *    5.7. Set : "Stage"               = "New"
...
test.describe('CRM-457_2.2.2.6 - Verify value at "Total" text = 1/1 after confirming re-assignation with 2 states', () => {
  ...
  test('CRM-457_2.2.2.6: Verify value at "Total" text = 1/1 after confirming re-assignation with 2 states', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.6: Country state = "Brussels Capital Region"  (second state)');
      await reAssignationPage.selectCountryState('Brussels Capital Region');
    ...
    await test.step('III. Verification - Step 1: Verify "Total" text = 1/1', async () => {
      ...
      console.log('✓ III.1: "Total" = "1/1" - verified correctly after confirming re-assignation with 2 states');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'III.1 - Total 1-1 verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.2.2.6 verification completed successfully');
      console.log(`   Lead#1 URL : ${url_Lead1}`);
      console.log('   III.1: "Total" = "1/1" - verified after confirming re-assignation with 2 states (Flanders + Brussels Capital Region)');
