 ...
 * Test Case ID: CRM-457_2.2.1.4
 *
 * Summary: Verify value of "Selected States" = value of "Country State" on "Confirmed Re-assignation" page correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.2\.1\.4: --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.2.1.4
...
test.describe('CRM-457_2.2.1.4 - Verify Selected States = Country State on Confirmed Re-assignation page', () => {
  ...
  test('CRM-457_2.2.1.4: Verify value of "Selected States" = value of "Country State" on "Confirmed Re-assignation" page correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('III. Verification - Step 1: Verify "Selected States" = selected_country_state#1', async () => {
      console.log('\n=== III. VERIFICATION POINTS ===');
      ...
      console.log(`  Expected (selected_country_state#1) : "${selectedCountryState1}"`);

      let __verifyPassed = false;
      try {
        expect(selectedStatesText).toBe(selectedCountryState1);

        console.log(`✓ III.1: "Selected States" = "${selectedCountryState1}" - verified correctly on Confirmed Re-assignation page`);
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1 - Selected States verified', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.2.1.4 verification completed successfully');
      console.log(`   Lead#1 URL                 : ${url_Lead1}`);
      console.log(`   selected_country_state#1   : ${selectedCountryState1}`);
      console.log(`   III.1: "Selected States" = "${selectedCountryState1}" - verified on Confirmed Re-assignation page`);
