 ...
 * Test Case ID: CRM-457_2.2.1.5
 *
 * Summary: Verify 2 values of "Selected States" = value of "Country State" on "Confirmed Re-assignation" page correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.2\.1\.5: --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.2.1.5
 ...
 *         (Remember the value of selected "Country state", called selected_country_state#1)
 *    5.6. Set : "Country state"       = "Wallonia"
 *         (Remember the value of selected "Country state", called selected_country_state#2)
 *    5.7. Set : "Stage"               = "New"
 * 6. Press "RE-ASSIGNMENT" button and wait
 *
 * III. Verification points:
 * 1. The value at "Selected States" field = selected_country_state#1
 * 2. The value at "Selected States" field = selected_country_state#2
 *
...
test.describe('CRM-457_2.2.1.5 - Verify 2 Selected States = Country State on Confirmed Re-assignation page', () => {
  ...
  test('CRM-457_2.2.1.5: Verify 2 values of "Selected States" = value of "Country State" on "Confirmed Re-assignation" page correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.5: Country state = "Flanders" (remember as selected_country_state#1)');
      await reAssignationPage.selectCountryState('Flanders');
      selectedCountryState1 = 'Flanders';
      console.log(`  ✓ selected_country_state#1 = "${selectedCountryState1}"`);

      console.log('  5.6: Country state = "Wallonia" (remember as selected_country_state#2)');
      await reAssignationPage.selectCountryState('Wallonia');
      selectedCountryState2 = 'Wallonia';
      console.log(`  ✓ selected_country_state#2 = "${selectedCountryState2}"`);
    ...
    await test.step('III. Verification - Step 1: Verify "Selected States" contains selected_country_state#1 and selected_country_state#2', async () => {
      console.log('\n=== III. VERIFICATION POINTS ===');
      const selectedStatesText = await reAssignationPage.getSelectedStatesText();
      console.log(`  Selected States received : "${selectedStatesText}"`);
      console.log(`  Expected to contain (selected_country_state#1) : "${selectedCountryState1}"`);
      console.log(`  Expected to contain (selected_country_state#2) : "${selectedCountryState2}"`);

      let __verifyPassed = false;
      try {
        expect(selectedStatesText).toContain(selectedCountryState1);
        console.log(`✓ III.1: "Selected States" contains "${selectedCountryState1}" - verified correctly`);

        expect(selectedStatesText).toContain(selectedCountryState2);
        console.log(`✓ III.2: "Selected States" contains "${selectedCountryState2}" - verified correctly`);
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'III.1-2 - Selected States verified', passed: __verifyPassed }).catch(() => {});
      }
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.2.1.5 verification completed successfully');
      console.log(`   Lead#1 URL                 : ${url_Lead1}`);
      console.log(`   selected_country_state#1   : ${selectedCountryState1}`);
      console.log(`   selected_country_state#2   : ${selectedCountryState2}`);
      console.log(`   III.1: "Selected States" contains "${selectedCountryState1}" - verified on Confirmed Re-assignation page`);
      console.log(`   III.2: "Selected States" contains "${selectedCountryState2}" - verified on Confirmed Re-assignation page`);
