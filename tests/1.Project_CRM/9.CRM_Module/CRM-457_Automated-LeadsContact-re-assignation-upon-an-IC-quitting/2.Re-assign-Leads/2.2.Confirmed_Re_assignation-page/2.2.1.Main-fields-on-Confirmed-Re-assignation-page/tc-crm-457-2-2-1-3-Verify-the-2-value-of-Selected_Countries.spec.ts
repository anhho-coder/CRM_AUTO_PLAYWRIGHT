 ...
 * Test Case ID: CRM-457_2.2.1.3
 *
 * Summary: Verify 2 values of "Selected Countries" = value of "Countries" on "Confirmed Re-assignation" page correctly
 *
 * Command to run:
 * npx playwright test --grep CRM-457_2\.2\.1\.3: --project=chromium
 *
 ...
 *    - Lead name textbox        = TEST Lead 1 CRM-457_2.2.1.3
 ...
 *         (Remember the value of selected "Country", called selected_country#1)
 *    5.5. Set : "Country"             = "Germany"
 *         (Remember the value of selected "Country", called selected_country#2)
 *    5.6. Set : "Country state"       = "Flanders"
 *    5.7. Set : "Stage"               = "New"
 * 6. Press "RE-ASSIGNMENT" button and wait
 *
 * III. Verification points:
 * 1. The value at "Selected Countries" field = selected_country#1
 * 2. The value at "Selected Countries" field = selected_country#2
 *
...
test.describe('CRM-457_2.2.1.3 - Verify 2 Selected Countries = Countries on Confirmed Re-assignation page', () => {
  ...
  test('CRM-457_2.2.1.3: Verify 2 values of "Selected Countries" = value of "Countries" on "Confirmed Re-assignation" page correctly', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.4: Country = "Belgium" (remember as selected_country#1)');
      await reAssignationPage.selectCountry('Belgium');
      selectedCountry1 = 'Belgium';
      console.log(`  ✓ selected_country#1 = "${selectedCountry1}"`);

      console.log('  5.5: Country = "Germany" (remember as selected_country#2)');
      await reAssignationPage.selectCountry('Germany');
      selectedCountry2 = 'Germany';
      console.log(`  ✓ selected_country#2 = "${selectedCountry2}"`);
    ...
    await test.step('III. Verification - Step 1: Verify "Selected Countries" contains selected_country#1 and selected_country#2', async () => {
      console.log('\n=== III. VERIFICATION POINTS ===');
      const selectedCountriesText = await reAssignationPage.getSelectedCountriesText();
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.2.1.3 verification completed successfully');
      console.log(`   Lead#1 URL           : ${url_Lead1}`);
      console.log(`   selected_country#1   : ${selectedCountry1}`);
      console.log(`   selected_country#2   : ${selectedCountry2}`);
      console.log(`   III.1: "Selected Countries" contains "${selectedCountry1}" - verified on Confirmed Re-assignation page`);
      console.log(`   III.2: "Selected Countries" contains "${selectedCountry2}" - verified on Confirmed Re-assignation page`);
