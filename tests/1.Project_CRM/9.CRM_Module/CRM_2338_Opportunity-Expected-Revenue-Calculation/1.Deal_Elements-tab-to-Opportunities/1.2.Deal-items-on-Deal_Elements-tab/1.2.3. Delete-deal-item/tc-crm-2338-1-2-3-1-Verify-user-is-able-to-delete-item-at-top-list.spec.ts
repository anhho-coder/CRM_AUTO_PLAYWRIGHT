 ...
 * Test Case ID: CRM-2338_1.2.3.1
 *
 * Summary: Verify that the user is able to delete the deal item at top of list
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.2\.3\.1:" --project=chromium
 *
 ...
 * 4. Press SAVE button
 * 5. Press EDIT button
 * 6. Press Trash Bin icon to remove product line named [A2144B]
 * 7. Press SAVE button
 *
 * VIII. Verification:
 * 1. There is no more product line named [A2144B]
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.2.3.1 - Verify that the user is able to delete the deal item at top of list', () => {
  ...
  test('CRM-2338_1.2.3.1: Verify that the user is able to delete the deal item at top of list', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6: Press Trash Bin icon to remove product line "${product_line1}"`, async () => {
      console.log(`Step VII.6: Removing product line ${product_line1} via Trash Bin`);
      await dealElementPage.removeProductLineByName(product_line1);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6 - Product line ${product_line1} removed`);
    });
    ...
    await test.step(`VIII.1: Verify product line "${product_line1}" is no longer present`, async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      console.log(`Step VIII.1: Verifying product line ${product_line1} is removed`);
      const stillPresent = await dealElementPage.isProductInOrderLines(product_line1);
      expect(stillPresent, `VIII.1: Product line ${product_line1} should NOT be present after deletion`).toBe(false);
      console.log(`\u2713 VIII.1: Product line "${product_line1}" confirmed REMOVED from Order Lines`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VIII.1 - Deletion verified`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.2.3.1 verification completed successfully');
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1     : ${url_Opp1}`);
      console.log(`   VII.6: Product line "${product_line1}" deleted via Trash Bin icon`);
      console.log(`   VIII.1: Product line "${product_line1}" no longer in Order Lines \u2014 confirmed`);
