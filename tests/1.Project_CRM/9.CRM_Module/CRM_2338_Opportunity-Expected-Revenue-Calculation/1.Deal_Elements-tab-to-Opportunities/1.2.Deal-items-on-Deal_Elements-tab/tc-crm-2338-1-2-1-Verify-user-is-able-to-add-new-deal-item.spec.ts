 ...
 * Test Case ID: CRM-2338_1.2.1
 *
 * Summary: Verify that the user is able to add new deal item
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.2\.1:" --project=chromium
 *
 ...
 *
 * VIII. Verification:
 * 1. There are 2 Products added in the Order Lines section
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.2.1 - Verify that the user is able to add new deal item', () => {
  ...
  test('CRM-2338_1.2.1: Verify that the user is able to add new deal item', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step VII.3a: Add product [A2144B], Ordered Qty = 1, UoM = Socket', async () => {
      console.log('Step VII.3a: Adding product [A2144B], Qty=1, UoM=Socket');
      await dealElementPage.addProductLine('[A2144B]', 1, 'Socket');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.3a - Product [A2144B] added');
    });

    await test.step('Step VII.3b: Add product [A2145B], Ordered Qty = 1, UoM = Socket', async () => {
      console.log('Step VII.3b: Adding product [A2145B], Qty=1, UoM=Socket');
      await dealElementPage.addProductLine('[A2145B]', 1, 'Socket');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.3b - Product [A2145B] added');
    });
    ...
    await test.step('VIII.1: Verify there are 2 products in Order Lines', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.2.1 verification completed successfully');
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log('   VIII.1: 2 products added to Order Lines — confirmed');
