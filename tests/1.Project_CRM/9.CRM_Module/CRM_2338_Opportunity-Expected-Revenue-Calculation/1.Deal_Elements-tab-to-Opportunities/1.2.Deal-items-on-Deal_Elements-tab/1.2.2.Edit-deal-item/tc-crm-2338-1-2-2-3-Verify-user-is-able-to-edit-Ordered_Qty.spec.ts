 ...
 * Test Case ID: CRM-2338_1.2.2.3
 *
 * Summary: Verify that the user is able to edit Ordered Qty
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.2\.2\.3:" --project=chromium
 *
 ...
 * 7. Change the value of "Ordered Qty" field to "10"
 * 8. Press SAVE button
 *
 * VIII. Verification:
 * 1. The value of "Ordered Qty" field at product line named [A2144B] is changed to "10"
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.2.2.3 - Verify that the user is able to edit Ordered Qty', () => {
  ...
  test('CRM-2338_1.2.2.3: Verify that the user is able to edit Ordered Qty', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Ordered Qty to "${qty_edit}"`, async () => {
      console.log(`Step VII.6-7: Changing Ordered Qty of ${product_line1} row to "${qty_edit}"`);
      await dealElementPage.changeQtyInRow(product_line1, qty_edit);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Ordered Qty changed to ${qty_edit}`);
    });
    ...
    await test.step(`VIII.1: Verify Ordered Qty at ${product_line1} row is changed to "${qty_edit}"`, async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      console.log(`Step VIII.1: Verifying Ordered Qty at ${product_line1} row is "${qty_edit}"`);
      const hasQty = await dealElementPage.isQtyInOrderLine(product_line1, qty_edit);
      expect(hasQty, `VIII.1: Ordered Qty at ${product_line1} row should be "${qty_edit}"`).toBe(true);
      console.log(`\u2713 VIII.1: Ordered Qty "${qty_edit}" confirmed on ${product_line1} row`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VIII.1 - Ordered Qty verified`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.2.2.3 verification completed successfully');
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log(`   VIII.1: Ordered Qty "${qty_edit}" on ${product_line1} row \u2014 confirmed`);
