 ...
 * Test Case ID: CRM-2338_1.2.2.6
 *
 * Summary: Verify that the user is able to edit Taxes
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.2\.2\.6:" --project=chromium
 *
 ...
 * 7. Change the value of "Taxes" field to "Tax 15.00%"
 * 8. Press SAVE button
 *
 * VIII. Verification:
 * 1. The value of "Taxes" field at product line named [A2144B] is changed to "Tax 15.00%"
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.2.2.6 - Verify that the user is able to edit Taxes', () => {
  ...
  test('CRM-2338_1.2.2.6: Verify that the user is able to edit Taxes', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Taxes to "${tax_edit}"`, async () => {
      console.log(`Step VII.6-7: Changing Taxes of ${product_line1} row to "${tax_edit}"`);
      await dealElementPage.changeTaxInRow(product_line1, tax_edit);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Taxes changed to ${tax_edit}`);
    });
    ...
    await test.step(`VIII.1: Verify Taxes at ${product_line1} row is changed to "${tax_edit}"`, async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      console.log(`Step VIII.1: Verifying Taxes at ${product_line1} row is "${tax_edit}"`);
      const hasTax = await dealElementPage.isTaxInOrderLine(product_line1, tax_edit);
      expect(hasTax, `VIII.1: Taxes at ${product_line1} row should be "${tax_edit}"`).toBe(true);
      console.log(`\u2713 VIII.1: Tax "${tax_edit}" confirmed on ${product_line1} row`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VIII.1 - Taxes verified`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.2.2.6 verification completed successfully');
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log(`   VIII.1: Tax "${tax_edit}" on ${product_line1} row \u2014 confirmed`);
