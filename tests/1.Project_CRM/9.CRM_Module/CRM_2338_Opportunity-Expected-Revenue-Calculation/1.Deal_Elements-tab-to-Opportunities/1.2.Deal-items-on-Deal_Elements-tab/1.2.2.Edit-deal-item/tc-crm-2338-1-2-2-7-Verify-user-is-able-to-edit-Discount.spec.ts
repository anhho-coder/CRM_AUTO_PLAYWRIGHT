 ...
 * Test Case ID: CRM-2338_1.2.2.7
 *
 * Summary: Verify that the user is able to edit Discount %
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.2\.2\.7:" --project=chromium
 *
 ...
 * 7. Change the value of "Discount %" field to "10.00"
 * 8. Press SAVE button
 *
 * VIII. Verification:
 * 1. The value of "Discount %" field at product line named [A2144B] is changed to "10.00"
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.2.2.7 - Verify that the user is able to edit Discount %', () => {
  ...
  test('CRM-2338_1.2.2.7: Verify that the user is able to edit Discount %', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    console.log(`  discount_edit  : ${discount_edit}`);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Discount % to "${discount_edit}"`, async () => {
      console.log(`Step VII.6-7: Changing Discount % of ${product_line1} row to "${discount_edit}"`);
      await dealElementPage.changeDiscountInRow(product_line1, discount_edit);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Discount changed to ${discount_edit}`);
    });
    ...
    await test.step(`VIII.1: Verify Discount % at ${product_line1} row is changed to "${discount_edit}"`, async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      console.log(`Step VIII.1: Verifying Discount % at ${product_line1} row is "${discount_edit}"`);
      const hasDiscount = await dealElementPage.isDiscountInOrderLine(product_line1, discount_edit);
      expect(hasDiscount, `VIII.1: Discount % at ${product_line1} row should be "${discount_edit}"`).toBe(true);
      console.log(`\u2713 VIII.1: Discount "${discount_edit}" confirmed on ${product_line1} row`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VIII.1 - Discount verified`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.2.2.7 verification completed successfully');
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log(`   VIII.1: Discount "${discount_edit}" on ${product_line1} row \u2014 confirmed`);
