 ...
 * Test Case ID: CRM-2338_1.2.2.2
 *
 * Summary: Verify that the user is able to edit description
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.2\.2\.2:" --project=chromium
 *
 ...
 * 7. Change the Description to "TEST"
 * 8. Press SAVE button
 *
 * VIII. Verification:
...
test.describe('CRM-2338_1.2.2.2 - Verify that the user is able to edit description', () => {
  ...
  test('CRM-2338_1.2.2.2: Verify that the user is able to edit description', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    console.log(`  description_edit: ${description_edit}`);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Description to "${description_edit}"`, async () => {
      console.log(`Step VII.6-7: Changing description of ${product_line1} row to "${description_edit}"`);
      await dealElementPage.changeDescriptionInRow(product_line1, description_edit);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Description changed to ${description_edit}`);
    });
    ...
    await test.step(`VIII.1: Verify description at ${product_line1} row is changed to "${description_edit}"`, async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      console.log(`Step VIII.1: Verifying description at ${product_line1} row is "${description_edit}"`);
      const hasDescription = await dealElementPage.isDescriptionInOrderLine(product_line1, description_edit);
      expect(hasDescription, `VIII.1: Description at ${product_line1} row should be "${description_edit}"`).toBe(true);
      console.log(`\u2713 VIII.1: Description "${description_edit}" confirmed on ${product_line1} row`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VIII.1 - Description verified`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.2.2.2 verification completed successfully');
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log(`   VIII.1: Description "${description_edit}" on ${product_line1} row — confirmed`);
