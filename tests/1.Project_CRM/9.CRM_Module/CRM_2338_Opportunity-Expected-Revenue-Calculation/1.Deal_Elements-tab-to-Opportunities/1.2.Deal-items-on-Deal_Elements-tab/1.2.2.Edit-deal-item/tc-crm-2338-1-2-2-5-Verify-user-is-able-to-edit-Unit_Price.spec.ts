 ...
 * Test Case ID: CRM-2338_1.2.2.5
 *
 * Summary: Verify that the user is able to edit Unit Price
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.2\.2\.5:" --project=chromium
 *
 ...
 * 7. Change the value of "Unit Price" field to "100.00"
 * 8. Press SAVE button
 *
 * VIII. Verification:
 * 1. The value of "Unit Price" field at product line named [A2144B] is changed to "100.00"
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.2.2.5 - Verify that the user is able to edit Unit Price', () => {
  ...
  test('CRM-2338_1.2.2.5: Verify that the user is able to edit Unit Price', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    console.log(`  unit_price_edit : ${unit_price_edit}`);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Unit Price to "${unit_price_edit}"`, async () => {
      console.log(`Step VII.6-7: Changing Unit Price of ${product_line1} row to "${unit_price_edit}"`);
      await dealElementPage.changeUnitPriceInRow(product_line1, unit_price_edit);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Unit Price changed to ${unit_price_edit}`);
    });
    ...
    await test.step(`VIII.1: Verify Unit Price at ${product_line1} row is changed to "${unit_price_edit.toFixed(2)}"`, async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      console.log(`Step VIII.1: Verifying Unit Price at ${product_line1} row is "${unit_price_edit.toFixed(2)}"`);
      const hasUnitPrice = await dealElementPage.isUnitPriceInOrderLine(product_line1, unit_price_edit);
      expect(hasUnitPrice, `VIII.1: Unit Price at ${product_line1} row should be "${unit_price_edit.toFixed(2)}"`).toBe(true);
      console.log(`\u2713 VIII.1: Unit Price "${unit_price_edit.toFixed(2)}" confirmed on ${product_line1} row`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VIII.1 - Unit Price verified`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.2.2.5 verification completed successfully');
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log(`   VIII.1: Unit Price "${unit_price_edit.toFixed(2)}" on ${product_line1} row \u2014 confirmed`);
