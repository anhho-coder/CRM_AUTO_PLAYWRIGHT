 ...
 * Test Case ID: CRM-2338_1.2.2.4
 *
 * Summary: Verify that the user is able to edit Unit of Measure
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.2\.2\.4:" --project=chromium
 *
 ...
 * 7. Change the value of "Unit of Measure" field to "Database"
 * 8. Press SAVE button
 *
 * VIII. Verification:
 * 1. The value of "Unit of Measure" field at product line named [A2144B] is changed to "Database"
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.2.2.4 - Verify that the user is able to edit Unit of Measure', () => {
  ...
  test('CRM-2338_1.2.2.4: Verify that the user is able to edit Unit of Measure', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Unit of Measure to "${uom_edit}"`, async () => {
      console.log(`Step VII.6-7: Changing Unit of Measure of ${product_line1} row to "${uom_edit}"`);
      await dealElementPage.changeUoMInRow(product_line1, uom_edit);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Unit of Measure changed to ${uom_edit}`);
    });
    ...
    await test.step(`VIII.1: Verify Unit of Measure at ${product_line1} row is changed to "${uom_edit}"`, async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      console.log(`Step VIII.1: Verifying Unit of Measure at ${product_line1} row is "${uom_edit}"`);
      const hasUoM = await dealElementPage.isUoMInOrderLine(product_line1, uom_edit);
      expect(hasUoM, `VIII.1: Unit of Measure at ${product_line1} row should be "${uom_edit}"`).toBe(true);
      console.log(`\u2713 VIII.1: Unit of Measure "${uom_edit}" confirmed on ${product_line1} row`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VIII.1 - Unit of Measure verified`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.2.2.4 verification completed successfully');
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log(`   VIII.1: Unit of Measure "${uom_edit}" on ${product_line1} row \u2014 confirmed`);
