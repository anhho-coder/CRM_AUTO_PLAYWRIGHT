 ...
 * Test Case ID: CRM-2338_1.1.8.2
 *
 * Summary: Verify the Deal Element.Validity is a required field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.8\.2:" --project=chromium
 *
 ...
 * 3. Set Validity field = [BLANK]
 * 4. Press SAVE button
 *
 * VIII. Verification:
 * 1. Error message: "The following fields are invalid: Validity"
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.8.2 - Verify the Deal Element.Validity is a required field', () => {
  ...
  test('CRM-2338_1.1.8.2: Verify the Deal Element.Validity is a required field', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step VII.3: Set Validity field = [BLANK]', async () => {
      console.log('Step VII.3: Clearing Validity field');
      await dealElementPage.clearValidityField();
      console.log('\u2713 Pricelist field cleared (set to blank)');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.3 - Validity cleared');
    });
    ...
    await test.step('VIII.1: Verify error message "The following fields are invalid: Validity"', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      ...
      expect(errorMessage).toContain('Validity');
    ...
    await test.step('Final Summary', async () => {
      ...
      console.log(`   URL_Opp#1         : ${url_Opp1}`);
      console.log('   VIII.1: Error message "The following fields are invalid: Validity" confirmed');
