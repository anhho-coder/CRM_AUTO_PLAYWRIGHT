 ...
 * Test Case ID: CRM-2338_1.1.7.2
 *
 * Summary: Verify the Deal Element.Pricelist getting from Opp.Reseller.Pricelist
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.7\.2:" --project=chromium
 *
 ...
 * 12. Close the browser
 *
 * IV. Pre-condition#2 - Create Reseller#1 (Contact with Bronze level + Pricelist)
 * V.  Pre-condition#3 - Create EndUser#1 (Contact, no partner level)
 * VI. Pre-condition#4 - Create Opp#1 (Reseller=Reseller#1, NO Distributor)
 *
 * VII. Steps to reproduce:
 * 1. Press "DEAL ELEMENT" button
 *
 * VIII. Verification:
 * 1. Pricelist field value = Pricelist_Reseller#1 (Public Pricelist_EUR)
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.7.2 - Verify the Deal Element.Pricelist getting from Opp.Reseller.Pricelist', () => {
  ...
  test('CRM-2338_1.1.7.2: Verify the Deal Element.Pricelist getting from Opp.Reseller.Pricelist', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition IV.1: Login and navigate to Contacts', async () => {
      console.log(`\n=== IV. PRE-CONDITION#2: Create Reseller#1 ===`);
      console.log(`Step IV.1: Logging in as ${users.admin_crm.displayName}`);
    ...
    await test.step('Pre-condition IV.3: Set Pricelist on Sales & Purchases tab', async () => {
      pricelist_Reseller1 = 'Public Pricelist_EUR';
      console.log(`Step IV.3: Setting Pricelist = "${pricelist_Reseller1}" on Sales & Purchases tab`);
      await contactPage.clickSalesPurchasesTab();
      await contactPage.selectPricelist(pricelist_Reseller1);
      console.log(`\u2713 IV.3: Pricelist set to "${pricelist_Reseller1}"`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.3 - Pricelist set');
    });

    await test.step('Pre-condition IV.4: Save Reseller#1 and copy URL', async () => {
      console.log('Step IV.4: Saving Reseller#1 and capturing URL');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.4 - Reseller#1 complete');
    });

    await test.step('Pre-condition IV.5: Return to home page', async () => {
      console.log('Step IV.5: Returning to home page');
    ...
    await test.step('VIII. Verification - Step 1: Verify Pricelist = Pricelist_Reseller#1', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.7.2 verification completed successfully');
      console.log(`   Reseller#1             : "${name_Reseller1}"`);
      console.log(`   Pricelist_Reseller#1   : "${pricelist_Reseller1}"`);
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log('   VIII.1: Deal Element.Pricelist = Pricelist_Reseller#1 (from Opp.Reseller.Pricelist)');
