 ...
 * Test Case ID: CRM-2338_1.1.7.4
 *
 * Summary: Verify the Deal Element.Pricelist is a required field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.7\.4:" --project=chromium
 *
 ...
 * 12. Close the browser
 *
 * V.  Pre-condition#3 - Create EndUser#1 (Contact with Pricelist)
 * VI. Pre-condition#4 - Create Opp#1 (NO Reseller, NO Distributor)
 *
 * VII. Steps to reproduce:
 * 1. Press "DEAL ELEMENT" button
 * 2. Set Payment Terms = Immediate Payment
 * 3. Set Pricelist field = [BLANK]
 * 4. Press SAVE button
 *
 * VIII. Verification:
 * 1. Error message: "The following fields are invalid: Pricelist"
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.7.4 - Verify the Deal Element.Pricelist is a required field', () => {
  ...
  test('CRM-2338_1.1.7.4: Verify the Deal Element.Pricelist is a required field', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step VII.2: Set Payment Terms = Immediate Payment', async () => {
      console.log('Step VII.2: Setting Payment Terms = Immediate Payment');
      await dealElementPage.selectPaymentTerm('Immediate Payment');
      console.log('\u2713 Payment Terms = "Immediate Payment"');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.2 - Payment Terms set');
    });

    await test.step('Step VII.3: Set Pricelist field = [BLANK]', async () => {
      console.log('Step VII.3: Clearing Pricelist field');
      await dealElementPage.clearPricelistField();
      console.log('\u2713 Pricelist field cleared (set to blank)');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.3 - Pricelist cleared');
    });
    ...
    await test.step('VIII.1: Verify error message "The following fields are invalid: Pricelist"', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      ...
      expect(errorMessage).toContain('Pricelist');
      console.log('\u2713 VIII.1: Error message confirmed - "The following fields are invalid: Pricelist"');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VIII.1 - Validation error verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.7.4 verification completed successfully');
      console.log(`   EndUser#1         : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1         : ${url_Opp1}`);
      console.log('   VIII.1: Error message "The following fields are invalid: Pricelist" confirmed');
