 ...
 * Test Case ID: CRM-2338_1.1.7.1
 *
 * Summary: Verify the Deal Element.Pricelist getting from Opp.Distributor.Pricelist
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.7\.1:" --project=chromium
 *
 ...
 * 12. Close the browser
 *
 * III. Pre-condition#1 - Create Distributor#1 (Contact with Distributor level + Pricelist)
 ...
 *
 * VIII. Verification:
 * 1. Pricelist field value = Pricelist_Distributor#1 (Public Pricelist_EUR)
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.7.1 - Verify the Deal Element.Pricelist getting from Opp.Distributor.Pricelist', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    url_Opp1              = '';
    email_EndUser1        = '';
    pricelist_Distributor1 = '';
  });

  test('CRM-2338_1.1.7.1: Verify the Deal Element.Pricelist getting from Opp.Distributor.Pricelist', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition III.3: Set Pricelist on Sales & Purchases tab', async () => {
      pricelist_Distributor1 = 'Public Pricelist_EUR';
      console.log(`Step III.3: Setting Pricelist = "${pricelist_Distributor1}" on Sales & Purchases tab`);
      await contactPage.clickSalesPurchasesTab();
      await contactPage.selectPricelist(pricelist_Distributor1);
      console.log(`\u2713 III.3: Pricelist set to "${pricelist_Distributor1}"`);
    ...
    await test.step('Pre-condition V.3: Save EndUser#1 and copy URL', async () => {
      console.log('Step V.3: Saving EndUser#1 and capturing URL');
      await contactPage.clickSaveIfEditable();
      await contactPage.waitForSaveComplete();
      await page.waitForFunction(
        () => { const m = window.location.href.match(/[?&#]id=(\d+)/); return m && m[1]; },
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'V.3 - EndUser#1 complete');
    });

    await test.step('Pre-condition V.4: Return to home page', async () => {
      console.log('Step V.4: Returning to home page');
      await homePage.returnToHome();
      await homePage.waitForPageFullyLoaded();
      console.log('\u2713 Home page loaded');
    });
    ...
    await test.step('VIII. Verification - Step 1: Verify Pricelist = Pricelist_Distributor#1', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      const pricelistValue = await dealElementPage.getPricelistValue();
      console.log(`  Pricelist value received: "${pricelistValue}"`);
      console.log(`  Expected                : "${pricelist_Distributor1}"`);
      expect(pricelistValue).toContain(pricelist_Distributor1);
      console.log(`\u2713 VIII.1: Pricelist = "${pricelist_Distributor1}" - correctly propagated from Opp.Distributor.Pricelist`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VIII.1 - Pricelist verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.7.1 verification completed successfully');
      console.log(`   Distributor#1          : "${name_Distributor1}"`);
      console.log(`   Pricelist_Distributor#1: "${pricelist_Distributor1}"`);
      console.log(`   Reseller#1             : "${name_Reseller1}"`);
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log('   VIII.1: Deal Element.Pricelist = Pricelist_Distributor#1 (from Opp.Distributor.Pricelist)');
