 ...
 * Test Case ID: CRM-2338_1.1.7.3
 *
 * Summary: Verify the Deal Element.Pricelist getting from Opp.End User.Pricelist
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.7\.3:" --project=chromium
 *
 ...
 *
 * VIII. Verification:
 * 1. Pricelist field value = Pricelist_EndUser#1 (Public Pricelist_EUR)
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.7.3 - Verify the Deal Element.Pricelist getting from Opp.End User.Pricelist', () => {
  ...
  test('CRM-2338_1.1.7.3: Verify the Deal Element.Pricelist getting from Opp.End User.Pricelist', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition V.3: Set Pricelist on Sales & Purchases tab', async () => {
      pricelist_EndUser1 = 'Public Pricelist_EUR';
      console.log(`Step V.3: Setting Pricelist = "${pricelist_EndUser1}" on Sales & Purchases tab`);
      await contactPage.clickEdit();
      await contactPage.clickSalesPurchasesTab();
      await contactPage.selectPricelist(pricelist_EndUser1);
      console.log(`\u2713 V.3: Pricelist set to "${pricelist_EndUser1}"`);
    ...
    await test.step('VIII. Verification - Step 1: Verify Pricelist = Pricelist_EndUser#1', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      const pricelistValue = await dealElementPage.getPricelistValue();
      console.log(`  Pricelist value received: "${pricelistValue}"`);
      console.log(`  Expected                : "${pricelist_EndUser1}"`);
      expect(pricelistValue).toContain(pricelist_EndUser1);
      console.log(`\u2713 VIII.1: Pricelist = "${pricelist_EndUser1}" - correctly propagated from Opp.End User.Pricelist`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VIII.1 - Pricelist verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.7.3 verification completed successfully');
      console.log(`   EndUser#1              : "${name_EndUser1}"`);
      console.log(`   Pricelist_EndUser#1    : "${pricelist_EndUser1}"`);
      console.log(`   URL_Opp#1              : ${url_Opp1}`);
      console.log('   VIII.1: Deal Element.Pricelist = Pricelist_EndUser#1 (from Opp.End User.Pricelist)');
