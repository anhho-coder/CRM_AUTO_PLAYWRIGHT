 ...
 * Test Case ID: CRM-2338_1.1.5.1
 *
 * Summary: Verify the Deal Element.Invoice Address getting from Opp.Distributor
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.5\.1:" --project=chromium
 *
 ...
 *
 * VIII. Verification:
 * 1. Invoice Address field value = Name_Distributor#1
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.5.1 - Verify the Deal Element.Invoice Address getting from Opp.Distributor', () => {
  ...
  test('CRM-2338_1.1.5.1: Verify the Deal Element.Invoice Address getting from Opp.Distributor', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('VIII. Verification - Step 1: Verify Invoice Address = Name_Distributor#1', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      ...
      expect(invoiceAddressValue).toBe(name_Distributor1);
      console.log(`\u2713 VIII.1: Invoice Address = "${name_Distributor1}" - Invoice Address correctly set from Distributor`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VIII.1 - Invoice Address verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.5.1 verification completed successfully');
      console.log(`   Distributor#1     : "${name_Distributor1}"`);
      console.log(`   Reseller#1        : "${name_Reseller1}"`);
      console.log(`   EndUser#1         : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1         : ${url_Opp1}`);
      console.log('   VIII.1: Deal Element.Invoice Address = Name_Distributor#1 (from Opp.Distributor)');
