 ...
 * Test Case ID: CRM-2338_1.1.5.4
 *
 * Summary: Verify the Deal Element.Invoice Address is a required field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.5\.4:" --project=chromium
 *
 ...
 * 2. Set Invoice Address field = [BLANK]
 * 3. Set Payment Terms = Immediate Payment
 * 4. Press SAVE button
 *
 * VIII. Verification:
 * 1. Error message: "The following fields are invalid: Invoice Address"
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.5.4 - Verify the Deal Element.Invoice Address is a required field', () => {
  ...
  test('CRM-2338_1.1.5.4: Verify the Deal Element.Invoice Address is a required field', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step VII.2: Set Invoice Address field = [BLANK]', async () => {
      console.log('Step VII.2: Clearing Invoice Address field');
      await dealElementPage.clearInvoiceAddressField();
      console.log('\u2713 Invoice Address field cleared (set to blank)');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.2 - Invoice Address cleared');
    });
    ...
    await test.step('Step VII.4: Press SAVE button', async () => {
      console.log('Step VII.4: Clicking SAVE button (expecting validation error)');
      await dealElementPage.clickSaveForValidation();
      console.log('\u2713 SAVE clicked');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.4 - After SAVE click');
    });
    ...
    await test.step('VIII.1: Verify error message "The following fields are invalid: Invoice Address"', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      ...
      expect(errorMessage).toContain('Invoice Address');
      console.log('\u2713 VIII.1: Error message confirmed - "The following fields are invalid: Invoice Address"');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VIII.1 - Validation error verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.5.4 verification completed successfully');
      console.log(`   EndUser#1         : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1         : ${url_Opp1}`);
      console.log('   VIII.1: Error message "The following fields are invalid: Invoice Address" confirmed');
