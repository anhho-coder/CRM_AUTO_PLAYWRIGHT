 ...
 * Test Case ID: CRM-2338_1.1.4.3
 *
 * Summary: Verify the Deal Element.Payer getting from Opp.End User
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.4\.3:" --project=chromium
 *
 ...
 *
 * VIII. Verification:
 * 1. Payer field value = Name_EndUser#1
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.4.3 - Verify the Deal Element.Payer getting from Opp.End User', () => {
  ...
  test('CRM-2338_1.1.4.3: Verify the Deal Element.Payer getting from Opp.End User', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('VIII. Verification - Step 1: Verify Payer = Name_EndUser#1', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      ...
      expect(payerValue).toBe(name_EndUser1);
      console.log(`\u2713 VIII.1: Payer = "${name_EndUser1}" - Payer correctly set from End User`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VIII.1 - Payer verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.4.3 verification completed successfully');
      console.log(`   EndUser#1         : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1         : ${url_Opp1}`);
      console.log('   VIII.1: Deal Element.Payer = Name_EndUser#1 (from Opp.End User)');
