 ...
 * Test Case ID: CRM-2338_1.1.4.2
 *
 * Summary: Verify the Deal Element.Payer getting from Opp.Reseller (when no Distributor)
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.4\.2:" --project=chromium
 *
 ...
 * 12. Close the browser
 *
 * III. Pre-condition#1 - Create Reseller#1 (Contact with Bronze level)
 * IV.  Pre-condition#2 - Create EndUser#1 (Contact, no partner level)
 * V.   Pre-condition#3 - Create Opp#1 (Reseller=Reseller#1, NO Distributor)
 *
 * VII. Steps to reproduce:
 * 1. Press "DEAL ELEMENT" button
 *
 * VIII. Verification:
 * 1. Payer field value = Name_Reseller#1
 *
 * IX. Tear down:
 * 1. Delete Reseller#1 via URL_Reseller#1
 * 2. Delete EndUser#1 via URL_EndUser#1
 * 3. Delete Opp#1 via URL_Opp#1
 * 4. Close all browsers
 */

test.describe('CRM-2338_1.1.4.2 - Verify the Deal Element.Payer getting from Opp.Reseller (when no Distributor)', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    await test.step('IX.1: Delete Reseller#1', async () => {
      if (!url_Reseller1) { console.log('  \u26a0 No URL for Reseller#1 - skipping'); return; }
      console.log(`Step IX.1: Deleting Reseller#1 at ${url_Reseller1}`);
      await teardownPage.deleteContactByURL(url_Reseller1);
      console.log('\u2713 IX.1: Reseller#1 deleted');
    });

    await test.step('IX.2: Delete EndUser#1', async () => {
      if (!url_EndUser1) { console.log('  \u26a0 No URL for EndUser#1 - skipping'); return; }
      console.log(`Step IX.2: Deleting EndUser#1 at ${url_EndUser1}`);
      await teardownPage.deleteContactByURL(url_EndUser1);
      console.log('\u2713 IX.2: EndUser#1 deleted');
    });

    await test.step('IX.3: Delete Opp#1', async () => {
      if (!url_Opp1) { console.log('  \u26a0 No URL for Opp#1 - skipping'); return; }
      console.log(`Step IX.3: Deleting Opp#1 at ${url_Opp1}`);
      await teardownPage.deleteContactByURL(url_Opp1);
      console.log('\u2713 IX.3: Opp#1 deleted');
    });
  ...
  test('CRM-2338_1.1.4.2: Verify the Deal Element.Payer getting from Opp.Reseller (when no Distributor)', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    // ==============================================================
    // III. PRE-CONDITION#1: Create Reseller#1
    // ==============================================================
    ...
    await test.step('Pre-condition III.2: Create Reseller#1 (Bronze level)', async () => {
      ...
      console.log(`Step III.2: Creating Reseller#1 - Name: "${name_Reseller1}"`);
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'III.2 - Partner level Bronze set');
    });

    await test.step('Pre-condition III.3: Save Reseller#1 and copy URL', async () => {
      console.log('Step III.3: Saving Reseller#1 and capturing URL');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'III.3 - Reseller#1 complete');
    });
    ...
    // ==============================================================
    // IV. PRE-CONDITION#2: Create EndUser#1
    // ==============================================================

    await test.step('Pre-condition IV.1: Navigate to Contacts for EndUser#1', async () => {
      console.log(`\n=== IV. PRE-CONDITION#2: Create EndUser#1 ===`);
      await homePage.navigateToContactsFromHome();
      await homePage.waitForPageReady();
      console.log('\u2713 Navigated to Contacts');
    ...
    await test.step('Pre-condition IV.2: Create EndUser#1', async () => {
      name_EndUser1  = `TEST-EndUser#1_${tcId}_${timestamp}`;
      email_EndUser1 = `Test-EndUser@EndUser-company${timestamp}.com`;
      console.log(`Step IV.2: Creating EndUser#1 - Name: "${name_EndUser1}"`);
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.2 - EndUser#1 saved');
    });

    await test.step('Pre-condition IV.3: Return to home page', async () => {
      console.log('Step IV.3: Returning to home page');
      await homePage.returnToHome();
      await homePage.waitForPageFullyLoaded();
      console.log('\u2713 Home page loaded');
    });
    ...
    // ==============================================================
    // V. PRE-CONDITION#3: Create Opp#1
    // ==============================================================

    await test.step('Pre-condition V.1: Navigate to CRM and switch to list view', async () => {
      console.log(`\n=== V. PRE-CONDITION#3: Create Opp#1 ===`);
      await homePage.navigateToCRM();
      await homePage.waitForPageReady();
      await opportunityPage.switchToListView();
      console.log('\u2713 CRM list view activated');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'V.1 - CRM list view');
    });

    await test.step('Pre-condition V.2: Click CREATE for Opp#1', async () => {
      console.log('Step V.2: Clicking CREATE button');
      await opportunityPage.clickCreate();
      console.log('\u2713 Opp creation form opened');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'V.2 - Opp creation form');
    });

    await test.step('Pre-condition V.3: Fill Opp#1 information', async () => {
      const oppName = `TEST Opp 1 ${tcId}`;
      console.log(`Step V.3: Filling Opp#1 - Name: "${oppName}"`);
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'V.3 - Opp#1 fields filled');
    });

    await test.step('Pre-condition V.4: Click CRM Developer tab and set Lead form', async () => {
      console.log('Step V.4: Setting Lead form = Download Free Trial');
      await opportunityPage.clickCRMDeveloperTab();
      await opportunityPage.fillLeadForm('Download Free Trial');
      console.log('\u2713 Lead form = "Download Free Trial"');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'V.4 - CRM Developer tab');
    });

    await test.step('Pre-condition V.5: Select Stage = NEW', async () => {
      console.log('Step V.5: Selecting Stage = NEW');
      await opportunityPage.selectStage('New');
      console.log('\u2713 Stage = NEW');
    });

    await test.step('Pre-condition V.6: Save Opp#1 and copy URL', async () => {
      console.log('Step V.6: Saving Opp#1');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'V.6 - Opp#1 saved');
    });

    await test.step('Pre-condition V.7: Refresh page to verify Contact field (up to 5 times)', async () => {
      console.log('Step V.7: Waiting for Contact field to populate (up to 5 refreshes)');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'V.7 - After contact field refresh');
    });
    ...
    await test.step('VIII. Verification - Step 1: Verify Payer = Name_Reseller#1', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.4.2 verification completed successfully');
      console.log(`   Reseller#1        : "${name_Reseller1}"`);
      console.log(`   EndUser#1         : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1         : ${url_Opp1}`);
      console.log('   VIII.1: Deal Element.Payer = Name_Reseller#1 (from Opp.Reseller, no Distributor)');
