 ...
 * Test Case ID: CRM-2338_1.1.4.4
 *
 * Summary: Verify the Deal Element.Payer is a required field
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.4\.4:" --project=chromium
 *
 ...
 * 2. Set Payer field = [BLANK]
 * 3. Set Payment Terms = Immediate Payment
 * 4. Set Invoice Address = Name_EndUser#1
 * 5. Set Delivery Address = Name_EndUser#1
 * 6. Press SAVE button
 *
 * VIII. Verification:
 * 1. Error message: "The following fields are invalid: Payer"
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.4.4 - Verify the Deal Element.Payer is a required field', () => {
  ...
  test('CRM-2338_1.1.4.4: Verify the Deal Element.Payer is a required field', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step VII.2: Set Payer field = [BLANK]', async () => {
      console.log('Step VII.2: Clearing Payer field');
      await dealElementPage.clearPayerField();
      console.log('\u2713 Payer field cleared (set to blank)');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.2 - Payer cleared');
    });

    await test.step('Step VII.3: Set Payment Terms = Immediate Payment', async () => {
      console.log('Step VII.3: Setting Payment Terms = Immediate Payment');
      await dealElementPage.selectPaymentTerm('Immediate Payment');
      console.log('\u2713 Payment Terms = "Immediate Payment"');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.3 - Payment Terms set');
    });

    await test.step('Step VII.4: Set Invoice Address = Name_EndUser#1', async () => {
      console.log(`Step VII.4: Setting Invoice Address = "${name_EndUser1}"`);
      await dealElementPage.setInvoiceAddress(name_EndUser1);
      console.log(`\u2713 Invoice Address = "${name_EndUser1}"`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.4 - Invoice Address set');
    });

    await test.step('Step VII.5: Set Delivery Address = Name_EndUser#1', async () => {
      console.log(`Step VII.5: Setting Delivery Address = "${name_EndUser1}"`);
      await dealElementPage.setDeliveryAddress(name_EndUser1);
      console.log(`\u2713 Delivery Address = "${name_EndUser1}"`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.5 - Delivery Address set');
    });

    await test.step('Step VII.6: Press SAVE button', async () => {
      console.log('Step VII.6: Clicking SAVE button (expecting validation error)');
      await dealElementPage.clickSaveForValidation();
      console.log('\u2713 SAVE clicked');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.6 - After SAVE click');
    });
    ...
    await test.step('VIII.1: Verify error message "The following fields are invalid: Payer"', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      ...
      expect(errorMessage).toContain('Payer');
      console.log('\u2713 VIII.1: Error message confirmed - "The following fields are invalid: Payer"');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VIII.1 - Validation error verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.4.4 verification completed successfully');
      console.log(`   EndUser#1         : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1         : ${url_Opp1}`);
      console.log('   VIII.1: Error message "The following fields are invalid: Payer" confirmed');
