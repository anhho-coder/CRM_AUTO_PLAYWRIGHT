 ...
 * Test Case ID: CRM-2338_1.1.6.3
 *
 * Summary: Verify the Deal Element.Delivery Address getting from Opp.End User
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.6\.3:" --project=chromium
 *
 ...
 * 12. Close the browser
 *
 * III. Pre-condition#1 - Create EndUser#1 (Contact, no partner level)
 * IV.  Pre-condition#2 - Create Opp#1 (NO Reseller, NO Distributor)
 *
 * VII. Steps to reproduce:
 * 1. Press "DEAL ELEMENT" button
 *
 * VIII. Verification:
 * 1. Delivery Address field value = Name_EndUser#1
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.6.3 - Verify the Deal Element.Delivery Address getting from Opp.End User', () => {
  ...
  test('CRM-2338_1.1.6.3: Verify the Deal Element.Delivery Address getting from Opp.End User', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    // ==============================================================
    // III. PRE-CONDITION#1: Create EndUser#1
    // ==============================================================
    ...
    await test.step('Pre-condition III.2: Create EndUser#1', async () => {
      name_EndUser1  = `TEST-EndUser#1_${tcId}_${timestamp}`;
      email_EndUser1 = `Test-EndUser@EndUser-company${timestamp}.com`;
      console.log(`Step III.2: Creating EndUser#1 - Name: "${name_EndUser1}"`);
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'III.2 - EndUser#1 saved');
    });

    await test.step('Pre-condition III.3: Return to home page', async () => {
      console.log('Step III.3: Returning to home page');
    ...
    // ==============================================================
    // IV. PRE-CONDITION#2: Create Opp#1
    // ==============================================================

    await test.step('Pre-condition IV.1: Navigate to CRM and switch to list view', async () => {
      console.log(`\n=== IV. PRE-CONDITION#2: Create Opp#1 ===`);
    ...
    await test.step('Pre-condition IV.2: Click CREATE for Opp#1', async () => {
      console.log('Step IV.2: Clicking CREATE button');
      await opportunityPage.clickCreate();
      console.log('\u2713 Opp creation form opened');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.2 - Opp creation form');
    });

    await test.step('Pre-condition IV.3: Fill Opp#1 information', async () => {
      const oppName = `TEST Opp 1 ${tcId}`;
      console.log(`Step IV.3: Filling Opp#1 - Name: "${oppName}"`);
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.3 - Opp#1 fields filled');
    });

    await test.step('Pre-condition IV.4: Click CRM Developer tab and set Lead form', async () => {
      console.log('Step IV.4: Setting Lead form = Download Free Trial');
      await opportunityPage.clickCRMDeveloperTab();
      await opportunityPage.fillLeadForm('Download Free Trial');
      console.log('\u2713 Lead form = "Download Free Trial"');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.4 - CRM Developer tab');
    });

    await test.step('Pre-condition IV.5: Select Stage = NEW', async () => {
      console.log('Step IV.5: Selecting Stage = NEW');
      await opportunityPage.selectStage('New');
      console.log('\u2713 Stage = NEW');
    });

    await test.step('Pre-condition IV.6: Save Opp#1 and copy URL', async () => {
      console.log('Step IV.6: Saving Opp#1');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.6 - Opp#1 saved');
    });

    await test.step('Pre-condition IV.7: Refresh page to verify Contact field (up to 5 times)', async () => {
      console.log('Step IV.7: Waiting for Contact field to populate (up to 5 refreshes)');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.7 - After contact field refresh');
    });
    ...
    await test.step('VIII. Verification - Step 1: Verify Delivery Address = Name_EndUser#1', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      ...
      expect(deliveryAddressValue).toBe(name_EndUser1);
      console.log(`\u2713 VIII.1: Delivery Address = "${name_EndUser1}" - Delivery Address correctly set from End User`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VIII.1 - Delivery Address verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.6.3 verification completed successfully');
      console.log(`   EndUser#1         : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1         : ${url_Opp1}`);
      console.log('   VIII.1: Deal Element.Delivery Address = Name_EndUser#1 (from Opp.End User)');
