 ...
 * Test Case ID: CRM-2338_1.1.6.1
 *
 * Summary: Verify the Deal Element.Delivery Address getting from Opp.Distributor
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.1\.6\.1:" --project=chromium
 *
 ...
 *
 * VIII. Verification:
 * 1. Delivery Address field value = Name_Distributor#1
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.1.6.1 - Verify the Deal Element.Delivery Address getting from Opp.Distributor', () => {
  ...
  test('CRM-2338_1.1.6.1: Verify the Deal Element.Delivery Address getting from Opp.Distributor', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('VIII. Verification - Step 1: Verify Delivery Address = Name_Distributor#1', async () => {
      console.log(`\n=== VIII. VERIFICATION POINTS ===`);
      ...
      expect(deliveryAddressValue).toBe(name_Distributor1);
      console.log(`\u2713 VIII.1: Delivery Address = "${name_Distributor1}" - Delivery Address correctly set from Distributor`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VIII.1 - Delivery Address verified');
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.1.6.1 verification completed successfully');
      console.log(`   Distributor#1     : "${name_Distributor1}"`);
      console.log(`   Reseller#1        : "${name_Reseller1}"`);
      console.log(`   EndUser#1         : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1         : ${url_Opp1}`);
      console.log('   VIII.1: Deal Element.Delivery Address = Name_Distributor#1 (from Opp.Distributor)');
