 ...
 * Test Case ID: CRM-2338_1.3.1.3
 *
 * Summary: Verify the Deal Element.Total is updated automatically after adding Reseller
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.1\.3:" --project=chromium
 *
 ...
 *
 * VI. Pre-condition#4 - Create Opp#1 (WITH Reseller#1):
 ...
 * 3. Delete Reseller#1 via URL_Reseller#1
 * 4. Close all browsers
 */

test.describe('CRM-2338_1.3.1.3 - Verify the Deal Element Total is updated automatically after adding Reseller', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    await test.step('IX.3: Delete Reseller#1', async () => {
      if (!url_Reseller1) { console.log('  ⚠ No URL for Reseller#1 - skipping'); return; }
      console.log(`Step IX.3: Deleting Reseller#1 at ${url_Reseller1}`);
      await teardownPage.deleteContactByURL(url_Reseller1);
      console.log('✓ IX.3: Reseller#1 deleted');
    });
  ...
  test('CRM-2338_1.3.1.3: Verify the Deal Element Total is updated automatically after adding Reseller', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.3.1.3 verification completed successfully');
