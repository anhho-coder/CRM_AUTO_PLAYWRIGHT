 ...
 * Test Case ID: CRM-2338_1.3.1.2
 *
 * Summary: Verify the Deal Element.Total is updated automatically after adding Distributor
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.1\.2:" --project=chromium
 *
 ...
 *
 * VI. Pre-condition#4 - Create Opp#1 (WITH Distributor#1):
 ...
 * 4.  Click CRM Developer tab → Lead form = Download Free Trial
 * 5.  Stage = New → SAVE
 * 6.  Copy URL → URL_Opp#1
 * 7.  Refresh up to 5 times to verify Contact field
 *
...
test.describe('CRM-2338_1.3.1.2 - Verify the Deal Element Total is updated automatically after adding Distributor', () => {
  ...
  test('CRM-2338_1.3.1.2: Verify the Deal Element Total is updated automatically after adding Distributor', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition V.1: Navigate to Contacts', async () => {
      console.log(`\n=== V. PRE-CONDITION#3: Create EndUser#1 ===`);
      await homePage.navigateToContactsFromHome();
      await homePage.waitForPageReady();
      console.log('✓ Navigated to Contacts');
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.3.1.2 verification completed successfully');
