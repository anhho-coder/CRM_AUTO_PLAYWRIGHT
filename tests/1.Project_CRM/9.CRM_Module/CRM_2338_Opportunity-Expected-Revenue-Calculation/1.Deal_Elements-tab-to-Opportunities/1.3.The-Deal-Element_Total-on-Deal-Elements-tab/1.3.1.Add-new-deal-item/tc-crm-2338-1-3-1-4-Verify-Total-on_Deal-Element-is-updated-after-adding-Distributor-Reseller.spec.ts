 ...
 * Test Case ID: CRM-2338_1.3.1.4
 *
 * Summary: Verify the Deal Element.Total is updated automatically after adding Distributor and Reseller
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.1\.4:" --project=chromium
 *
 ...
 * 1.  Navigate to Contacts and click CREATE
 * 2.  Set Company = TRUE, Name = TEST-Distributor#1 + TC ID + timestamp
 * 3.  Set Email, Country = Chile, State = Antofagasta, Sales Team = BDEU, Salesperson = Thomas Semerich
 * 4.  Press SAVE, then EDIT
 * 5.  Click "Partner Assignation" tab
 * 6.  Set Activation Date = current date
 * 7.  Press "CHANGE LEVEL" → Target Level = Distributor, Comment = "Distributor + TC ID + timestamp"
 * 8.  Press SUBMIT, then SAVE
 * 9.  Copy URL → URL_Distributor#1
 *
 * IV. Pre-condition#2 - Create Reseller#1 (Bronze level):
 * 1.  Navigate to Contacts and click CREATE
 * 2.  Set Company = TRUE, Name = TEST-Reseller#1 + TC ID + timestamp
 * 3.  Set Email, Country = Chile, State = Antofagasta, Sales Team = BDEU, Salesperson = Thomas Semerich
 * 4.  Press SAVE, then EDIT
 * 5.  Click "Partner Assignation" tab
 * 6.  Set Activation Date = current date
 * 7.  Press "CHANGE LEVEL" → Target Level = Bronze, Comment = "Bronze level + TC ID + timestamp"
 * 8.  Set Level period end = current date
 * 9.  Press SUBMIT, then SAVE
 * 10. Copy URL → URL_Reseller#1
 *
 * VI. Pre-condition#4 - Create Opp#1 (WITH Distributor#1 AND Reseller#1):
 * 1.  Navigate to CRM list view → CREATE
 * 2.  Fill Opp name, Email = Email_EndUser#1, Company Name, Street, Country, State, Sales Team, Salesperson
 * 3.  Set Distributor = Distributor#1
 * 4.  Set Reseller = Reseller#1
 * 5.  Click CRM Developer tab → Lead form = Download Free Trial
 * 6.  Stage = New → SAVE
 * 7.  Copy URL → URL_Opp#1
 * 8.  Refresh up to 5 times to verify Contact field
 *
 * VII. Steps to reproduce:
 * 1. Press "DEAL ELEMENT" button
 * 2. Set Pricelist = Public Pricelist_USD (USD) ; Payment Term = Immediate Payment
 ...
 * 6. Sum number of "Subtotal After All Discounts" on every Order Line = Sum_Subtotal_After_All_Discounts
 *
 * VIII. Verification:
 ...
 * 3. Delete Distributor#1 via URL_Distributor#1
 * 4. Delete Reseller#1 via URL_Reseller#1
 * 5. Close all browsers
 */

test.describe('CRM-2338_1.3.1.4 - Verify the Deal Element Total is updated automatically after adding Distributor and Reseller', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    await test.step('IX.3: Delete Distributor#1', async () => {
      if (!url_Distributor1) { console.log('  ⚠ No URL for Distributor#1 - skipping'); return; }
      console.log(`Step IX.3: Deleting Distributor#1 at ${url_Distributor1}`);
      await teardownPage.deleteContactByURL(url_Distributor1);
      console.log('✓ IX.3: Distributor#1 deleted');
    });

    await test.step('IX.4: Delete Reseller#1', async () => {
      if (!url_Reseller1) { console.log('  ⚠ No URL for Reseller#1 - skipping'); return; }
      console.log(`Step IX.4: Deleting Reseller#1 at ${url_Reseller1}`);
      await teardownPage.deleteContactByURL(url_Reseller1);
      console.log('✓ IX.4: Reseller#1 deleted');
    });
  ...
  test('CRM-2338_1.3.1.4: Verify the Deal Element Total is updated automatically after adding Distributor and Reseller', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Pre-condition III.1: Navigate to Contacts', async () => {
      console.log(`\n=== III. PRE-CONDITION#1: Create Distributor#1 ===`);
      await homePage.navigateToContactsFromHome();
      await homePage.waitForPageReady();
      console.log('✓ Navigated to Contacts');
    ...
    await test.step('Pre-condition III.2: Create Distributor#1', async () => {
      name_Distributor1 = `TEST-Distributor#1_${tcId}_${timestamp}`;
      const email_Distributor1 = `Test-Distributor@Distributor-company${timestamp}.com`;
      const comment = `Distributor ${tcId} ${timestamp}`;
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'III.2 - Distributor#1 created');
    });

    await test.step('Pre-condition III.3: Save Distributor#1 and copy URL', async () => {
      console.log('Step III.3: Capturing URL_Distributor#1');
      await page.waitForFunction(
        () => { const m = window.location.href.match(/[?&#]id=(\d+)/); return m && m[1]; },
        { timeout: 30000 }
      ).catch(() => {});
    ...
    await test.step('Pre-condition IV.1: Navigate to Contacts', async () => {
    ...
    await test.step('Pre-condition IV.2: Create Reseller#1', async () => {
      name_Reseller1 = `TEST-Reseller#1_${tcId}_${timestamp}`;
      const email_Reseller1 = `Test-Reseller@Reseller-company${timestamp}.com`;
      const comment = `Bronze level ${tcId} ${timestamp}`;
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.2 - Reseller#1 created');
    });

    await test.step('Pre-condition IV.3: Save Reseller#1 and copy URL', async () => {
      console.log('Step IV.3: Capturing URL_Reseller#1');
      await page.waitForFunction(
        () => { const m = window.location.href.match(/[?&#]id=(\d+)/); return m && m[1]; },
        { timeout: 30000 }
      ).catch(() => {});
    ...
    await test.step('Step VII.2: Set Pricelist = Public Pricelist_USD and Payment Term = Immediate Payment', async () => {
      console.log('Step VII.2: Setting Pricelist and Payment Term (form already in edit mode)');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.2 - Pricelist and Payment Term set');
    });

    await test.step(`Step VII.3: Add product ${product_line1}, Ordered Qty = 1, UoM = Socket`, async () => {
      console.log(`Step VII.3: Adding product ${product_line1}, Qty=1, UoM=Socket`);
      await dealElementPage.addProductLine(product_line1, 1, 'Socket');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.3 - Product ${product_line1} added`);
    });
    ...
    await test.step(`Step VII.5: Add product ${product_line2}, Ordered Qty = 1, UoM = Socket`, async () => {
      console.log(`Step VII.5: Clicking EDIT then adding product ${product_line2}, Qty=1, UoM=Socket`);
      await dealElementPage.clickEdit();
      console.log('  ✓ Edit mode activated');
      await dealElementPage.addProductLine(product_line2, 1, 'Socket');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.5 - Product ${product_line2} added`);
    });

    await test.step('Step VII.6: Press SAVE and sum Sub Totals = Sum_Subtotal_After_All_Discounts', async () => {
      console.log('Step VII.6: Saving and reading Sub Totals');
      await dealElementPage.save();
      console.log('✓ Deal Element saved with both products');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.6 - Deal Element saved with 2 products');
    });
    ...
    await test.step('VIII.1: Verify Total at bottom of Order Lines = Sum_Subtotal_After_All_Discounts', async () => {
      ...
      const subTotal1   = await dealElementPage.getSubtotalAfterAllDiscountsForProduct(product_line1);
      const subTotal2   = await dealElementPage.getSubtotalAfterAllDiscountsForProduct(product_line2);
      const sumSubtotalAfterAllDiscounts = parseFloat((subTotal1 + subTotal2).toFixed(2));
      console.log(`  Subtotal After All Discounts ${product_line1} : ${subTotal1}`);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.3.1.4 verification completed successfully');
      console.log(`   Distributor#1 : "${name_Distributor1}"`);
      console.log(`   Reseller#1    : "${name_Reseller1}"`);
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1    : ${url_Opp1}`);
      console.log(`   VIII.1: Order Lines Total = Sum_Subtotal_After_All_Discounts (${product_line1} + ${product_line2}) \u2014 confirmed`);
