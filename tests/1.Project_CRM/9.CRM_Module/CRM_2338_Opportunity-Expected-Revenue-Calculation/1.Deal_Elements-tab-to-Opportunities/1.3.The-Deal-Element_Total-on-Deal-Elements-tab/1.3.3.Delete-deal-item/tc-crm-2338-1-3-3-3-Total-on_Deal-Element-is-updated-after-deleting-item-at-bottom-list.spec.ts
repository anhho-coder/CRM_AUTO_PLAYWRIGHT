 ...
 * Test Case ID: CRM-2338_1.3.3.3
 *
 * Summary: Verify the Deal Element.Total is updated automatically after deleting the deal item at bottom of list
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.3\.3:" --project=chromium
 *
 ...
 * 4. Press SAVE button
 * 5. Press EDIT button
 * 6. Press Trash Bin icon to remove product line named [A2146B]
 * 7. Press SAVE button
 *
 * VIII. Verification:
 * 1. There is no more product line named [A2146B]
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.3.3.3 - Verify the Deal Element Total is updated automatically after deleting the deal item at bottom of list', () => {
  ...
  test('CRM-2338_1.3.3.3: Verify the Deal Element Total is updated automatically after deleting the deal item at bottom of list', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6: Press Trash Bin icon to remove product line "${product_line3}"`, async () => {
      console.log(`Step VII.6: Removing product line ${product_line3} via Trash Bin`);
      await dealElementPage.removeProductLineByName(product_line3);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6 - Product line ${product_line3} removed`);
    });
    ...
    await test.step(`VIII.1: Verify product line "${product_line3}" is no longer present`, async () => {
      console.log(`\n=== VIII. VERIFICATION ===`);
      console.log(`Step VIII.1: Verifying product line ${product_line3} is removed`);
      const stillPresent = await dealElementPage.isProductInOrderLines(product_line3);
      expect(stillPresent, `VIII.1: Product line ${product_line3} should NOT be present after deletion`).toBe(false);
      console.log(`\u2713 VIII.1: Product line "${product_line3}" confirmed REMOVED from Order Lines`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VIII.1 - Deletion verified`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.3.3.3 verification completed successfully');
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1     : ${url_Opp1}`);
      console.log(`   VII.6: Product line "${product_line3}" (bottom) deleted via Trash Bin icon`);
      console.log(`   VIII.1: Product line "${product_line3}" no longer in Order Lines \u2014 confirmed`);
