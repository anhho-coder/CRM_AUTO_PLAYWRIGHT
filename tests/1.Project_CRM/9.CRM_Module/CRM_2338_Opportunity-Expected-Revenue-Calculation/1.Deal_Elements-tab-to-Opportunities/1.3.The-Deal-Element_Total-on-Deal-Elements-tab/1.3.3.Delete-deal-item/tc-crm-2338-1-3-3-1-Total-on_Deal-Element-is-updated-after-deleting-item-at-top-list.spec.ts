 ...
 * Test Case ID: CRM-2338_1.3.3.1
 *
 * Summary: Verify the Deal Element.Total is updated automatically after deleting the deal item at top of list
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.3\.1:" --project=chromium
 *
 ...
 * 7. Press SAVE button
 * 8. Sum number of "Subtotal After All Discounts" on every Order Line = Sum_Subtotal_After_All_Discounts
 *
 * VIII. Verification:
...
test.describe('CRM-2338_1.3.3.1 - Verify the Deal Element Total is updated automatically after deleting the deal item at top of list', () => {
  ...
  test('CRM-2338_1.3.3.1: Verify the Deal Element Total is updated automatically after deleting the deal item at top of list', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('VIII.1: Verify Total at bottom of Order Lines = Sum_Subtotal_After_All_Discounts', async () => {
      console.log(`\n=== VIII. VERIFICATION ===`);
      console.log('Step VIII.1: Reading Subtotal After All Discounts for remaining Order Lines');
      const subTotal2 = await dealElementPage.getSubtotalAfterAllDiscountsForProduct(product_line2);
      const subTotal3 = await dealElementPage.getSubtotalAfterAllDiscountsForProduct(product_line3);
      const sumSubtotalAfterAllDiscounts = parseFloat((subTotal2 + subTotal3).toFixed(2));
      console.log(`  Subtotal After All Discounts ${product_line2} : ${subTotal2}`);
      console.log(`  Subtotal After All Discounts ${product_line3} : ${subTotal3}`);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.3.3.1 verification completed successfully');
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1     : ${url_Opp1}`);
      console.log(`   VII.6: Product line "${product_line1}" deleted via Trash Bin icon`);
      console.log(`   VIII.1: Order Lines Total = Sum_Subtotal_After_All_Discounts (${product_line2} + ${product_line3}) \u2014 confirmed`);
