 ...
 * Test Case ID: CRM-2338_1.3.3.2
 *
 * Summary: Verify the Deal Element.Total is updated automatically after deleting the deal item at middle of list
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.3\.2:" --project=chromium
 *
 ...
 *    - Add [A2144B]: Ordered Qty = 1
 *    - Add [A2145B]: Ordered Qty = 1
 *    - Add [A2146B]: Ordered Qty = 1
 * 4. Press SAVE button
 * 5. Press EDIT button
 * 6. Press Trash Bin icon to remove product line named [A2145B]
 * 7. Press SAVE button
 *
 * VIII. Verification:
 * 1. There is no more product line named [A2145B]
 *
 * IX. Tear down:
...
test.describe('CRM-2338_1.3.3.2 - Verify the Deal Element Total is updated automatically after deleting the deal item at middle of list', () => {
  ...
  test('CRM-2338_1.3.3.2: Verify the Deal Element Total is updated automatically after deleting the deal item at middle of list', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    console.log(`  product_line3 : ${product_line3}`);
    ...
    await test.step(`Step VII.3a: Add product ${product_line1}`, async () => {
      console.log(`Step VII.3a: Adding product ${product_line1}`);
    ...
    await test.step(`Step VII.3b: Add product ${product_line2}`, async () => {
      console.log(`Step VII.3b: Adding product ${product_line2}`);
    ...
    await test.step(`Step VII.3c: Add product ${product_line3}`, async () => {
      console.log(`Step VII.3c: Adding product ${product_line3}`);
      await dealElementPage.addProductLine(product_line3, 1, 'Socket');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.3c - Product ${product_line3} added`);
    });
    ...
    await test.step(`Step VII.6: Press Trash Bin icon to remove product line "${product_line2}"`, async () => {
      console.log(`Step VII.6: Removing product line ${product_line2} via Trash Bin`);
      await dealElementPage.removeProductLineByName(product_line2);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6 - Product line ${product_line2} removed`);
    });

    await test.step('Step VII.7: Press SAVE button', async () => {
      console.log('Step VII.7: Clicking SAVE button');
      await dealElementPage.save();
      console.log('✓ Deal Element saved after deletion');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.7 - Deal Element saved after deletion');
    });
    ...
    await test.step(`VIII.1: Verify product line "${product_line2}" is no longer present`, async () => {
      console.log(`\n=== VIII. VERIFICATION ===`);
      console.log(`Step VIII.1: Verifying product line ${product_line2} is removed`);
      const stillPresent = await dealElementPage.isProductInOrderLines(product_line2);
      expect(stillPresent, `VIII.1: Product line ${product_line2} should NOT be present after deletion`).toBe(false);
      console.log(`\u2713 VIII.1: Product line "${product_line2}" confirmed REMOVED from Order Lines`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VIII.1 - Deletion verified`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-2338_1.3.3.2 verification completed successfully');
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1     : ${url_Opp1}`);
      console.log(`   VII.6: Product line "${product_line2}" (middle) deleted via Trash Bin icon`);
      console.log(`   VIII.1: Product line "${product_line2}" no longer in Order Lines \u2014 confirmed`);
