 ...
 * Test Case ID: CRM-2338_1.3.2.4
 *
 * Summary: Verify the Deal Element.Total is updated automatically after editing Unit of Measure
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.2\.4:" --project=chromium
 *
 ...
 * 8. Press SAVE button
 * 9. Sum number of "Subtotal After All Discounts" on every Order Line = Sum_Subtotal_After_All_Discounts
 *
 * VIII. Verification:
...
test.describe('CRM-2338_1.3.2.4 - Verify the Deal Element Total is updated automatically after editing Unit of Measure', () => {
  ...
  test('CRM-2338_1.3.2.4: Verify the Deal Element Total is updated automatically after editing Unit of Measure', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Unit of Measure to "Database"`, async () => {
      console.log(`Step VII.6-7: Changing Unit of Measure of ${product_line1} → "Database"`);
      await dealElementPage.changeUoMInRow(product_line1, 'Database');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Unit of Measure changed to Database`);
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.3.2.4 verification completed successfully');
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1     : ${url_Opp1}`);
      console.log(`   VIII.1: Order Lines Total = Sum_Subtotal_After_All_Discounts (${product_line1} UoM=Database + ${product_line2}) — confirmed`);
