 ...
 * Test Case ID: CRM-2338_1.3.2.3
 *
 * Summary: Verify the Deal Element.Total is updated automatically after editing Ordered Qty
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.2\.3:" --project=chromium
 *
...
test.describe('CRM-2338_1.3.2.3 - Verify the Deal Element Total is updated automatically after editing Ordered Qty', () => {
  ...
  test('CRM-2338_1.3.2.3: Verify the Deal Element Total is updated automatically after editing Ordered Qty', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Ordered Qty to 10`, async () => {
      console.log(`Step VII.6-7: Changing Ordered Qty of ${product_line1} → 10`);
      await dealElementPage.changeQtyInRow(product_line1, 10);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Ordered Qty changed to 10`);
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.3.2.3 verification completed successfully');
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1     : ${url_Opp1}`);
      console.log(`   VIII.1: Order Lines Total = Sum_Subtotal_After_All_Discounts (${product_line1} Qty=10 + ${product_line2}) — confirmed`);
