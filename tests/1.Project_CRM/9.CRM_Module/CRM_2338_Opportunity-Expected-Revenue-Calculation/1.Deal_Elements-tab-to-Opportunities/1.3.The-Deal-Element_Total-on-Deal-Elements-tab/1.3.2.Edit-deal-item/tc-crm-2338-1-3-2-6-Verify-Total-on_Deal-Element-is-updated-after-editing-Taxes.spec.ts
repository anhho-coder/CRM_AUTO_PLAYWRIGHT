 ...
 * Test Case ID: CRM-2338_1.3.2.6
 *
 * Summary: Verify the Deal Element.Total is updated automatically after editing Taxes
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.2\.6:" --project=chromium
 *
...
test.describe('CRM-2338_1.3.2.6 - Verify the Deal Element Total is updated automatically after editing Taxes', () => {
  ...
  test('CRM-2338_1.3.2.6: Verify the Deal Element Total is updated automatically after editing Taxes', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Taxes to "Tax 15.00%"`, async () => {
      console.log(`Step VII.6-7: Changing Taxes of ${product_line1} → "Tax 15.00%"`);
      await dealElementPage.changeTaxInRow(product_line1, 'Tax 15.00%');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Taxes changed to Tax 15.00%`);
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.3.2.6 verification completed successfully');
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1     : ${url_Opp1}`);
      console.log(`   VIII.1: Order Lines Total = Sum_Subtotal_After_All_Discounts (${product_line1} Taxes=Tax15% + ${product_line2}) — confirmed`);
