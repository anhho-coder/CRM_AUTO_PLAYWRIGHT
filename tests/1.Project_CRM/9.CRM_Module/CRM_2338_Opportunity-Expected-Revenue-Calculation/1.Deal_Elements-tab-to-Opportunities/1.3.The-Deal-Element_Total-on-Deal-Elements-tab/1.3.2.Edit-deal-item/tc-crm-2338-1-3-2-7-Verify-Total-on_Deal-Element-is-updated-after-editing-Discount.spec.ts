 ...
 * Test Case ID: CRM-2338_1.3.2.7
 *
 * Summary: Verify the Deal Element.Total is updated automatically after editing Discount %
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.2\.7:" --project=chromium
 *
 ...
 * 7. Change the value of "Discount %" field to "10"
 * 8. Press SAVE button
...
test.describe('CRM-2338_1.3.2.7 - Verify the Deal Element Total is updated automatically after editing Discount %', () => {
  ...
  test('CRM-2338_1.3.2.7: Verify the Deal Element Total is updated automatically after editing Discount %', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Discount % to 10`, async () => {
      console.log(`Step VII.6-7: Changing Discount % of ${product_line1} → 10`);
      await dealElementPage.changeDiscountInRow(product_line1, 10);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Discount % changed to 10`);
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.3.2.7 verification completed successfully');
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1     : ${url_Opp1}`);
      console.log(`   VIII.1: Order Lines Total = Sum_Subtotal_After_All_Discounts (${product_line1} Discount%=10 + ${product_line2}) — confirmed`);
