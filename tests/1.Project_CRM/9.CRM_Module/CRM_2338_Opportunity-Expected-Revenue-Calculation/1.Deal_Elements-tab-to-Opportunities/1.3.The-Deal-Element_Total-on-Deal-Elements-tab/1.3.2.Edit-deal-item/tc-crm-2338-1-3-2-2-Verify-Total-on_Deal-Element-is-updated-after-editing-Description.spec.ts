 ...
 * Test Case ID: CRM-2338_1.3.2.2
 *
 * Summary: Verify the Deal Element.Total is updated automatically after editing description
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.2\.2:" --project=chromium
 *
...
test.describe('CRM-2338_1.3.2.2 - Verify the Deal Element Total is updated automatically after editing description', () => {
  ...
  test('CRM-2338_1.3.2.2: Verify the Deal Element Total is updated automatically after editing description', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Description to "TEST"`, async () => {
      console.log(`Step VII.6-7: Changing description of ${product_line1} → "TEST"`);
      await dealElementPage.changeDescriptionInRow(product_line1, 'TEST');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Description changed to TEST`);
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.3.2.2 verification completed successfully');
