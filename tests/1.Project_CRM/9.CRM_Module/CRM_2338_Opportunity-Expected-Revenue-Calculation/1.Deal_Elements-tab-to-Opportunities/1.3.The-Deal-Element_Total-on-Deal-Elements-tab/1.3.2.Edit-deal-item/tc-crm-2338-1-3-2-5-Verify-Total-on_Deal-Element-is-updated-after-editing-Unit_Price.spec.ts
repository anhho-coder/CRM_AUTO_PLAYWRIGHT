 ...
 * Test Case ID: CRM-2338_1.3.2.5
 *
 * Summary: Verify the Deal Element.Total is updated automatically after editing Unit Price
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_1\.3\.2\.5:" --project=chromium
 *
 ...
 * 7. Change the value of "Unit Price" field to "100"
 * 8. Press SAVE button
...
test.describe('CRM-2338_1.3.2.5 - Verify the Deal Element Total is updated automatically after editing Unit Price', () => {
  ...
  test('CRM-2338_1.3.2.5: Verify the Deal Element Total is updated automatically after editing Unit Price', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.6-7: Select ${product_line1} row and change Unit Price to 100`, async () => {
      console.log(`Step VII.6-7: Changing Unit Price of ${product_line1} → 100`);
      await dealElementPage.changeUnitPriceInRow(product_line1, 100);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `VII.6-7 - Unit Price changed to 100`);
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_1.3.2.5 verification completed successfully');
      console.log(`   EndUser#1     : "${name_EndUser1}"`);
      console.log(`   URL_Opp#1     : ${url_Opp1}`);
      console.log(`   VIII.1: Order Lines Total = Sum_Subtotal_After_All_Discounts (${product_line1} UnitPrice=100 + ${product_line2}) — confirmed`);
