 ...
 * Test Case ID: CRM-2338_2.1.4
 *
 * Summary: Verify the "Expected revenue after probability" on Opportunity is re-calculating correctly
 *          after updating Distributor and Reseller
 *
 * Command to run:
 * npx playwright test --grep "CRM-2338_2\.1\.4:" --project=chromium
 *
 ...
 *     Level period end = current date
 * 8.  Press SUBMIT, then SAVE
 * 9.  Copy URL → URL_Reseller#1
 *
 ...
 *     - Set Distributor = Distributor#1
 *     - Set Reseller = Reseller#1
 *
 ...
 * 5. Get Total at bottom of Order Lines tab = Total_Subtotal_After_All_Discounts_Opp#1
 * 6. Press the Link of Opportunity to back to Opportunity screen
 * 7. Get Expected Revenue = Expected_Revenue_Opp#1
 * 8. Get Expected After Probability = Expected_Revenue_After_Probability_Opp#1
 *
 * VIII. Verification:
...
test.describe('CRM-2338_2.1.4 - Verify the Expected revenue after probability on Opportunity is re-calculating correctly after updating Distributor and Reseller', () => {
  ...
  test('CRM-2338_2.1.4: Verify the Expected revenue after probability on Opportunity is re-calculating correctly after updating Distributor and Reseller', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step(`Step VII.3: Add products ${product_line1} and ${product_line2} to Order Lines`, async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.3 - Both products added');
    });
    ...
    await test.step('Step VII.5: Get Total_Subtotal_After_All_Discounts_Opp#1', async () => {
      console.log('Step VII.5: Reading Total at bottom of Order Lines tab');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.5 - Total read');
    });

    await test.step('Step VII.6: Press the Link of Opportunity to back to Opportunity screen', async () => {
      console.log('Step VII.6: Navigating back to Opportunity');
      await dealElementPage.clickBackToOpportunity();
      console.log('✓ Returned to Opportunity screen');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.6 - Back to Opportunity');
    });

    await test.step('Step VII.7: Get Expected Revenue (Expected_Revenue_Opp#1)', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.7 - Expected Revenue read');
    });

    await test.step('Step VII.8: Get Expected After Probability (Expected_Revenue_After_Probability_Opp#1)', async () => {
      console.log('Step VII.8: Reading Expected After Probability and Probability from Opportunity');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'VII.8 - Expected After Probability read');
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-2338_2.1.4 verification completed successfully');
