...
/**
 * ============================================================================================
 *  pre-sale-7.1.2.1 - REQUEST SE SUPPORT is disabled when Expected Revenue < $100
 * --------------------------------------------------------------------------------------------
 *  Test Case ID:    pre-sale-7.1.2.1
 *  Jira:            (none - authored from an inline manual TC)
 *  Automation-Type: new
 *  Automation-Date: 2026-07-28
 * --------------------------------------------------------------------------------------------
 *  Summary:
 *    Verify that when the Opportunity's Expected Revenue is below the $100 threshold, the
 *    "REQUEST SE SUPPORT" button on the Opp form is present but DISABLED (greyed / not clickable),
 *    so the Salesperson cannot request SE support for a low-value Opp.
 *
 *  Command to run:
 *    npx playwright test --grep "pre-sale-7\.1\.2\.1:" --project=chromium
 * ============================================================================================
 *
 ...
 *   3. Set Expected Revenue = $50 (a value below the $100 threshold)
 *   4. Press "SAVE" button
 *   5. Verify the "REQUEST SE SUPPORT" button
 *
 *  Verification Point:
 *   5. The "REQUEST SE SUPPORT" button is present but DISABLED (greyed / not clickable),
 *      because the Expected Revenue ($50) is below the $100 threshold.
 */

// Cleanup toggle: best-effort delete of the created Opportunity on teardown (true = skip).
const SKIP_CLEANUP_OPP = false;

const TC_ID = 'pre-sale-7.1.2.1';
const EXPECTED_REVENUE_BELOW_THRESHOLD = '50'; // < $100

test.describe('pre-sale-7.1.2.1 - REQUEST SE SUPPORT disabled when Expected Revenue < $100', () => {
  ...
  test('pre-sale-7.1.2.1: Verify REQUEST SE SUPPORT is disabled when Expected Revenue is below $100', async ({ page }, testInfo) => {
    ...
    await test.step(`Step 3: Set Expected Revenue = $${EXPECTED_REVENUE_BELOW_THRESHOLD} (below the $100 threshold)`, async () => {
      console.log(`Step 3: Setting Expected Revenue to $${EXPECTED_REVENUE_BELOW_THRESHOLD}`);
      await opportunityPage.fillExpectedRevenueDeal(EXPECTED_REVENUE_BELOW_THRESHOLD);
      console.log('✓ Expected Revenue entered');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - Expected Revenue set below $100').catch(() => {});
    });
    ...
    await test.step('Step 5: Verify the "REQUEST SE SUPPORT" button', async () => {
      ...
      // ---- VERIFY block ----
      console.log('==================== VERIFY ====================');
      console.log(`Expected: Expected Revenue < 100 AND REQUEST SE SUPPORT present but DISABLED`);
      ...
      // VP5a: the Expected Revenue we set is below the $100 threshold.
      expect(revenue, `VP5a: Expected Revenue should be below $100 (was $${revenue})`).toBeLessThan(100);
      // VP5b: the button is still present on the form (not removed) ...
      expect(seState.present, 'VP5b: the REQUEST SE SUPPORT button should still be present on the form').toBeTruthy();
      // VP5c: ... but DISABLED (greyed / not clickable) because Expected Revenue < $100.
      expect(seState.disabled, 'VP5c: the REQUEST SE SUPPORT button should be DISABLED when Expected Revenue < $100').toBeTruthy();
      console.log('✅ REQUEST SE SUPPORT is disabled while Expected Revenue is below the $100 threshold');
    });
  });
});
