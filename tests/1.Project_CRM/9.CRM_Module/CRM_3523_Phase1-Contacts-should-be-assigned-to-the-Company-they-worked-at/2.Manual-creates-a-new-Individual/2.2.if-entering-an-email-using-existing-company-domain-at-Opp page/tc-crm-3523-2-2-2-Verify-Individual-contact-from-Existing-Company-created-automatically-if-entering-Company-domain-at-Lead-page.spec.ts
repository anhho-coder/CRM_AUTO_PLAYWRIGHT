 ...
 *   7.  Mouse over Company field → get URL_EndUser#1
 *
 * V. Verification:
...
test.describe('CRM-3523_2.2.2 - Verify an Individual contact will be created automatically if entering an email using existing company domain at Lead page', () => {
  ...
  test('CRM-3523_2.2.2: Verify an Individual contact will be created automatically if entering an email using existing company domain at Lead page', async ({ page }, testInfo) => {
    ...
    await test.step('Step IV.6: Refresh page to verify Company field (up to 5 times, max 5 min)', async () => {
      console.log('Step IV.6: Waiting for Company field to populate (up to 5 refreshes)');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.6 - After company field refresh');
    });

    await test.step('Step IV.7: Get URL of Company field (URL_EndUser#1)', async () => {
      console.log('Step IV.7: Getting URL from Company field hyperlink');
      const companyHref = await opportunityPage.getCompanyFieldUrl();
      if (companyHref) {
        console.log(`\u2713 Company field URL = ${companyHref}`);
      } else {
        console.log('  \u26a0 Could not retrieve Company field URL');
      }
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'IV.7 - Company field URL retrieved');
    });
    ...
    await test.step('Step V.1: Verify Company Name field = Company_Name_Lead#1', async () => {
      console.log(`\n=== V. VERIFICATION ===`);
      ...
      expect(companyNameFieldValue, `V.1: Company Name field ("${companyNameFieldValue}") should equal "${company_Name_Lead1}"`).toContain(company_Name_Lead1);
      console.log(`\u2713 V.1: Company Name field = "${company_Name_Lead1}" - confirmed`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'V.1 - Company Name field verified');
    });

    await test.step('Step V.2: Verify Company field is a Hyperlink with value = Company_Name_Lead#1', async () => {
      ...
      expect(companyFieldText.trim(), `V.2: Company field text ("${companyFieldText.trim()}") should equal "${company_Name_Lead1}"`).toContain(company_Name_Lead1);
...
/**
 * CRM-3523 - Phase 1: Contacts should be assigned to the Company they worked at
 * Test Case ID: CRM-3523_1.1.1
 *
 * Summary: Verify a Company contact will be created automatically if entering
 *          a new Company domain at Opp page
 *
 * Command to run:
 * npx playwright test --grep "CRM-3523_1\.1\.1:" --project=chromium
 *
 ...
 *       - Email = Email_Contact#1 (Test-company@company{timestamp}.com)
...
test.describe('CRM-3523_1.1.1 - Verify a Company contact will be created automatically if entering a new Company domain at Opp page', () => {
  ...
  test('CRM-3523_1.1.1: Verify a Company contact will be created automatically if entering a new Company domain at Opp page', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    const tcId      = 'CRM-3523_1.1.1';
    const timestamp = CommonUtils.generateTimestamp();

    email_Contact1 = `Test-company@company${timestamp}.com`;
    ...
    await test.step('Step III.3: Fill Opp#1 information', async () => {
      ...
      console.log(`  ✓ Email (Email_Contact#1): "${email_Contact1}"`);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-3523_1.1.1 verification completed successfully');
      console.log(`   Email_Contact#1    : "${email_Contact1}"`);
      console.log(`   Company_Name_Opp#1 : "${company_Name_Opp1}"`);
      console.log(`   URL_Opp#1          : ${url_Opp1}`);
      console.log(`   URL_EndUser#1      : ${url_EndUser1}`);
