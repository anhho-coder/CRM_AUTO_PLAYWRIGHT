...
/**
 * Apply an Automatically-Applied promotion to a qualifying sale order.
 * Test Case ID: CRM-10780_2.1.1.1   (Jira: CRM-10861 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Salesperson opens an Opportunity, goes to its Deal Element (sale.order), adds a product
 *          (qualifying order) and the Automatically-Applied "Promotion A" (10% on order) is applied -
 *          the order total is reduced by the promotion discount.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_2\.1\.1\.1:" --project=chromium
 *
 * Source manual TC (Jira CRM-10861)
 ...
 *   (the packaged create flow validated by CRM-10844).
 ...
 *   Order Lines ("X% discount on total amount") and the order total is reduced by the discount. The
 *   Promotion field must be set while the form is in edit mode (right after adding the product line).
...
test.describe('CRM-10780_2.1.1.1 - Apply promotion to a qualifying sale order', () => {
  let promoName = '';
  let promoUrl = '';
  let urlContact = '';
  let urlOpp = '';
  ...
  test('CRM-10780_2.1.1.1: Apply promotion to a qualifying sale order', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition A: Sales Manager creates Promotion A (Automatically Applied, 10% on order)', async () => {
      ...
      await loginPage.login(MANAGER.username, MANAGER.password, 120000);
      await loginPage.dismissLocationPermissionDialog();
      const created = await promotionPage.createPromotion({ namePrefix: 'TEST- Order - 10% ' });
    ...
    await test.step('Pre-condition B.1: Create EndUser Contact (with Pricelist)', async () => {
      ...
      contactName = `TEST-EndUser_CRM-10861_${timestamp}`;
      contactEmail = `test-enduser-crm10861-${timestamp}@enduser-company.com`;
    ...
    await test.step('Pre-condition B.2: Create a qualifying Opportunity (using the EndUser email)', async () => {
      ...
      const oppName = `TEST Opp CRM-10861 ${timestamp}`;
      await opportunityPage.fillOpportunityName(oppName);
      await opportunityPage.fillEmail(contactEmail);
      await opportunityPage.fillCompanyName('Company CRM-10861');
      ...
      // These three are CRM-2338-specific data-hygiene extras (not required by CRM-10861) and some are
    ...
    // ============================================================
    // Steps (mirrors Jira CRM-10861 manual steps 1-4)
    // ============================================================
    ...
    await test.step('Step 4: Apply promotion A', async () => {
      ...
      //  - Promotion A appears as a discount line in Order Lines.
