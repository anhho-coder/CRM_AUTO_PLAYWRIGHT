...
/**
 * Apply a "Discount Apply On = On Cheapest Product" promotion to a 2-product order.
 * Test Case ID: CRM-10780_2.1.1.7   (Jira: CRM-10867 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Salesperson opens an Opportunity, goes to its Deal Element (sale.order), adds two
 *          products (#1 and #2) and applies "Promotion A" (10% On Cheapest Product). The promotion is
 *          applied to the cheapest product between #1 and #2 - a discount line is added and the order
 *          total is reduced correctly.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_2\.1\.1\.7:" --project=chromium
 *
 * Source manual TC (Jira CRM-10867)
 ...
 *       _ Discount Apply On = On Cheapest Product
 *
 *   Steps:
 *     1. Open CRM module
 ...
 *     Promotion A is applied successfully to cheapest product between Product #1 and Product #2
 ...
 *   with { discountApplyOn: 'On Cheapest Product' } (10% Percentage is the default Apply Discount).
 * - "Apply promotion A" (step 4) on this Nakivo Deal Element = set the editable "Promotion" field
 *   (promotion_id Many2one) to Promotion A, then SAVE. With "On Cheapest Product" the promo is added as
 *   a discount line in Order Lines applied to the cheapest of the two products, and the order total is
 *   reduced by the discount. The Promotion field must be set while the form is in edit mode (right
 *   after adding the product lines).
 * - This is a POSITIVE case: step 3 adds TWO products ([A2144B] and [A2145B]) so there is a "cheapest"
 *   one to discount; step 4 asserts a discount line was added AND the order Total dropped.
...
test.describe('CRM-10780_2.1.1.7 - Apply promotion to the cheapest product', () => {
  let promoName = '';
  let promoUrl = '';
  let urlContact = '';
  let urlOpp = '';
  ...
  test('CRM-10780_2.1.1.7: Verify discount applied to cheapest product', async ({ page }, testInfo) => {
    ...
    //   Apply Discount = Percentage - 10% ; Discount Apply On = On Cheapest Product
    // ============================================================
    await test.step('Pre-condition A: Sales Manager creates Promotion A (10% On Cheapest Product)', async () => {
      await loginPage.navigateTo(baseUrl);
      await loginPage.login(MANAGER.username, MANAGER.password, 120000);
      await loginPage.dismissLocationPermissionDialog();
      const created = await promotionPage.createPromotion({ namePrefix: 'TEST- 2-1-1-7 ', discountApplyOn: 'On Cheapest Product' });
    ...
    await test.step('Pre-condition B.1: Create EndUser Contact (with Pricelist)', async () => {
      ...
      contactName = `TEST-EndUser_CRM-10867_${timestamp}`;
      contactEmail = `test-enduser-crm10867-${timestamp}@enduser-company.com`;
    ...
    await test.step('Pre-condition B.2: Create a qualifying Opportunity (using the EndUser email)', async () => {
      ...
      const oppName = `TEST Opp CRM-10867 ${timestamp}`;
      await opportunityPage.fillOpportunityName(oppName);
      await opportunityPage.fillEmail(contactEmail);
      await opportunityPage.fillCompanyName('Company CRM-10867');
      ...
      // These three are CRM-2338-specific data-hygiene extras (not required by CRM-10867) and some are
    ...
    // ============================================================
    // Steps (mirrors Jira CRM-10867 manual steps 1-4)
    // ============================================================
    ...
    await test.step('Step 3: Select product #1 and product #2', async () => {
      ...
      // Two products so there IS a "cheapest" one for the promotion to apply to.
