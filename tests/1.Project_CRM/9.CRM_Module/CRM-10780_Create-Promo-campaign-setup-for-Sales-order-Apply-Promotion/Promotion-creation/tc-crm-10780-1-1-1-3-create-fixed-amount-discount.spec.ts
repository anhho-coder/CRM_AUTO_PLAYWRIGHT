...
/**
 * Create a Promotion Program with a fixed discount amount (500$).
 * Test Case ID: CRM-10780_1.1.1.3   (Jira: CRM-10846 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Sales Manager creates a Promotion Program (Sales > Products > Promotion Programs) with
 *          Promo Code Usage = Automatically Applied, Reward = Discount, Apply Discount = Fixed Amount = 500$.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_1\.1\.1\.3:" --project=chromium
 *
 * Source manual TC (Jira CRM-10846)
 ...
 *     Record created successfully, discount_fixed_amount=500
 *
 * Design notes:
...
test.describe('CRM-10780_1.1.1.3 - Create promotion with fixed discount amount', () => {
  let createdUrl = '';
  let promoName = '';
  ...
  test('CRM-10780_1.1.1.3: Create promotion with fixed discount amount', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Create new Promotion with Fixed Amount discount = 500$', async () => {
      await promotionPage.clickCreate();
      promoName = promotionPage.generatePromotionName('TEST- Fixed 500 ');
      ...
      await promotionPage.selectApplyDiscount('Fixed Amount');
      await promotionPage.setDiscountFixedAmount(500);
      createdUrl = await promotionPage.save();
      console.log('Saved Promotion "' + promoName + '" at ' + createdUrl);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10846 - Promotion created');
