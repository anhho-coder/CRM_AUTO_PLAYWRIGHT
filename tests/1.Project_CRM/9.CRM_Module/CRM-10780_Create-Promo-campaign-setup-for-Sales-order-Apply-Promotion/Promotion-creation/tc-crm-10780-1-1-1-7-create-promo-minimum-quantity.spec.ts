...
/**
 * Create a Promotion Program with a minimum-quantity condition (auto-applied, 10% percentage discount on order, min quantity 5).
 * Test Case ID: CRM-10780_1.1.1.7   (Jira: CRM-10850 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Sales Manager creates a Promotion Program (Sales > Products > Promotion Programs) with
 *          Promo Code Usage = Automatically Applied, Reward = Discount, Apply Discount = 10% Percentage,
 *          Discount Apply On = On Order, and a minimum Quantity condition of 5.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_1\.1\.1\.7:" --project=chromium
 *
 * Source manual TC (Jira CRM-10850)
 ...
 *     Record created successfully, Promotion only applies to order with minimum quantity = 5
 *
 * Design notes:
...
test.describe('CRM-10780_1.1.1.7 - Create promotion with minimum quantity condition', () => {
  let createdUrl = '';
  let promoName = '';
  ...
  test('CRM-10780_1.1.1.7: Create promotion with minimum quantity condition', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Create new Promotion with auto-applied 10% percentage discount on order and min quantity 5', async () => {
      await promotionPage.clickCreate();
      promoName = promotionPage.generatePromotionName('TEST- MinQty 5 ');
      ...
      await promotionPage.setRuleMinQuantity(5);
      createdUrl = await promotionPage.save();
      console.log('Saved Promotion "' + promoName + '" at ' + createdUrl);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10850 - Promotion created');
