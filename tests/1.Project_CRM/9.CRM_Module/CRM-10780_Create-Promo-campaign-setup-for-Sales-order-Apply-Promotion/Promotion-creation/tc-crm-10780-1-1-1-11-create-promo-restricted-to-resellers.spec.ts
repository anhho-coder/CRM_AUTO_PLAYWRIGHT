...
/**
 * Create an auto-applied % discount promotion restricted to Resellers.
 * Test Case ID: CRM-10780_1.1.1.11   (Jira: CRM-10854 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Sales Manager creates a Promotion Program (Sales > Products > Promotion Programs) with
 *          Automatically Applied, Reward = Discount, Apply Discount = 10% (Percentage), Discount Apply On = On Order, For Reseller = true.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_1\.1\.1\.11:" --project=chromium
 *
 * Source manual TC (Jira CRM-10854)
 ...
 *     Record created successfully, Promotion only applies for Reseller
 *
 * Design notes:
...
test.describe('CRM-10780_1.1.1.11 - Create promotion restricted to Resellers', () => {
  let createdUrl = '';
  let promoName = '';
  ...
  test('CRM-10780_1.1.1.11: Create promotion restricted to Resellers', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Create new Promotion with Promo Code Usage = Automatically Applied, Reward = Discount, Apply Discount = Percentage - 10%, Discount Apply On = On Order, For Reseller = true', async () => {
      await promotionPage.clickCreate();
      promoName = promotionPage.generatePromotionName('TEST- Reseller 10% ');
      ...
      // TODO: set For Reseller = true - no PromotionPage method yet
      createdUrl = await promotionPage.save();
      console.log('Saved Promotion "' + promoName + '" at ' + createdUrl);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10854 - Promotion created');
