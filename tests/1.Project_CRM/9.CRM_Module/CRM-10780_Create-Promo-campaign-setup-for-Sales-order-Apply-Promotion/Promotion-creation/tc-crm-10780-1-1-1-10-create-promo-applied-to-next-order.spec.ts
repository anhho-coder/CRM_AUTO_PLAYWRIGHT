...
/**
 * Create auto-applied promotion with 10% discount on order, applicable only to the next order.
 * Test Case ID: CRM-10780_1.1.1.10   (Jira: CRM-10853 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Sales Manager creates a Promotion Program (Sales > Products > Promotion Programs) with
 *          Automatically Applied promo code usage, Reward = Discount, Apply Discount = 10% (Percentage),
 *          Discount Apply On = On Order, and Applicability = Apply On Next Order.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_1\.1\.1\.10:" --project=chromium
 *
 * Source manual TC (Jira CRM-10853)
 ...
 *        - Applicability = Apply On Next Order
 *
 *   Expected Result (step 3):
 *     Record created successfully, Promotion only applies to next order
 *
 * Design notes:
...
test.describe('CRM-10780_1.1.1.10 - Create promotion applied to next order', () => {
  let createdUrl = '';
  let promoName = '';
  ...
  test('CRM-10780_1.1.1.10: Create promotion applied to next order', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Create new Promotion with Automatically Applied, Reward = Discount, Apply Discount = 10% Percentage, Discount Apply On = On Order, Applicability = Apply On Next Order', async () => {
      await promotionPage.clickCreate();
      promoName = promotionPage.generatePromotionName('TEST- Next Order 10% ');
      ...
      await promotionPage.selectApplicability('Apply On Next Order');
      createdUrl = await promotionPage.save();
      console.log('Saved Promotion "' + promoName + '" at ' + createdUrl);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10853 - Promotion created');
