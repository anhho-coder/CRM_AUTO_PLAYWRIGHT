...
/**
 * Create auto-applied promotion with a Free Product reward.
 * Test Case ID: CRM-10780_1.1.1.13   (Jira: CRM-10856 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Sales Manager creates a Promotion Program (Sales > Products > Promotion Programs) with
 *          Automatically Applied, Reward = Free Product, Apply Discount = 10% (Percentage), Discount Apply On = On Order, Free Product = product 1, Product quantity = 3.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_1\.1\.1\.13:" --project=chromium
 *
 * Source manual TC (Jira CRM-10856)
 ...
 *     Record created successfully, free 3 x product 1 will be added to the order
 *
 * Design notes:
...
test.describe('CRM-10780_1.1.1.13 - Create promotion with Free Product reward', () => {
  let createdUrl = '';
  let promoName = '';
  ...
  test('CRM-10780_1.1.1.13: Create promotion with Free Product reward', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Create new Promotion with Reward = Free Product, Apply Discount = 10% (Percentage), Discount Apply On = On Order, Free Product = product 1, Product quantity = 3', async () => {
      await promotionPage.clickCreate();
      promoName = promotionPage.generatePromotionName('TEST- Free Product  ');
      await promotionPage.setName(promoName);
      await promotionPage.selectPromoCodeUsage('Automatically Applied');
      await promotionPage.selectReward('Free Product');
      await promotionPage.setFreeProduct('[A2144B]');
      await promotionPage.setRewardProductQuantity(3);
      createdUrl = await promotionPage.save();
      console.log('Saved Promotion "' + promoName + '" at ' + createdUrl);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10856 - Promotion created');
