...
/**
 * Create promotion requiring a promo code (Use a Code, % discount on entire order).
 * Test Case ID: CRM-10780_1.1.1.2   (Jira: CRM-10845 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-22
 *
 * Summary: A Sales Manager creates a Promotion Program (Sales > Products > Promotion Programs) with
 *          Promo Code Usage = Use a Code (code "Save10"), Reward = Discount, Apply Discount = 10% (Percentage), Discount Apply On = On Order.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10780_1\.1\.1\.2:" --project=chromium
 *
 * Source manual TC (Jira CRM-10845)
 ...
 *     Record created successfully, promo_code="SAVE10" saved
 *
 * Design notes:
...
test.describe('CRM-10780_1.1.1.2 - Create promotion requiring promo code', () => {
  let createdUrl = '';
  let promoName = '';
  ...
  test('CRM-10780_1.1.1.2: Create promotion requiring promo code', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Create new Promotion with Promo Code Usage = Use a Code (code Save10), Reward = Discount, Apply Discount = Percentage 10%, Discount Apply On = On Order', async () => {
      await promotionPage.clickCreate();
      promoName = promotionPage.generatePromotionName('TEST- Save10 Order 10% ');
      await promotionPage.setName(promoName);
      await promotionPage.selectPromoCodeUsage('Use a code');
      await promotionPage.setPromoCode('Save10');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10845 - Promotion created');
