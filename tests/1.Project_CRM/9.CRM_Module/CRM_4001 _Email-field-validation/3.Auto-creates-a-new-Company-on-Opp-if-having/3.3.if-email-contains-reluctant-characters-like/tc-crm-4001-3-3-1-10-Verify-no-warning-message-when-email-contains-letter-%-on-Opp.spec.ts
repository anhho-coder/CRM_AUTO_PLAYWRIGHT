 ...
 * Test Case ID: CRM-4001_3.3.1.10
 * Automation-Type: refactored
 * Automation-Date: 2026-06-26
 *
 * Summary: Verify a valid email whose local part contains "%" does NOT trigger the invalid-email
 *          warning log note when auto-creating a new Company on Opp. "%" is a legal local-part
 *          character (e.g. user+tag@gmail.com), so no warning should be posted to the Log.
 *
 * Command to run:
 * npx playwright test --grep "CRM-4001_3\.3\.1\.10:" --project=chromium
 *
 * Pre-condition:
 ...
 *     - Opp name textbox = TEST Opp 1 CRM-4001_3.3.1.10
 ...
 *       email validator correctly accepts "%" (and * & ! # $ % ^ _ =), so no warning is posted.
 */

const SKIP_CLEANUP = false; // true = skip deleting the Opp created by this test

test.describe('CRM-4001_3.3.1.10 - Verify a valid email containing "%" posts no warning on Opp (auto)', () => {

  const tcId    = 'CRM-4001_3.3.1.10';
  const oppName = `TEST Opp 1 ${tcId}`;

  // Email with percent character (Email_Contact#1) - a VALID local-part character
  ...
  test('CRM-4001_3.3.1.10: Verify a valid email containing "%" posts no warning on Opp (auto)', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-4001_3.3.1.10 verification completed successfully');
      console.log(`   TC ID          : ${tcId}`);
      console.log(`   Opp name       : "${oppName}"`);
      console.log(`   Email_Contact#1: "${email_Contact1}"`);
