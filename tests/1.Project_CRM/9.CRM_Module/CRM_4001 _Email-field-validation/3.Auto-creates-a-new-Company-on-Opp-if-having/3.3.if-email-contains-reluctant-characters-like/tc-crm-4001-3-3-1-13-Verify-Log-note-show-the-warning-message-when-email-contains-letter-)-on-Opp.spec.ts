 ...
 * Test Case ID: CRM-4001_3.3.1.13
 *
 ...
 *
 * Command to run:
 * npx playwright test --grep "CRM-4001_3\.3\.1\.13:" --project=chromium
 *
 * Pre-condition:
 ...
 *     - Opp name textbox = TEST Opp 1 CRM-4001_3.3.1.13
...
test.describe('CRM-4001_3.3.1.13 - Verify Error message appears when email contains ")" letter on Opp', () => {

  const tcId    = 'CRM-4001_3.3.1.13';
  const oppName = `TEST Opp 1 ${tcId}`;

  // Email with closing parenthesis character (Email_Contact#1)
  ...
  test('CRM-4001_3.3.1.13: Verify Error message appears when email contains ")" letter on Opp', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-4001_3.3.1.13 verification completed successfully');
      console.log(`   TC ID          : ${tcId}`);
      console.log(`   Opp name       : "${oppName}"`);
      console.log(`   Email_Contact#1: "${email_Contact1}"`);
