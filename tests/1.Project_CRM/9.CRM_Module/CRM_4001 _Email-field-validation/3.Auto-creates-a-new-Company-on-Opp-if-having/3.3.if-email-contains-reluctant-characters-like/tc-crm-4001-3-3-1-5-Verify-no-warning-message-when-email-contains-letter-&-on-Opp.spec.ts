 ...
 *
 * Command to run:
 * npx playwright test --grep "CRM-4001_3\.3\.1\.5:" --project=chromium
 *
 * Pre-condition:
 ...
 *     - Opp name textbox = TEST Opp 1 CRM-4001_3.3.1.5
...
test.describe('CRM-4001_3.3.1.5 - Verify a valid email containing "&" posts no warning on Opp (auto)', () => {

  const tcId    = 'CRM-4001_3.3.1.5';
  const oppName = `TEST Opp 1 ${tcId}`;

  // Email with ampersand character (Email_Contact#1) - a VALID local-part character
  ...
  test('CRM-4001_3.3.1.5: Verify a valid email containing "&" posts no warning on Opp (auto)', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-4001_3.3.1.5 verification completed successfully');
      console.log(`   TC ID          : ${tcId}`);
      console.log(`   Opp name       : "${oppName}"`);
      console.log(`   Email_Contact#1: "${email_Contact1}"`);
