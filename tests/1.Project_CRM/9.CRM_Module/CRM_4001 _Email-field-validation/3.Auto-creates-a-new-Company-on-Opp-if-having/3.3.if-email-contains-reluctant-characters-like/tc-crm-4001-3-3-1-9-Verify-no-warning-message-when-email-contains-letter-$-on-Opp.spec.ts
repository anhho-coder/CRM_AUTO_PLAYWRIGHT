...
test.describe('CRM-4001_3.3.1.9 - Verify a valid email containing "$" posts no warning on Opp (auto)', () => {

  const tcId    = 'CRM-4001_3.3.1.9';
  const oppName = `TEST Opp 1 ${tcId}`;

  // Email with dollar character (Email_Contact#1) - a VALID local-part character
  ...
  test('CRM-4001_3.3.1.9: Verify a valid email containing "$" posts no warning on Opp (auto)', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
