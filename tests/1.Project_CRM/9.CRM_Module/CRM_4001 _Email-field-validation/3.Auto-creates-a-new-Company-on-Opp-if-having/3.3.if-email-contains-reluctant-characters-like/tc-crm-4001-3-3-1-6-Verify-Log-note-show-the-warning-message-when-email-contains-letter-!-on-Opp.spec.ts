...
test.describe('CRM-4001_3.3.1.6 - Verify Error message appears when email contains "!" letter on Opp', () => {

  const tcId    = 'CRM-4001_3.3.1.6';
  const oppName = `TEST Opp 1 ${tcId}`;

  // Email with exclamation character (Email_Contact#1)
  ...
  test.skip('CRM-4001_3.3.1.6: Verify Error message appears when email contains "!" letter on Opp @CRM-10450', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
