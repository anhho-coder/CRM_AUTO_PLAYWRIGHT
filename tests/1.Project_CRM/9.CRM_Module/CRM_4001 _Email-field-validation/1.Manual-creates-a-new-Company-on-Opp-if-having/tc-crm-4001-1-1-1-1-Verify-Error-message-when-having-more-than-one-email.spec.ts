 ...
 *     - Opp name textbox = TEST Opp 1 CRM-4001_1.1.1.1
...
test.describe('CRM-4001_1.1.1.1 - Verify Error message appears when having more than one email on Opp', () => {

  const tcId    = 'CRM-4001_1.1.1.1';
  ...
  test('CRM-4001_1.1.1.1: Verify Error message appears when having more than one email on Opp', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-4001_1.1.1.1 verification completed successfully');
      console.log(`   TC ID          : ${tcId}`);
      console.log(`   Opp name       : "${oppName}"`);
      console.log(`   Email_Contact#1: "${email_Contact1}"`);
