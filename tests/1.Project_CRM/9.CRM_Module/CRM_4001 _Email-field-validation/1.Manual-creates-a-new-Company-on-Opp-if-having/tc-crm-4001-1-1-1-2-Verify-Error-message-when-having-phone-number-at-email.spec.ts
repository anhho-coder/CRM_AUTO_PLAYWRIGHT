 ...
 *     - Opp name textbox = TEST Opp 1 CRM-4001_1.1.1.2
...
test.describe('CRM-4001_1.1.1.2 - Verify Error message appears when having phone number instead of email on Opp', () => {

  const tcId    = 'CRM-4001_1.1.1.2';
  const oppName = `TEST Opp 1 ${tcId}`;

  // Phone number entered in the Email field (Email_Contact#1)
  ...
  test('CRM-4001_1.1.1.2: Verify Error message appears when having phone number instead of email on Opp', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-4001_1.1.1.2 verification completed successfully');
      console.log(`   TC ID          : ${tcId}`);
      console.log(`   Opp name       : "${oppName}"`);
      console.log(`   Email_Contact#1: "${email_Contact1}"`);
