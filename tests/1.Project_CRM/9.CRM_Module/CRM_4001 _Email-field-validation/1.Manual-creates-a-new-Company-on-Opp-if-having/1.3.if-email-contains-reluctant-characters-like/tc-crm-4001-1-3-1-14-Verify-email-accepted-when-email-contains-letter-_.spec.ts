 ...
 * Test Case ID: CRM-4001_1.3.1.14
 ...
 *          creating a new Company on Opp. "_" is a legal local-part character (e.g. user+tag@gmail.com),
 *          so the Opp must save with no "email is invalid" error.
 *
 * Command to run:
 * npx playwright test --grep "CRM-4001_1\.3\.1\.14:" --project=chromium
 *
 * Pre-condition:
 ...
 *     - Opp name textbox = TEST Opp 1 CRM-4001_1.3.1.14
...
test.describe('CRM-4001_1.3.1.14 - Verify a valid email containing "_" is accepted on Opp', () => {

  const tcId    = 'CRM-4001_1.3.1.14';
  ...
  test('CRM-4001_1.3.1.14: Verify a valid email containing "_" is accepted on Opp', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-4001_1.3.1.14 verification completed successfully');
      console.log(`   TC ID          : ${tcId}`);
      console.log(`   Opp name       : "${oppName}"`);
      console.log(`   Email_Contact#1: "${email_Contact1}"`);
