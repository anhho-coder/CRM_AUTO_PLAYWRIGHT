 ...
 *
 * Command to run:
 * npx playwright test --grep "CRM-4001_1\.3\.1\.9:" --project=chromium
 *
 * Pre-condition:
 ...
 *     - Opp name textbox = TEST Opp 1 CRM-4001_1.3.1.9
...
test.describe('CRM-4001_1.3.1.9 - Verify Error message appears when email contains "$" letter on Opp', () => {

  const tcId    = 'CRM-4001_1.3.1.9';
  ...
  test.skip('CRM-4001_1.3.1.9: Verify Error message appears when email contains "$" letter on Opp @CRM-10450', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CRM-4001_1.3.1.9 verification completed successfully');
      console.log(`   TC ID          : ${tcId}`);
      console.log(`   Opp name       : "${oppName}"`);
      console.log(`   Email_Contact#1: "${email_Contact1}"`);
