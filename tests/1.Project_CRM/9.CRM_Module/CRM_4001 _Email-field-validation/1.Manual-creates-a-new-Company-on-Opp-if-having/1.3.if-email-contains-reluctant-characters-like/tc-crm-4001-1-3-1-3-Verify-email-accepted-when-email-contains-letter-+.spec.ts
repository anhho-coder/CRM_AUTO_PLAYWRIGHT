 ...
 *
 * Command to run:
 * npx playwright test --grep "CRM-4001_1\.3\.1\.3:" --project=chromium
 *
 * Pre-condition:
 ...
 *     - Opp name textbox = TEST Opp 1 CRM-4001_1.3.1.3
...
test.describe('CRM-4001_1.3.1.3 - Verify a valid email containing "+" is accepted on Opp', () => {

  const tcId    = 'CRM-4001_1.3.1.3';
  ...
  test('CRM-4001_1.3.1.3: Verify a valid email containing "+" is accepted on Opp', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step 5 - Verification: Opp saved with no "email is invalid" error', async () => {
      ...
      try {
        ...
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: `${tcId} - Opp saved`, passed: __verifyPassed }).catch(() => {});
      }
      console.log(`✓ Verification passed: Opp saved at ${createdUrl} with no invalid-email error`);
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-4001_1.3.1.3 verification completed successfully');
      console.log(`   TC ID          : ${tcId}`);
      console.log(`   Opp name       : "${oppName}"`);
      console.log(`   Email_Contact#1: "${email_Contact1}"`);
