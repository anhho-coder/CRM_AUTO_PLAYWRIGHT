 ...
 *          if email contains reluctant characters like "_" letter
 *
 * Command to run:
...
test.describe('CRM-4001_2.3.1.14 - Verify Error message appears when email contains "_" letter on Lead', () => {

  const tcId    = 'CRM-4001_2.3.1.14';
  const oppName = `TEST Opp 1 ${tcId}`;

  // Email with underscore character (Email_Contact#1)
  ...
  test.skip('CRM-4001_2.3.1.14: Verify Error message appears when email contains "_" letter on Lead @CRM-10450', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
