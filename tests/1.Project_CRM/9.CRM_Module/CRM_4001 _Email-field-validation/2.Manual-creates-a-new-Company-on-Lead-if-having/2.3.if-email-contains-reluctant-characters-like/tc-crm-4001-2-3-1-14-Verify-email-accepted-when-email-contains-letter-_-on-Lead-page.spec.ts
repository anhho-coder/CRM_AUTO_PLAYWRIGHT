 ...
 * Test Case ID: CRM-4001_2.3.1.14
 * Automation-Type: refactored
 * Automation-Date: 2026-06-26
 *
 * Summary: Verify a valid email whose local part contains "_" is ACCEPTED (no error) when manually
 *          creating a new Company on Lead. "_" is a legal local-part character (e.g. user+tag@gmail.com),
 *          so the Lead must save with no "email is invalid" error.
 *
 * Command to run:
 * npx playwright test --grep "CRM-4001_2\.3\.1\.14:" --project=chromium
 *
 * Pre-condition:
 ...
 *     - Opp name textbox = TEST Opp 1 CRM-4001_2.3.1.14
 ...
 *    "The email is invalid!" dialog appears - "_" is a valid email character.
 *
 * Note: Resolved under CRM-10450. The original expectation (an error dialog) was incorrect - the
 *       email validator correctly accepts "_" (and * & ! # $ % ^ _ =). Verified live 2026-06-26.
 */

const SKIP_CLEANUP = false; // true = skip deleting the Lead created by this test

test.describe('CRM-4001_2.3.1.14 - Verify a valid email containing "_" is accepted on Lead', () => {

  const tcId    = 'CRM-4001_2.3.1.14';
  ...
  test('CRM-4001_2.3.1.14: Verify a valid email containing "_" is accepted on Lead', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    ...
    await test.step('Step 5 - Verification: Lead saved with no "email is invalid" error', async () => {
      ...
      expect(errorShown, `No "${invalidEmailError}" error should appear for a valid email ("_")`).toBeFalsy();
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-4001_2.3.1.14 verification completed successfully');
      console.log(`   TC ID          : ${tcId}`);
      console.log(`   Opp name       : "${oppName}"`);
      console.log(`   Email_Contact#1: "${email_Contact1}"`);
