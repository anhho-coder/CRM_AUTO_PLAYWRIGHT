...
/**
 * CRM-464 - Verify textbox displays when Development required checkbox is checked
 * Test Case ID: CRM-464_2
 *
 * Summary: Verify the textbox displays when "Development required" checkbox is checked
 *
 * Command to run:
 * npx playwright test --grep "CRM-464_2" --project=chromium
 *
 * I. Pre-condition:
 ...
 * 1. The "Development required" checkbox is CHECKED
 * 2. The "Development required" textbox = Test
 */

test.describe('CRM-464_2 - Verify textbox displays when Development required is checked', () => {
  ...
  test('CRM-464_2: Verify the textbox displays when Development required checkbox is checked', async ({ page }, testInfo) => {
    ...
    await test.step('II. Verification - Step 1: Open Opp#1 and verify "Development required" checkbox is CHECKED', async () => {
      console.log('\n=== II. VERIFICATION POINTS ===');
      ...
      await expect(devRequiredCheckbox.first()).toBeChecked({ timeout: 10000 });
      console.log('  ✓ II.1: "Development required" checkbox is CHECKED');
    });

    await test.step('II. Verification - Step 2: Verify "Development required" textbox = "Test"', async () => {
      const devRequiredTextbox = opportunityPage.getDevRequiredTextbox_Readonly();
      await expect(devRequiredTextbox.first()).toBeVisible({ timeout: 10000 });
      await expect(devRequiredTextbox.first()).toHaveText('Test', { timeout: 10000 });
      console.log('  ✓ II.2: "Development required" textbox = "Test"');
    });

    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-464_2 verification completed successfully');
      console.log(`   Opp#1 (${opp1Id}): Created Manually=TRUE, Lead Form=Download Free Trial, Development required=CHECKED`);
      console.log(`   II.1: "Development required" checkbox is CHECKED`);
      console.log(`   II.2: "Development required" textbox = "Test"`);
      console.log('==================================================\n');
    });
