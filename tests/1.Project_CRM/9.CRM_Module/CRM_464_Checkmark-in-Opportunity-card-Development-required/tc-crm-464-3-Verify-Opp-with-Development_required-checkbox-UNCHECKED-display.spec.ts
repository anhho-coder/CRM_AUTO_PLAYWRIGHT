...
/**
 * CRM-464 - Verify Opps with Development required UNCHECKED display with custom filter "is false"
 * Test Case ID: CRM-464_3
 *
 * Summary: Verify the Opps with "Development required" checkbox UNCHECKED display if user does
 * custom filter by "Development required" field with the value "is false"
 *
 * Command to run:
 * npx playwright test --grep "CRM-464_3" --project=chromium
 *
 * I. Pre-condition:
 ...
 * 6. Press "CRM" link on the top of page and wait
 * 7. On "CRM" page, click at "view list" button
 * 8. On "Search" textbox, remove "My Pipeline" and wait
 *
 ...
 * 4. On Dropdown_List#2, select "is false"
...
test.describe('CRM-464_3 - Verify Opp with Development required UNCHECKED displays with "is false" filter', () => {
  ...
  test('CRM-464_3: Verify the Opp with Development required UNCHECKED displays when filtering by "is false"', async ({ page }, testInfo) => {
    ...
    await test.step('Condition #1 Step 6: Navigate back to CRM page', async () => {
      await opportunityPage.clickCRMMenuLink();
      await opportunityPage.waitForPageReady();
      console.log('✓ Navigated back to CRM page');
    });

    await test.step('Condition #1 Step 7: Switch to list view', async () => {
      await opportunityPage.switchToListView();
      console.log('✓ Switched to list view');
    });

    await test.step('Condition #1 Step 8: Remove "My Pipeline" filter and wait', async () => {
    ...
    await test.step('Step 4: On Dropdown_List#2 - select "is false"', async () => {
      await opportunityPage.selectCustomFilterOperator('is false');
      console.log('✓ Operator "is false" selected in Dropdown_List#2');
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-464_3 verification completed successfully');
      console.log(`   Opp#1 (${opp1Id}): Development required=UNCHECKED (false)`);
      console.log(`   Custom filter applied: "Development required" = "is false"`);
