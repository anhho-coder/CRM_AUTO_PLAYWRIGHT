...
/**
 * Mass Mark as Duplicate - error when a Won lead + a non-won lead are selected together.
 * Test Case ID: CRM-10601_1.1.1.12   (Jira: CRM-10744 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-15
 *
 * Summary: Verify the system shows an error popup if the selection for "Mass Mark as Duplicate"
 *          contains a Won lead alongside a non-won lead (Won leads cannot be marked as lost).
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_1\.1\.1\.12:" --project=chromium
 *
 * Source manual TC (Jira CRM-10744)
 ...
 * - A "Setup" step creates a Won Opportunity + a non-won Opportunity. Selecting "Mass Mark as
 *   Duplicate" with the Won lead in the selection surfaces the server warning popup naming the Won
 *   lead: "The following selected lead(s) are Won and cannot be marked as lost...".
 */

const SKIP_CLEANUP_OPPS = false;

test.describe('CRM-10601_1.1.1.12 - Error when mass mark lost with a Won lead + a non-won lead', () => {
  ...
  test('CRM-10601_1.1.1.12: Verify system will show error if user mass mark lost with won lead + non-won lead', async ({ page }, testInfo) => {
    ...
    await test.step('Setup: Create a Won Opportunity and a non-won Opportunity', async () => {
      ...
      const stamp = opportunityPage.generateOpportunityName('TEST CRM-10744 ');
    ...
    // Step 5: Select Mass Mark as duplicate -> error popup (names the Won lead)
    await test.step('Step 5: Select Mass Mark as duplicate', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10744 - Error popup for Won + non-won selection');
