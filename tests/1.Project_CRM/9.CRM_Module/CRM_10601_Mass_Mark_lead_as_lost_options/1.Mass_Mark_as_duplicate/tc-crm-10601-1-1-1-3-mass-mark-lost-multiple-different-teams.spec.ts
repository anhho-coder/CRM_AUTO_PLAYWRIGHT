...
/**
 * Mass Mark as Lost (Duplicate) for multiple records with different teams.
 * Test Case ID: CRM-10601_1.1.1.3   (Jira: CRM-10735 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-12
 *
 * Summary: Verify a Sales Manager can mass-mark MULTIPLE Opportunities that belong to DIFFERENT Sales
 *          teams as lost via Action > "Mass Mark as Duplicate"; each gets a Lost Lead Approval
 *          request (assigned to its own team's director) + log note.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_1\.1\.1\.3:" --project=chromium
 *
 * Source manual TC (Jira CRM-10735)
...
test.describe('CRM-10601_1.1.1.3 - Mass Mark as Lost for multiple records with different teams', () => {
  ...
  test('CRM-10601_1.1.1.3: Verify user is able mass mark lost for multiple records with different teams', async ({ page }, testInfo) => {
    ...
    await test.step('Setup: Create 3 non_won Opportunities with different Sales teams (Stage = New)', async () => {
      ...
      const stamp = opportunityPage.generateOpportunityName('TEST CRM-10735 ');
    ...
    await test.step('Step 7: Login with sales manager and review all the Opp', async () => {
      ...
      try {
        for (let i = 0; i < createdOppUrls.length; i++) {
          ...
          expect(found, `Opp ${i + 1} (team ${TEAMS[i]}) should have a "marked as lost" log note`).toBeTruthy();
        ...
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'CRM-10735 - Last Opp after Mass Mark as Lost (different teams)', passed: __verifyPassed }).catch(() => {});
      }
      console.log('✅ All 3 Opportunities (different teams) have a pending approval');
    });
  });
});
