...
/**
 * Mass Mark as Duplicate - confirmation message shows 10 leads when selecting 10 leads.
 * Test Case ID: CRM-10601_1.1.1.5   (Jira: CRM-10737 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-12
 *
 * Summary: Verify the "Mass Mark as Duplicate" confirmation popup reports 10 leads when 10
 *          non-won Opportunities are selected.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_1\.1\.1\.5:" --project=chromium
 *
 * Source manual TC (Jira CRM-10737)
 ...
 * - The test does NOT submit; it Cancels the wizard so the created Opportunities stay non-won (deletable).
 */

const LEAD_COUNT = 10;
const SKIP_CLEANUP_OPPS = false;

test.describe('CRM-10601_1.1.1.5 - Confirmation message shows 10 leads when selecting 10 leads', () => {
  ...
  test('CRM-10601_1.1.1.5: Verify confirmation message shows 10 leads when selecting 10 leads', async ({ page }, testInfo) => {
    ...
    await test.step(`Setup: Create ${LEAD_COUNT} non_won Opportunities (Stage = New)`, async () => {
      ...
      const stamp = opportunityPage.generateOpportunityName('TEST CRM-10737 ');
    ...
    await test.step('Step 6: Observe popup', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `CRM-10737 - Mass Mark popup shows ${leadCount} leads`);
