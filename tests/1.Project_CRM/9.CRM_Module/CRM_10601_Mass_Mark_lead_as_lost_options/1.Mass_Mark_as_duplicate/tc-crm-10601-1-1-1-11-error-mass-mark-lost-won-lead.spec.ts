...
/**
 * Mass Mark as Duplicate - error when a Won lead is selected.
 * Test Case ID: CRM-10601_1.1.1.11   (Jira: CRM-10743 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-15
 *
 * Summary: Verify the system shows an error popup if the user tries to "Mass Mark as Duplicate" a
 *          Won lead (Won leads cannot be marked as lost).
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_1\.1\.1\.11:" --project=chromium
 *
 * Source manual TC (Jira CRM-10743)
 ...
 *   "Mass Mark as Duplicate" on a Won lead surfaces the server warning popup:
 *   "The following selected lead(s) are Won and cannot be marked as lost..." (no lost-reason wizard).
 */

const SKIP_CLEANUP_OPP = false;

test.describe('CRM-10601_1.1.1.11 - Error when mass mark lost with a Won lead', () => {
  let createdOppUrl = '';
  let createdOppName = '';
  ...
  test('CRM-10601_1.1.1.11: Verify system will show error if user mass mark lost with a won lead', async ({ page }, testInfo) => {
    ...
    await test.step('Setup: Create a Won Opportunity', async () => {
      ...
      createdOppName = opportunityPage.generateOpportunityName('TEST CRM-10743 WON ');
    ...
    // Step 5: Select Mass Mark as duplicate -> error popup
    await test.step('Step 5: Select Mass Mark as duplicate', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10743 - Error popup for Won lead');
