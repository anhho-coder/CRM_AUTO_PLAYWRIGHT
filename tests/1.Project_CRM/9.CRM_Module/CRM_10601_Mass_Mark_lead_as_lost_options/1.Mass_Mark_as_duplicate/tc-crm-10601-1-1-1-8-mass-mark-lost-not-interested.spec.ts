...
/**
 * Mass Mark as Lost (Duplicate) with Lost reason = "Not interested" - automated from a manual test case.
 * Test Case ID: CRM-10601_1.1.1.8   (Jira: CRM-10740 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-12
 *
 * Summary: Verify a Sales Manager can mass-mark a single Opportunity as lost via
 *          Action > "Mass Mark as Duplicate" choosing Lost reason "Not interested", which submits a
 *          Lost Lead Approval request and posts the corresponding log note.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_1\.1\.1\.8:" --project=chromium
 *
 * Source manual TC (Jira CRM-10740)
 ...
 *       _ Lost reason: Not interested
 *     There is a log note for lost lead approval:
 *       "The lead has been marked as lost by {submitter}. Lost reason: Not interested. Pending approval."
 *
 * Design notes:
 * - Sales Manager = Veronika (users.manager_veronika).
 * - A "Setup" step creates the non_won Opportunity that step 3 selects (the Jira precondition only
 *   covers login; the opportunity is implied by step 3). Step 7 is verified in the same session.
 */

const LOST_REASON = 'Not interested';
const SKIP_CLEANUP_OPP = false;

test.describe('CRM-10601_1.1.1.8 - Mass Mark as Lost with Lost reason "Not interested"', () => {
  let createdOppUrl = '';
  let createdOppName = '';
  ...
  test('CRM-10601_1.1.1.8: Verify user is able to mass mark lost with Lost reason = "Not interested"', async ({ page }, testInfo) => {
    ...
    await test.step('Setup: Create a non_won Opportunity (Stage = New) to act on', async () => {
      ...
      createdOppName = opportunityPage.generateOpportunityName('TEST CRM-10740 ');
    ...
    await test.step('Verification: Step 7 Expected Result (pending approval + lost-lead log note)', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10740 - Opportunity after Mass Mark as Lost (Not interested)');
      ...
      console.log('✅ Mass Mark as Lost with reason "Not interested" created a pending approval');
    });
  });
});
