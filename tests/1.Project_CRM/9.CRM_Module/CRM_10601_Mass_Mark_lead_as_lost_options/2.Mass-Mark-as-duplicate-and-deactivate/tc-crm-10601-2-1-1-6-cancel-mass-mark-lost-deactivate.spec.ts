...
/**
 * Mass Mark as Duplicate and Deactivate - cancel the wizard.
 * Test Case ID: CRM-10601_2.1.1.6   (Jira: CRM-10755 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-18
 *
 * Summary: Verify the user can cancel the "Mass Mark as Duplicate and Deactivate" wizard: the popup
 *          disappears without error and nothing is marked lost / deactivated.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_2\.1\.1\.6:" --project=chromium
 *
 * Source manual TC (Jira CRM-10755)
 ...
 * - Cancelling leaves all created Opportunities non-won and active, so teardown can delete them.
 */
...
test.describe('CRM-10601_2.1.1.6 - Cancel Mass Mark as Duplicate and Deactivate', () => {
  ...
  test('CRM-10601_2.1.1.6: Verify the user is able to cancel mass mark lost and deactivate', async ({ page }, testInfo) => {
    ...
    await test.step(`Setup: Create ${LEAD_COUNT} non_won Opportunities (Stage = New)`, async () => {
      ...
      const stamp = opportunityPage.generateOpportunityName('TEST CRM-10755 ');
