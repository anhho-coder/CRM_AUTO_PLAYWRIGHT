...
/**
 * Mass Mark as Duplicate and Deactivate for multiple records with the same team.
 * Test Case ID: CRM-10601_2.1.1.2   (Jira: CRM-10751 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-18
 *
 * Summary: Verify a Sales Manager can mass-mark MULTIPLE Opportunities (same Sales team) lost AND
 *          deactivate them via Action > "Mass Mark as Duplicate and Deactivate"; each is archived
 *          (active=false), marked lost, with NO pending approval.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_2\.1\.1\.2:" --project=chromium
 *
 * Source manual TC (Jira CRM-10751)
 ...
 *     3. Select 3 non_won opps that have the same Sales team
 *     4. Click Action
 ...
 *
 * Design notes:
 * - Sales Manager = Veronika (users.manager_veronika); the 3 created Opps default to her team (same team).
...
test.describe('CRM-10601_2.1.1.2 - Mass Mark as Duplicate and Deactivate for multiple records with the same team', () => {
  ...
  test('CRM-10601_2.1.1.2: Verify user is able mass mark lost and deactivate for multiple records with the same team', async ({ page }, testInfo) => {
    ...
    // Setup (needed for step 3): create 3 non_won Opportunities with the same team (Stage = New)
    await test.step(`Setup: Create ${LEAD_COUNT} non_won Opportunities with the same team (Stage = New)`, async () => {
      await opportunityPage.switchToListView();
      const stamp = opportunityPage.generateOpportunityName('TEST CRM-10751 ');
      ...
      console.log(`✓ Created ${createdOppUrls.length} Opportunities (same team)`);
    });
    ...
    // Step 3: Select 3 non_won opps that have the same Sales team
    await test.step(`Step 3: Select ${LEAD_COUNT} non_won opps that have the same Sales team`, async () => {
    ...
    await test.step('Step 5: Select Mass Mark as duplicate and deactivate', async () => {
      ...
      expect(leadCount, `The wizard should target exactly ${LEAD_COUNT} leads`).toBe(LEAD_COUNT);
    });
    ...
    await test.step('Step 7: Login with sales manager and review all the Opp', async () => {
      ...
      try {
        for (let i = 0; i < createdOppUrls.length; i++) {
          ...
          expect(found, `Opp ${i + 1} should have a "Marked as Duplicate + Deactivated" log note`).toBeTruthy();
        ...
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: 'CRM-10751 - Last Opp after Mass Mark as Duplicate and Deactivate (same team)', passed: __verifyPassed }).catch(() => {});
      }
      console.log(`✅ All ${LEAD_COUNT} Opportunities (same team) were marked lost AND deactivated directly`);
    });
  });
});
