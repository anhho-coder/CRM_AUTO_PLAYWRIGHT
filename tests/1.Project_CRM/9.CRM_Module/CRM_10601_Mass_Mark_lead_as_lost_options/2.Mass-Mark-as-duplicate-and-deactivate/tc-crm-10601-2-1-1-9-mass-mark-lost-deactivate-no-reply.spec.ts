...
/**
 * Mass Mark as Duplicate and Deactivate with Lost reason = "No reply / never responded to calls or emails".
 * Test Case ID: CRM-10601_2.1.1.9   (Jira: CRM-10758 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-18
 *
 * Summary: Verify a Sales Manager can mass-mark a single Opportunity lost AND deactivate it via
 *          Action > "Mass Mark as Duplicate and Deactivate" choosing Lost reason
 *          "No reply / never responded to calls or emails"; the record is archived (active=false),
 *          marked lost, with NO pending approval.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_2\.1\.1\.9:" --project=chromium
 *
 * Source manual TC (Jira CRM-10758)
 ...
 *     There is new pending approval (Lost reason: No reply / never responded to calls or emails)
 ...
 *   / "Opportunity lost" / "Lost Reason: No reply ..." log note.
 *
 * Design notes:
...
const LOST_REASON_ASSERT = 'no reply'; // stable distinctive substring asserted in the chatter note
const ACTION_OPTION = 'Mass Mark as Duplicate and Deactivate';
const SKIP_CLEANUP_OPP = false;

test.describe('CRM-10601_2.1.1.9 - Mass Mark as Duplicate and Deactivate with Lost reason "No reply..."', () => {
  let createdOppUrl = '';
  let createdOppName = '';
  ...
  test('CRM-10601_2.1.1.9: Verify user is able to mass mark lost and deactivate with Lost reason = "No reply / never responded to calls or emails"', async ({ page }, testInfo) => {
    ...
    await test.step('Setup: Create a non_won Opportunity (Stage = New) to act on', async () => {
      ...
      createdOppName = opportunityPage.generateOpportunityName('TEST CRM-10758 ');
    ...
    // Verification of step 7's ACTUAL behavior: deactivated + marked lost (No reply...) + no pending approval
    await test.step('Verification: Step 7 - Opportunity is deactivated and marked lost (No reply..., no pending approval)', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10758 - Opportunity after Mass Mark and Deactivate (No reply...)');
      ...
      console.log('✅ Opportunity marked lost AND deactivated directly with reason "No reply..." (no pending approval)');
    });
  });
});
