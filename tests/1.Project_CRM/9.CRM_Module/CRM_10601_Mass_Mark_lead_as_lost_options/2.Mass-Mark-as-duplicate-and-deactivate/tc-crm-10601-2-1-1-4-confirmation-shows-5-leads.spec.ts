...
/**
 * Mass Mark as Duplicate and Deactivate - confirmation message shows 5 leads when selecting 5 leads.
 * Test Case ID: CRM-10601_2.1.1.4   (Jira: CRM-10753 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-18
 *
 * Summary: Verify the "Mass Mark as Duplicate and Deactivate" confirmation popup reports 5 leads when
 *          5 non-won Opportunities are selected.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_2\.1\.1\.4:" --project=chromium
 *
 * Source manual TC (Jira CRM-10753)
...
test.describe('CRM-10601_2.1.1.4 - Confirmation message shows 5 leads when selecting 5 leads', () => {
  ...
  test('CRM-10601_2.1.1.4: Verify confirmation message shows 5 leads when selecting 5 leads', async ({ page }, testInfo) => {
    ...
    // Setup (needed for step 3): create 5 non_won Opportunities (Stage = New)
    await test.step(`Setup: Create ${LEAD_COUNT} non_won Opportunities (Stage = New)`, async () => {
      await opportunityPage.switchToListView();
      const stamp = opportunityPage.generateOpportunityName('TEST CRM-10753 ');
    ...
    // Step 6: Observe popup -> confirmation message shows 5 leads
    await test.step('Step 6: Observe popup', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `CRM-10753 - Mass Mark and Deactivate popup shows ${leadCount} leads`);
