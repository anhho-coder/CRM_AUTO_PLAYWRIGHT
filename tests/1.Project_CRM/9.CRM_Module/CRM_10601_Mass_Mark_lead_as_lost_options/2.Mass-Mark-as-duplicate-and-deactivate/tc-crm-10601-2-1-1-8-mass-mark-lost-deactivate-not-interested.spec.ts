...
/**
 * Mass Mark as Duplicate and Deactivate with Lost reason = "Not interested".
 * Test Case ID: CRM-10601_2.1.1.8   (Jira: CRM-10757 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-18
 *
 * Summary: Verify a Sales Manager can mass-mark a single Opportunity lost AND deactivate it via
 *          Action > "Mass Mark as Duplicate and Deactivate" choosing Lost reason "Not interested";
 *          the record is archived (active=false), marked lost, with NO pending approval.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_2\.1\.1\.8:" --project=chromium
 *
 * Source manual TC (Jira CRM-10757)
 ...
 *     6. Submit with "Not interested" Lost reason
 *     7. Login with sales manager and view this Opp
 *
 *   Expected Result (step 7) - AS STATED in the manual TC:
 *     There is new pending approval (Lost reason: Not interested) + a "marked as lost ... Pending approval." log note.
 *
 ...
 *   / "Opportunity lost" / "Lost Reason: Not interested" log note.
 *
 * Design notes:
 * - Sales Manager = Veronika (users.manager_veronika).
 * - A "Setup" step creates the non_won Opportunity that step 3 selects.
 */

const LOST_REASON = 'Not interested';
const ACTION_OPTION = 'Mass Mark as Duplicate and Deactivate';
const SKIP_CLEANUP_OPP = false;

test.describe('CRM-10601_2.1.1.8 - Mass Mark as Duplicate and Deactivate with Lost reason "Not interested"', () => {
  let createdOppUrl = '';
  let createdOppName = '';
  ...
  test('CRM-10601_2.1.1.8: Verify user is able to mass mark lost and deactivate with Lost reason = "Not interested"', async ({ page }, testInfo) => {
    ...
    await test.step('Setup: Create a non_won Opportunity (Stage = New) to act on', async () => {
      ...
      createdOppName = opportunityPage.generateOpportunityName('TEST CRM-10757 ');
    ...
    // Step 6: Submit with "Not interested" Lost reason
    await test.step('Step 6: Submit with "Not interested" Lost reason', async () => {
    ...
    // Verification of step 7's ACTUAL behavior: deactivated + marked lost (Not interested) + no pending approval
    await test.step('Verification: Step 7 - Opportunity is deactivated and marked lost (Not interested, no pending approval)', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10757 - Opportunity after Mass Mark and Deactivate (Not interested)');
      ...
      console.log('✅ Opportunity marked lost AND deactivated directly with reason "Not interested" (no pending approval)');
    });
  });
});
