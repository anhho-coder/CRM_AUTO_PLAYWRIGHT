...
/**
 * Visibility: "Mass Mark as Duplicate" IS visible for a regular sales user in the All leads list.
 * Test Case ID: CRM-10601_3.2.1.1   (Jira: CRM-10769 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-18
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_3\.2\.1\.1:" --project=chromium
 *
 * Source manual TC (Jira CRM-10769)
...
test.describe('CRM-10601_3.2.1.1 - "Mass Mark as Duplicate" visible for sales user in All lead list', () => {
  ...
  test('CRM-10601_3.2.1.1: "Mass Mark as Duplicate" visible for sales user in All lead list', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Select 1 lead and click Action dropdown', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10769 - All-leads Action menu (regular sales user)');
      expect(labels, `The Action menu should show "${OPTION}"`).toContain(OPTION);
      console.log(`✅ "${OPTION}" is visible for the regular sales user in the All leads list`);
    });
  });
});
