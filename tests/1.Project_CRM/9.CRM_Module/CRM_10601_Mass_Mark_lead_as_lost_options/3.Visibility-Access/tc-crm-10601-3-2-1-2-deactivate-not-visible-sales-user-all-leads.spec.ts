...
/**
 * Visibility: "Mass Mark as Duplicate and Deactivate" is NOT visible for a regular sales user in the All leads list.
 * Test Case ID: CRM-10601_3.2.1.2   (Jira: CRM-10770 - Post-EA Test Case)
 * Automation-Type: new
 * Automation-Date: 2026-06-18
 *
 * Command to run:
 *   npx playwright test --grep "CRM-10601_3\.2\.1\.2:" --project=chromium
 *
 * Source manual TC (Jira CRM-10770)
...
test.describe('CRM-10601_3.2.1.2 - "Mass Mark as Duplicate and Deactivate" NOT visible for regular sales user in All lead list', () => {
  ...
  test('CRM-10601_3.2.1.2: "Mass Mark as Duplicate and Deactivate" NOT visible for regular sales user in All lead list', async ({ page }, testInfo) => {
    ...
    await test.step('Step 3: Select 1 lead and click Action dropdown', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'CRM-10770 - All-leads Action menu (regular sales user)');
      ...
      console.log(`✅ "${OPTION}" is NOT visible for the regular sales user in the All leads list`);
    });
  });
});
