 ...
 * ===========================================================================
 *  Test Case ID    : TC.-A.8.4
 *  Automation-Type : new
 *  Automation-Date : 2026-06-24
 *
 *  Summary:
 *    As Thomas, create a deal-registration validated Invoice (Invoice Number #1);
 *    then as the Reseller verify "My invoices" lists multiple invoices including
 *    Invoice Number #1 (the list renders the Reseller's full invoice set).
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-A\.8\.4:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *
 *  Steps to reproduce #2  -  view the invoices as the Reseller:
 ...
 *
 *  Verification #1:
 *    1. "My invoices" lists multiple invoices (>= 2), including Invoice Number #1.
 *
 *  Note: the fresh Invoice Number #1 satisfies the independent-data rule (REQUIREMENT #2); the
 *  "multiple" assertion leverages the Reseller's accrued invoices (a reliable single-session check -
 *  creating two invoices in one run would require fragile cross-module Accounting->CRM navigation).
 * ===========================================================================
 */

const SKIP_CLEANUP_OPP = true; // validated Invoice cannot be cleanly deleted -> retain (see TC.-A.8.1)

test.describe('TC.-A.8.4 - Reseller sees multiple invoices listed', () => {
  ...
  test('TC.-A.8.4: Verify Reseller sees multiple invoices listed including the new one', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-A.8.4 ${compactDateTime}`;
    ...
    await test.step('Verification #1: "My invoices" lists multiple invoices including Invoice Number #1', async () => {
      const listed = await resellerPortalPage.getListedInvoiceNumbers();
      const hasNew = await resellerPortalPage.isInvoiceListed(invoice.invoiceNumber);
      console.log(`  - Listed invoices (${listed.length}): ${JSON.stringify(listed)}`);
      console.log(`  - Invoice Number #1 "${invoice.invoiceNumber}" listed: ${hasNew}`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.8.4 - Reseller My invoices (multiple invoices)');

      expect(hasNew, `Invoice Number #1 "${invoice.invoiceNumber}" should be listed on My invoices`).toBeTruthy();
      expect(listed.length, 'My invoices should list multiple invoices (>= 2)').toBeGreaterThanOrEqual(2);
      console.log('✅ Reseller sees multiple invoices listed, including the newly created one');
    });
  });
});
