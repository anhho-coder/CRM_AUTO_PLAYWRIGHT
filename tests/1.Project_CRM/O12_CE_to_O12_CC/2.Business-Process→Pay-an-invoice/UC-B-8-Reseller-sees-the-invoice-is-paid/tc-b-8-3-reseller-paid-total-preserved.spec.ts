 ...
 *    Paid; then as Reseller_1 open Invoice#1 and verify the invoice Total still equals InvoiceTotal#1
 *    (the original amount is preserved) while Amount Due = $0.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.8\.3:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *             - Opp name                  = TEST TC.-B.8.3 <current date time>
 ...
 *    1. The invoice "Total" still equals InvoiceTotal#1 (the original amount is preserved)
...
test.describe('TC.-B.8.3 - Reseller sees the paid invoice Total preserved while Amount Due is $0', () => {
  ...
  test('TC.-B.8.3: Verify the paid invoice Total is preserved while Amount Due is $0', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.8.3 ${compactDateTime}`;
    let invoiceNumber = '';
    ...
    await test.step('Verification Point: the invoice "Total" still equals InvoiceTotal#1 while Amount Due = $0', async () => {
      const breakdown = await resellerPortalPage.getDetailTotalsBreakdown();
      const totalRow = breakdown.find((r) => /^total$/i.test(r.label));
      const amountDue = await resellerPortalPage.getDetailAmountDue();
      console.log(`  - Totals breakdown : ${JSON.stringify(breakdown)}`);
      console.log(`  - Total row        : ${JSON.stringify(totalRow)} | InvoiceTotal#1: "${invoice.invoiceTotal}"`);
      console.log(`  - Amount Due       : "${amountDue}" (${money(amountDue)})`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Verification - Total preserved, Amount Due $0');

      expect(totalRow, 'A "Total" row should be shown on the paid invoice').toBeTruthy();
      expect(money(totalRow?.amount), 'The Total should be > 0 (preserved)').toBeGreaterThan(0);
      expect(money(totalRow?.amount), 'The Total should equal InvoiceTotal#1').toBeCloseTo(money(invoice.invoiceTotal), 2);
      expect(money(amountDue), 'Amount Due should be $0 on a fully-paid invoice').toBeCloseTo(0, 2);
      console.log('✅ The paid invoice Total is preserved and Amount Due is $0');
    });
  });
});
