 ...
 *    Paid; then as Reseller_1 open Invoice#1 and verify the paid invoice still lists its product line(s)
 *    on the detail screen while Amount Due = $0.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.8\.4:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *             - Opp name                  = TEST TC.-B.8.4 <current date time>
 ...
 *    1. The invoice still lists at least one product line (Description/Quantity/Amount)
...
test.describe('TC.-B.8.4 - Reseller sees the paid invoice still lists its product line', () => {
  ...
  test('TC.-B.8.4: Verify the paid invoice still lists its product line and Amount Due is $0', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.8.4 ${compactDateTime}`;
    let invoiceNumber = '';
    ...
    await test.step('Verification Point: the paid invoice still lists its product line(s) and Amount Due = $0', async () => {
      const lineCount = await resellerPortalPage.getDetailProductLineCount();
      const amountDue = await resellerPortalPage.getDetailAmountDue();
      console.log(`  - Product line count : ${lineCount}`);
      console.log(`  - Amount Due         : "${amountDue}" (${money(amountDue)})`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Verification - product line shown, Amount Due $0');

      expect(lineCount, 'The paid invoice should still list at least one product line').toBeGreaterThanOrEqual(1);
      expect(money(amountDue), 'Amount Due should be $0 on a fully-paid invoice').toBeCloseTo(0, 2);
      console.log('✅ The paid invoice still lists its product line and Amount Due is $0');
    });
  });
});
