 ...
 *    Paid; then as Reseller_1 open Invoice#1 directly via its portal deep-link URL (as if bookmarked) and
 *    verify it is paid (no "Pay Now", "This invoice is paid", Amount Due = $0).
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.8\.8:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *             - Opp name                  = TEST TC.-B.8.8 <current date time>
 ...
 *    2. Click "My Invoices" and open Invoice#1 to capture its portal deep-link URL
 *    3. Open Invoice#1 again via its direct portal URL (as a bookmarked deep-link)
 *
 *  Verification Point - on the deep-linked Invoice#1 detail screen:
...
test.describe('TC.-B.8.8 - Reseller opens the paid invoice via a direct deep-link', () => {
  ...
  test('TC.-B.8.8: Verify the Reseller sees the invoice is paid when opened via a direct deep-link', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.8.8 ${compactDateTime}`;
    let invoiceNumber = '';
    let deepLinkUrl = '';
    ...
    await test.step('Steps to reproduce - Step 2: Click "My Invoices" and open Invoice#1 to capture its portal deep-link URL', async () => {
      ...
      deepLinkUrl = await resellerPortalPage.openInvoiceByNumber(invoiceNumber);
      console.log(`  - Captured portal deep-link URL: ${deepLinkUrl}`);
      expect(deepLinkUrl, 'A portal invoice deep-link URL should be captured').toContain('/my/invoices/');
    });

    await test.step('Steps to reproduce - Step 3: Open Invoice#1 again via its direct portal URL (bookmarked deep-link)', async () => {
      // Navigate away to the list first, then re-enter via the captured deep-link URL.
      await resellerPortalPage.clickMyInvoices();
      await resellerPortalPage.openInvoiceByUrl(deepLinkUrl);
      await resellerPortalPage.waitForDetailLineTable();
      const opened = await resellerPortalPage.getDetailInvoiceNumber();
      console.log(`  - Deep-link opened invoice: "${opened}"`);
      expect(opened, 'The deep-link should open Invoice#1').toContain(invoiceNumber);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - Invoice#1 opened via deep-link').catch(() => {});
    });

    await test.step('Verification Point: "Pay Now" not displayed; "This invoice is paid" shown; Amount Due = $0', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Verification - deep-linked paid invoice');
      ...
      console.log('✅ The Reseller sees the invoice is paid when opened via a direct deep-link');
    });
  });
});
