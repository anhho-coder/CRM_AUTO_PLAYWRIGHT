 ...
 * ===========================================================================
 *  Test Case ID    : TC.-B.8.7
 *  Automation-Type : new
 *  Automation-Date : 2026-06-30
 *
 *  Summary:
 *    As Thomas create a deal-registration validated Invoice#1 on the EUR pricelist; as Faye register a
 *    full payment so it is Paid; then as Reseller_1 open Invoice#1 and verify it is paid (no "Pay Now",
 *    "This invoice is paid") with Amount Due = EUR 0.00 (the EUR currency is preserved).
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.8\.7:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *
 *  Pre-condition #2 - create an EUR invoice, then register a full payment so the invoice is Paid:
 *    1-3.   As Thomas: login; click "CRM" > "view list"; on the Opp page click "CREATE".
 *    4.     Enter the Opportunity details (one field per line):
 *             - Opp name                  = TEST TC.-B.8.7 <current date time>
 ...
 *             - Pricelist                 = Public Pricelist_EUR (EUR)   <- EUR currency
 ...
 *    19.    Click "Validate"; remember Invoice#1 + InvoiceTotal#1 (EUR).
 ...
 *    3. Amount Due = 0 in EUR (the EUR currency is preserved)
 * ===========================================================================
 */
...
test.describe('TC.-B.8.7 - Reseller sees a paid EUR-currency invoice', () => {
  ...
  test('TC.-B.8.7: Verify the Reseller sees a paid EUR-currency invoice with Amount Due 0', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.8.7 ${compactDateTime}`;
    let invoiceNumber = '';
    ...
    const invoice = await createValidatedInvoiceAsThomas(page, {
      ...
      pricelist: 'Public Pricelist_EUR', pricelistLabel: 'Public Pricelist_EUR (EUR)',
    ...
    await test.step('Verification Point: paid (no Pay Now, "This invoice is paid") with Amount Due 0 in EUR', async () => {
      ...
      console.log(`  - "Pay Now" present: ${hasPayNow} | paid message: ${paidMessageShown}`);
      console.log(`  - Amount Due: "${amountDue}" (${money(amountDue)}) | backend InvoiceTotal#1: "${invoice.invoiceTotal}"`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Verification - paid EUR invoice (Amount Due 0)');
      ...
      expect(invoice.invoiceTotal, 'The invoice should be in EUR (EUR / euro symbol in the total)').toMatch(/EUR|€/i);
      console.log('✅ The Reseller sees a paid EUR-currency invoice with Amount Due 0');
    });
  });
});
