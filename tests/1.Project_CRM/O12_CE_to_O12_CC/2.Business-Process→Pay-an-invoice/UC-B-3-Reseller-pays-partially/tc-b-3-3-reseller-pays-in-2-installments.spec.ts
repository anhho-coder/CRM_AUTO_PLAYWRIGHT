 ...
 *    pay it in 2 installments (1st partial with "Keep open", 2nd pays off the remaining balance) and
 *    verify the invoice ends Paid - Amount Due $0 - with exactly 2 payment records of Installment#1/#2.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.3\.3:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *    19. Split into 2 installments: Installment#1 = round(InvoiceTotal#1 / 2, 2);
 *        Installment#2 = InvoiceTotal#1 - Installment#1 (closes the invoice).
 *
 ...
 *  - "Keep open" only for the partial 1st installment; the 2nd = remaining balance closes the invoice
 *    (no Payment Difference). "Actually Received($)" is set per payment (it does not auto-fill).
 */
...
test.describe('TC.-B.3.3 - Reseller pays partially (2 installments)', () => {
  ...
  test('TC.-B.3.3: Verify Reseller pays partially in 2 installments', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.3.3 ${compactDateTime}`;
    let setup = { oppUrl: '', invoiceUrl: '', invoiceNumber: '', invoiceTotal: 0 };
    let i1 = 0, i2 = 0;

    // Register one installment: reload -> Register Payment -> set amount -> (Keep open) -> set Actually
    // Received -> Validate. POM-pure (only page-object + flow calls).
    ...
    await test.step('Pre-condition #2 - Step 19: Split InvoiceTotal#1 into 2 installments (2nd = exact remainder)', async () => {
      i1 = round2(setup.invoiceTotal / 2);
      i2 = round2(setup.invoiceTotal - i1);
      console.log(`  - InvoiceTotal#1 = ${setup.invoiceTotal} | Installment#1 = ${i1} | Installment#2 = ${i2}`);
    ...
    await test.step('Steps to reproduce - Step 3: Register a payment of Installment#2 (the remaining balance, Actually Received = Installment#2), then Validate', async () => {
      ...
      console.log(`✓ Installment#2 = ${i2.toFixed(2)} paid (closes the invoice)`);
    });

    await test.step('Verification Point: Invoice Paid, Amount Due $0, Actually Received = InvoiceTotal#1, and exactly 2 payment records = Installment#1/#2', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-B.3.3 - Paid Invoice + 2 payment records');
      ...
      console.log('✅ 2-installment partial payment verified: invoice Paid, Amount Due $0, 2 records, Actually Received = InvoiceTotal#1');
    });
  });
});
