 ...
 *    pay it in 2 UNEVEN installments - a small 10% partial ("Keep open") then the 90% remainder - and
 *    verify the invoice ends Paid - Amount Due $0 - with 2 payment records of the small/large amounts.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.3\.6:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *    19. Uneven split: Installment#1 = round(InvoiceTotal#1 * 0.10, 2) (small);
 *        Installment#2 = InvoiceTotal#1 - Installment#1 (large remainder, closes the invoice).
 *
 *  Steps to reproduce  (as Faye - accountant):
 *    1. Use the account of Faye (accountant) to login successful, then open Invoice#1 URL
 *    2. Register a small partial payment of Installment#1 with "Keep open" (Actually Received = Installment#1), then Validate
 *    3. Register a payment of Installment#2 (the remaining balance, Actually Received = Installment#2), then Validate
 *
 *  Verification Point:
 *    - Invoice#1 state = "Paid"
 *    - Amount Due = $0
 *    - Actually Received = InvoiceTotal#1
 *    - In the "Payments" tab there are exactly 2 payment records, with amounts = Installment#1 (small), Installment#2 (large)
 * ===========================================================================
 *
 *  IMPLEMENTATION NOTES
 *  - Financial chain; posted/paid invoice cannot be cleanly deleted -> cleanup SKIPPED (O12 convention).
 *  - Exercises an uneven split (small-then-large). "Keep open" only for the small 1st installment; the
 *    2nd = remaining balance closes the invoice. "Actually Received($)" is set per payment.
 */
...
test.describe('TC.-B.3.6 - Reseller pays partially (uneven small-then-large split)', () => {
  ...
  test('TC.-B.3.6: Verify Reseller pays partially with an uneven split', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.3.6 ${compactDateTime}`;
    ...
    await test.step('Pre-condition #2 - Step 19: Uneven split - Installment#1 = round(InvoiceTotal#1 * 0.10, 2); Installment#2 = remainder', async () => {
      i1 = round2(setup.invoiceTotal * 0.10);
      if (i1 <= 0) i1 = round2(setup.invoiceTotal / 10); // guard for tiny totals
      i2 = round2(setup.invoiceTotal - i1);
      console.log(`  - InvoiceTotal#1 = ${setup.invoiceTotal} | Installment#1 (small) = ${i1} | Installment#2 (large) = ${i2}`);
      expect(round2(i1 + i2)).toBeCloseTo(setup.invoiceTotal, 2);
      expect(i1).toBeGreaterThan(0);
      expect(i2, 'The remainder should be larger than the small first installment').toBeGreaterThan(i1);
    });
    ...
    await test.step('Steps to reproduce - Step 2: Register a small partial payment of Installment#1 with "Keep open" (Actually Received = Installment#1), then Validate', async () => {
      await payInstallment(i1, true);
      console.log(`✓ Installment#1 (small) = ${i1.toFixed(2)} paid (Keep open)`);
    });

    await test.step('Steps to reproduce - Step 3: Register a payment of Installment#2 (the remaining balance, Actually Received = Installment#2), then Validate', async () => {
      await payInstallment(i2, false);
      console.log(`✓ Installment#2 (large) = ${i2.toFixed(2)} paid (closes the invoice)`);
    });

    await test.step('Verification Point: Invoice Paid, Amount Due $0, Actually Received = InvoiceTotal#1, and exactly 2 payment records = Installment#1 (small) / #2 (large)', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-B.3.6 - Paid Invoice + uneven 2 payment records');

      expect(status, 'Invoice#1 should be "Paid"').toMatch(/Paid/i);
      expect(money(amountDue), 'Amount Due should be $0').toBeCloseTo(0, 2);
      expect(rowCount, 'The Payments tab should hold exactly 2 payment records').toBe(2);
      ...
      console.log('✅ Uneven small-then-large partial payment verified: invoice Paid, Amount Due $0, 2 records, Actually Received = InvoiceTotal#1');
    });
  });
});
