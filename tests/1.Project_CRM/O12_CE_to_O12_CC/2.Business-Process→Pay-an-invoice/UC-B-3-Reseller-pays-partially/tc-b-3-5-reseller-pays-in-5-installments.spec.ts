 ...
 *    pay it in 5 installments (first 4 partial with "Keep open", 5th pays off the remaining balance)
 *    and verify the invoice ends Paid - Amount Due $0 - with exactly 5 payment records.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.3\.5:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *    19. Split into 5 installments: #1..#4 = round(InvoiceTotal#1 / 5, 2);
 *        #5 = InvoiceTotal#1 - #1 - #2 - #3 - #4 (exact remainder, closes the invoice).
 *
 ...
 *    5. Register a partial payment in the Register Payment popup, then Validate:
 *          - Payment Amount       = Installment#4
 *          - Payment Difference   = Keep open
 *          - Actually Received($) = Installment#4
 *    6. Register a payment of the remaining balance in the Register Payment popup, then Validate:
 *          - Payment Amount       = Installment#5
 *          - Actually Received($) = Installment#5
 *
 *  Verification Point:
 *    - Invoice#1 state = "Paid"
 *    - Amount Due = $0
 *    - Actually Received = InvoiceTotal#1
 *    - In the "Payments" tab there are exactly 5 payment records, with amounts = Installment#1..#5
 * ===========================================================================
 *
 *  IMPLEMENTATION NOTES
 *  - Financial chain; posted/paid invoice cannot be cleanly deleted -> cleanup SKIPPED (O12 convention).
 *  - "Keep open" for installments #1..#4; #5 = remaining balance closes the invoice (no Payment
...
test.describe('TC.-B.3.5 - Reseller pays partially (5 installments)', () => {
  ...
  test('TC.-B.3.5: Verify Reseller pays partially in 5 installments', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.3.5 ${compactDateTime}`;
    ...
    await test.step(`Verification Point: Invoice Paid, Amount Due $0, Actually Received = InvoiceTotal#1, and exactly ${INSTALLMENT_COUNT} payment records = Installment#1..#${INSTALLMENT_COUNT}`, async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `TC.-B.3.5 - Paid Invoice + ${INSTALLMENT_COUNT} payment records`);
