 ...
 * ===========================================================================
 *  Test Case ID    : TC.-B.5.10
 *  Manual TC ID    : UC-B.5.10  (folder UC-B-5-Accountant-registers-a-manual-payment-wire-or-check)
 *  Jira            : (manual TC - Business Process > Pay an invoice)
 *  Automation-Type : new
 *  Automation-Date : 2026-06-29
 *
 *  Summary:
 *    As Thomas build a validated single-product Invoice; then as Faye (accountant) settle it with TWO
 *    manual installments using DIFFERENT journals - Installment#1 via "Bank Transfer" (wire, Keep open)
 *    and Installment#2 via "Bank Check" (closes the balance) - and verify the invoice ends Paid with
 *    Amount Due $0 and exactly 2 Payments-tab records, one per journal (Bank Transfer + Bank Check),
 *    with amounts = Installment#1 / Installment#2.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.5\.10:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *          Installment#1 = round(InvoiceTotal#1 / 2, 2); Installment#2 = remainder.
 *          Journal#1 = "Bank Transfer" (wire); Journal#2 = "Bank Check".
 *
 ...
 *    2. Register a partial payment of Installment#1, set:
 *          - Payment Journal      = Bank Transfer (wire)
 *          - Payment Difference   = Keep open
 *          - Actually Received($) = Installment#1
 *       then Validate
 *    3. Register a payment of Installment#2 (remaining balance), set:
 *          - Payment Journal      = Bank Check
 *          - Actually Received($) = Installment#2
 *       then Validate
 *
 *  Verification Point:
 *    1. Invoice#1 state = "Paid" and Amount Due = $0
 *    2. In the "Payments" tab there are exactly 2 payment records, one via "Bank Transfer" and one via
 *       "Bank Check", with amounts = Installment#1, Installment#2
 * ===========================================================================
 *
 *  IMPLEMENTATION NOTES
 *  - Financial chain; posted/paid invoice cannot be cleanly deleted -> cleanup SKIPPED (O12 convention).
 *  - Covers the "wire OR check" theme in one invoice: each installment uses a different manual journal;
 *    the chosen journal is recorded per payment (verified via the Payments-tab "Payment Journal" column).
 */

const SKIP_CLEANUP_OPP = true;
const JOURNAL_WIRE = 'Bank Transfer';
const JOURNAL_CHECK = 'Bank Check';

const money = (s: string | undefined | null): number => parseFloat((s || '').replace(/[^0-9.]/g, '')) || 0;
const round2 = (n: number): number => Math.round(n * 100) / 100;

test.describe('TC.-B.5.10 - Accountant settles an invoice with mixed manual journals (wire + check)', () => {
  ...
  test('TC.-B.5.10: Accountant settles an invoice with mixed manual journals (Bank Transfer + Bank Check)', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.5.10 ${compactDateTime}`;
    let setup = { oppUrl: '', invoiceUrl: '', invoiceNumber: '', invoiceTotal: 0 };
    let i1 = 0, i2 = 0;

    const payInstallmentViaJournal = async (amount: number, journal: string, keepOpen: boolean): Promise<void> => {
    ...
    await test.step('Pre-condition #1: Prepare the deal-registration Internal Note #1', async () => {
      console.log(`Pre-condition #1: Internal Note #1 prepared (Opp "${oppName}", Contact "${leadName}", Partner "${DEAL_REGISTRATION.partnerCompanyName}", Journals "${JOURNAL_WIRE}" + "${JOURNAL_CHECK}")`);
    ...
    await test.step('Pre-condition #2: As Thomas, create the deal-registration Opportunity and build a validated single-product Invoice (steps 1-19)', async () => {
      ...
      console.log(`  - InvoiceTotal#1 = ${setup.invoiceTotal} | i1 = ${i1} (${JOURNAL_WIRE}) | i2 = ${i2} (${JOURNAL_CHECK})`);
      expect(round2(i1 + i2)).toBeCloseTo(setup.invoiceTotal, 2);
      expect(i1, 'Installment#1 should be a positive partial (< total)').toBeGreaterThan(0);
      expect(i1).toBeLessThan(setup.invoiceTotal);
    ...
    await test.step(`Step 2: Register a partial payment of Installment#1: set Payment Journal = ${JOURNAL_WIRE} (wire), "Keep open" (Actually Received = Installment#1), then Validate`, async () => {
      await payInstallmentViaJournal(i1, JOURNAL_WIRE, true);
      console.log(`✓ Installment#1 = ${i1.toFixed(2)} paid via ${JOURNAL_WIRE} (Keep open)`);
    });

    await test.step(`Step 3: Register a payment of Installment#2 (remaining balance): set Payment Journal = ${JOURNAL_CHECK} (Actually Received = Installment#2), then Validate`, async () => {
      await payInstallmentViaJournal(i2, JOURNAL_CHECK, false);
      console.log(`✓ Installment#2 = ${i2.toFixed(2)} paid via ${JOURNAL_CHECK} (closes the invoice)`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - mixed wire + check installments recorded');
    });

    await test.step(`Verification Point: Invoice Paid + Amount Due $0; the Payments tab holds exactly 2 records - one via "${JOURNAL_WIRE}" and one via "${JOURNAL_CHECK}" - with amounts = Installment#1/#2`, async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `TC.-B.5.10 - Paid Invoice + mixed journals (${JOURNAL_WIRE} + ${JOURNAL_CHECK})`);
      ...
      const joined = journals.join(' | ');
      expect(joined, `One payment should use "${JOURNAL_WIRE}"`).toContain(JOURNAL_WIRE);
      expect(joined, `One payment should use "${JOURNAL_CHECK}"`).toContain(JOURNAL_CHECK);
      console.log(`✅ Mixed manual journals recorded: invoice Paid with one "${JOURNAL_WIRE}" + one "${JOURNAL_CHECK}" payment`);
    });
  });
});
