 ...
 * ===========================================================================
 *  Test Case ID    : TC.-B.5.4
 *  Manual TC ID    : UC-B.5.4   (folder UC-B-5-Accountant-registers-a-manual-payment-wire-or-check)
 ...
 *    register ONE full manual payment through the "Bank Remittance" Payment Journal (Payment Amount and
 *    Actually Received($) = InvoiceTotal#1, no Payment Difference), and verify the invoice ends Paid
 *    with Amount Due $0 and a single Payments-tab record whose Payment Journal = Bank Remittance and
 *    Actually Received($) = InvoiceTotal#1.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.5\.4:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *     3. In the popup, set Payment Journal = Bank Remittance
 ...
 *     3. In the "Payments" tab, the payment record has Payment Journal = Bank Remittance
 ...
 *  - "Bank Remittance" is one of the manual Payment Journals (see [[register-payment-journal-options]]);
 *    the journal is asserted applied, with getPaymentJournalOptions() logged for diagnosis.
 */

const SKIP_CLEANUP_OPP = true;
const JOURNAL = 'Bank Remittance';

const money = (s: string | undefined | null): number => parseFloat((s || '').replace(/[^0-9.]/g, '')) || 0;

test.describe('TC.-B.5.4 - Accountant registers a manual payment (Bank Remittance)', () => {
  ...
  test('TC.-B.5.4: Accountant registers a manual Bank Remittance payment for the full invoice', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.5.4 ${compactDateTime}`;
    ...
    await test.step('Step 6: (No "Payment Difference" appears, because the full amount is paid in one go)', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `TC.-B.5.4 - Register Payment (${JOURNAL}, full amount, no Payment Difference)`);
    });
    ...
    await test.step('Step 7: Click "Validate" -> the full payment is recorded', async () => {
      ...
      console.log('✓ Full Bank Remittance payment recorded');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - full Bank Remittance payment recorded');
    });
    ...
    await test.step('Verification Point 4: In the "Payments" tab, the payment record shows Actually Received($) = InvoiceTotal#1', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `TC.-B.5.4 - Invoice Paid (${JOURNAL}, Actually Received = ${setup.invoiceTotal})`);
      ...
      console.log(`✅ Manual Bank Remittance payment recorded: Amount Due = $0, state = Paid, Payment Journal = ${JOURNAL}, Actually Received = ${setup.invoiceTotal}`);
    });
  });
});
