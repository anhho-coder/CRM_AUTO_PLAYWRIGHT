 ...
 * ===========================================================================
 *  Test Case ID    : TC.-B.5.2
 *  Manual TC ID    : UC-B.5.2   (folder UC-B-5-Accountant-registers-a-manual-payment-wire-or-check)
 ...
 *    register ONE full manual WIRE payment through the "Bank Transfer" Payment Journal (Payment Amount
 *    and Actually Received($) = InvoiceTotal#1, no Payment Difference), and verify the invoice ends
 *    Paid with Amount Due $0 and a single Payments-tab record whose Payment Journal = Bank Transfer and
 *    Actually Received($) = InvoiceTotal#1.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.5\.2:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *     3. In the "Payments" tab, the payment record has Payment Journal = Bank Transfer
 *     4. In the "Payments" tab, the payment record shows Actually Received($) = InvoiceTotal#1
 * ===========================================================================
 *
 *  IMPLEMENTATION NOTES
 *  - Financial chain (Opportunity -> Deal Element -> Quotation -> Sales Order -> VALIDATED + PAID
 *    Invoice). A validated/paid Invoice cannot be cleanly deleted -> cleanup SKIPPED (O12 convention).
 ...
 *  - "Bank Transfer" is the wire-transfer journal (see [[register-payment-journal-options]]); the journal
...
const JOURNAL = 'Bank Transfer'; // wire-transfer Payment Journal

const money = (s: string | undefined | null): number => parseFloat((s || '').replace(/[^0-9.]/g, '')) || 0;

test.describe('TC.-B.5.2 - Accountant registers a manual payment (Bank Transfer / wire)', () => {
  ...
  test('TC.-B.5.2: Accountant registers a manual wire payment (Bank Transfer) for the full invoice', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.5.2 ${compactDateTime}`;
    ...
    await test.step('Step 6: (No "Payment Difference" appears, because the full amount is paid in one go)', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `TC.-B.5.2 - Register Payment (${JOURNAL}, full amount, no Payment Difference)`);
    });
    ...
    await test.step('Step 7: Click "Validate" -> the full payment is recorded', async () => {
      ...
      console.log('✓ Full wire payment recorded');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - full wire payment recorded');
    });
    ...
    await test.step('Verification Point 4: In the "Payments" tab, the payment record shows Actually Received($) = InvoiceTotal#1', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `TC.-B.5.2 - Invoice Paid (${JOURNAL}, Actually Received = ${setup.invoiceTotal})`);
      ...
      console.log(`✅ Manual wire payment recorded: Amount Due = $0, state = Paid, Payment Journal = ${JOURNAL}, Actually Received = ${setup.invoiceTotal}`);
    });
  });
});
