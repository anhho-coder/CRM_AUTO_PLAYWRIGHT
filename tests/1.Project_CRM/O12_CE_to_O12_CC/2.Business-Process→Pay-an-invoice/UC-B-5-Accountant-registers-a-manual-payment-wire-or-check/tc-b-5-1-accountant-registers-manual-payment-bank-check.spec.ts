 ...
 * ===========================================================================
 *  Test Case ID    : TC.-B.5.1
 *  Manual TC ID    : UC-B.5.1   (folder UC-B-5-Accountant-registers-a-manual-payment-wire-or-check)
 ...
 *    register ONE full manual payment through the "Bank Check" Payment Journal (Payment Amount and
 *    Actually Received($) = InvoiceTotal#1, no Payment Difference), and verify the invoice ends Paid
 *    with Amount Due $0 and a single Payments-tab record whose Payment Journal = Bank Check and
 *    Actually Received($) = InvoiceTotal#1.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-B\.5\.1:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *    convention cleanup is SKIPPED by default (the records are retained) - see TC.-B.3.11 / TC.-B.4.1.
 *  - Pre-condition #2 is pure setup (NOT what this TC verifies) so it is run as ONE grouped block via
 *    the shared helper createValidatedInvoiceForPartialPayment(...) (REQUIREMENT #1 grouping). It does
 *    NOT change the Payer - this TC is about the Accountant's manual bank-check payment, not the payer.
 *  - The journal label is asserted to be applied; if "Bank Check" is not an exact <select> option the
 *    run logs getPaymentJournalOptions() so the correct label is visible without a blind failure.
 */

const SKIP_CLEANUP_OPP = true; // validated + PAID invoice cannot be cleanly deleted -> retain (O12 convention)
const JOURNAL = 'Bank Check';  // Payment Journal for the manual (check) payment

const money = (s: string | undefined | null): number => parseFloat((s || '').replace(/[^0-9.]/g, '')) || 0;

test.describe('TC.-B.5.1 - Accountant registers a manual payment (bank check)', () => {
  ...
  test.beforeEach(async ({ context, page }, testInfo) => {
    ...
    // REQUIREMENT #3: boundary screenshot at the final step of beforeEach (guarded).
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    // REQUIREMENT #3: boundary screenshot at the final step of afterEach (guarded).
    await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'afterEach - teardown done').catch(() => {});
  });

  test('TC.-B.5.1: Accountant registers a manual payment (bank check) for the full invoice', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-B.5.1 ${compactDateTime}`;

    // Captured facts (Pre-condition #2) used by the Faye payment + verification.
    ...
    await test.step('Pre-condition #1: Prepare the deal-registration Internal Note #1', async () => {
      ...
      console.log(`  - Contact name (Internal Note "Name") : ${leadName}`);
    ...
    await test.step('Pre-condition #2: As Thomas, create the deal-registration Opportunity and build a validated single-product Invoice (steps 1-19)', async () => {
      ...
      // REQUIREMENT #3: boundary screenshot at the final step of Pre-condition #2.
    ...
    await test.step('Step 4: Set Payment Amount = InvoiceTotal#1 (the full amount due)', async () => {
      // The dialog defaults "Payment Amount" to the full amount due (= InvoiceTotal#1); set it explicitly.
    ...
    await test.step('Step 6: (No "Payment Difference" appears, because the full amount is paid in one go)', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-B.5.1 - Register Payment (Bank Check, full amount, no Payment Difference)');
    });
    ...
    await test.step('Step 7: Click "Validate" -> the full payment is recorded', async () => {
      ...
      // REQUIREMENT #3: boundary screenshot at the final step of Steps to reproduce.
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - full bank-check payment recorded');
    });
    ...
    await test.step('Verification Point 1: Amount Due = $0 (Total due = 0)', async () => {
      // Reload-and-poll: the Paid state + Amount Due can settle asynchronously after the payment posts.
    ...
    await test.step('Verification Point 4: In the "Payments" tab, the payment record shows Actually Received($) = InvoiceTotal#1', async () => {
      ...
      if (receivedValues.length === 0) {
        // Fallback to the single-cell reader if the column header text differs slightly.
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `TC.-B.5.1 - Invoice Paid (Bank Check, Actually Received = ${setup.invoiceTotal})`);
      ...
      console.log(`✅ Manual bank-check payment recorded: Amount Due = $0, state = Paid, Payment Journal = ${JOURNAL}, Actually Received = ${setup.invoiceTotal}`);
    });
  });
});
