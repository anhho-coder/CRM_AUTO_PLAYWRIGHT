...
/**
 * O12 CE Main-Business Smoke - Log note - Sale Order created
 * Test Case ID: CRM-12325_2.4.24
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Summary:
 *   Verify that creating a Deal Element writes the "Sale Order created" note in the chatter, and that
 *   a newly created Deal Element carries exactly its two creation notes.
 *
 * Source manual TC (pre-production): TC.Performance.1.1.4.24 "Deal Element log note - Sale Order created".
 *
 ...
 *   - Read back from crm-mig (sale.order 257456 / 257454): exactly 2 chatter messages, both authored by
 *     "Ho Quoc Anh" - the first body is "Sale Order created", the second is the tracked-value note.
 *
 * Pre-conditions:
 ...
 *  12. Read the chatter of the saved Deal Element.
 *
 * Verification Points:
 *   1. The chatter carries the "Sale Order created" note.
...
test.describe(`${TC} - Log note - Sale Order created`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12325_2.4.24');
  });

  test('CRM-12325_2.4.24: Verify the creation of a Deal Element writes the Sale Order created note', async ({ page }, testInfo) => {
    ...
    await test.step('Verification', async () => {
      const note = await dealElementPage.waitForChatterMessage(/Sale Order created/);
      const messages = await dealElementPage.getChatterMessages();
      console.log('Log note area:');
      messages.forEach((m, i) => console.log(`   [${i + 1}] ${m.replace(/\n/g, ' / ')}`));

      record('"Sale Order created" note', 'FOUND', note === null ? 'NOT FOUND' : 'FOUND');
      record('Number of log notes on a new Deal Element', 2, messages.length);
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Chatter of the new Deal Element`);

      expect(note, 'the chatter must carry the "Sale Order created" note').not.toBeNull();
      expect(messages.length, 'a newly created Deal Element carries its two creation notes').toBe(2);
    });
  });
});
