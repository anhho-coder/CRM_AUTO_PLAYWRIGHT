...
/**
 * O12 CE Main-Business Smoke - Order line subtotals
 * Test Case ID: CRM-12325_2.4.23
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Summary:
 *   Verify that with no discount and no tax the three subtotal columns of the order line all report
 *   Unit Price x Ordered Qty.
 *
 * Source manual TC (pre-production): TC.Performance.1.1.4.23 "Deal Element order line subtotals".
 *
 ...
 *   - Product 28 carries no tax on crm-mig (`taxes_id` empty), and line 377702 stores
 *     subtotal_before_discount = subtotal_after_all_discounts = price_subtotal = 329.00 for qty 1.
 *
 * Pre-conditions:
 ...
 *  12. Open the Order Lines tab of the saved Deal Element and read the priced cells of the row.
 *
 * Verification Points:
 *   1. Unit Price x Ordered Qty is greater than 0.
 *   2. Subtotal before discount = Unit Price x Ordered Qty.
 *   3. Subtotal After All Discounts = Unit Price x Ordered Qty (no discount applies).
...
test.describe(`${TC} - Order line subtotals`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    ...
    await sweepMigLeftoversAfterAll(browser, 'CRM-12325_2.4.23');
  });

  test('CRM-12325_2.4.23: Verify the three subtotal columns of the order line all report Unit Price x Ordered Qty', async ({ page }, testInfo) => {
    ...
    await test.step('Verification', async () => {
      ...
      record(
        ...
        String(subtotalBefore),
        Math.abs(subtotalBefore - expectedSubtotal) < 0.01
      );
      record(
        'Subtotal After All Discounts',
        String(expectedSubtotal),
        String(subtotalAfter),
        Math.abs(subtotalAfter - expectedSubtotal) < 0.01
      );
      record(
        'Subtotal',
        String(expectedSubtotal),
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Order line subtotals`);
