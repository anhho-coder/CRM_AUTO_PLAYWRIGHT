...
/**
 * O12 CE Main-Business Smoke - Payment Terms of the Deal Element
 * Test Case ID: CRM-12325_2.4.14
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Summary:
 *   Verify the Payment Terms field carries the "Immediate Payment" term selected before SAVE.
 *
 * Source manual TC (pre-production): TC.Performance.1.1.4.14 "Deal Element Payment Terms".
 *
 ...
 *  12. Read the Payment Terms back from the saved Deal Element.
 *
 * Verification Points:
...
test.describe(`${TC} - Payment Terms`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12325_2.4.14');
  });

  test('CRM-12325_2.4.14: Verify the Payment Term selected before SAVE is the one the saved Deal Element carries', async ({ page }, testInfo) => {
    ...
    await test.step('Verification', async () => {
      const paymentTerm = (await dealElementPage.getPaymentTermValue()) ?? '';

      record(
        'Payment Terms',
        O12CE_DATA.paymentTerm,
        paymentTerm.trim() === '' ? '(empty)' : paymentTerm.trim()
      );
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Payment Terms read back`);

      expect(paymentTerm.trim(), 'the Deal Element keeps the Payment Term selected before SAVE').toBe(
        O12CE_DATA.paymentTerm
      );
    });
  });
