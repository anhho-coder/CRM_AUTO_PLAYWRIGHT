...
/**
 * O12 CE Main-Business Smoke - Partner discounts of a Deal Element without partners
 * Test Case ID: CRM-12325_2.4.16
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Summary:
 *   Verify that an Opportunity without a Distributor and without a Reseller produces a Deal Element
 *   whose two partner discount rows both read 0.00 ( $ 0.00 ).
 *
 * Source manual TC (pre-production): TC.Performance.1.1.4.16 "Deal Element partner discounts default".
 *
 ...
 *   - Each discount row renders the percent AND the money it grants in ONE cell, and the row carries a
 *     hidden duplicate widget under the same field name - so the read is keyed on the LABEL
 *     (`getFieldRowTextByLabel`), never on the field node.
 *
 * Pre-conditions:
 ...
 *  12. Read the two partner discount rows of the saved Deal Element, keyed on their labels.
 *
 * Verification Points:
 *   1. Distributor Discount (%) reads 0.00 ( $ 0.00 ).
...
test.describe(`${TC} - Partner discounts default`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12325_2.4.16');
  });

  test('CRM-12325_2.4.16: Verify both partner discount rows stay at 0.00 when the Opportunity carries no partner', async ({ page }, testInfo) => {
    ...
    await test.step('Verification', async () => {
      const distributorDiscount = await dealElementPage.getFieldRowTextByLabel('Distributor Discount (%)');
      const resellerDiscount = await dealElementPage.getFieldRowTextByLabel('Reseller Discount (%)');

      record('Distributor Discount (%)', '0.00 ( $ 0.00 )', distributorDiscount);
      record('Reseller Discount (%)', '0.00 ( $ 0.00 )', resellerDiscount);
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Partner discount rows`);

      expect(distributorDiscount, 'no Distributor on the Opportunity means no distributor discount').toBe(
        '0.00 ( $ 0.00 )'
      );
      expect(resellerDiscount, 'no Reseller on the Opportunity means no reseller discount').toBe(
        '0.00 ( $ 0.00 )'
      );
    });
  });
