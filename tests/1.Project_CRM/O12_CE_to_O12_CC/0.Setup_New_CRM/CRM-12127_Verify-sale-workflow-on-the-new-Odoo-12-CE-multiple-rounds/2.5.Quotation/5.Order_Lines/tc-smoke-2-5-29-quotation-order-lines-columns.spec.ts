...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Order Lines columns of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12325_2.5.29
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify the Order Lines list of the Quotation carries exactly the 13 named pre-production
 *   columns, left to right.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.29 "Order Lines columns of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12325_2.5.29 <timestamp>
 ...
 *     6. Open the "Order Lines" tab and read its columns, left to right.
 *
 *   Verification:
 *     1. The Order Lines list carries exactly 13 named columns.
 *     2. The columns appear in the pre-production order.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.29:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.5.29';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the Order Lines columns',
  verify: 'Verification',
} as const;

/**
 * The Order Lines columns as PRE-PRODUCTION renders them on a Quotation, left to right -
 * the baseline this port asserts against. Columns Odoo renders without a caption (the drag
 * handle, the delete column) are not part of it.
 */
...
test.describe(`${TC} - Order Lines columns of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12325_2.5.29');
  });

  test(`${TC}: Verify the name, number and order of the Quotation Order Lines columns`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      ...
      columns = await quotationPage.getOrderLineColumns();
      console.log('  Order Lines columns read back, in order :');
      columns.forEach((c, i) => console.log(`      #${i + 1} ${c}`));

      record('Number of named columns', EXPECTED_COLUMNS.length, columns.length);
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation Order Lines columns`);

      expect(columns, 'the Order Lines list must carry exactly the expected columns, in order').toEqual(
        EXPECTED_COLUMNS
      );
    });
  });
