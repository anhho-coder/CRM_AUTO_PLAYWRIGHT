...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Payment Terms of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12325_2.5.23
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify the Quotation keeps the Payment Term that was selected on the Deal Element it was
 *   raised from.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.23 "Payment Terms of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12325_2.5.23 <timestamp>
 ...
 *     6. Read the "Payment Terms" field of the created Quotation.
 *
 *   Verification:
 *     1. The Payment Terms carries the term selected on the Deal Element.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.23:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.5.23';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the Payment Terms',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - Payment Terms of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12325_2.5.23');
  });

  test(`${TC}: Verify the Payment Terms on the Quotation matches the Payment Term selected on the Deal Element`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      console.log(`\n--- ${STEP.s13} ---`);
      paymentTerms = await quotationPage.getFieldDisplayValue('payment_term_id');

      console.log(`  - Payment Term on the Deal Element : "${O12CE_DATA.paymentTerm}"`);
      console.log(`  - Payment Terms on the Quotation   : "${paymentTerms}"`);

      record('Payment Terms carried over from the Deal Element', O12CE_DATA.paymentTerm, paymentTerms,
        paymentTerms.includes(O12CE_DATA.paymentTerm));
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation Payment Terms`);

      expect(paymentTerms, 'the Quotation must keep the Payment Term of the Deal Element').toContain(
        O12CE_DATA.paymentTerm
      );
    });
  });
});
