 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2.5.40:" --project=chromium
 */
...
test.describe('CRM-12325_2.5.40 - O12 CE smoke: Quotation Payer is proposed as email recipient', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12325_2.5.40');
  });

  test('CRM-12325_2.5.40: Verify the Quotation Payer is auto-filled as a recipient in the "Send by Email" composer on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12325_2.5.40';
