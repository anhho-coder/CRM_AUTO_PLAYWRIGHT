 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2.5.42:" --project=chromium
 */
...
test.describe('CRM-12325_2.5.42 - O12 CE smoke: the requester rejects his own Quotation', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12325_2.5.42');
  });

  test('CRM-12325_2.5.42: Verify the Quotation requester can REJECT his own Quotation on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12325_2.5.42';
