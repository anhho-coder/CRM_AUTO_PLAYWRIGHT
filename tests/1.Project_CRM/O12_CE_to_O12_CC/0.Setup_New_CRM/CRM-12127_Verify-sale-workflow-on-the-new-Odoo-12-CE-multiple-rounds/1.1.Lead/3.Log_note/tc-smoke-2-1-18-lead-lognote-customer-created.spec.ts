...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Log note - Customer created
 * Test Case ID: CRM-12325_2.1.18
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the customer-creation log note reports the automatically created Customer
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.18
 ...
 *   - Grounded on crm-mig lead 879682: the customer note carries the tracked keys Customer, Company Name, Partner Contact Email and Pricelist, and the system note "_contact_automatic_creation CREATE" is present - the same mechanism as pre-production.
 *   - Pre-production asserts the note names the entered Company Name; on O12 CE the Customer may be named after the e-mail domain, so the assertion compares the note against the Customer actually linked to the Lead.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying the customer-creation log note.
 *
 * Verification Points:
 *   1. A Customer is linked to the saved Lead.
 *   2. The customer-creation note names that Customer and carries a Pricelist.
 *   3. The system note "_contact_automatic_creation CREATE" is in the chatter.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12325_2.1.18 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.18:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.1.18';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the customer-creation log note',
};

test.describe(`${TC} - Log note - Customer created`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify the customer-creation log note reports the automatically created Customer`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      // Cleanup only (not an assertion).
      if (partner.partnerId && (partner.name === DATA.companyName || partner.name.includes(unique))) {
        registerMigRecord('res.partner', partner.partnerId, `Customer created by ${TC}`);
      }
      const note = await leadPage.getCustomerCreatedNote();
      expect(note, 'the customer-creation log note must exist in the chatter').not.toBeNull();
      const customer = note as { raw: string; fields: Record<string, string> };
      const systemNote = await leadPage.findChatterMessage('_contact_automatic_creation CREATE');
      console.log('==================== VERIFY ====================');
      console.log(customer.raw.split('\n').map((l) => '   | ' + l).join('\n'));
