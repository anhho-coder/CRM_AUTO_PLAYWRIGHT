...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Log note - Lead created
 * Test Case ID: CRM-12370_1.1.17
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the creation log note reports the entered Lead data
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.17
 ...
 *   - The pre-production note starts with the title "Opportunity created" and carries Company Name. On crm-mig the same event is stored as TRACKING VALUES with an empty body (lead 879682, message 16096633: Contact Name, Email, Phone, Expected Revenue, Active, Salesperson, Priority (System), Stage) and the Company Name arrives in the CUSTOMER note instead. The TC therefore locates the note by its tracked "Stage" key and asserts the keys O12 CE really carries.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying the creation log note.
 *
 * Verification Points:
 *   1. The chatter carries the creation note (tracked Stage).
 *   2. The note reports the entered Contact Name and Email.
 *   3. The note reports Stage = New and carries the system keys Priority (System) and Expected Revenue.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.1.17 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.17:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.1.17';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the creation log note',
};

test.describe(`${TC} - Log note - Lead created`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify the creation log note reports the entered Lead data`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const note = await leadPage.getOpportunityCreatedNote();
      expect(note, 'the "Opportunity created" log note must exist in the chatter').not.toBeNull();
      const created = note as { raw: string; fields: Record<string, string> };
      console.log('==================== VERIFY ====================');
