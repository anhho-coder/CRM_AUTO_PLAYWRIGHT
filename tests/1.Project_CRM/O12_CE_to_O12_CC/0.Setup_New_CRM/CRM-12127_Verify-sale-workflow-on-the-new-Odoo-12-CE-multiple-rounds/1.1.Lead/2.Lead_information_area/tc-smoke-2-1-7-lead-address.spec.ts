...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Address
 * Test Case ID: CRM-12325_2.1.7
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Address block on the saved Lead matches the entered values
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.7
 ...
 *   - `Texas (US)` is the display name of res.country.state id 52 on crm-mig - verified over XML-RPC, identical to pre-production.
 *
 * Pre-conditions:
 ...
 *
 * Verification Points:
 *   1. Street, State and Country on the saved Lead keep the entered values.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12325_2.1.7 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.7:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.1.7';
...
test.describe(`${TC} - Address`, () => {
  ...
  test(`${TC}: Verify the Address block on the saved Lead matches the entered values`, async ({ page }, testInfo) => {
