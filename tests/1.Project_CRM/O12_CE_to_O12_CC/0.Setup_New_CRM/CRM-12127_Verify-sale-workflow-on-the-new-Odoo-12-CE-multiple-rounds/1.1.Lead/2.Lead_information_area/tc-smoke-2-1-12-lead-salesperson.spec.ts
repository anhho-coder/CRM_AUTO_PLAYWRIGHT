 ...
 *   - Owner pair grounded on crm-mig: Sales Team "CMR" (crm.team 36) + Salesperson "Bear Lin" (res.users 59), who owns 1,308 leads in that team. Pre-production uses CMR + Sergio Yalovik.
 *
 * Pre-conditions:
 ...
 * carries the marker "TEST CRM-12325_2.1.12 <timestamp>" so the afterAll sweep can find it again even when
...
test.describe(`${TC} - Salesperson`, () => {
  ...
  test(`${TC}: Verify the Salesperson entered at creation is kept on the saved Lead`, async ({ page }, testInfo) => {
    ...
    const OWNER = { team: 'CMR', salesperson: 'Bear Lin' };
    let leadId = '';
