...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Create manually
 * Test Case ID: CRM-12325_2.1.15
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Create manually flag keeps the FALSE state set at creation
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.15
 ...
 *   - `is_create_manual` is the same field name on O12 CE as on pre-production - verified over XML-RPC.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying Create manually.
 *
 * Verification Points:
 *   1. The saved Lead keeps Create manually = FALSE.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12325_2.1.15 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.15:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.1.15';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying Create manually',
};

test.describe(`${TC} - Create manually`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify the Create manually flag keeps the FALSE state set at creation`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const createManually = await leadPage.getCreateManuallyChecked();
      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - Create manually on the saved Lead:');
