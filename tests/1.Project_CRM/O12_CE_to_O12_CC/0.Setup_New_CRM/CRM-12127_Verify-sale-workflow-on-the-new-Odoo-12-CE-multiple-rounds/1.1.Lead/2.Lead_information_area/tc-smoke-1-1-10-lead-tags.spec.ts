...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Tags
 * Test Case ID: CRM-12370_1.1.10
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Tag set on the Lead is saved and logged in the chatter
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.10
 ...
 *   - The tag "Trial download" exists on crm-mig as crm.lead.tag id 763 - verified over XML-RPC.
 *
 * Pre-conditions:
 ...
 *
 * Verification Points:
 *   1. The saved Lead carries exactly the tag "Trial download".
 *   2. The chatter logs "Tags CREATE changed" naming the new tag.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.1.10 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.1\\.10:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.1.10';
...
test.describe(`${TC} - Tags`, () => {
  ...
  test(`${TC}: Verify the Tag set on the Lead is saved and logged in the chatter`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s3, async () => {
      ...
      await leadPage.addTag('Trial download');
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const tags = await leadPage.getTagList();
      const note = await leadPage.waitForChatterMessage('Tags CREATE changed');
      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - Tags on the saved Lead:');
