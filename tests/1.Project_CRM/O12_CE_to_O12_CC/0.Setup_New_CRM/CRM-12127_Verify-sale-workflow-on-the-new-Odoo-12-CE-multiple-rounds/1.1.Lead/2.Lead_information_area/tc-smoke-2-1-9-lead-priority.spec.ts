...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Priority applied by the system
 * Test Case ID: CRM-12325_2.1.9
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Priority the system applies to a new Lead is shown on the form and logged
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.9
 ...
 *   - On crm-mig the system may log the Priority in TWO tracked steps (lead 879682: `-> Very High`, then `Very High -> Medium High`), where pre-production logs a single `Low -> Medium High`. The TC therefore asserts the FINAL state of the star widget plus the presence of a `Priority (System)` note, not the from-value of the first note.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Waiting for the Priority the system applies and verifying it.
 *
 * Verification Points:
 *   1. The chatter carries a "Priority (System)" tracking note.
 *   2. The Priority star widget ends on "Medium High" (2 of 4 stars filled).
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12325_2.1.9 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.9:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.1.9';
...
const STEP = {
  ...
  s5: 'Step 5: Waiting for the Priority the system applies and verifying it',
};

test.describe(`${TC} - Priority applied by the system`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify the Priority the system applies to a new Lead is shown on the form and logged`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      // Poll duration is the only adaptation (wall-clock): 3 min instead of the 8-min pre-prod
      // budget, because login + create + save already cost ~6 min of the test budget on crm-mig.
      // `^` with NO `m` flag - an unanchored pattern matches the "Priority (System): Low" line
      // INSIDE the creation note and would return instantly.
      const PRIORITY_POLL_MS = 180000;
      const note = await leadPage.waitForChatterMessage(/^Priority \(System\): /, PRIORITY_POLL_MS);
      expect(note, 'the chatter must log the Priority the system applies').not.toBeNull();
      const change = leadPage.splitTrackedChange(
        leadPage.parseLogNoteFields(note as string)['Priority (System)']
      );
      const priority = await leadPage.getPriority();
      console.log('==================== VERIFY ====================');
      console.log(`  chatter note  : ${(note as string).replace(/\n/g, ' | ')}`);
      console.log(`  logged change : "${change.from}" -> "${change.to}"`);
