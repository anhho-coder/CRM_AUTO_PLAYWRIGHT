...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Email
 * Test Case ID: CRM-12370_1.1.6
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Email on the saved Lead matches the entered value
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.6
 ...
 *
 * Verification Points:
 *   1. The saved Email (`email_from`) equals the entered value.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.1.6 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.6:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.1.6';
...
test.describe(`${TC} - Email`, () => {
  ...
  test(`${TC}: Verify the Email on the saved Lead matches the entered value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - Email on the saved Lead:');
