...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Timezone
 * Test Case ID: CRM-12325_2.1.16
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify how the Timezone fields are exposed on the O12 CE Lead form
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.16
 ...
 *   - `tz` and `tz_offset` exist on crm.lead on crm-mig but the arch walk of 2026-09-17 did not find them declared in the Lead form view, so this TC is expected to FAIL until they are back on the view - that failure is the finding.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying Timezone.
 *
 * Verification Points:
 *   1. The Lead is saved.
 *   2. Neither `tz` nor `tz_offset` is rendered on the O12 CE Lead form (parity gap).
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12325_2.1.16 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.16:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.1.16';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying Timezone',
};

test.describe(`${TC} - Timezone`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify how the Timezone fields are exposed on the O12 CE Lead form`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const tz = await leadPage.readFieldValue('tz');
      const tzOffset = await leadPage.readFieldValue('tz_offset');
      console.log('==================== VERIFY ====================');
