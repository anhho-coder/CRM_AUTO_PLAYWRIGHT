 ...
 * carries the marker "TEST CRM-12325_2.1.8 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.8:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.1.8';
...
test.describe(`${TC} - Lead Form`, () => {
  ...
  test(`${TC}: Verify the Lead Form on the saved Lead matches the selected value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      // getLeadFormValue() is the path the proven smoke spec CRM-12325_2.1.1 uses after save.
