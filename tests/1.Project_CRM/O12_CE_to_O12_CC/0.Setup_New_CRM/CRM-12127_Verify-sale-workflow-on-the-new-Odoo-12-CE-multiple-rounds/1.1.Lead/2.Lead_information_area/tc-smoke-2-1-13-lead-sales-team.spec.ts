...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Sales Team
 * Test Case ID: CRM-12325_2.1.13
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Sales Team entered at creation is kept on the saved Lead
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.13
 ...
 *   - Same cron-cadence deviation as CRM-12325_2.1.12: the O12 CE assignment job cannot be awaited inside a smoke run, so the TC verifies the Sales Team entered at creation is stored and rendered.
 *   - "CMR" is the DISPLAY name of crm.team 36 on crm-mig (its `name` field reads "CM_Channel Management") - verified over XML-RPC.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying the Sales Team.
 *
 * Verification Points:
 *   1. The saved Lead keeps the Sales Team entered at creation.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12325_2.1.13 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.13:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.1.13';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Sales Team',
};
...
test.describe(`${TC} - Sales Team`, () => {
  ...
  test(`${TC}: Verify the Sales Team entered at creation is kept on the saved Lead`, async ({ page }, testInfo) => {
