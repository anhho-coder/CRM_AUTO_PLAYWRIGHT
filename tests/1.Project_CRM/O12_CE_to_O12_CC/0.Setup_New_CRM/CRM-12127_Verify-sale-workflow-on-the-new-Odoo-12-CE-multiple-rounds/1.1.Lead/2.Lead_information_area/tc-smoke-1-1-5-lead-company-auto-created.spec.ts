...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Customer created automatically
 * Test Case ID: CRM-12370_1.1.5
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Customer is created automatically and linked to the saved Lead
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.5
 ...
 *   - The automatic contact creation runs on O12 CE (`_contact_automatic_creation CREATE` is logged on crm-mig), but the Customer it creates may be named after the e-mail DOMAIN rather than the entered Company Name - crm-mig lead 879682 shows the domain form. The assertion accepts both documented derivations and nothing else.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying the Customer created automatically.
 *
 * Verification Points:
 *   1. A res.partner is linked to the saved Lead.
 *   2. The Customer link points at model=res.partner.
 *   3. The created Customer is named after the entered Company Name or the e-mail domain.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.1.5 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.1\\.5:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.1.5';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Customer created automatically',
};

test.describe(`${TC} - Customer created automatically`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify the Customer is created automatically and linked to the saved Lead`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const companyNameBefore = await leadPage.readFieldValue('partner_name');
      const partner = await leadPage.waitForCompanyPartner();
      // Cleanup only (not an assertion): register the Customer this run created so teardown removes
      // it. When O12 CE does not expose the link, the afterAll sweep still finds it by name marker.
      ...
      console.log('==================== VERIFY ====================');
      console.log(`  Company Name before linking : "${companyNameBefore}"`);
      console.log(`  Company link                : "${partner.name}"`);
