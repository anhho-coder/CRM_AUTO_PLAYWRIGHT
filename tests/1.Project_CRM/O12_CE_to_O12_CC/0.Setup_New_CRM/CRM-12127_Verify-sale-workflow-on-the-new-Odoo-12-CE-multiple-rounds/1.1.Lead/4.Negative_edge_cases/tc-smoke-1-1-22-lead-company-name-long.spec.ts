...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Company Name - 200 characters
 * Test Case ID: CRM-12370_1.1.22
 * Automation-Type: new
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.22
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying Company Name - 200 characters.
 *
 * Verification Points:
 *   1. The entered Company Name is exactly 200 characters.
 *   2. The saved Company Name is 200 characters and equals the entered one.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.1.22 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.1\\.22:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.1.22';
...
test.describe(`${TC} - Company Name - 200 characters`, () => {
  ...
  test(`${TC}: Verify a 200-character Company Name is stored without truncation`, async ({ page }, testInfo) => {
    ...
    const DATA = {
      ...
      companyName: `TEST ${TC} ${unique} `.padEnd(200, 'X').slice(0, 200),
