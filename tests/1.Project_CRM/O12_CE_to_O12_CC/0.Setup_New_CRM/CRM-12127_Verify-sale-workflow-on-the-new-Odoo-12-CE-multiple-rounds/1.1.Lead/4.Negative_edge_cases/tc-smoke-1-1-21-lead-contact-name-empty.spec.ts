...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Contact Name - left empty
 * Test Case ID: CRM-12370_1.1.21
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify a Lead can be saved with an empty Contact Name
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.21
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying Contact Name - left empty.
 *
 * Verification Points:
 *   1. The Lead is saved although Contact Name was left empty.
 *   2. Contact Name reads empty and the other entered values are kept.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.1.21 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.21:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.1.21';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying Contact Name - left empty',
};

test.describe(`${TC} - Contact Name - left empty`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify a Lead can be saved with an empty Contact Name`, async ({ page }, testInfo) => {
