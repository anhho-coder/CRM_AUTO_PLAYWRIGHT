...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Company Name - special characters
 * Test Case ID: CRM-12370_1.1.20
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify a Company Name containing special characters is stored exactly as entered
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.20
 ...
 *   - The special-character string keeps the "TEST <TC id>" prefix so the afterAll sweep can still find the record by its marker.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying Company Name - special characters.
 *
 * Verification Points:
 *   1. The saved Company Name equals the entered string, character for character.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.1.20 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.20:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.1.20';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying Company Name - special characters',
};

test.describe(`${TC} - Company Name - special characters`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify a Company Name containing special characters is stored exactly as entered`, async ({ page }, testInfo) => {
    ...
    const DATA = {
      ...
      companyName: `TEST ${TC} & Co. "O'Brien" <${unique}>`,
