...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Priority default on the CREATE form
 * Test Case ID: CRM-12325_2.1.25
 * Automation-Type: new
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.25
 ...
 *   - `priority_new` is the visible star widget on O12 CE too (5 selection values Low / Medium / Medium High / High / Very High = Low + 4 stars) - verified over XML-RPC.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying the Priority of the freshly opened CREATE form.
 *
 * Verification Points:
 *   1. The freshly opened CREATE form shows Priority = Low with 0 of 4 stars filled.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12325_2.1.25 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.25:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.1.25';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Priority of the freshly opened CREATE form',
};
...
test.describe(`${TC} - Priority default on the CREATE form`, () => {
  ...
  test(`${TC}: Verify the Priority a freshly opened CREATE form starts on is Low with no star filled`, async ({ page }, testInfo) => {
