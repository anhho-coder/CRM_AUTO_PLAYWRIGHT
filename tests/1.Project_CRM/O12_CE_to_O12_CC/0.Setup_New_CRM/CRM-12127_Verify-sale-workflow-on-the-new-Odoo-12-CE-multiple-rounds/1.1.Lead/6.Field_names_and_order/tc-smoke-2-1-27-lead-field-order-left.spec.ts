...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Field names and order - left column
 * Test Case ID: CRM-12325_2.1.27
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the field names and their order in the left column of the Lead information area
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.27
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying the field names and order, left column.
 *
 * Verification Points:
 *   1. The Lead information area renders a left column.
 *   2. Company Name, Contact Name, Street, State, Country and Website are all rendered.
 *   3. They render in that relative order.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12325_2.1.27 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.1\\.27:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.1.27';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the field names and order, left column',
};

test.describe(`${TC} - Field names and order - left column`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify the field names and their order in the left column of the Lead information area`, async ({ page }, testInfo) => {
...
--- ${STEP.s5} ---`);
      ...
      const EXPECTED = [
        { label: 'Company Name', field: 'partner_name' },
        { label: 'Contact Name', field: 'contact_name' },
        { label: 'Company Name', field: 'partner_name' },
        { label: 'Address', field: 'street' },
        { label: 'Website', field: 'website' },
        { label: 'Country Lead', field: 'source_country_lead' },
        { label: 'Lead Form', field: 'lead_form' },
        { label: 'Salesperson', field: 'user_id' },
        { label: 'Create manually', field: 'is_create_manual' },
        { label: 'Sales Team', field: 'team_id' },
        { label: 'Lead Source', field: 'lead_source' },
        { label: 'Reseller', field: 'reseller_id' },
        { label: 'Distributor', field: 'distributor_id' },
      ];
      ...
      const actual = rows.left;
      console.log('==================== VERIFY ====================');
      console.log(`  left column of the Lead information area: expected ${EXPECTED.length} rows, got ${actual.length}`);
      actual.forEach((r, i) => {
        const e = EXPECTED[i];
