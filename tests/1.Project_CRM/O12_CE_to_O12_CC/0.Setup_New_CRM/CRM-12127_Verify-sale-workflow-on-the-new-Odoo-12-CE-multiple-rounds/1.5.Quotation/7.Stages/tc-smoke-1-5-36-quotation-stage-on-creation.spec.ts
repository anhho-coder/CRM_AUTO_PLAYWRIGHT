...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Stage of a newly created Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.36
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify a freshly created Quotation is highlighted on QUOTATION - the first stage of the
 *   statusbar - and that the server holds the same state.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.36 "Stage of a newly created Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.36 <timestamp>
 ...
 *     6. Read the highlighted stage of the new Quotation and the state the server holds.
 *
 *   Verification:
 *     1. The highlighted stage is QUOTATION.
 *     2. The highlighted stage is the first one of the statusbar.
 *     3. The server holds the Quotation state.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\.5\.36:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.36';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the stage of the new Quotation',
  verify: 'Verification',
} as const;

/** The stage a freshly created Quotation sits on. */
...
test.describe(`${TC} - Stage of a newly created Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.36');
  });

  test(`${TC}: Verify a newly created Quotation sits on the first stage of the statusbar`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      ...
      activeStage = await quotationPage.getActiveStatusBarStage();
      serverStatus = await quotationPage.getQuotationStatusFromServer();

      console.log(`  - Statusbar stages : ${stages.join(' | ')}`);
      console.log(`  - Highlighted      : ${activeStage}`);
      console.log(`  - Server status    : ${serverStatus}`);

      record('Highlighted stage', EXPECTED_STAGE, activeStage);
      record('Highlighted stage is the first one', stages[0] || '(none)', activeStage);
      record('Status held by the server', 'Quotation', serverStatus, /quotation/i.test(serverStatus));
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation stage on creation`);

      expect(activeStage, 'a new Quotation must sit on the first stage').toBe(EXPECTED_STAGE);
      expect(activeStage, 'the highlighted stage must be the first one of the statusbar').toBe(stages[0]);
      expect(serverStatus, 'the server must hold the Quotation state').toMatch(/quotation/i);
    });
  });
});
