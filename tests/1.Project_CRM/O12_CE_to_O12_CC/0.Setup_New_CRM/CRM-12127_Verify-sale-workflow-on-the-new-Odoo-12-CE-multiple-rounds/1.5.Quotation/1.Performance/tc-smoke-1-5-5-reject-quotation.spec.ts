...
test.describe('CRM-12370_1.5.5 - O12 CE smoke: reject a Quotation', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.5');
  });

  test('CRM-12370_1.5.5: Verify a Quotation can be rejected on the O12 CE Migration server', async ({ page, browser }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12370_1.5.5';
