 ...
 *
 * Source: the single-actor variant of TC.Performance.1.1.5.4 / CRM-12370_1.5.4 "Approve Quotation".
 * 2.5.4 has the manager (Max) approve from a second browser session; this one has the requester do
 * it alone. Sibling specs 2.5.42 (self REJECT), 2.5.43 (self Cancel) and 2.5.44 (self Duplicate) cover
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1.5.41:" --project=chromium
 */
...
test.describe('CRM-12370_1.5.41 - O12 CE smoke: the requester approves his own Quotation', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.41');
  });

  test('CRM-12370_1.5.41: Verify the Quotation requester can APPROVE his own Quotation on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12370_1.5.41';
