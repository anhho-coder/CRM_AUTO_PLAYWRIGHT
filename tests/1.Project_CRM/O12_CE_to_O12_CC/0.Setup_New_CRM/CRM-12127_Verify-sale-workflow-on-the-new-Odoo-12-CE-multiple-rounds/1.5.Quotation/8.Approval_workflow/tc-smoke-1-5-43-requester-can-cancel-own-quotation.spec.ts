 ...
 *
 * Sibling specs (all four buttons work for the requester): 2.5.41 (self APPROVE), 2.5.42 (self
 * REJECT), 2.5.44 (self Duplicate).
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1.5.43:" --project=chromium
 */
...
test.describe('CRM-12370_1.5.43 - O12 CE smoke: the requester can cancel his own pending-approval Quotation', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.43');
  });

  test('CRM-12370_1.5.43: Verify the Quotation requester can Cancel his own pending-approval Quotation on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12370_1.5.43';
    ...
    await test.step('Steps run - Step 3: Read the Quotation state back', async () => {
      ...
      // Server-fresh read, same reasoning as 2.5.41/2.5.42. The statusbar CAN show "Cancelled" (Odoo
