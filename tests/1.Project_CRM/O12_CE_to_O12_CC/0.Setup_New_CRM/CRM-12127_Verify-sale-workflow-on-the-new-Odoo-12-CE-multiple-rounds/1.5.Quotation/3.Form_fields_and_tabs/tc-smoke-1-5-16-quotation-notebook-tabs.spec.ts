...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Notebook tabs of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.16
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify the Quotation notebook carries exactly the 4 pre-production tabs, left to right.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.16 "Notebook tabs of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.16 <timestamp>
 ...
 *     6. Read the notebook tabs of the Quotation, left to right.
 *
 *   Verification:
 *     1. The notebook carries exactly 4 tabs.
 *     2. The tabs read Order Lines, Optional Products, Other Information, Approvals - in that order.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.16:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.16';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the notebook tabs',
  verify: 'Verification',
} as const;

/** The Quotation notebook tabs, left to right, as PRE-PRODUCTION renders them. */
const EXPECTED_TABS = [
  'Order Lines',
  'Optional Products',
  'Other Information',
...
test.describe(`${TC} - Notebook tabs of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.16');
  });

  test(`${TC}: Verify the name, number and order of the Quotation notebook tabs`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      console.log(`\n--- ${STEP.s13} ---`);
      tabs = await quotationPage.getNotebookTabs();
      console.log('  Notebook tabs read back, in order :');
      tabs.forEach((t, i) => console.log(`      #${i + 1} ${t}`));

      record('Number of notebook tabs', EXPECTED_TABS.length, tabs.length);
      EXPECTED_TABS.forEach((expected, i) => {
        record(`Tab #${i + 1}`, expected, tabs[i] === undefined ? '(missing)' : tabs[i]);
      });
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation notebook tabs`);

      expect(tabs, 'the notebook must carry exactly the expected tabs, in order').toEqual(EXPECTED_TABS);
    });
  });
});
