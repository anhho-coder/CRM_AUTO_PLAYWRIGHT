...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - CANCEL button on the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.13
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify CANCEL puts the Quotation into the cancelled state, consumes the CANCEL button and
 *   offers "Set to Quotation" instead.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.13 "CANCEL button on the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.13 <timestamp>
 ...
 *     6. Press "CANCEL" and read the resulting state of the Quotation.
 *
 *   Verification:
 *     1. The Quotation ends up cancelled.
 *     2. A cancelled Quotation offers "Set to Quotation".
 *     3. The CANCEL button is consumed by the cancellation.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.13:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.13';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Press CANCEL and verify the cancelled state',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - CANCEL button on the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.13');
  });

  test(`${TC}: Verify the CANCEL button cancels the Quotation`, async ({ page }, testInfo) => {
    ...
    let cancelPopup = '';
    let cancelled = false;
    let setToQuotationVisible = false;
    let cancelButtonGone = false;
    ...
    await test.step(STEP.s13, async () => {
      console.log(`\n--- ${STEP.s13} ---`);
      const cancel = await quotationPage.clickCancelQuotation();
      cancelPopup = cancel.popupText;
      const verdict = await quotationPage.verifyQuotationCancelled();
      cancelled = verdict.cancelled;
      setToQuotationVisible = verdict.setToQuotationVisible;
      cancelButtonGone = verdict.cancelButtonGone;

      console.log(`  Quotation cancelled        : ${cancelled}`);
      console.log(`  "Set to Quotation" offered : ${setToQuotationVisible}`);
      console.log(`  CANCEL button gone         : ${cancelButtonGone}`);
      console.log(`  Popup raised by CANCEL     : ${cancelPopup || '(none)'}`);

      record('Quotation cancelled', 'true', String(cancelled));
      record('"Set to Quotation" offered', 'true', String(setToQuotationVisible));
      record('CANCEL button gone', 'true', String(cancelButtonGone));
      record('Popup raised by CANCEL', '(none or handled)', cancelPopup === '' ? '(none)' : cancelPopup, true);
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation cancelled`);

      expect(setToQuotationVisible, 'a cancelled Quotation offers "Set to Quotation"').toBe(true);
      expect(cancelButtonGone, 'CANCEL must be consumed by the cancellation').toBe(true);
      expect(cancelled, 'the Quotation must end up cancelled').toBe(true);
    });
  });
});
