...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - SEND PRO-FORMA INVOICE button on the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.11
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify SEND PRO-FORMA INVOICE opens the mail composer already addressed to the Payer with
 *   the Quotation number in the subject, and that cancelling it leaves the stage untouched.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.11 "SEND PRO-FORMA INVOICE button on the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.11 <timestamp>
 ...
 *     6. Press "SEND PRO-FORMA INVOICE", read the composer recipients and subject, then Cancel it.
 *
 *   Verification:
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.11:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.11';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Press SEND PRO-FORMA INVOICE and verify the composer',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - SEND PRO-FORMA INVOICE button on the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.11');
  });

  test(`${TC}: Verify the SEND PRO-FORMA INVOICE button opens the mail composer for the pro-forma invoice`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      ...
      await quotationPage.clickSendProFormaInvoice();
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation pro-forma mail composer`);
