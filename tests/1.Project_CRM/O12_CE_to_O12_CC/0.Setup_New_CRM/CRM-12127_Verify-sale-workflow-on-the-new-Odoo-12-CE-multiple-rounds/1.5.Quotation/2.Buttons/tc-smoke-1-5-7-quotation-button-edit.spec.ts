...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - EDIT button on the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.7
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify pressing EDIT on a saved Quotation swaps the control panel to SAVE / DISCARD while
 *   the header buttons stay unchanged.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.7 "EDIT button on the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.7 <timestamp>
 ...
 *     6. Press "EDIT" and read the control panel and the header buttons.
 *
 *   Verification:
 *     1. The control panel in edit mode carries exactly SAVE and DISCARD, in that order.
 *     2. The header buttons are unchanged by edit mode.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.7:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.7';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Press EDIT and verify the control panel',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - EDIT button on the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.7');
  });

  test(`${TC}: Verify the EDIT button puts the Quotation form in edit mode`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      ...
      editModeButtons = await quotationPage.getControlPanelButtons();
      headerButtons = await quotationPage.getStatusbarButtons();
      console.log('  Control panel buttons in edit mode :');
      editModeButtons.forEach((b) => console.log(`      - ${b}`));
      console.log('  Header buttons in edit mode        :');
      headerButtons.forEach((b) => console.log(`      - ${b}`));

      record('Control panel button count in edit mode', 2, editModeButtons.length);
      record('Control panel buttons in edit mode, in order', 'SAVE | DISCARD', editModeButtons.join(' | '));
      record('Header button count in edit mode', EXPECTED_HEADER_BUTTONS.length, headerButtons.length);
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation in edit mode`);

      expect(editModeButtons, 'EDIT must put the form in edit mode - SAVE and DISCARD').toEqual([
        'SAVE',
        'DISCARD',
      ]);
      expect(headerButtons, 'the header buttons stay the same in edit mode').toEqual(EXPECTED_HEADER_BUTTONS);
    });
  });
});
