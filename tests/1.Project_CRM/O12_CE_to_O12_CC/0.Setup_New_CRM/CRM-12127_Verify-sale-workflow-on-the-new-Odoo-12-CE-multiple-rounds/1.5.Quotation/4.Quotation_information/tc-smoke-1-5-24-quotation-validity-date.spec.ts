...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Validity date of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.24
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify the Validity date of a freshly created Quotation is the creation day plus the 30-day
 *   company quotation validity.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.24 "Validity date of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.24 <timestamp>
 ...
 *     6. Read the "Validity" field of the created Quotation.
 *
 *   Verification:
 *     1. The Validity date is filled.
 *     2. The Validity date is the creation day + 30 days.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.24:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.24';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the Validity date',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - Validity date of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.24');
  });

  test(`${TC}: Verify the Validity date on the Quotation is 30 days after the day it is created`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      console.log(`\n--- ${STEP.s13} ---`);
      validityRaw = await quotationPage.getFieldDisplayValue('validity_date');

      // Odoo prints the date in the user's format (MM/DD/YYYY on this instance); compare on the
      // calendar day, not on the string, so the check survives a locale change.
      const expectedDate = new Date();
      expectedDate.setDate(expectedDate.getDate() + 30);
      expectedText = `${expectedDate.getMonth() + 1}/${expectedDate.getDate()}/${expectedDate.getFullYear()}`;
      const parsed = new Date(validityRaw);
      sameDay =
        !Number.isNaN(parsed.getTime()) &&
        parsed.getFullYear() === expectedDate.getFullYear() &&
        parsed.getMonth() === expectedDate.getMonth() &&
        parsed.getDate() === expectedDate.getDate();

      console.log(`  - Validity read back : "${validityRaw}"`);
      console.log(`  - Expected           : "${expectedText}" (today + 30 days)`);

      record('Validity is filled', 'not empty', validityRaw || '(empty)', validityRaw !== '');
      record('Validity = creation day + 30 days', expectedText, validityRaw || '(empty)', sameDay);
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation Validity date`);

      expect(validityRaw, 'the Validity date must be filled').not.toBe('');
      expect(sameDay, `the Validity date must be ${expectedText} - the form shows "${validityRaw}"`).toBe(
        true
      );
    });
  });
