...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Partner discounts of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.25
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify both partner discount fields read 0.00 on a Quotation raised without a Distributor
 *   and without a Reseller.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.25 "Partner discounts of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.25 <timestamp>
 ...
 *     6. Read the "Distributor Discount (%)" and "Reseller Discount (%)" fields of the created Quotation.
 *
 *   Verification:
 *     1. Distributor Discount (%) is 0.
 *     2. Reseller Discount (%) is 0.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\.5\.25:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.25';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the partner discounts',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - Partner discounts of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.25');
  });

  test(`${TC}: Verify the Distributor Discount (%) and the Reseller Discount (%) are 0.00 on a Quotation without partners`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      console.log(`\n--- ${STEP.s13} ---`);
      distributorDiscount = await quotationPage.getFieldNumber('distributor_discount');
      resellerDiscount = await quotationPage.getFieldNumber('reseller_discount');
      const distributorRow = await quotationPage.getFieldRowTextByLabel('Distributor Discount (%)');
      const resellerRow = await quotationPage.getFieldRowTextByLabel('Reseller Discount (%)');

      console.log(`  - Distributor Discount (%) row: "${distributorRow}"`);
      console.log(`  - Reseller Discount (%) row   : "${resellerRow}"`);

      record('Distributor Discount (%)', 0, distributorDiscount);
      record('Reseller Discount (%)', 0, resellerDiscount);
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation partner discounts`);

      expect(distributorDiscount, 'no Distributor means no Distributor discount').toBe(0);
      expect(resellerDiscount, 'no partner level means no Reseller discount').toBe(0);
    });
  });
});
