...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Payer of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.17
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify the Payer of the created Quotation is filled and carries the Company Odoo built
 *   for the Opportunity out of the e-mail domain.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.17 "Payer of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.17 <timestamp>
 ...
 *     6. Read the "Payer" field of the created Quotation.
 *
 *   Verification:
 *     1. The Payer is filled.
 *     2. The Payer equals the Company of the Opportunity.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.17:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.17';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the Payer',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - Payer of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.17');
  });

  test(`${TC}: Verify the Payer on the Quotation is filled and matches the Customer of the Opportunity`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      ...
      value = await quotationPage.getFieldPartnerName('partner_id');

      console.log(`  - Opportunity Company : "${oppCompany}"`);
      console.log(`  - Quotation Payer : "${value}"`);

      record('Payer is filled', 'not empty', value || '(empty)', value !== '');
      record('Payer matches the Opportunity Company', oppCompany, value);
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation Payer`);

      expect(value, 'the Quotation Payer must be filled').not.toBe('');
      expect(value, 'the Quotation Payer must be the Company of the Opportunity').toBe(oppCompany);
    });
  });
});
