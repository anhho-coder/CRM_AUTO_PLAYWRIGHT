...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Delivery Address of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.19
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify the Delivery Address of the created Quotation is filled and carries the Company Odoo built
 *   for the Opportunity out of the e-mail domain.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.19 "Delivery Address of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.19 <timestamp>
 ...
 *     6. Read the "Delivery Address" field of the created Quotation.
 *
 *   Verification:
 *     1. The Delivery Address is filled.
 *     2. The Delivery Address equals the Company of the Opportunity.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.19:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.19';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the Delivery Address',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - Delivery Address of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.19');
  });

  test(`${TC}: Verify the Delivery Address on the Quotation is filled and matches the Customer of the Opportunity`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      ...
      value = await quotationPage.getFieldPartnerName('partner_shipping_id');

      console.log(`  - Opportunity Company : "${oppCompany}"`);
      console.log(`  - Quotation Delivery Address : "${value}"`);

      record('Delivery Address is filled', 'not empty', value || '(empty)', value !== '');
      record('Delivery Address matches the Opportunity Company', oppCompany, value);
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation Delivery Address`);

      expect(value, 'the Quotation Delivery Address must be filled').not.toBe('');
      expect(value, 'the Delivery Address must be the Company of the Opportunity').toBe(oppCompany);
    });
  });
});
