...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Pricelist of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.22
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify the Quotation keeps the Pricelist that was selected on the Deal Element it was raised
 *   from.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.22 "Pricelist of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.22 <timestamp>
 ...
 *     6. Read the "Pricelist" field of the created Quotation.
 *
 *   Verification:
 *     1. The Pricelist carries the one selected on the Deal Element.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.22:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.22';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the Pricelist',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - Pricelist of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.22');
  });

  test(`${TC}: Verify the Pricelist on the Quotation matches the Pricelist selected on the Deal Element`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      console.log(`\n--- ${STEP.s13} ---`);
      pricelist = await quotationPage.getFieldDisplayValue('pricelist_id');

      console.log(`  - Pricelist on the Deal Element : "${O12CE_DATA.pricelist}"`);
      console.log(`  - Pricelist on the Quotation    : "${pricelist}"`);

      record('Pricelist carried over from the Deal Element', O12CE_DATA.pricelist, pricelist,
        pricelist.includes(O12CE_DATA.pricelist));
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation Pricelist`);

      expect(pricelist, 'the Quotation must keep the Pricelist of the Deal Element').toContain(
        O12CE_DATA.pricelist
      );
    });
  });
});
