...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - End User of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_1.5.20
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify the End User of the created Quotation is filled and carries the Company Odoo built
 *   for the Opportunity out of the e-mail domain.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.20 "End User of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_1.5.20 <timestamp>
 ...
 *     6. Read the "End User" field of the created Quotation.
 *
 *   Verification:
 *     1. The End User is filled.
 *     2. The End User equals the Company of the Opportunity.
 *
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.5\.20:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.5.20';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the End User',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - End User of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.5.20');
  });

  test(`${TC}: Verify the End User on the Quotation is filled and matches the Customer of the Opportunity`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      ...
      value = await quotationPage.getFieldPartnerName('partner_end_user_id');

      console.log(`  - Opportunity Company : "${oppCompany}"`);
      console.log(`  - Quotation End User : "${value}"`);

      record('End User is filled', 'not empty', value || '(empty)', value !== '');
      record('End User matches the Opportunity Company', oppCompany, value);
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation End User`);

      expect(value, 'the Quotation End User must be filled').not.toBe('');
      expect(value, 'the End User must be the Company of the Opportunity').toBe(oppCompany);
    });
  });
});
