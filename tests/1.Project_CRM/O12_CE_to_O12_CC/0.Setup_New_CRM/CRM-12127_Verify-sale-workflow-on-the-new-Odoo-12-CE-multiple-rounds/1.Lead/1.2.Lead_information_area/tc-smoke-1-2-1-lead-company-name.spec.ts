...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Company Name
 * Test Case ID: CRM-12370_1.2.1
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Company Name on the saved Lead matches the entered value
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.3
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying Company Name.
 *
 * Verification Points:
 *   1. The saved Company Name (`partner_name`) equals the entered value.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.2.1 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.2\\.1:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.2.1';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying Company Name',
};
...
test.describe(`${TC} - Company Name`, () => {
  ...
  test(`${TC}: Verify the Company Name on the saved Lead matches the entered value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - Company Name on the saved Lead:');
