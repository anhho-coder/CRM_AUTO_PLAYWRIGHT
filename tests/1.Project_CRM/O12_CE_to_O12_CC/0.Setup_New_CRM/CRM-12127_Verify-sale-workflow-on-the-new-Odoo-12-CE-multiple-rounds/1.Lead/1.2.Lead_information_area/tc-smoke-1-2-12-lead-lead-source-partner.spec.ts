...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Lead Source - Partner
 * Test Case ID: CRM-12370_1.2.12
 * Automation-Type: new
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.14
 ...
 *   - Grounded on crm-mig: `lead_source` is a selection [nakivo | partner] on crm.lead, and lead 879678 (Sales Team CMR + Salesperson Bear Lin) carries lead_source = "partner" - the same derivation as pre-production. The spec itself drives the pre-production baseline owner pair, CMR + "Sergio Yalovik" (res.users 8779), also a member of crm.team 36 - verified over XML-RPC, 2026-09-21.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying the Lead Source derived for a Lead created with an owner.
 *
 * Verification Points:
 *   1. The saved Lead keeps the Sales Team and Salesperson entered at creation.
 *   2. Lead Source reads "Partner".
 *   3. No Lead Source change is logged in the chatter (the value is derived at creation).
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.2.12 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.2\\.12:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.2.12';
...
test.describe(`${TC} - Lead Source - Partner`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify a Lead created with a Sales Team and a Salesperson is classified Lead Source = Partner`, async ({ page }, testInfo) => {
