...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Lead Form
 * Test Case ID: CRM-12370_1.2.6
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Lead Form on the saved Lead matches the selected value
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.8
 ...
 *   - The field is read as `lead_form` (a char field on the CRM Developer page of the O12 CE form); pre-production reads the Studio field `x_studio_lead_sorce`.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying Lead Form.
 *
 * Verification Points:
 *   1. The saved Lead Form (`lead_form`) equals the selected value.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.2.6 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.2\\.6:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.2.6';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying Lead Form',
};

test.describe(`${TC} - Lead Form`, () => {
  let leadUrl = '';
  ...
  test(`${TC}: Verify the Lead Form on the saved Lead matches the selected value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      // Do NOT click the CRM Developer tab again here: on the SAVED (readonly) O12 CE form that
      // click never settles and burns the whole 10-minute test timeout (measured 2026-09-17).
      // getLeadFormValue() is the path the proven smoke spec CRM-12370_1.1.1 uses after save.
      const leadForm = await leadPage.getLeadFormValue();
      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - Lead Form (CRM Developer tab) on the saved Lead:');
      console.log(`     Expected : "${DATA.leadForm}"`);
