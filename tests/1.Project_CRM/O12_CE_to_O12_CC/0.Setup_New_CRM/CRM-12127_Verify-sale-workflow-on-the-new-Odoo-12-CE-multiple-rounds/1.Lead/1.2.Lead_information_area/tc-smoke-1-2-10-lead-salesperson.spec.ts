...
/**
 * O12 CE Main-Business Smoke - Lead data verification - Salesperson
 * Test Case ID: CRM-12370_1.2.10
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Objective: Verify the Salesperson entered at creation is kept on the saved Lead
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.1.12
 ...
 *   - Pre-production waits for the assignment job to FILL the Salesperson. On O12 CE the assignment crons run on a 2-4 hour cadence (ir.cron 192 "Crm: Assigned leads without salesperson to Veronika" = 2h, ir.cron 195 "Automatically assign unassigned leads/opportunities to Sales Directors" = 4h), which no smoke run can wait for. This port verifies the deterministic half - the Salesperson entered at creation is stored and rendered on the saved Lead. The job-driven assignment stays with the dedicated assignment suite.
 *   - Owner pair is the pre-production baseline verbatim: Sales Team "CMR" (crm.team 36) + Salesperson "Sergio Yalovik" (res.users 8779, login sergey.yalovik@nakivo.com), a member of that team who owns 595 leads in it - verified over XML-RPC on crm-mig, 2026-09-21.
 *
 * Pre-conditions:
 ...
 *   6. Press "SAVE" button.
 *   7. Verifying the Salesperson.
 *
 * Verification Points:
 *   1. The saved Lead keeps the Salesperson entered at creation.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. Every record it makes
 * carries the marker "TEST CRM-12370_1.2.10 <timestamp>" so the afterAll sweep can find it again even when
 * the test dies mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.2\\.10:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.2.10';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Salesperson',
};
...
test.describe(`${TC} - Salesperson`, () => {
  ...
  test(`${TC}: Verify the Salesperson entered at creation is kept on the saved Lead`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const assignment = await leadPage.waitForSalesTeamAssignment(
      ...
      const jobNote = await leadPage.findChatterMessage('call_job_assign_leads_to_salesmen_for_specific_lead');
      console.log('==================== VERIFY ====================');
