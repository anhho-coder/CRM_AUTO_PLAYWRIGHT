...
/**
 * O12 CE Main-Business Smoke - Delivery Address taken from the Opportunity
 * Test Case ID: CRM-12370_1.4.9
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Summary:
 *   Verify the Deal Element Delivery Address is not left empty and carries the Company Odoo created for the
 *   Opportunity out of the e-mail domain.
 *
 * Source manual TC (pre-production): TC.Performance.1.1.4.9 "Deal Element Delivery Address from the Opportunity".
 *
 ...
 *  12. Read the Delivery Address back from the saved Deal Element and compare it with the Opportunity Company.
 *
 * Verification Points:
 *   1. Delivery Address is not empty on the saved Deal Element.
 *   2. Delivery Address is the Company the Opportunity carries.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.4\.9:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.4.9';
...
test.describe(`${TC} - Delivery Address from the Opportunity`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.4.9');
  });

  test('CRM-12370_1.4.9: Verify the Delivery Address is filled and matches the Company of the Opportunity', async ({ page }, testInfo) => {
    ...
    await test.step('Verification', async () => {
      const value = await dealElementPage.getFieldPartnerName('partner_shipping_id');

      record('Delivery Address is filled', 'not empty', value === '' ? '(empty)' : value, value !== '');
      record('Delivery Address = Opportunity Company', oppCompany, value === '' ? '(empty)' : value);
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Delivery Address read back`);

      expect(value, 'the Delivery Address must not be left empty on the Deal Element').not.toBe('');
