...
/**
 * O12 CE Main-Business Smoke - End User taken from the Opportunity
 * Test Case ID: CRM-12370_1.4.10
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Summary:
 *   Verify the Deal Element End User is not left empty and carries the Company Odoo created for the
 *   Opportunity out of the e-mail domain.
 *
 * Source manual TC (pre-production): TC.Performance.1.1.4.10 "Deal Element End User from the Opportunity".
 *
 ...
 *  12. Read the End User back from the saved Deal Element and compare it with the Opportunity Company.
 *
 * Verification Points:
 *   1. End User is not empty on the saved Deal Element.
 *   2. End User is the Company the Opportunity carries.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\.4\.10:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.4.10';
...
test.describe(`${TC} - End User from the Opportunity`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_1.4.10');
  });

  test('CRM-12370_1.4.10: Verify the End User is filled and matches the Company of the Opportunity', async ({ page }, testInfo) => {
    ...
    await test.step('Verification', async () => {
      const value = await dealElementPage.getFieldPartnerName('partner_end_user_id');

      record('End User is filled', 'not empty', value === '' ? '(empty)' : value, value !== '');
      record('End User = Opportunity Company', oppCompany, value === '' ? '(empty)' : value);
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - End User read back`);

      expect(value, 'the End User must not be left empty on the Deal Element').not.toBe('');
