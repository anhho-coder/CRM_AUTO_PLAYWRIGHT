...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Log note - field summary
 * Test Case ID: CRM-12370_1.3.35
 * Automation-Type: new
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.35
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the field-summary log note.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.35 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.35:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.35';
...
test.describe(`${TC} - Log note - field summary`, () => {
  ...
  test(`${TC}: Verify the Log note summary reports the Email and the Show on website flag of the Contact`, async ({ page }, testInfo) => {
