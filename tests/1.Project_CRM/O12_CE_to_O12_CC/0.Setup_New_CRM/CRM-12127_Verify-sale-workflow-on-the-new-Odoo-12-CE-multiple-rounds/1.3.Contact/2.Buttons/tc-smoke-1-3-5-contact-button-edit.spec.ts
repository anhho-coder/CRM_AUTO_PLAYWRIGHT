...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - EDIT button
 * Test Case ID: CRM-12370_1.3.5
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the EDIT button puts the Contact form in edit mode
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.5
 ...
 *   4. Press "SAVE" button.
 *   5. Pressing EDIT and verifying the form switches to edit mode.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.5 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.3\\.5:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.5';
...
const STEP = {
  ...
  s5: 'Step 5: Pressing EDIT and verifying the form switches to edit mode',
};

test.describe(`${TC} - EDIT button`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify the EDIT button puts the Contact form in edit mode`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const editableBefore = await contactPage.isFormEditable();
      await contactPage.clickEdit();
      await page.waitForTimeout(CommonUtils.waitTimes.extraLong);
      const editableAfter = await contactPage.isFormEditable();
      const cpAfter = await contactPage.getControlPanelButtons();
      const EXPECTED_CP = ['SAVE', 'DISCARD'];

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the saved Contact opens in READ mode:');
      console.log(`     Expected : false`);
      console.log(`     Actual   : ${editableBefore}`);
      console.log(`     Result   : ${editableBefore === false ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #2 - EDIT puts the form in EDIT mode:');
      console.log(`     Expected : true`);
      console.log(`     Actual   : ${editableAfter}`);
      console.log(`     Result   : ${editableAfter === true ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #3 - the control panel in edit mode:');
      console.log(`     Expected : ${EXPECTED_CP.join(' | ')}`);
      console.log(`     Actual   : ${cpAfter.join(' | ')}`);
      console.log(`     Result   : ${JSON.stringify(cpAfter) === JSON.stringify(EXPECTED_CP) ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
