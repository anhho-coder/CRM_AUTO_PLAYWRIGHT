...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Control-panel buttons
 * Test Case ID: CRM-12370_1.3.3
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the number, the names and the order of the control-panel buttons on a saved Contact
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.3
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the control-panel buttons of the saved Contact.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.3 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.3\\.3:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.3';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the control-panel buttons of the saved Contact',
};

test.describe(`${TC} - Control-panel buttons`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify the number, the names and the order of the control-panel buttons on a saved Contact`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      const actual = await contactPage.getControlPanelButtons();

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the control-panel buttons a saved Contact offers:');
      console.log(`     Expected : ${EXPECTED.join(' | ')}`);
      console.log(`     Actual   : ${actual.join(' | ')}`);
      console.log(`     Result   : ${JSON.stringify(actual) === JSON.stringify(EXPECTED) ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #2 - the NUMBER of control-panel buttons:');
