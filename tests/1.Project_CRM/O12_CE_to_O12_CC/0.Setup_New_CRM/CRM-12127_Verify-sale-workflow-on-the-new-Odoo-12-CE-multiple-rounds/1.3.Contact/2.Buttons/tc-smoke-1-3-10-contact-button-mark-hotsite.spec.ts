...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - MARK HOTSITE button
 * Test Case ID: CRM-12370_1.3.10
 * Automation-Type: new
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.10
 ...
 *   4. Press "SAVE" button.
 *   5. Pressing MARK HOTSITE and verifying the confirmation dialog.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.10 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.10:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.10';
...
test.describe(`${TC} - MARK HOTSITE button`, () => {
  ...
  test(`${TC}: Verify the MARK HOTSITE button asks for confirmation before it changes the Contact`, async ({ page }, testInfo) => {
