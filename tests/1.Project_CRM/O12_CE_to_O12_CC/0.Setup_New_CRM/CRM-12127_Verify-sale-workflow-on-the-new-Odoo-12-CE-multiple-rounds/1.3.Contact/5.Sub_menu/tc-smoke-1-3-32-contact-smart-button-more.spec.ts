...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Sub-menu "More" entries
 * Test Case ID: CRM-12370_1.3.32
 * Automation-Type: new
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.32
 ...
 *   4. Press "SAVE" button.
 *   5. Opening the "More" sub-menu and verifying its entries.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.32 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.32:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.32';
...
test.describe(`${TC} - Sub-menu "More" entries`, () => {
  ...
  test(`${TC}: Verify the entries the "More" sub-menu button of a Contact offers`, async ({ page }, testInfo) => {
