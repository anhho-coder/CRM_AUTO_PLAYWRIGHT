...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Sub-menu counters
 * Test Case ID: CRM-12370_1.3.31
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify every sub-menu counter of a newly created Contact reports zero
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.31
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the sub-menu counters of the saved Contact.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.31 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.3\\.31:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.31';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the sub-menu counters of the saved Contact',
};

test.describe(`${TC} - Sub-menu counters`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify every sub-menu counter of a newly created Contact reports zero`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const COUNTERS = ['Opportunities', 'Meetings', 'Sales', 'Subscriptions', 'Demo', 'Invoiced'];
      const actual: Array<{ caption: string; count: number }> = [];
      for (const caption of COUNTERS) {
        actual.push({ caption, count: await contactPage.getSmartButtonCount(caption) });
      }

      console.log('==================== VERIFY ====================');
      actual.forEach((a, i) => {
        console.log(`  Verify #${i + 1} - the "${a.caption}" counter of a newly created Contact:`);
        console.log(`     Expected : 0`);
        console.log(`     Actual   : ${Number.isNaN(a.count) ? 'BUTTON NOT ON THE SCREEN' : a.count}`);
        console.log(`     Result   : ${a.count === 0 ? 'PASS' : 'FAIL'}`);
      });
      console.log('===============================================');
