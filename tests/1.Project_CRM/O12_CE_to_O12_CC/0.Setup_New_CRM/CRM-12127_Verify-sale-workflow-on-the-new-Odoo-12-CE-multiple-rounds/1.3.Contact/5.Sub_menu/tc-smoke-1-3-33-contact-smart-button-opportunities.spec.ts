...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Sub-menu "Opportunities"
 * Test Case ID: CRM-12370_1.3.33
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Opportunities sub-menu button opens the Opportunities of the Contact
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.33
 ...
 *   4. Press "SAVE" button.
 *   5. Pressing the Opportunities sub-menu button and verifying the view it opens.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.33 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.33:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.33';
...
const STEP = {
  ...
  s5: 'Step 5: Pressing the Opportunities sub-menu button and verifying the view it opens',
};

test.describe(`${TC} - Sub-menu "Opportunities"`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify the Opportunities sub-menu button opens the Opportunities of the Contact`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const countBefore = await contactPage.getSmartButtonCount('Opportunities');
      await contactPage.clickSmartButtonByCaption('Opportunities');
      ...
      const scopedToContact = breadcrumb.includes(DATA.name);
      console.log(`  Opened: ${url}`);

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the counter of a newly created Contact:');
      console.log(`     Expected : 0`);
      console.log(`     Actual   : ${countBefore}`);
      console.log(`     Result   : ${countBefore === 0 ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #2 - the button opens the Opportunities (crm.lead) view:');
      console.log(`     Expected : the URL carries model=crm.lead`);
      console.log(`     Actual   : ${url}`);
      console.log(`     Result   : ${opensCrmLead ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #3 - the view is scoped to THIS Contact:');
      console.log(`     Expected : the breadcrumb carries "${DATA.name}" and "Opportunities"`);
      console.log(`     Actual   : ${breadcrumb.join(' > ')}`);
      console.log(`     Result   : ${scopedToContact && breadcrumb.includes('Opportunities') ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
