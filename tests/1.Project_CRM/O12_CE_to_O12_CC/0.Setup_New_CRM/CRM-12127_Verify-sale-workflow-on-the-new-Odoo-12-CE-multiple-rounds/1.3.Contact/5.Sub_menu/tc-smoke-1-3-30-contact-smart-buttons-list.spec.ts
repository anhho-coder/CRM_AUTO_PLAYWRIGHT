...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Sub-menu buttons
 * Test Case ID: CRM-12370_1.3.30
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the number, the names and the order of the sub-menu (smart) buttons of a Contact
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.30
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the sub-menu buttons of the saved Contact.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.30 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.3\\.30:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.30';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the sub-menu buttons of the saved Contact',
};

test.describe(`${TC} - Sub-menu buttons`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify the number, the names and the order of the sub-menu (smart) buttons of a Contact`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      const raw = await contactPage.getSmartButtons();
      const actual = await contactPage.getSmartButtonNames();

      console.log('==================== VERIFY ====================');
      console.log('  Sub-menu buttons as the screen prints them:');
      raw.forEach((b, i) => console.log(`     ${i + 1}. "${b.text}"`));
      console.log('  Verify #1 - the sub-menu button names and their order:');
      console.log(`     Expected : ${EXPECTED.join(' | ')}`);
      console.log(`     Actual   : ${actual.join(' | ')}`);
      console.log(`     Result   : ${JSON.stringify(actual) === JSON.stringify(EXPECTED) ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #2 - the NUMBER of sub-menu buttons:');
