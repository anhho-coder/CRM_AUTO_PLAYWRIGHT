...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Field names and order - left column
 * Test Case ID: CRM-12370_1.3.27
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the field names and their order in the left column of the Contact information area
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.27
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the field names and order in the left column.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.27 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.27:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.27';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the field names and order in the left column',
};
...
test.describe(`${TC} - Field names and order - left column`, () => {
  ...
  test(`${TC}: Verify the field names and their order in the left column of the Contact information area`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      ...
      const EXPECTED = [
        ...
        'Stage',
        'Support Type',
        '24/7 support promotion days remaining',
        'End Support date manual',
        'Send License Upfront',
        'Is a Nakivo Customer',
      ...
      const actual = await contactPage.getColumnFieldLabels('left');

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the field names and their order, left column:');
      ...
      console.log('  Verify #2 - the NUMBER of fields in the left column:');
