...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Support Type
 * Test Case ID: CRM-12370_1.3.23
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Support Type and the 24/7 support promotion days of a newly created Contact
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.23
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Support Type block on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.23 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.3\\.23:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.23';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Support Type block on the saved form',
};

test.describe(`${TC} - Support Type`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify the Support Type and the 24/7 support promotion days of a newly created Contact`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const supportType = await contactPage.getFieldDisplayValue('support_type');
      const promoDays = await contactPage.getFieldNumber('support_promotion_days_remain_247');

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the Support Type a newly created Contact starts on:');
      console.log(`     Expected : Standard`);
      console.log(`     Actual   : ${supportType}`);
      console.log(`     Result   : ${supportType === 'Standard' ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #2 - the 24/7 support promotion days remaining:');
      console.log(`     Expected : 0`);
      console.log(`     Actual   : ${promoDays}`);
      console.log(`     Result   : ${promoDays === 0 ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
