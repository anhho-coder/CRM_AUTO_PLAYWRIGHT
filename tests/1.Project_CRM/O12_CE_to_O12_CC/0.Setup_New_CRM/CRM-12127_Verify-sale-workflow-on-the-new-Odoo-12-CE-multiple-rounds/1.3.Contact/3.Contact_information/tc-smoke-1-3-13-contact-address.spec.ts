...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Address
 * Test Case ID: CRM-12370_1.3.13
 * Automation-Type: new
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.13
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Address block on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.13 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.3\\.13:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.13';
...
test.describe(`${TC} - Address`, () => {
  ...
  test(`${TC}: Verify the Address block (Street, State, Country) on the saved Contact matches the entered values`, async ({ page }, testInfo) => {
