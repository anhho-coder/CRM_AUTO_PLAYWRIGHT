...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Salesperson
 * Test Case ID: CRM-12370_1.3.19
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Salesperson on the saved Contact matches the selected value
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.19
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Salesperson on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.19 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.3\\.19:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.19';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Salesperson on the saved form',
};
...
test.describe(`${TC} - Salesperson`, () => {
  ...
  test(`${TC}: Verify the Salesperson on the saved Contact matches the selected value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s3, async () => {
      ...
      await contactPage.selectSalesperson(DATA.salesperson);
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const actual = await contactPage.getFieldPartnerName('user_id');
      const expected = DATA.salesperson;

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the Salesperson of the saved Contact:');
      console.log(`     Expected : ${expected}`);
      console.log(`     Actual   : ${actual}`);
      console.log(`     Result   : ${actual === expected ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
      console.log(`OVERALL: the Salesperson of the saved Contact reads "${actual}"`);

      expect(actual, 'the Salesperson on the saved form matches the selected value').toBe(expected);
