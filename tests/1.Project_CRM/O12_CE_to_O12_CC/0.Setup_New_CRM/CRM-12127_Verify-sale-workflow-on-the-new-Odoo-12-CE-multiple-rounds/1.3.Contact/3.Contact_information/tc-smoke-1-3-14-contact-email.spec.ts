...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Email
 * Test Case ID: CRM-12370_1.3.14
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Email on the saved Contact matches the entered value
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.14
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Email on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.14 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.14:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.14';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Email on the saved form',
};
...
test.describe(`${TC} - Email`, () => {
  ...
  test(`${TC}: Verify the Email on the saved Contact matches the entered value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const actual = await contactPage.getFieldDisplayValue('email');
      const expected = DATA.email;

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the Email of the saved Contact:');
      console.log(`     Expected : ${expected}`);
      console.log(`     Actual   : ${actual}`);
      console.log(`     Result   : ${actual === expected ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
      console.log(`OVERALL: the Email of the saved Contact reads "${actual}"`);

      expect(actual, 'the Email on the saved form matches the entered value').toBe(expected);
