...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Contact Name
 * Test Case ID: CRM-12370_1.3.12
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Contact Name on the saved Contact matches the entered value
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.12
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Contact Name on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.12 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.3\\.12:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.12';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Contact Name on the saved form',
};
...
test.describe(`${TC} - Contact Name`, () => {
  ...
  test(`${TC}: Verify the Contact Name on the saved Contact matches the entered value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const actual = await contactPage.getFieldDisplayValue('name');
      const expected = DATA.name;

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the Contact Name of the saved Contact:');
