...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Phone and Mobile
 * Test Case ID: CRM-12370_1.3.15
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Phone and the Mobile on the saved Contact match the entered values
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.15
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Phone and the Mobile on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.15 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.15:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.15';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Phone and the Mobile on the saved form',
};

test.describe(`${TC} - Phone and Mobile`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify the Phone and the Mobile on the saved Contact match the entered values`, async ({ page }, testInfo) => {
    ...
    const DATA = {
      ...
      state: 'Connecticut',
      // Odoo re-formats a phone number on save: "+1 203 555 0100" comes back as
      // "+1 203-555-0100" (grounded on pre-production 2026-09-18). The test data is therefore
      // already in the stored format, so this test case asserts what it means to assert - the
      // value survives the save unchanged - instead of asserting the formatter.
      phone: '+1 203-555-0100',
      mobile: '+1 203-555-0199',
    };
    ...
    await test.step(STEP.s3, async () => {
      ...
      await contactPage.fillTextFieldByName('phone', DATA.phone);
      await contactPage.fillTextFieldByName('mobile', DATA.mobile);
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const phone = await contactPage.getFieldDisplayValue('phone');
      const mobile = await contactPage.getFieldDisplayValue('mobile');

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the Phone of the saved Contact:');
      console.log(`     Expected : ${DATA.phone}`);
      console.log(`     Actual   : ${phone}`);
      console.log(`     Result   : ${phone === DATA.phone ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #2 - the Mobile of the saved Contact:');
      console.log(`     Expected : ${DATA.mobile}`);
      console.log(`     Actual   : ${mobile}`);
      console.log(`     Result   : ${mobile === DATA.mobile ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
