...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Website
 * Test Case ID: CRM-12370_1.3.16
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Website on the saved Contact matches the entered value
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.16
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Website on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.16 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.16:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.16';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Website on the saved form',
};

test.describe(`${TC} - Website`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify the Website on the saved Contact matches the entered value`, async ({ page }, testInfo) => {
    ...
    const DATA = {
      ...
      state: 'Connecticut',
      website: 'www.nakivo-autotest.com',
    };
    ...
    await test.step(STEP.s3, async () => {
      ...
      await contactPage.fillTextFieldByName('website', DATA.website);
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const actual = await contactPage.getFieldDisplayValue('website');
      const expected = DATA.website;

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the Website of the saved Contact:');
      console.log(`     Expected : ${expected}`);
      console.log(`     Actual   : ${actual}`);
      console.log(`     Result   : ${actual.includes(expected) ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
