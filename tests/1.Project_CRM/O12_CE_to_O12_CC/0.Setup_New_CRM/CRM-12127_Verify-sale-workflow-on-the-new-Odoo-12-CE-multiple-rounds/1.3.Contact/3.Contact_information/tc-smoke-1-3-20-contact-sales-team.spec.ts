...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Sales Team
 * Test Case ID: CRM-12370_1.3.20
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Sales Team on the saved Contact matches the selected value
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.20
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Sales Team on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_1.3.20 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\\.3\\.20:" --project=MigSmoke
 */

const TC = 'CRM-12370_1.3.20';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Sales Team on the saved form',
};
...
test.describe(`${TC} - Sales Team`, () => {
  ...
  test(`${TC}: Verify the Sales Team on the saved Contact matches the selected value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s3, async () => {
      ...
      await contactPage.selectSalesTeam(DATA.salesTeam);
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const actual = await contactPage.getFieldPartnerName('team_id');
      const expected = DATA.salesTeam;

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the Sales Team of the saved Contact:');
