...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Tags
 * Test Case ID: CRM-12325_2.3.18
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Tag selected at creation is saved on the Contact
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.18
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Tags on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12325_2.3.18 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.18:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.3.18';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Tags on the saved form',
};
...
test.describe(`${TC} - Tags`, () => {
  ...
  test(`${TC}: Verify the Tag selected at creation is saved on the Contact`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s3, async () => {
      ...
      chosenTag = await contactPage.addFirstAvailableTag();
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const tags = await contactPage.getTagList();

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the Tag selected at creation is on the saved Contact:');
      console.log(`     Expected : ${chosenTag}`);
      console.log(`     Actual   : ${tags.join(' | ') || '(no tag)'}`);
      console.log(`     Result   : ${tags.includes(chosenTag) ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #2 - the Contact carries exactly the ONE Tag that was selected:');
      console.log(`     Expected : 1`);
      console.log(`     Actual   : ${tags.length}`);
      console.log(`     Result   : ${tags.length === 1 ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
