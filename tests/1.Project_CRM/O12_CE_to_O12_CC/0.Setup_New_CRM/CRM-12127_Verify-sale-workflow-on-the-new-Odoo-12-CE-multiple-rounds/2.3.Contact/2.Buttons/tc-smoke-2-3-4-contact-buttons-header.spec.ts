...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Header buttons
 * Test Case ID: CRM-12325_2.3.4
 * Automation-Type: new
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.4
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the header buttons of the saved Contact.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12325_2.3.4 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.4:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.3.4';
...
test.describe(`${TC} - Header buttons`, () => {
  ...
  test(`${TC}: Verify the number, the names and the order of the header buttons on a saved Contact`, async ({ page }, testInfo) => {
