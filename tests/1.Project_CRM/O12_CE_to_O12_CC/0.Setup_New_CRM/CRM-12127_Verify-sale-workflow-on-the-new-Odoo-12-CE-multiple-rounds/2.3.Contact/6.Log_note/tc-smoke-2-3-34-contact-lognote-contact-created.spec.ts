...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Log note - Contact created
 * Test Case ID: CRM-12325_2.3.34
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Log note reports "Contact created" when the Contact is created
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.34
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the "Contact created" log note.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12325_2.3.34 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12325_2\\.3\\.34:" --project=MigSmoke
 */

const TC = 'CRM-12325_2.3.34';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the "Contact created" log note',
};

test.describe(`${TC} - Log note - Contact created`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify the Log note reports "Contact created" when the Contact is created`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const note = await contactPage.waitForChatterMessage(/^Contact created$/);

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the creation log note:');
      console.log(`     Expected : a log note reading "Contact created"`);
      console.log(`     Actual   : ${note === null ? 'NOT FOUND' : `"${note}"`}`);
      console.log(`     Result   : ${note !== null ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
