 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_2\.1\.1:" --project=chromium
 */
...
test.describe('CRM-12370_2.1.1 - O12 CE smoke: create a CRM Opportunity', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_2.1.1');
  });

  test('CRM-12370_2.1.1: Verify a CRM Opportunity can be created on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const opportunityPage = new OpportunityPage(page);

    const TC_ID = 'CRM-12370_2.1.1';
