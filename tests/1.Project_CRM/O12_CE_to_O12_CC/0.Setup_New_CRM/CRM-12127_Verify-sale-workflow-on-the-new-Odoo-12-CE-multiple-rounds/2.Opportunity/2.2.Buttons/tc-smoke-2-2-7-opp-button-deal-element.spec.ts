 ...
 * =============================================================================================
 *  Test Case ID    : CRM-12370_2.2.7
 *  Automation-Type : new
 *  Automation-Date : 2026-09-23
 *  Feature folder  : .../CRM-12127_.../2.Opportunity/2.2.Buttons
 *  Source manual TC: TC.Performance.1.1.2.9 (pre-production baseline, 37/37 PASS 2026-09-21)
 *
 *  Summary
 *  ---------------------------------------------------------------------------------------------
 *  Creates its own Opportunity on the O12 CE Migration server (crm-mig) and asserts that
 *  pressing DEAL ELEMENT on a saved Opportunity opens a new sale.order bound to that
 *  Opportunity, in edit mode, with the Opportunity kept in the breadcrumb.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "CRM-12370_2\\.2\\.7:" --project=MigSmoke
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *           - Opportunity     = TEST CRM-12370_2.2.7 <unique>
 *           - Contact Name    = TEST CRM-12370_2.2.7 <unique>
 ...
 *    4.   Press the DEAL ELEMENT button of the saved Opportunity  -> spec label "Step 8"
 *    5.   Read the record that opens  -> spec label "Step 9"
 *
 *  Verification:
 *    5. The record that opens is a sale.order bound to this Opportunity (active_id)
 *    - The breadcrumb keeps the Opportunity and ends on "New", and the form is in EDIT mode
 *
 *  BASELINE RULE (CLAUDE.md, crm-mig automation)
 *  ---------------------------------------------------------------------------------------------
 *  The expected values below are the PRE-PRODUCTION baseline, byte-identical to TC.Performance.1.1.2.9.
 ...
 *  `TEST CRM-12370_2.2.7 ...` - the same marker `sweepMigLeftoversOnO12CE` searches for.
 * =============================================================================================
 */

const TC = 'CRM-12370_2.2.7';

// One source of truth: the stdout banner and the test.step label are the same string.
const STEP = {
  s8: 'Step 8: Press the DEAL ELEMENT button of the saved Opportunity',
  s9: 'Step 9: Read the record that opens',
};
...
test.describe(`${TC} - O12 CE Opportunity Buttons`, () => {
  ...
  test(`${TC}: Verify the DEAL ELEMENT button opens a new Deal Element for the Opportunity`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s8, async () => {
      ...
      await opportunityPage.clickStatusbarButtonByName('action_create_deal_element');
    ...
    await test.step(STEP.s9, async () => {
      ...
      record('The record that opens is a sale.order', 'model=sale.order', targetUrl.includes('model=sale.order') ? 'model=sale.order' : targetUrl);
      record('It is bound to the Opportunity this test created', `active_id=${opp.oppId}`, targetUrl.includes(`active_id=${opp.oppId}`) ? `active_id=${opp.oppId}` : targetUrl);
      record('The breadcrumb keeps the Opportunity and ends on a new record', `${opp.oppName} > New`, breadcrumb.slice(-2).join(' > '));
      record('The new Deal Element opens in EDIT mode', 'true', String(editable));
      printVerify();

      let verifyPassed = false;
      try {
        expect(targetUrl, 'DEAL ELEMENT must open a sale.order').toContain('model=sale.order');
        expect(targetUrl, 'the new Deal Element must be bound to this Opportunity').toContain(`active_id=${opp.oppId}`);
        expect(breadcrumb[breadcrumb.length - 1], 'the breadcrumb of the new Deal Element').toBe('New');
        expect(breadcrumb[breadcrumb.length - 2], 'the breadcrumb must keep the Opportunity').toBe(opp.oppName);
        expect(editable, 'the new Deal Element must open in EDIT mode').toBe(true);
        verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, {
          name: `${TC} - New Deal Element`,
          passed: verifyPassed,
        }).catch(() => {});
      }
    });
