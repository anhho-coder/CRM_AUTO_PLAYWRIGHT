 ...
 * =============================================================================================
 *  Test Case ID    : CRM-12370_2.2.10
 *  Automation-Type : new
 *  Automation-Date : 2026-09-23
 *  Feature folder  : .../CRM-12127_.../2.Opportunity/2.2.Buttons
 *  Source manual TC: TC.Performance.1.1.2.12 (pre-production baseline, 37/37 PASS 2026-09-21)
 *
 *  Summary
 *  ---------------------------------------------------------------------------------------------
 *  Creates its own Opportunity on the O12 CE Migration server (crm-mig) and asserts that
 *  pressing MARK LOST opens the "Lost Reason" dialog with the expected field and buttons, and
 *  CANCEL leaves the Opportunity in its stage.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "CRM-12370_2\\.2\\.10:" --project=MigSmoke
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *           - Opportunity     = TEST CRM-12370_2.2.10 <unique>
 *           - Contact Name    = TEST CRM-12370_2.2.10 <unique>
 ...
 *    4.   Press the MARK LOST button of the saved Opportunity  -> spec label "Step 8"
 *    5.   Read the dialog that opens, then dismiss it with CANCEL  -> spec label "Step 9"
 *
 *  Verification:
 *    5. The dialog title is "Lost Reason" and it offers the field Lost Reason
 *    - The dialog offers SUBMIT | CANCEL, and CANCEL leaves the Opportunity in the NEW stage
 *
 *  BASELINE RULE (CLAUDE.md, crm-mig automation)
 *  ---------------------------------------------------------------------------------------------
 *  The expected values below are the PRE-PRODUCTION baseline, byte-identical to TC.Performance.1.1.2.12.
 ...
 *  `TEST CRM-12370_2.2.10 ...` - the same marker `sweepMigLeftoversOnO12CE` searches for.
 * =============================================================================================
 */

const TC = 'CRM-12370_2.2.10';

/** The MARK LOST dialog (PRE-PROD baseline, 2026-09-21). */
...
const STEP = {
  s8: 'Step 8: Press the MARK LOST button of the saved Opportunity',
