 ...
 * =============================================================================================
 *  Test Case ID    : CRM-12370_2.2.5
 *  Automation-Type : new
 *  Automation-Date : 2026-09-23
 *  Feature folder  : .../CRM-12127_.../2.Opportunity/2.2.Buttons
 *  Source manual TC: TC.Performance.1.1.2.7 (pre-production baseline, 37/37 PASS 2026-09-21)
 *
 *  Summary
 *  ---------------------------------------------------------------------------------------------
 *  Creates its own Opportunity on the O12 CE Migration server (crm-mig) and asserts that
 *  the Print menu of a saved Opportunity offers exactly the expected reports, in order.
 *
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "CRM-12370_2\\.2\\.5:" --project=MigSmoke
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *           - Opportunity     = TEST CRM-12370_2.2.5 <unique>
 *           - Contact Name    = TEST CRM-12370_2.2.5 <unique>
 ...
 *    4.   Open the Print menu of the saved Opportunity and read its entries  -> spec label "Step 8"
 *
 *  Verification:
 *    4. The Print menu offers exactly 1 report: Lead Survey Report
 *    - No other report is offered
 *
 *  BASELINE RULE (CLAUDE.md, crm-mig automation)
 *  ---------------------------------------------------------------------------------------------
 *  The expected values below are the PRE-PRODUCTION baseline, byte-identical to TC.Performance.1.1.2.7.
 ...
 *  `TEST CRM-12370_2.2.5 ...` - the same marker `sweepMigLeftoversOnO12CE` searches for.
 * =============================================================================================
 */

const TC = 'CRM-12370_2.2.5';

/** The entries of the Print menu of a saved Opportunity (PRE-PROD baseline, 2026-09-21). */
const EXPECTED = [
    'Lead Survey Report',
  ];
...
const STEP = {
  s8: 'Step 8: Open the Print menu of the saved Opportunity and read its entries',
};
...
test.describe(`${TC} - O12 CE Opportunity Buttons`, () => {
  ...
  test(`${TC}: Verify the reports the Print menu of an Opportunity offers`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s8, async () => {
      ...
      const actual = await opportunityPage.getControlPanelMenuItems('Print');
      actual.forEach((item, i) => console.log(`  Print entry #${i + 1}: ${item}`));
      record('Number of Print entries', EXPECTED.length, actual.length);
      record('Print entries and their order', EXPECTED.join(' | '), actual.join(' | '));
      printVerify();

      let verifyPassed = false;
      try {
        expect(actual.length, 'the NUMBER of reports the Print menu offers').toBe(EXPECTED.length);
        expect(actual, 'the reports the Print menu offers, in order').toEqual(EXPECTED);
        verifyPassed = true;
      } finally {
