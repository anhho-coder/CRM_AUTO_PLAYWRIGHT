 ...
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "CRM-12370_2\\.3\\.7:" --project=MigSmoke
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *    4.   Read the Address block on the saved form  -> spec label "Step 8"
 *
 ...
 *  `TEST CRM-12370_2.3.7 ...` - the same marker `sweepMigLeftoversOnO12CE` searches for.
 * =============================================================================================
 */

const TC = 'CRM-12370_2.3.7';

/** The Street the pre-prod baseline enters; the mig chain helper takes it as an opt-in. */
const STREET = '123street';

// One source of truth: the stdout banner and the test.step label are the same string.
const STEP = {
  s8: 'Step 8: Read the Address block on the saved form',
};
...
test.describe(`${TC} - O12 CE Opportunity`, () => {
  ...
  test(`${TC}: Verify the Address block (Street, State, Country) on the saved Opportunity matches the entered values`, async ({ page }, testInfo) => {
    ...
    opp = await createOpportunityOnO12CE(page, TC, { street: STREET });
    ...
    await test.step(STEP.s8, async () => {
      ...
      record('The Street on the saved form', STREET, street);
      record('The Address block carries the State', O12CE_DATA.state, addressRow.includes(O12CE_DATA.state) ? O12CE_DATA.state : addressRow);
      record('The Address block carries the Country', O12CE_DATA.country, addressRow.includes(O12CE_DATA.country) ? O12CE_DATA.country : addressRow);
      printVerify();
      ...
      expect(street, 'the Street on the saved form').toBe(STREET);
      expect(addressRow, 'the Address block must carry the State').toContain(O12CE_DATA.state);
      expect(addressRow, 'the Address block must carry the Country').toContain(O12CE_DATA.country);
    });
  });
});
