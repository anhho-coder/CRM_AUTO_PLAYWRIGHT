 ...
 * =============================================================================================
 *  Test Case ID    : CRM-12370_2.3.6
 *  Automation-Type : new
 *  Automation-Date : 2026-09-23
 *  Feature folder  : .../CRM-12127_.../2.Opportunity/2.3.Opportunity_information
 *  Source manual TC: TC.Performance.1.1.2.19 (pre-production baseline, 37/37 PASS 2026-09-21)
 *
 *  Summary
 *  ---------------------------------------------------------------------------------------------
 ...
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "CRM-12370_2\\.3\\.6:" --project=MigSmoke
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *    4.   Read the Email on the saved form  -> spec label "Step 8"
 *
 *  BASELINE RULE (CLAUDE.md, crm-mig automation)
 *  ---------------------------------------------------------------------------------------------
 *  Every expected value below is the PRE-PRODUCTION baseline, copied from TC.Performance.1.1.2.19
 ...
 *  `TEST CRM-12370_2.3.6 ...` - the same marker `sweepMigLeftoversOnO12CE` searches for.
 * =============================================================================================
 */

const TC = 'CRM-12370_2.3.6';

// One source of truth: the stdout banner and the test.step label are the same string.
const STEP = {
  s8: 'Step 8: Read the Email on the saved form',
};
...
test.describe(`${TC} - O12 CE Opportunity`, () => {
  ...
  test(`${TC}: Verify the Email on the saved Opportunity matches the entered value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s8, async () => {
      ...
      record('The Email on the saved form', opp.email, actual);
      printVerify();
      ...
      expect(actual, 'the Email on the saved form').toBe(opp.email);
    });
  });
});
