 ...
 * =============================================================================================
 *  Test Case ID    : CRM-12370_2.3.1
 *  Automation-Type : new
 *  Automation-Date : 2026-09-23
 *  Feature folder  : .../CRM-12127_.../2.Opportunity/2.3.Opportunity_information
 *  Source manual TC: TC.Performance.1.1.2.14 (pre-production baseline, 37/37 PASS 2026-09-21)
 *
 *  Summary
 *  ---------------------------------------------------------------------------------------------
 ...
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "CRM-12370_2\\.3\\.1:" --project=MigSmoke
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *    4.   Read the Opportunity name on the saved form  -> spec label "Step 8"
 *
 *  BASELINE RULE (CLAUDE.md, crm-mig automation)
 *  ---------------------------------------------------------------------------------------------
 *  Every expected value below is the PRE-PRODUCTION baseline, copied from TC.Performance.1.1.2.14
 ...
 *  `TEST CRM-12370_2.3.1 ...` - the same marker `sweepMigLeftoversOnO12CE` searches for.
 * =============================================================================================
 */

const TC = 'CRM-12370_2.3.1';

// One source of truth: the stdout banner and the test.step label are the same string.
const STEP = {
  s8: 'Step 8: Read the Opportunity name on the saved form',
};
...
test.describe(`${TC} - O12 CE Opportunity`, () => {
  ...
  test(`${TC}: Verify the Opportunity name on the saved Opportunity matches the entered value`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s8, async () => {
      ...
      console.log(`  Opportunity name entered : ${opp.oppName}`);
      console.log(`  Opportunity name stored  : ${actual}`);
      console.log(`  Form title block         : ${title}`);
      record('The Opportunity name on the saved form', opp.oppName, actual);
      record('The form title carries the Opportunity name', 'contains the name', title.includes(opp.oppName) ? 'contains the name' : title);
      printVerify();
      ...
      expect(actual, 'the Opportunity name on the saved form').toBe(opp.oppName);
      expect(title, 'the form title must carry the Opportunity name').toContain(opp.oppName);
    });
  });
});
