 ...
 * =============================================================================================
 *  Test Case ID    : CRM-12370_2.3.2
 *  Automation-Type : new
 *  Automation-Date : 2026-09-23
 *  Feature folder  : .../CRM-12127_.../2.Opportunity/2.3.Opportunity_information
 *  Source manual TC: TC.Performance.1.1.2.15 (pre-production baseline, 37/37 PASS 2026-09-21)
 *
 *  Summary
 *  ---------------------------------------------------------------------------------------------
 ...
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "CRM-12370_2\\.3\\.2:" --project=MigSmoke
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *    4.   Read the Company Name on the saved form  -> spec label "Step 8"
 *
 *  BASELINE RULE (CLAUDE.md, crm-mig automation)
 *  ---------------------------------------------------------------------------------------------
 *  Every expected value below is the PRE-PRODUCTION baseline, copied from TC.Performance.1.1.2.15
 ...
 *  `TEST CRM-12370_2.3.2 ...` - the same marker `sweepMigLeftoversOnO12CE` searches for.
 * =============================================================================================
 */

const TC = 'CRM-12370_2.3.2';

// One source of truth: the stdout banner and the test.step label are the same string.
const STEP = {
  s8: 'Step 8: Read the Company Name on the saved form',
};
...
test.describe(`${TC} - O12 CE Opportunity`, () => {
  ...
  test(`${TC}: Verify the Company Name of a saved Opportunity is derived from the e-mail domain`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s8, async () => {
      ...
      const actual = await opportunityPage.waitForFieldDisplayValue('partner_name', expectedCompany, 5);
      console.log(`  Email entered        : ${opp.email}`);
      console.log(`  Domain of that email : ${expectedCompany}`);
      console.log(`  Company Name stored  : ${actual}`);
      record('The Company Name is the domain of the entered e-mail', expectedCompany, actual);
      printVerify();
      ...
      expect(actual, 'the Company Name derived from the e-mail domain').toBe(expectedCompany);
    });
  });
});
