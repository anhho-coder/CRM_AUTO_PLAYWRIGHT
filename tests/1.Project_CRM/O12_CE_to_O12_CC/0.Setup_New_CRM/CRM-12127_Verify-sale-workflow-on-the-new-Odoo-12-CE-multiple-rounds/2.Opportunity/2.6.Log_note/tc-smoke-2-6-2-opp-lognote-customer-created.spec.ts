 ...
 * =============================================================================================
 *  Test Case ID    : CRM-12370_2.6.2
 *  Automation-Type : new
 *  Automation-Date : 2026-09-23
 *  Feature folder  : .../CRM-12127_.../2.Opportunity/2.6.Log_note
 *  Source manual TC: TC.Performance.1.1.2.36 (pre-production baseline, 37/37 PASS 2026-09-21)
 *
 *  Summary
 *  ---------------------------------------------------------------------------------------------
 ...
 *  Command to run
 *  ---------------------------------------------------------------------------------------------
 *  npx playwright test --grep "CRM-12370_2\\.6\\.2:" --project=MigSmoke
 *
 *  Source manual TC
 *  ---------------------------------------------------------------------------------------------
 *  Pre-condition(s):
 ...
 *    4.   Read the customer-creation log note in the chatter  -> spec label "Step 8"
 *
 *  BASELINE RULE (CLAUDE.md, crm-mig automation)
 *  ---------------------------------------------------------------------------------------------
 *  Every expected value below is the PRE-PRODUCTION baseline, copied from TC.Performance.1.1.2.36
 ...
 *  `TEST CRM-12370_2.6.2 ...` - the same marker `sweepMigLeftoversOnO12CE` searches for.
 * =============================================================================================
 */

const TC = 'CRM-12370_2.6.2';

const EXPECTED_PRICELIST = 'Public Pricelist_USD (USD)';

// One source of truth: the stdout banner and the test.step label are the same string.
const STEP = {
  s8: 'Step 8: Read the customer-creation log note in the chatter',
};
...
test.describe(`${TC} - O12 CE Opportunity`, () => {
  ...
  test(`${TC}: Verify the customer-creation log note reports the Customer, the Company Name, the e-mail and the Pricelist`, async ({ page }, testInfo) => {
    ...
    /** The Company Odoo creates out of the e-mail domain - the same derivation the baseline uses. */
    const expectedCompany = opp.email.split('@')[1];
    ...
    await test.step(STEP.s8, async () => {
      ...
      record('Partner Contact Email reported by the log note', opp.email, fields['Partner Contact Email'] ?? '(missing)');
      ...
      expect(fields['Partner Contact Email'], 'the Partner Contact Email the log note reports').toBe(opp.email);
