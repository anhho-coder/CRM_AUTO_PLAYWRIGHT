 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_2\.1\.2:" --project=chromium
 */
...
test.describe('CRM-12370_2.1.2 - O12 CE smoke: edit a CRM Opportunity', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_2.1.2');
  });

  test('CRM-12370_2.1.2: Verify a CRM Opportunity can be edited on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const opportunityPage = new OpportunityPage(page);

    const TC_ID = 'CRM-12370_2.1.2';
