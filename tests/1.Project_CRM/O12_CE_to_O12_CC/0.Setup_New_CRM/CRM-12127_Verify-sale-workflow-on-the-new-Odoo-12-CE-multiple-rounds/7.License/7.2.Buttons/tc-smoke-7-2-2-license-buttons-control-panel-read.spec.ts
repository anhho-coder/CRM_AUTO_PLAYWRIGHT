 ...
 *  =============================================================================================
 * =============================================================================================
 *  Test Case ID    : CRM-12370_7.2.2
 *  Jira            : -   (ported from pre-production TC.Performance.7.2.2; no Xray manual TC)
 ...
 * ---------------------------------------------------------------------------------------------
 *  Summary
 *    Creates its own licence and verifies that, once saved, the control panel above the licence form
 *    carries exactly the three buttons a tester reads there - EDIT, Print and Action.
 * ---------------------------------------------------------------------------------------------
 *  Command to run
 *    npx playwright test --grep "CRM-12370_7\.2\.2:" --project=MigSmoke
 * ---------------------------------------------------------------------------------------------
 *  Source manual TC (pre-production): TC.Performance.7.2.2
 *    tests/1.Project_CRM/1.SalesReport_Performance/7.License/7.2.Buttons
 *
 ...
 *
 *  Steps to reproduce
 *   1. Read the control-panel buttons of the saved licence, left to right
 *
 *  Verification
 ...
 *    License, named with a fresh timestamp and the marker "TEST CRM-12370_7.2.2", so two
...
const STEP = {
  ...
  s1: 'Step 1: Read the control-panel buttons of the saved licence, left to right',
  verify: 'Verification',
} as const;

test.describe(`${TC} - The number and the names of the control-panel buttons of a saved License (read mode)`, () => {
  ...
  test(`${TC}: The number and the names of the control-panel buttons of a saved License (read mode)`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s1, async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - control-panel buttons read');
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      record('Number of control-panel buttons', EXPECTED.length, panelButtons.length);
      record('Control-panel button names and their order', EXPECTED.join(' | '), panelButtons.join(' | '));
      printVerify();

      expect(panelButtons, 'the saved licence must offer exactly 3 control-panel buttons').toHaveLength(EXPECTED.length);
      expect(panelButtons, 'the control-panel button names and their order must match the documented list').toEqual(EXPECTED);
    });
  });
});
