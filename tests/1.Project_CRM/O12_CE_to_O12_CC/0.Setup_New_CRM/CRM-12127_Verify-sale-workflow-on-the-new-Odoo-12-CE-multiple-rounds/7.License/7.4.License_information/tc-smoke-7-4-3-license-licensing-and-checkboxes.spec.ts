 ...
 *  =============================================================================================
 * =============================================================================================
 *  Test Case ID    : CRM-12370_7.4.3
 *  Jira            : -   (ported from pre-production TC.Performance.7.4.3; no Xray manual TC)
 ...
 * ---------------------------------------------------------------------------------------------
 *  Summary
 *    Creates its own licence from a perpetual product and verifies the three General fields that say
 *    how the licence was produced: Licensing = Perpetual, Msp License unticked, Generated by Odoo
 *    unticked (the licence came from the CRM chain, not from the automatic WebShop path).
 * ---------------------------------------------------------------------------------------------
 *  Command to run
 *    npx playwright test --grep "CRM-12370_7\.4\.3:" --project=MigSmoke
 * ---------------------------------------------------------------------------------------------
 *  Source manual TC (pre-production): TC.Performance.7.4.3
 ...
 *
 *  Steps to reproduce
 *   1. Read the "Licensing" field of the General group
 *   2. Read the "Msp License" and "Generated by Odoo" check boxes
 *
 *  Verification
 *   - "Licensing" is filled and reads Perpetual
 *   - "Msp License" is unticked
 *   - "Generated by Odoo" is unticked
 * ---------------------------------------------------------------------------------------------
 *  Grounding
 ...
 *    License, named with a fresh timestamp and the marker "TEST CRM-12370_7.4.3", so two
...
const STEP = {
  ...
  s1: 'Step 1: Read the "Licensing" field of the General group',
  s2: 'Step 2: Read the "Msp License" and "Generated by Odoo" check boxes',
  verify: 'Verification',
} as const;

test.describe(`${TC} - General - Licensing reads Perpetual and the two check boxes of a chain-generated License are unticked`, () => {
  ...
  test(`${TC}: General - Licensing reads Perpetual and the two check boxes of a chain-generated License are unticked`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s2, async () => {
      console.log(`\n--- ${STEP.s2} ---`);
      mspLicense = await licensePage.isCheckboxChecked('msp_license');
      generatedByOdoo = await licensePage.isGeneratedByOdooChecked();
      console.log(`  - Msp License        : ${mspLicense ? 'ticked' : 'unticked'}`);
      console.log(`  - Generated by Odoo  : ${generatedByOdoo ? 'ticked' : 'unticked'}`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - Licensing and check boxes read');
    });

    await test.step(STEP.verify, async () => {
      console.log(`\n--- ${STEP.verify} ---`);
      record('"Licensing" is filled', 'a non-empty value', licensing || '(empty)', licensing.length > 0);
      record('"Licensing" value', 'Perpetual', licensing);
      record('"Msp License" check box', 'unticked', mspLicense ? 'ticked' : 'unticked', !mspLicense);
      record('"Generated by Odoo" check box', 'unticked', generatedByOdoo ? 'ticked' : 'unticked', !generatedByOdoo);
      printVerify();

      expect(licensing, '"Licensing" must not be empty').not.toBe('');
      expect(licensing, '"Licensing" must read Perpetual for a perpetual product').toBe('Perpetual');
      expect(mspLicense, '"Msp License" must stay unticked on a licence the CRM chain generated').toBe(false);
      expect(generatedByOdoo, '"Generated by Odoo" must stay unticked on a licence the CRM chain generated').toBe(false);
    });
  });
});
