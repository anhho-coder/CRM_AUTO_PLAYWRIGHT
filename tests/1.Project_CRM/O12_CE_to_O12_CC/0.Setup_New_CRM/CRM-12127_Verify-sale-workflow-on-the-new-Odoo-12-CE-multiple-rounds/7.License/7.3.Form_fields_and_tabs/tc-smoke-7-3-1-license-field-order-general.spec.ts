 ...
 *  =============================================================================================
 * =============================================================================================
 *  Test Case ID    : CRM-12370_7.3.1
 *  Jira            : -   (ported from pre-production TC.Performance.7.3.1; no Xray manual TC)
 ...
 * ---------------------------------------------------------------------------------------------
 *  Summary
 *    Creates its own licence and verifies that the "General" group of the saved licence carries the
 *    documented fields, in the documented order, exactly as a tester reads them on the screen.
 * ---------------------------------------------------------------------------------------------
 *  Command to run
 *    npx playwright test --grep "CRM-12370_7\.3\.1:" --project=MigSmoke
 * ---------------------------------------------------------------------------------------------
 *  Source manual TC (pre-production): TC.Performance.7.3.1
 ...
 *
 *  Steps to reproduce
 *   1. Read the field labels of the "General" group of the licence, top to bottom
 *
 *  Verification
 *   - The "General" group carries exactly 8 fields
 *   - They read Installer provider, Owner Name, Uuid, Licensing, Msp License, Generated by Odoo, License to Renew, Cancel reason in that order
 * ---------------------------------------------------------------------------------------------
 *  Grounding
 ...
 *    License, named with a fresh timestamp and the marker "TEST CRM-12370_7.3.1", so two
...
const STEP = {
  ...
  s1: 'Step 1: Read the field labels of the "General" group of the licence, top to bottom',
  verify: 'Verification',
} as const;

test.describe(`${TC} - The field names and their order in the "General" group of the License information area`, () => {
  ...
  test(`${TC}: The field names and their order in the "General" group of the License information area`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s1, async () => {
      console.log(`\n--- ${STEP.s1} ---`);
      const columns = await licensePage.getGroupColumns('General');
      labels = columns.left;
      labels.forEach((l, i) => console.log(`  - Field #${i + 1}         : ${l}`));
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Steps to reproduce - "General" field list read');
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      record('Number of fields in the "General" group', EXPECTED.length, labels.length);
      record('Field names and their order', EXPECTED.join(' | '), labels.join(' | '));
      printVerify();

      expect(labels, 'the "General" group must carry exactly ' + EXPECTED.length + ' fields').toHaveLength(EXPECTED.length);
