...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Empty fields on creation
 * Test Case ID: CRM-12370_3.3.15
 * Automation-Type: new
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.26
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the fields that stay empty on the saved form.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_3.3.15 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_3\\.3\\.15:" --project=MigSmoke
 */

const TC = 'CRM-12370_3.3.15';
...
test.describe(`${TC} - Empty fields on creation`, () => {
  ...
  test(`${TC}: Verify the Contact information fields that stay empty on a newly created Contact`, async ({ page }, testInfo) => {
