...
/**
 * O12 CE Main-Business Smoke - Contact screen verification - Lifetime Revenue and Recent engagement
 * Test Case ID: CRM-12370_3.3.13
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Objective: Verify the Lifetime Revenue and the Recent engagement of a newly created Contact are zero
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.1.1.3.24
 ...
 *   4. Press "SAVE" button.
 *   5. Verifying the Lifetime Revenue and the Recent engagement.
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Contact it makes carries
 * the marker "TEST CRM-12370_3.3.13 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_3\\.3\\.13:" --project=MigSmoke
 */

const TC = 'CRM-12370_3.3.13';
...
const STEP = {
  ...
  s5: 'Step 5: Verifying the Lifetime Revenue and the Recent engagement',
};

test.describe(`${TC} - Lifetime Revenue and Recent engagement`, () => {
  let contactUrl = '';
  ...
  test(`${TC}: Verify the Lifetime Revenue and the Recent engagement of a newly created Contact are zero`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s5, async () => {
      console.log(`\n--- ${STEP.s5} ---`);
      const revenue = await contactPage.getFieldNumber('lifetime_revenue');
      const revenueText = await contactPage.getFieldDisplayValue('lifetime_revenue');
      const engagement = await contactPage.getFieldNumber('recent_engagement_score');

      console.log('==================== VERIFY ====================');
      console.log('  Verify #1 - the Lifetime Revenue of a Contact that has never bought:');
      console.log(`     Expected : 0 (printed as a monetary amount)`);
      console.log(`     Actual   : ${revenueText} -> ${revenue}`);
      console.log(`     Result   : ${revenue === 0 ? 'PASS' : 'FAIL'}`);
      console.log('  Verify #2 - the Recent engagement (minutes):');
      console.log(`     Expected : 0`);
      console.log(`     Actual   : ${engagement}`);
      console.log(`     Result   : ${engagement === 0 ? 'PASS' : 'FAIL'}`);
      console.log('===============================================');
