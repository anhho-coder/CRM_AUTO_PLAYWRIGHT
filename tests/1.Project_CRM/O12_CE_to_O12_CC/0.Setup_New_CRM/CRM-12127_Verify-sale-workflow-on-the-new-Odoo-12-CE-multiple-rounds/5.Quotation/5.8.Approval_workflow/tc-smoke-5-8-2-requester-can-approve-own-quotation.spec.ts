 ...
 *
 * Source: the single-actor variant of TC.Performance.1.1.5.4 / CRM-12370_5.1.4 "Approve Quotation".
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_5.8.2:" --project=chromium
 */
...
test.describe('CRM-12370_5.8.2 - O12 CE smoke: the requester approves his own Quotation', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.8.2');
  });

  test('CRM-12370_5.8.2: Verify the Quotation requester can APPROVE his own Quotation on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12370_5.8.2';
