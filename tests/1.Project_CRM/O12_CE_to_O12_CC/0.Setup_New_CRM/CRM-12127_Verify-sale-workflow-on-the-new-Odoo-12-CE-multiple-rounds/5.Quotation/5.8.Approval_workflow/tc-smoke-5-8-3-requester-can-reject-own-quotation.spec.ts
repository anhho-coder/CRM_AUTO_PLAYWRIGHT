 ...
 *
 * Source: the single-actor variant of TC.Performance.1.1.5.5 / CRM-12370_5.1.5 "Reject Quotation".
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_5.8.3:" --project=chromium
 */
...
test.describe('CRM-12370_5.8.3 - O12 CE smoke: the requester rejects his own Quotation', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.8.3');
  });

  test('CRM-12370_5.8.3: Verify the Quotation requester can REJECT his own Quotation on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12370_5.8.3';
