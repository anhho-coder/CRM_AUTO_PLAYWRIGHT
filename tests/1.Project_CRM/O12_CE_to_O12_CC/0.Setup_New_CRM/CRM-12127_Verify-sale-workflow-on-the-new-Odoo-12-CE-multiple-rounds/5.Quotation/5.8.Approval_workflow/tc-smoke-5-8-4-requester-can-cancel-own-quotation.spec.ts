 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_5.8.4:" --project=chromium
 */
...
test.describe('CRM-12370_5.8.4 - O12 CE smoke: the requester can cancel his own pending-approval Quotation', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.8.4');
  });

  test('CRM-12370_5.8.4: Verify the Quotation requester can Cancel his own pending-approval Quotation on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12370_5.8.4';
