 ...
 * by Email composer" (raised from CRM-12370_5.1.3; root cause per CRM-12413 = a mail add-on enabled
 * only on the Migration server replaced the recipient list; the add-on is now disabled on crm-mig to
 * match Production). This TC is the REGRESSION GUARD for that fix: CRM-12370_5.1.3 only presses SEND
 ...
 *     CRM-12370_5.1.3, the TC this case guards; CRM > Pipeline opened in list view by URL hash.
 ...
 *   - This TC does NOT send the email (sending is covered by CRM-12370_5.1.3): after reading the
 *     Recipients row it closes the composer with "Cancel", so no mail leaves the Migration server.
 *   - Like CRM-12370_5.1.3 it needs the created Quotation OPEN on screen, so it asserts that
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_5.8.1:" --project=chromium
 */
...
test.describe('CRM-12370_5.8.1 - O12 CE smoke: Quotation Payer is proposed as email recipient', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.8.1');
  });

  test('CRM-12370_5.8.1: Verify the Quotation Payer is auto-filled as a recipient in the "Send by Email" composer on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12370_5.8.1';
