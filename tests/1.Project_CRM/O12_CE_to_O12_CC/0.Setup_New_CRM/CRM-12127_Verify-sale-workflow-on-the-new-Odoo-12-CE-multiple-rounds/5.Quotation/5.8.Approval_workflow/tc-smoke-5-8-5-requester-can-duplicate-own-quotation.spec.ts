...
test.describe('CRM-12370_5.8.5 - O12 CE smoke: the requester can duplicate his own pending-approval Quotation', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.8.5');
  });

  test('CRM-12370_5.8.5: Verify the Quotation requester can Duplicate his own pending-approval Quotation on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12370_5.8.5';
