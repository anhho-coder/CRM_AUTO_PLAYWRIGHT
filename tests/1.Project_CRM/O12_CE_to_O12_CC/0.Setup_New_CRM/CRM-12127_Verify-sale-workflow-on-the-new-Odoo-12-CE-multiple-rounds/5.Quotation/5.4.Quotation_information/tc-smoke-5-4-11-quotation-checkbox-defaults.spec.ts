...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - Checkbox defaults of the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_5.4.11
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify a freshly created Quotation mails nothing by itself (Send mail unticked) but offers
 *   online payment (Online Payment ticked).
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.27 "Checkbox defaults of the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_5.4.11 <timestamp>
 ...
 *     6. Read the "Send mail" and "Online Payment" checkboxes of the created Quotation.
 *
 *   Verification:
 *     1. Send mail is unticked.
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_5\.4\.11:" --project=MigSmoke
 */

const TC = 'CRM-12370_5.4.11';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Verify the checkbox defaults',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - Checkbox defaults of the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.4.11');
  });

  test(`${TC}: Verify Send mail is unticked and Online Payment is ticked on a new Quotation`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      console.log(`\n--- ${STEP.s13} ---`);
      sendMail = await quotationPage.isFieldChecked('send_mail');
      onlinePayment = await quotationPage.isFieldChecked('require_payment');

      console.log(`  - Send mail      : ${sendMail ? 'ticked' : 'unticked'}`);
      console.log(`  - Online Payment : ${onlinePayment ? 'ticked' : 'unticked'}`);

      record('Send mail', 'unticked', sendMail ? 'ticked' : 'unticked');
      record('Online Payment', 'ticked', onlinePayment ? 'ticked' : 'unticked');
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation checkbox defaults`);

      expect(sendMail, 'a new Quotation must not mail anything by itself').toBe(false);
      expect(onlinePayment, 'a new Quotation must offer online payment').toBe(true);
    });
  });
});
