 ...
 *          - Opp name    = TEST CRM-12370_5.4.10 <timestamp>
 ...
 *   - Total in Company Currency is a developer-only field (see CRM-12370_5.4.13), so this TC has to
 *   - reopen the record with ?debug on to read it at all.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_5\.4\.10:" --project=MigSmoke
 */

const TC = 'CRM-12370_5.4.10';
...
test.describe(`${TC} - Total in Company Currency of a foreign-currency Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.4.10');
  });
