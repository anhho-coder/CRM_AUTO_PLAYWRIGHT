...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - CONFIRM button on the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_5.2.9
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify CONFIRM moves the record on to the SALE ORDER stage, consumes the CONFIRM button and
 *   offers LOCK instead.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.14 "CONFIRM button on the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_5.2.9 <timestamp>
 ...
 *     6. Press "CONFIRM" and read the resulting stage of the record.
 *
 *   Verification:
 *     1. The record moves on to the SALE ORDER stage.
 *     2. The CONFIRM button is consumed by the confirmation.
 *     3. The record ends up a Sale Order.
 *
 ...
 *
 * O12 CE notes:
 *   - Odoo does not re-render the statusbar / header buttons after a state action, so the page
 *   - objects behind these reads reload before each poll pass.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_5\.2\.9:" --project=MigSmoke
 */

const TC = 'CRM-12370_5.2.9';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Press CONFIRM and verify the Sale Order stage',
  verify: 'Verification',
} as const;

/** The stage the record sits on once CONFIRM has been pressed. */
const EXPECTED_STAGE_AFTER_CONFIRM = 'SALE ORDER';
...
test.describe(`${TC} - CONFIRM button on the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.2.9');
  });

  test(`${TC}: Verify the CONFIRM button turns the Quotation into a Sale Order`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      ...
      const verdict = await quotationPage.verifyConfirmedToSalesOrder();
      confirmed = verdict.confirmed;
      confirmButtonGone = verdict.confirmButtonGone;
      lockVisible = verdict.lockVisible;
      stageAfter = await quotationPage.getActiveStatusBarStage();

      console.log(`  Confirmed to Sale Order : ${confirmed}`);
      console.log(`  Stage after CONFIRM     : ${stageAfter}`);
      console.log(`  CONFIRM button gone     : ${confirmButtonGone}`);
      console.log(`  LOCK button offered     : ${lockVisible}`);

      record('Confirmed to Sale Order', 'true', String(confirmed));
      record('Stage after CONFIRM', EXPECTED_STAGE_AFTER_CONFIRM, stageAfter);
      record('CONFIRM button gone', 'true', String(confirmButtonGone));
      record('LOCK button offered', 'true', String(lockVisible));
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation confirmed to Sale Order`);

      expect(stageAfter, 'CONFIRM must move the record on to the Sale Order stage').toBe(
        EXPECTED_STAGE_AFTER_CONFIRM
      );
      expect(confirmButtonGone, 'CONFIRM must be consumed by the confirmation').toBe(true);
      expect(confirmed, 'the record must end up a Sale Order').toBe(true);
    });
  });
});
