...
/**
 * =============================================================================
 * O12 CE Main-Business Smoke - PRINT button on the Quotation
 * =============================================================================
 * Test Case ID   : CRM-12370_5.2.4
 * Jira           : CRM-12325
 * Automation-Type: new
 * Automation-Date: 2026-09-21
 *
 * Summary:
 *   Verify PRINT hands the browser the Quotation document as a PDF and raises no Odoo error
 *   dialog.
 *
 * -----------------------------------------------------------------------------
 * Source manual TC (pre-production): TC.Performance.1.1.5.9 "PRINT button on the Quotation"
 * -----------------------------------------------------------------------------
 *   Pre-condition(s):
 ...
 *          - Opp name    = TEST CRM-12370_5.2.4 <timestamp>
 ...
 *     6. Press "PRINT" and read the document the browser is handed.
 *
 *   Verification:
 *     1. A download starts.
 *     2. The downloaded file is a PDF.
 *     3. PRINT raises no Odoo error dialog.
 *
 ...
 *
 * O12 CE notes:
 *   - crm-mig has no wkhtmltopdf installed, so every /report/pdf route answers 500. Under the
 *   - baseline rule this TC is therefore EXPECTED TO FAIL on crm-mig until wkhtmltopdf is
 *   - installed - the assertion is NOT relaxed to accommodate it.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_5\.2\.4:" --project=MigSmoke
 */

const TC = 'CRM-12370_5.2.4';

/** Step labels - one source of truth for the test.step() label AND the stdout banner. */
const STEP = {
  s12: 'Step 12: Press "NEW QUOTATION" on the saved Deal Element to create the Quotation',
  s13: 'Step 13: Press PRINT and verify the document',
  verify: 'Verification',
} as const;
...
test.describe(`${TC} - PRINT button on the Quotation`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.2.4');
  });

  test(`${TC}: Verify the PRINT button produces the Quotation PDF`, async ({ page }, testInfo) => {
    ...
    await test.step(STEP.s13, async () => {
      console.log(`\n--- ${STEP.s13} ---`);
      const print = await quotationPage.clickPrintAndWaitForDownload();
      printStarted = print.started;
      printFile = print.suggestedFilename;
      printPopup = print.popupText;
      console.log(`  Download started    : ${printStarted}`);
      console.log(`  Suggested file name : ${printFile || '(none)'}`);
      console.log(`  Odoo popup text     : ${printPopup || '(none)'}`);

      record('A download started', 'true', String(printStarted));
      record('Downloaded file is a PDF', 'true', String(/\.pdf$/i.test(printFile)));
      record('Odoo error dialog', '(none)', printPopup === '' ? '(none)' : printPopup);
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      printVerify();
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Quotation PRINT outcome`);

      expect(printStarted, 'PRINT must hand the browser the Quotation document').toBe(true);
      expect(printFile, 'the Quotation document must be a PDF').toMatch(/\.pdf$/i);
      expect(printPopup, 'PRINT must not raise an Odoo error dialog').toBe('');
    });
  });
});
