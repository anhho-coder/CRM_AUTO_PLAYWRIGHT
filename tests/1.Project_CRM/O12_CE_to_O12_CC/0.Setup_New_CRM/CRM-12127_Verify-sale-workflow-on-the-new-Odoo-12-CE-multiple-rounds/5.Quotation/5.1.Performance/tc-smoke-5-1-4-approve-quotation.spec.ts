...
test.describe('CRM-12370_5.1.4 - O12 CE smoke: approve a Quotation', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_5.1.4');
  });

  test('CRM-12370_5.1.4: Verify a Quotation can be approved on the O12 CE Migration server', async ({ page, browser }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const quotationPage = new QuotationPage(page);

    const TC_ID = 'CRM-12370_5.1.4';
