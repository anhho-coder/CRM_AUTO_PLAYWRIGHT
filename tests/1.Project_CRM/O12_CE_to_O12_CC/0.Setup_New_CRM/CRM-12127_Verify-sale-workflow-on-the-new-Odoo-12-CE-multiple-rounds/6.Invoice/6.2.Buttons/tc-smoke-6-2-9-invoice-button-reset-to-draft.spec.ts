...
/**
 * O12 CE Main-Business Smoke - Invoice button - RESET TO DRAFT
 * Test Case ID: CRM-12370_6.2.9
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify pressing RESET TO DRAFT on a cancelled Invoice puts it back on the Draft stage with the Draft header set.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.2.9
 ...
 *   2. Press RESET TO DRAFT to move it back to Draft.
 *   3. Read the stage, caption, and header buttons.
 *
 * Verification:
 *   - Active stage after RESET TO DRAFT is DRAFT
 *   - Caption after RESET TO DRAFT is "Draft Invoice"
 *   - Header buttons after RESET TO DRAFT are VALIDATE, PREVIEW, CANCEL
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Opportunity it makes carries
 * the marker "TEST CRM-12370_6.2.9 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.2\.9:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 15: Validating, cancelling, resetting, then verifying the Draft Invoice state',
};

test.describe('CRM-12370_6.2.9 - Invoice RESET TO DRAFT button', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.2.9');
  });

  test('CRM-12370_6.2.9: Verify pressing RESET TO DRAFT restores the Invoice to Draft state', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.2.9';
    ...
    await test.step(STEP.s2, async () => {
      ...
      // Same stale-render trap in the other direction: the bar can still highlight CANCELLED
      // when action_invoice_draft returns. Poll until the product's own DRAFT state is on the
      // screen, then assert it.
      await invoicePage.waitForActiveStage('DRAFT');
      ...
      console.log(`  - After RESET TO DRAFT: stage="${stage}" caption="${caption}" buttons="${buttons.join(' | ')}"`);

      record('Active stage after RESET TO DRAFT', 'DRAFT', stage);
      record('Caption after RESET TO DRAFT', 'Draft Invoice', caption);
      record('Header buttons after RESET TO DRAFT', 'VALIDATE | PREVIEW | CANCEL', buttons.join(' | '));
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC_ID} - Draft Invoice after reset`);

      expect(stage, 'RESET TO DRAFT must put the Invoice back on DRAFT').toBe('DRAFT');
      expect(caption, 'a Draft Invoice must print "Draft Invoice" where the number goes').toBe('Draft Invoice');
      expect(buttons, 'the Draft header set must be offered again').toEqual(['VALIDATE', 'PREVIEW', 'CANCEL']);
    });
  });
});
