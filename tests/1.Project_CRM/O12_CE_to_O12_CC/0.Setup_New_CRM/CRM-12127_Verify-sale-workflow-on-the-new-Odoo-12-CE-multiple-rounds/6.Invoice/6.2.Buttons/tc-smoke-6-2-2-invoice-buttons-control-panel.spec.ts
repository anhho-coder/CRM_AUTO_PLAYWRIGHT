...
/**
 * O12 CE Main-Business Smoke - Invoice buttons - Control panel
 * Test Case ID: CRM-12370_6.2.2
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify the control panel of an Invoice opened straight from its Sales Order offers EDIT, Print and Action - and no CREATE, because the Sales Order route opens the record without the create right of the Invoices menu.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.2.2
 ...
 *
 * Steps run:
 *   1. Read the buttons of the control panel above the Invoice form, left to right.
 *
 * Verification:
 *   - The control panel carries exactly 3 buttons
 *   - They read EDIT, Print, Action in that order
 *   - CREATE is NOT offered on this route
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Opportunity it makes carries
 * the marker "TEST CRM-12370_6.2.2 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.2\.2:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 15: Verifying the control-panel buttons of the Invoice',
};

test.describe('CRM-12370_6.2.2 - Invoice control-panel buttons', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.2.2');
  });

  test('CRM-12370_6.2.2: Verify the control-panel buttons of the Invoice', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.2.2';
    ...
    await test.step(STEP.s2, async () => {
      console.log(`\n--- ${STEP.s2} ---`);
      const EXPECTED = ['EDIT', 'Print', 'Action'];

      const invoicePage = new InvoicePage(page);
      const buttons = await invoicePage.getControlPanelButtons();

      record('Number of control-panel buttons', EXPECTED.length, buttons.length);
      record('Control-panel button names and order', EXPECTED.join(' | '), buttons.join(' | '));
      record('CREATE is not offered', 'absent', buttons.includes('CREATE') ? 'present' : 'absent');
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC_ID} - Control panel buttons`);

      expect(buttons, 'the Invoice control panel must carry exactly 3 buttons').toHaveLength(EXPECTED.length);
      expect(buttons, 'the Invoice control panel must read EDIT, Print, Action').toEqual(EXPECTED);
    });
  });
});
