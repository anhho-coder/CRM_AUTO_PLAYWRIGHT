...
/**
 * O12 CE Main-Business Smoke - Invoice button - Print menu
 * Test Case ID: CRM-12370_6.2.10
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify the Print dropdown of the Invoice control panel offers the three invoice reports - Invoices, Invoices without Payment and Invoices Siemens.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.2.10
 ...
 *
 * Steps run:
 *   1. Read the items in the Print dropdown menu of the Invoice control panel.
 *
 * Verification:
 *   - The Print menu carries exactly 3 reports
 *   - They are Invoices, Invoices without Payment, Invoices Siemens in that order
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Opportunity it makes carries
 * the marker "TEST CRM-12370_6.2.10 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.2\.10:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 15: Verifying the Print menu items',
};

test.describe('CRM-12370_6.2.10 - Invoice Print menu', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.2.10');
  });

  test('CRM-12370_6.2.10: Verify the Print menu items', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.2.10';
    ...
    await test.step(STEP.s2, async () => {
      console.log(`\n--- ${STEP.s2} ---`);
      const EXPECTED = ['Invoices', 'Invoices without Payment', 'Invoices Siemens'];

      const invoicePage = new InvoicePage(page);
      const items = await invoicePage.getControlPanelMenuItems('Print');
      console.log(`  - Print menu: ${items.join(' | ')}`);

      record('Number of Print menu items', EXPECTED.length, items.length);
      record('Print menu items and order', EXPECTED.join(' | '), items.join(' | '));
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC_ID} - Print menu`);

      expect(items, 'the Invoice Print menu must carry exactly 3 reports').toHaveLength(EXPECTED.length);
      expect(items, 'the Invoice Print menu must offer the documented reports in order').toEqual(EXPECTED);
    });
  });
});
