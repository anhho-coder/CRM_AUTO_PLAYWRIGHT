...
/**
 * O12 CE Main-Business Smoke - Invoice button - Action menu
 * Test Case ID: CRM-12370_6.2.11
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify the Action dropdown of the Invoice control panel offers the six documented entries, Delete among them.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.2.11
 ...
 *
 * Steps run:
 *   1. Read the items in the Action dropdown menu of the Invoice control panel.
 *
 * Verification:
 *   - The Action menu carries exactly 6 items
 *   - They are Bookings, Dasboard - Invoices, Delete, Share, Export Transaction, Send Reminder in that order
 ...
 * the marker "TEST CRM-12370_6.2.11 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.2\.11:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 15: Verifying the Action menu items',
};

test.describe('CRM-12370_6.2.11 - Invoice Action menu', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.2.11');
  });

  test('CRM-12370_6.2.11: Verify the Action menu items', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.2.11';
    ...
    await test.step(STEP.s2, async () => {
      console.log(`\n--- ${STEP.s2} ---`);
      // "Dasboard - Invoices" is the caption the product ships - the typo is reproduced on purpose,
      // asserting the corrected spelling would make this test case fail against a correct product.
      const EXPECTED = ['Bookings', 'Dasboard - Invoices', 'Delete', 'Share', 'Export Transaction', 'Send Reminder'];

      const invoicePage = new InvoicePage(page);
      const items = await invoicePage.getControlPanelMenuItems('Action');
      console.log(`  - Action menu: ${items.join(' | ')}`);

      record('Number of Action menu items', EXPECTED.length, items.length);
      record('Action menu items and order', EXPECTED.join(' | '), items.join(' | '));
      record('Action menu offers Delete', 'present', items.includes('Delete') ? 'present' : 'absent');
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC_ID} - Action menu`);

      expect(items, 'the Invoice Action menu must carry exactly 6 items').toHaveLength(EXPECTED.length);
      expect(items, 'the Invoice Action menu must offer the documented entries in order').toEqual(EXPECTED);
    });
  });
});
