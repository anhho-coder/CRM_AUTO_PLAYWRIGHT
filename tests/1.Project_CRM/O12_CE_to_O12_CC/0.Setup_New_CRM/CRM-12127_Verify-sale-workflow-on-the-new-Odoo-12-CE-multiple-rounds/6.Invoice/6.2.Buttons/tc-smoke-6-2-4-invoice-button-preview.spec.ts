...
/**
 * O12 CE Main-Business Smoke - Invoice button - PREVIEW
 * Test Case ID: CRM-12370_6.2.4
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify pressing PREVIEW on the Invoice opens the customer-facing portal page of that invoice.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.2.4
 ...
 *
 * Steps run:
 *   1. Press the PREVIEW button on the Invoice header.
 *   2. Verify the portal page opened.
 *
 * Verification:
 *   - The PREVIEW action opens a new window / tab with the portal URL
 *   - The portal page is accessible
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Opportunity it makes carries
 * the marker "TEST CRM-12370_6.2.4 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.2\.4:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 15: Pressing PREVIEW and verifying the portal page opens',
};

test.describe('CRM-12370_6.2.4 - Invoice PREVIEW button', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.2.4');
  });

  test('CRM-12370_6.2.4: Verify pressing PREVIEW opens the customer-facing portal page', async ({ page, context }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.2.4';
    ...
    await test.step(STEP.s2, async () => {
      ...
      let portalPageUrl = '';
      let portalPageOpened = false;

      // Listen for popup window
      const [portalPage] = await Promise.all([
        context.waitForEvent('page'),
        invoicePage.clickStatusbarButtonByName('preview_invoice', CommonUtils.waitTimes.pageLoad),
      ]);

      portalPageUrl = portalPage.url();
      portalPageOpened = portalPageUrl.length > 0 && /share|portal/.test(portalPageUrl);
      console.log(`  - Portal page URL: ${portalPageUrl}`);
      console.log(`  - Portal page opened: ${portalPageOpened}`);

      record('PREVIEW opens a portal page', 'yes', portalPageOpened ? 'yes' : 'no');
      record('Portal URL contains share or portal', 'yes', /share|portal/.test(portalPageUrl) ? 'yes' : 'no');
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC_ID} - PREVIEW portal`);

      expect(portalPageOpened, 'PREVIEW must open a portal page with a URL').toBe(true);
      expect(/share|portal/.test(portalPageUrl), 'the portal URL must contain share or portal').toBe(true);
