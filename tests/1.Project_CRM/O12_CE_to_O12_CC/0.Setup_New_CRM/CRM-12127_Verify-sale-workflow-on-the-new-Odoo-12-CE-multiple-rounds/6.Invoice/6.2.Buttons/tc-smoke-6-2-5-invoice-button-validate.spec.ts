...
/**
 * O12 CE Main-Business Smoke - Invoice button - VALIDATE
 * Test Case ID: CRM-12370_6.2.5
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify pressing VALIDATE moves the Invoice from Draft to Open and replaces the "Draft Invoice" caption with the invoice number Odoo assigns.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.2.5
 ...
 *
 * Steps run:
 *   1. Read the caption and the stage of the Draft Invoice.
 *   2. Press the "VALIDATE" button of the Invoice header.
 *   3. Read the stage and the invoice number again.
 *
 * Verification:
 *   - Before VALIDATE the sheet prints "Draft Invoice" and the stage is DRAFT
 *   - After VALIDATE the stage is OPEN
 *   - After VALIDATE the Invoice carries a number of the form INV/<year>/<sequence>
 *
 * crm-mig house rule: this spec CREATES data and removes it in teardown. The Opportunity it makes carries
 * the marker "TEST CRM-12370_6.2.5 <timestamp>" so the afterAll sweep can find it again even when the test dies
 * mid-way.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.2\.5:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 15: Pressing VALIDATE and verifying the Invoice is posted',
};

test.describe('CRM-12370_6.2.5 - Invoice VALIDATE button', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.2.5');
  });

  test('CRM-12370_6.2.5: Verify the VALIDATE button posts the Invoice and gives it a number', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.2.5';
    ...
    await test.step(STEP.s2, async () => {
      ...
      const captionBefore = await invoicePage.getInvoiceTitleCaption();
      const stageBefore = await invoicePage.getActiveStatusBarStage();
      console.log(`  - Before VALIDATE: caption="${captionBefore}" stage="${stageBefore}"`);
      ...
      const stageAfter = await invoicePage.getActiveStatusBarStage();
      const number = await invoicePage.getInvoiceNumberOrEmpty();
      const numberOk = /^INV\/\d{4}\/\d+$/.test(number);
      console.log(`  - After VALIDATE : stage="${stageAfter}" number="${number}"`);

      record('Caption before VALIDATE', 'Draft Invoice', captionBefore);
      record('Stage before VALIDATE', 'DRAFT', stageBefore);
      record('Stage after VALIDATE', 'OPEN', stageAfter);
      record('Invoice number after VALIDATE', 'INV/<year>/<sequence>', number || '(empty)', numberOk);
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC_ID} - Invoice validated`);

      expect(captionBefore, 'a Draft Invoice must print "Draft Invoice" where the number goes').toBe('Draft Invoice');
      expect(stageBefore, 'a newly created Invoice must sit on DRAFT').toBe('DRAFT');
      expect(stageAfter, 'VALIDATE must move the Invoice to OPEN').toBe('OPEN');
      expect(numberOk, `VALIDATE must assign an INV/<year>/<sequence> number, got "${number}"`).toBe(true);
    });
  });
});
