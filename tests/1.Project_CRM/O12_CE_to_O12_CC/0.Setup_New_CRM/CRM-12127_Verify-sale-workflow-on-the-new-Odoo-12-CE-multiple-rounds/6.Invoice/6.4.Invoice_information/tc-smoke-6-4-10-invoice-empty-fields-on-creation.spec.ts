...
/**
 * O12 CE Main-Business Smoke - Invoice data verification - Numeric defaults and empty fields on new Invoice
 * Test Case ID: CRM-12370_6.4.10
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify numeric defaults and empty fields on new Invoice.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.4.10
 ...
 *
 * Steps run:
 *   1. Read the empty and numeric fields.
 *
 * Verification Points:
 *   1. Actually Received = 0.00.
 *   2. Quantity of used license = 0.
 *   3. PO is empty.
 *   4. Postpone Date is empty.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.4\.10:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 1: Verifying numeric defaults and empty fields',
};

test.describe('CRM-12370_6.4.10 - Numeric defaults and empty fields on new Invoice', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.4.10');
  });

  test('CRM-12370_6.4.10: Verify numeric defaults and empty fields on new Invoice', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.4.10';
    ...
    await test.step(STEP.s2, async () => {
      console.log('\n--- Step 1: Verifying numeric defaults and empty fields ---');
      const invoicePage = new InvoicePage(page);
      const actuallyReceived = await invoicePage.getFieldNumber('actually_received');
      const quantityUsedLicense = await invoicePage.getFieldNumber('quantity_used_license');
      const poNumber = await invoicePage.getFieldDisplayValue('po_number');
      const postponeDate = await invoicePage.getFieldDisplayValue('postpone_date');
      console.log(`  - Actually Received: ${actuallyReceived}`);
      console.log(`  - Quantity of used license: ${quantityUsedLicense}`);
      console.log(`  - PO: "${poNumber}"`);
      console.log(`  - Postpone Date: "${postponeDate}"`);

      record('Actually Received = 0.00', '0.00', actuallyReceived);
      record('Quantity of used license = 0', '0', quantityUsedLicense);
      record('PO is empty', '(empty)', poNumber || '(empty)');
      record('Postpone Date is empty', '(empty)', postponeDate || '(empty)');
      printVerify();

      let __verifyPassed = false;
      try {
        expect(actuallyReceived, 'Actually Received must default to 0.00').toBe('0.00');
        expect(quantityUsedLicense, 'Quantity of used license must default to 0').toBe('0');
        expect(poNumber, 'PO must be empty on a new Invoice').toBe('');
        expect(postponeDate, 'Postpone Date must be empty on a new Invoice').toBe('');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: `${TC_ID} - Invoice field defaults`, passed: __verifyPassed }).catch(() => {});
      }
    });
  });
});
