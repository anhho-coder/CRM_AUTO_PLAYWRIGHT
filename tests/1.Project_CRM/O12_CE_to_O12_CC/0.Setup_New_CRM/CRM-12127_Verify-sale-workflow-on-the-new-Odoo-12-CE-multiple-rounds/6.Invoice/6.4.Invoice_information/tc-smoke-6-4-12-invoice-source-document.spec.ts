...
/**
 * O12 CE Main-Business Smoke - Invoice data verification - Source Document
 * Test Case ID: CRM-12370_6.4.12
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify Source Document references the Sales Order the Invoice was generated from.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.4.12
 ...
 *
 * Steps run:
 *   1. Open Other Info tab and read the Source Document field.
 *
 * Verification Points:
 *   1. Source Document is filled.
 *   2. Source Document matches SO<number> pattern.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.4\.12:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 1: Verifying Source Document',
};

test.describe('CRM-12370_6.4.12 - Source Document', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.4.12');
  });

  test('CRM-12370_6.4.12: Verify Source Document references the Sales Order', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.4.12';
    ...
    await test.step(STEP.s2, async () => {
      console.log('\n--- Step 1: Verifying Source Document ---');
      const invoicePage = new InvoicePage(page);
      // Source Document lives on the Other Info tab and is hidden while Invoice Lines is visible
      await invoicePage.openOtherInfoTab();
      const sourceDocument = await invoicePage.getSourceDocument();
      await invoicePage.clickInvoiceLinesTab();
      console.log(`  - Source Document: "${sourceDocument}"`);

      const sourceDocMatches = /^SO\d+/.test(sourceDocument);

      record('Source Document is filled', 'not empty', sourceDocument || '(empty)', sourceDocument !== '');
      record('Source Document matches SO<number> pattern', 'matches SO<number>', sourceDocument, sourceDocMatches);
      printVerify();

      let __verifyPassed = false;
      try {
        expect(sourceDocument, 'the Source Document must be filled').not.toBe('');
        expect(sourceDocument, 'the Source Document must match the SO<number> pattern').toMatch(/^SO\d+/);
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: `${TC_ID} - Invoice Source Document verification`, passed: __verifyPassed }).catch(() => {});
      }
    });
  });
});
