...
/**
 * O12 CE Main-Business Smoke - Invoice data verification - Distributor, Reseller and their contacts are empty on an Invoice raised from a direct Opportunity
 * Test Case ID: CRM-12370_6.4.3
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify an Invoice whose Opportunity names no partner leaves Distributor, Distributor contact, Reseller and Reseller contact empty.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.4.3
 ...
 *
 * Steps run:
 *   1. Read "Distributor", "Distributor contact", "Reseller" and "Reseller contact" fields.
 *
 * Verification Points:
 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.4\.3:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 1: Verifying Distributor, Reseller and their contacts are empty',
};

test.describe('CRM-12370_6.4.3 - Distributor, Reseller and their contacts are empty on an Invoice raised from a direct Opportunity', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.4.3');
  });

  test('CRM-12370_6.4.3: Verify Distributor, Reseller and their contacts are empty on an Invoice raised from a direct Opportunity', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.4.3';
    ...
    await test.step(STEP.s2, async () => {
      console.log('\n--- Step 1: Verifying Distributor, Reseller and their contacts are empty ---');
      const invoicePage = new InvoicePage(page);
      const distributor = await invoicePage.getFieldPartnerName('distributor_id');
      const distributorContact = await invoicePage.getFieldPartnerName('distributor_contact_id');
      const reseller = await invoicePage.getFieldPartnerName('reseller_id');
      const resellerContact = await invoicePage.getFieldPartnerName('reseller_contact_id');
      console.log(`  - Distributor: "${distributor}" | contact: "${distributorContact}"`);
      console.log(`  - Reseller   : "${reseller}" | contact: "${resellerContact}"`);

      record('Distributor is empty', '(empty)', distributor || '(empty)');
      record('Distributor contact is empty', '(empty)', distributorContact || '(empty)');
      record('Reseller is empty', '(empty)', reseller || '(empty)');
      record('Reseller contact is empty', '(empty)', resellerContact || '(empty)');
      printVerify();

      let __verifyPassed = false;
      try {
        expect(distributor, 'a direct Opportunity must leave the Invoice Distributor empty').toBe('');
        expect(distributorContact, 'a direct Opportunity must leave the Invoice Distributor contact empty').toBe('');
        expect(reseller, 'a direct Opportunity must leave the Invoice Reseller empty').toBe('');
        expect(resellerContact, 'a direct Opportunity must leave the Invoice Reseller contact empty').toBe('');
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: `${TC_ID} - Invoice partner fields verification`, passed: __verifyPassed }).catch(() => {});
      }
    });
  });
});
