...
/**
 * O12 CE Main-Business Smoke - Invoice data verification - The End User on the Invoice is filled and matches the Customer of the Opportunity
 * Test Case ID: CRM-12370_6.4.2
 * Automation-Type: refactored
 * Automation-Date: 2026-09-22
 *
 * Objective: Verify the Invoice inherits its End User through the chain Opportunity -> Deal Element -> Quotation -> Sales Order -> Invoice.
 *
 ...
 *
 * Source manual TC (pre-production): TC.Performance.6.4.2
 ...
 *
 * Steps run:
 *   1. Read the "End User" field of the Invoice information area.
 *
 * Verification Points:
 *   1. The End User is filled (not empty).
 *   2. The End User is the Company of the Opportunity.
 *   3. The End User equals the Payer, because the Opportunity names no separate end customer.
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_6\.4\.2:" --project=chromium
 */
...
const STEP = {
  ...
  s2: 'Step 1: Verifying the End User',
};

test.describe('CRM-12370_6.4.2 - The End User on the Invoice is filled and matches the Customer of the Opportunity', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_6.4.2');
  });

  test('CRM-12370_6.4.2: Verify the End User on the Invoice is filled and matches the Customer of the Opportunity', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const TC_ID = 'CRM-12370_6.4.2';
    ...
    await test.step(STEP.s2, async () => {
      console.log('\n--- Step 1: Verifying the End User ---');
      const invoicePage = new InvoicePage(page);
      const endUser = await invoicePage.getFieldPartnerName('partner_shipping_id');
      ...
      record('End User is filled', 'not empty', endUser || '(empty)', endUser !== '');
      record('End User matches the Opportunity Company', opp?.companyValue, endUser);
      record('End User equals the Payer', payer, endUser);
      printVerify();

      let __verifyPassed = false;
      try {
        expect(endUser, 'the Invoice End User must be filled').not.toBe('');
        expect(endUser, 'the Invoice End User must be the Company of the Opportunity').toBe(opp?.companyValue);
        expect(endUser, 'the End User must equal the Payer when no separate end customer is named').toBe(payer);
        __verifyPassed = true;
      } finally {
        await CommonUtils.captureVerifyEvidence(page, testInfo, { name: `${TC_ID} - Invoice End User verification`, passed: __verifyPassed }).catch(() => {});
      }
    });
  });
});
