...
test.describe('CRM-12370_4.1.2 - O12 CE smoke: edit a Deal Element', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_4.1.2');
  });

  test('CRM-12370_4.1.2: Verify a Deal Element can be edited on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const dealElementPage = new DealElementPage(page);

    const TC_ID = 'CRM-12370_4.1.2';
