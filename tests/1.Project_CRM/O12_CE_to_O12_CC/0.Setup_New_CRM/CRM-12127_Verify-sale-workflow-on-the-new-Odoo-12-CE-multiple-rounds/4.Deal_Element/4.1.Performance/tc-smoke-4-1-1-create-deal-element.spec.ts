 ...
 *
 * Command to run:
 *   npx playwright test --grep "CRM-12370_1\.4\.1:" --project=chromium
 */
...
test.describe('CRM-12370_4.1.1 - O12 CE smoke: create a Deal Element', () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_4.1.1');
  });

  test('CRM-12370_4.1.1: Verify a Deal Element can be created on the O12 CE Migration server', async ({ page }, testInfo) => {
    test.setTimeout(config.timeouts.test);
    await page.setViewportSize({ width: 1920, height: 1080 });

    const dealElementPage = new DealElementPage(page);

    const TC_ID = 'CRM-12370_4.1.1';
