...
/**
 * O12 CE Main-Business Smoke - Checkbox defaults of the information area
 * Test Case ID: CRM-12370_4.4.12
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Summary:
 *   Verify the two checkboxes of the information area keep their defaults on a Deal Element that was
 *   only created - Send mail unticked, Online Payment ticked.
 *
 * Source manual TC (pre-production): TC.Performance.1.1.4.18 "Deal Element checkbox defaults".
 *
 ...
 *   - `default_get` on crm-mig returns `send_mail` = false and `require_payment` = true, so these are
 *     the server defaults the screen must show.
 *
 * Pre-conditions:
 ...
 *  12. Read the Send mail and Online Payment checkboxes of the saved Deal Element.
 *
 * Verification Points:
...
test.describe(`${TC} - Checkbox defaults`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_4.4.12');
  });

  test('CRM-12370_4.4.12: Verify the Send mail and Online Payment checkboxes keep their defaults', async ({ page }, testInfo) => {
    ...
    await test.step('Verification', async () => {
      const sendMail = await dealElementPage.isFieldChecked('send_mail');
      const onlinePayment = await dealElementPage.isFieldChecked('require_payment');
      ...
      expect(sendMail, 'Send mail is off until the Deal Element is sent').toBe(false);
      expect(onlinePayment, 'Online Payment is on by default').toBe(true);
    });
  });
});
