...
/**
 * O12 CE Main-Business Smoke - Invoice Address taken from the Opportunity
 * Test Case ID: CRM-12370_4.4.2
 * Automation-Type: new
 * Automation-Date: 2026-09-17
 *
 * Summary:
 *   Verify the Deal Element Invoice Address is not left empty and carries the Company Odoo created for the
 *   Opportunity out of the e-mail domain.
 *
 * Source manual TC (pre-production): TC.Performance.1.1.4.8 "Deal Element Invoice Address from the Opportunity".
 *
 ...
 *  12. Read the Invoice Address back from the saved Deal Element and compare it with the Opportunity Company.
 *
 * Verification Points:
 *   1. Invoice Address is not empty on the saved Deal Element.
...
test.describe(`${TC} - Invoice Address from the Opportunity`, () => {
  ...
  test.afterAll(async ({ browser }) => {
    await sweepMigLeftoversAfterAll(browser, 'CRM-12370_4.4.2');
  });

  test('CRM-12370_4.4.2: Verify the Invoice Address is filled and matches the Company of the Opportunity', async ({ page }, testInfo) => {
    ...
    await test.step('Verification', async () => {
      const value = await dealElementPage.getFieldPartnerName('partner_invoice_id');

      record('Invoice Address is filled', 'not empty', value === '' ? '(empty)' : value, value !== '');
      record('Invoice Address = Opportunity Company', oppCompany, value === '' ? '(empty)' : value);
      printVerify();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `${TC} - Invoice Address read back`);

      expect(value, 'the Invoice Address must not be left empty on the Deal Element').not.toBe('');
