...
/**
 * ===========================================================================
 * CRM-12325 Part 2-A.2 - Contacts app opens cleanly and within a reasonable time
 * ===========================================================================
 ...
 *
 * Summary:
 *   Verifies the Contacts app opens on the Migration server with no server error / traceback
 ...
 *
 * ---------------------------------------------------------------------------
 * Source manual TC - Jira CRM-12338, Xray Manual Steps (verbatim, in order)
 * ---------------------------------------------------------------------------
 * Pre-conditions:
 *   _ Login: anh.ho@nakivo.com (admin_crm_mig) on crm-mig.nakivo.site
 *
 * Steps to reproduce #1:
 *   1. Open the Contacts app (via its menu) and let its action view render.
 ...
 *   _ The Contacts app loads within a reasonable time (response time under the ~30s budget)
 * ---------------------------------------------------------------------------
 */
...
const STEP = {
  ...
  s1:     'Step 1: Open the Contacts app (via its menu) and let its action view render.',
  verify: 'Verification',
} as const;
...
test.describe('CRM-12325 Part 2-A.2 - Contacts opens cleanly and fast', () => {
  ...
  test('CRM-12325_1.1.3: [Part2-A] Contacts app opens with no server error and within a reasonable time', async ({ page }, testInfo) => {
    ...
    console.log('========== CRM-12325_1.1.3 - Contacts app opens cleanly and fast ==========');
    ...
    await test.step(STEP.s1, async () => {
      ...
      console.log(`  Contacts opened - errorDialog=${hasError}, response time=${ms} ms`);
    });
    ...
    await test.step(STEP.verify, async () => {
      ...
      console.log('Verify #2 - The Contacts app loads within a reasonable time (response time under the ~30s budget):');
