...
/**
 * Invoice Cancel/Reset Test - Salesperson cancels the Invoice and resets it to Draft (single browser)
 * Test Case ID: TC.-A.7.4
 * Automation-Type: refactored
 * Automation-Date: 2026-06-05
 *
 * Summary: Verify the Salesperson can cancel the Invoice and then reset it to Draft successfully.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.7\.4:" --project=chromium
 *
 ...
 * 2. Press the "Set to Draft" button to reset the cancelled Invoice back to Draft
 *
 * Verification:
 * - The Invoice is cancelled then reset to "Draft" successfully.
 */

test.describe('TC.-A.7.4 - Salesperson cancels the Invoice and resets it to Draft', () => {
  ...
  test('TC.-A.7.4: Verify the Salesperson can cancel the Invoice and reset it to Draft successfully', async ({ page }, testInfo) => {
    ...
    // Step run 2: Reset the cancelled Invoice back to Draft
    await test.step('Step run 2: Press "Set to Draft" to reset the Invoice to Draft', async () => {
      console.log('Step run 2: Resetting the cancelled Invoice to Draft');
      await invoicePage.clickSetToDraft();
      await invoicePage.dismissErrorDialogWithRetry();
      console.log('✓ "Set to Draft" pressed');
    });

    // Verification: the Invoice is back to Draft
    await test.step('Verification: The Invoice is reset to Draft', async () => {
      console.log('Verification: Confirming the Invoice is back to Draft');
      let status = '';
      try {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.7.4 - Invoice after cancel + reset to Draft');
      expect(
        status,
        `After cancel + reset, the Invoice should be "Draft" (was "${status}")`
      ).toMatch(/Draft/i);
      console.log('✅ Invoice cancelled and reset to Draft successfully');
    });
  });
});
