...
/**
 * Quotation Approval Test - Sales Manager REJECTS a Quotation (two browsers)
 * Test Case ID: TC.-A.6.2
 *
 * Summary: Verify the Sales Manager can reject the Quotation successfully.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.6\.2:" --project=chromium
 *
 ...
 * 5. (Max) Press the "REJECT" button, enter a reason and confirm the rejection
 *
 * Verification:
 * - The rejection process is successful: after Max rejects, the pending-approval buttons
 *   ("APPROVE" / "REJECT") are gone and the Quotation returns to an editable state.
 */

test.describe('TC.-A.6.2 - Sales Manager rejects a Quotation', () => {
  ...
  test('TC.-A.6.2: Verify the Sales Manager can reject the Quotation successfully', async ({ page, browser }, testInfo) => {
    ...
    // Steps run 3-5: Max (Sales Manager) opens the URL in a second browser and REJECTS
    await test.step('Step run 3-5: Login as Max in a second browser, open the URL, and press REJECT', async () => {
      ...
      // clear it robustly (retry) before touching the form, otherwise it interferes with REJECT.
      ...
      console.log('Step run 5: Pressing REJECT as the Sales Manager');
      const rejectMs = await managerQuotationPage.clickReject();
      console.log(`✓ REJECT pressed (took ${(rejectMs / 1000).toFixed(1)}s)`);

      // Clear any "Odoo Client Error" popup that may surface around REJECT (same handling as Step run 4).
      await managerQuotationPage.dismissErrorDialogWithRetry();
      // Reload to reflect the committed post-rejection state, then clear the popup again if it reappears.
      await managerPage.reload({ waitUntil: 'domcontentloaded' });
      await managerQuotationPage.dismissErrorDialogWithRetry();
      await managerQuotationPage.waitForFormView(CommonUtils.waitTimes.pageLoad);

      // Verification: the rejection process is successful
      console.log('Verification: Confirming the rejection process is successful');
      const result = await managerQuotationPage.verifyRejectionSuccess();
      await CommonUtils.captureAndAttachScreenshot(managerPage, testInfo, 'TC.-A.6.2 - Quotation after Sales Manager rejection');
      expect(
        result.rejected,
        'The rejection should be successful: the pending-approval buttons ("APPROVE" / "REJECT") gone after the rejection'
      ).toBeTruthy();
      console.log('✅ Rejection process successful - the Sales Manager rejected the Quotation');
    });
  });
});
