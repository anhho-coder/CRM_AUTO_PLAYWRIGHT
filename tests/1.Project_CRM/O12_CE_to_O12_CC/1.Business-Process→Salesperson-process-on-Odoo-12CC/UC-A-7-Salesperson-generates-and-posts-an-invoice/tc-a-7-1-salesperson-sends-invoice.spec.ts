...
/**
 * Invoice Send Test - Salesperson sends an Invoice (single browser)
 * Test Case ID: TC.-A.7.1
 * Automation-Type: new
 * Automation-Date: 2026-06-05
 *
 * Summary: Verify the Salesperson can send the Invoice successfully.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.7\.1:" --project=chromium
 *
 ...
 * 17. Press "SEND & PRINT" button and wait (opens the "Send Invoice" window)
 *
 * Step run:
 * 1. Once the "Send Invoice" window appears, press the "SEND" button and wait
 *
 * Verification:
 * - Sending the Invoice is successful: the "Send Invoice" dialog closes and the Invoice form returns
 *   to readonly (the SEND operation completes), and the Invoice is in a posted/validated state
 *   ("Open" in this O12-based Odoo, i.e. validated and awaiting payment - not "Draft").
 */

test.describe('TC.-A.7.1 - Salesperson sends an Invoice', () => {
  ...
  test('TC.-A.7.1: Verify the Salesperson can send the Invoice successfully', async ({ page }, testInfo) => {
    ...
    // Pre-condition 17: Open the Send Invoice window
    await test.step('Step 17: Press SEND & PRINT', async () => {
      console.log('Step 17: Clicking SEND & PRINT');
      await invoicePage.dismissErrorDialog();
      await invoicePage.clickSendAndPrint();
      console.log('✓ SEND & PRINT pressed (Send Invoice window opened)');
    });

    // Step run 1: Press SEND in the Send Invoice window
    await test.step('Step run 1: Press SEND in the Send Invoice window', async () => {
      console.log('Step run 1: Clicking SEND in the Send Invoice window');
      const sendMs = await invoicePage.clickSendAndWaitForCompletion();
      console.log(`✓ SEND completed - the Send Invoice window closed and the Invoice form returned to readonly (took ${(sendMs / 1000).toFixed(1)}s)`);
    });

    // Verification: sending the Invoice is successful
    await test.step('Verification: Sending the Invoice is successful', async () => {
      console.log('Verification: Confirming the Invoice was sent successfully');
      // Reaching here means clickSendAndWaitForCompletion completed (the Send dialog closed and the
      // form returned to readonly). Corroborate that the Invoice is a valid "Posted" invoice.
      let invoiceNumber = '';
      try {
        invoiceNumber = await invoicePage.getInvoiceNumber();
      } catch {
        console.log('  ⚠ Could not read the Invoice number');
      }
      let status = '';
      try {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.7.1 - Invoice after SEND');
      // This Odoo (O12-based) uses the older invoice states: Draft -> Open (validated/posted) -> Paid.
      // A validated-and-sent invoice reads "Open" (not "Posted"); accept any posted/validated state.
      expect(
        status,
        `After sending, the Invoice should be in a posted/validated state (e.g. "Open" or "Posted"), not "Draft" - was "${status}"`
      ).toMatch(/Open|Posted|Paid/i);
      console.log('✅ Sending the Invoice is successful - the Salesperson sent the Invoice');
    });
  });
});
