...
/**
 * Quotation Confirmation Test - Confirm an approved Quotation to a Sales Order (two browsers)
 * Test Case ID: TC.-A.6.3
 *
 * Summary: Verify an approved Quotation can be confirmed to a Sales Order successfully.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.6\.3:" --project=chromium
 *
 ...
 * 6. (Max) Press the "CONFIRM" button to confirm the Quotation to a Sales Order
 *
 * Verification:
 * - The approved Quotation is confirmed to a Sales Order: status becomes "Sales Order",
 *   the CONFIRM button disappears and the LOCK button appears.
 */

test.describe('TC.-A.6.3 - Confirm an approved Quotation to a Sales Order', () => {
  ...
  test('TC.-A.6.3: Verify an approved Quotation can be confirmed to a Sales Order successfully', async ({ page, browser }, testInfo) => {
    ...
    // Steps run 3-6: Max (Sales Manager) opens the URL, APPROVES, then CONFIRMS to a Sales Order
    await test.step('Step run 3-6: Login as Max, open the URL, press APPROVE, then CONFIRM to a Sales Order', async () => {
      ...
      // Clear any "Odoo Client Error" popup around APPROVE, then reload to reflect the committed state.
      await managerQuotationPage.dismissErrorDialogWithRetry();
      await managerPage.reload({ waitUntil: 'domcontentloaded' });
      await managerQuotationPage.dismissErrorDialogWithRetry();
      await managerQuotationPage.waitForFormView(CommonUtils.waitTimes.pageLoad);

      const approval = await managerQuotationPage.verifyApprovalSuccess();
      expect(
        approval.approved,
        'The approval should be successful before confirming: APPROVE gone and a post-approval action available'
      ).toBeTruthy();
      console.log('✓ Approval successful - proceeding to CONFIRM');

      console.log('Step run 6: Pressing CONFIRM to confirm the Quotation to a Sales Order');
      await managerQuotationPage.clickConfirm();
      console.log('✓ CONFIRM pressed');
      // Clear any "Odoo Client Error" popup around CONFIRM, then reload to reflect the Sales Order state.
      await managerQuotationPage.dismissErrorDialogWithRetry();
      await managerPage.reload({ waitUntil: 'domcontentloaded' });
      await managerQuotationPage.dismissErrorDialogWithRetry();
      await managerQuotationPage.waitForFormView(CommonUtils.waitTimes.pageLoad);

      // Verification: the Quotation is confirmed to a Sales Order
      console.log('Verification: Confirming the Quotation became a Sales Order');
      const result = await managerQuotationPage.verifyConfirmedToSalesOrder();
      await CommonUtils.captureAndAttachScreenshot(managerPage, testInfo, 'TC.-A.6.3 - Quotation confirmed to Sales Order');
      expect(
        result.confirmed,
        'The Quotation should be confirmed to a Sales Order (status "Sales Order", or CONFIRM gone and LOCK visible)'
      ).toBeTruthy();
      console.log('✅ Quotation confirmed to a Sales Order successfully');
    });
  });
});
