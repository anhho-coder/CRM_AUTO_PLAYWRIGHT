...
/**
 * Lead-to-Opportunity Conversion Test - "Assign this opportunity to" (Sales Team set, Salesperson cleared)
 * Test Case ID: TC.-A.4.3.8
 * Automation-Type: new
 * Automation-Date: 2026-06-05
 *
 * Summary: Verify converting a qualified lead to Opportunity with the Sales Team from salesteam.users
 *          (sale_ic_bdeu_thomas -> BDEU) and the Salesperson explicitly CLEARED (left blank) is successful.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.4\.3\.8:" --project=chromium
 *
 * Steps:
 ...
 * 8. Assign this opportunity to: Sales Team from salesteam.users (sale_ic_bdeu_thomas), then CLEAR the Salesperson.
 * 9. Customers = "Link to an existing customer".
 * 10. Press "CREATE OPPORTUNITY".
 *
 * Verification (Step 11):
 * - Stage "New" appears, Sales Team = the salesteam.users value (BDEU), AND the Salesperson is empty
...
test.describe('TC.-A.4.3.8 - Convert Lead to Opportunity with Sales Team from salesteam.users and Salesperson cleared', () => {
  ...
  test(`TC.-A.4.3.8: Verify converting a qualified lead to Opportunity with Sales Team = ${sp.team} and the Salesperson cleared is successful`, async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Enter lead information', async () => {
      const leadName = 'TEST Lead 1 TC.-A.4.3.8';
    ...
    await test.step('Step 8: Set Sales Team (from salesteam.users) and clear the Salesperson', async () => {
      console.log(`Step 8: Setting Sales Team = ${sp.team}, then clearing the Salesperson`);
      await leadPage.selectConvertSalesTeam(sp.team);
      await leadPage.clearConvertSalesperson();
      console.log('✓ Sales Team set and Salesperson cleared');
    });
    ...
    await test.step('Step 11: Verify Stage "New", Sales Team, and that Salesperson is empty', async () => {
      console.log('Step 11: Verifying Stage "New", Sales Team, and that Salesperson is empty');
      ...
      recordOppAssignmentForDeferredVerify(page, { salesTeam });
      console.log(`  - Stage New visible: ${stageNewVisible}`);
      console.log(`  - Sales Team: "${salesTeam}" (expected to contain "${sp.team}")`);
      console.log(`  - Salesperson (expected empty): "${salesperson}"`);

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `TC.-A.4.3.8 - Converted Opportunity (${sp.team} / Salesperson cleared)`);
      ...
      expect(salesperson.trim(), 'Salesperson should be empty (cleared) on the converted Opportunity').toBe('');
      console.log('✅ Conversion successful with Sales Team set and Salesperson cleared');
    });
  });
});
