 ...
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.4\.3\.1:" --project=chromium
 ...
 * 8. Assign this opportunity to: Sales Team + Salesperson from salesteam.users (sale_ic_bdeu_thomas).
...
test.describe('TC.-A.4.3.1 - Convert Lead to Opportunity assigning the BDEU team Salesperson/Sales Team (salesteam.users)', () => {
  ...
  test.skip(`TC.-A.4.3.1: Verify converting a qualified lead to Opportunity assigning Sales Team = ${sp.team} and Salesperson = ${sp.displayName} is successful [CRM-11410]`, async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Enter lead information', async () => {
      const leadName = 'TEST Lead 1 TC.-A.4.3.1';
    ...
    await test.step('Step 8: Assign the BDEU team Sales Team and Salesperson (from salesteam.users)', async () => {
    ...
    await test.step('Step 11: Verify Stage "New" and the assigned Sales Team / Salesperson', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `TC.-A.4.3.1 - Converted Opportunity (${sp.team} / ${sp.displayName})`);
