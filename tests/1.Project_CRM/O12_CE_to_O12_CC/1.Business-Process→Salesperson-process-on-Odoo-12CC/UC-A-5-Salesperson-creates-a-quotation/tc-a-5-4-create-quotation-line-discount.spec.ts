...
/**
 * Quotation Creation Test - Salesperson creates a Quotation with a line-item discount
 * Test Case ID: TC.-A.5.4
 *
 * Summary: Verify the Salesperson creates a Quotation with a discount applied on the order line successfully.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.5\.4:" --project=chromium
 *
 * Pre-condition / Steps:
 ...
 * 10. At "Order Lines" section: add product [A2144B] (Qty 1) and set a 10% discount on the line
 * 11. Press "SAVE" button on the Deal Element and wait
 * 12. Verify the order line reflects the 10% discount
 *
 * Step run:
 * 1. Press "NEW QUOTATION" button on the Deal Element
 *
 * Verification:
 * - The Quotation (Sale Order) is created successfully with the discounted line, logged in the
 *   Deal Element chatter ("Sale Order created" plus the new record's "Status: Quotation").
 */

test.describe('TC.-A.5.4 - Salesperson creates a Quotation with a line-item discount', () => {
  ...
  test('TC.-A.5.4: Verify the Salesperson creates a Quotation with a line-item discount successfully', async ({ page }, testInfo) => {
    ...
    const TC_ID = 'TC.-A.5.4';
    const PRODUCT = '[A2144B]';
    let contactName1 = '';
    ...
    // Pre-condition 10: Add a product and apply a 10% line discount
    await test.step('Step 10: Add a product and apply a 10% line discount', async () => {
      console.log('Step 10: Adding a product and applying a 10% discount in Order Lines');
      await dealElementPage.addProductLine(PRODUCT, 1);
      console.log(`  - Product added: ${PRODUCT} (Qty 1)`);
      // Set the discount on the active (just-added) row directly - changeDiscountInRow needs the
      // readonly product span which doesn't exist while the row is being edited.
      await dealElementPage.setLastRowDiscount(10);
      console.log(`  - Discount: 10% applied on ${PRODUCT}`);
    });
    ...
    // Pre-condition 12: Verify the 10% discount on the order line
    await test.step('Step 12: Verify the 10% discount on the order line', async () => {
      console.log('Step 12: Verifying the discount on the order line');
      const discountOk = await dealElementPage.isDiscountInOrderLine(PRODUCT, 10);
      const subtotal = await dealElementPage.getSubtotalAfterAllDiscountsForProduct(PRODUCT);
      console.log(`  - Discount 10% present on ${PRODUCT}: ${discountOk}`);
      console.log(`  - Subtotal after discount: ${subtotal}`);
      expect(discountOk, `The order line for ${PRODUCT} should reflect the 10% discount`).toBeTruthy();
      console.log('✓ 10% discount applied on the order line');
    });
    ...
    await test.step('Verification: The Quotation is created successfully', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.5.4 - Quotation created with line discount (Deal Element chatter)');
      ...
      console.log('✅ Quotation created successfully with a line-item discount');
    });
  });
});
