...
/**
 * Quotation Creation Test - Salesperson creates a Quotation with the EUR pricelist
 * Test Case ID: TC.-A.5.3
 *
 * Summary: Verify the Salesperson creates a Quotation with a different pricelist/currency
 *          (Public Pricelist_EUR) successfully.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.5\.3:" --project=chromium
 *
 * Pre-condition / Steps:
 ...
 *    - Pricelist    = Public Pricelist_EUR (EUR)   <-- different currency vs TC.-A.5.1
 *    - Payment Term = Immediate Payment
 * 10. At "Order Lines" section: press "Add a product", then select the first product in the dropdown (Qty 1)
 * 11. Press "SAVE" button on the Deal Element and wait
 * 12. Verify the Deal Element pricelist persisted as the EUR pricelist
 *
 * Step run:
 * 1. Press "NEW QUOTATION" button on the Deal Element
 *
 * Verification:
 * - The Quotation (Sale Order) is created successfully on the EUR pricelist, logged in the
 *   Deal Element chatter ("Sale Order created" plus the new record's "Status: Quotation").
 */

test.describe('TC.-A.5.3 - Salesperson creates a Quotation with the EUR pricelist', () => {
  ...
  test('TC.-A.5.3: Verify the Salesperson creates a Quotation with the EUR pricelist successfully', async ({ page }, testInfo) => {
    ...
    const TC_ID = 'TC.-A.5.3';
    let contactName1 = '';
    // The pricelist must be read while the Deal Element is in EDIT mode (after SAVE it renders as a
    // readonly link and getPricelistValue() - which reads inputValue() - returns ""). Capture it in Step 9.
    let eurPricelistValue = '';
    ...
    // Pre-condition 9: Pricelist (EUR) + Payment Term
    await test.step('Step 9: Set the EUR Pricelist and Payment Term on the Deal Element', async () => {
      console.log('Step 9: Filling Deal Element information');
      await dealElementPage.waitForAutoPopulate();
      const pricelistSelected = await dealElementPage.selectPricelist('Public Pricelist_EUR');
      console.log(pricelistSelected ? '  - Pricelist: Public Pricelist_EUR (EUR)' : '  - Pricelist: Public Pricelist_EUR not selected');
      // Capture the pricelist NOW (edit mode) - after SAVE it becomes a readonly link and reads "".
      eurPricelistValue = await dealElementPage.getPricelistValue();
      console.log(`  - Pricelist value (edit mode): "${eurPricelistValue}"`);
    ...
    // Pre-condition 12: Verify the EUR pricelist persisted on the Deal Element
    await test.step('Step 12: Verify the EUR pricelist was applied', async () => {
      console.log('Step 12: Verifying the EUR pricelist captured in edit mode');
      console.log(`  - Pricelist value: "${eurPricelistValue}"`);
      expect(eurPricelistValue, 'The Deal Element pricelist should be the EUR pricelist').toContain('EUR');
      console.log('✓ EUR pricelist applied on the Deal Element');
    });
    ...
    await test.step('Verification: The Quotation is created successfully', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.5.3 - Quotation created on EUR pricelist (Deal Element chatter)');
      ...
      console.log('✅ Quotation created successfully on the EUR pricelist');
    });
  });
});
