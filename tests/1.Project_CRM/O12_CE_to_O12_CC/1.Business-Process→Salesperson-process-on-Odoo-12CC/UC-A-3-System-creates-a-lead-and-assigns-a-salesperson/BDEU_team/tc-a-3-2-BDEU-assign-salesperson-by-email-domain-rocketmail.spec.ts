 ...
 * 
 * Command to run:
 * npx playwright test --grep "TC\.-A\.3\.BDEU\.2:" --project=chromium
 * 
 * Pre-condition:
 ...
 * 6. Press "SAVE" button
 * 7. Wait for at least 4 minutes until "Salesperson" dropdown list fulfilled
 * 
 * Verification:
 * - The value at "Salesperson" dropdown list is set (any salesperson)
 */

test.describe('TC.-A.3.BDEU.2 - Salesperson Assignment by Email Domain (@rocketmail.com)', () => {
  ...
  test('TC.-A.3.BDEU.2: Verify the lead is assigned to BDEU team if Email Domain = @rocketmail.com', async ({ page }, testInfo) => {
    ...
    // Step 7: Wait for Salesperson auto-assignment (4 minutes)
    let assignmentResult: Awaited<ReturnType<typeof leadPage.waitForSalesTeamAssignment>>;
    await test.step('Step 7: Wait for Salesperson auto-assignment (4 minutes)', async () => {
      console.log('Step 7: Waiting for Salesperson auto-assignment');
      console.log('  - Waiting up to 4 minutes for Salesperson to be assigned...');

      assignmentResult = await leadPage.waitForSalesTeamAssignment(
        CommonUtils.waitTimes.leadAssignmentWait, // 4 minutes
        config.timeouts.salesTeamAssignment.checkInterval
      );

      if (!assignmentResult.salespersonAssigned) {
        throw new Error(`Salesperson was not automatically assigned within 4 minutes. Last value: "${assignmentResult.salespersonValue}"`);
      }
