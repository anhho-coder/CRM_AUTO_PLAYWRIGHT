 ...
 * 
 * Command to run:
 * npx playwright test --grep "TC\.-A\.3\.BDR\.1:" --project=chromium
 * 
 * Pre-condition:
 ...
 * 6. Press "SAVE" button
 * 7. Wait for at least 4 minutes until "Sales Team" dropdown list fulfilled
 * 
 * Verification:
 * - The value at "Sales Team" dropdown list = "BDR"
 */

test.describe('TC.-A.3.BDR.1 - BDR Team Assignment for Partner Sign up Lead Form', () => {
  ...
  test('TC.-A.3.BDR.1: Verify the lead is assigned to BDR team if Lead form = Partner Sign up', async ({ page }, testInfo) => {
    ...
    await test.step('Step 7: Wait for Sales Team auto-assignment (4 minutes)', async () => {
      ...
      if (!salesTeamAssigned) {
        ...
        throw new Error(`Sales Team was not automatically assigned within 4 minutes. Last value: "${salesTeamValue}"`);
      }
