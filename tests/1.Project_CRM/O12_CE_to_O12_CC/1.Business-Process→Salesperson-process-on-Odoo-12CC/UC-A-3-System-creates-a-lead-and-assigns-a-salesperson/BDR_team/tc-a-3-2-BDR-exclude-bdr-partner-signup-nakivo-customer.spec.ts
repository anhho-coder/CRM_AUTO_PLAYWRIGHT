 ...
 * 
 * Command to run:
 * npx playwright test --grep "TC\.-A\.3\.BDR\.2:" --project=chromium
 * 
 * Pre-condition:
 ...
 * 6. Press "SAVE" button
 * 7. Wait for at least 4 minutes until "Sales Team" dropdown list fullfilled
 * 
 * Verification:
 * 7. - The value at "Sales Team" dropdown list is NOT BDR
 */

test.describe('TC.-A.3.BDR.2 - BDR Team Exclusion for Partner Signup with Nakivo Customer', () => {
  ...
  test('TC.-A.3.BDR.2: Verify the lead is NOT assigned to BDR team if Lead form = Partner Signup and Nakivo Customer is true', async ({ page }, testInfo) => {
    ...
    // Step 7: Wait for Sales Team auto-assignment (4 minutes)
    await test.step('Step 7: Wait for Sales Team auto-assignment (4 minutes)', async () => {
      console.log('Step 7: Waiting for Sales Team auto-assignment');
      console.log('  ⏳ Waiting up to 4 minutes for Sales Team to be assigned...');

      const startWaitTime = Date.now();
      const maxWaitTime = CommonUtils.waitTimes.leadAssignmentWait; // 4 minutes
    ...
    await test.step('Verification: Confirm Sales Team is NOT BDR', async () => {
      ...
      if (salesTeamValue && salesTeamValue !== '' && salesTeamValue !== 'Sales Team') {
        ...
        console.log(`  ✗ Verification FAILED: Sales Team is not assigned after 4 minutes`);
        console.log(`  ✗ Expected: Sales Team should be assigned to a team other than BDR`);
        throw new Error(`Sales Team was not assigned after 4 minutes. Current value: "${salesTeamValue}"`);
      }
    });
  });
});
