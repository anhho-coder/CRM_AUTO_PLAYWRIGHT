 ...
 * ===========================================================================
 *  Test Case ID    : TC.-A.8.9
 *  Automation-Type : new
 *  Automation-Date : 2026-06-24
 *
 *  Summary:
 *    As Thomas, create a deal-registration validated Invoice and capture its backend
 *    Amount Due; then as the Reseller verify the "My invoices" row Amount Due equals
 *    the backend invoice amount.
 *
 *  Command to run:
 *    npx playwright test --grep "TC\.-A\.8\.9:" --project=chromium
 *
 * ---------------------------------------------------------------------------
 ...
 *          AND VIEW INVOICES; remember Invoice Number #1; VALIDATE (capture the backend Amount Due).
 *          Opportunity:
 ...
 *
 *  Verification #1:
 *    1. The "My invoices" row Amount Due for Invoice Number #1 equals the backend invoice Amount Due.
 * ===========================================================================
 */
...
test.describe('TC.-A.8.9 - Reseller sees the invoice Amount Due matching the invoice total', () => {
  ...
  test('TC.-A.8.9: Verify the Reseller invoice Amount Due matches the invoice total', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-A.8.9 ${compactDateTime}`;
    ...
    await test.step('Verification #1: The portal row Amount Due equals the backend invoice Amount Due', async () => {
      const row = await resellerPortalPage.getInvoiceRowData(invoice.invoiceNumber);
      const portalAmount = money(row?.amountDue);
      const backendAmount = money(invoice.amountDue);
      console.log(`  - Portal Amount Due: "${row?.amountDue}" (${portalAmount}) | Backend Amount Due: "${invoice.amountDue}" (${backendAmount})`);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.8.9 - Reseller My invoices (Amount Due matches total)');

      expect(row, 'The invoice row should be found on My invoices').not.toBeNull();
      expect(portalAmount, 'The portal Amount Due should be a positive number').toBeGreaterThan(0);
      expect(backendAmount, 'The backend Amount Due should have been captured').toBeGreaterThan(0);
      expect(portalAmount, 'The portal Amount Due should equal the backend invoice Amount Due').toBeCloseTo(backendAmount, 2);
      console.log('✅ The portal Amount Due matches the invoice total');
    });
  });
});
