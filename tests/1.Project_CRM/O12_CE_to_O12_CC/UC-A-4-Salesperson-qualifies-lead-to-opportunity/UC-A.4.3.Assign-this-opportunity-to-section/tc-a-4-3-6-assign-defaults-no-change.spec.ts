...
/**
 * Lead-to-Opportunity Conversion Test - "Assign this opportunity to" (defaults, no change)
 * Test Case ID: TC.-A.4.3.6
 * Automation-Type: new
 * Automation-Date: 2026-06-05
 *
 * Summary: Verify converting a qualified lead to Opportunity WITHOUT changing the Assign section
 *          (Salesperson + Sales Team left at the wizard defaults) is successful.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.4\.3\.6:" --project=chromium
 *
 * Steps:
 ...
 * 8. Assign this opportunity to: leave Salesperson + Sales Team at the wizard defaults (no change).
 * 9. Customers = "Link to an existing customer".
 * 10. Press "CREATE OPPORTUNITY".
 *
 * Verification (Step 11):
 * - Stage "New" appears on the converted Opportunity (the default assignment is environment-specific,
...
test.describe('TC.-A.4.3.6 - Convert Lead to Opportunity leaving the Assign section at defaults', () => {
  ...
  test('TC.-A.4.3.6: Verify converting a qualified lead to Opportunity leaving the Assign section at defaults is successful', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Enter lead information', async () => {
      const leadName = 'TEST Lead 1 TC.-A.4.3.6';
    ...
    await test.step('Step 8: Leave the Assign section at defaults (no change)', async () => {
      console.log('Step 8: Leaving Salesperson and Sales Team at the wizard defaults (no change)');
      console.log('✓ Assign section left untouched');
    });
    ...
    await test.step('Step 11: Verify Stage "New" on the converted Opportunity', async () => {
      console.log('Step 11: Verifying Stage "New" (default assignment is env-specific, logged only)');
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.4.3.6 - Converted Opportunity (Assign defaults)');

      expect(stageNewVisible, 'Stage "New" should appear after a successful conversion').toBeTruthy();
      console.log('✅ Conversion successful with the Assign section left at defaults - Stage "New" is visible');
    });
  });
});
