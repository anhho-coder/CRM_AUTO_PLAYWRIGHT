...
/**
 * Lead-to-Opportunity Conversion Test - Convert with default values
 * Test Case ID: TC.-A.4.1
 *
 * Summary: Verify the converting process of a qualified lead to Opportunity by
 *          selecting default values is successful.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.4\.1:" --project=chromium
 *
 * Pre-condition / Steps:
 ...
 * 10. Customer field     = leave as is
 * 11. Press "CREATE OPPORTUNITY" and wait
 *
 * Verification:
 * - Stage "New" appears on the Opportunity form, signalling the Lead was converted successfully.
 */

test.describe('TC.-A.4.1 - Convert qualified Lead to Opportunity with default values', () => {
  ...
  test('TC.-A.4.1: Verify the converting process of a qualified lead to Opportunity by selecting default values is successful', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Enter lead information', async () => {
      const leadName = 'TEST Lead 1 TC.-A.4.1';
    ...
    // Steps 9-10: Customer linking (leave the Customer field as is)
    await test.step('Step 9-10: Select "Link to an existing customer" (leave Customer as is)', async () => {
      console.log('Step 9: Selecting "Link to an existing customer"');
      await leadPage.selectLinkToExistingCustomer();
      console.log('Step 10: Leaving the Customer field as is');
      console.log('✓ Customer linking option selected');
    });

    // Step 11: Create the opportunity
    await test.step('Step 11: Click CREATE OPPORTUNITY', async () => {
      console.log('Step 11: Clicking CREATE OPPORTUNITY');
      await leadPage.clickCreateOpportunity();
      console.log('✓ Opportunity creation submitted');
    });

    // Step 12: Verify conversion success via Stage "New"
    await test.step('Step 12: Verify Stage "New" appears on the Opportunity', async () => {
      console.log('Step 12: Verifying Stage "New" is present on the Opportunity');
      const stageNewVisible = await opportunityPage.isStageNewVisible();

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.4.1 - Converted Opportunity (Stage New)');

      expect(stageNewVisible, 'Stage "New" should appear after a successful Lead-to-Opportunity conversion').toBeTruthy();
      console.log('✅ Lead converted to Opportunity successfully - Stage "New" is visible');
    });
  });
});
