...
/**
 * Lead-to-Opportunity Conversion Test - Convert and Do not link to a customer
 * Test Case ID: TC.-A.4.4.2
 *
 * Summary: Verify the converting process of a qualified lead to Opportunity by
 *          selecting "Do not link to a customer" is successful.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.4\.4\.2:" --project=chromium
 *
 * Pre-condition / Steps:
 ...
 * 9. Customers          = "Do not link to a customer"
...
test.describe('TC.-A.4.4.2 - Convert qualified Lead to Opportunity without linking a customer', () => {
  ...
  test('TC.-A.4.4.2: Verify the converting process of a qualified lead to Opportunity by selecting "Do not link to a customer" is successful', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Enter lead information', async () => {
      const leadName = 'TEST Lead 1 TC.-A.4.4.2';
    ...
    // Step 9: Customer section - do not link to a customer
    await test.step('Step 9: Select "Do not link to a customer"', async () => {
      console.log('Step 9: Selecting "Do not link to a customer"');
      await leadPage.selectDoNotLinkCustomer();
      console.log('✓ Customer option set to "Do not link to a customer"');
    });
    ...
    await test.step('Step 11: Verify Stage "New" appears on the Opportunity', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.4.4.2 - Converted Opportunity (Do not link customer, Stage New)');

      expect(stageNewVisible, 'Stage "New" should appear after a successful Lead-to-Opportunity conversion').toBeTruthy();
      console.log('✅ Lead converted to Opportunity successfully (no customer linked) - Stage "New" is visible');
    });
  });
});
