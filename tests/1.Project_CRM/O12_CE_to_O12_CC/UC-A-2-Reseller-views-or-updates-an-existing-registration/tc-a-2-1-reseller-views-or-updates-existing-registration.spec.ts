 ...
 *
 * Pre-condition #1: Build Internal Note #1 from the deal-registration template, then edit the <...>
 *   placeholders with dynamic values (random 4-digit suffix; Name = "TEST <current date time>";
 *   Email = "Test@company<date><time>.com"; Created Date = <current date time>; random 9-digit phone).
 *   generateDealRegistrationNote() returns the dynamic values + the assembled note.
 *
 * Steps to reproduce #1 (create the registration as Thomas):
 *  1-9. Login as Thomas; CRM > view list > CREATE; enter Opp/Contact/Company/Email, Country=United States,
 *       State=Maryland, IP, Create manually=FALSE, clear Sales Team/Salesperson; CRM Developer Lead form;
 *       Assigned Partner = TEST-Reseller#Automation-Jun10; Internal Note #1; SAVE; capture Opp URL #1.
 *
