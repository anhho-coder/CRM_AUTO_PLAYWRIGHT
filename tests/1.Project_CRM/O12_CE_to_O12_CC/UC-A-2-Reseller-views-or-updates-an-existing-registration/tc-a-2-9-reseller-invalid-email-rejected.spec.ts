 ...
 * Test Case ID: TC.-A.2.9
 * Automation-Type: new
 * Automation-Date: 2026-06-23
 *
 * Summary: Verify an INVALID Email entered by the Reseller is NOT persisted. Thomas creates the
 *          Opportunity (Opp Name #1, Assigned Partner = Reseller); the Reseller enters an invalid Email
 *          (no "@") and Confirms; Thomas re-opens Opp URL #1 and verifies the Email was not changed to it.
 *
 * Command to run:
 * npx playwright test --grep "TC\.-A\.2\.9:" --project=chromium
 *
 ...
 *
 * Steps to reproduce #2 (enter an invalid Email, as the Reseller):
 ...
 *  4. Enter the following information: Email = <invalid email, no "@">
 *  5. Press "CONFIRM"
 *
 ...
 *
 * Verification #1:
 *  1. The Opp's Email was NOT changed to the invalid value (invalid Email rejected / not saved).
 */

const SKIP_CLEANUP_OPP = false; // false = delete the created Opportunity on teardown

test.describe('TC.-A.2.9 - Reseller invalid Email is rejected (not saved)', () => {
  ...
  test('TC.-A.2.9: Verify Reseller invalid Email is rejected (not saved) on an existing registration', async ({ page }, testInfo) => {
    ...
    const oppName = `TEST TC.-A.2.9 ${compactDateTime}`;
    const invalidEmail = `not-an-email-${compactDateTime}`; // no "@" -> invalid format

    await test.step('Pre-condition 1: Prepare Internal Note #1 (deal-registration template)', async () => {
      console.log(`Pre-condition 1: Opp Name #1 = ${oppName} | created Email = ${companyEmail} | invalid Email = ${invalidEmail}`);
    });
    ...
    await test.step('Steps to reproduce #2 - Step 4 (data): Enter an invalid Email (no "@")', async () => {
      console.log(`Steps to reproduce #2 - Step 4 (data): Entering invalid Email = ${invalidEmail}`);
      await resellerPortalPage.fillContactEmail(invalidEmail);
      console.log('✓ Invalid Email entered in the Edit contact modal');
    });
    ...
    await test.step('Verification #1: The Opp Email was NOT changed to the invalid value', async () => {
      const oppEmail = await opportunityPage.getEmailReadonly();
      console.log(`  - Opportunity Email read from the form: "${oppEmail}" | invalid value (must NOT appear): "${invalidEmail}" | original: "${companyEmail}"`);

      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'TC.-A.2.9 - Opportunity Email after invalid entry');

      // Expected behaviour: the portal rejects an invalid email; the Opportunity Email is unchanged.
      expect(oppEmail, 'An invalid Email must NOT be saved to the Opportunity').not.toBe(invalidEmail);
      console.log('✅ Invalid Email was not persisted to the Opportunity');
    });
  });
});
