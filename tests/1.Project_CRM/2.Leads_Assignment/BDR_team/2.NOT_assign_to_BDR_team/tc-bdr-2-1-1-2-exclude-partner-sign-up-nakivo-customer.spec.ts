...
/**
 * Lead Assignment Test - BDR Team - Partner Sign up with Nakivo Customer Exclusion
 * Test Case ID: TC.BDR_2.1.1.2
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is NOT assigned to Jayden belong to BDR team if Lead form = Partner Sign up and Nakivo Customer is true
 * 
 * Command to run:
 * npx playwright test tests/1.Project_CRM/2.Leads_Assignment/BDR_team/2.NOT_assign_to_BDR_team/tc-bdr-2-1-1-2-exclude-partner-sign-up-nakivo-customer.spec.ts --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.BDR.2.1.1.2 - BDR Team Exclusion for Partner Sign up with Nakivo Customer', () => {
  ...
  test('TC.BDR.2.1.1.2: Verify the lead is NOT assigned to BDR team if Lead form = Partner Sign up and Nakivo Customer is true', async ({ page }, testInfo) => {
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      ...
      test.skip(true, assignmentDeferSkipReason('TC.BDR_2.1.1.2', fieldName));
    }
