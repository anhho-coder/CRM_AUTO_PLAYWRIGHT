...
/**
 * Lead Assignment Test - THD Team - India (State Blank) with Nakivo Customer = TRUE and New Partner = TRUE
 * Test Case ID: TC.THD_3.2.2.5.2
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * 
 * Summary: Verify the lead is assigned to THD team if Nakivo customer = TRUE, New Partner = TRUE, Country = India and State is BLANK
 * 
 * Command to run:
 * npx playwright test --grep "TC\.THD_3\.2\.2\.5\.2 -" --project=chromium
 * 
 * Pre-condition:
 ...
 *      - "State" dropdown list = BLANK (not selected)
...
test.describe('TC.THD_3.2.2.5.2 - THD Team Assignment for India (State Blank) with Nakivo Customer = TRUE and New Partner = TRUE', () => {
  ...
  test('TC.THD_3.2.2.5.2: Verify the lead is assigned to THD team if Nakivo customer = TRUE, New Partner = TRUE, Country = India and State is BLANK', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.THD_3.2.2.5.2');
      ...
      // State is BLANK - do not select any state
      console.log(`  - State: BLANK (not selected)`);
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.THD_3.2.2.5.2', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is THD and Salesperson is assigned', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - THD Team Assignment (India-State Blank with Nakivo Customer = TRUE, New Partner = TRUE)`);
      ...
      console.log('   Lead with India-State Blank, Nakivo Customer = TRUE, New Partner = TRUE');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.THD_3.2.2.5.2</div>
      ...
      <div class="info-row"><span class="label">State:</span> BLANK (not selected)</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with India-State Blank, Nakivo Customer = TRUE, New Partner = TRUE<br>
