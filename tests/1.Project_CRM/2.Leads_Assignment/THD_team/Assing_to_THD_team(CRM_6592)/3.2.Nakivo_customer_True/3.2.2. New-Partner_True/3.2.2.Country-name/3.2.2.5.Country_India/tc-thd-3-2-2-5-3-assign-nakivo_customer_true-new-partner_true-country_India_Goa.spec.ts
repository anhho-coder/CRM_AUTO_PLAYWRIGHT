...
/**
 * Lead Assignment Test - THD Team - India (Goa) with Nakivo Customer = TRUE and New Partner = TRUE
 * Test Case ID: TC.THD_3.2.2.5.3
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * 
 * Summary: Verify the lead is assigned to THD team if Nakivo customer = TRUE, New Partner = TRUE, Country = India and State is Goa (NOT in list: Andhra Pradesh, Karnataka, Kerala, Tamil Nadu, Telangana, West Bengal)
 * 
 * Command to run:
 * npx playwright test --grep "TC\.THD_3\.2\.2\.5\.3 -" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.THD_3.2.2.5.3 - THD Team Assignment for India (Goa) with Nakivo Customer = TRUE and New Partner = TRUE', () => {
  ...
  test('TC.THD_3.2.2.5.3: Verify the lead is assigned to THD team if Nakivo customer = TRUE, New Partner = TRUE, Country = India and State is Goa', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.THD_3.2.2.5.3');
    ...
    // Step 7: Set Nakivo Customer and New Partner checkboxes to TRUE
    await test.step('Step 7: Set Nakivo Customer and New Partner checkboxes to TRUE', async () => {
      console.log('Step 7: Setting Nakivo Customer and New Partner checkboxes to TRUE');
      ...
      // Step 7.2: Navigate to CRM Developer tab and check Nakivo Customer and New Partner
      console.log('  7.2: Checking Nakivo Customer and New Partner in CRM Developer tab');
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.THD_3.2.2.5.3', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is THD and Salesperson is assigned', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - THD Team Assignment (India-Goa with Nakivo Customer = TRUE, New Partner = TRUE)`);
      ...
      console.log('   Lead with India-Goa, Nakivo Customer = TRUE, New Partner = TRUE');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.THD_3.2.2.5.3</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with India-Goa, Nakivo Customer = TRUE, New Partner = TRUE<br>
