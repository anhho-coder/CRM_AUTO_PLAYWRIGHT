...
/**
 * Lead Assignment Test - THD Team - Vietnam with Nakivo Customer = TRUE and New Partner = TRUE
 * Test Case ID: TC.THD_3.2.2.4
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * 
 * Summary: Verify the lead is assigned to THD team if Nakivo customer = TRUE, New Partner = TRUE and Country = Vietnam
 * 
 * Command to run:
 * npx playwright test --grep "TC\.THD_3\.2\.2\.4 -" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.THD_3.2.2.4 - THD Team Assignment for Vietnam with Nakivo Customer = TRUE and New Partner = TRUE', () => {
  ...
  test('TC.THD_3.2.2.4: Verify the lead is assigned to THD team if Nakivo customer = TRUE, New Partner = TRUE and Country = Vietnam', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.THD_3.2.2.4');
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.THD_3.2.2.4', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is THD and Salesperson is assigned', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - THD Team Assignment (Vietnam with Nakivo Customer = TRUE, New Partner = TRUE)`);
      ...
      console.log('   Lead with Vietnam, Nakivo Customer = TRUE, New Partner = TRUE');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.THD_3.2.2.4</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with Vietnam, Nakivo Customer = TRUE, New Partner = TRUE<br>
