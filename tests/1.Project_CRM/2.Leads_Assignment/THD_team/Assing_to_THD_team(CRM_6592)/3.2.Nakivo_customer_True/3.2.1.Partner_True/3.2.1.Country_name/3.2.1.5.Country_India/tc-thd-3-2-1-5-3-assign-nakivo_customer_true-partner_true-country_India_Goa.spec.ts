...
/**
 * Lead Assignment Test - THD Team - India (Goa) with Nakivo Customer = TRUE and Partner = TRUE
 * Test Case ID: TC.THD_3.2.1.5.3
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is assigned to THD team if Nakivo customer = TRUE, Partner = TRUE, Country = India and State is Goa (NOT in list: Andhra Pradesh, Karnataka, Kerala, Tamil Nadu, Telangana, West Bengal)
 * 
 * Command to run:
 * npx playwright test --grep "TC\.THD_3\.2\.1\.5\.3 -" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.THD_3.2.1.5.3 - THD Team Assignment for India (Goa) with Nakivo Customer = TRUE and Partner = TRUE', () => {
  ...
  test('TC.THD_3.2.1.5.3: Verify the lead is assigned to THD team if Nakivo customer = TRUE, Partner = TRUE, Country = India and State = Goa (NOT in specific list)', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.THD_3.2.1.5.3');
      ...
      await leadPage.selectCountry('India');
      console.log(`  - Country: India`);
      
      // Select state - Goa (NOT in specific list)
      await leadPage.selectState('Goa');
      console.log(`  - State: Goa (NOT in list: Andhra Pradesh, Karnataka, Kerala, Tamil Nadu, Telangana, West Bengal)`);
    ...
    // Step 7: Set Nakivo Customer and Partner checkboxes to TRUE
    await test.step('Step 7: Set Nakivo Customer and Partner checkboxes to TRUE', async () => {
      console.log('Step 7: Setting Nakivo Customer and Partner checkboxes to TRUE');
      ...
      // Step 7.2: Navigate to CRM Developer tab and check Nakivo Customer and Partner
      console.log('  7.2: Checking Nakivo Customer and Partner in CRM Developer tab');
      await leadPage.clickCRMDeveloperTab();
      await page.waitForTimeout(CommonUtils.waitTimes.short);
      
      // Check Nakivo Customer checkbox
      const navivoCustomerChecked = await leadPage.checkNakivoCustomer();
      console.log(`  ✓ Nakivo Customer: ${navivoCustomerChecked ? 'Checked (TRUE)' : 'Already checked'}`);
      
      // Check Partner checkbox
      const partnerChecked = await leadPage.checkPartner();
      console.log(`  ✓ Partner: ${partnerChecked ? 'Checked (TRUE)' : 'Already checked'}`);
      ...
      console.log('✓ Nakivo Customer and Partner set to TRUE and saved');
    });
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.THD_3.2.1.5.3', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is THD and Salesperson is assigned', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - THD Team Assignment (India-Goa with Nakivo Customer = TRUE, Partner = TRUE)`);
      ...
      console.log('   Lead with India-Goa (NOT in specific list), Nakivo Customer = TRUE, Partner = TRUE');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.THD_3.2.1.5.3</div>
      ...
      <div class="info-row"><span class="label">Country:</span> India</div>
      <div class="info-row"><span class="label">State:</span> Goa (NOT in list: Andhra Pradesh, Karnataka, Kerala, Tamil Nadu, Telangana, West Bengal)</div>
      <div class="info-row"><span class="label">Nakivo Customer:</span> TRUE</div>
      <div class="info-row"><span class="label">Partner:</span> TRUE</div>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with India-Goa (NOT in specific list), Nakivo Customer = TRUE, Partner = TRUE<br>
