...
/**
 * Lead Assignment Test - THD Team - India (Karnataka) with Nakivo Customer = TRUE and Partner = TRUE
 * Test Case ID: TC.THD_3.2.1.5.4
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is assigned to THD team if Nakivo customer = TRUE, Partner = TRUE, Country = India and State is Karnataka (IN list: Andhra Pradesh, Karnataka, Kerala, Tamil Nadu, Telangana, West Bengal)
 * 
 * Command to run:
 * npx playwright test --grep "TC\.THD_3\.2\.1\.5\.4 -" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.THD_3.2.1.5.4 - THD Team Assignment for India (Karnataka) with Nakivo Customer = TRUE and Partner = TRUE', () => {
  ...
  test('TC.THD_3.2.1.5.4: Verify the lead is assigned to THD team if Nakivo customer = TRUE, Partner = TRUE, Country = India and State = Karnataka (IN specific list)', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.THD_3.2.1.5.4');
      ...
      // Select state - Karnataka (IN specific list)
      await leadPage.selectState('Karnataka');
      console.log(`  - State: Karnataka (IN list: Andhra Pradesh, Karnataka, Kerala, Tamil Nadu, Telangana, West Bengal)`);
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.THD_3.2.1.5.4', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is THD and Salesperson is assigned', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - THD Team Assignment (India-Karnataka with Nakivo Customer = TRUE, Partner = TRUE)`);
      ...
      console.log('   Lead with India-Karnataka (IN specific list), Nakivo Customer = TRUE, Partner = TRUE');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.THD_3.2.1.5.4</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with India-Karnataka (IN specific list), Nakivo Customer = TRUE, Partner = TRUE<br>
