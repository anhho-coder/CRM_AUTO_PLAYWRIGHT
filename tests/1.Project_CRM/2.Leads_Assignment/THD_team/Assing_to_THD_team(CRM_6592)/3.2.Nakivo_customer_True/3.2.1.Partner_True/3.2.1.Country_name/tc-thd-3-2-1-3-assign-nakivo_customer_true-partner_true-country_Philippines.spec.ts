...
/**
 * Lead Assignment Test - THD Team - Philippines with Nakivo Customer = TRUE and Partner = TRUE
 * Test Case ID: TC.THD_3.2.1.3
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is assigned to THD team if Nakivo customer = TRUE, Partner = TRUE and Country = Philippines
 * 
 * Command to run:
 * npx playwright test --grep "TC\.THD_3\.2\.1\.3 -" --project=chromium
 * 
 * Pre-condition:
 ...
 *      - "Country" dropdown list = Philippines
...
test.describe('TC.THD_3.2.1.3 - THD Team Assignment for Philippines with Nakivo Customer = TRUE and Partner = TRUE', () => {
  ...
  test('TC.THD_3.2.1.3: Verify the lead is assigned to THD team if Nakivo customer = TRUE, Partner = TRUE and Country = Philippines', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.THD_3.2.1.3');
      ...
      await leadPage.selectCountry('Philippines');
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.THD_3.2.1.3', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is THD and Salesperson is assigned', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - THD Team Assignment (Philippines with Nakivo Customer = TRUE, Partner = TRUE)`);
      ...
      console.log('   Lead with Philippines, Nakivo Customer = TRUE, Partner = TRUE');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.THD_3.2.1.3</div>
      ...
      <div class="info-row"><span class="label">Country:</span> Philippines</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with Philippines, Nakivo Customer = TRUE, Partner = TRUE<br>
