 ...
 * 
 * Command to run:
 * npx playwright test --grep "TC\.THD_3\.1\.1\.5\.1 -" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.THD_3.1.1.5.1 - THD Team Assignment for India with Delhi State and Nakivo Customer = NOT True', () => {
  ...
  test('TC.THD_3.1.1.5.1: Verify the lead is assigned to THD team if Nakivo customer = NOT True and Country = India and State = Delhi', async ({ page }, testInfo) => {
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.THD_3.1.1.5.1', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
