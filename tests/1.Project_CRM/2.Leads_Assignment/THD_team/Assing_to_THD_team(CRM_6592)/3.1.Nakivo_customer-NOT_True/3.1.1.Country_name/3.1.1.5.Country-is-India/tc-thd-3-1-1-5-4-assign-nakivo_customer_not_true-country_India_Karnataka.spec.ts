...
/**
 * Lead Assignment Test - THD Team - India with Karnataka State and Nakivo Customer = NOT True
 * Test Case ID: TC.THD_3.1.1.5.4
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is assigned to THD team if Nakivo customer = NOT True and Country = India and State is in (Andhra Pradesh, Karnataka, Kerala, Tamil Nadu, Telangana, West Bengal)
 * 
 * Command to run:
 * npx playwright test --grep "TC\.THD_3\.1\.1\.5\.4 -" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.THD_3.1.1.5.4 - THD Team Assignment for India with Karnataka State and Nakivo Customer = NOT True', () => {
  ...
  test('TC.THD_3.1.1.5.4: Verify the lead is assigned to THD team if Nakivo customer = NOT True and Country = India and State is in (Andhra Pradesh, Karnataka, Kerala, Tamil Nadu, Telangana, West Bengal)', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.THD_3.1.1.5.4');
      ...
      // Select country - India (selectCountry handles exact match logic internally)
    ...
    await test.step('Step 7: Set Nakivo Customer checkbox to FALSE', async () => {
      ...
      console.log(`  ✓ Nakivo Customer: ${navivoCustomerUnchecked ? 'Unchecked (FALSE)' : 'Already unchecked'}`);
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.THD_3.1.1.5.4', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is THD and Salesperson is assigned', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - THD Team Assignment (India-Karnataka with Nakivo Customer = NOT True)`);
      ...
      console.log('   Lead with India-Karnataka, Nakivo Customer = NOT True');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.THD_3.1.1.5.4</div>
      ...
      <div class="info-row"><span class="label">State:</span> Karnataka</div>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with India-Karnataka, Nakivo Customer = NOT True<br>
