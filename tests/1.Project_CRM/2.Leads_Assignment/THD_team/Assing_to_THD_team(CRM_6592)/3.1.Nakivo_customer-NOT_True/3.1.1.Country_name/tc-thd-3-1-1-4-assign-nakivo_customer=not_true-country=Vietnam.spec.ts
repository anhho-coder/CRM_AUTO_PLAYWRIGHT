 ...
 * 
 * Command to run:
 * npx playwright test --grep "TC\.THD_3\.1\.1\.4 -" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.THD_3.1.1.4 - THD Team Assignment for Vietnam with Nakivo Customer = NOT True', () => {
  ...
  test('TC.THD_3.1.1.4: Verify the lead is assigned to THD team if Nakivo customer = NOT True and Country = Vietnam', async ({ page }, testInfo) => {
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.THD_3.1.1.4', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
