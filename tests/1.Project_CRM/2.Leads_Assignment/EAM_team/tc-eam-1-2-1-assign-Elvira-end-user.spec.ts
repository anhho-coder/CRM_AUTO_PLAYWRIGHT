 ...
 * 
 * Command to run:
 * npx playwright test tests/1.Project_CRM/2.Leads_Assignment/EAM_team/tc-eam-1-2-1-assign-Elvira-end-user.spec.ts --project=chromium
 * 
 * Pre-condition:
 ...
 *    - "Email" textbox = Company email (with template Test-EndUser@EndUser-company + current date + current time + millisecond.com)
 *    - (in the Address section)
 *      - "Country" dropdown list = Ukraine
 *    - "Sales Team" dropdown list is "EAM"
 *    - "Salesperson" dropdown list is "Elvira Fridrikh"
 ...
 * 5. Press "SAVE" button
 * 6. Wait for at least 1.5 minutes until "Sales Team" dropdown list and "Salesperson" dropdown list fullfilled
 * 
 * Verification:
 * 6. - The value at "Sales Team" dropdown list = "EAM"
...
test.describe('TC.EAM_1.2.1 - EAM Team Assignment for End User to Bilal Saab (Elvira Fridrikh)', () => {
  ...
  test('TC.EAM_1.2.1: Verify the lead is assigned to Bilal belong to EAM team if End User. Salesperson = Elvira Fridrikh', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 4: Fill contact information', async () => {
      ...
      // Set Salesperson to Elvira Fridrikh
      await contactPage.selectSalesperson('Elvira Fridrikh');
      console.log(`  - Salesperson: Elvira Fridrikh`);
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      ...
      test.skip(true, assignmentDeferSkipReason('TC.EAM_1.2.1', missingField));
    }
