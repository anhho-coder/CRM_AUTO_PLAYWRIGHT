 ...
 * 
 * Command to run:
 * npx playwright test "tests/1.Project_CRM/2.Leads_Assignment/Marketing_BDEU/2.2.Lead_Form-contain/tc-mbdeu-2-2-5-assign-france-lead-form_contains-White-Paper.spec.ts" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.MBDEU_2.2.5 - Marketing BDEU Team Assignment for Afghanistan with Lead Form contains "White Paper"', () => {
  ...
  test('TC.MBDEU_2.2.5: Verify the lead is assigned to Marketing - BDEU team if Lead Form="TEST White Paper", Country=Afghanistan', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.MBDEU_2.2.5');
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.MBDEU_2.2.5', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.MBDEU_2.2.5</div>
