 ...
 * 
 * Command to run:
 * npx playwright test "tests/1.Project_CRM/2.Leads_Assignment/Marketing_BDEU/2.2.Lead_Form-contain/tc-mbdeu-2-2-6-assign-france-lead-form_contains-WHITE-PAPER.spec.ts" --project=chromium
 * 
 * Pre-condition:
 ...
 *    - "Lead form" textbox = TEST WHITE PAPER
 * 6. Press "SAVE" button
...
test.describe('TC.MBDEU_2.2.6 - Marketing BDEU Team Assignment for Afghanistan with Lead Form contains "WHITE PAPER"', () => {
  ...
  test('TC.MBDEU_2.2.6: Verify the lead is assigned to Marketing - BDEU team if Lead Form="TEST WHITE PAPER", Country=Afghanistan', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.MBDEU_2.2.6');
      // Fill lead name
    ...
    await test.step('Step 5: Fill Lead Form in CRM Developer tab', async () => {
      ...
      await leadPage.fillLeadForm('TEST WHITE PAPER');
      console.log(`  - Lead Form: TEST WHITE PAPER`);
    ...
    await test.step('Step 7: Wait for Sales Team and Salesperson auto-assignment', async () => {
      ...
      // Capture screenshot as evidence and attach to report (after assignment)
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - Marketing BDEU Team Assignment (Afghanistan with Lead Form contains "WHITE PAPER")`);
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.MBDEU_2.2.6', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is Marketing - BDEU and Salesperson is assigned', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - Marketing BDEU Team Assignment (Afghanistan with Lead Form contains "WHITE PAPER")`);
            ...
            console.log('   Lead with Afghanistan, Lead Form contains "WHITE PAPER"');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.MBDEU_2.2.6</div>
    ...
    <div class="checkpoint">
      ...
      <div class="checkpoint-detail"><span class="result-pass">Result: ✓ PASSED</span></div>
        ...
        Lead with Afghanistan, Lead Form contains "WHITE PAPER"<br>
    <div class="checkpoint">
