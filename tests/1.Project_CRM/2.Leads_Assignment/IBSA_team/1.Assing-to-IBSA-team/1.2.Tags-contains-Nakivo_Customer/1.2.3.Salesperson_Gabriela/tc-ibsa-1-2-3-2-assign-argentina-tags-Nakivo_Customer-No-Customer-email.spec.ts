...
/**
 * Lead Assignment Test - IBSA Team - Argentina with Tags "Nakivo Customer" and no email
 * Test Case ID: TC.IBSA_1.2.3.2
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is assigned to Gabriela Caballero belong to IBSA team if Tags contains "Nakivo Customer"
 * and Country = Argentina
 * 
 * Command to run:
 * npx playwright test "tests/Leads_Assignment/IBSA_team/1.Assing-to-IBSA-team/1.2.Tags-contains-Nakivo_Customer/1.2.3.Salesperson_Gabriela/tc-ibsa-1-2-3-2-assign-argentina-tags-Nakivo_Customer-No-Customer-email.spec.ts" --project=chromium
 * 
 * Pre-condition:
 ...
 *      - "Country" dropdown list = Argentina
...
test.describe('TC.IBSA_1.2.3.2 - IBSA Team Assignment for Argentina with Tags "Nakivo Customer" (No Email)', () => {
  ...
  test('TC.IBSA_1.2.3.2: Verify the lead is assigned to IBSA team if Tags="Nakivo Customer", Country=Argentina', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Click CREATE button and fill lead information', async () => {
      ...
      leadName = CommonUtils.generateLeadNameWithTestCase('TC.IBSA_1.2.3.2');
      
      // Fill lead name
      await leadPage.fillLeadOpportunity(leadName);
      console.log(`  - Lead Name: ${leadName}`);
      
      // NO EMAIL - Customer is not set for this scenario
      console.log(`  - Email: (Not set - Customer email is empty)`);
      
      // Select country - Argentina
      await leadPage.selectCountry('Argentina');
    ...
    // Step 6: Wait for Sales Team and Salesperson auto-assignment
    await test.step('Step 6: Wait for Sales Team and Salesperson auto-assignment', async () => {
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      ...
      test.skip(true, assignmentDeferSkipReason('TC.IBSA_1.2.3.2', fieldName));
    }
    ...
    await test.step('Verification: Confirm Sales Team is IBSA and Salesperson is assigned', async () => {
      ...
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, `Lead ${leadId} - IBSA Team Assignment (Argentina with Tags "Nakivo Customer")`);
      ...
      console.log('   Lead with Argentina, Tags="Nakivo Customer"');
      console.log('   correctly assigned to IBSA team');
      console.log('==================================================\n');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.IBSA_1.2.3.2</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with Argentina, Tags="Nakivo Customer"<br>
        correctly assigned to IBSA team
      </div>
    </div>
  </div>
</body>
