 ...
 * 
 * Command to run:
 * npx playwright test tests/Leads_Assignment/BDEU_team/2.NOT_assign_to_BDEU_team/tc-bdeu-2-1-1-3-exclude-aws-snapshot-vs-backup.spec.ts --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.BDEU.2.1.1.3 - BDEU Team Exclusion for AWS Snapshot vs. Backup Lead Form', () => {
  ...
  test('TC.BDEU.2.1.1.3: Verify the lead is NOT assigned to BDEU team if Lead form = AWS Snapshot vs. Backup', async ({ page }, testInfo) => {
    ...
    await test.step('Step 5: Fill CRM Developer tab', async () => {
      ...
      const leadFormFilled = await leadPage.fillLeadForm('AWS Snapshot vs. Backup');
      console.log(`  - Lead Form: ${leadFormFilled ? 'AWS Snapshot vs. Backup' : 'Field not found in CRM Developer tab, skipping'}`);
    ...
    await test.step('Verification: Confirm Sales Team is NOT BDEU', async () => {
      ...
      await testInfo.attach(`Lead ${leadId} - Sales Team Assignment (AWS Snapshot Exclusion)`, {
        body: screenshot,
      ...
      console.log('\n✅ TEST PASSED: Lead with AWS Snapshot vs. Backup form is correctly NOT assigned to BDEU team');
    });
  });
});
