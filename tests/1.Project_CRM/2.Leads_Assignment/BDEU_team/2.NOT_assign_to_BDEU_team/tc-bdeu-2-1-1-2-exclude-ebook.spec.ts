...
/**
 * Lead Assignment Test - BDEU Team - Lead Form Exclusion (eBook)
 * Test Case ID: TC.BDEU.2.1.1.2
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is NOT assigned to Sergey Y belong to BDEU team if Lead form = eBook
 * 
 * Command to run:
 * npx playwright test tests/Leads_Assignment/BDEU_team/2.NOT_assign_to_BDEU_team/tc-bdeu-2-1-1-2-exclude-ebook.spec.ts --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.BDEU.2.1.1.2 - BDEU Team Exclusion for eBook Lead Form', () => {
  ...
  test('TC.BDEU.2.1.1.2: Verify the lead is NOT assigned to BDEU team if Lead form = eBook', async ({ page }, testInfo) => {
    ...
    await test.step('Step 5: Fill CRM Developer tab', async () => {
      ...
      const leadFormFilled = await leadPage.fillLeadForm('Ebook');
      console.log(`  - Lead Form: ${leadFormFilled ? 'Ebook' : 'Field not found in CRM Developer tab, skipping'}`);
    ...
    if (!salesTeamAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.BDEU.2.1.1.2', 'Sales Team'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is NOT BDEU', async () => {
      ...
      await testInfo.attach(`Lead ${leadId} - Sales Team Assignment (eBook Exclusion)`, {
        body: screenshot,
      ...
      console.log('\n✅ TEST PASSED: Lead with eBook form is correctly NOT assigned to BDEU team');
    });
  });
});
