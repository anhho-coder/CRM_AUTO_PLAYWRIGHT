...
/**
 * Lead Assignment Test - BDEU Team - Lead Form Exclusion (Disaster Recovery Plan Templates)
 * Test Case ID: TC.BDEU.2.1.1.4
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is NOT assigned to Sergey Y belong to BDEU team if Lead form = Disaster recovery plan templates
 * 
 * Command to run:
 * npx playwright test tests/Leads_Assignment/BDEU_team/2.NOT_assign_to_BDEU_team/tc-bdeu-2-1-1-4-exclude-disaster-recovery-plan-templates.spec.ts --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.BDEU.2.1.1.4 - BDEU Team Exclusion for Disaster Recovery Plan Templates Lead Form', () => {
  ...
  test('TC.BDEU.2.1.1.4: Verify the lead is NOT assigned to BDEU team if Lead form = Disaster recovery plan templates', async ({ page }, testInfo) => {
    ...
    await test.step('Step 5: Fill CRM Developer tab', async () => {
      ...
      const leadFormFilled = await leadPage.fillLeadForm('Disaster recovery plan templates');
      console.log(`  - Lead Form: ${leadFormFilled ? 'Disaster recovery plan templates' : 'Field not found in CRM Developer tab, skipping'}`);
    ...
    if (!salesTeamAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.BDEU.2.1.1.4', 'Sales Team'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is NOT BDEU', async () => {
      ...
      await testInfo.attach(`Lead ${leadId} - Sales Team Assignment (Disaster Recovery Exclusion)`, {
        body: screenshot,
      ...
      console.log('\n✅ TEST PASSED: Lead with Disaster recovery plan templates form is correctly NOT assigned to BDEU team');
    });
  });
});
