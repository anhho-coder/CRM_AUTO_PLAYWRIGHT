...
/**
 * Lead Assignment Test - CMR Team - Ukraine with Partner Signup Lead Form
 * Test Case ID: TC.CMR_1.5.1.2
 ...
 * and Nakivo Customer is not set if Lead form = Partner Signup
 * 
 * Command to run:
 * npx playwright test tests/Leads_Assignment/CMR_team/5.Assign_to_CMR_team_directCustomer_countries_PartnerSignUp/tc-cmr-1-5-1-2-assign-ukraine-partner-signup.spec.ts --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.CMR_1.5.1.2 - CMR Team Assignment for Ukraine with Partner Signup Lead Form', () => {
  ...
  test('TC.CMR_1.5.1.2: Verify the lead is assigned to CMR team if Country=Ukraine, Nakivo Customer=FALSE, Lead Form=Partner Signup', async ({ page }, testInfo) => {
    ...
    await test.step('Step 4: Fill lead information', async () => {
      ...
      // Generate unique lead name and email with TEST prefix + Test case ID
      leadName = `TEST_TC.CMR_1.5.1.2_${CommonUtils.generateTimestamp()}`;
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.CMR_1.5.1.2', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is CMR and Salesperson is assigned', async () => {
      ...
      await testInfo.attach(`Lead ${leadId} - CMR Team Assignment (Ukraine with Partner Signup)`, {
        body: screenshot,
        contentType: 'image/png'
      });
      ...
      console.log('   Partner Signup correctly assigned to CMR team');
      console.log('==================================================\n');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.CMR_1.5.1.2</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Partner Signup correctly assigned to CMR team
      </div>
    </div>
  </div>
</body>
