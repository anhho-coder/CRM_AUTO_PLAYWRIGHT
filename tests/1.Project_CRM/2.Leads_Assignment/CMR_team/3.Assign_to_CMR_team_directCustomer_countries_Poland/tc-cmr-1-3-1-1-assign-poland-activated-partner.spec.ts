...
/**
 * Lead Assignment Test - CMR Team - Poland with Activated Partner (Direct Customer Countries)
 * Test Case ID: TC.CMR_1.3.1.1
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is assigned to Sergey Karachin belong to CMR team if Country is Poland
 * and Activated Partner is set and Nakivo Customer is not set if Country Group = CMR_directCustomer_countries
 * 
 * Command to run:
 * npx playwright test tests/Leads_Assignment/CMR_team/3.Assign_to_CMR_team_directCustomer_countries_Poland/tc-cmr-1-3-1-1-assign-poland-activated-partner.spec.ts --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.CMR_1.3.1.1 - CMR Team Assignment for Poland with Activated Partner (Direct Customer Countries)', () => {
  ...
  test('TC.CMR_1.3.1.1: Verify the lead is assigned to CMR team if Country=Poland, Activated Partner=TRUE, Nakivo Customer=FALSE, Lead Form=License', async ({ page }, testInfo) => {
    ...
    // Step 7.2: Set Nakivo Customer=FALSE and Activated Partner=TRUE
    await test.step('Step 7.2: Set Nakivo Customer and Activated Partner checkboxes', async () => {
      console.log('Step 7.2: Setting Nakivo Customer and Activated Partner checkboxes');
      ...
      console.log('✓ Nakivo Customer = FALSE, Activated Partner = TRUE');
    });
    ...
    await test.step('Step 7.3: Save the lead again (second save)', async () => {
      ...
      console.log('✓ Lead saved successfully with Nakivo Customer=FALSE and Activated Partner=TRUE');
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.CMR_1.3.1.1', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
    ...
    await test.step('Verification: Confirm Sales Team is CMR and Salesperson is assigned', async () => {
      ...
      await testInfo.attach(`Lead ${leadId} - CMR Team Assignment (Poland with Activated Partner)`, {
        body: screenshot,
        contentType: 'image/png'
      });
      ...
      console.log('   Lead with Poland, Activated Partner=TRUE, and');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.CMR_1.3.1.1</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with Poland, Activated Partner=TRUE, and<br>
        Partner Sign up Promote 2020* correctly assigned to CMR team
      </div>
    </div>
  </div>
</body>
