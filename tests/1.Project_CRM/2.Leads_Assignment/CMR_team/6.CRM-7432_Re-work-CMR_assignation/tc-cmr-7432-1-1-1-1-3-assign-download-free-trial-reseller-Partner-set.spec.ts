 ...
 * Test Case ID: TC.CMR-7432_1.1.1.1.3
...
test.describe('TC.CMR-7432_1.1.1.1.3 - CMR Team Assignment for Download Free Trial with Reseller (Basic)', () => {
  ...
  test('TC.CMR-7432_1.1.1.1.3: Verify the lead is assigned to CMR team if Lead form=Download Free Trial, Nakivo Customer=FALSE, Reseller contact selected', async ({ page }, testInfo) => {
    ...
    // Steps 3-8: Create contact with Basic partner level
    await test.step('Steps 3-8: Create contact with Basic partner level', async () => {
      ...
      contactName = `TEST-Contact_TC.CMR-7432_1.1.1.1.3_${timestamp}`;
      emailAddress = `Test-Reseller@Reseller-company${timestamp}.com`;
      
      const currentDate = new Date().toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' });
      const comment = `Basic level_TC.CMR-7432_1.1.1.1.3_${timestamp}`;
    ...
    await test.step('Step 14: Fill Lead information', async () => {
      ...
      const timestamp = CommonUtils.generateTimestamp();
      leadName = `TEST_TC.CMR-7432_1.1.1.1.3_${timestamp}`;
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.CMR-7432_1.1.1.1.3', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
