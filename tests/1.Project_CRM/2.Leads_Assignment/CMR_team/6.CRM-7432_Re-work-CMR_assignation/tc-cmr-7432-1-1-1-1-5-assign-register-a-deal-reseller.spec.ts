...
/**
 * Lead Assignment Test - CMR Team - Register a deal with Reseller Contact (Basic Partner Level)
 * Test Case ID: TC.CMR-7432_1.1.1.1.5
 *
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is assigned to CMR team if Lead form = Register a deal ; Nakivo Customer is not set; Tag = "Deal Registration"
 * 
 * Command to run:
 * npx playwright test --grep "TC\.CMR-7432_1\.1\.1\.1\.5 -" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.CMR-7432_1.1.1.1.5 - CMR Team Assignment for Register a deal with Reseller', () => {
  ...
  test('TC.CMR-7432_1.1.1.1.5: Verify the lead is assigned to CMR team if Lead form=Register a deal, Nakivo Customer=FALSE, Activated Partner=TRUE', async ({ page }, testInfo) => {
    ...
    await test.step('Steps 3-8: Create contact with Basic partner level and Activation Date', async () => {
      ...
      contactName = `TEST-Contact_TC.CMR-7432_1.1.1.1.5_${timestamp}`;
      emailAddress = `Test-Reseller@Reseller-company${timestamp}.com`;
      
      const currentDate = new Date().toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' });
      const comment = `Basic level_TC.CMR-7432_1.1.1.1.5_${timestamp}`;
    ...
    await test.step('Step 14: Fill Lead information', async () => {
      ...
      const timestamp = CommonUtils.generateTimestamp();
      leadName = `TEST_TC.CMR-7432_1.1.1.1.5_${timestamp}`;
    ...
    await test.step('Step 15: Set Lead form = Register a deal', async () => {
      ...
      // Set Lead form to "Register a deal"
      const leadFormFilled = await leadPage.fillLeadForm('Register a deal');
      console.log(`  - Lead Form: ${leadFormFilled ? 'Register a deal' : 'Field not found in CRM Developer tab, skipping'}`);
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.CMR-7432_1.1.1.1.5', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
