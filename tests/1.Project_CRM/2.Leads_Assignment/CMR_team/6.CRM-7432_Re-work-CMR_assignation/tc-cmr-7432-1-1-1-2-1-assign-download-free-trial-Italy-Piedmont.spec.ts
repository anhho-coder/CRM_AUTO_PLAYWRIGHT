 ...
 * 
 * Command to run:
 * npx playwright test --grep "TC\.CMR-7432_1\.1\.1\.2\.1:" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.CMR-7432_1.1.1.2.1 - CMR Team Assignment for Italy-Piedmont with Download Free Trial', () => {
  ...
  test('TC.CMR-7432_1.1.1.2.1: Verify the lead is assigned to CMR team if Country=Italy, State=Piedmont, Lead Form=Download Free Trial', async ({ page }, testInfo) => {
    ...
    await test.step('Pre-condition Step 4: Fill contact information', async () => {
      ...
      contactName = `TEST-ContactTC.CMR-7432_1.1.1.2.1${year}${month}${day}${hours}${minutes}${seconds}${milliseconds}`;
      contactEmail = `Test-Reseller@Reseller-company${year}${month}${day}${hours}${minutes}${seconds}${milliseconds}.com`;
    ...
    await test.step('Pre-condition Step 5: Save the contact', async () => {
      ...
      console.log(`  Contact Name (Reseller#1): ${contactName}`);
      
      // Capture screenshot of created contact
      const screenshotContact = await page.screenshot({ fullPage: true });
      await testInfo.attach(`Contact ${contactId} - Created Reseller`, {
    ...
    await test.step('Step 4: Fill lead information', async () => {
      ...
      // Select state - Piedmont
      await leadPage.selectState('Piedmont');
      console.log(`  - State: Piedmont`);
    ...
    await test.step('Verification: Confirm Sales Team is CMR and Salesperson is assigned', async () => {
      ...
      await testInfo.attach(`Lead ${leadId} - CMR Team Assignment (Italy-Piedmont with Download Free Trial)`, {
        body: screenshot,
        contentType: 'image/png'
      });
      ...
      console.log('   Lead with Italy-Piedmont, Download Free Trial');
      console.log('   correctly assigned to CMR team');
      console.log('==================================================\n');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.CMR-7432_1.1.1.2.1</div>
      ...
      <div class="info-row"><span class="label">Contact (Reseller):</span> ${contactName}</div>
    </div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with Italy-Piedmont, Download Free Trial<br>
        correctly assigned to CMR team
      </div>
    </div>
  </div>
</body>
