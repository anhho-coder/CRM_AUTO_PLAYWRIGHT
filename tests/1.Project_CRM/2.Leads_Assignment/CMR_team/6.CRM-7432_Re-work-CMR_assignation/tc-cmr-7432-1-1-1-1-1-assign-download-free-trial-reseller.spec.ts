...
/**
 * Lead Assignment Test - CMR Team - Download Free Trial with Reseller Contact
 * Test Case ID: TC.CMR-7432_1.1.1.1.1
 ...
 *    8.1. At "Target Level" dropdown list, select "Bronze"
 *    8.2. At "Comment" textbox, enter: "Bronze level + Test case ID + current date time millisecond"
...
test.describe('TC.CMR-7432_1.1.1.1.1 - CMR Team Assignment for Download Free Trial with Reseller', () => {
  ...
  test('TC.CMR-7432_1.1.1.1.1: Verify the lead is assigned to CMR team if Lead form=Download Free Trial, Nakivo Customer=FALSE, Reseller contact selected', async ({ page }, testInfo) => {
    ...
    // Steps 3-8: Create contact with Bronze partner level
    await test.step('Steps 3-8: Create contact with Bronze partner level', async () => {
      ...
      contactName = `TEST-Contact_TC.CMR-7432_1.1.1.1.1_${timestamp}`;
      emailAddress = `Test-Reseller@Reseller-company${timestamp}.com`;
      
      const currentDate = new Date().toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' });
      const comment = `Bronze level_TC.CMR-7432_1.1.1.1.1_${timestamp}`;
    ...
    await test.step('Step 14: Fill Lead information', async () => {
      ...
      const timestamp = CommonUtils.generateTimestamp();
      leadName = `TEST_TC.CMR-7432_1.1.1.1.1_${timestamp}`;
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.CMR-7432_1.1.1.1.1', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
