...
/**
 * Lead Assignment Test - CMR Team - Download Free Trial with Reseller Contact (Basic Partner Level with Activation Date)
 * Test Case ID: TC.CMR-7432_1.1.1.1.4
 *
 * Automation-Type: refactored
 * Automation-Date: 2026-08-04
 *
 * Summary: Verify the lead is assigned to CMR team if Lead form = Download Free Trial ; Nakivo Customer is not set; New Partner is set
 * 
 * Command to run:
 * npx playwright test --grep "TC\.CMR-7432_1\.1\.1\.1\.4 -" --project=chromium
 * 
 * Pre-condition:
...
test.describe('TC.CMR-7432_1.1.1.1.4 - CMR Team Assignment for Download Free Trial with Reseller (Basic + New Partner)', () => {
  ...
  test('TC.CMR-7432_1.1.1.1.4: Verify the lead is assigned to CMR team if Lead form=Download Free Trial, Nakivo Customer=FALSE, New Partner=TRUE', async ({ page }, testInfo) => {
    ...
    await test.step('Steps 3-8: Create contact with Basic partner level and Activation Date', async () => {
      ...
      contactName = `TEST-Contact_TC.CMR-7432_1.1.1.1.4_${timestamp}`;
      emailAddress = `Test-Reseller@Reseller-company${timestamp}.com`;
      
      const currentDate = new Date().toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' });
      const comment = `Basic level_TC.CMR-7432_1.1.1.1.4_${timestamp}`;
    ...
    await test.step('Step 14: Fill Lead information', async () => {
      ...
      const timestamp = CommonUtils.generateTimestamp();
      leadName = `TEST_TC.CMR-7432_1.1.1.1.4_${timestamp}`;
    ...
    // Step 17: Edit lead and set Nakivo Customer and New Partner checkboxes
    await test.step('Step 17: Edit lead and set checkboxes (Nakivo Customer=FALSE, New Partner=TRUE)', async () => {
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.CMR-7432_1.1.1.1.4', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }
