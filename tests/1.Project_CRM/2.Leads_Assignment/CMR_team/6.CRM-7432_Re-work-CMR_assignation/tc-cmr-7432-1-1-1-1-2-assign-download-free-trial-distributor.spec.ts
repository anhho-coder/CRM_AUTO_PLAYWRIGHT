...
/**
 * Lead Assignment Test - CMR Team - Download Free Trial with Distributor Contact
 * Test Case ID: TC.CMR-7432_1.1.1.1.2
 ...
 * 
 * Command to run:
 * npx playwright test --grep "TC\.CMR-7432_1\.1\.1\.1\.2 -" --project=chromium
 * 
 * Pre-condition:
 ...
 *    7.1. Press "CHANGE LEVEL" button and wait
 * 8. On "Change partner level" window:
 *    8.1. At "Target Level" dropdown list, select "Distributor"
 *    8.2. At "Comment" textbox, enter a value with the template: "Distributor level + Test case ID + current date time millisecond"
 *    8.3. Press "SUBMIT" button and wait
 ...
 *     - "Distributor" dropdown list is set as "Reseller#1" created above
...
test.describe('TC.CMR-7432_1.1.1.1.2 - CMR Team Assignment for Download Free Trial with Distributor', () => {
  ...
  test('TC.CMR-7432_1.1.1.1.2: Verify the lead is assigned to CMR team if Lead form=Download Free Trial, Nakivo Customer=FALSE, Distributor contact selected', async ({ page }, testInfo) => {
    ...
    // Steps 3-8: Create contact with Distributor partner level
    await test.step('Steps 3-8: Create contact with Distributor partner level', async () => {
      console.log('Steps 3-8: Creating contact with partner level assignment');
      
      // Generate unique contact name and email
      const timestamp = CommonUtils.generateTimestamp();
      contactName = `TEST-Contact_TC.CMR-7432_1.1.1.1.2_${timestamp}`;
      emailAddress = `Test-Reseller@Reseller-company${timestamp}.com`;
      
      const currentDate = new Date().toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' });
      const comment = `Distributor level_TC.CMR-7432_1.1.1.1.2_${timestamp}`;
      
      // Create contact with partner level
      ...
      console.log(`  - Target Level: Distributor`);
      console.log(`  - Comment: ${comment}`);
      console.log(`✓ Partner level set to Distributor`);
      
      // Capture screenshot after setting partner level
      const screenshotPartnerLevel = await page.screenshot({ fullPage: true });
      await testInfo.attach(`Contact ${contactId} - Partner Level Distributor`, {
    ...
    await test.step('Step 14: Fill Lead information', async () => {
      ...
      const timestamp = CommonUtils.generateTimestamp();
      leadName = `TEST_TC.CMR-7432_1.1.1.1.2_${timestamp}`;
      ...
      // Set Distributor to the created contact (Reseller#1)
      await leadPage.fillDistributor(contactName);
      console.log(`  - Distributor: ${contactName}`);
    ...
    // Step 17: Edit lead and set Nakivo Customer and Activated Partner checkboxes
    await test.step('Step 17: Edit lead and set checkboxes (Nakivo Customer=FALSE, Activated Partner=TRUE)', async () => {
    ...
    // Step 18: Wait for Sales Team assignment
    ...
    if (!salesTeamAssigned || !salespersonAssigned) {
      test.skip(true, assignmentDeferSkipReason('TC.CMR-7432_1.1.1.1.2', !salesTeamAssigned ? 'Sales Team' : 'Salesperson'));
    }

    // Step 19: Verify Sales Team and Salesperson assignment
