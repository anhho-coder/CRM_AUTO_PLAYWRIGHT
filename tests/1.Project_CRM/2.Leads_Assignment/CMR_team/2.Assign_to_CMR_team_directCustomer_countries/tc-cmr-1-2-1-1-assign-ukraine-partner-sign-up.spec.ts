 ...
 * 
 * Command to run:
 * npx playwright test tests/Leads_Assignment/CMR_team/2.Assign_to_CMR_team_directCustomer_countries/tc-cmr-1-2-1-1-assign-ukraine-partner-sign-up.spec.ts --project=chromium
 * 
 * Pre-condition:
 ...
 * 6. Press "SAVE" button
 * 7. (Notice: Because the "Nakivo Customer" and "Partner" checkboxes only accept data after Save the Lead)
...
test.describe('TC.CMR_1.2.1.1 - CMR Team Assignment for Ukraine with Partner (Direct Customer Countries)', () => {
  ...
  test('TC.CMR_1.2.1.1: Verify the lead is assigned to CMR team if Country=Ukraine, Partner=TRUE, Nakivo Customer=FALSE, Lead Form=License', async ({ page }, testInfo) => {
    ...
    // Step 5: Fill CRM Developer tab (without Nakivo Customer and Partner yet)
    await test.step('Step 5: Fill CRM Developer tab', async () => {
      ...
      console.log('✓ CRM Developer tab filled (without Nakivo Customer and Partner yet)');
    });
    ...
    // Step 7.2: Set Nakivo Customer=FALSE and Partner=TRUE
    await test.step('Step 7.2: Set Nakivo Customer and Partner checkboxes', async () => {
      console.log('Step 7.2: Setting Nakivo Customer and Partner checkboxes');
      ...
      console.log('✓ Nakivo Customer = FALSE, Partner = TRUE');
    });
    ...
    await test.step('Step 7.3: Save the lead again (second save)', async () => {
      ...
      console.log('✓ Lead saved successfully with Nakivo Customer=FALSE and Partner=TRUE');
...
await test.step('Step 8: Wait for Sales Team and Salesperson auto-assignment (up to 1.5 minutes)', async () => {
  ...
  console.log(`✓ Auto-assignment check completed in ${result.totalWaitTime} seconds`);
    ...
    await test.step('Verification: Confirm Sales Team is CMR and Salesperson is assigned', async () => {
      ...
      await testInfo.attach(`Lead ${leadId} - CMR Team Assignment (Ukraine with Partner)`, {
        body: screenshot,
        contentType: 'image/png'
      });
      ...
      console.log('   Lead with Ukraine, Partner=TRUE, and');
...
<body>
  <div class="container">
    ...
    <div class="info-section">
      <div class="info-row"><span class="label">Test Case:</span> TC.CMR_1.2.1.1</div>
    ...
    <div class="summary">
      ...
      <div class="summary-text">
        ...
        Lead with Ukraine, Partner=TRUE, and<br>
