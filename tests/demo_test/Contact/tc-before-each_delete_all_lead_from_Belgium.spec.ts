 ...
 *    - Lead name textbox      = TEST Lead 1 CRM-457_2.1.9
 ...
 *         (Handle the unexpected error message by pressing "OK" on
 ...
 *    1.3. Press "OK" button on "Are you sure you want to delete this record?" window and wait
 */
...
test.describe('CRM-457_2.1.10 - Verify qualified Leads with Stage=Interested transfered to another Salesperson', () => {
  ...
  test.beforeEach(async ({ page, context }) => {
    ...
    await test.step('beforeEach Step 3: Filter by Salesperson = Thomas Semerich', async () => {
      ...
      // Click "Add Custom Filter" button
    ...
    await test.step('beforeEach Step 7: Check if list has records', async () => {
      ...
      const isEmpty = await opportunityPage.isRecordListEmpty();
      if (isEmpty) {
    ...
    // beforeEach Step 12: Close extra pages but keep main test page open
    await test.step('beforeEach Step 12: Close extra pages (keep main test page open)', async () => {
      console.log('beforeEach Step 12: Closing extra pages (keeping main test page open)');
      const allPages = page.context().pages();
      for (const p of allPages) {
        if (!p.isClosed() && p !== page) {
          await p.close();
        }
      }
      console.log('✓ beforeEach Step 12: Extra pages closed - main test page kept open');
    });
  });
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    if (url_Lead1) {
      ...
      await test.step('IV. Tear down - Step 1.1: Launch URL_Lead#1 (handle modification dialog if any)', async () => {
        console.log(`Step IV.1.1: Opening new tab and launching URL_Lead#1: ${url_Lead1}`);
        newTab = await page.context().newPage();
        // Handle "The record has been modified, your changes will be discarded. Do you want to proceed?" dialog
        newTab.on('dialog', async (dialog) => {
          console.log(`  ℹ️ Dialog appeared: "${dialog.message()}" - clicking OK`);
          await dialog.accept();
        });
  ...
  test('CRM-457_2.1.10: Verify the qualified Leads with Stage=Purchase Approval have been transfered to another Salesperson correctly', async ({ page }, testInfo) => {
    ...
    await test.step('Step II.5: Set filter field values on Re-assignation page', async () => {
      ...
      console.log('  5.7: Stage = "Interested"');
      await reAssignationPage.selectStage('Interested');

      console.log('✓ All filter fields set');
      await page.waitForTimeout(CommonUtils.waitTimes.standard);
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'Step II.5.7 - Stage set to Interested');
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n✅ TEST PASSED: CRM-457_2.1.9 verification completed successfully');
      console.log(`   Lead#1 URL   : ${url_Lead1}`);
      console.log('   III.1: Total = "0/1" - qualified Lead with Stage=Interested correctly counted for re-assignment');
