...
/**
 * Demo Test - Create Deal Element with 2 Product Lines
 * Test Case ID: CMR-Create-DealElement-2-product-lines
 *
 * Summary: Verify the Opp has Reseller and Distributor
 *
 * Command to run:
 * npx playwright test --grep "CMR-Create-DealElement-2-product-lines:" --project=chromium
 *
 ...
 *
 * III. Verification:
 * 1. There are 2 Products added
 *
 */

test.describe('CMR-Create-DealElement-2-product-lines - Verify the Opp has Reseller and Distributor', () => {
  ...
  test('CMR-Create-DealElement-2-product-lines: Verify the Opp has Reseller and Distributor', async ({ page }, testInfo) => {
    ...
    const tcId = 'CMR-Create-DealElement-2-product-lines';
    ...
    await test.step('Step II.3a: Add product [A2144B], Ordered Qty = 1, UoM = Socket', async () => {
      console.log('Step II.3a: Adding product [A2144B], Qty=1, UoM=Socket');
    ...
    await test.step('III.1: Verify there are 2 products in Order Lines', async () => {
      console.log(`\n=== III. VERIFICATION ===`);
    ...
    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CMR-Create-DealElement-2-product-lines completed successfully');
      console.log(`   Name_Opp#1     : "${name_Opp1}"`);
      console.log(`   URL_Opp#1      : ${url_Opp1}`);
      console.log(`   Name_EndUser#1 : "${name_EndUser1}"`);
      console.log('   Deal Element#1 created with [A2144B] and [A2145B]');
      console.log('   III.1: 2 products in Order Lines \u2014 confirmed');
      console.log('==================================================\n');
    });
  });
});
