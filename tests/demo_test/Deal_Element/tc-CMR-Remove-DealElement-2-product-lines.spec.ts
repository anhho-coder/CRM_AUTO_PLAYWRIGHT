 ...
 *
 * Command to run:
 * npx playwright test "tests/demo_test/Deal_Element/tc-CMR-Remove-DealElement-2-product-lines.spec.ts" --project=chromium
 *
...
test.describe('CMR-Edit-DealElement-2-product-lines - Verify the Opp has Reseller and Distributor', () => {
  ...
  test('CMR-Edit-DealElement-2-product-lines: Verify the Opp has Reseller and Distributor', async ({ page }, testInfo) => {
    ...
    await test.step('Step II.3-4: Select [A2144B] row and press Trash Bin icon to remove it', async () => {
      console.log('Step II.3-4: Removing product line [A2144B]');
      await dealElementPage.removeProductLineByName('[A2144B]');
      await CommonUtils.captureAndAttachScreenshot(page, testInfo, 'II.3-4 - Product line [A2144B] removed');
    });
    ...
    await test.step('Final Summary', async () => {
      console.log('\n\u2705 TEST PASSED: CMR-Remove-DealElement-2-product-lines completed successfully');
      console.log(`   Name_Opp#1     : "${name_Opp1}"`);
      console.log(`   URL_Opp#1      : ${url_Opp1}`);
      console.log(`   Name_EndUser#1 : "${name_EndUser1}"`);
      console.log('   II.3-4: Product line [A2144B] removed via Trash Bin icon');
      console.log('   III.1 : Product line [A2144B] no longer in Order Lines \u2014 confirmed');
      console.log('==================================================\n');
    });
  });
});
