/**
 * Page Object Model - Index
 * Export all page objects for easy import
 */

export { BasePage } from './BasePage';
export { LoginPage } from './LoginPage';
export { HomePage } from './HomePage';
export { LeadPage } from './LeadPage';
export { OpportunityPage } from './OpportunityPage';
export { ContactPage } from './ContactPage';
export { DealElementPage } from './DealElementPage';
export { QuotationPage } from './QuotationPage';
export { InvoicePage } from './InvoicePage';
export { LicensePage } from './LicensePage';
export { InvestmentPage } from './InvestmentPage';
export { ReAssignationPage } from './ReAssignationPage';
export { PromotionPage } from './PromotionPage';
export { ResellerPortalPage } from './ResellerPortalPage';
export { PaymentPage } from './PaymentPage';
export { CurrencyPage } from './CurrencyPage';
export type { CurrencyRateRow } from './CurrencyPage';
export { CurrencySettingsPage } from './CurrencySettingsPage';
export type { AutomaticRatesSettings } from './CurrencySettingsPage';
export { ScheduledActionPage } from './ScheduledActionPage';
export type { ScheduledActionState } from './ScheduledActionPage';
export { SubscriptionPage } from './SubscriptionPage';
export type { GenerateInvoiceResult } from './SubscriptionPage';
export { SubscriptionTemplatePage } from './SubscriptionTemplatePage';
export type { TemplateSettings } from './SubscriptionTemplatePage';
export { SESupportPage } from './SESupportPage';
export { HelpdeskPage } from './HelpdeskPage';
export { SEMeetingPage } from './SEMeetingPage';
