        ...
        stage('Run Playwright test (headless on Chrome)') {
            steps {
                script {
                    ...
                    // 240 even on a plain Build. CRM_O12_BusinessProcess runs all 74 specs of the
                    // 1.Business-Process (Salesperson) sub-tree serially (~6-9h incl. slow async
                    ...
                    if (spec) {
                        ...
                        runPlaywright = { bat 'npx playwright test "tests/1.Project_CRM/1.SalesReport_Performance/1.Lead/1.1.Performance/tc-performance-1-1-1-create-lead.spec.ts" --project=chrome-headless' }
                    }
